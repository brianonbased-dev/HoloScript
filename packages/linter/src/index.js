        ...
        check(context) {
            ...
            const checkTraits = (nodes) => {
                for (const node of nodes) {
                    ...
                    if (node.children)
                        checkTraits(node.children);
                }
            };
            if (context.ast.children)
                checkTraits(context.ast.children);
            return diagnostics;
        },
    },
    // Deprecated traits
                        ...
                        for (const dir of node.directives) {
                            if (dir.type === 'trait' && dir.name in deprecated) {
                                diagnostics.push({
                                    ...
                                    message: `Trait "@${dir.name}" is deprecated. ${deprecated[dir.name]}`,
                                    severity: 'warning',
                                    line: dir.loc?.start.line || 1,
                                    column: dir.loc?.start.column || 1,
                                });
 ...
 */
export function lint(source, filePath = 'input.holo') {
...
// Rule exports
export { noDeadCodeRule, createNoDeadCodeRule } from './rules/no-dead-code';
export { deprecationWarningRule, createDeprecationWarningRule, } from './rules/deprecation-warning';
// Default export
