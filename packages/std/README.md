...
This proves the declared i32 scalar subset, affine aggregate-valued Vec3 subset, recursively nested immutable-POD AABB subset, immutable fixed-size `List3<i32>` projection, local owned-buffer allocation/whole-owner move/explicit drop/automatic return cleanup, call-scoped shared and mutable aggregate parameters with scalar field load/store, finite f32 scalar subset with operation-by-operation binary32 rounding, and finite f64 scalar subset. Owned-buffer parameters, returns, aggregate fields, borrowed buffer element access, borrowed aggregate returns, stored reference locals, escaping leases, non-finite floating-point edge semantics, quaternions, noise, dynamic collection indexing, variable-length allocation, general `List`/`Map`/`Set` parity, OS-level air-gap behavior, and a general stable systems ABI remain outside the proof.
