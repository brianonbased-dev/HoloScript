...
`@holoscript/std/uaal-abi` is the packaged UAAL host adapter for the numeric binary contracts, `hs.aggregate.value.v1`, `hs.aggregate.value.v2`, and `hs.buffer.owned.v1`. Aggregate construction produces frozen record envelopes. Flat projection verifies the semantic layout, field index, and scalar type; recursive projection additionally verifies every nested envelope boundary before pushing a scalar leaf. Owned buffers use explicit UAAL allocation, move, load, store, drop, and length opcodes. A move rotates a frozen owner token without cloning its backing cell, and malformed bytecode fails closed on stale owners, double drop, element-type mismatch, and bounds violations.
...
This proves the declared i32 scalar subset, affine aggregate-valued Vec3 subset, recursively nested immutable-POD AABB subset, local owned-buffer allocation/whole-owner move/explicit drop/automatic return cleanup, finite f32 scalar subset with operation-by-operation binary32 rounding, and finite f64 scalar subset. Owned-buffer parameters, returns, aggregate fields, borrowed element access, mutable or borrowed aggregate transfer, non-finite floating-point edge semantics, quaternions, noise, collections, OS-level air-gap behavior, and a general stable systems ABI remain outside the proof.
