...
export const IoTTraits: TraitDefinition[] = [
  {
    name: '@sensor_stream',
    type: 'data_binding',
    description: 'Binds a virtual digital twin property directly to a real-world IoT sensor telemetry stream.',
    parameters: [
      {
        name: 'sensor_id',
        type: 'string',
      ...
      },
      {
        ...
        type: 'string',
        required: true,
        validation: (val: any) => ['temperature', 'pressure', 'heart_rate', 'spo2', 'motion', 'custom'].includes(val),
  ...
  },
  {
    name: '@actuator',
    type: 'behavior',
    description: 'Defines this HoloScript node as a control schema that can send physical actions back to real-world mechanisms.',
    parameters: [
      {
        name: 'device_id',
        type: 'string',
      ...
      },
      {
        ...
        type: 'array',
        required: false,
        description: 'An array characterizing min/max values or discrete states to prevent hardware damage.'
      }
    ],
    validation: (node: any) => {
  ...
  },
  {
    name: '@hardware_fault',
    type: 'event_hook',
    description: 'Registers an alert topology for when a linked physical sensor goes offline or returns anomalous readings.',
    parameters: [
      {
