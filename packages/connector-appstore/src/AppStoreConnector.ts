...
export class AppStoreConnector extends ServiceConnector {
  ...
  private async executeAppleTool(name: string, args: Record<string, unknown>): Promise<unknown> {
    if (!this.appleClient) {
      throw new Error(
        'Apple App Store Connect is not configured. Set APPLE_KEY_ID, APPLE_ISSUER_ID, and APPLE_PRIVATE_KEY environment variables.'
      );
    }
    ...
    switch (name) {
      ...
      case 'apple_builds_list': {
        const app = await this.appleClient.getApp(args.bundleId as string);
        const builds = await this.appleClient.listBuilds(
          app.appId,
      ...
      case 'apple_version_localization_update': {
        ...
        await this.appleClient.updateVersionLocalization(args.versionId as string, localization);
  ...
  private async executeGoogleTool(name: string, args: Record<string, unknown>): Promise<unknown> {
    if (!this.googleClient) {
      throw new Error(
        'Google Play Developer is not configured. Set GOOGLE_SERVICE_ACCOUNT environment variable.'
      );
    }
    ...
    switch (name) {
      ...
      case 'google_release_promote': {
        ...
        return {
          success: true,
          message: `Release promoted from ${args.fromTrack} to ${args.toTrack}`,
        };
      }
   ...
   */
  private async executeCrossPlatformTool(
    name: string,
