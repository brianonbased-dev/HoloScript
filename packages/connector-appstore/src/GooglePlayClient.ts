...
import { GoogleCredentials, GoogleAppMetadata, GooglePlayTrack, BuildArtifact, UploadProgress } from './types.js';
...
export class GooglePlayClient {
    ...
    async uploadBuild(
        ...
        try {
            ...
            if (onProgress) {
                onProgress({ bytesUploaded: totalBytes, totalBytes, percentage: 100, status: 'Assigning to track...' });
            }

            // Step 3: Assign to track
            await this.assignToTrack(appMetadata.packageName, editId, track, [versionCode], artifact.releaseNotes);
            ...
            if (onProgress) {
                onProgress({ bytesUploaded: totalBytes, totalBytes, percentage: 100, status: 'Upload complete' });
            }
