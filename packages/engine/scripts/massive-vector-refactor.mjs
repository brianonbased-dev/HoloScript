...
function processFile(filePath) {
  ...
  // Fix return literals
  content = content.replace(
    /return\s*\{\s*x:\s*([^,]+),\s*y:\s*([^,]+),\s*z:\s*([^\}]+)\s*\}/g,
    (match, x, y, z) => {
      changed = true;
  ...
  props.forEach((p) => {
    ...
    if (regX.test(content)) {
      content = content.replace(regX, `${p}[0]`);
      changed = true;
    }
    if (regY.test(content)) {
      content = content.replace(regY, `${p}[1]`);
      changed = true;
    }
    if (regZ.test(content)) {
      content = content.replace(regZ, `${p}[2]`);
      changed = true;
    }
  });
