...
export class RigidBody {
  ...
  /**
   * Apply an internal solver position correction without resetting the sleep
   * timer. External transform writes must continue to use the public setter so
   * teleports wake a body; contact stabilization must not wake it every frame.
   */
  public setSolverPosition(value: IVector3): void {
    this._position = [...value];
  }
  ...
  /**
   * Apply an impulse generated by the contact/constraint solver.
   *
   * Resting contacts must not reset the sleep timer on every fixed step.
   * A meaningful impact still wakes a sleeping body before applying velocity.
   */
  public applySolverImpulse(impulse: AnyVec3): void {
    if (this.type !== 'dynamic') return;

    const _im = toVec3(impulse);
    const deltaVelocity: IVector3 = [
      _im[0] * this._inverseMass,
      _im[1] * this._inverseMass,
      _im[2] * this._inverseMass,
    ];
    if (this._isSleeping) {
      if (this.vectorLength(deltaVelocity) <= PHYSICS_DEFAULTS.sleepThreshold) return;
      this.wakeUp();
    }

    this._linearVelocity[0] += deltaVelocity[0];
    this._linearVelocity[1] += deltaVelocity[1];
    this._linearVelocity[2] += deltaVelocity[2];
    this._linearVelocity = this.clampVelocity(this._linearVelocity, PHYSICS_DEFAULTS.maxVelocity);
  }
  ...
  /**
   * Apply an angular impulse generated by the internal solver without
   * resetting the sleep timer for an already-settling body.
   */
  public applySolverTorqueImpulse(impulse: AnyVec3): void {
    if (this.type !== 'dynamic') return;

    const _ai = toVec3(impulse);
    const deltaAngularVelocity: IVector3 = [
      _ai[0] * this._inverseInertia[0],
      _ai[1] * this._inverseInertia[1],
      _ai[2] * this._inverseInertia[2],
    ];
    if (this._isSleeping) {
      if (this.vectorLength(deltaAngularVelocity) <= PHYSICS_DEFAULTS.sleepThreshold) return;
      this.wakeUp();
    }

    this._angularVelocity[0] += deltaAngularVelocity[0];
    this._angularVelocity[1] += deltaAngularVelocity[1];
    this._angularVelocity[2] += deltaAngularVelocity[2];
    this._angularVelocity = this.clampVelocity(
      this._angularVelocity,
      PHYSICS_DEFAULTS.maxAngularVelocity
    );
