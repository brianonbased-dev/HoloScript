...
export class RaycastSystem {
  ...
  private rayAABB(origin: Vector3, dir: Vector3, aabb: AABB, entityId: string): RayHit | null {
  ...
  private rayPlane(origin: Vector3, dir: Vector3, plane: Plane, entityId: string): RayHit | null {
    const normalV = this.toArr(plane.normal);
    const denom = normalV[0] * dir[0] + normalV[1] * dir[1] + normalV[2] * dir[2];
    if (Math.abs(denom) < 1e-10) return null;

    const t =
      -(normalV[0] * origin[0] + normalV[1] * origin[1] + normalV[2] * origin[2] + plane.distance) /
      denom;
    if (t < 0) return null;
