...
export interface Gaussian3D {
  N: number;
  x: Float64Array; y: Float64Array; z: Float64Array; // mean3d
  sx: Float64Array; sy: Float64Array; sz: Float64Array; // scale
  qr: Float64Array; qx: Float64Array; qy: Float64Array; qz: Float64Array; // quat (r,x,y,z)
  op: Float64Array; r: Float64Array; gr: Float64Array; bl: Float64Array; // opacity + color
}
...
export interface Gaussian3DGrad {
  ...
  op: Float64Array; r: Float64Array; gr: Float64Array; bl: Float64Array;
}
...
export interface SplatCamera {
  ...
  fx: number; fy: number;
  cx?: number; cy?: number; // principal point (default image centre)
}

/** Cached forward intermediates needed by backward3D. */
export interface ProjectIntermediates {
  camx: Float64Array; camy: Float64Array; camz: Float64Array;
  tx: Float64Array; ty: Float64Array;
  J00: Float64Array; J02: Float64Array; J11: Float64Array; J12: Float64Array;
  T: number[][]; M: number[][]; R: number[][];
  sx: Float64Array; sy: Float64Array; sz: Float64Array;
  cov00: Float64Array; cov01: Float64Array; cov11: Float64Array; det: Float64Array;
  Vrow: readonly number[]; fx: number; fy: number;
}
...
function matmul(A: readonly number[], ra: number, ca: number, B: readonly number[], cb: number): number[] {
...
function symMMt(M: readonly number[]): number[] {
  ...
  for (let a = 0; a < 3; a++) for (let b = 0; b < 3; b++) {
    let s = 0; for (let k = 0; k < 3; k++) s += M[a * 3 + k] * M[b * 3 + k];
    S[a * 3 + b] = s;
  }
  return S;
}
...
export function forward3D(G3: Gaussian3D, cam: SplatCamera, W: number, H: number): Forward3DResult {
  ...
  const g2: Gaussian2D = {
    ...
    r: G3.r, gr: G3.gr, bl: G3.bl, op: G3.op, // color + opacity pass straight through
  };
  const I: ProjectIntermediates = {
    camx: new Float64Array(N), camy: new Float64Array(N), camz: new Float64Array(N),
    tx: new Float64Array(N), ty: new Float64Array(N),
    J00: new Float64Array(N), J02: new Float64Array(N), J11: new Float64Array(N), J12: new Float64Array(N),
    T: new Array<number[]>(N), M: new Array<number[]>(N), R: new Array<number[]>(N),
    sx: G3.sx, sy: G3.sy, sz: G3.sz,
    cov00: new Float64Array(N), cov01: new Float64Array(N), cov11: new Float64Array(N), det: new Float64Array(N),
    Vrow, fx, fy,
  };
  const limX = 1.3 * (0.5 * W / fx), limY = 1.3 * (0.5 * H / fy);
  for (let i = 0; i < N; i++) {
    const px = G3.x[i], py = G3.y[i], pz = G3.z[i];
    ...
    I.camx[i] = camx; I.camy[i] = camy; I.camz[i] = camz;
    const tz = camz;
    const txn = Math.min(Math.max(camx / tz, -limX), limX);
    const tyn = Math.min(Math.max(camy / tz, -limY), limY);
    const tx = txn * tz, ty = tyn * tz;
    I.tx[i] = tx; I.ty[i] = ty;
    const J00 = fx / tz, J02 = -fx * tx / (tz * tz), J11 = fy / tz, J12 = -fy * ty / (tz * tz);
    I.J00[i] = J00; I.J02[i] = J02; I.J11[i] = J11; I.J12[i] = J12;

    const R = quatToR(G3.qr[i], G3.qx[i], G3.qy[i], G3.qz[i]); I.R[i] = R;
    const s0 = G3.sx[i], s1 = G3.sy[i], s2 = G3.sz[i];
    ...
    I.T[i] = T;
    const T00 = T[0], T01 = T[1], T02 = T[2], T11 = T[4], T12 = T[5], T22 = T[8];
    ...
    cov00 += 0.3; cov11 += 0.3; // 0.3px low-pass dilation (classic mode, no opacity comp)
    I.cov00[i] = cov00; I.cov01[i] = cov01; I.cov11[i] = cov11;
    const det = cov00 * cov11 - cov01 * cov01; I.det[i] = det;

    g2.a[i] = cov11 / det; g2.b[i] = -cov01 / det; g2.c[i] = cov00 / det; // conic = inv(cov2d)
...
export function backward3D(
  G3: Gaussian3D, _cam: SplatCamera, _W: number, _H: number, I: ProjectIntermediates, dG2: Gaussian2DGrad,
): Gaussian3DGrad {
  const N = G3.N;
  const { Vrow, fx, fy } = I;
  const out: Gaussian3DGrad = {
    ...
    op: dG2.op, r: dG2.r, gr: dG2.gr, bl: dG2.bl, // pass-through
  };
  const VT = transpose(Vrow, 3, 3);
  for (let i = 0; i < N; i++) {
    const da = dG2.a[i], db = dG2.b[i], dc = dG2.c[i];
    const dposx = dG2.posx[i], dposy = dG2.posy[i];

    // --- conic = inv(cov2d) -> cov2d (honest unique).  A=cov00 B=cov01 D=cov11, det=AD-B^2 ---
    const A = I.cov00[i], B = I.cov01[i], D = I.cov11[i], det = I.det[i], d2 = det * det;
    const dcov00 = da * (-D * D / d2) + db * (B * D / d2) + dc * (-B * B / d2);
    const dcov01 = da * (2 * B * D / d2) + db * (-1 / det - 2 * B * B / d2) + dc * (2 * A * B / d2);
    const dcov11 = da * (-B * B / d2) + db * (A * B / d2) + dc * (-A * A / d2);

    // --- cov2d = J T Jᵀ (+dilation) -> G_T, G_J via matrix identities (convention F) ---
    const J00 = I.J00[i], J02 = I.J02[i], J11 = I.J11[i], J12 = I.J12[i];
    const Tm = I.T[i];
    ...
    const dJ00 = GJ[0], dJ02 = GJ[2], dJ11 = GJ[4], dJ12 = GJ[5];
    ...
    const R = I.R[i], svec = [I.sx[i], I.sy[i], I.sz[i]];
    const dR = new Array<number>(9);
    let ds0 = 0, ds1 = 0, ds2 = 0;
    for (let a = 0; a < 3; a++) {
    ...
    out.sx[i] = ds0; out.sy[i] = ds1; out.sz[i] = ds2;

    // --- R(quat) -> dquat.   r=qr x=qx y=qy z=qz ---
    const qr = G3.qr[i], qx = G3.qx[i], qy = G3.qy[i], qz = G3.qz[i];
    let dqr = 0, dqx = 0, dqy = 0, dqz = 0;
    dqy += dR[0] * (-4 * qy); dqz += dR[0] * (-4 * qz); // R00
    dqx += dR[1] * (2 * qy); dqy += dR[1] * (2 * qx); dqr += dR[1] * (-2 * qz); dqz += dR[1] * (-2 * qr); // R01
    dqx += dR[2] * (2 * qz); dqz += dR[2] * (2 * qx); dqr += dR[2] * (2 * qy); dqy += dR[2] * (2 * qr); // R02
    dqx += dR[3] * (2 * qy); dqy += dR[3] * (2 * qx); dqr += dR[3] * (2 * qz); dqz += dR[3] * (2 * qr); // R10
    dqx += dR[4] * (-4 * qx); dqz += dR[4] * (-4 * qz); // R11
    dqy += dR[5] * (2 * qz); dqz += dR[5] * (2 * qy); dqr += dR[5] * (-2 * qx); dqx += dR[5] * (-2 * qr); // R12
    dqx += dR[6] * (2 * qz); dqz += dR[6] * (2 * qx); dqr += dR[6] * (-2 * qy); dqy += dR[6] * (-2 * qr); // R20
    dqy += dR[7] * (2 * qz); dqz += dR[7] * (2 * qy); dqr += dR[7] * (2 * qx); dqx += dR[7] * (2 * qr); // R21
    dqx += dR[8] * (-4 * qx); dqy += dR[8] * (-4 * qy); // R22
    out.qr[i] = dqr; out.qx[i] = dqx; out.qy[i] = dqy; out.qz[i] = dqz;

    // --- camPos gradient (J coupling + mean2d), then dmean3d = Vᵀ dcam ---
    const tz = I.camz[i], tx = I.tx[i], ty = I.ty[i], tz2 = tz * tz, tz3 = tz2 * tz;
    let dcx = 0, dcy = 0, dcz = 0;
    dcx += dJ02 * (-fx / tz2);
    dcy += dJ12 * (-fy / tz2);
    dcz += dJ00 * (-fx / tz2) + dJ11 * (-fy / tz2) + dJ02 * (2 * fx * tx / tz3) + dJ12 * (2 * fy * ty / tz3);
    dcx += dposx * J00;
    dcy += dposy * J11;
