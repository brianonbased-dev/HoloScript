...
describe('densifyAndPrune — clone / split / prune', () => {
  it('prunes transparent, clones small high-grad, splits large high-grad; origin map is correct', () => {
    const g: Gaussian3D = {
      ...
      sx: Float64Array.from([0.1, 0.05, 0.5]),
      sy: Float64Array.from([0.1, 0.05, 0.5]),
      sz: Float64Array.from([0.1, 0.05, 0.5]),
      qr: Float64Array.from([1, 1, 1]),
      qx: Float64Array.from([0, 0, 0]),
      qy: Float64Array.from([0, 0, 0]),
      qz: Float64Array.from([0, 0, 0]),
      op: Float64Array.from([0.005, 0.5, 0.5]), // g0 transparent → pruned
      r: Float64Array.from([0.1, 0.2, 0.3]),
      gr: Float64Array.from([0.1, 0.2, 0.3]),
      bl: Float64Array.from([0.1, 0.2, 0.3]),
    };
  ...
  it('does nothing when no gaussian is transparent or over-threshold (fixed-cardinality)', () => {
    ...
    const { gaussians, origin } = densifyAndPrune(
      g,
      { avgGrad2d: new Float64Array(8) /* all 0 */ },
