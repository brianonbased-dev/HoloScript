...
import {
  cameraFromArcorePose, backprojectDepth, captureToGaussianInit, captureToViews,
  type CaptureIntrinsics, type CaptureFrame, type CaptureManifest,
} from '../GaussianCaptureLoader';

const INTR: CaptureIntrinsics = { fx: 1247.77, fy: 1244.69, cx: 968.61, cy: 547.37, imageWidth: 1920, imageHeight: 1080 };
const DW = 160, DH = 90;
...
function depthFrameWith(pose: number[], du: number, dv: number, depthMm: number): CaptureFrame {
  ...
  return { index: 0, cameraTransformColumnMajor4x4: pose, depthWidth: DW, depthHeight: DH, depthMillimeters: depth };
}
...
describe('GaussianCaptureLoader — ARCore→CV convention (back-project ∘ project round-trip)', () => {
  for (const [name, Rcw, t] of [
    ...
    it(`reprojects a depth pixel to itself (${name})`, () => {
      const du = 40, dv = 30, depthMm = 2500;
      ...
      const uImg = (du + 0.5) * (INTR.imageWidth / DW), vImg = (dv + 0.5) * (INTR.imageHeight / DH);
...
describe('GaussianCaptureLoader — init + views assembly', () => {
  const manifest: CaptureManifest = {
    ...
    frames: [0, 1].map((i) => {
      ...
      return { index: i, cameraTransformColumnMajor4x4: makePose(rotY(i * 0.2), [i * 0.1, 0, 0]), depthWidth: DW, depthHeight: DH, depthMillimeters: depth };
    }),
  };
  const decode = (_i: number, w: number, h: number) => Float64Array.from({ length: w * h * 3 }, (_, k) => (k % 3) / 3);
  ...
  it('captureToGaussianInit produces a depth-fused Gaussian3D with finite positions and valid params', () => {
    ...
    for (let i = 0; i < g.N; i++) { expect(Number.isFinite(g.x[i])).toBe(true); expect(g.sx[i]).toBeGreaterThan(0); }
