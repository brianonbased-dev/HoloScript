...
const CA = Math.cos(0.2), SA = Math.sin(0.2);
...
describe('GaussianTrainer3D projection backward (gradient check)', () => {
  it('analytic gradients match central differences for mean3d / scale / quat', () => {
    ...
    const lossOf = (gg: Gaussian3D): number => {
      ...
      let L = 0; for (let k = 0; k < img.length; k++) { const d = img[k] - target[k]; L += 0.5 * d * d; }
      return L;
    };
    ...
    for (const p of params) {
      for (let i = 0; i < N; i++) {
        const orig = g[p][i];
        g[p][i] = orig + eps; const Lp = lossOf(g);
        g[p][i] = orig - eps; const Lm = lossOf(g);
        g[p][i] = orig;
  ...
  it('Adam fits real 3D gaussians to a posed-view target (>100x L2 reduction)', () => {
    const W = 48, H = 36, N = 100, ITERS = 250;
    const cam: SplatCamera = { Vrow: [Math.cos(0.15), 0, Math.sin(0.15), 0, 1, 0, -Math.sin(0.15), 0, Math.cos(0.15)], t: [0, 0, 6], fx: 60, fy: 60 };
    const truth = makeScene(99, N);
    const target = forward2D(forward3D(truth, cam, W, H).g2, W, H).img;
    const g = makeScene(3, N);

    const P = ['x', 'y', 'z', 'sx', 'sy', 'sz', 'qr', 'qx', 'qy', 'qz', 'op', 'r', 'gr', 'bl'] as const;
    const lr: Record<string, number> = { x: 0.02, y: 0.02, z: 0.02, sx: 0.004, sy: 0.004, sz: 0.004, qr: 0.01, qx: 0.01, qy: 0.01, qz: 0.01, op: 0.01, r: 0.01, gr: 0.01, bl: 0.01 };
