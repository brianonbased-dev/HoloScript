...
function typeBytes(t: string): number {
  switch (t) {
    ...
    case 'uint16':
      return 2;
    default:
      throw new Error(`GaussianPlyLoader: unsupported PLY property type '${t}'`);
  }
}
...
export function loadGaussianPly(data: Uint8Array | ArrayBuffer): Gaussian3D {
  ...
  if (isAscii)
    throw new Error('GaussianPlyLoader: ASCII PLY not supported (3DGS exports are binary)');
  ...
  const read = (v: number, name: string): number => {
    ...
    switch (p.type) {
      case 'double':
      case 'float64':
        return dv.getFloat64(at, littleEndian);
      case 'float':
      case 'float32':
      default:
        return dv.getFloat32(at, littleEndian);
    }
  };
