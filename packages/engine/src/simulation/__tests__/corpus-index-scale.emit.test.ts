...
const OUT = join(
  ...
  'scripts',
  'conjecture-corpus',
  'corpus-scale-results.json'
);

describe('semantic corpus index — full-scale validation (P3)', () => {
  it(
    ENABLED
      ? 'builds the full arXiv index and runs real queries'
      : 'inert unless EMIT_CORPUS_SCALE=1',
    async (ctx) => {
      ...
      writeFileSync(
        OUT,
        JSON.stringify(
          { corpusSize: index.entries.length, dim: index.dim, buildMs, results },
          null,
          2
        ),
        'utf8'
