/**
 * native-render data layer (D.083, task 2h3s, Inc 0 + 1): pure-data scene extraction +
 * primitive mesh generation. Headless — no GPUDevice, no Three.js — so it runs in CI.
 * The GPU resource builder (DrawSpec[] + GPUDevice → IDrawCall[]) is a separate opt-in
 * backend gated on a browser receipt; it is intentionally NOT covered here.
 */
