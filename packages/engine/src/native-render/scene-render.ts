...
const BUF_VERTEX = 0x0020, BUF_INDEX = 0x0010, BUF_COPY_DST = 0x0008, BUF_UNIFORM = 0x0040;
...
export function orthographic(e = 2): Float32Array {
  ...
  m[0] = 1 / e; m[5] = 1 / e; m[10] = -0.05; m[14] = 0.5; m[15] = 1; // z in [-1,1]→[0,1]-ish; depth unused for color
  return m;
}
...
export async function renderDrawSpecs(
  ...
  const draws = specs
    ...
    .map((s) => {
      ...
      const vbuf = device.createBuffer({ size: mesh.positions.byteLength, usage: BUF_VERTEX | BUF_COPY_DST });
      device.queue.writeBuffer(vbuf, 0, mesh.positions);
      const ibuf = device.createBuffer({ size: mesh.indices.byteLength, usage: BUF_INDEX | BUF_COPY_DST });
  ...
  const texture = device.createTexture({ size: [size, size], format, usage: TEX_RENDER_ATTACHMENT | TEX_COPY_SRC });
  ...
  encoder.copyTextureToBuffer({ texture }, { buffer: readback, bytesPerRow, rowsPerImage: size }, { width: size, height: size, depthOrArrayLayers: 1 });
  device.queue.submit([encoder.finish()]);

  await readback.mapAsync(MAP_READ);
  const data = new Uint8Array(readback.getMappedRange().slice(0));
  readback.unmap(); readback.destroy(); texture.destroy();
  for (const d of draws) { d.vbuf.destroy(); d.ibuf.destroy(); }
