  ...
  for (let py = 0; py < H; py++)
    for (let px = 0; px < W; px++) {
      ...
      for (let k = hits.length - 1; k >= 0; k--) {
        ...
        const dSdx = g.a[i] * h.dx + g.b[i] * h.dy,
          dSdy = g.c[i] * h.dy + g.b[i] * h.dx;
...
async function main() {
    ...
    for (let py = 0; py < H; py++)
      for (let px = 0; px < W; px++) {
        ...
        for (let i = 0; i < N; i++) {
          const dx = px + 0.5 - g.posx[i],
            dy = py + 0.5 - g.posy[i];
          const s = 0.5 * (g.a[i] * dx * dx + g.c[i] * dy * dy) + g.b[i] * dx * dy;
          if (s < 0) continue;
          let al = g.op[i] * Math.exp(-s);
          if (al < 1 / 255) continue;
          al = Math.min(al, 0.999);
          cr += T * al * g.r[i];
          cg += T * al * g.gr[i];
          cb += T * al * g.bl[i];
          T *= 1 - al;
  ...
  try {
    ...
    const out = await page.evaluate(
      async ({ shaderSource, splats, dL32, W, H, N }) => {
        ...
        const messages = (ci?.messages ?? []).map((m) => ({
          type: m.type,
          line: m.lineNum,
          msg: m.message,
        }));
        if (messages.some((m) => m.type === 'error'))
          return { status: 'compile-error', info, messages };
        let pipeline;
        try {
          pipeline = await device.createComputePipelineAsync({
            layout: 'auto',
            compute: { module, entryPoint: 'cs_train_backward' },
          });
        } catch (e) {
          return { status: 'pipeline-error', info, messages, error: String(e) };
        }

        const splatBuf = device.createBuffer({
          size: splats.length * 4,
          usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
        });
        device.queue.writeBuffer(splatBuf, 0, new Float32Array(splats));
        const dlBuf = device.createBuffer({
          size: dL32.length * 4,
          usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
        });
    ...
    if (out.status !== 'completed') {
      ...
      if (out.messages?.length)
        console.error('  shader messages:', JSON.stringify(out.messages, null, 2));
      if (out.error) console.error('  error:', out.error);
      return 2;
    }
    console.log(
      `[gpu-validate] adapter: ${JSON.stringify(out.info)}  GPU(webgl): ${out.glRenderer}`
    );
    if (out.messages?.length)
      console.log(`[gpu-validate] shader warnings: ${JSON.stringify(out.messages)}`);

    // compare GPU (dequantized) vs CPU backward2D
    const K = ['posx', 'posy', 'a', 'b', 'c', 'r', 'gr', 'bl', 'op'];
    const ABS_FLOOR = 5e-3,
      ABS_TOL = 2e-3,
      REL_TOL = 1e-2;
    let worstRel = 0,
      worstAbs = 0,
    ...
    for (let i = 0; i < N; i++)
      for (let k = 0; k < 9; k++) {
        ...
        if (Math.abs(c) > ABS_FLOOR) {
          nMeaningful++;
          const rel = ae / Math.abs(c);
          if (rel > worstRel) {
            worstRel = rel;
            worstRelAt = `${K[k]}[${i}]=${c.toFixed(4)}`;
          }
        } else if (ae > worstAbs) {
          worstAbs = ae;
          worstAbsAt = `${K[k]}[${i}]`;
    ...
    console.log(
      `[gpu-validate] meaningful grads: ${nMeaningful}  worstRel: ${worstRel.toExponential(2)} (${worstRelAt})  worstAbs: ${worstAbs.toExponential(2)} (${worstAbsAt})`
    );
    console.log(
      `[gpu-validate] ${pass ? 'PASS — GPU kernel matches CPU backward2D on real hardware' : 'FAIL — GPU diverges from CPU reference'}`
    );
    return pass ? 0 : 2;
  } catch (e) {
