...
const STEPS = [
  {
    ...
    description: "Every HoloScript trait implements the TraitDefinition interface with a name, param schema, and compile method.",
    lines: [
      { content: 'import type { TraitDefinition, TraitCompileContext } from "@holoscript/core"', highlight: true },
      { content: "" },
      { content: "interface TraitDefinition<TParams = Record<string, unknown>> {", highlight: true },
      ...
      { content: "    ctx: TraitCompileContext,", type: "added" as const, annotation: "target + helpers" },
  ...
  },
  {
    ...
    lines: [
      ...
      { content: "}" },
      { content: "" },
      { content: "export const HealthBarTrait: TraitDefinition<HealthBarParams> = {", highlight: true },
      ...
      { content: "`AddComponent<HealthBarUI>().SetHP(${pct}f);`", type: "added" as const, annotation: "C#" },
  ...
  },
  {
    ...
    description: "Register your trait with the global TraitRegistry so it can be referenced in any .holo scene file.",
    lines: [
      { content: 'import { TraitRegistry } from "@holoscript/core"', highlight: true },
      { content: 'import { HealthBarTrait } from "./traits/HealthBar"' },
      { content: "" },
      { content: "TraitRegistry.register(HealthBarTrait)", highlight: true, annotation: "global registration" },
      { content: "" },
      ...
      { content: "      HealthBar { hp: 75, maxHp: 100, color: '#ff3322' }", type: "added" as const, annotation: "custom trait" },
      { content: "    ]", type: "added" as const },
      { content: "  }", type: "added" as const },
      { content: "}", type: "added" as const },
    ],
  },
  {
    ...
    description: "Write vitest tests using createComposition() to verify your trait compiles correctly for each target.",
    lines: [
      ...
      { content: 'import { HealthBarTrait } from "../traits/HealthBar"' },
      { content: "" },
      { content: "beforeAll(() => TraitRegistry.register(HealthBarTrait))", type: "added" as const },
      { content: "" },
      ...
      { content: "      traits: [{ name: 'HealthBar', params: { hp: 50, maxHp: 100 } }],", type: "added" as const },
      { content: "    })", type: "added" as const },
      { content: "    const out = new UnityCompiler().compile(comp)", type: "added" as const },
      { content: "    expect(out).toContain('SetHP(50f)')", type: "added" as const, annotation: "assert output" },
      { content: "  })" },
      { content: "})" },
    ],
  },
  {
    ...
    description: "Structure your trait package with the correct peer dependencies and holoscript-trait keyword for discoverability.",
    lines: [
      ...
      { content: '    "@holoscript/core": ">=3.0.0"', type: "added" as const, annotation: "not a direct dep" },
      { content: "  }," },
