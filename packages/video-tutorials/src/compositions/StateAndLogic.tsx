...
const STEPS = [
  {
    ...
    description:
      'The state block declares reactive variables — when they change, the scene updates.',
    lines: [
  ...
  },
  {
    ...
    description:
      'Use when() blocks to bind visual state to data — the door opens when doorOpen is true.',
    lines: [
      { content: '  object Door {' },
      ...
      { content: '    when (state.doorOpen) {', highlight: true, annotation: 'reactive binding' },
      {
        ...
        type: 'added' as const,
        annotation: 'door swings open',
      },
      {
        content: '      animate: RotateTo { duration: 0.5, easing: easeOut }',
        type: 'added' as const,
      },
      { content: '    }' },
      { content: '  }' },
  ...
  },
  {
    ...
    description:
      'on() handlers fire when user events occur — clicks, collisions, proximity, timers.',
    lines: [
  ...
  },
  {
    ...
    description:
      'Schedule logic with after() and every() — spawn enemies, run countdowns, pulse effects.',
    lines: [
