...
const STEPS = [
  {
    ...
    description:
      'Install the unified LLM provider package and create your first provider instance.',
    lines: [
      { content: '# Install the SDK', dim: true },
      {
        content: 'npm install @holoscript/llm-provider',
        highlight: true,
        annotation: 'unified interface',
      },
      { content: '' },
  ...
  },
  {
    ...
    description:
      'Use the Anthropic provider to call Claude models with the standard generateText interface.',
    lines: [
      { content: 'import { createProvider } from "@holoscript/llm-provider"', highlight: true },
      { content: '' },
      ...
      { content: '' },
      {
        content: 'console.log(result.text)   // the generated .holo scene',
        type: 'added' as const,
        annotation: '→ string',
      },
      {
        content: 'console.log(result.usage)  // { inputTokens, outputTokens }',
        type: 'added' as const,
      },
    ],
  },
  {
    ...
    description:
      'Swap to GPT-4o with a single line change — the interface is identical across all providers.',
    lines: [
      { content: 'import { createProvider } from "@holoscript/llm-provider"', highlight: true },
      { content: '' },
      ...
      { content: '' },
      {
        content: '// Drop-in replacement — no other code changes needed',
        type: 'added' as const,
        highlight: true,
      },
    ],
  },
  {
    ...
    description:
      'Use prompt engineering to reliably generate valid .holo scene code from natural language.',
    lines: [
      { content: 'import { createProvider } from "@holoscript/llm-provider"', highlight: true },
      ...
      {
        content: "if (!parseResult.ok) throw new Error('LLM generated invalid .holo')",
        type: 'added' as const,
      },
    ],
  },
  {
    ...
    description:
      'Use streamText for real-time output and batchGenerate to compile multiple targets simultaneously.',
    lines: [
      ...
      {
        content: '  process.stdout.write(chunk.delta)  // print as it arrives',
        type: 'added' as const,
      },
