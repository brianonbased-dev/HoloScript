...
const STEPS = [
  {
    ...
    lines: [
      ...
      { content: "    strategy:" },
      { content: "      matrix:" },
      { content: "        node-version: ['18.x', '20.x', '22.x']", type: "added" as const, annotation: "3 Node versions" },
    ],
  },
  {
    title: "Build Pipeline",
    description: "Install dependencies with pnpm, build the core package, then run full type-checking.",
    lines: [
      ...
      { content: "        run: pnpm install --frozen-lockfile", annotation: "reproducible installs" },
      { content: "" },
      ...
      { content: "        run: pnpm -r exec tsc --noEmit", type: "added" as const, annotation: "zero-error policy" },
    ],
  },
  {
    title: "E2E Export Tests",
    description: "Run the ExportTargets end-to-end test suite to verify all 15 compiler targets produce valid output.",
    lines: [
      { content: "      - name: Run unit tests", highlight: true },
      { content: "        run: pnpm -r test --reporter=verbose" },
      { content: "" },
      { content: "      - name: Run E2E export tests", highlight: true, annotation: "all 15 compilers" },
      { content: "        run: |" },
      { content: "          pnpm --filter @holoscript/core vitest run \\" },
      { content: "            src/compiler/__tests__/ExportTargets.e2e.test.ts", type: "added" as const },
      { content: "" },
      ...
      { content: "          token: ${{ secrets.CODECOV_TOKEN }}", type: "added" as const, annotation: "secret" },
  ...
  },
  {
    ...
    description: "semantic-release reads conventional commits, bumps the version, and publishes to npm automatically.",
    lines: [
      { content: "# .github/workflows/release.yml", dim: true },
      { content: "      - name: Release", highlight: true },
      { content: "        uses: cycjimmy/semantic-release-action@v4", annotation: "semantic-release" },
      { content: "        env:" },
      { content: "          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}", type: "added" as const },
      { content: "          NPM_TOKEN: ${{ secrets.NPM_TOKEN }}", type: "added" as const, annotation: "publish token" },
      { content: "" },
      ...
      { content: '    "@semantic-release/github"', type: "added" as const, annotation: "creates GitHub release" },
      { content: "  ]" },
      { content: "}" },
    ],
  },
  {
    ...
    description: "Coverage reports are uploaded to Codecov on every CI run, enforcing the 80% threshold gate.",
    lines: [
      ...
      { content: "[![Coverage](https://codecov.io/gh/holoscript/holoscript/graph/badge.svg)]", highlight: true, annotation: "badge" },
