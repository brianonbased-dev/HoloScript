...
const STEPS = [
  {
    title: 'Why a Sandbox?',
    description:
      'User-submitted .holo files and third-party plugins can execute arbitrary code — isolation is essential.',
    lines: [
      ...
      {
        content: 'const result = eval(userSubmittedCode)  // ← runs in host process',
  ...
  },
  {
    ...
    description:
      'Import createSandbox and configure isolation options before executing any untrusted code.',
    lines: [
      ...
      { content: '' },
      {
        content: 'const sandbox = createSandbox(options)',
        highlight: true,
        annotation: '→ Sandbox',
      },
    ],
  },
  {
    ...
    description:
      'Pass untrusted .holo source or plugin code to sandbox.execute() and receive a safe result object.',
    lines: [
      ...
      { content: '' },
      {
        content: '  console.log(result.output)    // captured console.log calls',
        type: 'added' as const,
      },
      {
        content: '  console.log(result.returnVal) // return value of last expression',
        type: 'added' as const,
      },
  ...
  },
  {
    ...
    description:
      'Fine-tune memory, CPU, module access, and network restrictions to match your security policy.',
    lines: [
      { content: 'const sandbox = createSandbox({', highlight: true },
      { content: '' },
      { content: '  // Execution limits', dim: true },
      {
        content: '  timeout: 3000,         // 3 s wall-clock timeout',
        annotation: 'default: 5000',
      },
      ...
      {
        content: '  allowedModules: [],    // no require() at all',
        type: 'added' as const,
        highlight: true,
      },
      ...
      {
        content: '  networkAccess: false,  // block fetch / XMLHttpRequest',
        type: 'added' as const,
        highlight: true,
      ...
      {
        content: "  fsAccess: 'none',      // 'none' | 'read-only' | 'read-write'",
        type: 'added' as const,
      },
      { content: '})' },
    ],
  },
  {
    title: 'Audit Logging',
    description:
      'Subscribe to sandbox events to build a complete audit trail of all untrusted code executions.',
    lines: [
      {
        content: 'import { createSandbox, SandboxEvent } from "@holoscript/security-sandbox"',
        highlight: true,
      },
      ...
      { content: '  })', type: 'added' as const },
      {
        ...
        type: 'added' as const,
        annotation: 'append to log',
      },
      { content: '})' },
      { content: '' },
      {
        content: "sandbox.on('violation', (v) => alertSecurityTeam(v))",
        highlight: true,
        annotation: 'real-time alerts',
      },
    ],
  },
];
