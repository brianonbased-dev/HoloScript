...
const STEPS = [
  ...
  },
  {
    ...
    description: "Load a .holo file, parse it into a composition, then compile to your target platform.",
    lines: [
      { content: "from holoscript import parse_file, compile_to_target", highlight: true },
      { content: "from pathlib import Path" },
      { content: "" },
      { content: "# Parse the source file", dim: true },
      { content: "composition = parse_file('scenes/SpinningCube.holo')", highlight: true, annotation: "→ Composition" },
      { content: "" },
      { content: "# Compile to Unity C#", dim: true },
      { content: "result = compile_to_target(composition, target='unity', options={", highlight: true },
      { content: "    'namespace': 'MyGame'," },
      { content: "    'useURP': True," },
      { content: "})" },
      { content: "" },
      { content: "Path('output/SpinningCube.cs').write_text(result.code)", type: "added" as const, annotation: "write output" },
    ],
  },
  {
    ...
    lines: [
      ...
      { content: "" },
      { content: "urdf_xml = export_urdf(composition, options={", highlight: true, annotation: "→ URDF string" },
  ...
  },
  {
    ...
    description: "Batch-compile an entire directory of .holo files to multiple targets in one script.",
    lines: [
      ...
      { content: "        result = compile_to_target(composition, target=target)", type: "added" as const },
  ...
  },
  {
    ...
    lines: [
      ...
      { content: "" },
      { content: "launch_py = generate_ros2_launch(composition, config={", highlight: true, annotation: "→ launch.py" },
