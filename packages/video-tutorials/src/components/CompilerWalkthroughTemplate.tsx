...
const SummarySlide: React.FC<{ target: string; items: string[] }> = ({ target, items }) => {
  ...
  return (
    ...
    >
            ...
            >
              <div
                style={{
                  ...
                  color: theme.text, fontFamily: theme.font, fontWeight: 700, fontSize: 18,
                  flexShrink: 0,
                }}
              >
                {isLast ? "→" : i + 1}
              </div>
              <span style={{ color: isLast ? theme.accent : theme.text, fontSize: 28, fontWeight: isLast ? 600 : 400 }}>
                {text}
              </span>
            </div>
          );
      ...
      <div
        style={{
          padding: "20px 28px", background: theme.surface, borderRadius: theme.borderRadius,
          border: `1px solid ${theme.accentGlow}44`, display: "flex", gap: 12,
          alignItems: "center", fontFamily: theme.font, fontSize: 20, color: theme.textMuted,
          marginTop: 8,
        }}
      >
