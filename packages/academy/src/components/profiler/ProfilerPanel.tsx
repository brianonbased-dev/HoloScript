...
function WaterfallBar({ frameMs, maxMs }: { frameMs: number; maxMs: number }) {
  ...
  const color = frameMs <= 16.67 ? '#4ade80' : frameMs <= 33.33 ? '#facc15' : '#f87171';
  return (
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            {
              label: 'p95',
              value: `${snap.p95FrameMs}ms`,
              color:
                snap.p95FrameMs > 33
                  ? 'text-red-400'
                  : snap.p95FrameMs > 16.67
                    ? 'text-yellow-400'
                    : 'text-green-400',
            },
          ].map((s) => (
            <div
              key={s.label}
              className="rounded-xl border border-studio-border bg-studio-surface p-2 text-center"
            >
        ...
        <div className="rounded-xl border border-studio-border bg-studio-surface p-3">
          <p className="mb-2 text-[10px] font-semibold text-studio-text">
            Frame Quality Distribution
          </p>
          ...
          <div className="mb-1.5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-[9px] text-studio-muted">
              <span className="flex items-center gap-0.5">
                <span className="inline-block h-1.5 w-3 rounded-full bg-green-400" /> &lt;17ms
              </span>
              <span className="flex items-center gap-0.5">
                <span className="inline-block h-1.5 w-3 rounded-full bg-yellow-400" /> &lt;33ms
              </span>
              <span className="flex items-center gap-0.5">
                <span className="inline-block h-1.5 w-3 rounded-full bg-red-400" /> slow
              </span>
            </div>
          </div>
        ...
        <div className="rounded-xl border border-studio-border bg-studio-surface/50 p-3 text-[10px] text-studio-muted space-y-1">
          <p className="flex items-center gap-1.5 font-semibold text-studio-text">
            <TrendingUp className="h-3 w-3" /> Tips
          </p>
