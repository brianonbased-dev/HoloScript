...
const TOOL_CATEGORIES: Record<string, string[]> = {
  core: [
    'parse_hs', 'parse_holo', 'validate_holoscript', 'suggest_traits', 'list_traits',
    'explain_trait', 'generate_object', 'generate_scene', 'generate_3d_object',
    'convert_format', 'render_preview', 'get_examples', 'get_syntax_reference',
    'analyze_code', 'explain_code', 'create_share_link',
  ],
  compiler: [
    'compile_holoscript', 'compile_to_r3f', 'compile_to_unity', 'compile_to_unreal',
    'compile_to_urdf', 'compile_to_webgpu', 'compile_to_gltf',
    'get_compilation_status', 'get_circuit_breaker_status', 'list_export_targets',
  ],
  ide: [
    'hs_scan_project', 'hs_diagnostics', 'hs_autocomplete', 'hs_refactor',
    'hs_docs', 'hs_code_action', 'hs_hover', 'hs_go_to_definition', 'hs_find_references',
  ],
  graph: [
    'holo_parse_to_graph', 'holo_visualize_flow', 'holo_get_node_connections',
    'holo_design_graph', 'holo_diff_graphs', 'holo_suggest_connections',
  ],
  ai: ['hs_ai_explain_error', 'hs_ai_fix_code', 'hs_ai_review', 'hs_ai_scaffold'],
  codebase: [
    'holo_absorb_repo', 'holo_query_codebase', 'holo_impact_analysis',
    'holo_detect_changes', 'holo_graph_status',
  ],
...
function validate(body: unknown): { ok: true; tool: string } | { ok: false; status: number; error: string } {
  ...
  if (!ALL_TOOLS.has(tool)) return { ok: false, status: 400, error: `Tool "${tool}" is not available` };
  return { ok: true, tool };
}
