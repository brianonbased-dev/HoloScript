...
import {
  carbon14RemainingFraction,
  estimateAgeFromC14,
  isC14Dateable,
  carbonDateRange,
  stratumThickness,
  identifyPeriodByDepth,
  stratigraphicSequenceValid,
  artifactsByMaterial,
  artifactsByStratum,
...
describe('Scenario: Archaeological Dig — Artifacts & Reconstruction', () => {
  const artifacts: ArtifactRecord[] = [
    {
      ...
      dimensions: { lengthCm: 8, widthCm: 5, heightCm: 0.5 },
      weight: 45,
      condition: 'good',
      description: 'Painted rim shard',
      photoUrls: [],
      fragments: ['a2'],
    },
    {
      ...
      name: 'Pottery Shard B',
      material: 'ceramic',
      stratumId: 's2',
      position: { x: 1.1, y: -0.5, z: 2.1 },
      dimensions: { lengthCm: 6, widthCm: 4, heightCm: 0.5 },
      weight: 30,
      condition: 'fair',
      description: 'Body shard',
      photoUrls: [],
      fragments: ['a1'],
    },
    {
      ...
      dimensions: { lengthCm: 7, widthCm: 0.5, heightCm: 0.5 },
      weight: 12,
      condition: 'poor',
      description: 'Roman iron nail',
      photoUrls: [],
      fragments: [],
    },
  ];
  ...
  it('reconstructionCompleteness() labels correctly', () => {
    expect(
      reconstructionCompleteness({ id: 'r1', name: 'Pot', fragmentIds: [], completeness: 0.95 })
    ).toBe('near-complete');
    expect(
      reconstructionCompleteness({ id: 'r2', name: 'Bowl', fragmentIds: [], completeness: 0.65 })
    ).toBe('substantial');
    expect(
      reconstructionCompleteness({ id: 'r3', name: 'Tile', fragmentIds: [], completeness: 0.35 })
    ).toBe('partial');
    expect(
      reconstructionCompleteness({ id: 'r4', name: 'Unknown', fragmentIds: [], completeness: 0.1 })
    ).toBe('fragmentary');
  });
