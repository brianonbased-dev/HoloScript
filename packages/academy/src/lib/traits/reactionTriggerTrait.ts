...
export class ReactionTriggerTrait {
  ...
  private triggerAnimation(
    ...
    switch (resolved.condition) {
      ...
      default:
        console.warn('[ReactionTriggerTrait] Unknown trigger condition:', resolved.condition);
    }
  }
  ...
  private executeAnimation(animationName: string, options: AnimationTriggerOptions): void {
    if (!this.viralPoseTrait) {
      console.warn('[ReactionTriggerTrait] ViralPoseTrait not attached, cannot play animation:', animationName);
      return;
    }
  ...
  resolveProximityAnimation(animationName: string): void {
    ...
    if (!options || options.condition !== 'proximity') {
      console.warn('[ReactionTriggerTrait] No pending proximity animation:', animationName);
      return;
    }
  ...
  resolveInteractionAnimation(animationName: string): void {
    ...
    if (!options || options.condition !== 'interaction') {
      console.warn('[ReactionTriggerTrait] No pending interaction animation:', animationName);
      return;
    }
  ...
  private dispatchAnimationEvent(event: AnimationEvent): void {
    ...
    this.onAnimationCallbacks.forEach((callback) => {
      try {
        ...
        console.error('[ReactionTriggerTrait] Animation callback error:', error);
      }
    });
