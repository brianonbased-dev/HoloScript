...
export async function issueTenantKey(tenantId: string, tier: 'free' | 'pro' | 'enterprise') {
  ...
  const randString = crypto.randomBytes(32).toString('base64').replace(/[^a-zA-Z0-9]/g, '');
  const apiKey = `holoscript_live_${randString}`;

  const limits = {
    free: { maxVideoDurationSec: 30, maxMediaResolution: 1024, allowProcessExec: false, rateLimitRequestsPerMin: 10 },
    pro: { maxVideoDurationSec: 300, maxMediaResolution: 2048, allowProcessExec: false, rateLimitRequestsPerMin: 100 },
    enterprise: { maxVideoDurationSec: 3600, maxMediaResolution: 4096, allowProcessExec: true, rateLimitRequestsPerMin: 1000 },
  }[tier] || { maxVideoDurationSec: 30, maxMediaResolution: 1024, allowProcessExec: false, rateLimitRequestsPerMin: 10 };
  ...
  if (!savedToDb && !process.env.UPSTASH_REDIS_REST_URL) {
    console.warn(`\x1b[33m⚠\x1b[0m Neither DATABASE_URL nor UPSTASH_REDIS_REST_URL are configured. Outputting dry-run only.`);
  }
