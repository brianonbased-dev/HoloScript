    ...
    switch (options.command) {
            ...
            try {
                ...
                lines.forEach((line, i) => {
                    for (const [typo, fix] of Object.entries(typos)) {
                        ...
                        if (typo === 'box' && line.includes('skybox'))
                            continue;
                ...
                for (const node of allNodes) {
                    if (node.directives) {
                        const hasGrabHook = node.directives.some((d) => d.hook === 'on_grab');
                        const hasGrabbableTrait = node.directives.some((d) => d.type === 'trait' && d.name === 'grabbable');
        ...
        case 'diff': {
            ...
            try {
                ...
                if (options.json) {
                    ...
                    const removed = result.changes.filter((c) => c.type === 'removed').length;
                    const modified = result.changes.filter((c) => c.type === 'modified').length;
                    const renamed = result.changes.filter((c) => c.type === 'renamed').length;
                    const moved = result.changes.filter((c) => c.type === 'moved').length;
                    ...
                    if (modified > 0)
                        console.log(`  \x1b[33m~ ${modified} modified\x1b[0m`);
                    if (renamed > 0)
                        console.log(`  \x1b[36m→ ${renamed} renamed\x1b[0m`);
                    if (moved > 0)
                        console.log(`  \x1b[35m↔ ${moved} moved\x1b[0m`);
        ...
        case 'wot-export': {
            ...
            try {
                ...
                const path = await import('path');
                const { HoloCompositionParser } = await import('@holoscript/core');
                const { ThingDescriptionGenerator, serializeThingDescription, validateThingDescription } = await import('@holoscript/core/wot');
                const filePath = path.resolve(options.input);
                if (!fs.existsSync(filePath)) {
                    console.error(`\x1b[31mError: File not found: ${filePath}\x1b[0m`);
                    process.exit(1);
        ...
        case 'headless': {
            ...
            try {
                ...
                const { createHeadlessRuntime, HEADLESS_PROFILE, getProfile } = await import('@holoscript/core/runtime/profiles');
                const filePath = path.resolve(options.input);
                if (!fs.existsSync(filePath)) {
                    console.error(`\x1b[31mError: File not found: ${filePath}\x1b[0m`);
                    process.exit(1);
                    ...
                    ast = {
                        root: {
                            ...
                            children: objects.map((obj) => ({
                                ...
                                traits: new Map(obj.traits?.map((t) => [t.name, t.config || {}]) || []),
                ...
                if (options.verbose) {
                    ...
                    runtime.on('runtime_stopped', (payload) => {
        ...
        case 'compile': {
            ...
            try {
                ...
                if (isHolo) {
                    ...
                    if (options.verbose)
                        console.log(`\x1b[2m[DEBUG] Using HoloScriptCodeParser...\x1b[0m`);
                    const { HoloScriptCodeParser } = await import('@holoscript/core');
                    const parser = new HoloScriptCodeParser();
                    const result = parser.parse(content);
                    if (!result.success) {
                    ...
                    ast = {
                        orbs: result.ast.filter((n) => n.type === 'orb'),
                        functions: result.ast.filter((n) => n.type === 'method'),
                    };
                }
                if (options.verbose) {
                ...
                if (stats.isFile()) {
                    ...
                    try {
                        ...
                        if (isHolo) {
                            ...
                            ast = { orbs: result.ast.filter((n) => n.type === 'orb') };
                        }
                        ...
                        if (options.split ||
                            ...
                            for (const chunk of chunks) {
                                ...
                                if (options.verbose)
                                    console.log(`  \x1b[2mChunk ${chunk.id} written (${chunk.objects.length} objects)\x1b[0m`);
                            }
        ...
        case 'login': {
            ...
            const registry = options.registry || process.env.HOLOSCRIPT_REGISTRY || 'https://registry.holoscript.net';
        ...
        case 'screenshot': {
            ...
            try {
                ...
                if (result.success && result.data) {
                    fs.writeFileSync(path.resolve(outputPath), result.data);
...
async function watchFile(options) {
    if (!options.input) {
