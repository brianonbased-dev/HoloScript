...
export function generateThreeJS(orbs: any[], functions: any[], _verbose: boolean): string {
...
`;
  ...
  for (const orb of orbs) {
    ...
    // Geometry creation — support all common types
    switch (geometry) {
      case 'cube':
      case 'box':
    ...
    // Material — handle emissive, opacity, metalness, roughness
    const isGlowing = traits.some((t: any) => t.name === 'glowing' || t.name === 'emissive');
    const matProps: string[] = [`color: '${color}'`];
    if (isGlowing) {
    ...
    // Physics annotation
    const hasPhysics = traits.some(
      (t: any) => t.name === 'physics' || t.name === 'rigid' || t.name === 'collidable'
    );
    if (hasPhysics) {
      code += `// Physics enabled for ${rawName} (@physics)\n`;
    }

    code += `scene.add(${name});\n\n`;
  }

  code += `camera.position.set(0, 3, 10);\ncontrols.target.set(0, 1, 0);\n\n`;
...
export function generateUnity(orbs: any[], _functions: any[], _verbose: boolean): string {
  let code = `// Generated by HoloScript Compiler
// Target: Unity C#
using UnityEngine;
...
`;
    ...
    const hasPhysics = traits.some(
      (t: any) => t.name === 'physics' || t.name === 'throwable' || t.name === 'grabbable'
    );
    if (hasPhysics) {
      code += `        // Add Physics
...
export function generateVRChat(orbs: any[], functions: any[], verbose: boolean): string {
  ...
  let code = `// Generated by HoloScript Compiler v${getVersionString()}
...
`;
  ...
  for (const orb of orbs) {
    ...
    // Physics traits
    const hasPhysics =
      hasTrait('physics') || hasTrait('throwable') || hasTrait('grabbable') || hasTrait('gravity');
    if (hasPhysics) {
  ...
  if (animatedOrbs.length > 0) {
    ...
    for (const orb of animatedOrbs) {
      const name = orb.name || 'object';
      const traits = (orb.traits || []).map((t: any) =>
        typeof t === 'string' ? { name: t, config: {} } : t
      );
      const hasTrait = (n: string) => traits.some((t: any) => t.name === n);
      const getConfig = (n: string) => traits.find((t: any) => t.name === n)?.config || {};
      ...
      if (hasTrait('spinning')) {
        ...
        const axis = cfg.axis || 'y';
        const axisVec =
          axis === 'x' ? 'Vector3.right' : axis === 'z' ? 'Vector3.forward' : 'Vector3.up';
        code += `        // @spinning ${name}\n`;
  ...
  if (verbose) {
    console.log(
      `Generated VRChat UdonSharp with ${orbs.length} objects, ${allTraitNames.size} trait types`
    );
  }
...
export function generateBabylon(orbs: any[], _functions: any[], _verbose: boolean): string {
  let code = `// Generated by HoloScript Compiler
// Target: Babylon.js
...
export function generateAFrame(orbs: any[], _functions: any[], _verbose: boolean): string {
  let code = `<!-- Generated by HoloScript Compiler -->
<!-- Target: A-Frame -->
<!DOCTYPE html>
<html>
...
export function generateWebXR(orbs: any[], _functions: any[], _verbose: boolean): string {
  let code = `// Generated by HoloScript Compiler
