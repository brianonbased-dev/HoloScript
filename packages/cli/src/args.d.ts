...
export interface CLIOptions {
    command: 'parse' | 'validate' | 'run' | 'ast' | 'repl' | 'watch' | 'node' | 'compile' | 'build' | 'add' | 'remove' | 'list' | 'traits' | 'suggest' | 'generate' | 'templates' | 'pack' | 'unpack' | 'inspect' | 'diff' | 'wot-export' | 'headless' | 'package' | 'deploy' | 'monitor' | 'publish' | 'login' | 'logout' | 'whoami' | 'access' | 'org' | 'token' | 'export' | 'import' | 'visualize' | 'screenshot' | 'prerender' | 'pdf' | 'absorb' | 'holoabsorb' | 'absorb-manifest' | 'graph-status' | 'impact' | 'impact-analysis' | 'query' | 'smoke' | 'self-improve' | 'daemon' | 'setup-hooks' | 'remove-hooks' | 'setup-mcp' | 'issue-key' | 'quickstart' | 'init' | 'help' | 'version';
    input?: string;
    output?: string;
    verbose: boolean;
    json: boolean;
