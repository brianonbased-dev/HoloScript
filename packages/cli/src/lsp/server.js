 ...
 * - Find references
 */
import { HoloScriptCodeParser, HoloCompositionParser, // Added
HoloScriptTypeChecker, VR_TRAITS, } from '@holoscript/core';
import { CompletionItemKind, DiagnosticSeverity, CodeActionKind, } from 'vscode-languageserver/node';
// Dynamic Trait Completions
const TRAIT_COMPLETIONS = VR_TRAITS.map((trait) => ({
    label: `@${trait}`,
...
export class HoloScriptLanguageServer {
    ...
    runCustomValidations(content, ast) {
        const diagnostics = [];
    ...
    getCompletions(uri, position) {
        const doc = this.documentCache.get(uri);
        if (!doc)
            return [...KEYWORDS, ...BUILTIN_FUNCTIONS];
    ...
    prepareRename(uri, position) {
        ...
        if (KEYWORDS.some((k) => k.label === word))
            return null;
        if (BUILTIN_FUNCTIONS.some((f) => f.label === word))
            return null;
