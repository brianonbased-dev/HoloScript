...
pub fn inspect_native_layouts(source: &str) -> Result<Vec<NativeStructLayout>, NativeCompileError> {
    ...
    let machine_contract = if has_borrowed_aggregate_buffer_subslice_return_machine_metadata(&ast) {
...
fn compile_unit(
    ...
    if has_borrowed_aggregate_buffer_subslice_return_machine_metadata(&ast) {
        Ok(CompiledObject {
        ...
        let result = if let Some(reference_type) = borrowed_result {
            ...
            match &reference_type.target {
                ...
                ReferenceTarget::Slice(element_type) => {
                    ...
                    if machine_contract
                        == BORROWED_AGGREGATE_BUFFER_SUBSLICE_RETURN_MACHINE_CONTRACT
                        && function_returns_aggregate_buffer_subslice_shape(ast, function, 2)
                    {
                        let sources = params
                            .iter()
                            .enumerate()
