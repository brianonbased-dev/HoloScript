...
describe('VectorSubsystem', () => {
  ...
  describe('connect', () => {
    ...
    it('should throw error if UPSTASH_VECTOR_URL is missing', async () => {
      ...
      await expect(vector.connect()).rejects.toThrow(
        'UPSTASH_VECTOR_URL and UPSTASH_VECTOR_TOKEN environment variables are required'
      );
    });
  ...
  describe('composition embeddings', () => {
    ...
    it('should upsert composition embedding', async () => {
      await vector.upsertComposition('comp-1', [0.1, 0.2, 0.3], {
        snippet: 'object Cube { @physics }',
        traits: ['@physics'],
        targets: ['unity'],
        tags: ['3d', 'physics'],
    ...
    it('should throw error when not connected', async () => {
      await vector.disconnect();
      await expect(
        vector.upsertComposition('comp-1', [0.1, 0.2], {
          snippet: 'test',
          traits: [],
          timestamp: Date.now(),
        })
  ...
  describe('text-based operations', () => {
    ...
    it('should search by text with filter', async () => {
      const results = await vector.searchByText('physics simulation', 5, 'namespace = "user123"');
      expect(results).toHaveLength(1);
    });
    ...
    it('should throw error for text upsert when not connected', async () => {
      await vector.disconnect();
      await expect(
        vector.upsertCompositionWithData('id', 'data', {
          snippet: 'test',
          traits: [],
          timestamp: Date.now(),
        })
