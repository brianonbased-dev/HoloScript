  ...
  inputSchema: {
    ...
    properties: {
      ...
      filter: {
        type: 'string',
        description:
          'Metadata filter string (e.g., "namespace = \\"user123\\" AND traits INCLUDES \\"@physics\\"")',
      },
      includeMetadata: {
        type: 'boolean',
...
export const upstashVectorSearchText: Tool = {
  ...
  description:
    'Find similar compositions by natural language query (generates embedding automatically)',
  inputSchema: {
    type: 'object',
    properties: {
      query: {
...
export const upstashVectorUpsertText: Tool = {
  ...
  description:
    'Add composition with automatic embedding generation from HoloScript source (requires embedding model on index)',
  inputSchema: {
    type: 'object',
    properties: {
      id: {
