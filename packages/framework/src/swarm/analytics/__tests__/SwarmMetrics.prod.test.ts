...
describe('SwarmMetrics — counter', () => {
  ...
  it('increment by 1', () => {
    ...
    m.register({ name: 'c', type: 'counter', description: 'test' });
    m.increment('c');
    expect(m.getCounter('c')).toBe(1);
  });
  it('increment by value', () => {
    const m = make();
    m.register({ name: 'c', type: 'counter', description: 'test' });
    m.increment('c', 5);
    expect(m.getCounter('c')).toBe(5);
  });
...
describe('SwarmMetrics — histogram', () => {
  it('observeHistogram adds to count', () => {
    const m = make();
    m.register({ name: 'h', type: 'histogram', description: 'test' });
    m.observeHistogram('h', 5);
    m.observeHistogram('h', 10);
    expect(m.getHistogram('h')!.count).toBe(2);
  });
  it('sum accumulates correctly', () => {
    const m = make();
    m.register({ name: 'h', type: 'histogram', description: 'test' });
    m.observeHistogram('h', 3);
    m.observeHistogram('h', 7);
  ...
  it('getStats histogram: avg = sum/count', () => {
    const m = make();
    m.register({ name: 'h', type: 'histogram', description: 'test' });
    m.observeHistogram('h', 4);
    m.observeHistogram('h', 6);
...
describe('SwarmMetrics — summary', () => {
  it('observeSummary stores values', () => {
    ...
    m.observeSummary('s', 10);
    m.observeSummary('s', 20);
  ...
  it('getStats summary: min/max/avg', () => {
    const m = make();
    m.register({ name: 's', type: 'summary', description: 'test' });
    m.observeSummary('s', 2);
    m.observeSummary('s', 4);
    m.observeSummary('s', 6);
