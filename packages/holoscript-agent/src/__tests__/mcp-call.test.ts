...
describe('runTool — mcp_call', () => {
  it('routes to the injected invokeMcpTool callback and returns its text', async () => {
    ...
    const res = await runTool(
      use({ tool: 'validate_holoscript', args: { code: '#version 6.0.0\nscene "S" {}' } }),
      { invokeMcpTool }
    );
    expect(invokeMcpTool).toHaveBeenCalledWith('validate_holoscript', {
      code: '#version 6.0.0\nscene "S" {}',
    });
  ...
  it('returns an error result when the MCP tool fails (ok:false)', async () => {
    const invokeMcpTool = vi.fn(async () => ({
      ok: false,
      text: 'mcp_call compile_holoscript error: parse failed',
    }));
