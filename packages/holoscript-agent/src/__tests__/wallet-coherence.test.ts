...
describe('decideWalletCoherence', () => {
  it('coherent: key-derived address matches the declared wallet', () => {
    expect(decideWalletCoherence({ derivedAddress: ORIN, declaredWallet: ORIN, hasBearer: true, halt: false }))
      .toEqual({ coherent: true });
  });

  it('coherent regardless of case (checksum vs lowercase)', () => {
    expect(decideWalletCoherence({ derivedAddress: ORIN.toLowerCase(), declaredWallet: ORIN, hasBearer: false, halt: false }))
      .toEqual({ coherent: true });
  });

  it('no local key → coherent (already bearer-only, nothing to reconcile)', () => {
    expect(decideWalletCoherence({ derivedAddress: undefined, declaredWallet: ORIN, hasBearer: true, halt: false }))
      .toEqual({ coherent: true });
  });

  it('stray key + bearer present → degrade to bearer-only (the holojetson self-heal)', () => {
    const d = decideWalletCoherence({ derivedAddress: STRAY, declaredWallet: ORIN, hasBearer: true, halt: false });
  ...
  it('stray key + NO bearer → halt (nothing carries the declared identity)', () => {
    const d = decideWalletCoherence({ derivedAddress: STRAY, declaredWallet: ORIN, hasBearer: false, halt: false });
    expect(d).toMatchObject({ coherent: false, action: 'halt' });
  });

  it('stray key + HALT flag → halt even with a bearer (strict/CI mode)', () => {
    const d = decideWalletCoherence({ derivedAddress: STRAY, declaredWallet: ORIN, hasBearer: true, halt: true });
    expect(d).toMatchObject({ coherent: false, action: 'halt' });
  });
