...
for (const obj of r.ast.objects.slice(0, 25)) {
  ...
  lines.push(`${obj.name}: geo=${geo?.value ?? '?'} pos=${JSON.stringify(pos?.value)} mat=${mat?.value ?? '-'} ch=${childCount}`);
}
...
if (result.json) {
  ...
  for (const node of (gltf.nodes || []).slice(0, 25)) {
    ...
    lines.push(`  ${node.name}: t=[${t}] ch=${node.children?.length ?? 0} mesh=${node.mesh ?? '-'}`);
  }
}
