...
/** Buffer one generated page. flushPages() decides whether it is written or compared. */
function emitFile(target: string, code: string): void {
  PENDING.push({ target, code });
}

/**
 * Format every buffered page, then write them all (build) or compare them all
 * against the committed copies (--check), through emitGeneratedSet. Both modes
 * consume the identical bytes, so the check cannot drift away from the build it is
 * guarding.
 *
 * A page prettier cannot parse is a compiler bug: the run refuses loudly, in build
 * and check alike, naming the file (independent review of #309,
 * task_1790066851748_j9di), and writes nothing. This runs before saveManifest, so
 * the manifest is left alone too; formatting and writing page by page used to stop
 * part-way, with 6 of 15 pages rewritten and the manifest stale (claude3-x402's
 * review of #316).
 *
...
async function flushPages(): Promise<void> {
  ...
  drift.push(...stale.map((target) => relative(STUDIO_ROOT, target)));
}
...
async function build(): Promise<void> {
  ...
  for (const holoPath of holoFiles) {
    ...
    // Buffered: flushPages() below writes (or, under --check, compares) every page at once
    emitFile(result.outputPath, result.code);
...
async function watch(): Promise<void> {
  ...
  watcher.on('change', async (path) => {
    ...
    if (result) {
      ...
      // raw compiler output, so a watch session left pages the next check called
      // drift. A page prettier cannot parse now ends the watch loudly.
      const stale = await emitGeneratedSet([{ target: result.outputPath, code: result.code }], {
        check: CHECK,
      });
      console.log(
        CHECK
          ? `  ${stale.length > 0 ? '✗ differs from the committed copy' : '✓ matches'}`
          : '  ✓ Recompiled'
      );
    }
  });
