...
test.describe('Reconstruction scan desktop (authenticated)', () => {
  test.skip(
    !hasAuthState,
    'Set PLAYWRIGHT_STORAGE_STATE to a saved login JSON to run desktop scan-room E2E.'
  );

  test('scan-room shows session panel after auth', async ({ page }) => {
    await page.goto('/scan-room');
    await expect(page.getByRole('heading', { name: /Mobile Room Scan/i })).toBeVisible({
      timeout: 30_000,
    });
