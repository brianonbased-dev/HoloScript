...
1. **`@holoscript/studio`** (port 3100) — The main IDE/creation platform. Live page/API/component/hook/lib/panel counts are generated by `pnpm --filter @holoscript/studio inventory` and guarded by `inventory:check`. Brittney AI powers the `/vibe` experience and `/create` chat panel.
...
**Historical v6.0.2 restructure**: Studio was reorganized into a progressive disclosure funnel with 3 spaces (HoloMesh public social, Teams private workspaces, Agents fleet management). The `/start` route handles user provisioning (GitHub OAuth → API key → repo → scaffold → daemon) with consent gates. Do not use this section for current inventory counts.
