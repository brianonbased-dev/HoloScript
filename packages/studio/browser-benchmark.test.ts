...
describe('quickBenchmark', () => {
  ...
  it('should run benchmark with correct test code and iterations', async () => {
    ...
    expect(mockWindow.CompilerBridge.compile).toHaveBeenCalledWith(expectedTestCode, { target: 'threejs' })
  ...
  it('should log results to console with correct format', async () => {
    ...
    const resultLogs = logCalls.filter(call => 
      typeof call[0] === 'string' && 
      (call[0].includes('Parse:') || call[0].includes('Compile:'))
    )
