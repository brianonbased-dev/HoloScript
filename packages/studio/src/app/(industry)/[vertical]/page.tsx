...
const V_CONFIG: Record<string, { title: string; desc: string }> = {
  healthcare: { title: 'Medical Simulation', desc: 'DICOM import, anatomical materials, compliance' },
...
export default function IndustryPortalPage() {
  ...
  const config = V_CONFIG[vertical] || { title: 'Professional Environment', desc: 'Tailored workflow tools' };
  ...
  return (
    <div className="flex h-full flex-col bg-studio-bg text-studio-text">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div style={{ width: leftPanelW }} className="flex flex-col border-r border-studio-border bg-studio-panel">
        ...
        <PanelSplitter orientation="vertical" onDrag={(d) => setLeftPanelW((p) => Math.max(200, p + d))} />
        ...
        <PanelSplitter orientation="vertical" onDrag={(d) => setRightPanelW((p) => Math.max(200, p - d))} />

        {/* Right Side: Professional Pipelines */}
        <div style={{ width: rightPanelW }} className="flex flex-col border-l border-studio-border bg-studio-panel">
          <div className="flex h-10 items-center justify-between border-b border-studio-border px-3 bg-studio-surface">
             <span className="flex items-center gap-2 text-xs font-semibold text-studio-muted uppercase tracking-wider">
