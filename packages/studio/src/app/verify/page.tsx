...
function VerifyForm() {
  ...
  const [result, setResult] = useState<
    { type: 'success'; data: VerifySuccess } | { type: 'error'; message: string } | null
  >(null);
  ...
  const canSubmit = form.teamId.trim().length > 0 && form.userCode.trim().length >= 4;
      ...
      <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-[#0a0a12] p-6 shadow-2xl">
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/50">Approve a new surface for your HoloMesh team</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Team ID */}
          <div>
            <label className="mb-1 block text-sm font-medium text-white/70">Team ID</label>
            <input
              type="text"
              value={form.teamId}
          ...
          {/* User Code */}
          <div>
            <label className="mb-1 block text-sm font-medium text-white/70">User Code</label>
            <input
              type="text"
              value={form.userCode}
              onChange={(e) => setForm((f) => ({ ...f, userCode: e.target.value.toUpperCase() }))}
          ...
          {/* Surface */}
          <div>
            <label className="mb-2 block text-sm font-medium text-white/70">Surface Type</label>
          ...
          {/* TTL */}
          <div>
            <label className="mb-1 block text-sm font-medium text-white/70">Token Lifetime</label>
            <select
              value={form.ttlSeconds}
              onChange={(e) => setForm((f) => ({ ...f, ttlSeconds: Number(e.target.value) }))}
