...
/**
 * HoloMesh Teams Discovery — /holomesh/teams
 *
 * Browse public teams, see their objectives, and join open ones.
 */
...
function teamTypeLabel(type?: string | null): { label: string; color: string } {
  switch (type) {
    case 'open':    return { label: 'Open',   color: 'var(--studio-accent)' };
    case 'invite':  return { label: 'Invite', color: '#f59e0b' };
    default:        return { label: 'Private', color: 'var(--studio-muted)' };
  }
}
    ...
    <div style={{ padding: '2rem', maxWidth: 920, margin: '0 auto', color: 'var(--studio-text)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
        <Link
          ...
          style={{ display:'flex', alignItems:'center', gap:'0.25rem', color:'var(--studio-muted)', textDecoration:'none', fontSize:'0.8rem' }}
        >
      ...
      <div style={{ position: 'relative', marginBottom: '1.25rem' }}>
        <Search size={15} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--studio-muted)', pointerEvents: 'none' }} />
        <input
          type="text"
      ...
      {!loading && error && (
        <div style={{ padding: '1rem', borderRadius: 8, background: '#fee2e2', color: '#b91c1c', fontSize: '0.875rem' }}>
          {error}
        </div>
      )}
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem' }}>
                    <Link
                      href={`/holomesh/team/${team.id}`}
                      style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--studio-text)', textDecoration: 'none', flex: 1, lineHeight: 1.3 }}
                    >
                      {team.name}
                    </Link>
                    <span style={{
                      fontSize: '0.7rem', padding: '0.18rem 0.55rem', borderRadius: 99,
                      background: `${typeColor}22`, color: typeColor,
                      border: `1px solid ${typeColor}55`, whiteSpace: 'nowrap',
                  ...
                  {/* Objective */}
                  {team.objective && (
                    <p style={{ margin: 0, fontSize: '0.82rem', color: 'var(--studio-muted)', lineHeight: 1.45, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                      {team.objective}
                    </p>
                  )}

                  {/* Meta row */}
                  <div style={{ display: 'flex', gap: '0.75rem', fontSize: '0.78rem', color: 'var(--studio-muted)', alignItems: 'center', flexWrap: 'wrap' }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                      <Users size={11} />
                      {team.memberCount ?? team.members?.length ?? 0} member{(team.memberCount ?? team.members?.length ?? 0) !== 1 ? 's' : ''}
                    </span>
                    {team.mode && (
                      <span style={{ background: 'var(--studio-bg)', border: '1px solid var(--studio-border)', borderRadius: 4, padding: '0.1rem 0.4rem' }}>
                        {modeLabel(team.mode)}
                      </span>
                    )}
                    {(team.tags ?? []).slice(0, 2).map(tag => (
                      <span
                        key={tag}
                        style={{ background: 'var(--studio-bg)', border: '1px solid var(--studio-border)', borderRadius: 4, padding: '0.1rem 0.4rem' }}
                      >
                        #{tag}
                      </span>
                    ))}
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.25rem' }}>
                    ...
                    {isJoined ? (
                      <span style={{
                        flex: 1, textAlign: 'center', padding: '0.4rem',
                        borderRadius: 7, background: '#d1fae5', color: '#065f46',
                        fontSize: '0.8rem',
                      }}>
                        ✓ Joined
                      </span>
                      ...
                      <span style={{
                        flex: 1, textAlign: 'center', padding: '0.4rem',
                        borderRadius: 7, border: '1px solid var(--studio-border)',
                        background: 'var(--studio-bg)', color: 'var(--studio-muted)',
                        fontSize: '0.8rem',
                      }}>
                        Invite only
                      </span>
