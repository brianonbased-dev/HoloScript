...
const REGISTRY: TraitEntry[] = [
    ...
    params: [
      {
        name: 'position',
        type: 'vector3',
        description: '[x, y, z] in world units',
        default: '[0, 0, 0]',
      },
      {
        name: 'rotation',
        type: 'vector3',
        description: '[rx, ry, rz] Euler degrees',
        default: '[0, 0, 0]',
      },
      {
        name: 'scale',
        type: 'vector3',
        description: '[sx, sy, sz] multipliers',
        default: '[1, 1, 1]',
      },
    ],
    snippet: `  @transform {\n    position: [0, 0, 0]\n    rotation: [0, 0, 0]\n    scale: [1, 1, 1]\n  }`,
  },
  {
    ...
    since: 'v1.0',
    description:
      'PBR material properties including albedo, metallic, roughness, emissive, and opacity.',
    ...
    params: [
      { name: 'albedo', type: 'color', description: 'Base color hex string', default: '"#ffffff"' },
      {
        name: 'metallic',
        type: 'number',
        description: '0.0 (dielectric) to 1.0 (metal)',
        default: 0,
      },
      {
        name: 'roughness',
        type: 'number',
        description: '0.0 (mirror) to 1.0 (matte)',
        default: 0.5,
      },
      ...
      { name: 'emissiveIntensity', type: 'number', description: 'Glow multiplier', default: 0 },
      {
        name: 'opacity',
        type: 'number',
        description: '0.0 (transparent) to 1.0 (opaque)',
        default: 1,
      },
      {
        name: 'fragmentShader',
        type: 'string',
        description: 'Shader preset ID from /api/shader-presets',
      },
    ],
    snippet: `  @material {\n    albedo: "#ffffff"\n    metallic: 0.0\n    roughness: 0.5\n    opacity: 1.0\n  }`,
  },
  {
    ...
    since: 'v1.0',
    description:
      'Rigid-body physics simulation. Type static = immovable, dynamic = falls+collides, kinematic = script-driven.',
  ...
  },
  {
    ...
    since: 'v1.0',
    description:
      'Attach spatial or ambient audio to an object. Supports loop, volume, pitch, and 3D falloff.',
  ...
  },
  {
    ...
    since: 'v1.1',
    description:
      'Attach keyframe animations or procedural motions (rotate, float, pulse) to an object.',
    ...
    params: [
      { name: 'idle', type: 'string', description: 'Idle animation clip name' },
      {
        name: 'rotate',
        type: 'vector3',
        description: 'Continuous rotation speed [rx, ry, rz] deg/s',
      },
      {
        name: 'float',
        type: 'string',
        description: 'Floating motion: "{ amplitude: 0.3 frequency: 0.5 }"',
      },
  ...
  },
  {
    ...
    since: 'v1.2',
    description:
      'GPU particle emitter with configurable type, rate, lifetime, size, and color gradient.',
    tags: ['vfx', 'emitter', 'fire', 'sparkle'],
    params: [
      {
        name: 'type',
        type: 'enum',
        description: 'Particle preset',
  ...
  },
  {
    ...
    since: 'v1.2',
    description:
      'Level of Detail distances. Objects swap to lower-poly meshes beyond each threshold distance.',
    tags: ['performance', 'culling', 'distance', 'optimization'],
    params: [
      {
        name: 'levels',
        type: 'string',
        description: 'Array of [high, medium, low] switch distances in world units',
        default: '[5, 20, 60]',
      },
      {
        name: 'cull',
        type: 'number',
        description: 'Distance beyond which object is culled entirely',
        default: 200,
      },
    ],
    snippet: `  @lod {\n    levels: [5, 20, 60]\n    cull: 200\n  }`,
  },
  {
    ...
    params: [
      ...
      { name: 'intensity', type: 'number', description: 'Light intensity', default: 1 },
      {
        name: 'range',
        type: 'number',
        description: 'Falloff range in world units (point/spot)',
        default: 10,
      },
  ...
  },
  {
    ...
    since: 'v1.4',
    description:
      'Spatial/XR behaviours: billboard mode, gaze tracking, hand attachment, interaction, lifespan.',
    ...
    params: [
      { name: 'billboard', type: 'boolean', description: 'Always face the camera', default: false },
      {
        name: 'followGaze',
        type: 'boolean',
        description: 'Smoothly follow gaze direction',
        default: false,
      },
      {
        name: 'attachTo',
        type: 'string',
        description: 'Bone/anchor to attach to (e.g. "leftWrist")',
      },
      {
        name: 'interactable',
        type: 'boolean',
        description: 'Enable grab/pinch interaction',
        default: false,
      },
  ...
  },
  {
    ...
    params: [
      ...
      { name: 'far', type: 'number', description: 'Far clip plane distance', default: 1000 },
      {
        name: 'active',
        type: 'boolean',
        description: 'Set as the active scene camera',
        default: false,
      },
    ],
    snippet: `  @camera {\n    fov: 60\n    near: 0.01\n    far: 1000\n    active: true\n  }`,
  },
  {
    ...
    since: 'v2.0',
    description:
      'Attach AI behaviour to an NPC: pathfinding goal, behaviour tree root, perception radius.',
    tags: ['npc', 'pathfinding', 'behaviour', 'ai'],
    params: [
      {
        name: 'goal',
        type: 'enum',
        description: 'High-level AI goal',
        options: ['idle', 'patrol', 'follow', 'flee', 'wander', 'attack'],
        default: 'idle',
      },
      { name: 'speed', type: 'number', description: 'Movement speed in units/s', default: 2 },
      {
        name: 'perception',
        type: 'number',
        description: 'Perception radius in world units',
        default: 10,
      },
