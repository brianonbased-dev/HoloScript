...
import {
  CombatManager,
  type HitBox, type HurtBox, type ComboChain,
} from '@holoscript/core';

export interface CombatEntity {
  id: string; name: string; hp: number; maxHp: number;
  position: { x: number; y: number; z: number };
  hitboxId?: string; hurtboxId?: string;
}
...
export function useCombat(): UseCombatReturn {
  ...
  const [hitLog, setHitLog] = useState<Array<{ hitboxId: string; hurtboxId: string; time: number }>>([]);
  const idCounter = useRef(0);

  const sync = useCallback(() => { setHitLog(mgrRef.current.getHitLog()); }, []);
  ...
  const spawnEntity = useCallback((name: string) => {
    ...
    const hbId = `hb-${id}`;
    const hrId = `hr-${id}`;
    mgrRef.current.addHitBox({ id: hbId, ownerId: id, position: pos, size: { x: 1, y: 1, z: 1 }, active: false, damage: 10, damageType: 'physical', knockback: 2 });
    mgrRef.current.addHurtBox({ id: hrId, ownerId: id, position: pos, size: { x: 0.8, y: 1.5, z: 0.8 }, active: true });
    setEntities(prev => [...prev, { id, name, hp: 100, maxHp: 100, position: pos, hitboxId: hbId, hurtboxId: hrId }]);
  }, []);

  const attack = useCallback((attackerId: string, targetId: string) => {
    const atkEntity = entities.find(e => e.id === attackerId);
    if (atkEntity?.hitboxId) { mgrRef.current.setHitBoxActive(atkEntity.hitboxId, true); }
    const hits = mgrRef.current.checkCollisions();
    for (const h of hits) {
      setEntities(prev => prev.map(e => e.hurtboxId === h.hurtbox.id ? { ...e, hp: Math.max(0, e.hp - h.hitbox.damage) } : e));
    }
    if (atkEntity?.hitboxId) { mgrRef.current.setHitBoxActive(atkEntity.hitboxId, false); }
    sync();
  }, [entities, sync]);
  ...
  const reset = useCallback(() => {
    ...
    idCounter.current = 0;
    setEntities([]); setCombos([]); setHitLog([]);
  }, []);

  return { manager: mgrRef.current, entities, combos, hitLog, spawnEntity, attack, registerCombo, advanceCombo, tick, reset };
}
