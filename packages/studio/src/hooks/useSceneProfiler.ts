...
const DRAW_CALL_WEIGHTS: Record<string, number> = {
  mesh: 1, splat: 4, camera: 0, light: 0.5, group: 0, audio: 0,
};
...
export function useSceneProfiler(): ProfilerMetrics {
  ...
  return useMemo<ProfilerMetrics>(() => {
    ...
    const shadowLights = nodes.filter((n) => n.type === 'light' && n.traits?.some((t) => t.properties?.castShadow)).length;
    const estimatedDrawCalls = nodes.reduce((sum, n) => sum + (DRAW_CALL_WEIGHTS[n.type] ?? 0), 0)
    ...
    const complexityLabel =
      complexityScore < 25 ? 'Lightweight' :
      complexityScore < 55 ? 'Moderate' :
      complexityScore < 80 ? 'Heavy' : 'Extreme';
    ...
    if (splatCount > 2) findings.push({ severity: 'warning', message: `${splatCount} Gaussian splats — each is GPU-intensive; consider LOD` });
    if (shadowLights > 2) findings.push({ severity: 'warning', message: `${shadowLights} shadow-casting lights multiply draw calls by ${meshCount}×` });
    if (particleCount > 3) findings.push({ severity: 'warning', message: `${particleCount} particle systems — cap emit rate or use instanced particles` });
    if (nodeCount > 50) findings.push({ severity: 'error', message: `${nodeCount} scene nodes — exceeds recommended limit of 50` });
    if (traitCoverage < 0.3 && nodeCount > 3) findings.push({ severity: 'tip', message: 'Low trait coverage — add @lod, @physics, or @material traits to improve quality' });
    if (lightCount > 4) findings.push({ severity: 'tip', message: `${lightCount} lights — prefer baked lighting for static geometry` });
    if (complexityScore < 20 && nodeCount < 3) findings.push({ severity: 'tip', message: 'Scene is nearly empty — start adding objects from the palette' });

    return {
      nodeCount, meshCount, lightCount, audioCount, particleCount: splatCount + particleCount,
      traitCount, traitCoverage, estimatedDrawCalls: Math.round(estimatedDrawCalls),
      complexityScore, complexityLabel, frameBudgetPercent, findings,
    };
  }, [nodes, code]);
}
