...
const DEMO_ENTITIES: ViewportEntity[] = [
  { id: 'ground', name: 'Ground', type: 'plane', position: [0, 0, 0], rotation: [-Math.PI / 2, 0, 0], scale: [20, 20, 1], color: '#3a5a3a' },
  { id: 'player', name: 'Player', type: 'box', position: [0, 1, 0], rotation: [0, 0, 0], scale: [1, 2, 1], color: '#4488ff' },
  { id: 'enemy', name: 'Enemy', type: 'sphere', position: [5, 1, -3], rotation: [0, 0, 0], scale: [1, 1, 1], color: '#ff4444' },
  { id: 'tower', name: 'Tower', type: 'cylinder', position: [-4, 2.5, 2], rotation: [0, 0, 0], scale: [0.8, 5, 0.8], color: '#888888' },
  { id: 'gem', name: 'Gem', type: 'torus', position: [3, 2, 4], rotation: [Math.PI / 4, 0, 0], scale: [0.5, 0.5, 0.5], color: '#ffaa00' },
];

const DEMO_LIGHTS: ViewportLight[] = [
  { id: 'sun', type: 'directional', position: [10, 15, 5], color: '#ffe8c0', intensity: 1.5, enabled: true },
  { id: 'fill', type: 'point', position: [-5, 3, 5], color: '#6699ff', intensity: 0.6, enabled: true },
  { id: 'ambient', type: 'ambient', position: [0, 0, 0], color: '#1a1a2e', intensity: 0.3, enabled: true },
];
...
export function useViewport(): UseViewportReturn {
        ...
        if (data?.lights) {
          setState(prev => ({
            ...
            lights: data.lights.map((l: any) => ({
              ...
              color: Array.isArray(l.color) ? `rgb(${Math.floor(l.color[0] * 255)},${Math.floor(l.color[1] * 255)},${Math.floor(l.color[2] * 255)})` : '#ffffff',
  ...
  const addEntity = useCallback((entity: Omit<ViewportEntity, 'id'>) => {
  ...
  const toggleGrid = useCallback(() => setState(prev => ({ ...prev, gridVisible: !prev.gridVisible })), []);
  const toggleAxes = useCallback(() => setState(prev => ({ ...prev, axesVisible: !prev.axesVisible })), []);
  const setBackground = useCallback((color: string) => setState(prev => ({ ...prev, backgroundColor: color })), []);
  ...
  return { state, addEntity, removeEntity, selectEntity, moveEntity, setMode, toggleGrid, toggleAxes, setBackground, updateLight, buildDemo, clear };
}
