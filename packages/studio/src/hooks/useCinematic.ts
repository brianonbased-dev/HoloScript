...
export function useCinematic(): UseCinematicReturn {
  ...
  const createDemoScene = useCallback(() => {
    ...
    dirRef.current.addCue('intro', {
      ...
      time: 1.5,
      type: 'dialogue',
      data: { speaker: 'hero', text: 'The time has come.' },
    });
    dirRef.current.addCue('intro', {
      id: 'c3',
      time: 3,
    ...
    dirRef.current.addCue('intro', {
      ...
      time: 3.5,
      type: 'dialogue',
      data: { speaker: 'villain', text: 'Indeed it has.' },
    });
    dirRef.current.addCue('intro', {
      id: 'c5',
      time: 4.5,
      type: 'effect',
      data: { type: 'fade_to_black' },
    });
    sync();
  }, [sync]);
  ...
  const play = useCallback(
    (sceneId: string) => {
      dirRef.current.playScene(sceneId);
      sync();
    },
    [sync]
  );
