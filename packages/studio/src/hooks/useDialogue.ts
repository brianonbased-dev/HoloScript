...
export function useDialogue(): UseDialogueReturn {
  ...
  const addText = useCallback(
    (id: string, speaker: string, text: string, nextId: string | null) => {
  ...
  const addChoice = useCallback(
    (
      ...
      speaker: string,
      text: string,
      cs: Array<{ text: string; nextId: string; condition?: string }>
    ) => {
  ...
  const addBranch = useCallback(
    (id: string, condition: string, trueId: string, falseId: string) => {
  ...
  const loadDemo = useCallback(() => {
    ...
    g.addTextNode(
      'weapons',
      'Merchant',
      'Fine steel from the Northern Forge! Take a look.',
      'buy-prompt'
    );
    g.addTextNode(
      'potions',
      'Merchant',
      'Healing elixirs, mana restorers, and antidotes.',
      'buy-prompt'
    );
