...
export function useSnapshots(sceneId: string) {
  ...
  const capture = useCallback(async (label?: string) => {
    ...
    try {
      ...
      let dataUrl = '';
      const canvas = document.querySelector<HTMLCanvasElement>('canvas.r3f-canvas, canvas[data-engine]');
      if (canvas) {
      ...
      const res = await fetch('/api/snapshots', {
        ...
        body: JSON.stringify({ sceneId, label: label ?? `Snapshot ${new Date().toLocaleTimeString()}`, dataUrl, code }),
      });
