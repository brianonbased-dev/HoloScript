...
export function useAssetLibrary() {
  ...
  const search = useCallback(async (q?: string, cat?: AssetCategory | '', pg?: number) => {
    setLoading(true);
    setError(null);
    const nextQ = q ?? query;
  ...
  return { results, total, page, pages, query, category, loading, error, search, setPage: (p: number) => search(query, category, p) };
}
