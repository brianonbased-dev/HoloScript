...
export function useNodeGraph() {
  ...
  const addNode = useCallback((def: NodeDef, x = 100 + _nodeCounter * 40, y = 80 + _nodeCounter * 50) => {
    _nodeCounter++;
    const id = `n-${_nodeCounter}`;
    setNodes((prev) => [...prev, {
      id, type: def.type, label: def.label, category: def.category,
      color: def.color, x, y, inputs: def.inputs, outputs: def.outputs,
    }]);
    return id;
  }, []);
  ...
  const clearGraph = useCallback(() => {
    ...
    _nodeCounter = 0; _edgeCounter = 0;
  }, []);

  return { nodes, edges, selected, setSelected, addNode, removeNode, moveNode, connect, disconnect, clearGraph };
}
