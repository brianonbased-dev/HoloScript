...
export function useMultiSelect() {
  ...
  const applyDelta = useCallback((delta: DeltaTransform) => {
  ...
  const applyAbsolute = useCallback((abs: DeltaTransform) => {
    for (const id of selectedIds) {
      const patch: Partial<SceneNode> = {};
  ...
  const centroid = useMemo<[number, number, number]>(() => {
    ...
    const sum = selectedNodes.reduce(
      (acc, n) => [acc[0] + n.position[0], acc[1] + n.position[1], acc[2] + n.position[2]] as [number, number, number],
      [0, 0, 0] as [number, number, number]
    );
    return [sum[0] / selectedNodes.length, sum[1] / selectedNodes.length, sum[2] / selectedNodes.length];
  }, [selectedNodes]);

  return {
    selectedIds, selectedNodes, centroid,
    select, selectAll, clearSelection, toggleSelect,
    applyDelta, applyAbsolute,
