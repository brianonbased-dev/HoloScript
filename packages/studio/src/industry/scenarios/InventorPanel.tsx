...
export function InventorPanel() {
  ...
  const buildDays = useMemo(
    () => estimateBuildTimeDays(complexity, components.length),
    [complexity, components.length]
  );
  const stressTest = useMemo(
    () => simulatePhysicsStressTest(components, maxLoad),
    [components, maxLoad]
  );
      ...
      <div style={s.section}>
        ...
        >
          ...
          <span style={{ fontWeight: 'bold', color: stressTest.passed ? '#34d399' : '#ef4444' }}>
            {stressTest.passed ? 'PASSED ✅' : 'FAILED ❌'} (Stress:{' '}
            {stressTest.stressFactor.toFixed(2)})
          </span>
        </div>
      </div>
    </div>
