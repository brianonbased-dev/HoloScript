...
export function AbsorbOrchestratorPanel() {
  const [tasks, setTasks] = useState<AbsorbTask[]>([
    {
      id: 'task-101',
      objective: 'Refactor Core Rendering',
      phase: 'idle',
    ...
    {
      id: 'task-102',
      objective: 'Synthesize RFI Filter rules',
      phase: 'graph_rag_query',
      knowledgeTokensProcessed: 2500,
      progressPercent: 20,
    },
    {
      id: 'task-103',
      objective: 'Deploy Swarm Protocol',
      phase: 'board_claim',
      knowledgeTokensProcessed: 5500,
      progressPercent: 60,
    },
  ]);
    ...
    <div style={s.panel} data-testid="absorb-orchestrator-panel">
      <div style={s.header}>
        ...
        <span
          style={{
            fontSize: 12,
            color: efficiency > 80 ? '#10b981' : '#f59e0b',
            fontWeight: 'bold',
          }}
        >
      ...
      <div style={s.box}>
        <div style={{ fontSize: 12, fontWeight: 'bold', color: '#d946ef', marginBottom: 12 }}>
          ACTIVE IDE SQUAD TASKS
        </div>
        {tasks.map((t) => (
          <div key={t.id} style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
              <span>{t.objective}</span>
              <span style={{ color: getPhaseColor(t.phase), fontWeight: 'bold' }}>
                {t.phase.toUpperCase()}
              </span>
            </div>
