...
const DEMO_CAPTURES: MotionCapture[] = [
  {
    ...
    fps: 240,
    peakForce: 2400,
    notes: 'Left leg push-off analysis',
    joints: [
    ...
    <div className="flex flex-col overflow-auto">
      ...
      {captures.map((c) => (
        <div
          ...
          className={`flex items-center gap-2 border-b border-studio-border/30 px-3 py-2 cursor-pointer ${selected === c.id ? 'bg-orange-500/10' : 'hover:bg-studio-panel/50'}`}
        >
          ...
          <div className="flex-1">
            ...
            <div className="text-[10px] text-studio-muted">
              {c.sport} · {c.fps}fps · {c.duration}s
            </div>
          </div>
        ...
        <>
          <div className="border-t border-studio-border px-3 py-2">
            ...
            {sel.joints.map((j) => (
              <div
                key={j.name}
                className="flex items-center gap-2 py-1.5 border-b border-studio-border/20"
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="h-1.5 rounded-full bg-studio-panel">
                    <div
                      className="h-1.5 rounded-full bg-orange-400"
                      style={{ width: `${(j.angle / j.rom[1]) * 100}%` }}
                    />
                  </div>
                </div>
                {/* Velocity */}
