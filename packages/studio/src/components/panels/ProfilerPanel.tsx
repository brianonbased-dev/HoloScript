    ...
    <div className="p-3 space-y-3 text-xs">
        ...
        <div>
          ...
          <div className="space-y-0.5">
            {slowest.map((s) => (
              <div
                key={s.name}
                className="flex items-center gap-1.5 bg-studio-panel/30 rounded px-2 py-0.5"
              >
                <span className="text-studio-text font-mono text-[10px] w-16 truncate">
                  {s.name}
                </span>
                <div className="flex-1 bg-studio-panel rounded-full h-1.5">
                  <div
                    className={`h-1.5 rounded-full ${s.maxTime > 5 ? 'bg-red-400' : s.maxTime > 2 ? 'bg-amber-400' : 'bg-emerald-400'}`}
                    style={{
                      width: `${Math.min(100, (s.avgTime / (slowest[0]?.maxTime || 1)) * 100)}%`,
                    }}
                  />
                </div>
                <span className="text-studio-muted text-[10px] font-mono w-14 text-right">
                  {s.avgTime.toFixed(2)}ms
                </span>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="space-y-0.5 max-h-[60px] overflow-y-auto">
            {memory.map((m, i) => (
              <div
                key={i}
                className="flex items-center justify-between bg-studio-panel/20 rounded px-2 py-0.5 text-[10px]"
              >
                <span className="text-studio-text">{m.label || `Snap ${i}`}</span>
                <span className="font-mono text-studio-muted">
                  {(m.heapUsed / 1024 / 1024).toFixed(1)}MB
                </span>
              </div>
            ))}
          </div>
      ...
      {summaries.length === 0 && (
        <p className="text-studio-muted text-center py-2">
          Simulate frames to collect profiling data.
        </p>
      )}
    </div>
  );
