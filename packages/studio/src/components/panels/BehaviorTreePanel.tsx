...
export function BehaviorTreePanel() {
  const { trees, trace, lastStatus, createTree, tick, tickAll, abort, remove, reset } = useBehaviorTree();
  ...
  return (
    <div className="p-3 space-y-3 text-xs">
      ...
      <div className="flex gap-1.5">
        <select value={treeType} onChange={e => setTreeType(e.target.value as 'patrol' | 'guard' | 'idle')}
        ...
        <button onClick={createDemo} className="px-2 py-1 bg-studio-accent/20 text-studio-accent rounded hover:bg-studio-accent/30 transition">+ Create</button>
        <button onClick={() => tickAll()} className="px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded hover:bg-emerald-500/30 transition">⟳ Tick All</button>
      ...
      <div className="space-y-1">
        ...
        {trees.map(t => (
          <div key={t.id} className="flex items-center justify-between bg-studio-panel/30 rounded px-2 py-1.5">
            <div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button onClick={() => tick(t.id)} className="text-studio-accent hover:opacity-80 transition" title="Tick">⟳</button>
              <button onClick={() => abort(t.id)} className="text-amber-400 hover:opacity-80 transition" title="Abort">⏹</button>
              <button onClick={() => remove(t.id)} className="text-red-400 hover:opacity-80 transition" title="Remove">✕</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {trace.length > 0 && (
        <div>
          ...
          <div className="space-y-0.5 max-h-[150px] overflow-y-auto font-mono text-[10px]">
            {trace.slice(-20).reverse().map((t, i) => (
              <div key={i} className="flex items-center gap-2 bg-studio-panel/20 rounded px-1.5 py-0.5">
