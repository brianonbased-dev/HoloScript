  ...
  return (
    <div className="p-3 space-y-2 text-xs">
      ...
      {view === 'stream' && (
        <>
          ...
          <div ref={scrollRef} className="space-y-0.5 max-h-[200px] overflow-y-auto font-mono">
            {filtered.length === 0 && (
              <p className="text-studio-muted text-center py-4">No events. Interact with panels.</p>
            )}
            {filtered.map((e, i) => (
              <div
                key={i}
                className="flex items-center gap-1.5 px-1.5 py-0.5 bg-studio-panel/20 rounded text-[9px]"
              >
                ...
                <span
                  ...
                  style={{
                    backgroundColor: `${CHANNEL_COLORS[e.channel] || '#666'}22`,
                    color: CHANNEL_COLORS[e.channel] || '#999',
                  }}
                >
                  {e.channel}
                </span>
      ...
      {view === 'log' && (
        <div
          ref={scrollRef}
          className="space-y-0.5 max-h-[280px] overflow-y-auto font-mono text-[9px]"
        >
          {filtered.length === 0 && (
            <p className="text-studio-muted text-center py-6">Waiting for events...</p>
          )}
          {filtered.map((e, i) => (
            <div key={i} className="flex gap-1.5 px-1 py-0.5 bg-studio-panel/15 rounded">
              ...
              <span className="text-studio-muted truncate">
                {typeof e.data === 'object' && e.data
                  ? `(${Object.keys(e.data as object).join(', ')})`
                  : ''}
              </span>
            </div>
          ))}
