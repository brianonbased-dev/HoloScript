  ...
  return (
    <div className="p-2 text-[11px] text-studio-text">
      <div className="uppercase tracking-wider text-[10px] text-studio-muted mb-2 flex justify-between">
        ...
        <button
          onClick={() => alert('New absorb project flow (opens workspace import)')}
        ...
        {projects.map((p) => (
          <div key={p.id} className="border border-studio-border/40 rounded p-2 bg-studio-panel/20">
            ...
            <div className="text-[9px] text-studio-muted">
              {p.repo} • {p.entry_count?.toLocaleString() || '?'} entries
            </div>
            ...
            <div className="mt-1 flex gap-2 text-[9px]">
              <button
                onClick={() => runAbsorb(p.id)}
                className="underline hover:text-studio-accent"
              >
                Re-absorb
              </button>
              <button
                onClick={() => alert(`Query ${p.name} (opens absorb query UI)`)}
                className="underline hover:text-studio-accent"
              >
                Query
              </button>
              <button
                onClick={() => alert('Diff view (what changed since last absorb)')}
                className="underline hover:text-studio-accent"
              >
                Diff
              </button>
      ...
      <div className="mt-2 text-[8px] text-studio-muted">
        GET/POST /api/absorb/projects • live progress via SSE or polling
      </div>
    </div>
  );
}
