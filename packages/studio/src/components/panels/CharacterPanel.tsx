...
const CHARACTER_PRESETS: CharacterPreset[] = [
  {
    ...
    name: 'Hero',
    icon: '🦸',
    body: { height: 1.8, build: 'athletic', skinTone: '#c68642' },
    outfit: 'armor',
    tags: ['game', 'action'],
  },
  {
    ...
    name: 'Merchant',
    icon: '🧑‍💼',
    body: { height: 1.7, build: 'average', skinTone: '#f5d0a9' },
    outfit: 'robes',
    tags: ['game', 'rpg'],
  },
  {
    ...
    name: 'Robot',
    icon: '🤖',
    body: { height: 2.0, build: 'heavy', skinTone: '#888888' },
    outfit: 'plating',
    tags: ['sci-fi', 'iot'],
  },
  {
    ...
    name: 'Avatar',
    icon: '👤',
    body: { height: 1.75, build: 'average', skinTone: '#e0ac69' },
    outfit: 'casual',
    tags: ['vr', 'social'],
  },
  {
    ...
    name: 'Creature',
    icon: '🐉',
    body: { height: 3.0, build: 'heavy', skinTone: '#2d6a4f' },
    outfit: 'scales',
    tags: ['game', 'fantasy'],
  },
  {
    ...
    name: 'Brittney',
    icon: '✨',
    body: { height: 1.7, build: 'slim', skinTone: '#f0c8a0' },
    outfit: 'holographic',
    tags: ['ai', 'agent'],
  },
];
...
export function CharacterPanel() {
  ...
  const updateChar = useCallback(
    (updates: Partial<ActiveCharacter>) => {
      setActive((prev) => {
        if (!prev) return prev;
  ...
  const processCommand = useCallback(
    (text: string) => {
      ...
      if (lower.includes('tall')) {
        updateChar({ height: (active?.height ?? 1.8) + 0.1 });
        response = '📏 Made character taller!';
      } else if (lower.includes('short')) {
        updateChar({ height: (active?.height ?? 1.8) - 0.1 });
        response = '📏 Made character shorter!';
      } else if (lower.includes('slim')) {
        updateChar({ build: 'slim' });
        response = '🏋️ Set build to slim!';
      } else if (lower.includes('athletic')) {
        updateChar({ build: 'athletic' });
        response = '🏋️ Set build to athletic!';
      } else if (lower.includes('heavy')) {
        updateChar({ build: 'heavy' });
        response = '🏋️ Set build to heavy!';
      } else if (lower.includes('smile')) {
        updateChar({ expression: 'happy' });
        response = '😊 Character is smiling!';
      } else if (lower.includes('angry')) {
        updateChar({ expression: 'angry' });
        response = '😠 Character looks angry!';
      }
    ...
    <div className="p-3 space-y-3 text-xs">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-studio-muted">
          {active ? active.preset.name : 'No character'}
        </span>
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="bg-studio-panel/30 rounded-lg p-2 space-y-1.5">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-studio-text font-mono text-[10px] w-8">
                {active.height.toFixed(1)}m
              </span>
            </div>
          ...
          <div className="flex gap-1">
            {['neutral', 'happy', 'angry', 'surprised', 'sad'].map((expr) => (
              <button
                key={expr}
                onClick={() => updateChar({ expression: expr })}
      ...
      <div className="space-y-1">
        ...
        <div className="flex gap-1">
          <input
            type="text"
            placeholder="e.g. make taller, smile..."
            value={command}
            onChange={(e) => setCommand(e.target.value)}
            onKeyDown={(e) => {
              if (e.key === 'Enter' && command.trim()) processCommand(command.trim());
            }}
          ...
          />
          <button
            onClick={() => command.trim() && processCommand(command.trim())}
            className="px-2 py-1 bg-studio-accent/20 text-studio-accent rounded text-[10px]"
          >
            Send
          </button>
        </div>
        {chatLog.length > 0 && (
          <div className="max-h-[60px] overflow-y-auto space-y-0.5">
            {chatLog.slice(-4).map((m, i) => (
              <div
                key={i}
                className={`text-[9px] px-1.5 py-0.5 rounded ${m.role === 'user' ? 'text-studio-muted' : 'text-studio-accent bg-studio-accent/5'}`}
              >
                {m.role === 'user' ? '> ' : '✨ '}
                {m.text}
              </div>
