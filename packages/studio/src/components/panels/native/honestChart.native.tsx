...
export function HonestChartComponent() {
  const [readings, setReadings] = useState([
    { label: 'T1', value: 42, prov: 'measured' },
    { label: 'T2', value: 38, prov: 'measured' },
    { label: 'T3', value: 45, prov: 'measured' },
    { label: 'fcast', value: 58, prov: 'inferred' },
    { label: 'AI', value: 63, prov: 'generative' },
  ]);

  return (
    <div
      className="holoscript-2d-root w-full h-full"
      data-holo-view-contract="3118f0ae258ac47faf006974833fc9513573dfa2ef19d733269d4e6599b0e3a6"
    >
      <div
      ...
      >
        ...
        <svg
          data-holo-projects="readings"
          className="w-full"
          data-baseline="zero"
          data-clamped={String(
            ((__a) => (__a ?? []).some((d) => (Number(d?.value) || 0) < 0))(readings)
          )}
          viewBox="0 0 280 120"
        >
          <defs>
            ...
            >
              <line
                ...
                x2="0"
                y2="4"
                className="stroke-studio-accent"
                strokeWidth="1.2"
              />
            </pattern>
            <pattern id="holo-dots" width="3" height="3" patternUnits="userSpaceOnUse">
              ...
              return (
                <g key={i}>
                  <rect
                    ...
                    data-provenance-class={String(d?.prov ?? '')}
                    rx="0.5"
                  />
                  <text
                    x={__x + __bw / 2}
        ...
        <svg
          data-holo-provenance='{"source":"sensorBus","class":"measured"}'
        ...
        >
          ...
          <polyline
            ...
            strokeWidth="1.5"
            points={((__a) => {
              const __v = (__a ?? []).map((d) => Number(d?.value) || 0);
              if (!__v.length) return '';
              const __mn = 0,
                __r = Math.max(1, ...__v),
                __sx = __v.length > 1 ? 268 / (__v.length - 1) : 0;
              return __v
                .map(
                  (y, i) =>
                    (6 + i * __sx).toFixed(2) +
                    ',' +
                    (74 - ((Math.max(0, y) - __mn) / __r) * 66).toFixed(2)
                )
                .join(' ');
            })(readings)}
          />
...
export const holoViewContract = {
  ...
  projections: [
    { element: 'Chart', node: 'readings', identity: false },
    { element: 'Trend', node: 'readings', identity: false },
  ],
  stateRoots: ['readings'],
  contractHash: '3118f0ae258ac47faf006974833fc9513573dfa2ef19d733269d4e6599b0e3a6',
} as const;
