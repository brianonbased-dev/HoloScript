...
const SAMPLE_ROWS: EvidenceRow[] = [
  {
    segment: 'scene_loaded',
    surface: 'world-model-pixel-replay',
    status: 'partial',
    note: 'Static scene load is a direct screenshot plus replay context, not a replay-driven pose transition.',
  },
  {
    segment: 'all',
    surface: 'target-device-webxr-quest',
    status: 'blocked',
    note: 'No Quest/WebXR frame receipt was captured in this local run.',
  },
];
...
export function ReplayEvidenceNavigator() {
  ...
  const pngPath =
    '/bench-logs/format-stress/2026-05-18_codex-realism-gap-pass/novel/replay-evidence-room.png';
    ...
    <div className="p-2 text-[11px] text-studio-text">
      ...
      <div className="border border-studio-border/40 rounded p-2 mb-2 bg-black/20">
        ...
        <div className="grid grid-cols-2 gap-x-3 text-[10px]">
          <div>
            coverage: <span className="text-emerald-400">{state.coverage}</span>
          </div>
          <div>
            replayPixels: <span className="text-amber-400">{state.replayPixels}</span>
          </div>
          <div>
            targetDeviceReceipt: <span className="text-red-400">{state.targetDeviceReceipt}</span>
          </div>
          <div>
            status: <span className="text-amber-400">{state.status}</span>
          </div>
        </div>
      </div>
          ...
          {rows.map((r, i) => (
            <div key={i} className="border border-studio-border/40 rounded p-1.5 text-[10px]">
              ...
              <div className="mt-1 flex gap-2 text-[9px]">
                <button
                  onClick={() => openSegmentReceipts(r.segment)}
                ...
                {r.status === 'blocked' && (
                  <button
                    onClick={() => captureTargetDeviceFrameReceipt('quest')}
      ...
      <div className="flex gap-2 mb-2">
        <button onClick={showReplayGap} className="text-[9px] underline text-amber-400">
          Show gap (target-device P2)
        </button>
        <button
          onClick={() => captureTargetDeviceFrameReceipt('quest')}
          className="text-[9px] underline text-emerald-400"
        >
          Request capture
        </button>
      </div>
      ...
      <div className="mt-2 text-[8px] text-studio-muted">
        Source: replay-evidence-room.hs/.hsplus + sample.json + PNG. Makes format-stress receipts
        first-class for Quest/WebXR proof work.
      </div>
    </div>
  );
}
