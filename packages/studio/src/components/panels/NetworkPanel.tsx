  ...
  return (
    <div className="p-3 space-y-3 text-xs">
      <div className="flex items-center justify-between">
        ...
        <span
          className={`text-[10px] font-medium ${connected ? 'text-emerald-400' : 'text-red-400'}`}
        >
      ...
      <div className="space-y-0.5 max-h-[80px] overflow-y-auto">
        {peers.length === 0 && (
          <p className="text-studio-muted text-center py-1">No peers connected.</p>
        )}
        {peers.map((p) => (
          <div
            key={p.id}
            className="flex items-center justify-between bg-studio-panel/30 rounded px-2 py-0.5"
          >
            <span className="text-studio-text text-[10px]">
              {p.displayName} <span className="text-studio-muted">({p.id})</span>
            </span>
            <div className="flex items-center gap-1">
              <span className="text-studio-muted text-[10px] font-mono">{p.latency}ms</span>
              <button onClick={() => removePeer(p.id)} className="text-red-400 text-[10px]">
                ✕
              </button>
            </div>
          </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-[10px] bg-studio-panel/30 rounded-lg p-2">
        <div>
          <span className="text-studio-muted">You</span>
          <br />
          <span className="text-studio-text font-mono">{peerId}</span>
        </div>
        <div>
          <span className="text-studio-muted">Peers</span>
          <br />
          <span className="text-studio-text font-mono">{peers.length}</span>
        </div>
        <div>
          <span className="text-studio-muted">Msgs</span>
          <br />
          <span className="text-studio-text font-mono">{messageCount}</span>
        </div>
      </div>
    </div>
  );
