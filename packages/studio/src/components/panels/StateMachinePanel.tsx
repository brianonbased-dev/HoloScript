    ...
    <div className="p-3 space-y-3 text-xs">
      ...
      <div className="flex gap-1 flex-wrap">
        {states.map((s) => (
          <span
            key={s.id}
            className={`px-2 py-0.5 rounded text-[10px] font-mono ${s.id === currentState ? 'bg-studio-accent text-black font-bold' : 'bg-studio-panel/30 text-studio-muted'}`}
          >
            {s.id === currentState && '▶ '}
            {s.id}
          </span>
      ...
      <div className="flex gap-1 flex-wrap">
        {EVENT_PRESETS.map((e) => (
          <button
            key={e}
            onClick={() => send(e)}
            className="px-1.5 py-0.5 bg-studio-panel/40 text-studio-muted rounded text-[10px] hover:text-studio-text hover:bg-studio-accent/20 transition font-mono"
          >
            {e}
          </button>
        ))}
      ...
      {history.length > 0 && (
        <div className="bg-black/30 rounded-lg p-2 font-mono text-[10px] max-h-[60px] overflow-y-auto">
          {history.map((h, i) => (
            <span
              key={i}
              className={`${i === history.length - 1 ? 'text-studio-accent' : 'text-studio-muted'}`}
            >
              {i > 0 && ' → '}
              {h}
            </span>
