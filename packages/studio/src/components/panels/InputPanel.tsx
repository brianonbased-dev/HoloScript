...
export function InputPanel() {
  const { keys, mousePos, gamepads, actions, pressKey, releaseKey, buildDemo, reset } = useInputManager();

  return (
    <div className="p-3 space-y-3 text-xs">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-studio-text">🎮 Input</h3>
        <span className="text-[10px] text-studio-muted">{keys.length} keys · {gamepads} pads</span>
      </div>
      ...
      <div className="grid grid-cols-4 gap-1">
        {KEY_PRESETS.map(k => (
          <button key={k}
            onMouseDown={() => pressKey(k)} onMouseUp={() => releaseKey(k)}
      ...
      <div className="flex items-center gap-2 text-[10px] bg-studio-panel/30 rounded px-2 py-1">
        ...
        <span className="text-studio-text font-mono">X:{mousePos.x} Y:{mousePos.y}</span>
      ...
      {actions.length > 0 && (
        <div className="flex gap-1 flex-wrap">
          {actions.map(a => (
            <span key={a} className="px-1.5 py-0.5 bg-emerald-500/20 text-emerald-400 rounded text-[10px] font-mono">{a}</span>
          ))}
        </div>
      )}
    </div>
