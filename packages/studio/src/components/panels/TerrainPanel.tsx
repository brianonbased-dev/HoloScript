  ...
  return (
    <div className="p-3 space-y-3 text-xs">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-studio-muted">{terrainId ? `${resolution}×${resolution}` : 'No terrain'}</span>
      </div>
      ...
      {heights.length > 0 && (
        <div className="bg-studio-panel/30 rounded-lg p-1">
          <div className="grid gap-px" style={{ gridTemplateColumns: `repeat(${resolution}, 1fr)` }}>
          ...
          <p className="text-[10px] text-studio-muted mt-1 text-center">Click cells to raise terrain</p>
        </div>
      )}
