...
export function TraitInspector({ initialTrait, showCulture = true }: TraitInspectorProps) {
  ...
  const filteredTraits = traits.filter((t) => t.name.toLowerCase().includes(filter.toLowerCase()));
    ...
    <div style={styles.container}>
      ...
      {showCulture && (
        <div style={styles.tabs}>
          <button
            style={tab === 'traits' ? styles.tabActive : styles.tab}
            onClick={() => setTab('traits')}
          >
            Traits ({traits.length})
          </button>
          <button
            style={tab === 'norms' ? styles.tabActive : styles.tab}
        ...
        <>
            ...
            <div style={styles.cultureSection}>
              ...
              {cultureTraits.map((t) => (
                <TraitRow
                  key={t.name}
                  trait={t}
                  selected={selectedTrait?.name === t.name}
          ...
          {selectedTrait && (
            <div style={styles.detail}>
              ...
              <div
                style={{ ...styles.detailDanger, color: DANGER_COLORS[selectedTrait.dangerLevel] }}
              >
                {selectedTrait.dangerLevel.toUpperCase()}
              </div>
              <div style={styles.detailEffects}>
                {selectedTrait.effects.length > 0 ? (
                  selectedTrait.effects.map((e) => (
                    <span key={e} style={styles.effectTag}>
                      {e}
                    </span>
                  ))
                ) : (
...
function TraitRow({
  ...
  return (
    <div
      style={{
        ...styles.traitRow,
        background: selected ? '#3030aa30' : 'transparent',
        borderColor: selected ? '#4040aa' : 'transparent',
      }}
...
function NormCard({
  ...
  norm: CulturalNorm;
  population: number;
  criticalMass: (id: string, pop: number) => number;
}) {
  ...
  return (
    <div style={styles.normCard}>
      <div style={styles.normHeader}>
        ...
        <span
          style={{
            ...styles.normEnforcement,
            color:
              norm.enforcement === 'hard'
                ? '#ef4444'
                : norm.enforcement === 'soft'
                  ? '#f59e0b'
                  : '#10b981',
          }}
        >
      ...
      <div style={styles.normMeta}>
        ...
        <span>
          Critical mass: {mass}/{population} agents
        </span>
      </div>
      {norm.forbiddenEffects && (
        <div style={styles.normForbidden}>Forbidden: {norm.forbiddenEffects.join(', ')}</div>
      )}
    </div>
  );
}
