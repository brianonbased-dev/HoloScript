...
export function MarketplacePanel({ worldId = 'default', category }: MarketplacePanelProps) {
  const { results, installed, stats, search, install, uninstall, selected, select } = useMarketplace(worldId);
        ...
        <>
          ...
          <div style={styles.results}>
            {results?.listings.map(listing => {
              ...
              return (
                <div key={listing.metadata.id} style={styles.card} onClick={() => select(listing.metadata.id)}>
                  <div style={styles.cardHeader}>
                    <span style={styles.cardName}>{listing.metadata.name}</span>
                    <span style={styles.cardVerdict}>
                      {listing.safetyReport.verdict === 'safe' ? '✅' : listing.safetyReport.verdict === 'warnings' ? '⚠️' : '🛑'}
                    </span>
                  </div>
                  ...
                  <div style={styles.cardTags}>
                    {listing.metadata.tags.slice(0, 3).map(t => (
                      <span key={t} style={styles.cardTag}>{t}</span>
                    ))}
                  </div>
                  <button
                    style={isInstalled ? styles.btnUninstall : styles.btnInstall}
                    onClick={e => { e.stopPropagation(); isInstalled ? uninstall(listing.metadata.id, worldId) : install(listing.metadata.id, worldId); }}
                  >
        ...
        <div style={styles.results}>
          {installed.map(manifest => (
            <div key={manifest.packageId} style={styles.card}>
              ...
              <div style={styles.cardMeta}>
                <span>v{manifest.version.major}.{manifest.version.minor}.{manifest.version.patch}</span>
                <span>Danger: {manifest.dangerScore}/10</span>
              </div>
              <button style={styles.btnUninstall} onClick={() => uninstall(manifest.packageId, worldId)}>
                Uninstall
              </button>
            </div>
          ))}
          {installed.length === 0 && (
            <div style={styles.empty}>No packages installed</div>
          )}
        </div>
      )}
    </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  header: { fontWeight: 700, fontSize: 14, marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  ...
  results: { display: 'flex', flexDirection: 'column' as const, gap: 8, maxHeight: 400, overflowY: 'auto' as const },
  card: { padding: 10, background: '#1e1e3a', borderRadius: 6, border: '1px solid #2a2a5a', cursor: 'pointer' },
  ...
  cardTag: { padding: '1px 6px', background: '#2a2a5a', borderRadius: 3, fontSize: 10, color: '#a0a0ff' },
  btnInstall: { padding: '4px 12px', background: '#10b981', border: 'none', borderRadius: 4, color: '#fff', cursor: 'pointer', fontSize: 11, fontWeight: 600 },
  btnUninstall: { padding: '4px 12px', background: '#ef4444', border: 'none', borderRadius: 4, color: '#fff', cursor: 'pointer', fontSize: 11, fontWeight: 600 },
