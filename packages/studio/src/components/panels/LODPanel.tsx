  ...
  return (
    <div className="p-3 space-y-3 text-xs">
      ...
      <div className="space-y-1 max-h-[120px] overflow-y-auto">
        {objects.length === 0 && <p className="text-studio-muted text-center py-2">Load demo to see LOD objects.</p>}
        {objects.map(o => (
          <div key={o.id} className="flex items-center justify-between bg-studio-panel/30 rounded px-2 py-1">
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: LOD_COLORS[Math.min(o.level, LOD_COLORS.length - 1)] }} />
            ...
            <div className="flex items-center gap-2">
              ...
              {o.transitioning && <span className="text-amber-400 text-[10px] animate-pulse">⟳</span>}
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-2 text-[10px] text-studio-muted">
        {LOD_COLORS.map((c, i) => (
          <span key={i} className="flex items-center gap-0.5">
            <span className="w-2 h-2 rounded-full inline-block" style={{ backgroundColor: c }} /> L{i}
          </span>
        ))}
      </div>
    </div>
