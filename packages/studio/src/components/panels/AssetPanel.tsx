    ...
    <div className="p-3 space-y-3 text-xs">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-studio-muted">
          {assets.length} items · {formatSize(cacheSize)}
        </span>
      </div>
      ...
      <input
        ...
        className="w-full bg-studio-panel/30 text-studio-text rounded px-2 py-1 text-[10px] outline-none focus:ring-1 ring-studio-accent/40"
      />

      {/* Asset list */}
      <div className="space-y-0.5 max-h-[140px] overflow-y-auto">
        {assets.length === 0 && (
          <p className="text-studio-muted text-center py-2">Load demo to browse assets.</p>
        )}
        {assets.map((a) => (
          <div
            key={a.id}
          ...
          >
            ...
            <div className="flex items-center gap-1 text-[10px]">
              ...
              {a.tags.slice(0, 2).map((t) => (
                <span key={t} className="text-studio-accent/60 bg-studio-accent/10 px-1 rounded">
                  {t}
                </span>
              ))}
            </div>
