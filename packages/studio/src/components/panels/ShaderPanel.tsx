...
const NODE_COLORS: Record<string, string> = {
  Color: 'bg-pink-500/20 text-pink-400', Texture: 'bg-cyan-500/20 text-cyan-400',
  Noise: 'bg-emerald-500/20 text-emerald-400', Mix: 'bg-amber-500/20 text-amber-400',
  Normal: 'bg-violet-500/20 text-violet-400', Fresnel: 'bg-blue-500/20 text-blue-400',
  Math: 'bg-slate-500/20 text-slate-400', Output: 'bg-red-500/20 text-red-400',
};

export function ShaderPanel() {
  const { nodes, connections, compiled, nodeTypes, addNode, removeNode, compile, buildDemo, clear } = useShaderGraph();

  return (
    <div className="p-3 space-y-3 text-xs">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-studio-text">✨ Shader Graph</h3>
        <span className="text-[10px] text-studio-muted">{nodes.length} nodes · {connections.length} links</span>
      </div>
      ...
      <div className="flex gap-1.5 flex-wrap">
        ...
        <button onClick={compile} className="px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded hover:bg-emerald-500/30 transition">⚡ Compile</button>
      ...
      <div className="space-y-1 max-h-[120px] overflow-y-auto">
        {nodes.map(n => (
          <div key={n.id} className={`flex items-center justify-between rounded px-2 py-1 ${NODE_COLORS[n.type] || 'bg-studio-panel/30'}`}>
            <span className="font-mono text-[10px]">{n.type} <span className="text-studio-muted">#{n.id}</span></span>
            <button onClick={() => removeNode(n.id)} className="text-red-400 text-[10px]">✕</button>
          </div>
        ))}
      </div>
      ...
      {compiled && (
        <div className="bg-studio-panel/50 rounded-lg p-2 space-y-1">
          ...
          <div className="text-[10px] text-studio-text font-mono bg-black/30 rounded p-1.5 max-h-[80px] overflow-auto whitespace-pre">
            {compiled.fragmentCode.slice(0, 300)}{compiled.fragmentCode.length > 300 ? '...' : ''}
          </div>
