    ...
    <div className="p-3 space-y-3 text-xs">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-studio-muted">
          {Object.keys(state).length} keys · {changes} changes
        </span>
      </div>
      ...
      <div className="space-y-0.5 max-h-[100px] overflow-y-auto">
        {Object.keys(state).length === 0 && (
          <p className="text-studio-muted text-center py-2">Load demo or add keys below.</p>
        )}
      ...
      <div className="flex gap-1">
        <input
          ...
          placeholder="key"
          className="flex-1 bg-studio-panel/30 text-studio-text rounded px-1.5 py-0.5 text-[10px] outline-none"
        />
        <input
          type="text"
          value={newVal}
        ...
        <button
          onClick={handleAdd}
          className="px-2 py-0.5 bg-emerald-500/20 text-emerald-400 rounded hover:bg-emerald-500/30 transition text-[10px]"
        >
          +
        </button>
      </div>
