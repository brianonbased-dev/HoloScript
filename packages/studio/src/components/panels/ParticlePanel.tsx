...
export function ParticlePanel() {
  const { activeCount, isEmitting, presetNames, particles, loadPreset, toggleEmitting, burst, step, reset } = useParticles();

  return (
    <div className="p-3 space-y-3 text-xs">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-studio-text">✨ Particles</h3>
        <span className={`text-[10px] font-medium ${isEmitting ? 'text-emerald-400' : 'text-studio-muted'}`}>
      ...
      <div className="flex gap-1.5 flex-wrap">
        <button onClick={toggleEmitting} className={`px-2 py-1 rounded transition ${isEmitting ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
          {isEmitting ? '⏸ Pause' : '▶ Emit'}
        </button>
        <button onClick={() => burst(30)} className="px-2 py-1 bg-studio-accent/20 text-studio-accent rounded hover:bg-studio-accent/30 transition">💥 Burst 30</button>
          ...
          {particles.slice(0, 80).map((p, i) => (
            <div key={i} className="absolute rounded-full"
              style={{
                ...
                width: Math.max(2, p.size * 8), height: Math.max(2, p.size * 8),
                backgroundColor: `rgba(${Math.round(p.color.r * 255)}, ${Math.round(p.color.g * 255)}, ${Math.round(p.color.b * 255)}, ${p.color.a})`,
              }} />
          ))}
        </div>
        {particles.length === 0 && <span className="absolute inset-0 flex items-center justify-center text-studio-muted text-[10px]">Select a preset & step</span>}
      </div>
