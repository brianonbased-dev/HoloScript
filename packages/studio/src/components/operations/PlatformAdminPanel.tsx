...
type AdminTab = 'overview' | 'users' | 'projects' | 'agents' | 'health';

/**
 * PlatformAdminPanel — content extracted from /admin/page.tsx.
 * Mounted inside the Operations console under the "Admin" tab.
 * Carries its own fetch logic (/api/admin/*); the founder gate is enforced
 * by the /operations page wrapper — this component does not re-gate.
 */
export function PlatformAdminPanel() {
      ...
      try {
        switch (t) {
          ...
          case 'projects': {
            ...
            setProjects((data as { projects: AdminProject[] }).projects);
            break;
          }
          case 'agents': {
            const data = await fetchData('agents?limit=100');
            setAgents((data as { agents: AdminAgent[] }).agents);
            break;
          }
          case 'health': {
            const data = await fetchData('health-matrix');
            setHealth((data as { matrix: HealthNode[] }).matrix);
            break;
          }
        }
      } catch (err) {
  ...
  const forceStop = async (agentId: string, agentName: string) => {
    ...
    try {
      ...
      logger.warn('[PlatformAdminPanel] force-stop agent failed:', err);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[#050510] text-[#e4e4e7]">
      {/* Sub-tab Bar */}
      <div className="border-b border-[#2a2a3e] bg-[#0d0d14] px-4 py-2 flex items-center justify-between">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {loading && tab === 'overview' && !stats && !error && (
