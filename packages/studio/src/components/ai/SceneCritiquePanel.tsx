...
import { Lightbulb, Loader2, RefreshCw, X, Send, ArrowRight, AlertTriangle, Info, Wrench } from 'lucide-react';
  ...
  return (
    <div className="flex h-full flex-col bg-studio-panel text-studio-text">
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-studio-border px-3 py-2.5">
        ...
        {isStale && <span className="rounded-full bg-yellow-600/20 px-1.5 py-0.5 text-[7px] text-yellow-400">stale</span>}
        ...
        {loading && (
          <div className="flex flex-col items-center gap-3 py-8 text-center">
            ...
            <div className="flex gap-1">
              {[0, 150, 300].map((d) => (
                <div key={d} className="h-1.5 w-1.5 animate-bounce rounded-full bg-studio-accent"
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex items-center gap-3 rounded-xl border border-studio-border bg-studio-surface p-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-full"
                style={{ backgroundColor: result.score >= 80 ? '#16a34a22' : result.score >= 60 ? '#d9770622' : '#dc262622' }}>
                <span className="text-base font-bold"
                  style={{ color: result.score >= 80 ? '#4ade80' : result.score >= 60 ? '#fb923c' : '#f87171' }}>
                  {result.grade}
                </span>
              </div>
              <div>
                ...
                <p className="text-[8px] text-studio-muted">
                  Score {result.score}/100 · {result.objectCount} objects · {Math.round(result.traitCoverage * 100)}% trait coverage
                </p>
              </div>
            </div>
            ...
            {result.findings.map((finding, i) => {
              const Icon = finding.severity === 'error' ? AlertTriangle
                : finding.severity === 'warning' ? Wrench : Info;
              const col = finding.severity === 'error' ? '#f87171'
                : finding.severity === 'warning' ? '#fb923c' : '#60a5fa';
              return (
                <div key={i}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {finding.suggestion && (
                      <p className="mt-0.5 text-[8px] text-studio-muted italic">{finding.suggestion}</p>
                    )}
                  </div>
                  {onSendToBrittney && finding.suggestion && (
                    <button onClick={() => onSendToBrittney(finding.suggestion!)}
