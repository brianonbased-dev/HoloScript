...
const CATEGORY_COLORS: Record<string, string> = {
  outdoor: '#ffaa44', indoor: '#88ccff', space: '#aa66ff',
  fantasy: '#44ffaa', abstract: '#ff44aa',
};

const SKY_ICON: Record<string, string> = {
  procedural: '🌤', hdri: '🌐', solid: '🎨',
};

interface EnvironmentPanelProps { onClose: () => void; }
...
export function EnvironmentPanel({ onClose }: EnvironmentPanelProps) {
  ...
  const apply = () => { if (selected) applyPreset(selected.traitSnippet); };
  ...
  return (
    <div className="flex h-full flex-col bg-studio-panel text-studio-text">
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-studio-border px-3 py-2.5">
        ...
        {hasEnvironment && (
          <span className="rounded-full bg-green-600/20 px-1.5 py-0.5 text-[7px] text-green-400">Active</span>
        )}
      ...
      {hasEnvironment && (
        <div className="shrink-0 border-b border-studio-border bg-green-950/30 px-3 py-2 flex items-center gap-2">
          ...
          <button onClick={removeEnvironment}
      ...
      <div className="flex-1 overflow-y-auto grid grid-cols-2 gap-1.5 p-2">
        {presets.map((p) => (
          <button key={p.id} onClick={() => setSelected(p)}
            ...
            <p className="text-[7px] text-studio-muted leading-snug line-clamp-2">{p.description}</p>
            <div className="flex items-center gap-1 mt-0.5">
              <span className="text-[8px]">{SKY_ICON[p.sky.type]}</span>
              {p.fog.enabled && <span className="text-[7px] text-studio-muted">fog</span>}
              <div className="ml-auto h-2.5 w-2.5 rounded-full border border-white/10"
      ...
      {selected && (
        <div className="shrink-0 border-t border-studio-border space-y-2 p-2.5">
          ...
          <div className="flex flex-wrap gap-1.5">
            <span className="rounded-full border border-studio-border px-2 py-0.5 text-[7px]"
              style={{ borderColor: CATEGORY_COLORS[selected.category] ?? '#888', color: CATEGORY_COLORS[selected.category] ?? '#888' }}>
              {selected.category}
            </span>
            ...
            <span className="flex items-center gap-1 rounded-full border border-studio-border px-2 py-0.5 text-[7px] text-studio-muted">
              <div className="h-2 w-2 rounded-full" style={{ backgroundColor: selected.ambient.color }} />
