...
export const CodebaseVisualization3D: React.FC<CodebaseVisualization3DProps> = ({
  ...
  return (
    <div
      className="flex flex-col h-full bg-[#050508] text-slate-200 select-none relative"
      data-testid="codebase-visualization-3d"
    >
        ...
        {communityIds.map((cId) => (
          <div key={cId} className="flex items-center gap-1 text-[10px] uppercase text-slate-400">
            <span
              ...
              style={{
                backgroundColor: getCommunityColor(cId),
                boxShadow: `0 0 8px ${getCommunityColor(cId)}`,
              }}
            />
            {cId}
          </div>
