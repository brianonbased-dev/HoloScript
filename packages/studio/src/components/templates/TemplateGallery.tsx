...
const CATEGORY_EMOJI: Record<string, string> = {
  environment: '🌿', 'sci-fi': '🚀', fantasy: '🏰', minimal: '⬜', game: '🎮', architecture: '🏛️',
};

interface TemplateGalleryProps { onClose: () => void; }
...
export function TemplateGallery({ onClose }: TemplateGalleryProps) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/templates?category=${encodeURIComponent(activeCategory)}&q=${encodeURIComponent(q)}`)
      .then((r) => r.json())
  ...
  return (
    <div className="flex h-full flex-col bg-studio-panel text-studio-text">
      ...
      <div className="flex-1 overflow-y-auto p-2.5 space-y-2">
        {loading && <p className="py-8 text-center text-[10px] text-studio-muted animate-pulse">Loading templates…</p>}
            ...
            <div className="p-2.5">
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="mt-0.5 text-[9px] text-studio-muted line-clamp-2">{t.description}</p>
                </div>
                <span className={`shrink-0 text-[9px] font-medium ${COMPLEXITY_COLOR[t.complexity] ?? 'text-studio-muted'}`}>
                  {t.complexity}
                </span>
              </div>
              <div className="mt-1.5 flex items-center gap-2">
                <div className="flex items-center gap-1 text-[9px] text-studio-muted">
                  <Layers className="h-2.5 w-2.5" />{t.objectCount} objects
                </div>
                <div className="ml-auto flex gap-1 flex-wrap">
                  {t.tags.slice(0, 3).map((tag) => (
                    <span key={tag} className="rounded bg-studio-border/60 px-1 py-0.5 text-[8px] text-studio-muted">{tag}</span>
                  ))}
                </div>
              </div>
              <button onClick={() => apply(t)}
                ...
                {applied === t.id ? '✅ Applied!' : <><ChevronRight className="h-3 w-3" /> Use Template</>}
              </button>
            </div>
          </div>
        ))}
