...
function SnapshotCard({ snap, onRestore, onDelete }: { snap: Snapshot; onRestore: () => void; onDelete: () => void }) {
  return (
    <div className="overflow-hidden rounded-xl border border-studio-border bg-studio-surface">
      ...
      <div className="relative aspect-video w-full overflow-hidden bg-black/40">
        {snap.dataUrl && snap.dataUrl.length > 100 ? (
          ...
          <div className="flex h-full items-center justify-center text-[10px] text-studio-muted">No preview</div>
        )}
      </div>
      {/* Info + actions */}
      <div className="p-2">
...
export function SnapshotGallery({ onClose, sceneId = 'default' }: SnapshotGalleryProps) {
  ...
  const { snapshots, loading, capturing, error, load, capture, restore, remove } = useSnapshots(sceneId);
    ...
    <div className="flex h-full flex-col bg-studio-panel text-studio-text">
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-studio-border px-3 py-2.5">
        ...
        <div className="ml-auto flex items-center gap-1">
          <button onClick={load} disabled={loading} title="Refresh" className="rounded p-1 text-studio-muted hover:text-studio-text">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        {snapshots.length === 0 && !loading && (
          <p className="py-8 text-center text-[10px] text-studio-muted">
            No snapshots yet.<br />Click <strong>Snap</strong> to capture the current viewport.
          </p>
        )}
