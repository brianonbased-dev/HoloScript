  ...
  useEffect(() => {
    ...
    fetch('/api/admin/deprecated')
      ...
      .catch(() => {
        ...
        setInventory([
          {
            symbolName: 'OldNetworkStrategy',
            filePath: 'core/src/network/_deprecated/v1.ts',
            classification: 'DEAD',
            importerCount: 0,
          },
          {
            symbolName: 'HoloScriptTraitAnnotationParser',
            filePath: 'core/src/parser/HoloScriptPlusParser.ts',
    ...
    <div className="deprecated-dashboard p-6 bg-slate-900 border border-slate-700 text-white rounded-xl shadow-2xl">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-2">
          ...
          <span className="bg-orange-900 px-3 py-1 rounded text-orange-200">
          ...
          <tbody className="divide-y divide-slate-700">
            {inventory.map((item, idx) => (
              <tr key={idx} className="hover:bg-slate-700/50 transition-colors">
                ...
                <td className="px-6 py-4 text-sm text-slate-400 truncate max-w-sm">
                  {item.filePath}
                </td>
      ...
      <div className="p-4 bg-slate-800 rounded border border-slate-700 text-sm">
        ...
        <p className="text-slate-400">
          Purging objects automatically rewrites the parent <code>index.ts</code> exports tree.
        </p>
      </div>
    </div>
  );
