...
interface AudioTraitPanelProps { onClose: () => void; }
...
export function AudioTraitPanel({ onClose }: AudioTraitPanelProps) {
  ...
  const copySnippet = (preset: AudioPreset) => {
    navigator.clipboard.writeText(preset.traitSnippet).catch(() => {});
    setCopied(preset.id);
  ...
  return (
    <div className="flex h-full flex-col bg-studio-panel text-studio-text">
      ...
      <div className="shrink-0 border-b border-studio-border p-2.5 space-y-2">
        <div className="flex items-center gap-2 rounded-lg border border-studio-border bg-studio-surface px-2.5 py-1.5">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && load()}
            placeholder="Search audio…" className="flex-1 bg-transparent text-[11px] outline-none placeholder-studio-muted/40" />
        </div>
        <div className="flex gap-1">
          {(['', 'ambient', 'triggered', 'spatial'] as const).map((t) => (
            <button key={t} onClick={() => setActiveType(t as AudioType | '')}
      ...
      <div className="flex-1 overflow-y-auto p-2.5 space-y-1.5">
        {loading && <p className="py-8 text-center text-[10px] text-studio-muted animate-pulse">Loading presets…</p>}
        {!loading && presets.length === 0 && <p className="py-8 text-center text-[10px] text-studio-muted">No presets found.</p>}

        {presets.map((p) => (
          <div key={p.id} className="rounded-xl border border-studio-border bg-studio-surface p-2.5 space-y-1.5">
            <div className="flex items-start gap-2">
              <div className={`mt-0.5 flex shrink-0 items-center gap-1 rounded-full border px-1.5 py-0.5 text-[8px] font-medium ${TYPE_COLORS[p.type]}`}>
            ...
            <div className="flex gap-1.5">
              <button onClick={() => insertIntoScene(p)}
              ...
              <button onClick={() => copySnippet(p)}
                ...
                {copied === p.id ? <Check className="h-3 w-3 text-green-400" /> : <Copy className="h-3 w-3" />}
              </button>
            </div>
          </div>
        ))}
