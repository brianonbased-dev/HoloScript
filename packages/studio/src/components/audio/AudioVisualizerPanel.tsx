...
interface AudioPreset {
  ...
  emoji: string; bpm: number; traitSnippet: string;
  waveform: string;
}

const CATEGORY_COLORS: Record<string, string> = {
  ambient: '#4488ff', music: '#cc44ff', sfx: '#ff8844',
};
...
interface AudioVisualizerPanelProps { onClose: () => void; }
...
export function AudioVisualizerPanel({ onClose }: AudioVisualizerPanelProps) {
  ...
  useEffect(() => {
    fetch('/api/audio-presets')
      .then((r) => r.json())
      .then((d: { presets: AudioPreset[] }) => { setPresets(d.presets); if (d.presets[0]) setSelected(d.presets[0]); })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="flex h-full flex-col bg-studio-panel text-studio-text">
      ...
      <div className="relative shrink-0 border-b border-studio-border bg-[#0a0a12]">
        <canvas ref={viz.canvasRef as React.RefObject<HTMLCanvasElement>} width={288} height={120} className="w-full" />
        {!viz.isPlaying && (
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-studio-border px-3 py-2">
        {/* Play/Stop */}
        <button onClick={viz.isPlaying ? viz.stop : viz.play}
          ...
          {viz.isPlaying ? <><Square className="h-3 w-3" /> Stop</> : <><Play className="h-3 w-3" /> Play Demo</>}
        </button>

        {/* Mode pills */}
        <div className="flex gap-1 ml-auto">
          {MODES.map((m) => (
            <button key={m.id} onClick={() => viz.setMode(m.id)}
      ...
      <div className="shrink-0 border-b border-studio-border px-3 py-2">
        ...
        <input type="range" min={0} max={1} step={0.01} value={viz.gain} onChange={(e) => viz.setGain(Number(e.target.value))}
