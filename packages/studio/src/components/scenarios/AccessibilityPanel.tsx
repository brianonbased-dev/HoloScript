...
import { contrastRatio, meetsWCAG, rampCompliant, doorWidthCompliant, auditScore, type AccessibilityIssue, type WCAGLevel, type SpaceAudit } from '@/lib/accessibilityAuditor';

const LEVEL_COLORS: Record<WCAGLevel, string> = { A: '#f59e0b', AA: '#22c55e', AAA: '#06b6d4' };

const s = {
  panel: { background: 'linear-gradient(180deg, #0f1210 0%, #15201a 100%)', borderRadius: 12, padding: 20, color: '#c8e8d8', fontFamily: "'Inter', sans-serif", minHeight: 600, maxWidth: 720 } as React.CSSProperties,
...
export function AccessibilityPanel() {
  ...
  const issues: AccessibilityIssue[] = [
    { id: 'i1', type: 'visual', description: 'Missing alt text on hero image', severity: 'major', wcagCriteria: '1.1.1', location: 'Homepage', recommendation: 'Add descriptive alt text' },
    { id: 'i2', type: 'motor', description: 'No keyboard navigation on dropdown', severity: 'critical', wcagCriteria: '2.1.1', location: 'Navigation', recommendation: 'Add tabIndex and key handlers' },
    { id: 'i3', type: 'cognitive', description: 'Complex form lacks instructions', severity: 'minor', wcagCriteria: '3.3.2', location: 'Contact Form', recommendation: 'Add field labels and helper text' },
  ];
    ...
    <div style={s.panel}>
      ...
      <div style={s.section}>
        ...
        <div style={{ padding: 12, background: bg, borderRadius: 6, marginBottom: 8 }}>
          <span style={{ color: fg, fontSize: 16, fontWeight: 700 }}>Sample Text · Ratio: {contrast.toFixed(2)}:1</span>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {(['A', 'AA', 'AAA'] as WCAGLevel[]).map(level => {
            const passes = level === 'A' ? wcagA : level === 'AA' ? wcagAA : wcagAAA;
            return (
              <div key={level} style={{ padding: '4px 12px', borderRadius: 6, fontSize: 12, fontWeight: 700,
      ...
      <div style={s.section}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
          <div style={{ padding: 10, borderRadius: 6, border: `1px solid ${rampOk ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)'}` }}>
          ...
          <div style={{ padding: 10, borderRadius: 6, border: `1px solid ${doorOk ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)'}` }}>
      ...
      <div style={s.section}>
        ...
        {issues.map(issue => (
          <div key={issue.id} style={{ padding: '8px 10px', background: 'rgba(255,255,255,0.03)', borderRadius: 6, marginBottom: 4, borderLeft: `3px solid ${SEV_COLORS[issue.severity]}`, fontSize: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontWeight: 600 }}>{issue.description}</span>
              <span style={{ color: SEV_COLORS[issue.severity], fontWeight: 700, fontSize: 11 }}>{issue.severity}</span>
            </div>
