...
const FORMATS: { id: V2Format; label: string; ext: string; desc: string; badge?: string }[] = [
  {
    ...
    label: 'OBJ + MTL',
    ext: '.obj',
    desc: 'Wavefront OBJ with material library. Compatible with Blender, Cinema 4D, Maya.',
    badge: 'NEW',
  },
  {
    id: 'fbx',
    label: 'FBX ASCII',
    ext: '.fbx',
    desc: 'ASCII FBX 7.4 scaffold with full transform data. Import into Maya or Blender.',
    badge: 'NEW',
  },
  {
    id: 'gltf',
    label: 'glTF JSON',
    ext: '.gltf',
    desc: 'Node-based glTF 2.0 scene graph. Ready for Three.js, Babylon.js, model viewers.',
  },
  {
    id: 'usd',
    label: 'USD/USDA',
    ext: '.usda',
    desc: 'Universal Scene Description. Compatible with Apple Reality Composer, Omniverse.',
  },
  {
    id: 'json',
    label: 'HoloScript JSON',
    ext: '.json',
    desc: 'Structured JSON of the parsed scene graph + original source. For tooling.',
  },
];

interface ExportPipelinePanelProps {
  onClose: () => void;
}
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2 rounded-xl border border-studio-border bg-studio-surface p-2.5 text-center">
          {[
            ['Lines', lines],
            ['Objects', objects],
            ['KB', (new Blob([code]).size / 1024).toFixed(1)],
          ].map(([l, v]) => (
            <div key={String(l)}>
            ...
            {FORMATS.map((f) => (
              ...
              >
                <div
                  ...
                  style={{
                    borderColor: format === f.id ? 'var(--studio-accent)' : '#555',
                    backgroundColor: format === f.id ? 'var(--studio-accent)' : 'transparent',
                  }}
                />
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    {f.badge && (
                      <span className="rounded bg-studio-accent/20 px-1 py-0.5 text-[7px] font-bold text-studio-accent">
                        {f.badge}
                      </span>
                    )}
                  </div>
