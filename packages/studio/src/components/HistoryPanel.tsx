...
const UndoTreePanel = dynamic(
  () => import('@/components/history/UndoTreePanel').then((m) => ({ default: m.UndoTreePanel })),
  {
    ssr: false,
    loading: () => (
      <div className="flex-1 flex items-center justify-center text-xs text-studio-muted animate-pulse">
        Loading tree…
      </div>
    ),
  }
...
export function HistoryPanel({ onClose }: HistoryPanelProps) {
  ...
  /** Action labels recorded alongside each temporal snapshot (oldest→newest). */
  const storedLabels = useHistoryLabelStore((s) => s.labels);

  const canUndo = past.length > 0;
  const canRedo = future.length > 0;
  const allEntries = [...past].reverse(); // most-recent first

  /** Map a past-entry index (0 = most-recent) to its human-readable action label. */
  function getLabel(indexFromTop: number): string {
    const reversed = [...storedLabels].reverse();
    if (reversed[indexFromTop]) return reversed[indexFromTop]!;
    return indexFromTop === allEntries.length - 1 ? 'Initial state' : `Step ${indexFromTop + 1}`;
  }
    ...
    <div className="flex h-full flex-col border-l border-studio-border bg-studio-panel">
      ...
      <div className="flex h-10 items-center justify-between border-b border-studio-border px-3">
        ...
        <div className="flex items-center rounded-lg border border-studio-border p-0.5">
          <button
            ...
            title="List view"
            className={`rounded p-1 transition ${viewMode === 'list' ? 'bg-studio-accent/20 text-studio-accent' : 'text-studio-muted hover:text-studio-text'}`}
          >
          ...
          <button
            ...
            title="Tree view"
            className={`rounded p-1 transition ${viewMode === 'tree' ? 'bg-studio-accent/20 text-studio-accent' : 'text-studio-muted hover:text-studio-text'}`}
          >
      ...
      {viewMode === 'tree' ? (
        ...
        <>
          ...
          {/* Future states (greyed — redo branch) */}
          {future.length > 0 && (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[11px] text-studio-text">{getLabel(i)}</p>
                      <p className="text-[10px] text-studio-muted">
                        {nodeCount} node{nodeCount !== 1 ? 's' : ''}
                      </p>
                    </div>
                    <span className="shrink-0 text-[10px] text-studio-muted/60">-{stepsBack}</span>
                  </button>
                );
              })
            )}
