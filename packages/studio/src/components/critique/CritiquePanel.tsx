...
const CATEGORY_COLORS: Record<string, string> = {
  performance: '#ff6644',
  naming: '#ffcc44',
  'missing-trait': '#aa66ff',
  structure: '#44aaff',
  'best-practice': '#44cc88',
};
      ...
      <div className="p-2.5 space-y-1.5">
        ...
        {f.snippet && (
          <div className="rounded-lg bg-[#080810] p-1.5 flex items-start gap-1.5">
            <pre className="flex-1 text-[7.5px] text-green-300/80 overflow-x-auto leading-relaxed">
              {f.snippet}
            </pre>
            <button
              onClick={copySnippet}
            ...
            <div className="flex border-b border-studio-border">
              {(['error', 'warning', 'tip'] as const).map((sev) => {
                ...
                return (
                  <div
                    key={sev}
                    className={`flex-1 flex items-center justify-center gap-1 py-1.5 text-[8px] ${SEVERITY_COLOR[sev]}`}
                  >
                    {sev === 'tip' ? (
