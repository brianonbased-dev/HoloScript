...
function ColorProp({ prop, onChange }: { prop: SceneProp; onChange: (v: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <input
        type="color"
        value={color.startsWith('"') ? color.slice(1, -1) : color}
...
function Vec3Prop({
  ...
  const vec = Array.isArray(prop.value)
    ? (prop.value as [number, number, number])
...
function BoolProp({ prop, onChange }: { prop: SceneProp; onChange: (v: boolean) => void }) {
  return (
    ...
    >
      <span
        className={`h-4 w-4 rounded-full bg-white shadow transition-transform duration-200 ${prop.value ? 'translate-x-5' : 'translate-x-0'}`}
      />
    </button>
  );
}
...
function GroupCard({
  ...
  group: PropGroup;
  onSet: (key: string, v: SceneProp['value']) => void;
}) {
          ...
          {group.props.map((prop) => (
            <div key={prop.key} className="grid grid-cols-[auto_1fr] items-center gap-2 px-3 py-2">
              ...
              <div className="min-w-0">
                {prop.type === 'color' && (
                  <ColorProp prop={prop} onChange={(v) => onSet(prop.key, v)} />
                )}
                {prop.type === 'float' && (
                  <FloatProp prop={prop} onChange={(v) => onSet(prop.key, v)} />
                )}
                {prop.type === 'vec3' && (
                  <Vec3Prop prop={prop} onChange={(v) => onSet(prop.key, v)} />
                )}
                {prop.type === 'boolean' && (
                  <BoolProp prop={prop} onChange={(v) => onSet(prop.key, v)} />
                )}
                {prop.type === 'enum' && (
                  <EnumProp prop={prop} onChange={(v) => onSet(prop.key, v)} />
                )}
                {prop.type === 'string' && (
                  <StringProp prop={prop} onChange={(v) => onSet(prop.key, v)} />
                )}
              </div>
            </div>
          ))}
  ...
  return (
    <div className="flex h-full flex-col bg-studio-panel text-studio-text">
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-studio-border px-3 py-2.5">
        ...
        {objectName && (
          <code className="ml-1 rounded bg-studio-surface px-1.5 py-0.5 text-[9px] text-studio-accent">
            {objectName}
          </code>
        )}
        {lineRange && (
          <span className="text-[7px] text-studio-muted/60">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {!objectName && (
          <div className="flex flex-col items-center justify-center gap-3 py-10 text-center">
            ...
            <p className="text-[8px] text-studio-muted/50">
              Properties will appear here automatically
            </p>
          </div>
        )}
        ...
        {groups.map((group) => (
          <GroupCard
            key={group.trait}
            group={group}
            onSet={(key, val) => handleSet(group.trait, key, val)}
          />
        ))}
      </div>
      ...
      {objectName && (
        <div className="shrink-0 border-t border-studio-border px-3 py-2 flex items-center gap-2 text-[7px] text-studio-muted">
          ...
          {lineRange && (
            <>
              <span>·</span>
              <span>{lineRange[1] - lineRange[0] + 1} lines</span>
            </>
          )}
          {groups.length > 0 && (
            <>
              <span>·</span>
              <span>{groups.reduce((a, g) => a + g.props.length, 0)} props</span>
            </>
          )}
