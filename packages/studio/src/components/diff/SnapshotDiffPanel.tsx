...
interface SnapshotDiffPanelProps { onClose: () => void; }

export function SnapshotDiffPanel({ onClose }: SnapshotDiffPanelProps) {
  const { diff, stats, allCodes, currentIndex, indexA, indexB, setIndexA, setIndexB } = useSnapshotDiff();
    ...
    <div className="flex h-full flex-col bg-[#0a0a12] text-studio-text">
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-studio-border bg-studio-panel px-3 py-2.5">
        ...
        <div className="ml-2 flex items-center gap-1">
          <span className="rounded-full bg-red-900/40 px-1.5 py-0.5 text-[8px] text-red-400">−{stats.removed}</span>
          <span className="rounded-full bg-green-900/40 px-1.5 py-0.5 text-[8px] text-green-400">+{stats.added}</span>
        </div>
      ...
      <div className="flex-1 overflow-auto">
        {diff.length === 0 && (
          <p className="py-8 text-center text-[10px] text-studio-muted">No differences to show. Make some edits to the scene.</p>
        )}
        {diff.every((d) => d.type === 'same') && diff.length > 0 && (
          <p className="py-4 text-center text-[10px] text-green-500/80">✓ Snapshots are identical</p>
        )}
