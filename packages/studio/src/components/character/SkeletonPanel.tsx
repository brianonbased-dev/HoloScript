...
function getBoneCategory(name: string): 'spine' | 'arm' | 'leg' | 'head' | 'hand' | 'foot' | 'other' {
  const n = name.toLowerCase();
  if (n.includes('spine') || n.includes('hip') || n.includes('pelvis') || n.includes('chest')) return 'spine';
  if (n.includes('shoulder') || n.includes('upper_arm') || n.includes('lower_arm') || n.includes('forearm') || n.includes('arm')) return 'arm';
  if (n.includes('thigh') || n.includes('shin') || n.includes('knee') || n.includes('calf') || n.includes('leg')) return 'leg';
  if (n.includes('head') || n.includes('neck') || n.includes('jaw')) return 'head';
  if (n.includes('hand') || n.includes('finger') || n.includes('thumb') || n.includes('index') || n.includes('middle') || n.includes('ring') || n.includes('pinky')) return 'hand';
  ...
  const bonesByCategory = categories.reduce<Record<string, { name: string; index: number }[]>>(
    (acc, cat) => {
      acc[cat] = boneNames
        ...
        .filter(({ name }) => getBoneCategory(name) === cat && name.toLowerCase().includes(filter.toLowerCase()));
      return acc;
    },
          ...
          {categories.map((cat) => {
            ...
            return (
              <div key={cat}>
                ...
                >
                  {isCollapsed ? <ChevronRight className="h-3 w-3 text-studio-muted" /> : <ChevronDown className="h-3 w-3 text-studio-muted" />}
                  <span className={`text-[10px] font-semibold uppercase tracking-wide ${CATEGORY_COLOR[cat]}`}>
                    {CATEGORY_EMOJI[cat]} {cat} ({bones.length})
                  </span>
                </button>
                {!isCollapsed && bones.map(({ name, index }) => (
                  <button
                    key={index}
                    onClick={() => setSelectedBoneIndex(selectedBoneIndex === index ? null : index)}
