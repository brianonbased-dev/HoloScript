...
export function LODEditor({ onChange }: { onChange?: (c: LODConfig) => void }) {
  ...
  const addLevel = useCallback(() => {
    ...
    update({
      levels: [
        ...
        {
          id: nextLodId++,
          label: `LOD ${config.levels.length}`,
          distance: maxDist + 30,
          polyCount: Math.max(2, Math.floor(minPoly / 5)),
          textureRes: 256,
          enabled: true,
        },
      ].sort((a, b) => a.distance - b.distance),
  ...
  const totalSaved =
    config.levels.length > 0
      ? (
          (1 - config.levels[config.levels.length - 1].polyCount / config.levels[0].polyCount) *
          100
        ).toFixed(0)
      : '0';
    ...
    <div className="flex flex-col overflow-auto">
      <div className="flex items-center justify-between border-b border-studio-border px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-studio-text">LOD Manager</span>
        </div>
      ...
      {config.levels.map((level, _i) => (
        ...
        >
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <button
              onClick={() => updateLevel(level.id, { enabled: !level.enabled })}
              className="text-studio-muted/40 hover:text-studio-text"
            >
              {level.enabled ? <Eye className="h-3 w-3" /> : <Eye className="h-3 w-3 opacity-30" />}
            </button>
          ...
          <div className="grid grid-cols-3 gap-2">
            <label className="flex flex-col gap-0.5 text-[9px] text-studio-muted">
              ...
              <input
                ...
                step={5}
                onChange={(e) =>
                  updateLevel(level.id, { distance: parseFloat(e.target.value) || 0 })
                }
                className="rounded border border-studio-border bg-transparent px-1 py-0.5 text-xs text-studio-text outline-none font-mono"
              />
            </label>
            <label className="flex flex-col gap-0.5 text-[9px] text-studio-muted">
              Polygons
              <input
                type="number"
                value={level.polyCount}
                min={2}
                step={100}
                onChange={(e) =>
                  updateLevel(level.id, { polyCount: parseInt(e.target.value) || 2 })
                }
                className="rounded border border-studio-border bg-transparent px-1 py-0.5 text-xs text-studio-text outline-none font-mono"
              />
            </label>
            <label className="flex flex-col gap-0.5 text-[9px] text-studio-muted">
              Texture (px)
              <select
                value={level.textureRes}
                onChange={(e) => updateLevel(level.id, { textureRes: parseInt(e.target.value) })}
                className="rounded border border-studio-border bg-transparent px-1 py-0.5 text-xs text-studio-text outline-none"
              >
                {[64, 128, 256, 512, 1024, 2048, 4096].map((r) => (
          ...
          <div className="mt-1.5 h-1 w-full rounded-full bg-studio-panel">
            <div
              ...
              style={{
                width: `${Math.min(100, (level.distance / (config.levels[config.levels.length - 1]?.distance || 100)) * 100)}%`,
              }}
            />
          </div>
        </div>
      ...
      <button
        onClick={addLevel}
        className="flex items-center gap-1 px-3 py-2 text-[10px] text-studio-muted hover:text-studio-text border-b border-studio-border"
      >
      ...
      <div className="flex flex-col gap-2 px-3 py-2">
        ...
        <div>
          <div className="flex justify-between text-[10px] text-studio-muted">
            <span>Quality Bias</span>
            <span className="font-mono">{config.bias.toFixed(1)}</span>
          </div>
          <input
            type="range"
            min={-1}
            ...
            step={0.1}
            value={config.bias}
            onChange={(e) => update({ bias: parseFloat(e.target.value) })}
            className="w-full accent-teal-500"
          />
          <div className="flex justify-between text-[8px] text-studio-muted/40">
        ...
        <label className="flex items-center gap-2 text-[10px] text-studio-muted cursor-pointer">
          <input
            type="checkbox"
            checked={config.fadeTransition}
            onChange={(e) => update({ fadeTransition: e.target.checked })}
            className="rounded border-studio-border"
          />
          Fade Transition
        </label>
        {config.fadeTransition && (
          <div>
            <div className="flex justify-between text-[10px] text-studio-muted">
              ...
              <span className="font-mono">{config.fadeDuration}s</span>
            </div>
            <input
              type="range"
              min={0.05}
