...
const TYPE_LABEL: Record<string, string> = { fire: 'fire', snow: 'snow', sparks: 'sparks', rain: 'rain', smoke: 'smoke', magic: 'magic', bubbles: 'bubbles', leaves: 'leaves' };

interface ParticlePanelProps { onClose: () => void; }
    ...
    <div className="flex h-full flex-col bg-studio-panel text-studio-text">
      ...
      <div className="shrink-0 space-y-2 border-b border-studio-border p-2.5">
        ...
        <div className="flex flex-wrap gap-1">
          ...
          {types.map((t) => (
            <button key={t} onClick={() => setActiveType(t === activeType ? '' : t)}
      ...
      <div className="flex-1 space-y-2 overflow-y-auto p-2.5">
        ...
        {presets.map((p) => (
          <div key={p.id} className="overflow-hidden rounded-xl border border-studio-border bg-studio-surface transition hover:border-studio-accent/40">
            {/* Color bar */}
            ...
            <div className="p-2.5">
              <div className="flex items-center gap-2">
                ...
                <span className="shrink-0 rounded-full border border-studio-border px-1.5 py-0.5 text-[7px] text-studio-muted">{p.type}</span>
              </div>
              {/* Snippet preview */}
