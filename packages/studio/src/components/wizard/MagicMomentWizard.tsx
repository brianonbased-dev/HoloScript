...
import { useMagicMoment, MAGIC_MOMENT_DEFAULT_CODE } from '@/hooks/useMagicMoment';
...
import type { MagicMomentStep } from '@/hooks/useMagicMoment';
...
export function MagicMomentWizard({ onClose }: MagicMomentWizardProps) {
  ...
  // Scene store — to pass the final code to the workspace
  const setSceneCode = useSceneStore((s) => s.setCode);
  ...
  const handleBegin = useCallback(() => {
    setDissolving(true);
    // Transfer code to the main scene store
    setSceneCode(code);
    setTimeout(() => {
      completeWizard();
      onClose();
    }, 600);
  }, [code, setSceneCode, completeWizard, onClose]);
  ...
  const handleSkip = useCallback(() => {
    ...
    skipToWorkspace();
    onClose();
  }, [code, setSceneCode, skipToWorkspace, onClose]);
  ...
  const stepLabels = ['Hello', 'Type', 'Enchant', 'Share', 'Reveal', 'Begin'];
