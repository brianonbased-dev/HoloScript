...
export function ScriptDebugger() {
  ...
  const addBreakpoint = useCallback((file: string, line: number, condition?: string) => {
    setBreakpoints((prev) => [
      ...prev,
      { id: bpId++, file, line, enabled: true, condition, hitCount: 0 },
    ]);
  }, []);
  ...
  const stateColors: Record<DebugState, string> = {
  ...
  return (
    <div className="flex flex-col overflow-auto">
          ...
          {state === 'idle' ? (
            ...
            <>
              <button
                ...
                className="rounded bg-amber-500/20 p-1 text-amber-400"
                title="Pause"
              >
              ...
              <button
                ...
                className="rounded bg-red-500/20 p-1 text-red-400"
                title="Stop"
              >
      ...
      <div className="border-b border-studio-border">
        ...
        {breakpoints.map((bp) => (
          <div key={bp.id} className="flex items-center gap-2 px-3 py-1 text-[11px]">
            <button
              ...
              className={`h-2.5 w-2.5 rounded-full ${bp.enabled ? 'bg-red-500' : 'bg-studio-muted/30'}`}
            />
            <span className="flex-1 font-mono text-studio-text">
              {bp.file}:{bp.line}
            </span>
        ...
        <div className="flex items-center gap-1 px-3 py-1">
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && bpInput.includes(':')) {
                const [f, l] = bpInput.split(':');
                addBreakpoint(f, parseInt(l));
                setBpInput('');
              }
            }}
      ...
      {/* Watch */}
      <div className="border-b border-studio-border">
        <div className="px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-studio-muted">
        ...
        <div className="flex items-center gap-1 px-3 py-1">
          <input
            ...
            onKeyDown={(e) => e.key === 'Enter' && addWatch(watchInput)}
            className="flex-1 bg-transparent text-[10px] text-studio-text outline-none font-mono"
          />
          <button
      ...
      <div>
        ...
        {callStack.length === 0 && (
          <div className="px-3 py-2 text-[11px] text-studio-muted/50">Not paused</div>
        )}
        {callStack.map((frame, i) => (
          <div
            key={frame.id}
            className={`flex items-center gap-2 px-3 py-1 text-[11px] cursor-pointer ${i === 0 ? 'bg-amber-500/10 text-amber-400' : 'text-studio-muted hover:bg-studio-panel/50'}`}
          >
            <span className="font-semibold">{frame.name}</span>
            <span className="flex-1" />
            <span className="font-mono text-[9px] text-studio-muted/50">
