...
import { Bug, X, Play, StepForward, RotateCcw, FastForward, CheckCircle, AlertCircle } from 'lucide-react';
import { useDebugger, type DebugFrame, type DebugVar } from '@/hooks/useDebugger';
import { useSceneStore } from '@/lib/store';

interface DebuggerPanelProps { onClose: () => void; }
...
export function DebuggerPanel({ onClose }: DebuggerPanelProps) {
  ...
  const { frames, currentFrame, variables, status, error, start, step, cont, reset, toggleBreakpoint } = useDebugger();
    ...
    <div className="flex h-full flex-col bg-studio-panel text-studio-text">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        <div className="flex gap-1.5">
          <button onClick={() => start(code)} title="Start / Restart"
          ...
          <button onClick={() => step(code)} disabled={status !== 'paused'}
            title="Step" className="rounded-xl border border-studio-border px-3 py-1.5 text-studio-muted hover:text-studio-text disabled:opacity-40">
            <StepForward className="h-3.5 w-3.5" />
          </button>
          <button onClick={() => cont(code)} disabled={status !== 'paused'}
            title="Continue to next breakpoint" className="rounded-xl border border-studio-border px-3 py-1.5 text-studio-muted hover:text-studio-text disabled:opacity-40">
            <FastForward className="h-3.5 w-3.5" />
          </button>
          <button onClick={() => reset(code)} title="Reset"
        ...
        {frames.length > 0 && (
          <div className="space-y-1">
            ...
            <div className="max-h-56 overflow-y-auto space-y-0.5 pr-1">
              {frames.map((f) => (
                <button key={f.index} onClick={() => toggleBreakpoint(f.line)}
                  ...
                  <span className={`mt-0.5 text-[9px] font-mono ${FRAME_COLORS[f.type] ?? 'text-studio-muted'}`}>
                    {f.isBreakpoint ? '🔴' : f.index === currentFrame ? '▶' : ' '} L{f.line}
                  </span>
                  <div className="min-w-0 flex-1">
                    <span className="text-[10px] font-medium truncate block">{f.label}</span>
                    {f.detail && <span className="text-[9px] text-studio-muted truncate block">{f.detail}</span>}
                  </div>
                </button>
              ))}
            </div>
        ...
        {variables.length > 0 && (
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              ...
              <input value={varFilter} onChange={(e) => setVarFilter(e.target.value)}
                placeholder="Filter…" className="ml-auto w-24 rounded border border-studio-border bg-studio-surface px-1.5 py-0.5 text-[9px] outline-none focus:border-studio-accent" />
            </div>
                ...
                <div key={scope}>
                  ...
                  <div className="rounded-xl border border-studio-border overflow-hidden">
                    {(vars as DebugVar[]).map((v, i) => (
                      <div key={v.name} className={`flex gap-2 px-2 py-1 text-[10px] ${i % 2 === 0 ? 'bg-studio-surface' : 'bg-studio-panel'}`}>
                        <span className="text-studio-muted font-mono truncate flex-1">{v.name}</span>
                        <span className="shrink-0 text-[9px] text-violet-400">{v.type}</span>
                        <span className="shrink-0 font-mono text-studio-text max-w-[80px] truncate">{v.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {status === 'idle' && frames.length === 0 && (
          <p className="py-4 text-center text-[10px] text-studio-muted">
            ...
            <br />Click any frame to toggle a breakpoint (🔴).
          </p>
        )}
        {status === 'finished' && (
