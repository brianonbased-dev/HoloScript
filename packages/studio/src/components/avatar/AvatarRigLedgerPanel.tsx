...
/**
 * AvatarRigLedgerPanel
 *
 * Right-rail "verify card" for /create?mode=avatar — the rig-integrity ledger.
 *
 * Displays the derived-rig gate status from the most recent
 * POST /api/avatar/compile, plus the build report (bone count, coverage,
 * expressions, rig digest, wall time) and a determinism badge (two
 * identical-config builds -> identical rig digest).
 *
 * This is a derivation report with a state digest — NOT a full
 * SimulationContract receipt. The honest framing matches GameGateLedgerPanel.
 *
 * State comes from useAvatarModeState (single source of truth).
 */

import React, { useCallback, useState } from 'react';
import { X, CheckCircle2, XCircle, Copy, Check, ShieldCheck } from 'lucide-react';
...
function DigestRow({
  ...
  return (
    <div className="flex items-center justify-between text-[11px] py-1.5">
      <span className="text-studio-muted">Rig digest</span>
...
function DeterminismBadge({ match }: { match: boolean }) {
  return (
    ...
    >
      ...
      {match ? 'Build twice = same rig digest' : 'Digest changed (source/params modified)'}
    </div>
  );
}

export interface AvatarRigLedgerPanelProps {
  onClose?: () => void;
}

export function AvatarRigLedgerPanel({ onClose }: AvatarRigLedgerPanelProps) {
  const result = useAvatarModeState((s) => s.result);
  const lastDigest = useAvatarModeState((s) => s.lastDigest);
  const [copied, setCopied] = useState(false);

  const handleCopy = useCallback(() => {
    if (!result?.report?.rigDigest) return;
    void navigator.clipboard.writeText(result.report.rigDigest).then(() => {
      setCopied(true);
  ...
  const showDeterminismBadge =
    report !== null && lastDigest !== null && report.rigDigest === lastDigest;
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-studio-border px-3 py-2">
        <div className="flex items-center gap-2">
          <ShieldCheck className="h-3.5 w-3.5 text-studio-muted" aria-hidden="true" />
        ...
        {onClose && (
          <button
            onClick={onClose}
            title="Close"
            aria-label="Close avatar rig ledger panel"
            className="rounded p-0.5 text-studio-muted hover:text-studio-text hover:bg-white/[0.06] transition"
          >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        {!report ? (
          <div className="flex flex-col items-center justify-center h-full gap-2 px-4 py-8 text-center">
            <ShieldCheck className="h-8 w-8 text-studio-muted/30" aria-hidden="true" />
            <p className="text-[11px] text-studio-muted/60">Build a rig to see the ledger.</p>
          </div>
        ) : (
          <div className="px-3 py-3 space-y-3">
            ...
            <div className="rounded-lg border border-studio-border/60 bg-studio-surface/50 px-3 py-2 space-y-1.5">
              ...
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-studio-muted">Bones</span>
                <span className="font-mono tabular-nums text-studio-text text-[10px]">
                  {report.boneCount} ({report.requiredBonesPresent}/{report.requiredBonesTotal} req)
                </span>
              </div>
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-studio-muted">Coverage</span>
                <span className="font-mono tabular-nums text-studio-text">
                  {(report.boneCoverage * 100).toFixed(0)}%
                </span>
              </div>
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-studio-muted">Expressions · Layers</span>
                <span className="font-mono tabular-nums text-studio-text">
                  {report.expressionCount} · {report.layerCount}
                </span>
              </div>
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-studio-muted">Scale</span>
                <span className="font-mono tabular-nums text-studio-text">
                  {report.scale.toFixed(2)}×
                </span>
              </div>
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-studio-muted">Rig size</span>
                <span className="font-mono tabular-nums text-studio-text">
                  {(report.rigBytes / 1024).toFixed(1)} KB
                </span>
              </div>
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-studio-muted">Derive time</span>
                <span className="font-mono tabular-nums text-studio-text">
            ...
            <div className="rounded-lg border border-studio-border/60 bg-studio-surface/50 px-3 py-1">
              <DigestRow digest={report.rigDigest} copied={copied} onCopy={handleCopy} />
            </div>

            <p className="text-[9px] text-studio-muted/40 leading-snug">
              This is a VRM-shaped rig-derivation report with integrity gates and a state digest —
              not a full SimulationContract receipt.
            </p>
          </div>
        )}
      </div>
