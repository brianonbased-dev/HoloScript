...
describe('Scenario: Neuroscience — Cognitive States', () => {
  it('dominant delta = deep-sleep', () => {
    expect(detectCognitiveState({ delta: 80, theta: 5, alpha: 5, beta: 5, gamma: 5 })).toBe('deep-sleep');
  });

  it('high alpha = relaxed', () => {
    expect(detectCognitiveState({ delta: 5, theta: 5, alpha: 50, beta: 10, gamma: 5 })).toBe('relaxed');
  });

  it('dominant beta = focused', () => {
    expect(detectCognitiveState({ delta: 5, theta: 10, alpha: 15, beta: 35, gamma: 10 })).toBe('focused');
  });

  it('dominant gamma = flow', () => {
    expect(detectCognitiveState({ delta: 5, theta: 5, alpha: 5, beta: 10, gamma: 50 })).toBe('flow');
  });

  it('pathwayStrength() averages connectivity', () => {
    const pathways: NeuralPathway[] = [
      { id: 'p1', name: 'Arc Fascic', source: 'broca', target: 'wernicke', strength: 0.8, neurotransmitter: 'glutamate' },
      { id: 'p2', name: 'Nigrostriatal', source: 'brainstem', target: 'motor', strength: 0.6, neurotransmitter: 'dopamine' },
    ];
