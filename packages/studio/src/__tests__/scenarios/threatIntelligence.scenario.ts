...
describe('Scenario: SOC Threat Intelligence', () => {
  ...
  it('generates dynamic firewall heuristics', () => {
    ...
    expect(generateFirewallRule(nodes[1])).toBe(
      'THROTTLE IPv4 2.2.2.2 BANDWIDTH 500Kbps --reason "ANOMALOUS_SPIKE"'
    );
    expect(generateFirewallRule(nodes[2])).toBe(
      'BLOCK_ALL IPv4 3.3.3.3 PORT * --reason "ACTIVE_COMPROMISE"'
    );
  });
});
