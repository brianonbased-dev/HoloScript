...
function makeStroke(overrides?: Partial<Stroke>): Stroke {
  return {
    ...
    points: overrides?.points ?? [[0,0,0],[1,0,0],[2,1,0]],
...
describe('Scenario: Sketch Mode — Brush Configuration', () => {
  beforeEach(() => {
    useSketchStore.setState({ strokes:[], activeStroke:null, brushColor:'#6366f1', brushSize:0.015, brushMaterial:'neon' });
  });

  it('default brush color is #6366f1', () => expect(useSketchStore.getState().brushColor).toBe('#6366f1'));
...
describe('Scenario: Sketch Mode — Stroke Management', () => {
  beforeEach(() => {
    useSketchStore.setState({ strokes: [makeStroke({id:'a'}), makeStroke({id:'b'}), makeStroke({id:'c'})], activeStroke:null });
  });
  ...
  it('export strokes to .glb as geometry (tube meshes)', () => {
    useSketchStore.getState().addStroke(makeStroke({ points: [[0,0,0], [1,0,0]] }));
...
describe('Scenario: Sketch Mode — Catmull-Rom Smoothing', () => {
  it('catmullRomInterpolate() returns more points than raw input', () => {
    const raw: Vec3[] = [[0,0,0],[1,0,0],[2,1,0],[3,0,0]];
  ...
  it('catmullRomInterpolate() first point matches first raw point', () => {
    const raw: Vec3[] = [[0,0,0],[1,0,0],[2,0,0]];
  ...
  it('catmullRomPoint() at t=0 ≈ p1', () => {
    const p0: Vec3 = [0,0,0], p1: Vec3 = [1,0,0], p2: Vec3 = [2,0,0], p3: Vec3 = [3,0,0];
  ...
  it('gaussianSmoothStroke() 1 pass reduces jitter (sum of squared distances shrinks)', () => {
    const jittery: Vec3[] = [[0,0,0],[0.5,0.8,0],[1,0,0],[1.5,0.9,0],[2,0,0]];
  ...
  it('gaussianSmoothStroke() preserves endpoints', () => {
    const pts: Vec3[] = [[0,0,0],[0.5,1,0],[1,0,0]];
  ...
  it('smooth stroke interpolation applied on pointer release (post-commit)', () => {
    const raw: Vec3[] = [[0,0,0],[0.5,0.8,0],[1,0,0]];
