...
const PAIRING_MATRIX: Record<
  FoodCategory,
  { preferColors: WineColor[]; preferBody: [number, number]; preferTannins: [number, number] }
> = {
  ...
  cheese: {
    preferColors: ['red', 'white', 'dessert', 'fortified'],
    preferBody: [2, 5],
    preferTannins: [1, 4],
  },
  salad: {
  ...
  dessert: {
    preferColors: ['dessert', 'sparkling', 'fortified'],
    preferBody: [2, 5],
    preferTannins: [1, 3],
  },
};
...
export function servingTemperatureC(color: WineColor): { min: number; max: number } {
  const temps: Record<WineColor, { min: number; max: number }> = {
    ...
    dessert: { min: 10, max: 14 },
    fortified: { min: 14, max: 18 },
  };
  return temps[color];
}
...
export function blindTastingScore(observedNotes: TastingNote[]): BlindTastingResult {
  ...
  for (const [varietal, sig] of Object.entries(VARIETAL_SIGNATURES) as [
    GrapeVarietal,
    TastingNote[],
  ][]) {
...
export function terroirIndex(
  ...
  score +=
    altitudeM >= 200 && altitudeM <= 600 ? 25 : altitudeM >= 100 && altitudeM <= 800 ? 15 : 5;
