...
function resultWithMutation(): ArmModelResult {
  return {
    rawOutput: '{"tool":"set_trait_property","input":{"target":"lamp","trait":"glow","property":"intensity","value":1}}',
    mutation: { tool: 'set_trait_property', input: { target: 'lamp', trait: 'glow', property: 'intensity', value: 1 } },
    inputTokens: 100,
    outputTokens: 20,
  };
}
...
describe('verifyBrittneyCaelRecord', () => {
  ...
  it('is deterministic — same inputs reproduce the same chain (replayability)', () => {
    const input = {
      sessionId: 'exp1:t', round: 0, model: 'm', messages: [],
      finalText: 'out', toolCalls: [{ name: 'x', input: { a: 1 } }],
      evidencePaths: [], simContractCheck: { passed: false, constraints: ['c'] }, prevChain: null,
    };
...
describe('caelProvenance (C3 checker)', () => {
  ...
  it('reports no claim when the model emitted no mutation', async () => {
    const noMutation: ArmModelResult = { rawOutput: '', mutation: null, inputTokens: 5, outputTokens: 0 };
