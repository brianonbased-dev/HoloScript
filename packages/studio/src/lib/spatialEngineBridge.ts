...
export interface WasmModule {
  ...
  sphere_sphere_test(ax: number, ay: number, az: number, ar: number, bx: number, by: number, bz: number, br: number): number;
  aabb_overlap(aminX: number, aminY: number, aminZ: number, amaxX: number, amaxY: number, amaxZ: number, bminX: number, bminY: number, bminZ: number, bmaxX: number, bmaxY: number, bmaxZ: number): number;
  alloc(size: number): number;
  dealloc(ptr: number, size: number): void;
  astar_find_path(gridPtr: number, width: number, height: number, startX: number, startY: number, endX: number, endY: number, resultPtr: number): number;
...
export function getBridgeStatus(): BridgeStatus {
  return {
    ...
    moduleSize: 0,
    functions: ['perlinNoise2D', 'perlinNoise3D', 'fbmNoise', 'sphereSphereTest', 'aabbOverlap', 'astarFindPath'],
  };
}
...
export function sphereSphereTest(a: Sphere, b: Sphere): boolean {
  if (wasmModule) {
    return wasmModule.sphere_sphere_test(
...
export function aabbOverlap(a: AABB, b: AABB): boolean {
  if (wasmModule) {
    return wasmModule.aabb_overlap(
...
export function generateHeightmap(
  ...
  scale: number = 0.05, seed: number = 42
): number[][] {
  const map: number[][] = [];
