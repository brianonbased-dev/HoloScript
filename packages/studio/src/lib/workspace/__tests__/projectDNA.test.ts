...
function makeInput(
  ...
  return {
    ...
    leafFirstOrder: overrides.paths ?? ['src/index.ts', 'src/utils.ts', 'package.json'],
  };
}
...
describe('detectProjectDNA', () => {
  ...
  it('detects monorepo shape', () => {
    const dna = detectProjectDNA(
      makeInput({
        paths: ['packages/core/src/index.ts', 'packages/cli/src/main.ts', 'pnpm-workspace.yaml'],
      })
    );
