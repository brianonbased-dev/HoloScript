...
describe('createSecretGrant', () => {
  ...
  it('checks HoloDoor policy before issuing a brokered secret grant', () => {
  ...
  it('blocks a secret grant when HoloDoor policy denies the handle', () => {
    expect.assertions(4);
    try {
      createPolicyGatedSecretGrant(
        {
        ...
        {
          secretGrants: {
            allowedSecretRefPrefixes: ['secret://workspace/ws_octocat/holoscript/'],
            allowedCapabilityRefs: ['cap://daemon/secrets/broker-only'],
          },
          enforcement: { onViolation: 'block' },
        }
  ...
  it('warns and clamps TTL when HoloDoor policy allows a shorter grant', () => {
