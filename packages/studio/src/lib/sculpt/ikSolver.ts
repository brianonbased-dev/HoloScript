    ...
    for (let iter = 0; iter < this.maxIterations; iter++) {
      ...
      for (let i = this.joints.length - 1; i >= 0; i--) {
        ...
        _axisWorld
          ...
          .applyQuaternion(
            joint.mesh.parent
              ? joint.mesh.parent.getWorldQuaternion(new THREE.Quaternion())
              : new THREE.Quaternion()
          )
          .normalize();

        // Project vectors onto the plane normal to the rotation axis
        const t1 = _toEffector
          .clone()
          .sub(_axisWorld.clone().multiplyScalar(_toEffector.dot(_axisWorld)))
          .normalize();
        const t2 = _toTarget
          .clone()
          .sub(_axisWorld.clone().multiplyScalar(_toTarget.dot(_axisWorld)))
          .normalize();
