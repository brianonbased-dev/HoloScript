...
export function planetariumPath(
  ...
  for (let i = 0; i <= totalSamples; i++) {
    ...
    const alt =
      Math.sin((star.coord.dec * Math.PI) / 180) * Math.sin((latitude * Math.PI) / 180) +
      Math.cos((star.coord.dec * Math.PI) / 180) *
