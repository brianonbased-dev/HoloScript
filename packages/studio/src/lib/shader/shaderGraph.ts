...
const NODE_REGISTRY: Record<
  string,
  { label: string; inputs: IShaderPort[]; outputs: IShaderPort[] }
> = {
  // Math
...
export class ShaderGraph {
  ...
  createNode(type: string, position: { x: number; y: number } = { x: 0, y: 0 }): IShaderNode {
    ...
    const category = (() => {
      for (const [cat, templates] of Object.entries(NODE_REGISTRY)) {
        if (templates.label === def.label || cat === type.toLowerCase())
          return cat as ShaderNodeCategory;
      }
  ...
  addCustomNode(node: IShaderNode): void {
    this.nodes.set(node.id, {
      ...node,
      ports: node.ports ?? [...(node.inputs ?? []), ...(node.outputs ?? [])],
    });
    this.version++;
    this.updatedAt = Date.now();
  }
  ...
  getNodeConnections(nodeId: string): IShaderConnection[] {
    return this.connections.filter((c) => c.fromNodeId === nodeId || c.toNodeId === nodeId);
  }

  disconnectPort(nodeId: string, portName: string): void {
    this.connections = this.connections.filter(
      (c) =>
        !(c.toNodeId === nodeId && c.toPort === portName) &&
...
export const NODE_TEMPLATES: Record<ShaderNodeCategory, INodeTemplate[]> = {
  ...
  output: [
    ...
    },
    {
      ...
      name: 'Vertex Output',
      category: 'output',
      inputs: [{ name: 'gl_Position', type: 'vec4' }],
      outputs: [],
    },
  ],
  math: [
  ...
  volumetric: [
    {
      ...
      name: 'Fog',
      category: 'volumetric',
      inputs: [{ name: 'density', type: 'float' }],
