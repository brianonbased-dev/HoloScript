...
export function applyGrabBrush(positions: Float32Array, hitNormals: Float32Array, center: Vec3, radius: number, strength: number): Float32Array {
  const radiusSq = radius * radius;
  // Calculate average hit normal for the displacement direction
  let nx = 0, ny = 0, nz = 0;
  if (hitNormals && hitNormals.length >= 3) {
      nx = hitNormals[0]; ny = hitNormals[1]; nz = hitNormals[2];
  }
...
export function applySmoothBrush(positions: Float32Array, indices: number[], center: Vec3, radius: number, strength: number, iterations = 1): Float32Array {
  const radiusSq = radius * radius;
...
export function applyInflateBrush(positions: Float32Array, normals: Float32Array, center: Vec3, radius: number, strength: number): Float32Array {
  const radiusSq = radius * radius;
...
export function applyCreaseBrush(positions: Float32Array, center: Vec3, radius: number, strength: number, planeNormal: Vec3): Float32Array {
  const radiusSq = radius * radius;
  for (let i = 0; i < positions.length; i += 3) {
    const vx = positions[i];
    const vy = positions[i + 1];
