...
if (getEnvironment === undefined) {
  const err = new Error(
    "unexpectedly undefined local import 'getEnvironment', was 'getEnvironment' available at instantiation?"
  );
...
if (exit === undefined) {
  const err = new Error(
    "unexpectedly undefined local import 'exit', was 'exit' available at instantiation?"
  );
  console.error('ERROR:', err.toString());
  throw err;
}
...
if (getStderr === undefined) {
  const err = new Error(
    "unexpectedly undefined local import 'getStderr', was 'getStderr' available at instantiation?"
  );
  console.error('ERROR:', err.toString());
  throw err;
}
...
if (getStdin === undefined) {
  const err = new Error(
    "unexpectedly undefined local import 'getStdin', was 'getStdin' available at instantiation?"
  );
  console.error('ERROR:', err.toString());
  throw err;
}
...
if (getStdout === undefined) {
  const err = new Error(
    "unexpectedly undefined local import 'getStdout', was 'getStdout' available at instantiation?"
  );
  console.error('ERROR:', err.toString());
  throw err;
}
...
if (getDirectories === undefined) {
  const err = new Error(
    "unexpectedly undefined local import 'getDirectories', was 'getDirectories' available at instantiation?"
  );
  console.error('ERROR:', err.toString());
  throw err;
}
...
if (Descriptor === undefined) {
  const err = new Error(
    "unexpectedly undefined local import 'Descriptor', was 'Descriptor' available at instantiation?"
  );
  console.error('ERROR:', err.toString());
  throw err;
}
...
if (filesystemErrorCode === undefined) {
  const err = new Error(
    "unexpectedly undefined local import 'filesystemErrorCode', was 'filesystemErrorCode' available at instantiation?"
  );
  console.error('ERROR:', err.toString());
  throw err;
}
...
if (Error$1 === undefined) {
  const err = new Error(
    "unexpectedly undefined local import 'Error$1', was 'Error' available at instantiation?"
  );
  console.error('ERROR:', err.toString());
  throw err;
}
...
if (InputStream === undefined) {
  const err = new Error(
    "unexpectedly undefined local import 'InputStream', was 'InputStream' available at instantiation?"
  );
  console.error('ERROR:', err.toString());
  throw err;
}
...
if (OutputStream === undefined) {
  const err = new Error(
    "unexpectedly undefined local import 'OutputStream', was 'OutputStream' available at instantiation?"
  );
  console.error('ERROR:', err.toString());
  throw err;
}
...
class GlobalComponentAsyncLowers {
  ...
  static lookup(componentIdx, qualifiedImportFn) {
    ...
    //
    if (
      qualifiedImportFn.includes('[stream-write-') ||
      qualifiedImportFn.includes('[stream-read-')
    ) {
      return async (...args) => {
...
class GlobalAsyncParamLowers {
  ...
  static generateKey(args) {
    ...
    if (iface === undefined) {
      throw new TypeError('missing iface name');
    }
    if (fnName === undefined) {
...
function _prepareCall(
  ...
  if (currentCallerTask.componentIdx() !== callerInstanceIdx) {
    throw new Error(
      `task component idx [${currentCallerTask.componentIdx()}] !== [${callerInstanceIdx}] (callee ${calleeInstanceIdx})`
    );
  }
...
function _asyncStartCall(args, callee, paramCount, resultCount, flags) {
  ...
  const taskMeta = getCurrentTask(
    ASYNC_CURRENT_COMPONENT_IDXS.at(-1),
    ASYNC_CURRENT_TASK_IDS.at(-1)
  );
  if (!taskMeta) {
    throw new Error('invalid/missing current async task meta during prepare call');
  }
  ...
  if (!preparedTask) {
    throw new Error('unexpectedly missing task in task meta during prepare call');
  }
  ...
  if (paramCount !== params.length) {
    throw new Error(
      `unexpected callee param count [${params.length}], _asyncStartCall invocation expected [${paramCount}]`
    );
  }
  ...
  subtask.registerOnResolveHandler((res) => {
    ...
    if (subtaskCallMeta.returnFn) {
      _debugLog(
        '[_asyncStartCall()] return function present while ahndling subtask result, returning early (skipping lower)'
      );
      return;
    }
    ...
    if (!subtaskCallMeta.resultPtr) {
      _debugLog(
        '[_asyncStartCall()] no result ptr during subtask result handling, returning early (skipping lower)'
      );
      return;
    }
    ...
    if (!lowerFns || lowerFns.length === 0) {
      throw new Error(
        `missing result lower metadata for guest->guests call (subtask [${subtask.id()}])`
      );
    }

    if (lowerFns.length !== 1) {
      throw new Error(
        `only single result supported for guest->guest calls (subtask [${subtask.id()}])`
      );
    }
...
function getCurrentTask(componentIdx) {
  if (componentIdx === undefined || componentIdx === null) {
    throw new Error(
      'missing/invalid component instance index [' + componentIdx + '] while getting current task'
    );
  }
...
class AsyncTask {
  ...
  mayEnter(task) {
    ...
    if (!callingSyncExportWithSyncPending) {
      _debugLog('[AsyncTask#mayEnter()] disallowed due to sync export w/ sync pending', {
        taskID: this.#id,
      });
      return false;
    }
  ...
  async immediateSuspend(opts) {
    // NOTE: equivalent to thread.suspend()
  ...
  cancel() {
    ...
    if (!this.taskState() !== AsyncTask.State.CANCEL_DELIVERED) {
      throw new Error(
        `(component [${this.#componentIdx}]) task [${this.#id}] invalid task state for cancellation`
      );
    }
    ...
    if (this.needsExclusiveLock() && !state.isExclusivelyLocked()) {
      throw new Error(
        'task [' +
          ...
          '] exit: component [' +
          this.#componentIdx +
          '] should have been exclusively locked'
      );
    }
...
function _lowerFlatU32(ctx) {
  ...
  if (vals[0] > 4_294_967_295 || vals[0] < 0) {
...
function _lowerFlatU64(memory, vals, storagePtr, storageLen) {
  ...
  if (vals[0] > 18_446_744_073_709_551_615n || vals[0] < 0n) {
...
function _lowerFlatVariant(metadata, extra) {
  ...
  return function _lowerFlatVariantInner(ctx) {
    ...
    if (!variant) {
      throw new Error(
        `missing/invalid variant, no tag matches [${tag}] (options were ${variantMetas.map((vm) => vm.tag)})`
      );
    }
    ...
    let bytesWritten;
    let discriminantLowerArgs = {
      memory,
      realloc,
      vals: [variant.discriminant],
      storagePtr,
      componentIdx,
    };
...
class ComponentAsyncState {
  ...
  removeHandler(args) {
    ...
    this.#handlerMap.set(
      event,
      this.#handlerMap.get(event).filter((h) => h.id !== handlerID)
    );
  }
  ...
  getStreamEnd(args) {
    ...
    if (!tbl) {
      throw new Error(
        `missing stream table [${tableIdx}] in component [${this.#componentIdx}] while getting stream`
      );
    }
  ...
  removeStreamEnd(args) {
    ...
    if (!tbl) {
      throw new Error(
        `missing stream table [${tableIdx}] in component [${this.#componentIdx}] while removing stream end`
      );
    }
    ...
    if (!removed) {
      throw new Error(
        `missing stream [${streamIdx}] (table [${tableIdx}]) in component [${this.#componentIdx}] while removing stream end`
      );
    }
...
function trampoline4() {
  _debugLog(
    '[iface="wasi:cli/stderr@0.2.3", function="get-stderr"] [Instruction::CallInterface] (sync, @ enter)'
  );
  ...
  taskCreation: {
    ...
    if (isHostAsyncImport) {
      ...
      if (!subtask) {
        throw new Error(
          'Missing subtask for host import, has the import been lowered? (ensure asyncImports are set properly)'
        );
      }
...
function trampoline5() {
  _debugLog(
    '[iface="wasi:cli/stdin@0.2.3", function="get-stdin"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
...
function trampoline6() {
  _debugLog(
    '[iface="wasi:cli/stdout@0.2.3", function="get-stdout"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
...
function trampoline7(arg0) {
  ...
  _debugLog(
    '[iface="wasi:cli/exit@0.2.3", function="exit"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
...
function trampoline8(arg0) {
  _debugLog(
    '[iface="wasi:cli/environment@0.2.3", function="get-environment"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
  ...
  _debugLog(
    '[iface="wasi:cli/environment@0.2.3", function="get-environment"][Instruction::Return]',
    {
...
function trampoline9(arg0, arg1) {
  ...
  _debugLog(
    '[iface="wasi:filesystem/types@0.2.3", function="filesystem-error-code"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
  ...
  _debugLog(
    '[iface="wasi:filesystem/types@0.2.3", function="filesystem-error-code"][Instruction::Return]',
    {
...
function trampoline10(arg0, arg1, arg2) {
  ...
  curResourceBorrows.push(rsc0);
  _debugLog(
    '[iface="wasi:filesystem/types@0.2.3", function="[method]descriptor.write-via-stream"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
  ...
  _debugLog(
    '[iface="wasi:filesystem/types@0.2.3", function="[method]descriptor.write-via-stream"][Instruction::Return]',
    {
...
function trampoline11(arg0, arg1) {
  ...
  curResourceBorrows.push(rsc0);
  _debugLog(
    '[iface="wasi:filesystem/types@0.2.3", function="[method]descriptor.append-via-stream"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
  ...
  _debugLog(
    '[iface="wasi:filesystem/types@0.2.3", function="[method]descriptor.append-via-stream"][Instruction::Return]',
    {
...
function trampoline12(arg0, arg1) {
  ...
  curResourceBorrows.push(rsc0);
  _debugLog(
    '[iface="wasi:filesystem/types@0.2.3", function="[method]descriptor.get-type"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
  ...
  _debugLog(
    '[iface="wasi:filesystem/types@0.2.3", function="[method]descriptor.get-type"][Instruction::Return]',
    {
...
function trampoline13(arg0, arg1) {
  ...
  curResourceBorrows.push(rsc0);
  _debugLog(
    '[iface="wasi:filesystem/types@0.2.3", function="[method]descriptor.stat"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
  ...
  _debugLog(
    '[iface="wasi:filesystem/types@0.2.3", function="[method]descriptor.stat"][Instruction::Return]',
    {
...
function trampoline14(arg0, arg1) {
  ...
  curResourceBorrows.push(rsc0);
  _debugLog(
    '[iface="wasi:io/streams@0.2.3", function="[method]output-stream.check-write"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
  ...
  _debugLog(
    '[iface="wasi:io/streams@0.2.3", function="[method]output-stream.check-write"][Instruction::Return]',
    {
...
function trampoline15(arg0, arg1, arg2, arg3) {
  ...
  _debugLog(
    '[iface="wasi:io/streams@0.2.3", function="[method]output-stream.write"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
  ...
  _debugLog(
    '[iface="wasi:io/streams@0.2.3", function="[method]output-stream.write"][Instruction::Return]',
    {
...
function trampoline16(arg0, arg1) {
  ...
  curResourceBorrows.push(rsc0);
  _debugLog(
    '[iface="wasi:io/streams@0.2.3", function="[method]output-stream.blocking-flush"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
  ...
  _debugLog(
    '[iface="wasi:io/streams@0.2.3", function="[method]output-stream.blocking-flush"][Instruction::Return]',
    {
...
function trampoline17(arg0, arg1, arg2, arg3) {
  ...
  var result3 = new Uint8Array(memory0.buffer.slice(ptr3, ptr3 + len3 * 1));
  _debugLog(
    '[iface="wasi:io/streams@0.2.3", function="[method]output-stream.blocking-write-and-flush"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
  ...
  _debugLog(
    '[iface="wasi:io/streams@0.2.3", function="[method]output-stream.blocking-write-and-flush"][Instruction::Return]',
    {
...
function trampoline18(arg0) {
  _debugLog(
    '[iface="wasi:filesystem/preopens@0.2.3", function="get-directories"] [Instruction::CallInterface] (sync, @ enter)'
  );
  let hostProvided = false;
  ...
  dataView(memory0).setUint32(arg0 + 0, result3, true);
  _debugLog(
    '[iface="wasi:filesystem/preopens@0.2.3", function="get-directories"][Instruction::Return]',
    {
...
GlobalComponentAsyncLowers.define({
  ...
  fn: _lowerImport.bind(null, {
    ...
    resultLowerFns: [
      _lowerFlatResult([
        { discriminant: 0, tag: 'ok', lowerFn: _lowerFlatOwn.bind(null, 1), align32: 4 },
        { discriminant: 1, tag: 'error', lowerFn: _lowerFlatEnum.bind(null, 2), align32: 1 },
      ]),
    ],
    getCallbackFn: () => null,
    getPostReturnFn: () => null,
...
GlobalComponentAsyncLowers.define({
  ...
  fn: _lowerImport.bind(null, {
    ...
    resultLowerFns: [
      _lowerFlatResult([
        { discriminant: 0, tag: 'ok', lowerFn: _lowerFlatEnum.bind(null, 3), align32: 1 },
        { discriminant: 1, tag: 'error', lowerFn: _lowerFlatEnum.bind(null, 2), align32: 1 },
      ]),
    ],
    getCallbackFn: () => null,
      ...
      _lowerFlatResult([
        {
          discriminant: 0,
          tag: 'ok',
          lowerFn: _lowerFlatRecord([
            { field: 'type', lowerFn: _lowerFlatEnum.bind(null, 3), align32: 8 },
            { field: 'link-count', lowerFn: _lowerFlatU64, align32: 8 },
            { field: 'size', lowerFn: _lowerFlatU64, align32: 8 },
            { field: 'data-access-timestamp', lowerFn: _lowerFlatOption.bind(null, 4), align32: 8 },
            {
              field: 'data-modification-timestamp',
              lowerFn: _lowerFlatOption.bind(null, 4),
              align32: 8,
            },
            {
...
GlobalComponentAsyncLowers.define({
  ...
  fn: _lowerImport.bind(null, {
    ...
    resultLowerFns: [
      _lowerFlatResult([
        { discriminant: 0, tag: 'ok', lowerFn: _lowerFlatU64, align32: 8 },
        {
          discriminant: 1,
          tag: 'error',
          lowerFn: _lowerFlatVariant({
            discriminantSizeBytes: 1,
            lowerMetas: [
              {
                discriminant: 0,
                tag: 'last-operation-failed',
                lowerFn: _lowerFlatOwn.bind(null, 0),
                align32: 4,
              },
              { discriminant: 1, tag: 'closed', lowerFn: null, align32: null },
            ],
          }),
          align32: 4,
        },
...
GlobalComponentAsyncLowers.define({
  ...
  fn: _lowerImport.bind(null, {
    ...
    resultLowerFns: [
      _lowerFlatResult([
        { discriminant: 0, tag: 'ok', lowerFn: null, align32: null },
        {
          discriminant: 1,
          tag: 'error',
          lowerFn: _lowerFlatVariant({
...
function parse(arg0) {
  ...
  _debugLog(
    '[iface="holoscript:core/parser@1.0.0", function="parse"][Instruction::CallWasm] enter',
    {
      funcName: 'parse',
      paramCount: 2,
      async: false,
          ...
          for (let i = 0; i < len67; i++) {
            ...
            switch (dataView(memory0).getUint8(base + 8, true)) {
              case 0: {
                ...
                var result61 = TEXT_DECODER_UTF8.decode(
                  new Uint8Array(memory0.buffer, ptr61, len61)
                );
                variant65 = {
                  tag: 'string-val',
                  val: result61,
              ...
              case 3: {
                ...
                var result63 = TEXT_DECODER_UTF8.decode(
                  new Uint8Array(memory0.buffer, ptr63, len63)
                );
                variant65 = {
                  tag: 'array-val',
                  val: result63,
              ...
              case 4: {
                ...
                var result64 = TEXT_DECODER_UTF8.decode(
                  new Uint8Array(memory0.buffer, ptr64, len64)
                );
                variant65 = {
                  tag: 'object-val',
                  val: result64,
        ...
        for (let i = 0; i < len108; i++) {
          ...
          switch (dataView(memory0).getUint8(base + 8, true)) {
            case 0: {
              ...
              var result102 = TEXT_DECODER_UTF8.decode(
                new Uint8Array(memory0.buffer, ptr102, len102)
              );
              variant106 = {
                tag: 'string-val',
                val: result102,
            ...
            case 3: {
              ...
              var result104 = TEXT_DECODER_UTF8.decode(
                new Uint8Array(memory0.buffer, ptr104, len104)
              );
              variant106 = {
                tag: 'array-val',
                val: result104,
            ...
            case 4: {
              ...
              var result105 = TEXT_DECODER_UTF8.decode(
                new Uint8Array(memory0.buffer, ptr105, len105)
              );
              variant106 = {
                tag: 'object-val',
                val: result105,
      ...
      for (let i = 0; i < len116; i++) {
        ...
        switch (dataView(memory0).getUint8(base + 8, true)) {
          ...
          case 1: {
            ...
            var result112 = TEXT_DECODER_UTF8.decode(
              new Uint8Array(memory0.buffer, ptr112, len112)
            );
      ...
      for (let i = 0; i < len123; i++) {
        ...
        switch (dataView(memory0).getUint8(base + 40, true)) {
          ...
          case 1: {
            ...
            var result121 = TEXT_DECODER_UTF8.decode(
              new Uint8Array(memory0.buffer, ptr121, len121)
            );
...
function parseHeader(arg0) {
  ...
  _debugLog(
    '[iface="holoscript:core/parser@1.0.0", function="parse-header"][Instruction::CallWasm] enter',
    {
  ...
  _debugLog(
    '[iface="holoscript:core/parser@1.0.0", function="parse-header"][Instruction::Return]',
    {
      funcName: 'parse-header',
      paramCount: 1,
      async: false,
...
function validate(arg0) {
  ...
  _debugLog(
    '[iface="holoscript:core/validator@1.0.0", function="validate"][Instruction::CallWasm] enter',
    {
      funcName: 'validate',
      paramCount: 2,
      async: false,
...
function traitExists(arg0) {
  ...
  _debugLog(
    '[iface="holoscript:core/validator@1.0.0", function="trait-exists"][Instruction::CallWasm] enter',
    {
  ...
  var bool1 = ret;
  _debugLog(
    '[iface="holoscript:core/validator@1.0.0", function="trait-exists"][Instruction::Return]',
    {
      funcName: 'trait-exists',
      paramCount: 1,
      async: false,
...
function getTrait(arg0) {
  ...
  _debugLog(
    '[iface="holoscript:core/validator@1.0.0", function="get-trait"][Instruction::CallWasm] enter',
    {
      funcName: 'get-trait',
      paramCount: 2,
      async: false,
  ...
  _debugLog(
    '[iface="holoscript:core/validator@1.0.0", function="get-trait"][Instruction::Return]',
    {
      funcName: 'get-trait',
      paramCount: 1,
      async: false,
...
function listTraits() {
  _debugLog(
    '[iface="holoscript:core/validator@1.0.0", function="list-traits"][Instruction::CallWasm] enter',
    {
  ...
  _debugLog(
    '[iface="holoscript:core/validator@1.0.0", function="list-traits"][Instruction::Return]',
    {
      funcName: 'list-traits',
      paramCount: 1,
      async: false,
...
function listTraitsByCategory(arg0) {
  ...
  _debugLog(
    '[iface="holoscript:core/validator@1.0.0", function="list-traits-by-category"][Instruction::CallWasm] enter',
    {
  ...
  _debugLog(
    '[iface="holoscript:core/validator@1.0.0", function="list-traits-by-category"][Instruction::Return]',
    {
      funcName: 'list-traits-by-category',
      paramCount: 1,
      async: false,
...
function compile(arg0, arg1) {
  ...
  _debugLog(
    '[iface="holoscript:core/compiler@1.0.0", function="compile"][Instruction::CallWasm] enter',
    {
      funcName: 'compile',
      paramCount: 3,
      async: false,
...
function compileAst(arg0, arg1) {
  ...
  if (variant19 === null || variant19 === undefined) {
    ...
    for (let i = 0; i < vec14.length; i++) {
      const e = vec14[i];
      const base = result14 + i * 56;
      var { name: v4_0, value: v4_1, span: v4_2 } = e;
      ...
      switch (variant9.tag) {
        ...
        default: {
          throw new TypeError(
            `invalid variant tag value \`${JSON.stringify(variant9.tag)}\` (received \`${variant9}\`) specified for \`PropertyValue\``
          );
        }
      }
      var variant13 = v4_2;
  ...
  for (let i = 0; i < vec60.length; i++) {
    ...
    for (let i = 0; i < vec34.length; i++) {
      const e = vec34[i];
      const base = result34 + i * 56;
      var { name: v24_0, value: v24_1, span: v24_2 } = e;
      ...
      switch (variant29.tag) {
        ...
        default: {
          throw new TypeError(
            `invalid variant tag value \`${JSON.stringify(variant29.tag)}\` (received \`${variant29}\`) specified for \`PropertyValue\``
          );
        }
      }
      var variant33 = v24_2;
    ...
    for (let i = 0; i < vec45.length; i++) {
      const e = vec45[i];
      const base = result45 + i * 56;
      var { name: v35_0, value: v35_1, span: v35_2 } = e;
      ...
      switch (variant40.tag) {
        ...
        default: {
          throw new TypeError(
            `invalid variant tag value \`${JSON.stringify(variant40.tag)}\` (received \`${variant40}\`) specified for \`PropertyValue\``
          );
        }
      }
      var variant44 = v35_2;
    ...
    for (let i = 0; i < vec55.length; i++) {
      const e = vec55[i];
      const base = result55 + i * 52;
      var { name: v46_0, parameters: v46_1, body: v46_2, span: v46_3 } = e;
  ...
  for (let i = 0; i < vec82.length; i++) {
    const e = vec82[i];
    const base = result82 + i * 64;
    var { name: v61_0, template: v61_1, traits: v61_2, properties: v61_3, span: v61_4 } = e;
    ...
    for (let i = 0; i < vec77.length; i++) {
      const e = vec77[i];
      const base = result77 + i * 56;
      var { name: v67_0, value: v67_1, span: v67_2 } = e;
      ...
      switch (variant72.tag) {
        ...
        default: {
          throw new TypeError(
            `invalid variant tag value \`${JSON.stringify(variant72.tag)}\` (received \`${variant72}\`) specified for \`PropertyValue\``
          );
        }
      }
      var variant76 = v67_2;
  ...
  for (let i = 0; i < vec111.length; i++) {
    const e = vec111[i];
    const base = result111 + i * 44;
    var { name: v83_0, objects: v83_1, span: v83_2 } = e;
    ...
    for (let i = 0; i < vec106.length; i++) {
      const e = vec106[i];
      const base = result106 + i * 64;
      var { name: v85_0, template: v85_1, traits: v85_2, properties: v85_3, span: v85_4 } = e;
      ...
      for (let i = 0; i < vec101.length; i++) {
        const e = vec101[i];
        const base = result101 + i * 56;
        var { name: v91_0, value: v91_1, span: v91_2 } = e;
        ...
        switch (variant96.tag) {
          ...
          default: {
            throw new TypeError(
              `invalid variant tag value \`${JSON.stringify(variant96.tag)}\` (received \`${variant96}\`) specified for \`PropertyValue\``
            );
          }
        }
  ...
  for (let i = 0; i < vec135.length; i++) {
    const e = vec135[i];
    const base = result135 + i * 44;
    var { name: v125_0, entries: v125_1, span: v125_2 } = e;
    ...
    for (let i = 0; i < vec130.length; i++) {
      const e = vec130[i];
      const base = result130 + i * 24;
      var { time: v127_0, target: v127_1, action: v127_2 } = e;
  ...
  for (let i = 0; i < vec154.length; i++) {
    const e = vec154[i];
    const base = result154 + i * 52;
    var { lightType: v136_0, name: v136_1, properties: v136_2, span: v136_3 } = e;
    ...
    for (let i = 0; i < vec149.length; i++) {
      const e = vec149[i];
      const base = result149 + i * 56;
      var { name: v139_0, value: v139_1, span: v139_2 } = e;
      ...
      switch (variant144.tag) {
        ...
        default: {
          throw new TypeError(
            `invalid variant tag value \`${JSON.stringify(variant144.tag)}\` (received \`${variant144}\`) specified for \`PropertyValue\``
          );
        }
      }
  ...
  for (let i = 0; i < vec173.length; i++) {
    const e = vec173[i];
    const base = result173 + i * 52;
    var { cameraType: v155_0, name: v155_1, properties: v155_2, span: v155_3 } = e;
    ...
    for (let i = 0; i < vec168.length; i++) {
      const e = vec168[i];
      const base = result168 + i * 56;
      var { name: v158_0, value: v158_1, span: v158_2 } = e;
      ...
      switch (variant163.tag) {
        ...
        default: {
          throw new TypeError(
            `invalid variant tag value \`${JSON.stringify(variant163.tag)}\` (received \`${variant163}\`) specified for \`PropertyValue\``
          );
        }
      }
  ...
  for (let i = 0; i < vec183.length; i++) {
    const e = vec183[i];
    const base = result183 + i * 56;
    var { eventType: v174_0, target: v174_1, body: v174_2, span: v174_3 } = e;
  ...
  _debugLog(
    '[iface="holoscript:core/compiler@1.0.0", function="compile-ast"][Instruction::CallWasm] enter',
    {
      ...
      for (let i = 0; i < len196; i++) {
        ...
        switch (dataView(memory0).getUint8(base + 40, true)) {
          ...
          case 1: {
            ...
            var result194 = TEXT_DECODER_UTF8.decode(
              new Uint8Array(memory0.buffer, ptr194, len194)
            );
  ...
  _debugLog(
    '[iface="holoscript:core/compiler@1.0.0", function="compile-ast"][Instruction::Return]',
    {
      funcName: 'compile-ast',
      paramCount: 1,
      async: false,
...
function listTargets() {
  _debugLog(
    '[iface="holoscript:core/compiler@1.0.0", function="list-targets"][Instruction::CallWasm] enter',
    {
  ...
  _debugLog(
    '[iface="holoscript:core/compiler@1.0.0", function="list-targets"][Instruction::Return]',
    {
      funcName: 'list-targets',
      paramCount: 1,
      async: false,
...
function generateObject(arg0) {
  ...
  _debugLog(
    '[iface="holoscript:core/generator@1.0.0", function="generate-object"][Instruction::CallWasm] enter',
    {
  ...
  _debugLog(
    '[iface="holoscript:core/generator@1.0.0", function="generate-object"][Instruction::Return]',
    {
      funcName: 'generate-object',
      paramCount: 1,
      async: false,
...
function generateScene(arg0) {
  ...
  _debugLog(
    '[iface="holoscript:core/generator@1.0.0", function="generate-scene"][Instruction::CallWasm] enter',
    {
  ...
  _debugLog(
    '[iface="holoscript:core/generator@1.0.0", function="generate-scene"][Instruction::Return]',
    {
      funcName: 'generate-scene',
      paramCount: 1,
      async: false,
...
function suggestTraits(arg0) {
  ...
  _debugLog(
    '[iface="holoscript:core/generator@1.0.0", function="suggest-traits"][Instruction::CallWasm] enter',
    {
  ...
  _debugLog(
    '[iface="holoscript:core/generator@1.0.0", function="suggest-traits"][Instruction::Return]',
    {
      funcName: 'suggest-traits',
      paramCount: 1,
      async: false,
...
const $init = (() => {
  let gen = (function* _initGenerator() {
    ...
    const module2 = base64Compile(
      ''
    );
    const module3 = base64Compile(
      ''
    );
    ...
    validator100ListTraitsByCategory =
      exports1['holoscript:core/validator@1.0.0#list-traits-by-category'];
...
export {
  compiler100 as compiler,
  generator100 as generator,
  parser100 as parser,
  validator100 as validator,
  compiler100 as 'holoscript:core/compiler@1.0.0',
  generator100 as 'holoscript:core/generator@1.0.0',
  parser100 as 'holoscript:core/parser@1.0.0',
  validator100 as 'holoscript:core/validator@1.0.0',
};
