...
import { SybilAttack, SYBIL_DEFAULT_INFLATION_FACTOR, type SybilConfig } from '../src/sybil.js';
...
describe('SybilAttack — constructor validation', () => {
  it('rejects K < 2 (single-identity is not Sybil)', () => {
    expect(() => new SybilAttack({ K: 1, compoundRounds: 10, baselineTrust: 0.2 })).toThrow(
      RangeError
    );
    expect(() => new SybilAttack({ K: 0, compoundRounds: 10, baselineTrust: 0.2 })).toThrow(
      RangeError
    );
    expect(() => new SybilAttack({ K: -1, compoundRounds: 10, baselineTrust: 0.2 })).toThrow(
      RangeError
  ...
  it('accepts spec-defined K values {2, 5, 10}', () => {
    for (const K of [2, 5, 10] as const) {
      expect(() => new SybilAttack({ K, compoundRounds: 10, baselineTrust: 0.2 })).not.toThrow();
    }
  });

  it('rejects non-integer K', () => {
    expect(() => new SybilAttack({ K: 2.5, compoundRounds: 10, baselineTrust: 0.2 })).toThrow(
      RangeError
    );
  });

  it('rejects non-positive-integer compoundRounds', () => {
    expect(() => new SybilAttack({ K: 2, compoundRounds: 0, baselineTrust: 0.2 })).toThrow(
      RangeError
    );
    expect(() => new SybilAttack({ K: 2, compoundRounds: 1.5, baselineTrust: 0.2 })).toThrow(
      RangeError
    );
  });

  it('rejects baselineTrust <= 0 (success threshold would degenerate)', () => {
    expect(() => new SybilAttack({ K: 2, compoundRounds: 10, baselineTrust: 0 })).toThrow(
      RangeError
    );
    expect(() => new SybilAttack({ K: 2, compoundRounds: 10, baselineTrust: -0.1 })).toThrow(
      RangeError
    );
  });
...
describe('SybilAttack.step — cross-vouching every round (§4.2 step 2)', () => {
  ...
  it('K=2 emits 1 vouch (the other server); K=10 emits 9 vouches', () => {
    ...
    expect(attackK10.step(mockContext([0.2]), 1).notes).toBe('phase=cross-vouching, vouches=9');
  });
