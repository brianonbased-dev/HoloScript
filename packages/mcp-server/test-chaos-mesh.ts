...
async function runChaosTest() {
    ...
    console.log("   - Server actively drifted entity while partitioned to:", serverState.position.value);
    ...
    console.log("   - Server receives T=1500 Packet delayed due to Chaos Mesh. Value:", serverState.position.value);
    ...
    if (finalPos.x === 20 && finalPos.y === 5 && serverState.position.timestamp === 2000) {
        ...
        console.log("  - The T=1500 packet was properly rejected because T=2000 already mutated the LWW boundary natively.");
    } else {
