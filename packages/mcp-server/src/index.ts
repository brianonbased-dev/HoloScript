...
async function handleOracleConsult(args: Record<string, unknown>): Promise<{ content: Array<{ type: string; text: string }> }> {
  ...
  if (apiKey) {
    ...
    } catch { /* timeout or network — continue without knowledge */ }
  }
