...
const HOLOSCRIPT_GATES: Record<string, GateSpec> = {
  ...
  'doctrine-slots': {
    description:
      'Doctrine-slot falsifier: required local HoloCI breadcrumb slots must be non-null so hook registration cannot masquerade as execution',
    step: 'HOLOCI_ALLOW_MISSING_WORKLOAD=1 pnpm verify:doctrine',
    profiles: ['quick', 'full'],
    resource_requirements: { max_dph: 0.2 },
  },
 ...
 */
export async function submitWorkload(workload: HoloCiWorkload['workload']): Promise<{
  ok: boolean;
  workloadId?: string;
