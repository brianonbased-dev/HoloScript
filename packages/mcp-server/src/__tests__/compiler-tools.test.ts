...
describe('compiler tools', () => {
  ...
  it('compiles R3F through generated SceneIR TSX instead of handwritten JSX', async () => {
    const result = (await handleCompilerTool('compile_to_r3f', {
      code: robotComposition,
      options: { componentName: 'RobotScene', sourcePath: 'robot.holo' },
    })) as {
      success?: boolean;
      output?: string;
      target?: string;
    ...
    expect(result.output).toContain('@generated by HoloScript SceneIRTsxEmitter');
    expect(result.output).toContain("import { HoloSceneIRRenderer } from '@holoscript/r3f-renderer';");
    expect(result.output).toContain('<HoloSceneIRRenderer node={RobotSceneSceneIR} />');
    expect(result.output).toContain('"id": "base_link"');
    expect(result.output).not.toContain('<mesh');
  });

  it('can return raw SceneIR through compile_to_r3f', async () => {
    const result = (await handleCompilerTool('compile_to_r3f', {
      code: robotComposition,
      options: { format: 'scene-ir' },
    })) as {
      success?: boolean;
      output?: string;
      target?: string;
    ...
    expect(result.success).toBe(true);
    expect(result.target).toBe('r3f');
    const scene = JSON.parse(result.output ?? '{}') as {
      type?: string;
      children?: Array<{ id?: string; type?: string }>;
    };
    expect(scene.type).toBe('group');
    expect(scene.children?.some((child) => child.id === 'base_link')).toBe(true);
  });
  ...
  it('compiles a typed TypeScript SDK from a service contract through its convenience tool', async () => {
    ...
    expect(files['src/RegistryClient.ts']).toContain(
