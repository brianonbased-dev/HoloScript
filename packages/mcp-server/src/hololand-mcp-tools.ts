...
// =============================================================================
// TWIN EARTH ROBOT / AI SOVEREIGN HANDLERS (task_1778618552503_a6rb)
// =============================================================================
...
async function handleTwinEarthUpdateIdentity(
  ...
  if (identity.revoked) {
    return { error: `Identity ${agentId} is revoked and cannot be updated.` };
  }

  if (args.handle !== undefined) identity.handle = args.handle as string;
  if (args.role !== undefined) identity.role = args.role as string;
  if (args.mode !== undefined) identity.mode = args.mode as string;
  if (args.brainCompositionId !== undefined) {
    identity.brainCompositionId = args.brainCompositionId as string;
  }
...
async function handleTwinEarthRevokeIdentity(
  ...
  if (!granter || (granter.role !== 'founder' && granter.role !== 'steward')) {
    return { error: `granterId must be a founder or steward. ${granterId} is ${granter?.role || 'unknown'}.` };
  }

  identity.revoked = true;
  identity.revokedAt = new Date().toISOString();

  // Invalidate all safety envelopes for this identity
  for (const envelope of safetyEnvelopeRegistry.values()) {
    if (envelope.agentId === agentId) {
      envelope.allowedActions = [];
      envelope.blockedActions = [];
    }
  }
  ...
  return {
    ...
    granterId,
    revocationSignature,
    note: 'All active safety envelopes for this identity have been invalidated.',
  };
}
...
async function handleTwinEarthDeleteSafetyEnvelope(
  ...
  return {
    ...
    envelopeId,
    deleted: true,
    note: 'Participant is now subject to the default restrictive envelope.',
  };
}
...
async function handleTwinEarthGrantPermission(
  ...
  if (grantee.revoked) {
    return { error: `Grantee identity ${granteeId} is revoked.` };
  }

  const granter = twinEarthIdentityRegistry.get(granterId);
  if (!granter || (granter.role !== 'founder' && granter.role !== 'steward' && granter.role !== 'operator')) {
    return { error: `granterId must be founder, steward, or operator. ${granterId} is ${granter?.role || 'unknown'}.` };
  }

  const canonical = `${granteeId}:${granterId}:${action}:${scope}:${expiresAt || 'never'}`;
  const hash = await simpleHash(`permission:${canonical}`);
...
async function handleTwinEarthRevokePermission(
  ...
  return {
    ...
    revoked: true,
    revocationSignature,
    note: 'Permission grant is now revoked. The grant record is retained for audit.',
  };
}
...
async function handleTwinEarthValidatePermission(
  ...
  const grantee = twinEarthIdentityRegistry.get(granteeId);
  if (!grantee) {
    return { error: `Identity not found: ${granteeId}`, valid: false };
  }
  if (grantee.revoked) {
    return { error: `Identity ${granteeId} is revoked.`, valid: false };
  }

  // Founder and steward bypass permission checks
  if (grantee.role === 'founder' || grantee.role === 'steward') {
    return { success: true, valid: true, granteeId, action, scope, bypass: true };
  }

  const matching = Array.from(permissionGrantRegistry.values()).filter(
    (g) =>
  ...
  if (matching.length === 0) {
    return { success: true, valid: false, granteeId, action, scope, note: 'No matching unrevoked, unexpired permission grant found.' };
  }

  return { success: true, valid: true, granteeId, action, scope, grants: matching.map((g) => g.hash) };
}
...
async function handleTwinEarthAIInvoke(
  ...
  if (envelope.maxTickDurationMs < 100) {
    return {
      error: `Safety envelope ${envelope.id} maxTickDurationMs (${envelope.maxTickDurationMs}ms) is too low for AI invocation.`,
      rejectedByEnvelope: true,
    };
  }
  ...
  if (envelope.localOnly) {
    return {
      error: 'AI invocation is blocked: localOnly safety envelope prohibits outbound model calls.',
      rejectedByEnvelope: true,
    };
  }

  // Check memory budget
  if (envelope.maxMemoryBytes < 1048576) {
    return {
      error: `Safety envelope ${envelope.id} maxMemoryBytes (${envelope.maxMemoryBytes}) is below minimum for AI invocation.`,
      rejectedByEnvelope: true,
    };
  }
  ...
  if (permResult.valid !== true) {
    return {
      error: `Permission denied for ai:invoke in scope ${shardId || '*'}.`,
      permissionDenied: true,
    };
  }

  // Simulate AI response (sovereign fallback if no model available)
  const receiptId = genId('ai');
  const simulatedResponse = `AI ${identity.handle} processed: "${prompt.slice(0, 40)}..." (maxTokens=${maxTokens})`;
  ...
  const receipt: StoredTwinEarthReceipt = {
    ...
    timestamp: new Date().toISOString(),
    status: 'success',
    hash: await simpleHash(`ai:${agentId}:${prompt}:${Date.now()}`),
    envelopeId: envelope.id,
  };
  ...
  return {
    ...
    status: 'success',
    simulated: true,
    note: 'AI response is simulated (canary). In production this routes to the configured model provider.',
  };
}
...
async function handleTwinEarthCaptureReceipt(
  ...
  const receipt: StoredTwinEarthReceipt = {
    ...
    timestamp: new Date().toISOString(),
    status,
    hash: await simpleHash(`receipt:${actorId}:${action}:${status}:${Date.now()}`),
    envelopeId,
    payloadHash,
  };
