...
/**
 * Generate a self-contained HTML page that boots the sovereign WebGPU renderer.
 *
 * `compiledCode` is the raw output of WebGPUCompiler — inline JS/TS that uses
 * the native WebGPU API (device init, WGSL shaders, render loop).  We strip any
 * TypeScript-only syntax (type annotations) before embedding so the page works
 * in a plain browser script tag without a build step.
 *
 * This is the render-surface the task "Wire WebGPURenderer into preview/deploy
 * path" targets — replacing the Three.js shim with the sovereign GPU path.
 */
export function generateWebGPUBrowserTemplate(compiledCode: string, title: string): string {
  // Strip lightweight TypeScript-only constructs so the snippet runs as plain JS.
  // WebGPUCompiler emits TypeScript type annotations (`: GPUDevice`, `as HTMLCanvasElement`,
  // etc.) that a browser <script> cannot parse. Strip them with a best-effort regex
  // pass — good enough for the generated output's constrained grammar; a full TS
  // transpile step is not available server-side here.
...
<head>
  ...
  <title>${safeTitle} — HoloScript WebGPU</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    #hs-renderer-badge {
      ...
      font: 11px/1.4 monospace; color: rgba(255,255,255,0.45);
...
<body>
  ...
  <script type="module">
    ...
    // Auto-generated by HoloScript WebGPUCompiler.  Do not edit manually.
    // Three.js / Babylon / R3F are NOT used here — this page runs entirely on
    // the native WebGPU path.
