...
async function runMemoryScorerTest() {
    ...
    const scores = events.map(e => ({ action: e.action, score: MemoryScorer.computeImportance(e) }));
    ...
    if (culledActions.includes("Craft_Sword") && culledActions.includes("Combat_Attack") && !culledActions.includes("Idle") && !culledActions.includes("Wait")) {
