...
export function resolveWebSurfaceConfig(node: R3FNode): Record<string, unknown> | null {
  const trait = node.traits?.get('web_surface') as Record<string, unknown> | undefined;
  if (trait) return trait;
  const p = node.props;
  if (p.webSurface && typeof p.webSurface === 'object')
    return p.webSurface as Record<string, unknown>;
  return null;
}
...
export function isBatchableDraftMesh(node: R3FNode): boolean {
  return (
    node.type === 'mesh' && node.assetMaturity === 'draft' && resolveGaussianSplatSrc(node) === null
  );
}
