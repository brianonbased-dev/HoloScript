...
describe('Dumb Glass Rendering Contract (P3-CENTER)', () => {
  test('Scene-graph and Pixel-chunk provenance hashing overhead (Multi-Run)', async () => {
    ...
    console.log(
      `Scene-Graph Hash (750KB) per frame: ${sgStats.median.toFixed(3)} ms (median), ${sgStats.p99.toFixed(3)} ms (p99)`
    );
    console.log(
      `Pixel-Tile Hash (256KB) per tile: ${ptStats.median.toFixed(3)} ms (median), ${ptStats.p99.toFixed(3)} ms (p99)`
    );
    console.log(
      `Total Provenance Overhead per Frame: ${frameStats.median.toFixed(3)} ms (median), ${frameStats.p99.toFixed(3)} ms (p99)`
    );
    console.log(
