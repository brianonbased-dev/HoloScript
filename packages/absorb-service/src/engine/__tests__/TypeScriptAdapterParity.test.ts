...
const EXPECTED_SYMBOLS: ExternalSymbolDefinition[] = [
  ...
  },
  {
    ...
    endColumn: 1,
    visibility: 'public',
    signature: 'function topLevel((a: T, b: number)): : string',
    isExported: true,
    lineCount: 5,
  },
  {
  ...
  },
  {
    ...
    endColumn: 2,
    visibility: 'public',
    signature: 'const arrow = ((x: number)) => ...',
    isExported: true,
    lineCount: 4,
  },
  {
  ...
  },
  {
    ...
    endColumn: 3,
    visibility: 'public',
    signature: 'Widget.constructor((name: string))',
    owner: 'Widget',
    lineCount: 3,
  },
  {
  ...
  },
  {
    ...
    endColumn: 3,
    visibility: 'private',
    signature: 'Widget.compute((a: number, b: number))',
    owner: 'Widget',
    lineCount: 3,
  },
  {
  ...
  },
  {
    ...
    endColumn: 3,
    visibility: 'public',
    signature: 'Widget.label((v: string))',
    owner: 'Widget',
    lineCount: 3,
  },
  {
...
const EXPECTED_IMPORTS: ImportEdge[] = [
  {
    fromFile: 'sample.ts',
    toModule: './default-mod',
    line: 2,
    namedImports: ['defaultExport'],
    isWildcard: false,
    isDefault: true,
  },
  {
  ...
  {
    fromFile: 'sample.ts',
    toModule: './type-only-mod',
    line: 7,
    namedImports: ['OnlyType'],
    isWildcard: false,
    isDefault: false,
  },
  {
    ...
    toModule: './mixed-mod',
    line: 8,
    namedImports: ['defaultAnd', 'alsoNamed'],
    isWildcard: false,
    isDefault: true,
  },
...
beforeAll(async () => {
  try {
    ...
    console.warn(
      '[TypeScriptAdapterParity] tree-sitter-typescript unavailable, skipping extraction:',
      err
    );
    tree = null;
  }
...
describe('TypeScriptAdapterParity (native typescript.holo trait == frozen TypeScriptAdapter snapshot)', () => {
  ...
  it('typescript.holo produced a LanguageTrait in LANGUAGE_TRAITS', () => {
    expect(
      TS_TRAIT,
      'typescript trait must be generated from language-adapters/typescript.holo'
    ).toBeDefined();
  ...
  it('extracts symbols identical to the bespoke TypeScriptAdapter', () => {
    ...
    const data = prune(
      sortSymbols(new TreeSitterTraitAdapter(TS_TRAIT!).extractSymbols(tree, FILE))
    );
  ...
  it('extracts imports identical to the bespoke TypeScriptAdapter', () => {
    if (!tree) return;
    const data = prune(
      sortImports(new TreeSitterTraitAdapter(TS_TRAIT!).extractImports(tree, FILE))
    );
  ...
  it('extracts emit sites identical to the bespoke TypeScriptAdapter', () => {
    if (!tree) return;
    const data = prune(
      sortSites(new TreeSitterTraitAdapter(TS_TRAIT!).extractEmitSites!(tree, FILE))
    );
  ...
  it('extracts listen sites identical to the bespoke TypeScriptAdapter', () => {
    if (!tree) return;
    const data = prune(
      sortSites(new TreeSitterTraitAdapter(TS_TRAIT!).extractListenSites!(tree, FILE))
    );
