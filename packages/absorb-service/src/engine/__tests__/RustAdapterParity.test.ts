...
const EXPECTED_SYMBOLS: ExternalSymbolDefinition[] = [
  ...
  },
  {
    ...
    endColumn: 1,
    visibility: 'public',
    signature: 'fn free_fn((a: i32, b: i32)) -> i32',
    isExported: true,
    lineCount: 4,
  },
  {
  ...
  },
  {
    ...
    endColumn: 5,
    visibility: 'public',
    signature: 'Point::new((x: i32, y: i32)) -> Point',
    owner: 'Point',
    lineCount: 3,
  },
  {
...
const EXPECTED_IMPORTS: ImportEdge[] = [
  {
    fromFile: 'sample.rs',
    toModule: 'std::collections::HashMap',
    line: 1,
    namedImports: [],
    isWildcard: false,
  },
  {
    ...
    toModule: 'std::io',
    line: 2,
    namedImports: ['Read', 'Write'],
    isWildcard: false,
  },
  {
    fromFile: 'sample.rs',
    toModule: 'std::fmt as formatting',
    line: 3,
    namedImports: [],
    isWildcard: false,
  },
...
describe('RustAdapterParity (native rust.holo trait == frozen RustAdapter snapshot)', () => {
  ...
  it('rust.holo produced a LanguageTrait in LANGUAGE_TRAITS', () => {
    expect(
      RUST_TRAIT,
      'rust trait must be generated from language-adapters/rust.holo'
    ).toBeDefined();
  ...
  it('extracts symbols identical to the bespoke RustAdapter', () => {
    if (!tree) return;
    const data = prune(
      sortSymbols(new TreeSitterTraitAdapter(RUST_TRAIT!).extractSymbols(tree, FILE))
    );
  ...
  it('extracts imports identical to the bespoke RustAdapter', () => {
    if (!tree) return;
    const data = prune(
      sortImports(new TreeSitterTraitAdapter(RUST_TRAIT!).extractImports(tree, FILE))
    );
