...
| **Studio contract tests** | **`a2904bee`** — `packages/studio/src/__tests__/scenarios/film3-xr-anchors.scenario.ts` (painter-order + depth guard). |
...
### Film3 opcode coverage (Studio scenarios)

**PowerShell (verbatim):**

```text
=== film3-previz.scenario.ts ===
  opCode 1 (PUSH), 0xB1 (OP_RENDER_HOLOGRAM), 255 (HALT) — see file for full lines.
=== film3-xr-anchors.scenario.ts ===
  (no UAAL opcodes — policy/painter contract only; pair with previz for stack coverage)
```

**Coverage note:** `film3-previz.scenario.ts` exercises **3** opcodes (`1`, `0xB1`, `255`). `film3-xr-anchors.scenario.ts` adds **0** opcodes; end-to-end Film3 still needs more opcode paths for splat scrub / timeline (future).
...
| **gaze_interactable** | **`InteractableType.GAZE_AND_PINCH`** and **`setOnClickListener`** must match real SceneCore API; wire **`handleUaalEvent`** to your UAAL / Film3 bridge. | Ensure **hand + gaze** subscriptions are active and **null-safe** if tracking is off. |
...
## Minimal Film3 test scene (narrative + code)

**Code:** `FILM3_XR_MINIMAL_TEST_SCENE` in `packages/studio/src/__tests__/scenarios/film3-xr-anchors.scenario.ts` — three objects: `occlusion_mesh`, `environment_probe`, `gaze_interactable`.
...
| 5 | **OpenUSD export** (your Film3 path) includes **anchored bounds** consistent with stage — **fidelity** may still lag DCC tools (see below). |

---

## Brutally honest: Film3 end-to-end pre-viz (after these three traits + CRDT/WebRTC + hologram + manifest)

**You can run:** Studio (or pipeline) → **compile Android XR** → **sideload Quest 3** → **walk soundstage** → see **grounded, occludable hologram** with **HDR-ish lighting response** and **director-grade gaze/pinch** on a panel — then **export OpenUSD** from your existing Film3 exporter **if** that exporter consumes the same scene graph (often **partial**; verify bindings).
...
**Authorize next narrow ticket?** **Yes — on-device hardening only** for `occlusion_mesh`, `environment_probe`, `gaze_interactable` (no scope creep to ML TODOs at lines 2470+ until Film3 path is green).
...
Host-side **vitest** + `FILM3_XR_MINIMAL_TEST_SCENE`. Run:

```bash
cd packages/studio
pnpm exec vitest run src/__tests__/scenarios/film3-xr-anchors.scenario.ts
```
...
- **@antigravity-core:** Authorize **narrow** on-device tickets; do not expand to ML/TFLite TODO block until Film3 depth+gaze+probe are green.
