...
function fixFile(filePath) {
  ...
  const propertyRegex =
    /\b(?!(?:this|e|event|config|props|state|settings|options|ctx|context|math|Math|window|document|console|JSON)\b)([a-zA-Z0-9_$]+)\.(x|y|z|w)\b/g;
  ...
  content = content.replace(
    /\{\s*x:\s*([^,}]*?)\s*,\s*y:\s*([^,}]*?)\s*,\s*z:\s*([^,}]*?)\s*\}/g,
    '[$1, $2, $3]'
  );
  content = content.replace(
    /\{\s*x:\s*([^,}]*?)\s*,\s*y:\s*([^,}]*?)\s*,\s*z:\s*([^,}]*?)\s*,\s*w:\s*([^,}]*?)\s*\}/g,
    '[$1, $2, $3, $4]'
  );
