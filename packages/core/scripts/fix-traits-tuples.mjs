...
function fixFile(filePath) {
  ...
  content = content.replace(
    /(\b(?:position|rotation|scale|velocity|force|direction|center|min|max|offset|gravity|wind|acceleration|target|anchor|pose|start|end|axis|normal|up|forward|right|currentPos|lastPosition|userPos|nodePos|soundPos|attackerPos))\.(x|y|z|w)\b/g,
    (match, p1, p2) => {
  ...
  content = content.replace(
    /(\b(?:particle|joint|bone|point|jointPose|bonePose|interpolated|prev|current|targetPos|newPos|currentPos))\.(x|y|z|w)\b/g,
    (match, p1, p2) => {
      const map = { x: '0', y: '1', z: '2', w: '3' };
      return `${p1}[${map[p2]}]`;
    }
  ...
  // Pattern: { x: ..., y: ..., z: ... }
  content = content.replace(
    /\{\s*x:\s*([^,}]*),\s*y:\s*([^,}]*),\s*z:\s*([^,}]*)\s*\}/g,
    '[$1, $2, $3]'
  );
  // Pattern: { x: ..., y: ..., z: ..., w: ... }
  content = content.replace(
    /\{\s*x:\s*([^,}]*),\s*y:\s*([^,}]*),\s*z:\s*([^,}]*),\s*w:\s*([^,}]*)\s*\}/g,
    '[$1, $2, $3, $4]'
  );
  ...
  content = content.replace(
    /:\s*\{\s*x:\s*number;\s*y:\s*number;\s*z:\s*number;\s*\}/g,
    ': [number, number, number]'
  );
  content = content.replace(
    /:\s*\{\s*x:\s*number;\s*y:\s*number;\s*z:\s*number;\s*w:\s*number;\s*\}/g,
