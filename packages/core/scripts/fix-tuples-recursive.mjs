...
function fixFile(filePath) {
  ...
  content = content.replace(
    /(\b(?:position|rotation|scale|velocity|force|direction|normal|forward|up|right|size|offset|min|max|center|gravity|wind|acceleration|target|anchor|pose|start|end|axis))\s*=\s*\{\s*x:\s*([^,}]*),\s*y:\s*([^,}]*),\s*z:\s*([^,}]*)\s*\}/g,
    '$1 = [$2, $3, $4]'
  );
  ...
  content = content.replace(
    /(\b(?:position|rotation|scale|velocity|force))\s*:\s*\{\s*x:\s*([^,}]*),\s*y:\s*([^,}]*),\s*z:\s*([^,}]*)\s*\}/g,
    '$1: [$2, $3, $4]'
  );

  // Handle { x, y, z, w } for rotation
  content = content.replace(
    /(\b(?:rotation|pose))\s*=\s*\{\s*x:\s*([^,}]*),\s*y:\s*([^,}]*),\s*z:\s*([^,}]*),\s*w:\s*([^,}]*)\s*\}/g,
    '$1 = [$2, $3, $4, $5]'
  );
  content = content.replace(
    /(\b(?:rotation|pose))\s*:\s*\{\s*x:\s*([^,}]*),\s*y:\s*([^,}]*),\s*z:\s*([^,}]*),\s*w:\s*([^,}]*)\s*\}/g,
