...
export const blobStoreHandler: TraitHandler<BlobStoreConfig> = {
  ...
  onEvent(node: any, _config: BlobStoreConfig, context: any, event: any): void {
    ...
    switch (t) {
      case 'blob:put': state.blobs.set(event.blobId as string, (event.size as number) ?? 0); context.emit?.('blob:stored', { blobId: event.blobId, total: state.blobs.size }); break;
      case 'blob:get': context.emit?.('blob:retrieved', { blobId: event.blobId, exists: state.blobs.has(event.blobId as string) }); break;
    }
  },
};
