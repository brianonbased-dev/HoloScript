...
export const AdvancedTexturingTrait: TraitHandler<AdvancedTexturingConfig> = {
  ...
  validate(config: AdvancedTexturingConfig): boolean {
    ...
    if (config.mode === 'displacement' && !config.displacement)
      throw new Error('displacement mode requires displacement config');
    if (config.mode === 'pom' && !config.pom) throw new Error('pom mode requires pom config');
    if (config.mode === 'triplanar' && !config.triplanar)
      throw new Error('triplanar mode requires triplanar config');
    if (config.mode === 'detail' && !config.detail)
      throw new Error('detail mode requires detail config');
    ...
    if (config.atlas) {
      if (config.atlas.width <= 0 || config.atlas.height <= 0)
        throw new Error('atlas dimensions must be > 0');
    }
    return true;
  },

  compile(config: AdvancedTexturingConfig, target: string): string {
    const self = this as unknown as Record<string, (c: AdvancedTexturingConfig) => string>;
    switch (target) {
      case 'unity':
...
public class AdvancedTexturingSetup : MonoBehaviour {
    void Start() {
        ...
        ${
          t
            ? `// Triplanar Mapping (custom shader / ShaderGraph)
...
${
  d
    ? `// Displacement: World Position Offset node
...
${
  p
    ? `// Parallax Occlusion Mapping: Use "Parallax Occlusion Mapping" material function
// HeightMap: "${p.heightMap}", HeightRatio: ${p.scale}, Steps: ${p.steps}`
    : ''
}
${
  t
    ? `// Triplanar: Use "Triplanar Texture Mapping" material function
...
${
  p
    ? `// POM — requires custom ShaderMaterial
...
${
  t
    ? `// Triplanar — custom ShaderMaterial or onBeforeCompile injection
