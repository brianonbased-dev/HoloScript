...
export const temporalGuardHandler: TraitHandler<TemporalGuardConfig> = {
  ...
  onAttach(node: any): void { node.__tempState = { guards: new Map<string, { deadline: number; property: string }>(), violations: 0 }; },
  onDetach(node: any): void { delete node.__tempState; },
  onUpdate(): void {},
  onEvent(node: any, config: TemporalGuardConfig, context: any, event: any): void {
