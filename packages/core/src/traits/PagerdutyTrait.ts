...
export const pagerdutyHandler: TraitHandler<PagerdutyConfig> = {
  ...
  onEvent(node: any, config: PagerdutyConfig, context: any, event: any): void {
    ...
    switch (t) {
      case 'pagerduty:trigger': state.incidents++; context.emit?.('pagerduty:triggered', { incidentId: `PD-${state.incidents}`, severity: (event.severity as string) ?? config.severity, summary: event.summary }); break;
      case 'pagerduty:resolve': context.emit?.('pagerduty:resolved', { incidentId: event.incidentId }); break;
    }
  },
};
