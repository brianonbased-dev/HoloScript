...
export const graphqlHandler: TraitHandler<GraphqlConfig> = {
  ...
  onEvent(node: any, _config: GraphqlConfig, context: any, event: any): void {
    const state = node.__gqlState as { resolvers: Map<string, string>; queries: number } | undefined;
    if (!state) return;
    const t = typeof event === 'string' ? event : event.type;
    switch (t) {
      case 'gql:register': state.resolvers.set(event.typeName as string, (event.field as string) ?? ''); context.emit?.('gql:registered', { typeName: event.typeName, total: state.resolvers.size }); break;
      case 'gql:query': state.queries++; context.emit?.('gql:result', { query: event.query, queryCount: state.queries }); break;
    }
  },
};
