...
export const sqlQueryHandler: TraitHandler<SqlQueryConfig> = {
  name: 'sql_query',
  defaultConfig: { max_results: 1000 },
  onAttach(node: HSPlusNode): void {
    node.__sqlState = { queries: 0, results: [] as unknown[] };
  },
  onDetach(node: HSPlusNode): void {
    delete node.__sqlState;
  },
  onUpdate(): void {},
  onEvent(
    node: HSPlusNode,
    ...
    switch (t) {
      case 'sql:exec':
        state.queries++;
        context.emit?.('sql:result', {
          query: event.query,
          queryCount: state.queries,
          maxResults: config.max_results,
        });
        break;
      case 'sql:prepare':
        context.emit?.('sql:prepared', { statement: event.statement, params: event.params });
        break;
    }
  },
};
