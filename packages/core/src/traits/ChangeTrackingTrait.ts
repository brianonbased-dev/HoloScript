...
export const changeTrackingHandler: TraitHandler<ChangeTrackingConfig> = {
  ...
  onEvent(node: any, config: ChangeTrackingConfig, context: any, event: any): void {
    const state = node.__changeState as { history: Array<{ entityId: string; field: string; oldValue: unknown; newValue: unknown; ts: number }> } | undefined;
    if (!state) return;
    const t = typeof event === 'string' ? event : event.type;
    switch (t) {
      case 'change:record': {
        const entry = { entityId: event.entityId as string, field: event.field as string, oldValue: event.oldValue, newValue: event.newValue, ts: Date.now() };
