...
export const navmeshSolverHandler: TraitHandler<NavmeshSolverConfig> = {
  ...
  onAttach(node: HSPlusNode): void {
    node.__navState = { meshBuilt: false, polygons: 0 };
  },
  ...
  onEvent(
    ...
    switch (t) {
      case 'nav:build':
        ...
        state.polygons = (event.polygonCount as number) ?? 0;
        context.emit?.('nav:built', { polygons: state.polygons, cellSize: config.cell_size });
        break;
      case 'nav:query':
        context.emit?.('nav:path', { from: event.from, to: event.to, meshReady: state.meshBuilt });
        break;
    }
  },
};
