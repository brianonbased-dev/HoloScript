  ...
  onEvent(node: any, _config: WorkflowConfig, context: any, event: any): void {
    ...
    switch (t) {
      case 'workflow:create':
        state.workflows.set(event.workflowId as string, { steps: event.steps ?? [], current: 0, status: 'pending' });
      ...
      case 'workflow:advance': {
        ...
        if (wf && wf.current < wf.steps.length - 1) { wf.current++; wf.status = 'in_progress'; }
        else if (wf) { wf.status = 'completed'; }
        context.emit?.('workflow:advanced', { workflowId: event.workflowId, step: wf?.current, status: wf?.status });
        break;
      }
    }
  },
