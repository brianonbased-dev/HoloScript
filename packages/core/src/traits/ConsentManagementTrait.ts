...
export const consentManagementHandler: TraitHandler<ConsentManagementConfig> = {
  ...
  onEvent(node: any, _config: ConsentManagementConfig, context: any, event: any): void {
    const state = node.__consentState as { consents: Map<string, Map<string, boolean>> } | undefined;
    if (!state) return;
    ...
    switch (t) {
      ...
      case 'consent:check': {
        const granted = state.consents.get(event.userId as string)?.get(event.purpose as string) ?? false;
