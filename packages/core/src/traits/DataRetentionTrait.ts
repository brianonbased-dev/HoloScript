...
export const dataRetentionHandler: TraitHandler<DataRetentionConfig> = {
  ...
  onEvent(node: any, config: DataRetentionConfig, context: any, event: any): void {
    ...
    switch (t) {
      case 'retention:set': state.policies.set(event.dataType as string, { ttl: (event.ttl_days as number) ?? config.default_ttl_days, created: Date.now() }); context.emit?.('retention:policy_set', { dataType: event.dataType }); break;
      case 'retention:purge': { const purged: string[] = []; for (const [k] of state.policies) purged.push(k); context.emit?.('retention:purged', { purged }); break; }
    }
  },
};
