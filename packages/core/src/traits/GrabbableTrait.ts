...
export class GrabbableTrait implements Trait {
  ...
  private updateTwoHanded(node: any, left: VRHand, right: VRHand): void {
      ...
      if (this.initialHandAngle === null) {
          ...
          this.initialObjectRotation = node.properties.rotation ? { ...node.properties.rotation } : { x:0, y:0, z:0 };
      }
  ...
  private getDistance(p1: any, p2: any): number {
    const dx = (p1.x !== undefined) ? (p1.x - (p2.x !== undefined ? p2.x : p2[0])) : (p1[0] - (p2.x !== undefined ? p2.x : p2[0]));
    const dy = (p1.y !== undefined) ? (p1.y - (p2.y !== undefined ? p2.y : p2[1])) : (p1[1] - (p2.y !== undefined ? p2.y : p2[1]));
    const dz = (p1.z !== undefined) ? (p1.z - (p2.z !== undefined ? p2.z : p2[2])) : (p1[2] - (p2.z !== undefined ? p2.z : p2[2]));
