    ...
    // --- Interaction tracking (engagement) ---
      ...
      if (config.collect_engagement && config.privacy_level !== 'strict') {
        ...
        if (config.collect_heatmap && event.position) {
          const pos = event.position as [number, number, number] | [number, number, number];
          const x = Array.isArray(pos) ? (pos[0] ?? 0) : (pos[0] ?? 0);
          const y = Array.isArray(pos) ? (pos[1] ?? 0) : (pos[1] ?? 0);
          const z = Array.isArray(pos) ? (pos[2] ?? 0) : (pos[2] ?? 0);
