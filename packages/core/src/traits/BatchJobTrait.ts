...
export const batchJobHandler: TraitHandler<BatchJobConfig> = {
  ...
  onEvent(node: any, _config: BatchJobConfig, context: any, event: any): void {
    ...
    switch (t) {
      case 'batch:submit': state.jobs.set(event.jobId as string, { status: 'queued', progress: 0 }); context.emit?.('batch:queued', { jobId: event.jobId }); break;
      case 'batch:progress': { const j = state.jobs.get(event.jobId as string); if (j) { j.status = 'running'; j.progress = (event.progress as number) ?? 0; } context.emit?.('batch:progress', { jobId: event.jobId, progress: j?.progress }); break; }
      case 'batch:complete': { const j = state.jobs.get(event.jobId as string); if (j) j.status = 'completed'; context.emit?.('batch:completed', { jobId: event.jobId }); break; }
    }
  },
};
