...
const GEOMETRY_TRAITS: Record<
  string,
  { primitive: ProceduralGeometry['primitive']; params: Record<string, number> }
> = {
  // Nature
  tree: {
    primitive: 'tree',
    params: { trunkHeight: 1.5, trunkRadius: 0.15, crownRadius: 0.8, branches: 6 },
  },
  dead_tree: {
    primitive: 'tree',
    params: { trunkHeight: 2.0, trunkRadius: 0.12, crownRadius: 0.0, branches: 4 },
  },
  pine_tree: {
    primitive: 'tree',
    params: { trunkHeight: 2.5, trunkRadius: 0.1, crownRadius: 0.5, branches: 8, conical: 1 },
  },
  ...
  crystal: {
    primitive: 'crystal_cluster',
    params: { count: 5, maxHeight: 0.8, baseRadius: 0.08, spread: 0.3 },
  },
  // Terrain patches
  cliff: {
    primitive: 'terrain_patch',
    params: { width: 4, depth: 4, heightScale: 3, roughness: 0.8, seed: 12 },
  },
  hill: {
    primitive: 'terrain_patch',
    params: { width: 6, depth: 6, heightScale: 1.5, roughness: 0.4, seed: 5 },
  },
  flat_ground: {
    primitive: 'terrain_patch',
    params: { width: 4, depth: 4, heightScale: 0.05, roughness: 0.1, seed: 1 },
  },
  // Structures
...
function buildRock(p: Record<string, number>): Omit<ProceduralGeometry, 'primitive' | 'params'> {
  ...
  const facesToUse = subdivide
    ? subdivideFaces(baseVerts, baseFaces)
    : { verts: baseVerts, faces: baseFaces };
...
export function packGeometry(geo: Omit<ProceduralGeometry, 'primitive' | 'params'>): ArrayBuffer {
  ...
  let offset = header;
  new Float32Array(buf, offset, geo.vertices.length).set(geo.vertices);
  offset += vertBytes;
  new Float32Array(buf, offset, geo.normals.length).set(geo.normals);
  offset += normBytes;
  new Float32Array(buf, offset, geo.uvs.length).set(geo.uvs);
  offset += uvBytes;
...
export class ProceduralGeometryResolver implements AssetResolverPlugin {
  ...
  private generate(
    ...
    switch (primitive) {
      ...
      case 'rock':
        return buildRock(params);
      case 'terrain_patch':
        return buildTerrainPatch(params);
      case 'building':
        return buildBuilding(params);
      case 'arch':
        return buildArch(params);
