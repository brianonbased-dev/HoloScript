  ...
  onEvent(
    ...
    switch (eventType) {
      ...
      case 'networked:authority_granted': {
        trait.setOwner(true, (event as Record<string, unknown>).peerId as string | undefined);
        break;
      }

      // Ownership revoked
      case 'networked:authority_revoked': {
        trait.setOwner(false, (event as Record<string, unknown>).peerId as string | undefined);
        break;
      }
    }
  },
