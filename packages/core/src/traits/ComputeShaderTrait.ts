...
export const computeShaderHandler: TraitHandler<ComputeShaderConfig> = {
  ...
  onEvent(node: HSPlusNode, _config: ComputeShaderConfig, context: TraitContext, event: TraitEvent): void {
    ...
    switch (t) {
      case 'cs:compile': state.shaders.set(event.shaderId as string, { workgroups: (event.workgroups as number[]) ?? [64, 1, 1] }); context.emit?.('cs:compiled', { shaderId: event.shaderId }); break;
      case 'cs:dispatch': state.dispatches++; context.emit?.('cs:dispatched', { shaderId: event.shaderId, dispatches: state.dispatches }); break;
    }
  },
};
