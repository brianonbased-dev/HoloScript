...
export const reactiveStoreHandler: TraitHandler<ReactiveStoreConfig> = {
  ...
  onEvent(node: HSPlusNode, config: ReactiveStoreConfig, context: TraitContext, event: TraitEvent): void {
    ...
    switch (t) {
      case 'store:set': if (state.store.size < config.max_keys || state.store.has(event.key as string)) { const prev = state.store.get(event.key as string); state.store.set(event.key as string, event.value); context.emit?.('store:changed', { key: event.key, value: event.value, previous: prev }); } break;
      case 'store:get': context.emit?.('store:value', { key: event.key, value: state.store.get(event.key as string), exists: state.store.has(event.key as string) }); break;
      case 'store:subscribe': state.subscribers.set(event.key as string, (state.subscribers.get(event.key as string) ?? 0) + 1); context.emit?.('store:subscribed', { key: event.key, subscriberCount: state.subscribers.get(event.key as string) }); break;
    }
  },
};
