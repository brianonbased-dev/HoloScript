...
function buildBody(config: LocalLLMConfig, messages: LLMMessage[]): object {
  ...
  if (config.backend === 'ollama') {
    return { model: config.model, messages: all, stream: config.stream, options: { temperature: config.temperature, num_predict: config.max_tokens } };
  }
  return { model: config.model, messages: all, stream: config.stream, temperature: config.temperature, max_tokens: config.max_tokens };
}
...
export const localLLMHandler = {
  ...
  async onAttach(node: any, config: LocalLLMConfig, ctx: any): Promise<void> {
    ...
    try {
      ...
      state.isReady = true;
      ctx.emit('llm_model_loaded', { node, model: config.model, backend: config.backend, availableModels: models });
    } catch {
      if (config.fallback_to_remote && config.fallback_api_key) {
        state.usingFallback = true;
        state.activeModel = config.fallback_model;
        state.isReady = true;
        ctx.emit('llm_model_loaded', { node, model: config.fallback_model, backend: 'openai', fallback: true });
      } else {
        ctx.emit('llm_error', { node, requestId: null, error: `Cannot connect to ${config.backend} at ${config.base_url}` });
      }
    }
  },
  ...
  onDetach(node: any, _c: LocalLLMConfig, ctx: any): void {
    ...
    if (!s) return;
    for (const [id, ac] of s.activeRequests) { ac.abort(); ctx.emit('llm_cancelled', { node, requestId: id }); }
  ...
  onEvent(node: any, config: LocalLLMConfig, ctx: any, event: any): void {
    ...
    if (type === 'llm_prompt') {
      this._chat(s, node, config, ctx, { messages: [{ role: 'user', content: payload?.prompt ?? '' }], ...payload });
  ...
  onUpdate(_n: any, _c: any, _ctx: any, _dt: number): void { /* async only */ },
  ...
  _chat(s: LocalLLMState, node: any, config: LocalLLMConfig, ctx: any, payload: any): void {
    ...
    const cfg: LocalLLMConfig = s.usingFallback
      ? { ...config, backend: 'openai', model: config.fallback_model, base_url: 'https://api.openai.com' }
      : { ...config, model };
  ...
  async _exec(
    s: LocalLLMState, node: any, config: LocalLLMConfig, ctx: any,
    requestId: string, messages: LLMMessage[], ac: AbortController
  ): Promise<void> {
    ...
    const res = await fetch(chatEndpoint(config), {
      method: 'POST', headers, body: JSON.stringify(buildBody(config, messages)), signal: ac.signal,
    });
    if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);

    if (!config.stream || !res.body) {
      const data = await res.json();
      const text = config.backend === 'ollama' ? data.message?.content ?? '' : data.choices?.[0]?.message?.content ?? '';
      const tokens = data.eval_count ?? data.usage?.completion_tokens ?? text.split(' ').length;
      s.totalTokens += tokens;
      s.activeRequests.delete(requestId);
      ctx.emit('llm_complete', { node, requestId, text, model: config.model, duration_ms: Date.now() - t0, tokens });
      return;
    }

    const reader = res.body.getReader();
    const dec = new TextDecoder();
    let accumulated = ''; let tokenCount = 0; let buf = '';
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        let token = ''; let isDone = false;
        if (config.backend === 'ollama') {
          try { const d = JSON.parse(line); isDone = d.done; token = d.message?.content ?? ''; } catch { continue; }
        } else {
          if (!line.startsWith('data: ')) continue;
          const p = line.slice(6).trim();
          if (p === '[DONE]') { isDone = true; } else {
            try { token = JSON.parse(p).choices?.[0]?.delta?.content ?? ''; } catch { continue; }
          }
        }
        if (isDone) break;
        if (!token) continue;
        accumulated += token; tokenCount++; s.totalTokens++;
        ctx.emit('llm_token', { node, requestId, token, accumulated });
      }
    }
    s.activeRequests.delete(requestId);
    ctx.emit('llm_complete', { node, requestId, text: accumulated, model: config.model, duration_ms: Date.now() - t0, tokens: tokenCount });
  },
  ...
  _cancel(s: LocalLLMState, node: any, ctx: any, requestId?: string): void {
    if (!requestId) {
      ...
      if (!ac) return;
      ac.abort(); s.activeRequests.delete(requestId);
