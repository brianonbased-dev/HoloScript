...
export const tensorOpHandler: TraitHandler<TensorOpConfig> = {
  ...
  onAttach(node: any): void { node.__tensorState = { tensors: new Map<string, { shape: number[]; dtype: string }>() }; },
  onDetach(node: any): void { delete node.__tensorState; },
  onUpdate(): void {},
  onEvent(node: any, _config: TensorOpConfig, context: any, event: any): void {
