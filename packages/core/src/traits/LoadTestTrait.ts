...
export const loadTestHandler: TraitHandler<LoadTestConfig> = {
  ...
  onEvent(node: any, config: LoadTestConfig, context: any, event: any): void {
    const state = node.__loadState as { running: boolean; vus: number; requests: number; errors: number } | undefined;
    if (!state) return;
    ...
    switch (t) {
      case 'load:start':
        ...
        state.requests = 0; state.errors = 0;
      ...
      case 'load:stop':
        state.running = false;
        context.emit?.('load:completed', { requests: state.requests, errors: state.errors, errorRate: state.requests > 0 ? state.errors / state.requests : 0 });
        break;
    }
  },
};
