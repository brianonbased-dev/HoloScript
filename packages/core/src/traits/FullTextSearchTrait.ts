...
export const fullTextSearchHandler: TraitHandler<FullTextSearchConfig> = {
  ...
  onEvent(node: any, config: FullTextSearchConfig, context: any, event: any): void {
    ...
    switch (t) {
      case 'fts:index': state.index.set(event.docId as string, (event.content as string) ?? ''); context.emit?.('fts:indexed', { docId: event.docId, size: state.index.size }); break;
      case 'fts:search': {
        const q = ((event.query as string) ?? '').toLowerCase();
        const hits: string[] = [];
        for (const [id, content] of state.index) { if (content.toLowerCase().includes(q) && hits.length < config.max_results) hits.push(id); }
