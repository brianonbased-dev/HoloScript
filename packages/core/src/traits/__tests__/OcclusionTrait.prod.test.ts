...
describe('occlusionHandler.defaultConfig', () => {
  ...
  it('edge_smoothing = true', () =>
    expect(occlusionHandler.defaultConfig!.edge_smoothing).toBe(true));
  it('fade_distance = 0.5', () => expect(occlusionHandler.defaultConfig!.fade_distance).toBe(0.5));
  it('hand_occlusion = true', () =>
    expect(occlusionHandler.defaultConfig!.hand_occlusion).toBe(true));
  it('occlusion_bias = 0.001', () =>
    expect(occlusionHandler.defaultConfig!.occlusion_bias).toBe(0.001));
  it('soft_edges = true', () => expect(occlusionHandler.defaultConfig!.soft_edges).toBe(true));
  it('soft_edge_width = 0.02', () =>
    expect(occlusionHandler.defaultConfig!.soft_edge_width).toBe(0.02));
...
describe('occlusionHandler.onUpdate', () => {
  it('does not throw when no state', () => {
    ...
    const ctx = makeContext();
    expect(() =>
      occlusionHandler.onUpdate!(node, occlusionHandler.defaultConfig!, ctx, 0.016)
    ).not.toThrow();
  });
