...
function makeNode() { return { id: 'ft_node' }; }
...
describe('faceTrackingHandler.onAttach', () => {
  ...
  it('blendShapes is empty Map', () => expect(attach().node.__faceTrackingState.blendShapes.size).toBe(0));
  ...
  it('smoothedShapes is empty Map', () => expect(attach().node.__faceTrackingState.smoothedShapes.size).toBe(0));
  it('lipSyncPhoneme=null', () => expect(attach().node.__faceTrackingState.lipSyncPhoneme).toBeNull());
  it('emits face_tracking_start with topology + blendShapes + eyeTracking', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('face_tracking_start', expect.objectContaining({
      topology: 'meta', blendShapes: true, eyeTracking: false,
    }));
  });
});
...
describe('faceTrackingHandler.onUpdate', () => {
  ...
  it('emits avatar_blend_shapes when tracking and smoothedShapes non-empty', () => {
    ...
    ctx.emit.mockClear();
    faceTrackingHandler.onUpdate!(node, config, ctx, 0.016);
    expect(ctx.emit).toHaveBeenCalledWith('avatar_blend_shapes', expect.objectContaining({
  ...
  it('emits avatar_eye_gaze when tracking and eye_tracking=true and leftEye set', () => {
    ...
    ctx.emit.mockClear();
    faceTrackingHandler.onUpdate!(node, config, ctx, 0.016);
    expect(ctx.emit).toHaveBeenCalledWith('avatar_eye_gaze', expect.objectContaining({ leftEye: eye }));
  });
  it('does NOT emit avatar_eye_gaze when eye_tracking=false', () => {
    const { node, config, ctx } = attach({ eye_tracking: false });
    node.__faceTrackingState.isTracking = true;
    node.__faceTrackingState.leftEye = { direction: { x: 0, y: 0, z: -1 }, origin: { x: 0, y: 0, z: 0 }, confidence: 0.9 };
    ctx.emit.mockClear();
    faceTrackingHandler.onUpdate!(node, config, ctx, 0.016);
  ...
  it('emits avatar_head_pose when headPose is set', () => {
    ...
    ctx.emit.mockClear();
    faceTrackingHandler.onUpdate!(node, config, ctx, 0.016);
    expect(ctx.emit).toHaveBeenCalledWith('avatar_head_pose', expect.objectContaining({
      position: { x: 0, y: 1.7, z: 0 },
    }));
  });
...
describe('faceTrackingHandler.onEvent — face_data_update', () => {
  it('sets isTracking=true', () => {
    const { node, config, ctx } = attach();
    faceTrackingHandler.onEvent!(node, config, ctx, { type: 'face_data_update', blendShapes: {}, eyes: {} });
  ...
  it('smoothes blend shapes: smoothed = prev * smoothing + value * (1 - smoothing)', () => {
    ...
    faceTrackingHandler.onEvent!(node, config, ctx, { type: 'face_data_update', blendShapes: { jawOpen: 0.9 } });
  ...
  it('second update smooths from previous value (smoothing=0.5)', () => {
    ...
    faceTrackingHandler.onEvent!(node, config, ctx, { type: 'face_data_update', blendShapes: { jawOpen: 0.0 } });
    // prev=0, value=0 → smoothed=0. Now update with value=1.0:
    faceTrackingHandler.onEvent!(node, config, ctx, { type: 'face_data_update', blendShapes: { jawOpen: 1.0 } });
    // 0 * 0.5 + 1.0 * 0.5 = 0.5
  ...
  it('emits face_expression_update with smoothed shapes', () => {
    const { node, config, ctx } = attach({ blend_shapes: true, smoothing: 0 });
    ctx.emit.mockClear();
    faceTrackingHandler.onEvent!(node, config, ctx, { type: 'face_data_update', blendShapes: { eyeBlinkLeft: 1.0 } });
    expect(ctx.emit).toHaveBeenCalledWith('face_expression_update', expect.objectContaining({
      blendShapes: expect.objectContaining({ eyeBlinkLeft: 1.0 }),
    }));
  });
  it('stores left/right eye when eye_tracking=true', () => {
    const { node, config, ctx } = attach({ eye_tracking: true });
    const lEye = { direction: { x: 0, y: 0, z: -1 }, origin: { x: -0.03, y: 0, z: 0 }, confidence: 0.9 };
    const rEye = { direction: { x: 0, y: 0, z: -1 }, origin: { x: 0.03, y: 0, z: 0 }, confidence: 0.9 };
    faceTrackingHandler.onEvent!(node, config, ctx, { type: 'face_data_update', eyes: { left: lEye, right: rEye } });
  ...
  it('does NOT store eyes when eye_tracking=false', () => {
    ...
    faceTrackingHandler.onEvent!(node, config, ctx, { type: 'face_data_update', eyes: { left: lEye } });
...
describe('faceTrackingHandler — detectPhoneme via lip_sync', () => {
  function phonemeFor(shapes: Record<string, number>) {
    ...
    ctx.emit.mockClear();
    faceTrackingHandler.onEvent!(node, config, ctx, { type: 'face_data_update', blendShapes: shapes });
  ...
  it('mouthSmile avg>0.5 → EE', () => expect(phonemeFor({ mouthSmileLeft: 0.6, mouthSmileRight: 0.6 })).toBe('EE'));
  it('mouthClose>0.5 and jawOpen<0.2 → MM', () => expect(phonemeFor({ mouthClose: 0.6, jawOpen: 0.1 })).toBe('MM'));
  ...
  it('emits lip_sync_phoneme only on phoneme change (dedup)', () => {
    const { node, config, ctx } = attach({ blend_shapes: true, lip_sync: true, smoothing: 0 });
    faceTrackingHandler.onEvent!(node, config, ctx, { type: 'face_data_update', blendShapes: { jawOpen: 0.8 } });
    ctx.emit.mockClear();
  ...
  it('emits lip_sync_phoneme when phoneme changes', () => {
    ...
    faceTrackingHandler.onEvent!(node, config, ctx, { type: 'face_data_update', blendShapes: { jawOpen: 0.0, mouthFunnel: 0.7 } });
