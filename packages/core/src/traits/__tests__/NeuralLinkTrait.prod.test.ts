...
function makeNode(id = 'nl_node') { return { id }; }
...
describe('neuralLinkHandler.defaultConfig', () => {
  it('model = brittney-v4.gguf', () => expect(neuralLinkHandler.defaultConfig!.model).toBe('brittney-v4.gguf'));
  ...
  it('personality_anchor = empty string', () => expect(neuralLinkHandler.defaultConfig!.personality_anchor).toBe(''));
});
...
describe('neuralLinkHandler.onAttach', () => {
  ...
  it('emits neural_link_ready with nodeId and model', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('neural_link_ready', expect.objectContaining({
      nodeId: 'nl_node',
      model: 'gemma-7b.gguf',
    }));
  });
...
describe('neuralLinkHandler.onEvent — neural_link_execute', () => {
  it('sets neural_status = inferring', () => {
    const { node, cfg, ctx } = attachNode();
    neuralLinkHandler.onEvent!(node, cfg, ctx, { type: 'neural_link_execute', data: { prompt: 'hello' } });
  ...
  it('emits on_neural_inference_start with nodeId, model, prompt', () => {
    ...
    ctx.emit.mockClear();
    neuralLinkHandler.onEvent!(node, cfg, ctx, { type: 'neural_link_execute', data: { prompt: 'What is 2+2?' } });
    expect(ctx.emit).toHaveBeenCalledWith('on_neural_inference_start', expect.objectContaining({
      nodeId: 'nl_node',
      model: 'llama.gguf',
      prompt: 'What is 2+2?',
    }));
  ...
  it('works with missing prompt (undefined)', () => {
    const { node, cfg, ctx } = attachNode();
    expect(() => neuralLinkHandler.onEvent!(node, cfg, ctx, { type: 'neural_link_execute', data: {} })).not.toThrow();
  });
  it('works with missing data entirely', () => {
    const { node, cfg, ctx } = attachNode();
    expect(() => neuralLinkHandler.onEvent!(node, cfg, ctx, { type: 'neural_link_execute' })).not.toThrow();
  });
});
...
describe('neuralLinkHandler.onEvent — neural_link_response', () => {
  it('sets neural_status = idle', () => {
    ...
    neuralLinkHandler.onEvent!(node, cfg, ctx, { type: 'neural_link_response', data: { text: 'Hello!', generationTime: 120 } });
    expect((node as any).__neuralLinkState.neural_status).toBe('idle');
  });
  it('stores last_response', () => {
    const { node, cfg, ctx } = attachNode();
    neuralLinkHandler.onEvent!(node, cfg, ctx, { type: 'neural_link_response', data: { text: 'Done.', generationTime: 80 } });
    expect((node as any).__neuralLinkState.last_response).toBe('Done.');
  });
  it('stores last_inference_time', () => {
    const { node, cfg, ctx } = attachNode();
    neuralLinkHandler.onEvent!(node, cfg, ctx, { type: 'neural_link_response', data: { text: 'x', generationTime: 350 } });
    expect((node as any).__neuralLinkState.last_inference_time).toBe(350);
  });
  it('defaults last_inference_time = 0 when missing', () => {
    const { node, cfg, ctx } = attachNode();
    neuralLinkHandler.onEvent!(node, cfg, ctx, { type: 'neural_link_response', data: { text: 'y' } });
    expect((node as any).__neuralLinkState.last_inference_time).toBe(0);
  });
  it('emits on_neural_response with nodeId and text', () => {
    const { node, cfg, ctx } = attachNode();
    ctx.emit.mockClear();
    neuralLinkHandler.onEvent!(node, cfg, ctx, { type: 'neural_link_response', data: { text: 'Response!', generationTime: 50 } });
    expect(ctx.emit).toHaveBeenCalledWith('on_neural_response', expect.objectContaining({
      nodeId: 'nl_node',
      text: 'Response!',
    }));
  });
