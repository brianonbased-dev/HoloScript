...
function makeNode() { return { id: 'compute_node' }; }
...
describe('computeHandler.onAttach', () => {
  ...
  it('emits compute_init when shader_source is set', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('compute_init', expect.objectContaining({ shaderSource: '@compute fn main() {}' }));
  });
  it('compute_init includes workgroupSize', () => {
    const { ctx } = attach({ shader_source: 'src', workgroup_size: [128, 1, 1] });
    expect(ctx.emit).toHaveBeenCalledWith('compute_init', expect.objectContaining({ workgroupSize: [128, 1, 1] }));
  });
  it('compute_init includes sharedMemorySize', () => {
    const { ctx } = attach({ shader_source: 'src', shared_memory_size: 256 });
    expect(ctx.emit).toHaveBeenCalledWith('compute_init', expect.objectContaining({ sharedMemorySize: 256 }));
  });
...
describe('computeHandler.onDetach', () => {
  ...
  it('emits compute_destroy when isReady=true', () => {
    ...
    ctx.emit.mockClear();
    computeHandler.onDetach!(node, config, ctx);
    expect(ctx.emit).toHaveBeenCalledWith('compute_destroy', expect.objectContaining({ buffers: ['myBuf'] }));
  });
...
describe('computeHandler.onEvent — compute_initialized', () => {
  it('sets isReady=true', () => {
    ...
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_initialized', shaderModule: {}, pipeline: {} });
  ...
  it('stores shaderModule and pipeline', () => {
    ...
    const pipe = { id: 'pipe1' };
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_initialized', shaderModule: mod, pipeline: pipe });
...
describe('computeHandler.onEvent — compute_dispatch', () => {
  it('emits on_compute_error when not ready', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('on_compute_error', expect.objectContaining({ error: 'Compute not initialized' }));
  });
  it('emits compute_execute when ready', () => {
    const { node, ctx, config } = attach({ dispatch: [4, 2, 1] });
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_initialized', shaderModule: {}, pipeline: { tag: 'p' } });
    ctx.emit.mockClear();
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_dispatch' });
    expect(ctx.emit).toHaveBeenCalledWith('compute_execute', expect.objectContaining({ dispatch: [4, 2, 1] }));
  });
  it('uses event.dispatch when provided', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('compute_execute', expect.objectContaining({ dispatch: [8, 4, 2] }));
  });
...
describe('computeHandler.onEvent — compute_dispatch_indirect', () => {
  it('emits compute_execute_indirect', () => {
    ...
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_initialized', shaderModule: {}, pipeline: {} });
    ctx.emit.mockClear();
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_dispatch_indirect', buffer: {}, offset: 64 });
    expect(ctx.emit).toHaveBeenCalledWith('compute_execute_indirect', expect.objectContaining({ offset: 64 }));
  });
...
describe('computeHandler.onEvent — buffer events', () => {
  it('compute_write_buffer emits on_compute_error when buffer not found', () => {
    const { node, ctx, config } = attach();
    ctx.emit.mockClear();
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_write_buffer', buffer: 'missing', data: [] });
    expect(ctx.emit).toHaveBeenCalledWith('on_compute_error', expect.objectContaining({ error: expect.stringContaining('missing') }));
  });
  it('compute_write_buffer emits compute_buffer_write when buffer found', () => {
    const { node, ctx, config } = attach();
    node.__computeState.buffers.set('positions', { handle: 'h' });
    ctx.emit.mockClear();
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_write_buffer', buffer: 'positions', data: [1, 2, 3], offset: 4 });
    expect(ctx.emit).toHaveBeenCalledWith('compute_buffer_write', expect.objectContaining({ offset: 4 }));
  });
  it('compute_read_buffer emits compute_buffer_read_error when buffer not found', () => {
    const { node, ctx, config } = attach();
    ctx.emit.mockClear();
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_read_buffer', buffer: 'ghost', callbackId: 'cb1' });
    expect(ctx.emit).toHaveBeenCalledWith('compute_buffer_read_error', expect.objectContaining({ callbackId: 'cb1' }));
  });
  it('compute_read_buffer emits compute_buffer_read when found', () => {
    const { node, ctx, config } = attach();
    node.__computeState.buffers.set('velocities', { handle: 'h2' });
    ctx.emit.mockClear();
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_read_buffer', buffer: 'velocities', callbackId: 'cb2' });
    expect(ctx.emit).toHaveBeenCalledWith('compute_buffer_read', expect.objectContaining({ bufferName: 'velocities', callbackId: 'cb2' }));
  });
  it('compute_buffer_data emits on_compute_data', () => {
    const { node, ctx, config } = attach();
    ctx.emit.mockClear();
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_buffer_data', bufferName: 'positions', data: [1], callbackId: 'cb' });
    expect(ctx.emit).toHaveBeenCalledWith('on_compute_data', expect.objectContaining({ bufferName: 'positions' }));
  });
  it('compute_buffer_created stores handle in buffers map', () => {
    const { node, ctx, config } = attach();
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_buffer_created', name: 'buf1', handle: { h: 1 }, group: 0 });
    expect(node.__computeState.buffers.has('buf1')).toBe(true);
  });
  it('compute_buffer_created emits compute_update_bind_group', () => {
    const { node, ctx, config } = attach();
    ctx.emit.mockClear();
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_buffer_created', name: 'buf1', handle: {}, group: 0 });
    expect(ctx.emit).toHaveBeenCalledWith('compute_update_bind_group', expect.objectContaining({ group: 0 }));
  });
  it('compute_bind_group_created stores bind group', () => {
    const { node, ctx, config } = attach();
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_bind_group_created', group: 0, handle: { bg: 1 } });
  ...
  it('compute_create_buffer emits compute_allocate_buffer with correct size (f32=4 bytes)', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('compute_allocate_buffer', expect.objectContaining({ size: 256 })); // 64 * 4
  });
  it('compute_create_buffer uses mat4 (64 bytes)', () => {
    ...
    computeHandler.onEvent!(node, config, ctx, {
      type: 'compute_create_buffer',
      binding: { name: 'transforms', group: 0, binding: 0, usage: 'read_write', dataType: 'mat4', size: 10 },
    });
    expect(ctx.emit).toHaveBeenCalledWith('compute_allocate_buffer', expect.objectContaining({ size: 640 })); // 10 * 64
  });
});
...
describe('computeHandler.onEvent — complete, error, query', () => {
  ...
  it('compute_complete emits on_compute_complete with executionCount', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('on_compute_complete', expect.objectContaining({ executionCount: 1, executionTime: 3 }));
  });
  it('compute_error emits on_compute_error', () => {
    const { node, ctx, config } = attach();
    ctx.emit.mockClear();
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_error', error: 'GPU OOM' });
    expect(ctx.emit).toHaveBeenCalledWith('on_compute_error', expect.objectContaining({ error: 'GPU OOM' }));
  });
  it('compute_set_shader emits compute_compile_shader', () => {
    const { node, ctx, config } = attach({ workgroup_size: [32, 1, 1] });
    ctx.emit.mockClear();
    computeHandler.onEvent!(node, config, ctx, { type: 'compute_set_shader', source: 'new shader' });
    expect(ctx.emit).toHaveBeenCalledWith('compute_compile_shader', expect.objectContaining({ source: 'new shader', workgroupSize: [32, 1, 1] }));
  });
  it('compute_query emits compute_info with isReady and executionCount', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('compute_info', expect.objectContaining({
      queryId: 'q1',
      isReady: false,
      executionCount: 1,
      dispatch: [2, 2, 2],
...
describe('computeHandler.onUpdate', () => {
  ...
  it('emits compute_execute when dispatch_on_update=true and ready', () => {
    ...
    ctx.emit.mockClear();
    computeHandler.onUpdate!(node, config, ctx, 1);
    expect(ctx.emit).toHaveBeenCalledWith('compute_execute', expect.objectContaining({ dispatch: [1, 1, 1] }));
  });
