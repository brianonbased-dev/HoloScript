...
function makeNode(extras: any = {}) { return { id: 'portable_node', ...extras }; }
...
describe('portableHandler.onAttach', () => {
  ...
  it('isExportReady=true after attach (clean node score ≥ 0.5)', () => expect(attach().node.__portableState.isExportReady).toBe(true));
  it('lastExportTime=0', () => expect(attach().node.__portableState.lastExportTime).toBe(0));
  it('exportedFormats starts empty', () => expect(attach().node.__portableState.exportedFormats.size).toBe(0));
  it('emits portable_analysis_complete on attach', () => {
    const { ctx } = attach();
    expect(ctx.emit).toHaveBeenCalledWith('portable_analysis_complete', expect.objectContaining({ score: expect.any(Number) }));
  });
  ...
  it('customShader adds warning', () => {
    const { node } = attach({}, { customShader: true });
    expect(node.__portableState.warnings.some((w: string) => w.toLowerCase().includes('shader'))).toBe(true);
  });
  it('preserve_physics adds warning and reduces score', () => {
    ...
    expect(node.__portableState.warnings.some((w: string) => w.toLowerCase().includes('physics'))).toBe(true);
  });
  it('scripts without cross_platform adds warning', () => {
    const { node } = attach({ cross_platform: false }, { scripts: ['someScript'] });
    expect(node.__portableState.warnings.some((w: string) => w.toLowerCase().includes('script'))).toBe(true);
  });
...
describe('portableHandler.onEvent — portable_export', () => {
  it('emits portable_generate_export with format', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('portable_generate_export', expect.objectContaining({ format: 'glb' }));
  });
  it('uses first format when none specified', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('portable_generate_export', expect.objectContaining({ format: 'usdz' }));
  });
  it('rejects unsupported format with on_portable_error', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('on_portable_error', expect.objectContaining({ error: expect.stringContaining('fbx') }));
  });
  it('includes metadataStandard, preserveAnimations, optimizeForWeb in export event', () => {
    const { node, ctx, config } = attach({ export_formats: ['gltf'], preserve_animations: true, optimize_for_web: true });
    ctx.emit.mockClear();
...
describe('portableHandler.onEvent — portable_export_complete', () => {
  it('sets isExportReady=true', () => {
    ...
    portableHandler.onEvent!(node, config, ctx, { type: 'portable_export_complete', format: 'gltf', size: 1024, url: 'https://cdn/asset.gltf' });
    expect(node.__portableState.isExportReady).toBe(true);
  });
  it('adds format to exportedFormats set', () => {
    const { node, ctx, config } = attach();
    portableHandler.onEvent!(node, config, ctx, { type: 'portable_export_complete', format: 'glb', size: 512, url: 'x' });
    expect(node.__portableState.exportedFormats.has('glb')).toBe(true);
  });
  it('updates lastExportTime', () => {
    const before = Date.now();
    const { node, ctx, config } = attach();
    portableHandler.onEvent!(node, config, ctx, { type: 'portable_export_complete', format: 'gltf', size: 0, url: '' });
    expect(node.__portableState.lastExportTime).toBeGreaterThanOrEqual(before);
  });
  it('emits on_asset_ported', () => {
    const { node, ctx, config } = attach();
    ctx.emit.mockClear();
    portableHandler.onEvent!(node, config, ctx, { type: 'portable_export_complete', format: 'usdz', size: 2048, url: 'https://cdn/a.usdz' });
    expect(ctx.emit).toHaveBeenCalledWith('on_asset_ported', expect.objectContaining({ format: 'usdz', url: 'https://cdn/a.usdz' }));
  });
  it('tracks multiple formats separately', () => {
    const { node, ctx, config } = attach();
    portableHandler.onEvent!(node, config, ctx, { type: 'portable_export_complete', format: 'gltf', size: 0, url: '' });
    portableHandler.onEvent!(node, config, ctx, { type: 'portable_export_complete', format: 'glb', size: 0, url: '' });
    expect(Array.from(node.__portableState.exportedFormats)).toEqual(expect.arrayContaining(['gltf', 'glb']));
  });
});
...
describe('portableHandler.onEvent — portable_import', () => {
  it('emits portable_process_import', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('portable_process_import', expect.objectContaining({ format: 'gltf', data }));
  });
  it('defaults applyToNode=true when not specified', () => {
    const { node, ctx, config } = attach();
    ctx.emit.mockClear();
    portableHandler.onEvent!(node, config, ctx, { type: 'portable_import', data: new ArrayBuffer(0), format: 'vrm' });
...
describe('portableHandler.onEvent — portable_import_complete', () => {
  it('emits on_asset_imported', () => {
    const { node, ctx, config } = attach();
    ctx.emit.mockClear();
    portableHandler.onEvent!(node, config, ctx, { type: 'portable_import_complete', format: 'gltf' });
    expect(ctx.emit).toHaveBeenCalledWith('on_asset_imported', expect.objectContaining({ format: 'gltf' }));
  });
});
...
describe('portableHandler.onEvent — portable_validate', () => {
  it('emits portable_validation_result', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('portable_validation_result', expect.objectContaining({ score: expect.any(Number), isReady: expect.any(Boolean) }));
  });
...
describe('portableHandler.onEvent — portable_get_metadata', () => {
  it('emits portable_extract_metadata with standard', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('portable_extract_metadata', expect.objectContaining({ standard: 'vrm_meta' }));
  });
});
...
describe('portableHandler.onEvent — portable_query', () => {
  it('emits portable_info snapshot', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('portable_info', expect.objectContaining({
      queryId: 'pq1',
      isExportReady: true,
      exportedFormats: [],
