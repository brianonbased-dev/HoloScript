...
function makeNode() { return { id: 'role_node' }; }
...
describe('roleHandler.defaultConfig', () => {
  ...
  it('role_hierarchy has guest/user/editor/admin', () => {
    expect(Object.keys(d.role_hierarchy)).toEqual(expect.arrayContaining(['guest', 'user', 'editor', 'admin']));
  });
});
...
describe('roleHandler.onAttach', () => {
  ...
  it('currentRole = config.role_id', () => expect(attach({ role_id: 'admin' }).node.__roleState.currentRole).toBe('admin'));
  it('pendingRoleChange=null', () => expect(attach().node.__roleState.pendingRoleChange).toBeNull());
  ...
  it('no badge emit when display_badge=false', () => {
    expect(attach({ display_badge: false }).ctx.emit).not.toHaveBeenCalledWith('role_show_badge', expect.anything());
  });
  it('emits role_show_badge when display_badge=true', () => {
    const { ctx } = attach({ display_badge: true, role_id: 'admin', badge_color: '#ff0000' });
    expect(ctx.emit).toHaveBeenCalledWith('role_show_badge', expect.objectContaining({
      role: 'admin',
      color: '#ff0000',
    }));
  });
...
describe('roleHandler.onAttach — effective permissions', () => {
  it('user role gets view+interact from hierarchy', () => {
    const { node } = attach({ role_id: 'user', permissions: [], role_hierarchy: DEFAULT_HIERARCHY });
  ...
  it('admin role gets all permissions', () => {
    const { node } = attach({ role_id: 'admin', permissions: [], role_hierarchy: DEFAULT_HIERARCHY });
    const perms = node.__roleState.effectivePermissions;
  ...
  it('guest has only view', () => {
    const { node } = attach({ role_id: 'guest', permissions: [], role_hierarchy: DEFAULT_HIERARCHY });
    const perms = node.__roleState.effectivePermissions;
    expect(perms.has('view')).toBe(true);
    expect(perms.has('interact')).toBe(false);
  });
  it('config.permissions merged in', () => {
    const { node } = attach({ role_id: 'guest', permissions: ['transfer'], role_hierarchy: DEFAULT_HIERARCHY });
...
describe('roleHandler.onUpdate — pending role change', () => {
  ...
  it('applies pendingRoleChange and emits on_role_change', () => {
    const { node, config, ctx } = attach({ role_id: 'user', role_hierarchy: DEFAULT_HIERARCHY, permissions: [] });
    ...
    expect(ctx.emit).toHaveBeenCalledWith('on_role_change', expect.objectContaining({
      previousRole: 'user',
      newRole: 'admin',
    }));
  });
  it('permissions recalculated after role change', () => {
    const { node, config, ctx } = attach({ role_id: 'guest', role_hierarchy: DEFAULT_HIERARCHY, permissions: [] });
  ...
  it('emits role_update_badge after change when display_badge=true', () => {
    const { node, config, ctx } = attach({
      role_id: 'user', display_badge: true, role_hierarchy: DEFAULT_HIERARCHY, permissions: []
    });
    node.__roleState.pendingRoleChange = 'admin';
    ctx.emit.mockClear();
    roleHandler.onUpdate!(node, config, ctx, 0);
    expect(ctx.emit).toHaveBeenCalledWith('role_update_badge', expect.objectContaining({ role: 'admin' }));
  });
  it('no badge emit when display_badge=false', () => {
    const { node, config, ctx } = attach({
      role_id: 'user', display_badge: false, role_hierarchy: DEFAULT_HIERARCHY, permissions: []
    });
    node.__roleState.pendingRoleChange = 'admin';
    ctx.emit.mockClear();
    roleHandler.onUpdate!(node, config, ctx, 0);
...
describe('roleHandler.onEvent — role_check_permission', () => {
  it('emits role_permission_result granted=true for present permission', () => {
    const { node, config, ctx } = attach({ role_id: 'admin', permissions: [], role_hierarchy: DEFAULT_HIERARCHY });
    ctx.emit.mockClear();
    roleHandler.onEvent!(node, config, ctx, { type: 'role_check_permission', permission: 'delete', callbackId: 'c1' });
    expect(ctx.emit).toHaveBeenCalledWith('role_permission_result', expect.objectContaining({
      permission: 'delete',
      granted: true,
      callbackId: 'c1',
    }));
  ...
  it('emits granted=false for missing permission', () => {
    const { node, config, ctx } = attach({ role_id: 'guest', permissions: [], role_hierarchy: DEFAULT_HIERARCHY });
    ctx.emit.mockClear();
    roleHandler.onEvent!(node, config, ctx, { type: 'role_check_permission', permission: 'delete', callbackId: 'c2' });
    expect(ctx.emit).toHaveBeenCalledWith('role_permission_result', expect.objectContaining({ granted: false }));
  });
});
...
describe('roleHandler.onEvent — grant/revoke', () => {
  it('grant adds permission and emits on_permission_granted', () => {
    const { node, config, ctx } = attach({ role_id: 'guest', permissions: [], role_hierarchy: DEFAULT_HIERARCHY });
    ctx.emit.mockClear();
    roleHandler.onEvent!(node, config, ctx, { type: 'role_grant_permission', permission: 'transfer' });
    expect(node.__roleState.effectivePermissions.has('transfer')).toBe(true);
    expect(ctx.emit).toHaveBeenCalledWith('on_permission_granted', expect.objectContaining({ permission: 'transfer' }));
  });
  it('revoke removes permission and emits on_permission_revoked', () => {
    const { node, config, ctx } = attach({ role_id: 'admin', permissions: [], role_hierarchy: DEFAULT_HIERARCHY });
    ctx.emit.mockClear();
    roleHandler.onEvent!(node, config, ctx, { type: 'role_revoke_permission', permission: 'delete' });
    expect(node.__roleState.effectivePermissions.has('delete')).toBe(false);
    expect(ctx.emit).toHaveBeenCalledWith('on_permission_revoked', expect.objectContaining({ permission: 'delete' }));
  });
});
...
describe('roleHandler.onEvent — role_get_history', () => {
  it('emits role_history_result with history', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('role_history_result', expect.objectContaining({
      callbackId: 'h1',
...
describe('roleHandler.onEvent — role_can_perform', () => {
  it('canPerform=true for admin on modify', () => {
    const { node, config, ctx } = attach({ role_id: 'admin', permissions: [], role_hierarchy: DEFAULT_HIERARCHY });
    ctx.emit.mockClear();
    roleHandler.onEvent!(node, config, ctx, { type: 'role_can_perform', action: 'modify', callbackId: 'p1' });
    expect(ctx.emit).toHaveBeenCalledWith('role_action_check_result', expect.objectContaining({
      action: 'modify',
      canPerform: true,
      callbackId: 'p1',
    }));
  ...
  it('canPerform=false for guest on remove', () => {
    const { node, config, ctx } = attach({ role_id: 'guest', permissions: [], role_hierarchy: DEFAULT_HIERARCHY });
    ctx.emit.mockClear();
    roleHandler.onEvent!(node, config, ctx, { type: 'role_can_perform', action: 'remove', callbackId: 'p2' });
    expect(ctx.emit).toHaveBeenCalledWith('role_action_check_result', expect.objectContaining({
      canPerform: false,
  ...
  it('unknown action defaults to interact requirement', () => {
    const { node, config, ctx } = attach({ role_id: 'user', permissions: [], role_hierarchy: DEFAULT_HIERARCHY });
    ctx.emit.mockClear();
    roleHandler.onEvent!(node, config, ctx, { type: 'role_can_perform', action: 'do_unknown_thing' });
...
describe('roleHandler.onEvent — role_query', () => {
  it('emits role_info snapshot', () => {
    const { node, config, ctx } = attach({ role_id: 'editor', display_badge: false, permissions: [], role_hierarchy: DEFAULT_HIERARCHY });
    ctx.emit.mockClear();
    roleHandler.onEvent!(node, config, ctx, { type: 'role_query', queryId: 'q1' });
    expect(ctx.emit).toHaveBeenCalledWith('role_info', expect.objectContaining({
      queryId: 'q1',
      currentRole: 'editor',
      displayBadge: false,
    }));
