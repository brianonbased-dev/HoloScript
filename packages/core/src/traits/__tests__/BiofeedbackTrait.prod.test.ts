...
function makeNode() { return {} as any; }
function makeCtx(emitFn = vi.fn()) { return { emit: emitFn } as any; }

describe('BiofeedbackTrait — defaultConfig', () => {
  it('sources=[heart_rate]', () => expect(biofeedbackHandler.defaultConfig.sources).toEqual(['heart_rate']));
  ...
  it('emit_on_threshold=true', () => expect(biofeedbackHandler.defaultConfig.emit_on_threshold).toBe(true));
...
describe('BiofeedbackTrait — onAttach', () => {
  it('emits biofeedback_connect with sources', () => {
    const node = makeNode(); const emit = vi.fn();
    biofeedbackHandler.onAttach!(node, biofeedbackHandler.defaultConfig, makeCtx(emit));
    expect(emit).toHaveBeenCalledWith('biofeedback_connect', expect.objectContaining({
      sources: ['heart_rate'], sampleRateHz: 10,
    }));
  });
  ...
  it('multi-source config forwarded', () => {
    const node = makeNode(); const emit = vi.fn();
    const cfg = { ...biofeedbackHandler.defaultConfig, sources: ['heart_rate', 'gsr', 'pupil'] as any };
...
describe('BiofeedbackTrait — onEvent: biofeedback_sample', () => {
  it('stores sample in samples map', () => {
    ...
    biofeedbackHandler.onEvent!(node, biofeedbackHandler.defaultConfig, makeCtx(),
      { type: 'biofeedback_sample', source: 'heart_rate', value: 80 });
  ...
  it('normalized value clamped to [0,1] for out-of-range raw', () => {
    ...
    biofeedbackHandler.onAttach!(node, biofeedbackHandler.defaultConfig, makeCtx());
    biofeedbackHandler.onEvent!(node, biofeedbackHandler.defaultConfig, makeCtx(),
      { type: 'biofeedback_sample', source: 'heart_rate', value: 300 }); // above max 200
    const sample = (node as any).__biofeedbackState.samples.get('heart_rate');
  ...
  it('emits biofeedback_reading', () => {
    ...
    expect(emit).toHaveBeenCalledWith('biofeedback_reading', expect.objectContaining({ source: 'heart_rate', raw: 90 }));
  });
  ...
  it('uses custom range when provided', () => {
    ...
    // (50-0)/(100-0) = 0.5
    expect((node as any).__biofeedbackState.samples.get('heart_rate')?.normalized).toBeCloseTo(0.5, 2);
  });
});
...
describe('BiofeedbackTrait — threshold edge detection', () => {
  ...
  it('crossing into low zone emits biofeedback_threshold_crossed (low)', () => {
    ...
    biofeedbackHandler.onEvent!(node, cfg, makeCtx(emit),
      { type: 'biofeedback_sample', source: 'heart_rate', value: 45 }); // below 50
    expect(emit).toHaveBeenCalledWith('biofeedback_threshold_crossed', expect.objectContaining({
      source: 'heart_rate', direction: 'low', value: 45,
    }));
  });
  it('crossing into high zone emits biofeedback_threshold_crossed (high)', () => {
    ...
    biofeedbackHandler.onAttach!(node, cfg, makeCtx());
    biofeedbackHandler.onEvent!(node, cfg, makeCtx(emit),
      { type: 'biofeedback_sample', source: 'heart_rate', value: 160 }); // above 150
    expect(emit).toHaveBeenCalledWith('biofeedback_threshold_crossed', expect.objectContaining({ direction: 'high' }));
  });
  it('staying in normal range: no threshold event', () => {
    const node = makeNode(); const emit = vi.fn();
    biofeedbackHandler.onAttach!(node, cfg, makeCtx());
    biofeedbackHandler.onEvent!(node, cfg, makeCtx(emit),
      { type: 'biofeedback_sample', source: 'heart_rate', value: 80 }); // normal
  ...
  it('same edge twice → no duplicate event (edge detection)', () => {
    ...
    biofeedbackHandler.onAttach!(node, cfg, makeCtx());
    biofeedbackHandler.onEvent!(node, cfg, makeCtx(emit),
      { type: 'biofeedback_sample', source: 'heart_rate', value: 45 }); // → low
    biofeedbackHandler.onEvent!(node, cfg, makeCtx(emit),
      { type: 'biofeedback_sample', source: 'heart_rate', value: 40 }); // still low
  ...
  it('emit_on_threshold=false: no threshold events even when crossing', () => {
    ...
    biofeedbackHandler.onEvent!(node, noCfg, makeCtx(emit),
      { type: 'biofeedback_sample', source: 'heart_rate', value: 30 });
    expect(emit).not.toHaveBeenCalledWith('biofeedback_threshold_crossed', expect.anything());
  });
});
...
describe('BiofeedbackTrait — onEvent: query and calibrate', () => {
  it('biofeedback_query for specific source → biofeedback_response with sample', () => {
    ...
    biofeedbackHandler.onAttach!(node, biofeedbackHandler.defaultConfig, makeCtx());
    biofeedbackHandler.onEvent!(node, biofeedbackHandler.defaultConfig, makeCtx(),
      { type: 'biofeedback_sample', source: 'heart_rate', value: 72 });
    biofeedbackHandler.onEvent!(node, biofeedbackHandler.defaultConfig, makeCtx(emit),
      { type: 'biofeedback_query', queryId: 'bq1', source: 'heart_rate' });
    expect(emit).toHaveBeenCalledWith('biofeedback_response', expect.objectContaining({
      queryId: 'bq1', source: 'heart_rate',
    }));
  });
  it('biofeedback_query for unknown source → sample=null', () => {
    ...
    biofeedbackHandler.onAttach!(node, biofeedbackHandler.defaultConfig, makeCtx());
    biofeedbackHandler.onEvent!(node, biofeedbackHandler.defaultConfig, makeCtx(emit),
      { type: 'biofeedback_query', queryId: 'bq2', source: 'heart_rate' });
    expect(emit).toHaveBeenCalledWith('biofeedback_response', expect.objectContaining({ sample: null }));
  });
...
describe('BiofeedbackTrait — multi-source independence', () => {
  it('multiple sources stored independently', () => {
    ...
    biofeedbackHandler.onAttach!(node, cfg, makeCtx());
    biofeedbackHandler.onEvent!(node, cfg, makeCtx(), { type: 'biofeedback_sample', source: 'heart_rate', value: 75 });
    biofeedbackHandler.onEvent!(node, cfg, makeCtx(), { type: 'biofeedback_sample', source: 'gsr', value: 5 });
    const st = (node as any).__biofeedbackState;
