...
function authFail() {
  return { ok: false, status: 401, json: vi.fn() };
}
      ...
      outputs:
        status === 'complete'
          ? [
              {
                type: 'frame',
                url: 'https://cdn.rndr.com/out.png',
                format: 'png',
...
describe('submitRenderJob — retry logic via onEvent', () => {
  async function attachConnected() {
    const n = node();
    const c = ctx();
    await renderNetworkHandler.onAttach?.(
      n,
      { ...renderNetworkHandler.defaultConfig, api_key: '' } as any,
      c as any
    );
  ...
  it('submits job successfully (1st attempt OK)', async () => {
    ...
    fetchMock.mockResolvedValueOnce({
      ok: true,
      status: 200,
      json: vi.fn().mockResolvedValue({ job_id: 'api-job-001' }),
    });
    // And poll → complete
  ...
  it('retries on 500 then succeeds (2nd attempt)', async () => {
    ...
    fetchMock
      ...
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: vi.fn().mockResolvedValue({ job_id: 'retry-job' }),
      }); // attempt 2
...
describe('refreshCredits via onEvent', () => {
  ...
  it('updates balance on successful API response', async () => {
    ...
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: vi.fn().mockResolvedValue({ balance: 300, pending: 5, spent: 100, earned: 10 }),
    });
...
describe('webhook notifications on job complete', () => {
  ...
  it('does NOT POST webhook when webhook_url is empty', async () => {
    ...
    const anyWebhook = fetchMock.mock.calls.filter(([url]: [string]) =>
