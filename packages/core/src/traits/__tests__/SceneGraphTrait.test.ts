...
describe('SceneGraphTrait', () => {
  ...
  it('skip_existing prevents duplicate', () => {
    ...
    sendEvent(sceneGraphHandler, node, c2, ctx, { type: 'scene_graph_add_node', childId: 'child1', parentId: 'root' });
    sendEvent(sceneGraphHandler, node, c2, ctx, { type: 'scene_graph_add_node', childId: 'child1', parentId: 'root' });
    expect(getEventCount(ctx, 'on_node_added')).toBe(1);
  });

  it('remove node cleans up', () => {
    sendEvent(sceneGraphHandler, node, cfg, ctx, { type: 'scene_graph_add_node', childId: 'child1', parentId: 'root' });
    sendEvent(sceneGraphHandler, node, cfg, ctx, { type: 'scene_graph_remove_node', nodeId: 'child1' });
    expect((node as any).__sceneGraphState.nodes.has('child1')).toBe(false);
  });

  it('reparent moves node', () => {
    sendEvent(sceneGraphHandler, node, cfg, ctx, { type: 'scene_graph_add_node', childId: 'a', parentId: 'root' });
    sendEvent(sceneGraphHandler, node, cfg, ctx, { type: 'scene_graph_add_node', childId: 'b', parentId: 'root' });
    sendEvent(sceneGraphHandler, node, cfg, ctx, { type: 'scene_graph_reparent', nodeId: 'a', newParentId: 'b' });
    const s = (node as any).__sceneGraphState;
  ...
  it('set transform updates node', () => {
    ...
    expect((node as any).__sceneGraphState.nodes.get('child1').localTransform.position).toEqual([5, 0, 0]);
    expect(getEventCount(ctx, 'scene_graph_transform_updated')).toBe(1);
  });

  it('get children returns list', () => {
    sendEvent(sceneGraphHandler, node, cfg, ctx, { type: 'scene_graph_add_node', childId: 'c1', parentId: 'root' });
    sendEvent(sceneGraphHandler, node, cfg, ctx, { type: 'scene_graph_get_children', parentId: 'root', callbackId: 'cb1' });
    expect(getEventCount(ctx, 'scene_graph_children_result')).toBe(1);
  });

  it('import bulk loads nodes', () => {
    const data = [
      { id: 'imp1', name: 'Imported', parent: null, children: [], localTransform: { position: [0, 0, 0], rotation: [0, 0, 0, 1], scale: [1, 1, 1] } },
    ];
  ...
  it('update recalculates depth', () => {
    sendEvent(sceneGraphHandler, node, cfg, ctx, { type: 'scene_graph_add_node', childId: 'c1', parentId: 'root' });
    sendEvent(sceneGraphHandler, node, cfg, ctx, { type: 'scene_graph_add_node', childId: 'c2', parentId: 'c1' });
