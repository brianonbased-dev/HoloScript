...
const defaultConfig = { webhook_url: 'https://hooks.slack.com/services/test', default_channel: '#general' };
...
describe('SlackTrait', () => {
  ...
  it('slack:send calls fetch and emits slack:sent on success', async () => {
    ...
    await vi.waitFor(() => expect(node.emit).toHaveBeenCalledWith('slack:sent', expect.objectContaining({ channel: '#general', status: 200 })));
  ...
  it('slack:send emits slack:error when webhook_url is empty', () => {
    const node = makeNode();
    slackHandler.onAttach!(node as never, { webhook_url: '', default_channel: '#general' }, makeCtx(node) as never);
    slackHandler.onEvent!(node as never, { webhook_url: '', default_channel: '#general' }, makeCtx(node) as never, {
      type: 'slack:send', text: 'hello',
    } as never);
    expect(node.emit).toHaveBeenCalledWith('slack:error', expect.objectContaining({ error: 'No webhook_url configured' }));
  });
});
