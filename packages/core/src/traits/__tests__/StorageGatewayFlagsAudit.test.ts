...
describe('S3UploadTrait', () => {
  it('should upload and track count', () => {
    ...
    sendEvent(s3UploadHandler, node, { bucket: 'assets', max_size_mb: 100 }, ctx, {
      type: 's3:upload',
      key: 'img.png',
      size: 1024,
    });
...
describe('FileSystemTrait', () => {
  it('should write and read', () => {
    ...
    sendEvent(fileSystemHandler, node, { root: '/' }, ctx, {
      type: 'fs:write',
      path: '/a.txt',
      content: 'hello',
    });
...
describe('BlobStoreTrait', () => {
  it('should put and get', () => {
    ...
    sendEvent(blobStoreHandler, node, { max_blob_mb: 500 }, ctx, {
      type: 'blob:put',
      blobId: 'b1',
      size: 2048,
    });
...
describe('GraphqlTrait', () => {
  it('should register resolver and query', () => {
    ...
    sendEvent(graphqlHandler, node, { depth_limit: 10 }, ctx, {
      type: 'gql:register',
      typeName: 'User',
      field: 'name',
    });
...
describe('RestEndpointTrait', () => {
  it('should register route and handle request', () => {
    ...
    sendEvent(restEndpointHandler, node, { base_path: '/api' }, ctx, {
      type: 'rest:register',
      method: 'GET',
      path: '/users',
    });
...
describe('RpcTrait', () => {
  it('should register and call', () => {
    ...
    sendEvent(rpcHandler, node, { timeout_ms: 5000 }, ctx, {
      type: 'rpc:register',
      method: 'greet',
    });
...
describe('RolloutTrait', () => {
  it('should set and check rollout', () => {
    ...
    sendEvent(rolloutHandler, node, { default_percentage: 0 }, ctx, {
      type: 'rollout:set',
      feature: 'new_ui',
      percentage: 50,
    });
...
describe('CanaryTrait', () => {
  it('should start and promote canary', () => {
    ...
    sendEvent(canaryHandler, node, { initial_percentage: 5, increment: 10 }, ctx, {
      type: 'canary:start',
      version: '2.0',
      percentage: 10,
    });
...
describe('ChangeTrackingTrait', () => {
  it('should record and query changes', () => {
    ...
    sendEvent(changeTrackingHandler, node, { max_history: 100 }, ctx, {
      type: 'change:record',
      entityId: 'e1',
      field: 'name',
      oldValue: 'A',
...
describe('DataLineageTrait', () => {
  it('should register, transform, and trace', () => {
    ...
    sendEvent(dataLineageHandler, node, { max_depth: 50 }, ctx, {
