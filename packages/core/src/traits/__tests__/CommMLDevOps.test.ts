...
describe('SmsTrait', () => {
  ...
  it('should reject SMS over max_length', () => {
    ...
    sendEvent(smsHandler, node, { max_length: 5 }, ctx, {
      type: 'sms:send',
      to: '+1',
      message: 'too long msg',
    });
...
describe('PushNotificationTrait', () => {
  it('should send push notification', () => {
    ...
    sendEvent(pushNotificationHandler, node, {}, ctx, {
      type: 'push:send',
      token: 'abc',
      title: 'Test',
    });
...
describe('MqttPubTrait', () => {
  it('should publish mqtt message', () => {
    ...
    sendEvent(mqttPubHandler, node, {}, ctx, {
      type: 'mqtt:publish',
      topic: 'sensors/temp',
      payload: 22,
    });
...
describe('FineTuneTrait', () => {
  it('should start fine-tuning job', () => {
    const node = createMockNode('ft');
    const ctx = createMockContext();
    attachTrait(fineTuneHandler, node, {}, ctx);
    sendEvent(fineTuneHandler, node, {}, ctx, {
      type: 'finetune:start',
      modelId: 'base',
      dataset: 'train.jsonl',
    });
...
describe('VectorSearchTrait', () => {
  it('should index and search', () => {
    const node = createMockNode('vs');
    const ctx = createMockContext();
    attachTrait(vectorSearchHandler, node, {}, ctx);
    sendEvent(vectorSearchHandler, node, {}, ctx, {
      type: 'vsearch:index',
      collection: 'docs',
      docId: 'd1',
      vector: [1, 0, 0],
...
describe('DeployTrait', () => {
  it('should start deployment with stages', () => {
    const node = createMockNode('dep');
    const ctx = createMockContext();
    attachTrait(deployHandler, node, {}, ctx);
    sendEvent(deployHandler, node, {}, ctx, {
      type: 'deploy:start',
      version: '1.0',
      target: 'prod',
    });
...
describe('CanaryTrait', () => {
  it('should start and promote canary', () => {
    const node = createMockNode('can');
    const ctx = createMockContext();
    attachTrait(canaryHandler, node, {}, ctx);
    sendEvent(canaryHandler, node, {}, ctx, {
      type: 'canary:start',
      version: '2.0',
      percentage: 10,
    });
...
describe('FeatureFlagTrait', () => {
  it('should define and evaluate flag', () => {
    const node = createMockNode('ff');
    const ctx = createMockContext();
    attachTrait(featureFlagHandler, node, {}, ctx);
    sendEvent(featureFlagHandler, node, {}, ctx, {
      type: 'flag:define',
      flagId: 'dark_mode',
      defaultValue: true,
    });
...
describe('SecretTrait', () => {
  it('should store and retrieve secret', () => {
    const node = createMockNode('sec');
    const ctx = createMockContext();
    attachTrait(secretHandler, node, {}, ctx);
    sendEvent(secretHandler, node, {}, ctx, {
      type: 'secret:store',
      secretId: 'api_key',
      value: 'sk-123',
    });
