...
function makeNode() { return { id: 'rag_test' } as any; }
...
function st(node: any) { return node.__ragState as any; }
...
describe('RAGKnowledgeTrait — onAttach', () => {
  ...
  it('emits rag_init with knowledge_sources and chunk_size', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('rag_init', expect.objectContaining({
      knowledgeSources: [],
      chunkSize: 256,
    }));
  });
  ...
  it('emits rag_index_sources and sets is_indexing when sources provided', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('rag_index_sources', expect.objectContaining({
...
describe('RAGKnowledgeTrait — onDetach', () => {
  it('always emits rag_persist (even with empty docs)', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('rag_persist', expect.objectContaining({ documents: [] }));
  });

  it('persists ingested documents', () => {
    const node = makeNode();
    const { cfg, ctx } = attach(node, { chunk_size: 50, overlap: 0 });
    fire(node, cfg, ctx, { type: 'rag_ingest_document', id: 'doc1', content: 'Hello world', metadata: {} });
    ctx.emit.mockClear();
    ragKnowledgeHandler.onDetach!(node, cfg, ctx as any);
...
describe('RAGKnowledgeTrait — onEvent: rag_ingest_document', () => {
  it('indexes document, produces chunks, emits rag_request_embeddings + on_document_ingested', () => {
    ...
    fire(node, cfg, ctx, { type: 'rag_ingest_document', id: 'd1', content, metadata: { source: 'test' } });
    ...
    expect(ctx.emit).toHaveBeenCalledWith('rag_request_embeddings', expect.objectContaining({
      documentId: 'd1',
      chunks: doc.chunks,
    }));
    expect(ctx.emit).toHaveBeenCalledWith('on_document_ingested', expect.objectContaining({
      documentId: 'd1',
      chunkCount: 2,
    }));
  });
  ...
  it('chunk overlap reduces stride between chunks', () => {
    ...
    fire(node, cfg, ctx, { type: 'rag_ingest_document', id: 'd2', content: '123456789012345', metadata: {} });
...
describe('RAGKnowledgeTrait — onEvent: rag_embeddings_ready', () => {
  it('marks document as indexed and emits on_document_indexed', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('on_document_indexed', expect.objectContaining({ documentId: 'd1' }));
  });
  ...
  it('unknown documentId is handled gracefully', () => {
    const node = makeNode();
    const { cfg, ctx } = attach(node);
    expect(() => fire(node, cfg, ctx, { type: 'rag_embeddings_ready', documentId: 'ghost' })).not.toThrow();
...
describe('RAGKnowledgeTrait — onEvent: rag_query', () => {
  it('stores last_query and emits rag_request_retrieval with k', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('rag_request_retrieval', expect.objectContaining({
...
describe('RAGKnowledgeTrait — onEvent: rag_retrieval_results', () => {
  it('maps chunk IDs to doc chunks and emits on_knowledge_retrieved', () => {
    ...
    fire(node, cfg, ctx, { type: 'rag_ingest_document', id: 'doc1', content: 'chunk zero content here', metadata: {} });

    ctx.emit.mockClear();
    fire(node, cfg, ctx, { type: 'rag_retrieval_results', results: [
      { id: 'doc1_chunk_0', similarity: 0.85, metadata: {} },
    ]});

    expect(ctx.emit).toHaveBeenCalledWith('on_knowledge_retrieved', expect.objectContaining({
      query: '',
    }));
    const call = (ctx.emit as any).mock.calls.find((c: any[]) => c[0] === 'on_knowledge_retrieved')?.[1];
  ...
  it('rerank=true: keyword match boosts score by 0.1', () => {
    ...
    fire(node, cfg, ctx, { type: 'rag_ingest_document', id: 'x', content: 'HoloScript is great', metadata: {} });

    ctx.emit.mockClear();
    fire(node, cfg, ctx, { type: 'rag_retrieval_results', results: [
      { id: 'x_chunk_0', similarity: 0.7, metadata: {} },
    ]});
    const call = (ctx.emit as any).mock.calls.find((c: any[]) => c[0] === 'on_knowledge_retrieved')?.[1];
  ...
  it('rerank=true: no keyword match — score unchanged', () => {
    ...
    fire(node, cfg, ctx, { type: 'rag_ingest_document', id: 'y', content: 'some neutral content', metadata: {} });

    ctx.emit.mockClear();
    fire(node, cfg, ctx, { type: 'rag_retrieval_results', results: [
      { id: 'y_chunk_0', similarity: 0.6, metadata: {} },
    ]});
    const call = (ctx.emit as any).mock.calls.find((c: any[]) => c[0] === 'on_knowledge_retrieved')?.[1];
  ...
  it('rerank=true: results sorted descending by score', () => {
    ...
    fire(node, cfg, ctx, { type: 'rag_ingest_document', id: 'z', content: 'test content and more stuff', metadata: {} });
  ...
  it('missing chunk index falls back to empty string', () => {
    ...
    fire(node, cfg, ctx, { type: 'rag_retrieval_results', results: [
      { id: 'missing_doc_chunk_99', similarity: 0.9, metadata: {} },
    ]});
    const call = (ctx.emit as any).mock.calls.find((c: any[]) => c[0] === 'on_knowledge_retrieved')?.[1];
...
describe('RAGKnowledgeTrait — onEvent: rag_update_document', () => {
  it('replaces content and chunks, re-requests embeddings', () => {
    const node = makeNode();
    const { cfg, ctx } = attach(node, { chunk_size: 5, overlap: 0 });
    fire(node, cfg, ctx, { type: 'rag_ingest_document', id: 'up1', content: 'hello', metadata: {} });
    // Mark as indexed
    fire(node, cfg, ctx, { type: 'rag_embeddings_ready', documentId: 'up1' });
    expect(st(node).indexed_documents.get('up1').indexed).toBe(true);

    ctx.emit.mockClear();
    fire(node, cfg, ctx, { type: 'rag_update_document', documentId: 'up1', content: 'new content' });
    ...
    expect(ctx.emit).toHaveBeenCalledWith('rag_request_embeddings', expect.objectContaining({ documentId: 'up1' }));
  });

  it('unknown documentId is ignored gracefully', () => {
    const node = makeNode();
    const { cfg, ctx } = attach(node);
    expect(() => fire(node, cfg, ctx, { type: 'rag_update_document', documentId: 'ghost', content: 'x' })).not.toThrow();
