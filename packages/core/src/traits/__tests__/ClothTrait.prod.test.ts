...
describe('clothHandler.onAttach', () => {
  ...
  it('windForce={0,0,0}', () => expect(attach().node.__clothState.windForce).toEqual([0, 0, 0]));
  ...
  it('emits cloth_create', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith(
      'cloth_create',
      expect.objectContaining({ resolution: 4, stiffness: 0.9 })
    );
  });
...
describe('clothHandler.onEvent', () => {
  ...
  it('cloth_apply_force defaults radius to 0.5', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith(
      'cloth_external_force',
      expect.objectContaining({ radius: 0.5 })
    );
  });
  it('cloth_pin_vertex marks isPinned=true and emits cloth_update_pin', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith(
      'cloth_update_pin',
      expect.objectContaining({ vertex: [1, 2], pinned: true })
    );
  });
  ...
  it('cloth_pin_vertex graceful for out-of-bounds', () => {
    const { node, config, ctx } = attach({ resolution: 4 });
    expect(() =>
      clothHandler.onEvent!(node, config, ctx, { type: 'cloth_pin_vertex', x: 99, y: 99 })
    ).not.toThrow();
  });
