...
function makeNode() { return { id: 'pg_test' } as any; }
...
function st(node: any) { return node.__photogrammetryState as any; }
...
describe('PhotogrammetryTrait — onAttach', () => {
  ...
  it('emits photogrammetry_init with quality, featureMatching, autoAlign, geoReference', () => {
    const node = makeNode();
    const { ctx } = attach(node, { quality: 'high', feature_matching: 'superpoint', auto_align: false, geo_reference: true });
    expect(ctx.emit).toHaveBeenCalledWith('photogrammetry_init', expect.objectContaining({
      quality: 'high', featureMatching: 'superpoint', autoAlign: false, geoReference: true,
    }));
  });
});
...
describe('PhotogrammetryTrait — onEvent: photogrammetry_add_images', () => {
  it('accumulates imageCount and emits photogrammetry_upload', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('photogrammetry_upload', expect.objectContaining({
      images: imgs, sourceType: 'depth_images',
    }));
  });
...
describe('PhotogrammetryTrait — onEvent: photogrammetry_start', () => {
  ...
  it('starts processing when imageCount > 0: sets isProcessing+stage+progress=0, emits process', () => {
    const node = makeNode();
    const { cfg, ctx } = attach(node, { quality: 'ultra', mesh_simplification: 0.8, texture_resolution: 4096, mask_background: false });
    ...
    expect(ctx.emit).toHaveBeenCalledWith('photogrammetry_process', expect.objectContaining({
      quality: 'ultra', meshSimplification: 0.8, textureResolution: 4096, maskBackground: false,
    }));
  });
});
...
describe('PhotogrammetryTrait — onEvent: photogrammetry_progress', () => {
  it('updates stage and progress, emits on_photogrammetry_progress', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('on_photogrammetry_progress', expect.objectContaining({
      stage: 'aligning', progress: 35,
    }));
  });
});
...
describe('PhotogrammetryTrait — onEvent: photogrammetry_complete', () => {
  it('clears isProcessing, sets stage=complete, progress=100, stores mesh+boundingBox, emits both', () => {
    ...
    fire(node, cfg, ctx, { type: 'photogrammetry_complete', mesh, boundingBox: bb, vertexCount: 50000 });
    const s = st(node);
    expect(s.isProcessing).toBe(false);
    ...
    expect(ctx.emit).toHaveBeenCalledWith('photogrammetry_apply_mesh', expect.objectContaining({ mesh }));
    expect(ctx.emit).toHaveBeenCalledWith('on_capture_complete', expect.objectContaining({
      vertexCount: 50000, textureResolution: 2048,
    }));
  });
});
...
describe('PhotogrammetryTrait — onEvent: photogrammetry_error', () => {
  it('clears isProcessing and emits on_photogrammetry_error with stage', () => {
    ...
    expect(st(node).isProcessing).toBe(false);
    expect(ctx.emit).toHaveBeenCalledWith('on_photogrammetry_error', expect.objectContaining({
      error: 'OOM', stage: 'dense_cloud',
    }));
  });
});
...
describe('PhotogrammetryTrait — onEvent: photogrammetry_export', () => {
  it('emits photogrammetry_export_mesh with format and includeTexture=true by default', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('photogrammetry_export_mesh', expect.objectContaining({
      format: 'glb', includeTexture: true,
    }));
  });
  ...
  it('respects explicit format and includeTexture=false', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('photogrammetry_export_mesh', expect.objectContaining({
      format: 'obj', includeTexture: false,
    }));
  });
});
...
describe('PhotogrammetryTrait — onEvent: photogrammetry_query', () => {
  it('emits photogrammetry_info with full snapshot', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('photogrammetry_info', expect.objectContaining({
      queryId: 'pq1',
      isProcessing: false,
      stage: 'complete',
      progress: 100,
