...
const SAMPLE_BLUEPRINT = {
  ...
  environment: { skybox: 'night_city', fogDensity: 0.05, ambientColor: '#0a0a1a', ambientIntensity: 0.3 },
  camera: { fov: 75, near: 0.1, far: 2000, position: [0, 10, 50] as [number, number, number] },
  objects: [
    { id: 'building1', type: 'mesh' as const, geometry: 'box', position: [0, 25, 0] as [number, number, number], rotation: [0, 0, 0] as [number, number, number], scale: [10, 50, 10] as [number, number, number], material: 'glass_neon', traits: ['NeonLightTrait'] },
  ],
  lights: [
    { id: 'main_light', type: 'directional' as const, color: '#4444ff', intensity: 0.5, position: [100, 100, 50] as [number, number, number], castShadow: true },
  ],
  agents: [
    { id: 'guard', name: 'CityGuard', traits: ['PatrolTrait'], role: 'security', position: [5, 0, 5] as [number, number, number] },
  ],
  materials: {
    glass_neon: { type: 'holographic' as const, color: '#00ffff', emissive: '#00ffff', emissiveIntensity: 2, transparent: true, opacity: 0.6 },
  },
};
...
describe('SpatialAgentOrchestrator — scene_generate_from_blueprint', () => {
  ...
  it('increments totalGenerated', () => {
    const { node, ctx, config } = attach();
    spatialAgentHandler.onEvent(node, config, ctx, { type: 'scene_generate_from_blueprint', payload: { blueprint: SAMPLE_BLUEPRINT } });
  ...
  it('caps objects at max_objects', () => {
    const bigBlueprint = {
      ...
      objects: Array.from({ length: 100 }, (_, i) => ({
        id: `obj${i}`, type: 'mesh' as const, geometry: 'box',
        position: [i, 0, 0] as [number, number, number],
        rotation: [0, 0, 0] as [number, number, number],
        scale: [1, 1, 1] as [number, number, number],
        material: 'default', traits: [],
      })),
    };
    const { node, ctx, config } = attach({ max_objects: 5 });
    spatialAgentHandler.onEvent(node, config, ctx, { type: 'scene_generate_from_blueprint', payload: { blueprint: bigBlueprint } });
...
describe('SpatialAgentOrchestrator — scene_generate (LLM)', () => {
  ...
  it('emits scene_generation_started', () => {
    ...
    expect((ctx.of('scene_generation_started')[0].payload as any).prompt).toBe('A neon cyberpunk city');
  });
