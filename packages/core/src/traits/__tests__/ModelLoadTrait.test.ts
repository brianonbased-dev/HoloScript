...
const makeNode = () => ({
  ...
  emit: vi.fn(),
  __modelLoadState: undefined as unknown,
});
...
describe('ModelLoadTrait', () => {
  ...
  it('model:load registers model and emits model:loaded', () => {
    ...
    expect(node.emit).toHaveBeenCalledWith(
      'model:loaded',
      expect.objectContaining({ modelId: 'gpt4' })
    );
  });
  ...
  it('emits model:error when max_loaded exceeded', () => {
    const node = makeNode();
    modelLoadHandler.onAttach!(
      node as never,
      { max_loaded: 1, warmup_rounds: 1 },
      makeCtx(node) as never
    );
    modelLoadHandler.onEvent!(
      node as never,
