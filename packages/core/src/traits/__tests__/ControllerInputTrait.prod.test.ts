...
function makeNode() { return { id: 'ctrl_node' }; }
function makeCtx() { return { emit: vi.fn() }; }
function makeButtons(entries: [string, { pressed: boolean; touched: boolean; value: number }][] = []) {
...
describe('controllerInputHandler.onAttach', () => {
  it('creates __controllerInputState', () => expect(attach().node.__controllerInputState).toBeDefined());
  it('left.connected=false', () => expect(attach().node.__controllerInputState.left.connected).toBe(false));
  it('right.connected=false', () => expect(attach().node.__controllerInputState.right.connected).toBe(false));
  it('prevButtons.left is empty Map', () => expect(attach().node.__controllerInputState.prevButtons.left.size).toBe(0));
  it('prevTrigger.left=false', () => expect(attach().node.__controllerInputState.prevTrigger.left).toBe(false));
  it('prevGrip.right=false', () => expect(attach().node.__controllerInputState.prevGrip.right).toBe(false));
...
describe('controllerInputHandler.onUpdate — onPress mapping', () => {
  it('emits controller_action press on button transition', () => {
    const { node, config, ctx } = attach({ haptic_on_button: false, button_mapping: [{ action: 'jump', button: 'a', onPress: true }] });
    node.__controllerInputState.left.connected = true;
    node.__controllerInputState.left.buttons = makeButtons([['a', { pressed: true, touched: true, value: 1 }]]);
    ctx.emit.mockClear();
    controllerInputHandler.onUpdate!(node, config, ctx, 0.016);
    expect(ctx.emit).toHaveBeenCalledWith('controller_action', expect.objectContaining({ action: 'jump', hand: 'left', type: 'press' }));
  });
  ...
  it('emits haptic_pulse on press when haptic_on_button=true', () => {
    const { node, config, ctx } = attach({ haptic_on_button: true, haptic_intensity: 0.5, button_mapping: [{ action: 'jump', button: 'a', onPress: true }] });
    node.__controllerInputState.right.connected = true;
    node.__controllerInputState.right.buttons = makeButtons([['a', { pressed: true, touched: true, value: 1 }]]);
    ctx.emit.mockClear();
    controllerInputHandler.onUpdate!(node, config, ctx, 0.016);
    expect(ctx.emit).toHaveBeenCalledWith('haptic_pulse', expect.objectContaining({ intensity: 0.5 }));
  });
  ...
  it('skips mapping for wrong hand', () => {
    const { node, config, ctx } = attach({ haptic_on_button: false, button_mapping: [{ action: 'jump', button: 'a', hand: 'right', onPress: true }] });
    node.__controllerInputState.left.connected = true;
    node.__controllerInputState.left.buttons = makeButtons([['a', { pressed: true, touched: true, value: 1 }]]);
    ctx.emit.mockClear();
    controllerInputHandler.onUpdate!(node, config, ctx, 0.016);
...
describe('controllerInputHandler.onUpdate — onRelease', () => {
  it('emits controller_action type=release on button release', () => {
    const { node, config, ctx } = attach({ haptic_on_button: false, button_mapping: [{ action: 'stop', button: 'b', onRelease: true }] });
    node.__controllerInputState.left.connected = true;
    node.__controllerInputState.left.buttons = makeButtons([['b', { pressed: false, touched: false, value: 0 }]]);
    node.__controllerInputState.prevButtons.left.set('b', true);
    ctx.emit.mockClear();
    controllerInputHandler.onUpdate!(node, config, ctx, 0.016);
    expect(ctx.emit).toHaveBeenCalledWith('controller_action', expect.objectContaining({ type: 'release', button: 'b' }));
  });
});

describe('controllerInputHandler.onUpdate — onHold', () => {
  it('emits controller_action type=hold each frame while held', () => {
    const { node, config, ctx } = attach({ haptic_on_button: false, button_mapping: [{ action: 'fire', button: 'trigger', onHold: true }] });
    node.__controllerInputState.right.connected = true;
    node.__controllerInputState.right.buttons = makeButtons([['trigger', { pressed: true, touched: true, value: 1 }]]);
    ctx.emit.mockClear();
    controllerInputHandler.onUpdate!(node, config, ctx, 0.016);
    expect(ctx.emit).toHaveBeenCalledWith('controller_action', expect.objectContaining({ type: 'hold', delta: 0.016 }));
  });
});
...
describe('controllerInputHandler.onUpdate — trigger', () => {
  it('emits controller_trigger_press when crossing threshold upward', () => {
    ...
    ctx.emit.mockClear();
    controllerInputHandler.onUpdate!(node, config, ctx, 0.016);
    expect(ctx.emit).toHaveBeenCalledWith('controller_trigger_press', expect.objectContaining({ hand: 'left', value: 0.8 }));
  });
  it('emits controller_trigger_release when dropping below threshold', () => {
    ...
    ctx.emit.mockClear();
    controllerInputHandler.onUpdate!(node, config, ctx, 0.016);
    expect(ctx.emit).toHaveBeenCalledWith('controller_trigger_release', expect.objectContaining({ hand: 'right' }));
  });
...
describe('controllerInputHandler.onUpdate — grip', () => {
  it('emits controller_grip_press on grip threshold', () => {
    ...
    ctx.emit.mockClear();
    controllerInputHandler.onUpdate!(node, config, ctx, 0.016);
    expect(ctx.emit).toHaveBeenCalledWith('controller_grip_press', expect.objectContaining({ hand: 'right' }));
  });
...
describe('controllerInputHandler.onUpdate — thumbstick', () => {
  it('emits controller_thumbstick when outside deadzone', () => {
    const { node, config, ctx } = attach({ deadzone: 0.1, thumbstick_sensitivity: 1.0, invert_y: false });
    node.__controllerInputState.left.connected = true;
  ...
  it('inverts Y when invert_y=true', () => {
    const { node, config, ctx } = attach({ deadzone: 0.1, thumbstick_sensitivity: 1.0, invert_y: true });
    node.__controllerInputState.left.connected = true;
...
describe('controllerInputHandler.onEvent — controller_data', () => {
  it('emits controller_connected when connected transitions from false→true', () => {
    const { node, config, ctx } = attach();
    ctx.emit.mockClear();
    controllerInputHandler.onEvent!(node, config, ctx, { type: 'controller_data', hand: 'left', data: { connected: true } });
  ...
  it('emits controller_disconnected when connected transitions from true→false', () => {
    ...
    node.__controllerInputState.right.connected = true;
    ctx.emit.mockClear();
    controllerInputHandler.onEvent!(node, config, ctx, { type: 'controller_data', hand: 'right', data: { connected: false } });
    expect(ctx.emit).toHaveBeenCalledWith('controller_disconnected', { node, hand: 'right' });
  });
  it('no connection events when state unchanged', () => {
    const { node, config, ctx } = attach();
    ctx.emit.mockClear();
    controllerInputHandler.onEvent!(node, config, ctx, { type: 'controller_data', hand: 'left', data: { connected: false } });
  ...
  it('updates triggerValue and gripValue', () => {
    const { node, config, ctx } = attach();
    controllerInputHandler.onEvent!(node, config, ctx, { type: 'controller_data', hand: 'right', data: { triggerValue: 0.9, gripValue: 0.7 } });
...
describe('controllerInputHandler.onEvent — controller_vibrate', () => {
  it('emits haptic_pulse with event intensity + duration', () => {
    const { node, config, ctx } = attach();
    ctx.emit.mockClear();
    controllerInputHandler.onEvent!(node, config, ctx, { type: 'controller_vibrate', hand: 'right', intensity: 0.8, duration: 50 });
    expect(ctx.emit).toHaveBeenCalledWith('haptic_pulse', { hand: 'right', intensity: 0.8, duration: 50 });
  });
  it('uses config defaults when intensity/duration not provided', () => {
    const { node, config, ctx } = attach({ haptic_intensity: 0.4 });
    ctx.emit.mockClear();
    controllerInputHandler.onEvent!(node, config, ctx, { type: 'controller_vibrate', hand: 'left' });
    expect(ctx.emit).toHaveBeenCalledWith('haptic_pulse', expect.objectContaining({ intensity: 0.4, duration: 100 }));
  });
});
...
describe('controllerInputHandler.onEvent — get_controller_pose', () => {
  it('emits controller_pose_result with connected status', () => {
    ...
    node.__controllerInputState.left.connected = true;
    ctx.emit.mockClear();
    controllerInputHandler.onEvent!(node, config, ctx, { type: 'get_controller_pose', hand: 'left', queryId: 'q42' });
    expect(ctx.emit).toHaveBeenCalledWith('controller_pose_result', expect.objectContaining({ queryId: 'q42', hand: 'left', connected: true }));
  });
});
