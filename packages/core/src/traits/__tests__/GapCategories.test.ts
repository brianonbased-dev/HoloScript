...
describe('ComputeShaderTrait', () => {
  it('should compile and dispatch', () => {
    const n = createMockNode('cs'); const c = createMockContext();
    attachTrait(computeShaderHandler, n, { max_workgroups: 256 }, c);
    sendEvent(computeShaderHandler, n, { max_workgroups: 256 }, c, { type: 'cs:compile', shaderId: 's1', workgroups: [64, 1, 1] });
    expect(getEventCount(c, 'cs:compiled')).toBe(1);
    sendEvent(computeShaderHandler, n, { max_workgroups: 256 }, c, { type: 'cs:dispatch', shaderId: 's1' });
...
describe('RenderPipelineTrait', () => {
  it('should add pass and execute', () => {
    const n = createMockNode('rp'); const c = createMockContext();
    attachTrait(renderPipelineHandler, n, { max_passes: 8 }, c);
    sendEvent(renderPipelineHandler, n, { max_passes: 8 }, c, { type: 'rp:add_pass', passName: 'shadow' });
...
describe('PostProcessTrait', () => {
  it('should add and remove effects', () => {
    const n = createMockNode('pp'); const c = createMockContext();
    attachTrait(postProcessHandler, n, { max_effects: 16 }, c);
    sendEvent(postProcessHandler, n, { max_effects: 16 }, c, { type: 'pp:add', effectName: 'bloom', intensity: 0.8 });
    expect((getLastEvent(c, 'pp:added') as any).total).toBe(1);
    sendEvent(postProcessHandler, n, { max_effects: 16 }, c, { type: 'pp:remove', effectName: 'bloom' });
...
describe('RayTraceTrait', () => {
  it('should cast and hit', () => {
    const n = createMockNode('rt'); const c = createMockContext();
    attachTrait(rayTraceHandler, n, { max_bounces: 4, samples_per_pixel: 1 }, c);
    sendEvent(rayTraceHandler, n, { max_bounces: 4, samples_per_pixel: 1 }, c, { type: 'rt:cast', origin: [0,0,0], direction: [1,0,0] });
    expect((getLastEvent(c, 'rt:result') as any).maxBounces).toBe(4);
    sendEvent(rayTraceHandler, n, { max_bounces: 4, samples_per_pixel: 1 }, c, { type: 'rt:hit', hitPoint: [1,0,0], normal: [0,1,0], distance: 1 });
...
describe('TensorOpTrait', () => {
  it('should create tensor and matmul', () => {
    const n = createMockNode('t'); const c = createMockContext();
    attachTrait(tensorOpHandler, n, { max_dimensions: 4 }, c);
    sendEvent(tensorOpHandler, n, { max_dimensions: 4 }, c, { type: 'tensor:create', tensorId: 't1', shape: [3, 3] });
    expect(getEventCount(c, 'tensor:created')).toBe(1);
    sendEvent(tensorOpHandler, n, { max_dimensions: 4 }, c, { type: 'tensor:matmul', a: 't1', b: 't2' });
...
describe('OnnxRuntimeTrait', () => {
  it('should load and run model', () => {
    const n = createMockNode('o'); const c = createMockContext();
    attachTrait(onnxRuntimeHandler, n, { execution_provider: 'cpu' }, c);
    sendEvent(onnxRuntimeHandler, n, { execution_provider: 'cpu' }, c, { type: 'onnx:load', modelId: 'm1' });
    expect((getLastEvent(c, 'onnx:loaded') as any).provider).toBe('cpu');
    sendEvent(onnxRuntimeHandler, n, { execution_provider: 'cpu' }, c, { type: 'onnx:run', modelId: 'm1' });
...
describe('TrainingLoopTrait', () => {
  it('should start, step, and stop', () => {
    const n = createMockNode('tl'); const c = createMockContext();
    attachTrait(trainingLoopHandler, n, { max_epochs: 100, learning_rate: 0.001 }, c);
    sendEvent(trainingLoopHandler, n, { max_epochs: 100, learning_rate: 0.001 }, c, { type: 'train:start' });
    expect(getEventCount(c, 'train:started')).toBe(1);
    sendEvent(trainingLoopHandler, n, { max_epochs: 100, learning_rate: 0.001 }, c, { type: 'train:step', loss: 0.5 });
    expect((getLastEvent(c, 'train:progress') as any).epoch).toBe(1);
    sendEvent(trainingLoopHandler, n, { max_epochs: 100, learning_rate: 0.001 }, c, { type: 'train:stop' });
...
describe('SqlQueryTrait', () => {
  it('should exec and prepare', () => {
    const n = createMockNode('sq'); const c = createMockContext();
...
describe('OrmEntityTrait', () => {
  it('should create and read', () => {
    const n = createMockNode('orm'); const c = createMockContext();
    attachTrait(ormEntityHandler, n, { table_prefix: '' }, c);
    sendEvent(ormEntityHandler, n, { table_prefix: '' }, c, { type: 'orm:create', entityId: 'e1', data: { name: 'X' } });
...
describe('OfflineSyncTrait', () => {
  it('should queue and flush', () => {
    const n = createMockNode('os'); const c = createMockContext();
    attachTrait(offlineSyncHandler, n, { sync_interval_ms: 5000 }, c);
    sendEvent(offlineSyncHandler, n, { sync_interval_ms: 5000 }, c, { type: 'sync:queue', payload: { id: 1 } });
...
describe('ReactiveStoreTrait', () => {
  it('should set, get, and subscribe', () => {
    const n = createMockNode('rs'); const c = createMockContext();
    attachTrait(reactiveStoreHandler, n, { max_keys: 500 }, c);
    sendEvent(reactiveStoreHandler, n, { max_keys: 500 }, c, { type: 'store:set', key: 'score', value: 42 });
...
describe('AstarTrait', () => {
  it('should find path', () => {
    const n = createMockNode('a'); const c = createMockContext();
    attachTrait(astarHandler, n, { max_iterations: 10000, heuristic: 'euclidean' }, c);
    sendEvent(astarHandler, n, { max_iterations: 10000, heuristic: 'euclidean' }, c, { type: 'astar:find_path', from: [0,0], to: [10,10] });
...
describe('NavmeshSolverTrait', () => {
  it('should build and query', () => {
    const n = createMockNode('nm'); const c = createMockContext();
    attachTrait(navmeshSolverHandler, n, { cell_size: 0.5 }, c);
    sendEvent(navmeshSolverHandler, n, { cell_size: 0.5 }, c, { type: 'nav:build', polygonCount: 100 });
    expect((getLastEvent(c, 'nav:built') as any).polygons).toBe(100);
    sendEvent(navmeshSolverHandler, n, { cell_size: 0.5 }, c, { type: 'nav:query', from: [0,0,0], to: [5,0,5] });
...
describe('OptimizationTrait', () => {
  it('should solve', () => {
    const n = createMockNode('opt'); const c = createMockContext();
    attachTrait(optimizationHandler, n, { max_iterations: 1000, tolerance: 1e-6 }, c);
    sendEvent(optimizationHandler, n, { max_iterations: 1000, tolerance: 1e-6 }, c, { type: 'opt:solve', objective: 'minimize_cost', constraints: ['x > 0'] });
...
describe('TimeTravelDebugTrait', () => {
  it('should snapshot and rewind', () => {
    const n = createMockNode('ttd'); const c = createMockContext();
    attachTrait(timeTravelDebugHandler, n, { max_snapshots: 100 }, c);
    sendEvent(timeTravelDebugHandler, n, { max_snapshots: 100 }, c, { type: 'ttd:snapshot', frame: 0, data: { x: 1 } });
    sendEvent(timeTravelDebugHandler, n, { max_snapshots: 100 }, c, { type: 'ttd:snapshot', frame: 1, data: { x: 2 } });
...
describe('SpatialProfilerTrait', () => {
  it('should start, sample, and stop', () => {
    const n = createMockNode('sp'); const c = createMockContext();
    ...
    sendEvent(spatialProfilerHandler, n, { sample_rate_ms: 16 }, c, { type: 'prof:sample', fps: 60, drawCalls: 100 });
...
describe('CinematicSeqTrait', () => {
  it('should add clip and play', () => {
    const n = createMockNode('cin'); const c = createMockContext();
    attachTrait(cinematicSeqHandler, n, { fps: 24 }, c);
    sendEvent(cinematicSeqHandler, n, { fps: 24 }, c, { type: 'cin:add_clip', clipName: 'intro', startFrame: 0, endFrame: 120 });
...
describe('AiCameraTrait', () => {
  it('should track and frame', () => {
    const n = createMockNode('cam'); const c = createMockContext();
    attachTrait(aiCameraHandler, n, { tracking_speed: 1.0 }, c);
    sendEvent(aiCameraHandler, n, { tracking_speed: 1.0 }, c, { type: 'cam:track', targetId: 'hero' });
    expect((getLastEvent(c, 'cam:tracking') as any).target).toBe('hero');
    sendEvent(aiCameraHandler, n, { tracking_speed: 1.0 }, c, { type: 'cam:frame', composition: 'rule_of_thirds' });
...
describe('FfiTrait', () => {
  it('should bind and call', () => {
    const n = createMockNode('ffi'); const c = createMockContext();
    attachTrait(ffiHandler, n, { allowed_libs: ['libm'] }, c);
    sendEvent(ffiHandler, n, { allowed_libs: ['libm'] }, c, { type: 'ffi:bind', symbol: 'sqrt', lib: 'libm' });
...
describe('NativeCallTrait', () => {
  it('should invoke', () => {
    const n = createMockNode('nc'); const c = createMockContext();
    attachTrait(nativeCallHandler, n, { sandbox: true }, c);
    sendEvent(nativeCallHandler, n, { sandbox: true }, c, { type: 'native:invoke', api: 'filesystem', method: 'readdir' });
...
describe('WasmBridgeTrait', () => {
  it('should load and call', () => {
    const n = createMockNode('wb'); const c = createMockContext();
    attachTrait(wasmBridgeHandler, n, { max_memory_pages: 256 }, c);
    sendEvent(wasmBridgeHandler, n, { max_memory_pages: 256 }, c, { type: 'wasm:load', moduleId: 'physics' });
    expect((getLastEvent(c, 'wasm:loaded') as any).totalModules).toBe(1);
    sendEvent(wasmBridgeHandler, n, { max_memory_pages: 256 }, c, { type: 'wasm:call', moduleId: 'physics', fn: 'step' });
...
describe('SysIoTrait', () => {
  it('should read and deny write when disabled', () => {
    const n = createMockNode('si'); const c = createMockContext();
    attachTrait(sysIoHandler, n, { allow_write: false }, c);
    sendEvent(sysIoHandler, n, { allow_write: false }, c, { type: 'sysio:read', path: '/tmp/data' });
    expect((getLastEvent(c, 'sysio:data') as any).readCount).toBe(1);
    sendEvent(sysIoHandler, n, { allow_write: false }, c, { type: 'sysio:write', path: '/tmp/out' });
...
describe('ActorTrait', () => {
  it('should send and process', () => {
    ...
    sendEvent(actorHandler, n, { mailbox_size: 10 }, c, { type: 'actor:send', from: 'worker1', message: { task: 'compute' } });
...
describe('CspChannelTrait', () => {
  it('should create, send, and receive', () => {
    const n = createMockNode('csp'); const c = createMockContext();
    attachTrait(cspChannelHandler, n, { buffer_size: 10 }, c);
    sendEvent(cspChannelHandler, n, { buffer_size: 10 }, c, { type: 'csp:create', channelId: 'ch1' });
    sendEvent(cspChannelHandler, n, { buffer_size: 10 }, c, { type: 'csp:send', channelId: 'ch1', value: 42 });
...
describe('TemporalGuardTrait', () => {
  it('should assert and satisfy', () => {
    const n = createMockNode('tg'); const c = createMockContext();
    attachTrait(temporalGuardHandler, n, { default_timeout_ms: 5000 }, c);
    sendEvent(temporalGuardHandler, n, { default_timeout_ms: 5000 }, c, { type: 'tg:assert', guardId: 'g1', property: 'liveness' });
    expect(getEventCount(c, 'tg:asserted')).toBe(1);
    sendEvent(temporalGuardHandler, n, { default_timeout_ms: 5000 }, c, { type: 'tg:satisfy', guardId: 'g1' });
...
describe('DeadlockFreeTrait', () => {
  it('should acquire and detect contention', () => {
    const n = createMockNode('dl'); const c = createMockContext();
    attachTrait(deadlockFreeHandler, n, { max_resources: 100 }, c);
    sendEvent(deadlockFreeHandler, n, { max_resources: 100 }, c, { type: 'dl:acquire', resourceId: 'r1', ownerId: 'A' });
    expect(getEventCount(c, 'dl:acquired')).toBe(1);
    sendEvent(deadlockFreeHandler, n, { max_resources: 100 }, c, { type: 'dl:acquire', resourceId: 'r1', ownerId: 'B' });
