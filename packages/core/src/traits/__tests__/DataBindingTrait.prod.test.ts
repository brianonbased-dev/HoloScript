...
function makeNode(extras: any = {}) { return { id: 'db_node', ...extras }; }
...
describe('dataBindingHandler.onAttach', () => {
  ...
  it('connectionHandle=null', () => expect(attach().node.__dataBindingState.connectionHandle).toBeNull());
  ...
  it('emits data_binding_connect when source is set', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('data_binding_connect', expect.objectContaining({ source: 'https://api.example.com/data', sourceType: 'rest' }));
  });
  ...
  it('data_binding_connect includes authHeader', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('data_binding_connect', expect.objectContaining({ authHeader: 'Bearer token123' }));
  });
});
...
describe('dataBindingHandler.onEvent — data_binding_connected', () => {
  it('sets isConnected=true', () => {
    ...
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_connected', handle: 'h1' });
    expect(node.__dataBindingState.isConnected).toBe(true);
  });
  it('stores connectionHandle', () => {
    const { node, ctx, config } = attach();
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_connected', handle: 'handle_abc' });
...
describe('dataBindingHandler.onEvent — data_binding_data', () => {
  it('updates currentData', () => {
    const { node, ctx, config } = attach();
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_data', data: { temperature: 22.5 } });
  ...
  it('emits on_data_change', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('on_data_change', expect.objectContaining({ data: { v: 1 } }));
  });
  it('applies binding to node property (no interpolation)', () => {
    const { node, ctx, config } = attach({
      interpolation: false,
      bindings: [{ source_path: 'sensors.temp', target_property: 'temperature', transform: 'none' }]
    });
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_data', data: { sensors: { temp: 37 } } });
  ...
  it('resolves nested source_path via dot notation', () => {
    ...
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_data', data: { a: { b: { c: 99 } } } });
    expect((node as any).deepValue).toBe(99);
  });
  it('applies scale transform', () => {
    const { node, ctx, config } = attach({
      interpolation: false,
      bindings: [{ source_path: 'val', target_property: 'scaled', transform: 'scale', transform_params: { factor: 2 } }]
    });
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_data', data: { val: 10 } });
    expect((node as any).scaled).toBe(20);
  });
  it('applies normalize transform', () => {
    const { node, ctx, config } = attach({
      interpolation: false,
      bindings: [{ source_path: 'raw', target_property: 'norm', transform: 'normalize', transform_params: { min: 0, max: 100 } }]
    });
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_data', data: { raw: 50 } });
    expect((node as any).norm).toBeCloseTo(0.5);
  });
  it('applies map transform', () => {
    const { node, ctx, config } = attach({
      interpolation: false,
      bindings: [{ source_path: 'state', target_property: 'label', transform: 'map', transform_params: { mapping: { 0: 'off', 1: 'on' } } }]
    });
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_data', data: { state: 1 } });
  ...
  it('skips binding when source_path not found', () => {
    ...
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_data', data: { other: 1 } });
  ...
  it('does not apply numeric binding immediately when interpolation=true', () => {
    ...
    (node as any).pos = 0;
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_data', data: { val: 100 } });
...
describe('dataBindingHandler.onEvent — data_binding_error', () => {
  it('increments errorCount', () => {
    const { node, ctx, config } = attach();
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_error', error: 'timeout' });
  ...
  it('emits on_data_error with errorCount', () => {
    ...
    ctx.emit.mockClear();
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_error', error: 'net fail' });
    expect(ctx.emit).toHaveBeenCalledWith('on_data_error', expect.objectContaining({ error: 'net fail', errorCount: 1 }));
  });
});
...
describe('dataBindingHandler.onEvent — data_binding_refresh', () => {
  it('emits data_binding_fetch', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('data_binding_fetch', expect.objectContaining({ source: 'https://api.x.com', sourceType: 'rest' }));
  });
});
...
describe('dataBindingHandler.onEvent — data_binding_set_source', () => {
  it('emits data_binding_connect with new source', () => {
    ...
    ctx.emit.mockClear();
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_set_source', source: 'https://new.api.com' });
    expect(ctx.emit).toHaveBeenCalledWith('data_binding_connect', expect.objectContaining({ source: 'https://new.api.com' }));
  });
  ...
  it('disconnects existing connection first', () => {
    ...
    ctx.emit.mockClear();
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_set_source', source: 'https://new.api.com' });
    const disconnectCalls = ctx.emit.mock.calls.filter((c: any[]) => c[0] === 'data_binding_disconnect');
...
describe('dataBindingHandler.onEvent — data_binding_query', () => {
  it('emits data_binding_info snapshot', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('data_binding_info', expect.objectContaining({
      queryId: 'q1',
      isConnected: false,
      errorCount: 0,
      bindingCount: 1,
...
describe('dataBindingHandler.onUpdate — polling', () => {
  it('emits data_binding_fetch when refresh_rate elapsed for rest source', () => {
    const { node, ctx, config } = attach({ source: 'https://api.x.com', source_type: 'rest', refresh_rate: 1000 });
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_connected', handle: 'h' });
    node.__dataBindingState.lastRefresh = Date.now() - 1100; // 1.1s ago
    ctx.emit.mockClear();
    dataBindingHandler.onUpdate!(node, config, ctx, 0.016);
    expect(ctx.emit).toHaveBeenCalledWith('data_binding_fetch', expect.objectContaining({ source: 'https://api.x.com' }));
  });
  ...
  it('does not interpolate non-numeric values', () => {
    ...
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_connected', handle: 'h' });
    dataBindingHandler.onEvent!(node, config, ctx, { type: 'data_binding_data', data: { label: 'hello' } });
