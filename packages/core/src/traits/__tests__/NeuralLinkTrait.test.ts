...
describe('NeuralLinkTrait', () => {
  ...
  function state() { return (node as any).__neuralLinkState; }
  ...
  it('execute event sets status to inferring', () => {
    sendEvent(neuralLinkHandler, node, cfg, ctx, {
      type: 'neural_link_execute', data: { prompt: 'Hello' },
    });
  ...
  it('execute event includes prompt in emitted event', () => {
    sendEvent(neuralLinkHandler, node, cfg, ctx, {
      type: 'neural_link_execute', data: { prompt: 'Test prompt' },
    });
  ...
  it('multiple execute events tracked individually', () => {
    sendEvent(neuralLinkHandler, node, cfg, ctx, { type: 'neural_link_execute', data: { prompt: 'P1' } });
    sendEvent(neuralLinkHandler, node, cfg, ctx, { type: 'neural_link_execute', data: { prompt: 'P2' } });
  ...
  it('response event stores text in last_response', () => {
    sendEvent(neuralLinkHandler, node, cfg, ctx, {
      type: 'neural_link_response', data: { text: 'Hi!', generationTime: 100 },
    });
    expect(state().last_response).toBe('Hi!');
  });

  it('response event sets status back to idle', () => {
    sendEvent(neuralLinkHandler, node, cfg, ctx, { type: 'neural_link_execute', data: { prompt: 'p' } });
    sendEvent(neuralLinkHandler, node, cfg, ctx, { type: 'neural_link_response', data: { text: 'A', generationTime: 50 } });
    expect(state().neural_status).toBe('idle');
  });

  it('response event stores generationTime', () => {
    sendEvent(neuralLinkHandler, node, cfg, ctx, {
      type: 'neural_link_response', data: { text: 'X', generationTime: 250 },
    });
  ...
  it('response event emits on_neural_response', () => {
    sendEvent(neuralLinkHandler, node, cfg, ctx, {
      type: 'neural_link_response', data: { text: 'Y', generationTime: 10 },
    });
  ...
  it('response with missing generationTime defaults to 0', () => {
    sendEvent(neuralLinkHandler, node, cfg, ctx, {
      type: 'neural_link_response', data: { text: 'Z' },
    });
    expect(state().last_inference_time).toBe(0);
  });
