...
const defaultConfig = { webhook_url: 'https://hooks.slack.com/services/alerts', default_channel: '#alerts' };
...
describe('SlackAlertTrait', () => {
  ...
  it('slack_alert:send calls fetch and emits slack_alert:sent on success', async () => {
    ...
    await vi.waitFor(() => expect(node.emit).toHaveBeenCalledWith('slack_alert:sent', expect.objectContaining({
      channel: '#alerts', message: 'alert!', status: 200,
    })));
  ...
  it('slack_alert:send emits slack_alert:error when webhook_url is empty', () => {
    const node = makeNode();
    slackAlertHandler.onAttach!(node as never, { webhook_url: '', default_channel: '#alerts' }, makeCtx(node) as never);
    slackAlertHandler.onEvent!(node as never, { webhook_url: '', default_channel: '#alerts' }, makeCtx(node) as never, {
      type: 'slack_alert:send', message: 'alert!',
    } as never);
    expect(node.emit).toHaveBeenCalledWith('slack_alert:error', expect.objectContaining({ error: 'No webhook_url configured' }));
  });
});
