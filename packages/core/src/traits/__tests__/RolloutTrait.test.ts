...
const makeNode = () => ({
  ...
  emit: vi.fn(),
  __rolloutState: undefined as unknown,
});
...
describe('RolloutTrait', () => {
  ...
  it('rollout:set emits rollout:configured', () => {
    ...
    expect(node.emit).toHaveBeenCalledWith('rollout:configured', {
      feature: 'dark_mode',
      percentage: 50,
    });
  });
  ...
  it('rollout:check emits rollout:result', () => {
    ...
    expect(node.emit).toHaveBeenCalledWith(
      'rollout:result',
      expect.objectContaining({ feature: 'dark_mode' })
    );
  });
});
