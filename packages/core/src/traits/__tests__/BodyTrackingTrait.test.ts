...
describe('BodyTrackingTrait', () => {
  ...
  it('body_pose_update with sufficient joints activates tracking', () => {
    ...
    const upperJoints = ['head', 'neck', 'spine_chest', 'spine_mid', 'shoulder_left', 'shoulder_right',
      'elbow_left', 'elbow_right', 'wrist_left', 'wrist_right', 'hand_left', 'hand_right'];
    for (const j of upperJoints) {
      joints[j] = { position: { x: 0, y: 1, z: 0 }, rotation: { x: 0, y: 0, z: 0, w: 1 }, confidence: 0.9 };
    }
  ...
  it('low confidence joints do not count', () => {
    ...
    const upperJoints = ['head', 'neck', 'spine_chest', 'spine_mid', 'shoulder_left', 'shoulder_right',
      'elbow_left', 'elbow_right', 'wrist_left', 'wrist_right', 'hand_left', 'hand_right'];
    for (const j of upperJoints) {
      joints[j] = { position: { x: 0, y: 1, z: 0 }, rotation: { x: 0, y: 0, z: 0, w: 1 }, confidence: 0.1 };
    }
    sendEvent(bodyTrackingHandler, node, cfg, ctx, { type: 'body_pose_update', joints });
    expect((node as any).__bodyTrackingState.isTracking).toBe(false);
  });
  ...
  it('tracking lost emits body_tracking_lost', () => {
    ...
    for (const j of upperJoints) {
      lowJoints[j] = { position: { x: 0, y: 1, z: 0 }, rotation: { x: 0, y: 0, z: 0, w: 1 }, confidence: 0.1 };
    }
  ...
  it('avatar_pose_update emitted on update when tracking', () => {
    ...
    s.isTracking = true;
    s.joints.set('head', { position: { x: 0, y: 1.8, z: 0 }, rotation: { x: 0, y: 0, z: 0, w: 1 }, confidence: 0.9 });
  ...
  it('body_calibrate measures height and arm span', () => {
    const s = (node as any).__bodyTrackingState;
    s.joints.set('head', { position: { x: 0, y: 1.8, z: 0 }, rotation: { x: 0, y: 0, z: 0, w: 1 }, confidence: 1 });
    s.joints.set('foot_left', { position: { x: 0, y: 0, z: 0 }, rotation: { x: 0, y: 0, z: 0, w: 1 }, confidence: 1 });
    s.joints.set('hand_left', { position: { x: -0.9, y: 1.2, z: 0 }, rotation: { x: 0, y: 0, z: 0, w: 1 }, confidence: 1 });
    s.joints.set('hand_right', { position: { x: 0.9, y: 1.2, z: 0 }, rotation: { x: 0, y: 0, z: 0, w: 1 }, confidence: 1 });
