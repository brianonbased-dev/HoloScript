...
function mkCfg(o: Partial<PercConfig> = {}): PercConfig { return { ...perceptionHandler.defaultConfig!, ...o }; }
function mkNode(id = 'perc-node', position = { x: 0, y: 0, z: 0 }) { return { id, position, rotation: { x: 0, y: 0, z: 0 } } as any; }
...
function attach(cfg = mkCfg(), node = mkNode(), ctx = mkCtx()) {
  perceptionHandler.onAttach!(node, cfg, ctx as any); ctx.emitted.length = 0; return { node, ctx, cfg };
}
function detect(node: any, ctx: any, cfg: PercConfig, id: string, pos: { x: number; y: number; z: number }, threat = 0.5) {
  perceptionHandler.onEvent!(node, cfg, ctx as any, { type: 'perception_detect', target: { id, position: pos, threat, tags: [] } } as any);
}
...
describe('perceptionHandler — defaultConfig', () => {
  ...
  it('memory_duration = 10000', () => expect(perceptionHandler.defaultConfig?.memory_duration).toBe(10000));
  it('confidence_decay = 0.2', () => expect(perceptionHandler.defaultConfig?.confidence_decay).toBe(0.2));
  it('scan_interval = 0.1', () => expect(perceptionHandler.defaultConfig?.scan_interval).toBe(0.1));
});

describe('perceptionHandler — onAttach', () => {
  it('creates __perceptionState', () => { const { node } = attach(); expect((node as any).__perceptionState).toBeDefined(); });
  it('entities Map is empty', () => { const { node } = attach(); expect((node as any).__perceptionState.entities.size).toBe(0); });
  it('alertLevel = 0', () => { const { node } = attach(); expect((node as any).__perceptionState.alertLevel).toBe(0); });
  it('isSearching = false', () => { const { node } = attach(); expect((node as any).__perceptionState.isSearching).toBe(false); });
});

describe('perceptionHandler — onDetach', () => {
  it('removes __perceptionState', () => {
    const cfg = mkCfg(); const node = mkNode(); const ctx = mkCtx();
...
describe('perceptionHandler — onEvent: perception_detect', () => {
  it('adds entity within sight range + angle', () => {
    ...
    const { node, ctx, cfg } = attach(mkCfg({ sight_range: 20, sight_angle: 120, detection_layers: [] }));
  ...
  it('stores entity beyond sight/hearing as proximity type (no hard cutoff without layers)', () => {
    ...
    const { node, ctx, cfg } = attach(mkCfg({ sight_range: 5, hearing_range: 3, alert_radius: 1, detection_layers: [] }));
  ...
  it('filters by detection_layers', () => {
    const { node, ctx, cfg } = attach(mkCfg({ detection_layers: ['enemy'], sight_range: 20, hearing_range: 15 }));
    // Target has no enemy tag -> should be ignored
    perceptionHandler.onEvent!(node, cfg, ctx as any, { type: 'perception_detect', target: { id: 'npc', position: { x: 0, y: 0, z: 5 }, tags: ['friendly'], threat: 0.1 } } as any);
    expect((node as any).__perceptionState.entities.has('npc')).toBe(false);
  });
  it('accepts entity with matching tag in detection_layers', () => {
    const { node, ctx, cfg } = attach(mkCfg({ detection_layers: ['enemy'], sight_range: 20, hearing_range: 15 }));
    perceptionHandler.onEvent!(node, cfg, ctx as any, { type: 'perception_detect', target: { id: 'badguy', position: { x: 0, y: 0, z: 5 }, tags: ['enemy'], threat: 0.8 } } as any);
  ...
  it('uses senseType=hearing for entity within hearing range but outside FOV', () => {
    ...
    const { node, ctx, cfg } = attach(mkCfg({ sight_range: 20, sight_angle: 60, hearing_range: 15, alert_radius: 1, detection_layers: [] }));
  ...
  it('no-op when target id is missing', () => {
    ...
    expect(() => perceptionHandler.onEvent!(node, cfg, ctx as any, { type: 'perception_detect', target: { position: { x: 0, y: 0, z: 5 } } } as any)).not.toThrow();
...
describe('perceptionHandler — onEvent: perception_sound', () => {
  it('adds entity within hearing range', () => {
    ...
    perceptionHandler.onEvent!(node, cfg, ctx as any, { type: 'perception_sound', position: { x: 0, y: 0, z: 10 }, sourceId: 'snd1', threat: 0.3 } as any);
    expect((node as any).__perceptionState.entities.has('snd1')).toBe(true);
  });
  it('emits perception_sound_detected', () => {
    const { node, ctx, cfg } = attach(mkCfg({ hearing_range: 15 }));
    perceptionHandler.onEvent!(node, cfg, ctx as any, { type: 'perception_sound', position: { x: 5, y: 0, z: 0 }, sourceId: 'snd2' } as any);
  ...
  it('ignores sound beyond hearing range', () => {
    ...
    perceptionHandler.onEvent!(node, cfg, ctx as any, { type: 'perception_sound', position: { x: 100, y: 0, z: 0 }, sourceId: 'far_snd' } as any);
...
describe('perceptionHandler — onEvent: perception_damage', () => {
  it('registers attacker with threat=1.0 and confidence=1.0', () => {
    const { node, ctx, cfg } = attach();
    perceptionHandler.onEvent!(node, cfg, ctx as any, { type: 'perception_damage', attackerId: 'atk1', position: { x: 3, y: 0, z: 0 } } as any);
  ...
  it('sets alertLevel = 1.0 on damage', () => {
    const { node, ctx, cfg } = attach();
    perceptionHandler.onEvent!(node, cfg, ctx as any, { type: 'perception_damage', attackerId: 'atk2', position: { x: 1, y: 0, z: 0 } } as any);
    expect((node as any).__perceptionState.alertLevel).toBe(1.0);
  });
  it('emits perception_attacked', () => {
    const { node, ctx, cfg } = attach();
    perceptionHandler.onEvent!(node, cfg, ctx as any, { type: 'perception_damage', attackerId: 'atk3', position: { x: 2, y: 0, z: 0 } } as any);
    expect(ctx.emitted.some((e: any) => e.type === 'perception_attacked')).toBe(true);
  });
  it('no-op when attackerId missing', () => {
    const { node, ctx, cfg } = attach();
    expect(() => perceptionHandler.onEvent!(node, cfg, ctx as any, { type: 'perception_damage', position: { x: 0, y: 0, z: 0 } } as any)).not.toThrow();
  });
});
...
describe('perceptionHandler — onEvent: forget / clear', () => {
  it('perception_forget removes single entity', () => {
    ...
    perceptionHandler.onEvent!(node, cfg, ctx as any, { type: 'perception_forget', entityId: 'rem_me' } as any);
  ...
  it('perception_clear removes all entities and resets state', () => {
    const { node, ctx, cfg } = attach(mkCfg({ detection_layers: [] }));
    detect(node, ctx, cfg, 'a', { x: 0, y: 0, z: 5 }); detect(node, ctx, cfg, 'b', { x: 1, y: 0, z: 5 });
...
describe('perceptionHandler — onUpdate', () => {
  ...
  it('emits perception_alert when alertLevel crosses 0.5 threshold', () => {
    ...
    (node as any).__perceptionState.entities.set('threat_high', { id: 'threat_high', position: { x: 0, y: 0, z: 5 }, lastSeen: Date.now(), senseType: 'sight', confidence: 0.9, threat: 0.9, tags: [] });
  ...
  it('scan_request has correct sightRange and hearingRange', () => {
    const cfg = mkCfg({ scan_interval: 0.1, sight_range: 30, hearing_range: 20, detection_layers: [] });
    const { node, ctx } = attach(cfg);
