...
function makeNode() { return { id: 'bt_node' }; }
...
describe('bodyTrackingHandler.onAttach', () => {
  ...
  it('prevJoints is empty Map', () => expect(attach().node.__bodyTrackingState.prevJoints.size).toBe(0));
  ...
  it('emits body_tracking_start with mode + joints + prediction', () => {
    ...
    expect(ctx.emit).toHaveBeenCalledWith('body_tracking_start', expect.objectContaining({
      mode: 'hands_only',
      prediction: false,
...
describe('bodyTrackingHandler.onUpdate — tracking + avatar_binding', () => {
  it('emits avatar_pose_update when isTracking + avatar_binding + joints non-empty', () => {
    ...
    ctx.emit.mockClear();
    bodyTrackingHandler.onUpdate!(node, config, ctx, 0.016);
    expect(ctx.emit).toHaveBeenCalledWith('avatar_pose_update', expect.objectContaining({
...
describe('bodyTrackingHandler.onEvent — body_pose_update', () => {
  it('stores joints above confidence threshold', () => {
    const { node, config, ctx } = attach({ mode: 'upper_body', tracking_confidence_threshold: 0.5, joint_smoothing: 0 });
  ...
  it('ignores joints below confidence threshold', () => {
    const { node, config, ctx } = attach({ mode: 'upper_body', tracking_confidence_threshold: 0.8, joint_smoothing: 0 });
    bodyTrackingHandler.onEvent!(node, config, ctx, {
      type: 'body_pose_update',
  ...
  it('sets isTracking=true when >=50% joints valid', () => {
    ...
    const { node, config, ctx } = attach({ mode: 'upper_body', tracking_confidence_threshold: 0.5, joint_smoothing: 0 });
    const allJoints: any = {};
    for (const j of ['head','neck','spine_chest','spine_mid','shoulder_left','shoulder_right',
      'elbow_left','elbow_right','wrist_left','wrist_right','hand_left','hand_right']) {
      allJoints[j] = makePose(0, 0, 0, 1.0);
    }
    bodyTrackingHandler.onEvent!(node, config, ctx, { type: 'body_pose_update', joints: allJoints });
  ...
  it('does NOT emit body_tracking_found on subsequent updates', () => {
    const { node, config, ctx } = attach({ mode: 'hands_only', tracking_confidence_threshold: 0.5, joint_smoothing: 0 });
    const joints: any = { wrist_left: makePose(0,0,0,1), wrist_right: makePose(0,0,0,1), hand_left: makePose(0,0,0,1), hand_right: makePose(0,0,0,1) };
  ...
  it('emits body_pose_changed with confidence when tracking', () => {
    ...
    ctx.emit.mockClear();
    bodyTrackingHandler.onEvent!(node, config, ctx, { type: 'body_pose_update', joints });
    expect(ctx.emit).toHaveBeenCalledWith('body_pose_changed', expect.objectContaining({ confidence: 1.0 }));
  });
  it('emits body_tracking_lost when transitioning from tracking to lost', () => {
    ...
    bodyTrackingHandler.onEvent!(node, config, ctx, { type: 'body_pose_update', joints: badJoints });
  ...
  it('smoothes joint pose when joint_smoothing>0 and prevJoint exists', () => {
    const { node, config, ctx } = attach({ mode: 'hands_only', tracking_confidence_threshold: 0, joint_smoothing: 0.5 });
    // First update: prev = zero pose
    node.__bodyTrackingState.prevJoints.set('wrist_left', makePose(0, 0, 0));
    // Send wrist_left at x=1
    bodyTrackingHandler.onEvent!(node, config, ctx, {
      type: 'body_pose_update',
      joints: { wrist_left: makePose(1, 0, 0, 1), wrist_right: makePose(0,0,0,1), hand_left: makePose(0,0,0,1), hand_right: makePose(0,0,0,1) },
    });
  ...
  it('uses raw pose when joint_smoothing=0', () => {
    const { node, config, ctx } = attach({ mode: 'hands_only', tracking_confidence_threshold: 0, joint_smoothing: 0 });
    const joints: any = { wrist_left: makePose(3, 0, 0, 1), wrist_right: makePose(0,0,0,1), hand_left: makePose(0,0,0,1), hand_right: makePose(0,0,0,1) };
    bodyTrackingHandler.onEvent!(node, config, ctx, { type: 'body_pose_update', joints });
...
describe('bodyTrackingHandler.onEvent — body_calibrate', () => {
  it('computes bodyHeight = head.y - feet.y when head + foot_left set', () => {
    const { node, config, ctx } = attach({ mode: 'full_body', tracking_confidence_threshold: 0.5, joint_smoothing: 0 });
    node.__bodyTrackingState.joints.set('head', makePose(0, 1.8, 0));
  ...
  it('falls back to ankle_left when foot_left not set', () => {
    ...
    bodyTrackingHandler.onEvent!(node, config, ctx, { type: 'body_calibrate' });
    expect(node.__bodyTrackingState.bodyHeight).toBeCloseTo(1.70, 5);
  });
  ...
  it('emits body_calibrated with bodyHeight and armSpan', () => {
    ...
    ctx.emit.mockClear();
    bodyTrackingHandler.onEvent!(node, config, ctx, { type: 'body_calibrate' });
    expect(ctx.emit).toHaveBeenCalledWith('body_calibrated', expect.objectContaining({
