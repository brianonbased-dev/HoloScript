...
async function attach(cfg = mkCfg(), node = mkNode(), ctx = mkCtx()) {
  await volumetricCloudsHandler.onAttach!(node, cfg, ctx as any);
...
describe('volumetricCloudsHandler — onAttach', () => {
  ...
  it('windOffset initialized to [0,0,0]', async () => {
    const { node } = await attach();
...
describe('volumetricCloudsHandler — onDetach', () => {
  it('emits volumetric_clouds_destroy', async () => {
    const { node, ctx, cfg } = await attach();
    await volumetricCloudsHandler.onDetach!(node, cfg, ctx as any);
    expect(ctx.emitted.some((e: any) => e.type === 'volumetric_clouds_destroy')).toBe(true);
  });
  it('removes __cloudState', async () => {
    const { node, ctx, cfg } = await attach();
    await volumetricCloudsHandler.onDetach!(node, cfg, ctx as any);
...
describe('volumetricCloudsHandler — onUpdate', () => {
  it('emits volumetric_clouds_update each frame', async () => {
    const { node, ctx, cfg } = await attach();
    await volumetricCloudsHandler.onUpdate!(node, cfg, ctx as any, 0.016);
    expect(ctx.emitted.some((e: any) => e.type === 'volumetric_clouds_update')).toBe(true);
  });
  it('accumulates time', async () => {
    const { node, ctx, cfg } = await attach();
    await volumetricCloudsHandler.onUpdate!(node, cfg, ctx as any, 0.5);
...
describe('volumetricCloudsHandler — onEvent', () => {
  it('clouds_set_coverage emits update with coverage', async () => {
    const { node, ctx, cfg } = await attach();
    await volumetricCloudsHandler.onEvent!(
      node,
      cfg,
      ctx as any,
  ...
  it('clouds_pause sets active = false', async () => {
    const { node, ctx, cfg } = await attach();
    await volumetricCloudsHandler.onEvent!(node, cfg, ctx as any, { type: 'clouds_pause' } as any);
    expect((node as any).__cloudState.active).toBe(false);
  });
  it('clouds_resume sets active = true', async () => {
    const { node, ctx, cfg } = await attach();
    await volumetricCloudsHandler.onEvent!(node, cfg, ctx as any, { type: 'clouds_pause' } as any);
    await volumetricCloudsHandler.onEvent!(node, cfg, ctx as any, { type: 'clouds_resume' } as any);
    expect((node as any).__cloudState.active).toBe(true);
  });
  it('no-op when no state', () => {
    expect(() =>
