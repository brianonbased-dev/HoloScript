...
describe('SpatialHash', () => {
  ...
  it('insert + getNeighbors finds particle in same cell', () => {
    ...
    expect(hash.getNeighbors({ x: 0.01, y: 0, z: 0 })).toContain(0);
  });
  it('getNeighbors finds particle in adjacent cell', () => {
    hash.insert(1, { x: 0.05, y: 0, z: 0 }); // adjacent cell to 0,0,0
    expect(hash.getNeighbors({ x: 0, y: 0, z: 0 })).toContain(1);
  });
  it('does NOT find particle more than 1 cell away', () => {
    hash.insert(2, { x: 0.5, y: 0, z: 0 }); // ~12 cells away
    const neighbors = hash.getNeighbors({ x: 0, y: 0, z: 0 });
    expect(neighbors).not.toContain(2);
  });
  it('clear removes all entries', () => {
    hash.insert(0, { x: 0, y: 0, z: 0 });
    hash.clear();
    expect(hash.getNeighbors({ x: 0, y: 0, z: 0 })).toHaveLength(0);
  });
...
describe('FluidSimulationSystem — construction & defaults', () => {
  it('creates with water defaults', () => {
    ...
    expect(cfg.gravity.y).toBeCloseTo(-9.81);
...
describe('FluidSimulationSystem — particle management', () => {
  ...
  it('addParticle with velocity stores velocity', () => {
    const id = sim.addParticle({ x: 0, y: 0, z: 0 }, { x: 1, y: 2, z: 3 });
    expect(sim.getParticle(id)?.velocity).toEqual({ x: 1, y: 2, z: 3 });
  });
  it('addParticle without velocity defaults to zero', () => {
    const id = sim.addParticle({ x: 0, y: 0, z: 0 });
    expect(sim.getParticle(id)?.velocity).toEqual({ x: 0, y: 0, z: 0 });
  });
  ...
  it('clearParticles removes all and resets IDs', () => {
    ...
    sim.addParticle({ x: 1, y: 0, z: 0 });
    sim.clearParticles();
    expect(sim.getParticleCount()).toBe(0);
    const newId = sim.addParticle({ x: 0, y: 0, z: 0 });
...
describe('FluidSimulationSystem — step() physics', () => {
  it('gravity pulls particles down over time', () => {
    ...
    const id = sim.addParticle({ x: 0, y: 1, z: 0 }); // start at y=1
  ...
  it('custom dt overrides timeStep', () => {
    ...
    const p = sim.getParticle(id)!;
    expect(p.position.y).toBeGreaterThan(9.99); // barely moved
  });
});
...
describe('FluidSimulationSystem — plane boundary collision', () => {
  it('floor plane prevents particles from going below y=0', () => {
    ...
    const p = sim.getParticle(id)!;
    expect(p.position.y).toBeGreaterThanOrEqual(sim.getConfig().particleRadius - 0.001);
  });
});
...
describe('FluidSimulationSystem — box boundary', () => {
  it('box boundary clamps particles within extents', () => {
    ...
    sim.step();
    const p = sim.getParticle(id)!;
    expect(p.position.x).toBeLessThanOrEqual(1.0 - sim.getConfig().particleRadius + 0.01);
  });
});
...
describe('FluidSimulationSystem — getDensityAt / isInsideFluid', () => {
  it('getDensityAt returns 0 for empty fluid', () => {
    const sim = new FluidSimulationSystem();
    expect(sim.getDensityAt({ x: 0, y: 0, z: 0 })).toBe(0);
  });
  it('getDensityAt returns positive near particle cluster', () => {
    const sim = new FluidSimulationSystem({ smoothingRadius: 0.1 });
    sim.addParticle({ x: 0, y: 0, z: 0 });
    sim.addParticle({ x: 0.01, y: 0, z: 0 });
    sim.addParticle({ x: 0, y: 0.01, z: 0 });
    expect(sim.getDensityAt({ x: 0, y: 0, z: 0 })).toBeGreaterThan(0);
  });
  it('isInsideFluid returns false for empty simulation', () => {
    const sim = new FluidSimulationSystem();
    expect(sim.isInsideFluid({ x: 0, y: 0, z: 0 })).toBe(false);
  });
  it('isInsideFluid returns true at dense particle cluster', () => {
    ...
    for (let i = 0; i < 20; i++) {
      sim.addParticle({ x: (i % 4) * 0.01, y: Math.floor(i / 4) * 0.01, z: 0 });
    }
    expect(sim.isInsideFluid({ x: 0.01, y: 0.01, z: 0 })).toBe(true);
  });
});
