...
describe('aiCompanionHandler — defaultConfig', () => {
  it('personality = friendly', () => expect(aiCompanionHandler.defaultConfig?.personality).toBe('friendly'));
  it('interaction_range = 10', () => expect(aiCompanionHandler.defaultConfig?.interaction_range).toBe(10));
  it('memory_capacity = 100', () => expect(aiCompanionHandler.defaultConfig?.memory_capacity).toBe(100));
});
...
describe('aiCompanionHandler — onAttach', () => {
  ...
  it('emits ai_companion_create with personality and config', () => {
    const node = mkNode();
    const ctx = mkCtx();
    aiCompanionHandler.onAttach!(node, mkCfg({ personality: 'curious', idle_behavior: 'patrol' }), ctx as any);
...
describe('aiCompanionHandler — onEvent', () => {
  it('ai_companion_interact sets interactingWith and boosts curiosity', () => {
    ...
    aiCompanionHandler.onEvent!(node, cfg, ctx as any, {
  ...
  it('ai_companion_response clamps memoryCount to capacity', () => {
    ...
    aiCompanionHandler.onEvent!(node, cfg, ctx as any, { type: 'ai_companion_response', text: 'test' } as any);
  ...
  it('ai_companion_end_interaction resets to idle behavior', () => {
    ...
    aiCompanionHandler.onEvent!(node, cfg, ctx as any, { type: 'ai_companion_end_interaction' } as any);
  ...
  it('no-op when no state', () => {
    expect(() =>
      aiCompanionHandler.onEvent!(mkNode() as any, mkCfg(), mkCtx() as any, { type: 'ai_companion_interact' } as any)
    ).not.toThrow();
  });
});
