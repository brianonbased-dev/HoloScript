...
describe('GeospatialEnvTrait', () => {
  ...
  it('initialized event shifts to localizing', () => {
    sendEvent(geospatialEnvHandler, node, cfg, ctx, { type: 'geospatial_initialized', vpsAvailable: true });
  ...
  it('pose_update localizes and then tracks', () => {
    sendEvent(geospatialEnvHandler, node, cfg, ctx, { type: 'geospatial_initialized', vpsAvailable: false });
    sendEvent(geospatialEnvHandler, node, cfg, ctx, { type: 'geospatial_pose_update', accuracy: 3, verticalAccuracy: 2, headingAccuracy: 5, heading: 90 });
  ...
  it('applies compass smoothing', () => {
    sendEvent(geospatialEnvHandler, node, cfg, ctx, { type: 'geospatial_initialized', vpsAvailable: false });
    sendEvent(geospatialEnvHandler, node, cfg, ctx, { type: 'geospatial_pose_update', accuracy: 3, verticalAccuracy: 2, headingAccuracy: 5, heading: 100 });
    // heading = 0 * 0.8 + 100 * 0.2 = 20
    expect((node as any).__geospatialEnvState.heading).toBeCloseTo(20, 1);
  });

  it('set_origin updates origin', () => {
    sendEvent(geospatialEnvHandler, node, cfg, ctx, { type: 'geospatial_set_origin', latitude: 51.5, longitude: -0.12, altitude: 100 });
  ...
  it('unavailable event sets state', () => {
    sendEvent(geospatialEnvHandler, node, cfg, ctx, { type: 'geospatial_unavailable', reason: 'no GPS' });
    expect((node as any).__geospatialEnvState.state).toBe('unavailable');
  });

  it('query returns state info', () => {
    sendEvent(geospatialEnvHandler, node, cfg, ctx, { type: 'geospatial_query_state', queryId: 'q1' });
