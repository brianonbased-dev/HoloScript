...
export const envConfigHandler: TraitHandler<EnvConfigConfig> = {
  ...
  onDetach(node: any): void { delete node.__envConfigState; },
  onUpdate(): void {},

  onEvent(node: any, _config: EnvConfigConfig, context: any, event: any): void {
    const state = node.__envConfigState as { values: Map<string, { value: unknown; layer: string }> } | undefined;
    if (!state) return;
    const t = typeof event === 'string' ? event : event.type;

    switch (t) {
      case 'envconfig:set':
        state.values.set(event.key as string, { value: event.value, layer: (event.layer as string) ?? 'override' });
        break;
      case 'envconfig:get': {
        const entry = state.values.get(event.key as string);
        context.emit?.('envconfig:result', { key: event.key, value: entry?.value ?? null, source: entry?.layer ?? 'missing' });
        break;
      }
      case 'envconfig:list': {
        const entries = [...state.values.entries()].map(([k, v]) => ({ key: k, value: v.value, layer: v.layer }));
