...
export const HUMANOID_BONE_NAMES = [
  ...
  // Left leg (5)
  'left_upper_leg', 'left_lower_leg', 'left_foot', 'left_toes', 'left_toe_end',

  // Right leg (5)
  'right_upper_leg', 'right_lower_leg', 'right_foot', 'right_toes', 'right_toe_end',
] as const;
...
export const HUMANOID_65_SKELETON: BoneDefinition[] = [
  ...
  { name: 'hips', parent: 'root', position: [0, 0.95, 0], length: 0.12,
    rotationLimits: [-30, 30, -40, 40, -30, 30] },

  // Spine chain
  { name: 'spine', parent: 'hips', position: [0, 0.12, 0], length: 0.12,
    rotationLimits: [-20, 40, -20, 20, -20, 20] },
  { name: 'spine1', parent: 'spine', position: [0, 0.12, 0], length: 0.12,
    rotationLimits: [-20, 30, -15, 15, -15, 15] },
  { name: 'spine2', parent: 'spine1', position: [0, 0.12, 0], length: 0.12,
    rotationLimits: [-15, 25, -10, 10, -10, 10] },
  { name: 'neck', parent: 'spine2', position: [0, 0.12, 0], length: 0.08,
    rotationLimits: [-40, 40, -50, 50, -30, 30] },
  { name: 'head', parent: 'neck', position: [0, 0.08, 0], length: 0.20,
    rotationLimits: [-30, 30, -60, 60, -20, 20] },

  // Left arm
  { name: 'left_shoulder', parent: 'spine2', position: [0.08, 0.10, 0], length: 0.10,
    rotationLimits: [-15, 15, -10, 30, -30, 10] },
  { name: 'left_upper_arm', parent: 'left_shoulder', position: [0.10, 0, 0], length: 0.28,
    rotationLimits: [-90, 90, -60, 135, -90, 30] },
  { name: 'left_forearm', parent: 'left_upper_arm', position: [0.28, 0, 0], length: 0.25,
    rotationLimits: [0, 145, -90, 90, 0, 0] },
  { name: 'left_hand', parent: 'left_forearm', position: [0.25, 0, 0], length: 0.08,
    rotationLimits: [-70, 70, -30, 30, -15, 15] },

  // Left fingers — thumb
  { name: 'left_thumb_proximal', parent: 'left_hand', position: [0.03, -0.01, 0.02], length: 0.03 },
  { name: 'left_thumb_intermediate', parent: 'left_thumb_proximal', position: [0.03, 0, 0], length: 0.025 },
  { name: 'left_thumb_distal', parent: 'left_thumb_intermediate', position: [0.025, 0, 0], length: 0.02 },
  // Left fingers — index
  { name: 'left_index_proximal', parent: 'left_hand', position: [0.08, 0, 0.02], length: 0.035 },
  { name: 'left_index_intermediate', parent: 'left_index_proximal', position: [0.035, 0, 0], length: 0.025 },
  { name: 'left_index_distal', parent: 'left_index_intermediate', position: [0.025, 0, 0], length: 0.02 },
  // Left fingers — middle
  { name: 'left_middle_proximal', parent: 'left_hand', position: [0.08, 0, 0], length: 0.038 },
  { name: 'left_middle_intermediate', parent: 'left_middle_proximal', position: [0.038, 0, 0], length: 0.028 },
  { name: 'left_middle_distal', parent: 'left_middle_intermediate', position: [0.028, 0, 0], length: 0.02 },
  // Left fingers — ring
  { name: 'left_ring_proximal', parent: 'left_hand', position: [0.08, 0, -0.015], length: 0.035 },
  { name: 'left_ring_intermediate', parent: 'left_ring_proximal', position: [0.035, 0, 0], length: 0.025 },
  { name: 'left_ring_distal', parent: 'left_ring_intermediate', position: [0.025, 0, 0], length: 0.018 },
  // Left fingers — pinky
  { name: 'left_pinky_proximal', parent: 'left_hand', position: [0.075, 0, -0.03], length: 0.028 },
  { name: 'left_pinky_intermediate', parent: 'left_pinky_proximal', position: [0.028, 0, 0], length: 0.02 },
  { name: 'left_pinky_distal', parent: 'left_pinky_intermediate', position: [0.02, 0, 0], length: 0.015 },

  // Right arm (mirrored X)
  { name: 'right_shoulder', parent: 'spine2', position: [-0.08, 0.10, 0], length: 0.10,
    rotationLimits: [-15, 15, -30, 10, -10, 30] },
  { name: 'right_upper_arm', parent: 'right_shoulder', position: [-0.10, 0, 0], length: 0.28,
    rotationLimits: [-90, 90, -135, 60, -30, 90] },
  { name: 'right_forearm', parent: 'right_upper_arm', position: [-0.28, 0, 0], length: 0.25,
    rotationLimits: [0, 145, -90, 90, 0, 0] },
  { name: 'right_hand', parent: 'right_forearm', position: [-0.25, 0, 0], length: 0.08,
    rotationLimits: [-70, 70, -30, 30, -15, 15] },

  // Right fingers — thumb
  { name: 'right_thumb_proximal', parent: 'right_hand', position: [-0.03, -0.01, 0.02], length: 0.03 },
  { name: 'right_thumb_intermediate', parent: 'right_thumb_proximal', position: [-0.03, 0, 0], length: 0.025 },
  { name: 'right_thumb_distal', parent: 'right_thumb_intermediate', position: [-0.025, 0, 0], length: 0.02 },
  // Right fingers — index
  { name: 'right_index_proximal', parent: 'right_hand', position: [-0.08, 0, 0.02], length: 0.035 },
  { name: 'right_index_intermediate', parent: 'right_index_proximal', position: [-0.035, 0, 0], length: 0.025 },
  { name: 'right_index_distal', parent: 'right_index_intermediate', position: [-0.025, 0, 0], length: 0.02 },
  // Right fingers — middle
  { name: 'right_middle_proximal', parent: 'right_hand', position: [-0.08, 0, 0], length: 0.038 },
  { name: 'right_middle_intermediate', parent: 'right_middle_proximal', position: [-0.038, 0, 0], length: 0.028 },
  { name: 'right_middle_distal', parent: 'right_middle_intermediate', position: [-0.028, 0, 0], length: 0.02 },
  // Right fingers — ring
  { name: 'right_ring_proximal', parent: 'right_hand', position: [-0.08, 0, -0.015], length: 0.035 },
  { name: 'right_ring_intermediate', parent: 'right_ring_proximal', position: [-0.035, 0, 0], length: 0.025 },
  { name: 'right_ring_distal', parent: 'right_ring_intermediate', position: [-0.025, 0, 0], length: 0.018 },
  // Right fingers — pinky
  { name: 'right_pinky_proximal', parent: 'right_hand', position: [-0.075, 0, -0.03], length: 0.028 },
  { name: 'right_pinky_intermediate', parent: 'right_pinky_proximal', position: [-0.028, 0, 0], length: 0.02 },
  { name: 'right_pinky_distal', parent: 'right_pinky_intermediate', position: [-0.02, 0, 0], length: 0.015 },

  // Left leg
  { name: 'left_upper_leg', parent: 'hips', position: [0.09, -0.05, 0], length: 0.42,
    rotationLimits: [-20, 120, -45, 45, -40, 30] },
  { name: 'left_lower_leg', parent: 'left_upper_leg', position: [0, -0.42, 0], length: 0.40,
    rotationLimits: [-130, 0, 0, 0, 0, 0] },
  { name: 'left_foot', parent: 'left_lower_leg', position: [0, -0.40, 0], length: 0.15,
    rotationLimits: [-40, 40, -20, 20, -15, 15] },
  { name: 'left_toes', parent: 'left_foot', position: [0, 0, 0.15], length: 0.06,
    rotationLimits: [-30, 60, 0, 0, 0, 0] },
  { name: 'left_toe_end', parent: 'left_toes', position: [0, 0, 0.06], length: 0 },

  // Right leg (mirrored X)
  { name: 'right_upper_leg', parent: 'hips', position: [-0.09, -0.05, 0], length: 0.42,
    rotationLimits: [-20, 120, -45, 45, -30, 40] },
  { name: 'right_lower_leg', parent: 'right_upper_leg', position: [0, -0.42, 0], length: 0.40,
    rotationLimits: [-130, 0, 0, 0, 0, 0] },
  { name: 'right_foot', parent: 'right_lower_leg', position: [0, -0.40, 0], length: 0.15,
    rotationLimits: [-40, 40, -20, 20, -15, 15] },
  { name: 'right_toes', parent: 'right_foot', position: [0, 0, 0.15], length: 0.06,
    rotationLimits: [-30, 60, 0, 0, 0, 0] },
 ...
 */
export function getBoneChildren(boneName: string, skeleton: BoneDefinition[] = HUMANOID_65_SKELETON): BoneDefinition[] {
 ...
 */
export function getBoneChain(boneName: string, skeleton: BoneDefinition[] = HUMANOID_65_SKELETON): BoneDefinition[] {
