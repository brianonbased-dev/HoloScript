/**
 * SovereignWorldAdapter — native HoloScript world generation
 *
 * Powered by the Brittney v43 generative model. This is the canonical
 * first-class adapter for the sovereign-3d engine. All external bridge
 * adapters (HY-World, Spatial-TTT, etc.) have been formally removed.
 *
...
export class SovereignWorldAdapter implements WorldGeneratorAdapter {
  ...
  private async submitJob(req: WorldGenerationRequest): Promise<string> {
    ...
    if (!response.job_id) {
      throw new Error('[SovereignWorldAdapter] /api/generate did not return a job_id');
    }
  ...
  private async waitForCompletion(jobId: string): Promise<WorldGenerationResult> {
    ...
    while (Date.now() < deadline) {
      ...
      if (job.status === 'error') {
        throw new Error(
          `[SovereignWorldAdapter] Job ${jobId} failed: ${job.error ?? 'unknown error'}`
        );
      }
  ...
  private mapResult(job: SovereignJobResponse): WorldGenerationResult {
    if (!job.asset_url) {
      throw new Error(
        `[SovereignWorldAdapter] Job ${job.job_id} completed but returned no asset_url`
      );
    }
  ...
  private async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `[SovereignWorldAdapter] HTTP ${res.status} at ${path}${text ? ': ' + text : ''}`
      );
    }
