...
export class LWWRegister<T> {
  private value: T; private timestamp: number; private nodeId: string;
  constructor(initial: T, nodeId: string) { this.value = initial; this.timestamp = 0; this.nodeId = nodeId; }
  ...
  merge(other: { value: T; timestamp: number; nodeId: string }): void {
    if (other.timestamp > this.timestamp || (other.timestamp === this.timestamp && other.nodeId > this.nodeId)) {
      this.value = other.value; this.timestamp = other.timestamp;
    }
  }
  getState(): { value: T; timestamp: number; nodeId: string } { return { value: this.value, timestamp: this.timestamp, nodeId: this.nodeId }; }
}
...
export class GCounter {
  ...
  constructor(nodeId: string) { this.nodeId = nodeId; this.counts.set(nodeId, 0); }
  ...
  value(): number {
    let sum = 0; for (const v of this.counts.values()) sum += v; return sum;
  }
  ...
  getState(): Map<string, number> { return new Map(this.counts); }
}

/** PN-Counter: Positive-Negative Counter (grow + shrink) */
export class PNCounter {
  private pos: GCounter; private neg: GCounter;
  constructor(nodeId: string) { this.pos = new GCounter(nodeId); this.neg = new GCounter(nodeId); }
  increment(amount: number = 1): void { this.pos.increment(amount); }
  decrement(amount: number = 1): void { this.neg.increment(amount); }
  value(): number { return this.pos.value() - this.neg.value(); }
  merge(other: { pos: Map<string, number>; neg: Map<string, number> }): void {
    this.pos.merge(other.pos); this.neg.merge(other.neg);
  }
...
export class ORSet<T> {
  ...
  private nodeId: string; private tagCounter: number = 0;
  constructor(nodeId: string) { this.nodeId = nodeId; }
  private generateTag(): string { return `${this.nodeId}:${++this.tagCounter}:${Date.now()}`; }
  add(value: T): string {
  ...
  remove(value: T): void {
    ...
    for (const [tag, entry] of this.elements) {
      if (JSON.stringify(entry.value) === key) {
        this.tombstones.add(tag); this.elements.delete(tag);
      }
    }
  }
  has(value: T): boolean {
  ...
  values(): T[] {
    ...
    for (const entry of this.elements.values()) {
      ...
      if (!seen.has(key)) { seen.add(key); result.push(entry.value); }
    }
    return result;
  }
  size(): number { return this.values().length; }
  merge(other: { elements: Map<string, { value: T; tag: string }>; tombstones: Set<string> }): void {
