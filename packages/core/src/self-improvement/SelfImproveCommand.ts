...
export class SelfImproveCommand {
      ...
      try {
        ...
        if (vitestResult.passed) {
          ...
          if (this.consecutiveFailures >= this.config.maxConsecutiveFailures) {
            this.io.log('error', `Max consecutive failures (${this.config.maxConsecutiveFailures}) reached. Aborting.`);
            record.duration = Date.now() - iterStart;
            this.iterations.push(record);
  ...
  private buildResult(
    ...
    const lastQuality = this.iterations
      .filter((r) => r.qualityReport)
      .pop()?.qualityReport ?? null;
