...
describe('DelegationEngine', () => {
  ...
  beforeEach(() => {
    ...
    engine = new DelegationEngine({ hierarchyManager: mgr, config: { defaultTimeout: 5000, defaultRetries: 2, autoEscalate: false } });
    mgr.createHierarchy({ id: 'h1', name: 'Test', supervisor: sup, subordinates: [sub1, sub2], escalationPath: ['sub1', 'sub2', 'sup'] });
  });
  ...
  it('delegates a task to specified assignee', async () => {
    const task = await engine.delegate({ hierarchyId: 'h1', taskType: 'render', payload: { data: 1 }, assigneeId: 'sub1' });
  ...
  it('throws if no subordinates available', async () => {
    ...
    await expect(engine2.delegate({ hierarchyId: 'h2', taskType: 'x', payload: {} })).rejects.toThrow();
    engine2.destroy();
  });

  it('startTask transitions pending → in_progress', async () => {
    const task = await engine.delegate({ hierarchyId: 'h1', taskType: 'x', payload: {}, assigneeId: 'sub1' });
  ...
  it('escalateTask returns null when no more targets', async () => {
    const task = await engine.delegate({ hierarchyId: 'h1', taskType: 'x', payload: {}, assigneeId: 'sup' });
  ...
  it('getActiveTasks filters by active statuses', async () => {
    const t1 = await engine.delegate({ hierarchyId: 'h1', taskType: 'a', payload: {}, assigneeId: 'sub1' });
    await engine.delegate({ hierarchyId: 'h1', taskType: 'b', payload: {}, assigneeId: 'sub2' });
  ...
  it('getTasksByStatus', async () => {
    const t = await engine.delegate({ hierarchyId: 'h1', taskType: 'a', payload: {}, assigneeId: 'sub1' });
  ...
  it('createSubtask links parent and child', async () => {
    const parent = await engine.delegate({ hierarchyId: 'h1', taskType: 'big', payload: {}, assigneeId: 'sub1' });
    const child = await engine.createSubtask(parent.id, { taskType: 'small', payload: {}, assigneeId: 'sub2' });
  ...
  it('areSubtasksComplete returns true when all done', async () => {
    const parent = await engine.delegate({ hierarchyId: 'h1', taskType: 'big', payload: {}, assigneeId: 'sub1' });
    const child = await engine.createSubtask(parent.id, { taskType: 'a', payload: {}, assigneeId: 'sub2' });
