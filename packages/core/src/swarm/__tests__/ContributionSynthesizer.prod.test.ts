...
function make(cfg = {}) { return new ContributionSynthesizer(cfg); }
...
function session(contribs: IHiveContribution[]): IHiveSession {
  return { id: 'sess', topic: 't', goal: 'g', initiator: 'a1', status: 'active', participants: ['a1'], contributions: contribs };
}
...
describe('ContributionSynthesizer — defaults', () => {
  ...
  it('default minConfidenceThreshold=0.3', () => {
    ...
    const s = make(); const sess = session([c('idea', 'low', 0.1), c('idea', 'ok', 0.5)]);
...
describe('ContributionSynthesizer — synthesize with empty session', () => {
  it('empty session returns zero result', () => {
    const s = make(); const r = s.synthesize(session([]));
...
describe('ContributionSynthesizer — method selection', () => {
  it('hierarchical when solutions present', () => {
    const s = make(); const sess = session([c('solution', 'use redis'), c('idea', 'try caching')]);
...
describe('ContributionSynthesizer — hierarchical synthesis', () => {
  it('content includes solution section', () => {
    const s = make(); const sess = session([c('solution', 'deploy redis cluster'), c('idea', 'optimize queries')]);
  ...
  it('sources contain solution contribution ids', () => {
    ...
    const s = make(); const r = s.synthesize(session([sol]));
  ...
  it('solutions sorted by confidence (highest first)', () => {
    ...
    const s = make(); const content = s.synthesize(session([low, high])).synthesizedContent;
...
describe('ContributionSynthesizer — merge synthesis', () => {
  ...
  it('sources contain all included contribution ids', () => {
    const i1 = c('idea', 'first'); const i2 = c('critique', 'second');
    const s = make(); const r = s.synthesize(session([i1, i2]));
    expect(r.sourceContributions).toContain(i1.id); expect(r.sourceContributions).toContain(i2.id);
  });
});
...
describe('ContributionSynthesizer — synthesizeSubset', () => {
  ...
  it('empty subset → empty result', () => {
    ...
    expect(r.synthesizedContent).toBe(''); expect(r.confidence).toBe(0);
  });
});
...
describe('ContributionSynthesizer — merge()', () => {
  ...
  it('merged id contains both source ids', () => {
    const a = c('idea', 'a'); const b = c('idea', 'b'); const s = make();
    const m = s.merge(a, b);
    expect(m.id).toContain(a.id); expect(m.id).toContain(b.id);
  });
});

describe('ContributionSynthesizer — findSimilar()', () => {
  it('excludes self', () => {
    const s = make(); const target = c('idea', 'machine learning optimization');
  ...
  it('empty candidates returns empty', () => {
    const s = make(); const target = c('idea', 'any content');
...
describe('ContributionSynthesizer — metadata', () => {
  ...
  it('type counts correct', () => {
    const s = make();
    const sess = session([c('idea', 'a'), c('idea', 'b'), c('critique', 'c'), c('solution', 'd'), c('consensus', 'e')]);
    const r = s.synthesize(sess);
