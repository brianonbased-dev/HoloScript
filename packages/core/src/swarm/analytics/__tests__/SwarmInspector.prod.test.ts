...
import type { IAgentSnapshot, ISwarmSnapshot, IAgentRelation, IHealthCheck } from '../SwarmInspector';

function make(cfg = {}) { return new SwarmInspector(cfg); }

function agent(id: string, overrides: Partial<IAgentSnapshot> = {}): IAgentSnapshot {
  return { id, state: 'active', health: 1, load: 0, lastActive: Date.now(), properties: {}, ...overrides };
}
function swarm(id: string, overrides: Partial<ISwarmSnapshot> = {}): ISwarmSnapshot {
  return { id, memberCount: 0, state: 'active', createdAt: Date.now(), properties: {}, ...overrides };
}
...
describe('SwarmInspector — construction', () => {
  ...
  it('empty state on construction', () => { const s = make(); expect(s.getAllAgents()).toHaveLength(0); });
});

describe('SwarmInspector — agent tracking', () => {
  it('updateAgent stores snapshot', () => {
    const s = make(); s.updateAgent(agent('a1'));
    expect(s.getAgent('a1')).toBeDefined();
    expect(s.getAgent('a1')!.id).toBe('a1');
  });
  it('getAllAgents returns all', () => {
    const s = make(); s.updateAgent(agent('a1')); s.updateAgent(agent('a2'));
    expect(s.getAllAgents()).toHaveLength(2);
  });
  it('updateAgent overwrites existing', () => {
    const s = make(); s.updateAgent(agent('a1', { health: 1 })); s.updateAgent(agent('a1', { health: 0.5 }));
    expect(s.getAgent('a1')!.health).toBe(0.5);
  });
  it('removeAgent deletes snapshot', () => {
    const s = make(); s.updateAgent(agent('a1')); s.removeAgent('a1');
    expect(s.getAgent('a1')).toBeUndefined();
  });
  it('getAgent unknown=undefined', () => { expect(make().getAgent('ghost')).toBeUndefined(); });
...
describe('SwarmInspector — swarm tracking', () => {
  it('updateSwarm stores snapshot', () => {
    const s = make(); s.updateSwarm(swarm('sw1'));
    expect(s.getSwarm('sw1')).toBeDefined();
  });
  it('getAllSwarms returns all', () => {
    const s = make(); s.updateSwarm(swarm('sw1')); s.updateSwarm(swarm('sw2'));
  ...
  it('removeSwarm deletes snapshot', () => {
    const s = make(); s.updateSwarm(swarm('sw1')); s.removeSwarm('sw1');
...
describe('SwarmInspector — relations', () => {
  ...
  it('removeAgent also removes its relations', () => {
    const s = make();
    s.updateAgent(agent('a1')); s.updateAgent(agent('a2'));
    s.addRelation({ sourceId: 'a1', targetId: 'a2', type: 'neighbor', strength: 1 });
    s.removeAgent('a1');
    expect(s.getAllRelations()).toHaveLength(0);
  });
  it('getAllRelations returns copy', () => {
    const s = make(); const r1 = s.getAllRelations(); const r2 = s.getAllRelations();
...
describe('SwarmInspector — health checks', () => {
  it('registerHealthCheck stored', () => {
    const s = make(); s.registerHealthCheck(health('db'));
    expect(s.getHealthCheck('db')?.name).toBe('db');
  });
  it('getAllHealthChecks returns all', () => {
    const s = make(); s.registerHealthCheck(health('db')); s.registerHealthCheck(health('cache'));
    expect(s.getAllHealthChecks()).toHaveLength(2);
  });
  it('getOverallHealth healthy when all healthy', () => {
    const s = make(); s.registerHealthCheck(health('a')); s.registerHealthCheck(health('b'));
    expect(s.getOverallHealth()).toBe('healthy');
  });
  it('getOverallHealth degraded when any degraded', () => {
    const s = make();
    s.registerHealthCheck(health('a', 'healthy')); s.registerHealthCheck(health('b', 'degraded'));
    expect(s.getOverallHealth()).toBe('degraded');
  });
  it('getOverallHealth unhealthy when any unhealthy', () => {
    const s = make();
    s.registerHealthCheck(health('a', 'healthy')); s.registerHealthCheck(health('b', 'unhealthy'));
    expect(s.getOverallHealth()).toBe('unhealthy');
  });
  it('empty checks → healthy', () => { expect(make().getOverallHealth()).toBe('healthy'); });
  it('getHealthCheck unknown=undefined', () => { expect(make().getHealthCheck('ghost')).toBeUndefined(); });
});

describe('SwarmInspector — event log', () => {
  it('log stores event', () => {
    const s = make(); s.log('info', 'test', 'message');
    expect(s.getEventLog()).toHaveLength(1);
  });
  it('log helpers (trace/debug/info/warn/error)', () => {
    const s = make();
    s.trace('src', 'trace msg'); s.debug('src', 'debug msg'); s.info('src', 'info msg');
    s.warn('src', 'warn msg'); s.error('src', 'error msg');
    expect(s.getEventLog()).toHaveLength(5);
  });
  it('event has id, timestamp, level, source, message', () => {
    const s = make(); s.info('mySource', 'hello');
    const e = s.getEventLog()[0];
    expect(e.id).toBeTruthy(); expect(e.timestamp).toBeTruthy();
    expect(e.level).toBe('info'); expect(e.source).toBe('mySource'); expect(e.message).toBe('hello');
  });
  it('getEventLog filter by level', () => {
    const s = make(); s.info('s', 'm'); s.warn('s', 'w');
    expect(s.getEventLog({ level: 'warn' })).toHaveLength(1);
  });
  it('getEventLog filter by source', () => {
    const s = make(); s.info('alpha', 'a'); s.info('beta', 'b');
  ...
  it('clearEventLog empties log', () => {
    const s = make(); s.info('s', 'm'); s.clearEventLog();
...
describe('SwarmInspector — toGraph()', () => {
  it('returns nodes and edges', () => {
    const s = make(); const g = s.toGraph();
    expect(g).toHaveProperty('nodes'); expect(g).toHaveProperty('edges');
  });
  ...
  it('member edge from agent to swarm', () => {
    ...
    const g = s.toGraph();
    expect(g.edges.some((e) => e.source === 'a1' && e.target === 'sw1' && e.type === 'member')).toBe(true);
  });
  it('relation edge in graph', () => {
    const s = make();
    s.addRelation({ sourceId: 'a1', targetId: 'a2', type: 'neighbor', strength: 0.7 });
    const g = s.toGraph();
    expect(g.edges.some((e) => e.source === 'a1' && e.target === 'a2' && e.weight === 0.7)).toBe(true);
  });
});
...
describe('SwarmInspector — reset()', () => {
  it('clears all state', () => {
    const s = make();
    s.updateAgent(agent('a1')); s.updateSwarm(swarm('sw1'));
    s.addRelation({ sourceId: 'a1', targetId: '?', type: 'custom', strength: 1 });
    s.registerHealthCheck(health('db')); s.info('src', 'msg');
    s.reset();
