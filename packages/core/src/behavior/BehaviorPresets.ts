...
export function createPatrolBehavior(
  ...
  return new RepeaterNode(
    new SequenceNode([
      new ActionNode('selectNextWaypoint', (ctx: any) => {
        ctx.waypointIndex = ((ctx.waypointIndex || 0) + 1) % waypointCount;
        ctx.targetReached = false;
        return 'success';
      }),
      new ActionNode('moveToWaypoint', (ctx: any, delta: number) => {
...
export function createIdleBehavior(idleDuration: number = 3.0): BTNode {
  return new RepeaterNode(
    new SequenceNode([
      ...
      new ActionNode('lookAround', (ctx: any) => {
...
export function createInteractBehavior(): BTNode {
  return new SequenceNode([
    new ConditionNode('hasTarget', (ctx: any) => !!ctx.interactTarget),
    new ActionNode('approach', (ctx: any, delta: number) => {
    ...
    new ActionNode('interact', (ctx: any) => {
...
export function createFollowBehavior(minDist: number = 1.5): BTNode {
  return new RepeaterNode(
    new SelectorNode([
      ...
      new SequenceNode([
        new ConditionNode('closeEnough', (ctx: any) => {
      ...
      new ActionNode('moveToTarget', (ctx: any, delta: number) => {
...
export function createAlertBehavior(): BTNode {
  return new SelectorNode([
    // Fight if brave
    new SequenceNode([
      new ConditionNode('isBrave', (ctx: any) => ctx.courage > 0.5),
      new ConditionNode('hasThreat', (ctx: any) => !!ctx.threat),
      new ActionNode('fight', (ctx: any) => {
    ...
    new SequenceNode([
      new ConditionNode('hasThreat', (ctx: any) => !!ctx.threat),
      new ActionNode('flee', (ctx: any, delta: number) => {
