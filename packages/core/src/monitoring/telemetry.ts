...
class TelemetryRegistry {
    ...
    incrementCounter(name: string, value: number = 1, labels?: Record<string, string>) {
        this.measurements.push({
            name, type: 'counter', value, labels, timestamp: Date.now()
        });
     ...
     */
    async measureLatency<T>(name: string, fn: () => Promise<T>, labels?: Record<string, string>): Promise<T> {
        ...
        try {
            ...
            this.measurements.push({
                name, type: 'histogram', value: duration, labels, timestamp: Date.now()
            });
            if (this.measurements.length > this.maxHistory) this.measurements.shift();
            
            // SLA alert for severe lag
            if (duration > 500) {
                console.warn(`[SLO VIOLATION] ${name} exceeded 500ms latency threshold! (${duration.toFixed(2)}ms)`);
            }
        }
    }
