...
function policy(overrides: Partial<SecurityPolicy['code']> = {}, network: Partial<SecurityPolicy['network']> = {}): SecurityPolicy {
  return {
    code: {
      maxObjectCount: 10,
      maxTraitDepth: 5,
...
describe('validateComposition — trait depth', () => {
  ...
  it('no traits = depth of 0, passes always', () => {
    const ast = [obj('A', [])];
    expect(validateComposition(ast, policy({ maxTraitDepth: 0 })).violations
      .filter(v => v.category === 'trait_depth')).toHaveLength(0);
  });
});
...
describe('validateImports — allowedHosts', () => {
  it('allowed host passes', () => {
    ...
    expect(validateImports(imp, policy({}, { allowedHosts: ['cdn.example.com'] })).passed).toBe(true);
  });
  it('disallowed host fails with network_access error', () => {
    ...
    expect(result.passed).toBe(false);
    expect(result.violations.some(v => v.category === 'network_access' && v.severity === 'error')).toBe(true);
  });
...
describe('scanForVulnerabilities — code_injection', () => {
  it('eval() flagged as error', () => {
    ...
    expect(r.violations.some(v => v.category === 'code_injection' && v.severity === 'error')).toBe(true);
  });
  ...
  it('setTimeout with string flagged as warning', () => {
    ...
    expect(r.violations.some(v => v.category === 'code_injection' && v.severity === 'warning')).toBe(true);
  });
...
describe('scanForVulnerabilities — script_injection', () => {
  it('innerHTML assignment flagged as error', () => {
    ...
    expect(r.violations.some(v => v.category === 'script_injection' && v.severity === 'error')).toBe(true);
  });
  it('outerHTML assignment flagged as warning', () => {
    const r = scanForVulnerabilities('el.outerHTML = val;');
    expect(r.violations.some(v => v.category === 'script_injection' && v.severity === 'warning')).toBe(true);
  });
...
describe('scanForVulnerabilities — prototype_pollution', () => {
  it('__proto__ access flagged as error', () => {
    ...
    expect(r.violations.some(v => v.category === 'prototype_pollution' && v.severity === 'error')).toBe(true);
  });
...
describe('scanForVulnerabilities — dangerous_api', () => {
  it("require('child_process') flagged as error", () => {
    ...
    expect(r.violations.some(v => v.category === 'dangerous_api' && v.severity === 'error')).toBe(true);
  });
  it('exec() call flagged as warning', () => {
    const r = scanForVulnerabilities('exec("ls -la");');
    expect(r.violations.some(v => v.category === 'dangerous_api' && v.severity === 'warning')).toBe(true);
  });
...
describe('scanForVulnerabilities — line numbers and clean code', () => {
  ...
  it('clean code returns passed = true', () => {
    const r = scanForVulnerabilities('const x = 1 + 2;\nfunction greet(name) { return "Hello " + name; }');
