...
export class KeyboardSystem {
    ...
    private updateFocusVisuals() {
        ...
        if (textNode) {
            ...
            textNode.properties.color = text.length > 0 ? (node.properties.textColor ?? '#ffffff') : '#888888';
        }
        ...
        if (cursorNode) {
             ...
             this.context.emit('property_changed', { nodeId: `${this.focusedNodeId}_cursor`, property: 'position', value: cursorNode.properties.position });
             this.context.emit('property_changed', { nodeId: `${this.focusedNodeId}_cursor`, property: 'visible', value: true });
        }
        ...
        this.context.emit('property_changed', { nodeId: textNodeId, property: 'text', value: textNode?.properties.text });
    }
    ...
    setFocus(nodeId: string) {
        ...
        if (this.focusedNodeId && this.focusedNodeId !== nodeId) {
             ...
             if (prevCursor) {
                 ...
                 this.context.emit('property_changed', { nodeId: `${this.focusedNodeId}_cursor`, property: 'visible', value: false });
             }
        }
