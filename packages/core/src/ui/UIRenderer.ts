...
export class UIRenderer {
  ...
  getWorldRect(node: UINode): UIRect {
    let x = node.rect.x ?? (node.rect as unknown as number[])[0] ?? 0;
    let y = node.rect.y ?? (node.rect as unknown as number[])[1] ?? 0;
    let p = node.parent;
    while (p) {
      x += p.rect.x ?? (p.rect as unknown as number[])[0] ?? 0;
      y += p.rect.y ?? (p.rect as unknown as number[])[1] ?? 0;
      p = p.parent;
    }
