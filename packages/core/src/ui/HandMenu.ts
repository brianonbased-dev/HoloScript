...
import { HSPlusRuntime, VRHand, Vector3 } from '../types/HoloScriptPlus';
...
export class HandMenuSystem {
    ...
    constructor(private runtime: HSPlusRuntime, engine?: AnimationEngine) {
    ...
    update(delta: number) {
        ...
        const leftHand = (this.runtime as any).vrContext?.hands?.left;
        if (!leftHand) return;
    ...
    private showMenu(hand: VRHand) {
        ...
        const menu = createUIPanel(menuId, {
            width: 0.2,
            height: 0.15,
            color: '#1a1a1a'
        }, [
        ...
        (this.runtime as any).mountObject(menu);
        ...
        this.transitions.popIn(
            menuId,
            (s) => { if (this.menuNode?.properties) this.menuNode.properties.scale = s; },
            (o) => { if (this.menuNode?.properties) this.menuNode.properties.opacity = o; },
            { duration: 0.35, onComplete: () => { this.isTransitioning = false; } }
        );
    }
