 ...
 */
export function autoWireNPCs(
  objects: NPCObject[],
  context: NPCWireContext = {}
): {
  ...
  for (const obj of objects) {
    ...
    if (hasAINPC) {
      const cfg: Partial<AINPCBrainConfig> =
        obj.directives?.find((d) => d.name.toLowerCase() === 'ai_npc_brain')?.config || {};
      try {
        ainpcBrainHandler.onAttach?.(
          obj as any,
          { ...ainpcBrainHandler.defaultConfig, ...cfg } as AINPCBrainConfig,
          context as any
        );
 ...
 */
export function registerNPCTraits(traitSystem?: {
