 ...
 *
 * Co-located in `@holoscript/core` so `HoloScriptAgentRuntime` can import it
 * without cross-package circular dependencies. `HoloScriptRuntime` implements
 * this interface to become the native kernel behind the agent facade.
 *
 * @module runtime/IParentRuntime
 */
import type { ExecutionResult, HoloScriptValue } from '../types';
import type { Scope } from './holo-statement-executor';
