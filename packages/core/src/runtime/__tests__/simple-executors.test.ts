...
describe('executeStateMachine', () => {
  it('registers the node in the stateMachines map', async () => {
    const ctx = makeCtx();
    const node = { type: 'stateMachine', name: 'auth', states: [] } as unknown as Parameters<
      typeof executeStateMachine
    >[0];
  ...
  it('returns success:true', async () => {
    const ctx = makeCtx();
    const node = { type: 'stateMachine', name: 'login', states: [] } as unknown as Parameters<
      typeof executeStateMachine
    >[0];
  ...
  it('returns registered name in output', async () => {
    const ctx = makeCtx();
    const node = { type: 'stateMachine', name: 'myFSM', states: [] } as unknown as Parameters<
      typeof executeStateMachine
    >[0];
    const result = await executeStateMachine(node, ctx);
...
describe('executeEnvironment', () => {
  it('merges node.settings into current environment', async () => {
    ...
    await executeEnvironment(
      { type: 'environment', settings: { skybox: 'night' } } as unknown as Parameters<
        typeof executeEnvironment
      >[0],
      ctx
    );
  ...
  it('preserves existing environment keys not in settings', async () => {
    ...
    await executeEnvironment(
      { type: 'environment', settings: { gravity: 9.8 } } as unknown as Parameters<
        typeof executeEnvironment
      >[0],
      ctx
    );
  ...
  it('returns success:true', async () => {
    ...
    const result = await executeEnvironment(
      { type: 'environment', settings: {} } as unknown as Parameters<typeof executeEnvironment>[0],
      ctx
    );
    expect(result.success).toBe(true);
  });
...
describe('executeStructure', () => {
  it('returns success:true', async () => {
    const result = await executeStructure({ type: 'nexus' } as unknown as Parameters<
      typeof executeStructure
    >[0]);
    expect(result.success).toBe(true);
  });

  it('includes node.type and created:true in output', async () => {
    const result = await executeStructure({ type: 'tower' } as unknown as Parameters<
      typeof executeStructure
    >[0]);
  ...
  it('uses cube hologram for non-nexus types', async () => {
    const result = await executeStructure({ type: 'building' } as unknown as Parameters<
      typeof executeStructure
    >[0]);
  ...
  it('uses custom node.hologram when provided', async () => {
    ...
    const result = await executeStructure({
      type: 'nexus',
      hologram: custom,
    } as unknown as Parameters<typeof executeStructure>[0]);
    expect(result.hologram).toBe(custom);
  });

  it('passes node.position as spatialPosition', async () => {
    const position = [1, 2, 3];
    const result = await executeStructure({ type: 'nexus', position } as unknown as Parameters<
      typeof executeStructure
    >[0]);
...
describe('executeAssignment', () => {
  it('evaluates the value expression', async () => {
    ...
    await executeAssignment(
      { type: 'assignment', name: 'counter', value: '99' } as unknown as Parameters<
        typeof executeAssignment
      >[0],
      ctx
    );
  ...
  it('calls setVariable with the evaluated value', async () => {
    ...
    await executeAssignment(
      { type: 'assignment', name: 'msg', value: 'hello' } as unknown as Parameters<
        typeof executeAssignment
      >[0],
      ctx
    );
  ...
  it('returns success:true', async () => {
    ...
    const result = await executeAssignment(
      { type: 'assignment', name: 'x', value: '1' } as unknown as Parameters<
        typeof executeAssignment
      >[0],
      ctx
    );
  ...
  it('includes assigned name and value in output', async () => {
    const ctx = makeCtx({ evaluateExpression: vi.fn(() => 42) });
    const result = await executeAssignment(
      { type: 'assignment', name: 'score', value: '42' } as unknown as Parameters<
        typeof executeAssignment
      >[0],
      ctx
    );
...
describe('executeReturn', () => {
  it('evaluates node.value when present', async () => {
    ...
    await executeReturn(
      { type: 'return', value: 10 } as unknown as Parameters<typeof executeReturn>[0],
      ctx
    );
  ...
  it('falls back to node.expression when value is absent', async () => {
    ...
    await executeReturn(
      { type: 'return', expression: 'x * 2' } as unknown as Parameters<typeof executeReturn>[0],
      ctx
    );
  ...
  it('returns success:true', async () => {
    ...
    const result = await executeReturn(
      { type: 'return', value: 1 } as unknown as Parameters<typeof executeReturn>[0],
      ctx
    );
    expect(result.success).toBe(true);
  });
  ...
  it('returns the evaluated value as output', async () => {
    ...
    const result = await executeReturn(
      { type: 'return', value: 'x' } as unknown as Parameters<typeof executeReturn>[0],
      ctx
    );
...
describe('executeScale', () => {
  it('sets scale to parent * node.multiplier', async () => {
    ...
    await executeScale(
      { type: 'scale', multiplier: 3, magnitude: 'x6', body: [] } as unknown as Parameters<
        typeof executeScale
      >[0],
      ctx
    );
  ...
  it('emits scale:change on entry with new scale values', async () => {
    ...
    await executeScale(
      { type: 'scale', multiplier: 4, magnitude: 'x4', body: [] } as unknown as Parameters<
        typeof executeScale
      >[0],
      ctx
    );
    expect(ctx.emit).toHaveBeenCalledWith(
      'scale:change',
      expect.objectContaining({ multiplier: 4, magnitude: 'x4' })
    );
  });
  ...
  it('executes the body program', async () => {
    ...
    await executeScale(
      { type: 'scale', multiplier: 1, magnitude: 'x1', body } as unknown as Parameters<
        typeof executeScale
      >[0],
      ctx
    );
  ...
  it('emits scale:change on exit with parent multiplier', async () => {
    const ctx = makeCtx({ getScale: vi.fn(() => ({ multiplier: 1, magnitude: 'x1' })) });
    await executeScale(
      { type: 'scale', multiplier: 2, magnitude: 'x2', body: [] } as unknown as Parameters<
        typeof executeScale
      >[0],
      ctx
    );
  ...
  it('output describes the scale magnitude', async () => {
    const ctx = makeCtx();
    const result = await executeScale(
      { type: 'scale', multiplier: 5, magnitude: 'x5', body: [] } as unknown as Parameters<
        typeof executeScale
      >[0],
      ctx
    );
...
describe('executeComposition', () => {
  it('executes flat children when no body', async () => {
    ...
    await executeComposition(
      { type: 'composition', name: 'test', children } as unknown as Parameters<
        typeof executeComposition
      >[0],
      ctx
    );
  ...
  it('executes body.systems, body.configs, body.children when body present', async () => {
    ...
    await executeComposition(
      { type: 'composition', name: 'app', body } as unknown as Parameters<
        typeof executeComposition
      >[0],
      ctx
    );
  ...
  it('returns success:true when all results pass (body path)', async () => {
    ...
    const result = await executeComposition(
      ...
      } as unknown as Parameters<typeof executeComposition>[0],
      ctx
    );
    expect(result.success).toBe(true);
  });
  ...
  it('returns success:false when any result fails (body path)', async () => {
    ...
    const ctx = makeCtx({
      executeProgram: vi.fn(async () =>
        call++ === 1 ? [{ success: false }] : [{ success: true }]
      ),
    });
    const result = await executeComposition(
      {
  ...
  it('returns success:true for flat children when all pass', async () => {
    const ctx = makeCtx({ executeProgram: vi.fn(async () => [{ success: true }]) });
    const result = await executeComposition(
      { type: 'composition', name: 'flat', children: [{}] } as unknown as Parameters<
        typeof executeComposition
      >[0],
      ctx
    );
...
describe('executeFocus', () => {
  it('pushes the focus target onto focusHistory', async () => {
    const ctx = makeCtx();
    await executeFocus(
      { type: 'focus', target: 'hero', body: [] } as unknown as Parameters<typeof executeFocus>[0],
      ctx
    );
  ...
  it('output mentions the focus target', async () => {
    const ctx = makeCtx();
    const result = await executeFocus(
      { type: 'focus', target: 'dragon', body: [] } as unknown as Parameters<
        typeof executeFocus
      >[0],
      ctx
    );
