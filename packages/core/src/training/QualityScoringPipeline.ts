...
export interface QualityDetail { metric: string; score: number; message: string; }

export interface QualityScoringConfig {
  syntaxWeight: number; schemaWeight: number; semanticWeight: number;
  minPassScore: number; knownTraits: Set<string>; knownTypes: Set<string>;
}

export const DEFAULT_SCORING_CONFIG: QualityScoringConfig = {
  syntaxWeight: 0.4, schemaWeight: 0.3, semanticWeight: 0.3,
  minPassScore: 0.7,
...
export class QualityScoringPipeline {
  ...
  score(source: string): QualityScore {
    ...
    return { syntaxValidity: syntax, schemaCompliance: schema, semanticCorrectness: semantic, compositeScore: composite, details };
  }
  passes(score: QualityScore): boolean { return score.compositeScore >= this.config.minPassScore; }

  private scoreSyntax(source: string, details: QualityDetail[]): number {
    let score = 1.0;
    if (!source.trim()) { details.push({ metric: 'syntax', score: 0, message: 'Empty source' }); return 0; }
    const open = (source.match(/{/g)||[]).length; const close = (source.match(/}/g)||[]).length;
    if (open !== close) { score -= 0.5; details.push({ metric: 'syntax', score: 0.5, message: `Unbalanced braces: ${open} open, ${close} close` }); }
    ...
    } else { score -= 0.3; details.push({ metric: 'syntax', score: 0.7, message: 'No recognized top-level declaration' }); }
    return Math.max(0, score);
  }

  private scoreSchema(source: string, details: QualityDetail[]): number {
    let score = 1.0;
    const traitPattern = /(\w+)\s*{/g; let match;
    while ((match = traitPattern.exec(source)) !== null) {
      ...
      if (/^[A-Z]/.test(name) && !this.config.knownTraits.has(name)) {
        score -= 0.1; details.push({ metric: 'schema', score: 0.9, message: `Unknown trait: ${name}` });
      }
    }
    return Math.max(0, score);
  }
  ...
  private scoreSemantic(source: string, details: QualityDetail[]): number {
    ...
    if (/physics\s*{/.test(source) && !/mass|gravity|friction/.test(source)) {
      score -= 0.2; details.push({ metric: 'semantic', score: 0.8, message: 'Physics block without mass/gravity/friction' });
    }
    if (/animation\s*{/.test(source) && !/clip|duration|speed/.test(source)) {
      score -= 0.2; details.push({ metric: 'semantic', score: 0.8, message: 'Animation block without clip/duration' });
    }
    if (source.length < 20) { score -= 0.3; details.push({ metric: 'semantic', score: 0.7, message: 'Source too short for meaningful composition' }); }
    return Math.max(0, score);
  }
}
