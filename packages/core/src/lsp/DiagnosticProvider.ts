...
const KNOWN_DIRECTIVES = new Set([
    ...
    'transparent', 'double_sided', 'unlit',
    // Interaction traits
    'grabbable', 'scrollable', 'keyboard',
]);
...
const RANGE_CONSTRAINTS: Record<string, Record<string, [number, number]>> = {
    material: { roughness: [0, 1], metallic: [0, 1], opacity: [0, 1], clearcoat: [0, 1], sheen: [0, 1], anisotropy: [0, 1] },
    ...
    audio_source: { volume: [0, 1], pitch: [0.5, 3], min_distance: [0, Infinity], max_distance: [0, Infinity], spread: [0, 360], doppler_level: [0, 5] },
    reverb_zone: { decay_time: [0.1, 20], room_size: [0, 1], diffusion: [0, 1], high_freq_damping: [0, 1] },
    force_field: { strength: [-Infinity, Infinity] },
    wind_zone: { turbulence: [0, 1] },
    buoyancy_zone: { density: [0, Infinity], drag: [0, Infinity] },
    navmesh: { agent_radius: [0.01, Infinity], agent_height: [0.1, Infinity], max_slope: [0, 90], step_height: [0, Infinity] },
...
const VALID_FORCE_FIELD_TYPES = new Set([
    'force_field', 'gravity_zone', 'wind_zone', 'buoyancy_zone', 'magnetic_field', 'drag_zone',
]);
    ...
    check(ctx: DiagnosticContext): Diagnostic[] {
        ...
        for (const node of ctx.nodes) {
            ...
            const hasRequiredProp =
                keyword === 'force_field' ? ('strength' in props || 'direction' in props) :
                keyword === 'gravity_zone' ? 'gravity' in props :
                keyword === 'wind_zone' ? ('direction' in props || 'strength' in props) :
                keyword === 'buoyancy_zone' ? 'density' in props :
                keyword === 'magnetic_field' ? 'strength' in props :
                keyword === 'drag_zone' ? ('linear_drag' in props || 'angular_drag' in props) :
                true;
            if (!hasRequiredProp) {
                diagnostics.push({
                    severity: 'info',
...
const VALID_JOINT_TYPES = new Set([
    'joint', 'hinge', 'slider', 'ball_socket', 'fixed_joint', 'd6_joint', 'spring_joint', 'prismatic',
]);
...
const jointValidation: DiagnosticRule = {
    ...
    check(ctx: DiagnosticContext): Diagnostic[] {
        ...
        for (const node of ctx.nodes) {
            ...
            if ((keyword === 'hinge' || keyword === 'slider' || keyword === 'prismatic') && !('axis' in props)) {
                diagnostics.push({
                    severity: 'warning',
...
const particleSystemValidation: DiagnosticRule = {
    ...
    check(ctx: DiagnosticContext): Diagnostic[] {
        ...
        for (const node of ctx.nodes) {
            if (node.type !== 'DomainBlock') continue;
            if (node.keyword !== 'particles' && node.keyword !== 'particle_system' && node.keyword !== 'emitter') continue;
            const props = node.properties || {};
...
const postProcessingValidation: DiagnosticRule = {
    ...
    check(ctx: DiagnosticContext): Diagnostic[] {
        ...
        for (const node of ctx.nodes) {
            if (node.type !== 'DomainBlock') continue;
            if (node.keyword !== 'post_processing' && node.keyword !== 'post_fx' && node.keyword !== 'render_pipeline') continue;
...
const weatherValidation: DiagnosticRule = {
    ...
    check(ctx: DiagnosticContext): Diagnostic[] {
        ...
        for (const node of ctx.nodes) {
            if (node.type !== 'DomainBlock') continue;
            if (node.keyword !== 'weather' && node.keyword !== 'climate' && node.keyword !== 'atmosphere') continue;
            if (node.subBlocks) {
                for (const sub of node.subBlocks) {
...
const KNOWN_INPUT_BINDINGS = new Set([
    'left_stick', 'right_stick', 'button_a', 'button_b', 'button_x', 'button_y',
    'trigger_left', 'trigger_right', 'grip_left', 'grip_right',
    'dpad_up', 'dpad_down', 'dpad_left', 'dpad_right',
    'button_menu', 'button_start', 'button_select',
