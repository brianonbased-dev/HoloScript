...
describe('Type Inference', () => {
  ...
  describe('function type inference', () => {
    test('JS function object â†’ function type', () => {
      const fn = () => {};
      const t = checker.inferType(fn as unknown as import('../types').HoloScriptValue);
      expect(t.type).toBe('function');
    });

    test('arrow function string â†’ function type', () => {
      const t = checker.inferType('() => {}');
      expect(t.type).toBe('function');
    });

    test('arrow function with number return â†’ function returning number', () => {
      const t = checker.inferType('() => 42');
      expect(t.type).toBe('function');
      expect(t.returnType).toBe('number');
    });

    test('arrow function with string return â†’ function returning string', () => {
      const t = checker.inferType('() => "hello"');
      expect(t.type).toBe('function');
      expect(t.returnType).toBe('string');
    });

    test('parameterized arrow function â†’ function type', () => {
