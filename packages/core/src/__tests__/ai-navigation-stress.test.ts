...
describe('NavMesh Stress Testing & Fallbacks', () => {
  it('should return null when navigating across disjointed navmesh islands', () => {
    ...
    // Island A
    const a1 = nav.addPolygon([{ x: 0, z: 0 }, { x: 10, z: 0 }, { x: 10, z: 10 }, { x: 0, z: 10 }]);
    const a2 = nav.addPolygon([{ x: 10, z: 0 }, { x: 20, z: 0 }, { x: 20, z: 10 }, { x: 10, z: 10 }]);
    nav.connect(a1, a2);

    // Island B (disjointed)
    const b1 = nav.addPolygon([{ x: 100, z: 0 }, { x: 110, z: 0 }, { x: 110, z: 10 }, { x: 100, z: 10 }]);
    const b2 = nav.addPolygon([{ x: 110, z: 0 }, { x: 120, z: 0 }, { x: 120, z: 10 }, { x: 110, z: 10 }]);
    nav.connect(b1, b2);
  ...
  it('should fall back gracefully when internal polygon is deleted or walled off', () => {
     const nav = new NavMesh();
     const a = nav.addPolygon([{ x: 0, z: 0 }, { x: 10, z: 0 }, { x: 10, z: 10 }, { x: 0, z: 10 }]);
     const b = nav.addPolygon([{ x: 10, z: 0 }, { x: 20, z: 0 }, { x: 20, z: 10 }, { x: 10, z: 10 }]);
     const c = nav.addPolygon([{ x: 20, z: 0 }, { x: 30, z: 0 }, { x: 30, z: 10 }, { x: 20, z: 10 }]);
