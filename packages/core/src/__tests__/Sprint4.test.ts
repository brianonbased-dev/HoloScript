...
describe('Sprint 4 Exhaustive Match', () => {
  ...
  test('complete union - no errors', () => {
    ...
    expect(checker.checkExhaustiveMatch({ typeName: 'P', coveredPatterns: ['a','b','c'] })).toHaveLength(0);
  });
  ...
  test('wildcard _ exhaustive', () => {
    ...
    expect(checker.checkExhaustiveMatch({ typeName: 'P', coveredPatterns: ['a','_'] })).toHaveLength(0);
  });
...
describe('Sprint 4 Parallel Parsing', () => {
  ...
  beforeEach(() => { parser = makeSeqParser(); });
  afterEach(async () => { await parser.shutdown(); });
  ...
  test('parses multiple files', async () => {
    const files = Array.from({ length: 4 }, (_, i) => ({ path: `s${i}.hsplus`, content: `orb "O${i}" {}` }));
...
describe('Sprint 4 HashCalculator', () => {
  ...
  test('64-char hex', () => { expect(h.hashContent('hello')).toMatch(/^[0-9a-f]{64}$/); });
  test('same input same hash', () => { expect(h.hashContent('abc')).toBe(h.hashContent('abc')); });
  test('different input different hash', () => { expect(h.hashContent('abc')).not.toBe(h.hashContent('xyz')); });
  test('hashObject deterministic', () => { expect(h.hashObject({a:1})).toBe(h.hashObject({a:1})); });
  test('hashFile null for missing', () => { expect(h.hashFile('/no/such/file.ts')).toBeNull(); });
});
...
describe('Sprint 4 SourceMapV2', () => {
  test('construct', () => { expect(new SourceMapV2()).toBeDefined(); });
  test('addSource returns 0', () => { expect(new SourceMapV2().addSource('a.ts')).toBe(0); });
  ...
  test('addMapping no throw', () => {
    ...
    expect(() => sm.addMapping({ line: 1, column: 0 }, { line: 1, column: 0 }, s, -1)).not.toThrow();
  });
  test('toJSON version 3', () => { expect((new SourceMapV2()).toJSON()).toMatchObject({ version: 3 }); });
  test('toString valid JSON', () => { expect(() => JSON.parse(new SourceMapV2().toString())).not.toThrow(); });
  test('toInlineComment has base64', () => { expect(new SourceMapV2().toInlineComment()).toContain('base64,'); });
...
describe('Sprint 4 TreemapGenerator', () => {
  ...
  test('empty no throw', () => { expect(() => g.generate([])).not.toThrow(); });
});
...
describe('Sprint 4 BundleAnalyzer', () => {
  ...
  test('totalSize correct', () => {
    expect(a.analyze(mk([['a.ts', 300], ['b.ts', 200]])).totalSize).toBe(500);
  });
