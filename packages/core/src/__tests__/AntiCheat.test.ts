...
describe('AntiCheat', () => {
  ...
  it('accepts valid position update', () => {
    ...
    const r = ac.validatePositionUpdate('p1', [1, 0, 0]);
  ...
  it('rejects teleport beyond threshold', () => {
    ac.registerPlayer('p1', [0, 0, 0]);
    const r = ac.validatePositionUpdate('p1', [100, 0, 0]);
  ...
  it('accumulates violations', () => {
    ac.registerPlayer('p1', [0, 0, 0]);
    ac.validatePositionUpdate('p1', [100, 0, 0]);
    ac.validatePositionUpdate('p1', [200, 0, 0]);
    expect(ac.getViolationCount('p1')).toBe(2);
  });

  it('auto-bans after banThreshold violations', () => {
    ac.registerPlayer('p1', [0, 0, 0]);
    for (let i = 0; i < 3; i++) ac.validatePositionUpdate('p1', [(i + 1) * 100, 0, 0]);
    expect(ac.isBanned('p1')).toBe(true);
  });
