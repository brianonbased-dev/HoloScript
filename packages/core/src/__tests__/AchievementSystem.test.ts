...
describe('AchievementSystem', () => {
  ...
  it('register creates locked achievement with 0 progress', () => {
    const a = sys.register({ id: 'a1', name: 'First Kill', description: '', icon: '🗡️', rarity: 'bronze', maxProgress: 1, hidden: false, category: 'combat' });
    expect(a.unlocked).toBe(false);
    expect(a.currentProgress).toBe(0);
  });

  it('addProgress increments and unlocks at max', () => {
    sys.register({ id: 'a1', name: 'A', description: '', icon: '', rarity: 'silver', maxProgress: 3, hidden: false, category: 'test' });
  ...
  it('addProgress clamps to maxProgress', () => {
    sys.register({ id: 'a1', name: 'A', description: '', icon: '', rarity: 'bronze', maxProgress: 5, hidden: false, category: 'test' });
    sys.addProgress('a1', 100);
    expect(sys.get('a1')!.currentProgress).toBe(5);
  });

  it('addProgress returns false if already unlocked', () => {
    sys.register({ id: 'a1', name: 'A', description: '', icon: '', rarity: 'bronze', maxProgress: 1, hidden: false, category: 'test' });
    sys.addProgress('a1', 1);
    expect(sys.addProgress('a1', 1)).toBe(false);
  });

  it('unlock immediately unlocks achievement', () => {
    sys.register({ id: 'a1', name: 'A', description: '', icon: '', rarity: 'gold', maxProgress: 100, hidden: false, category: 'test' });
  ...
  it('totalPoints accumulates rarity-based points', () => {
    sys.register({ id: 'a1', name: 'A', description: '', icon: '', rarity: 'bronze', maxProgress: 1, hidden: false, category: 'test' });
    sys.register({ id: 'a2', name: 'B', description: '', icon: '', rarity: 'gold', maxProgress: 1, hidden: false, category: 'test' });
  ...
  it('getCompletionPercent returns ratio', () => {
    sys.register({ id: 'a1', name: 'A', description: '', icon: '', rarity: 'bronze', maxProgress: 1, hidden: false, category: 'test' });
    sys.register({ id: 'a2', name: 'B', description: '', icon: '', rarity: 'bronze', maxProgress: 1, hidden: false, category: 'test' });
    sys.unlock('a1');
    expect(sys.getCompletionPercent()).toBeCloseTo(50);
  });

  it('getByCategory filters correctly', () => {
    sys.register({ id: 'a1', name: 'A', description: '', icon: '', rarity: 'bronze', maxProgress: 1, hidden: false, category: 'combat' });
    sys.register({ id: 'a2', name: 'B', description: '', icon: '', rarity: 'bronze', maxProgress: 1, hidden: false, category: 'explore' });
    expect(sys.getByCategory('combat')).toHaveLength(1);
  });

  it('getByRarity filters correctly', () => {
    sys.register({ id: 'a1', name: 'A', description: '', icon: '', rarity: 'diamond', maxProgress: 1, hidden: false, category: 'test' });
    sys.register({ id: 'a2', name: 'B', description: '', icon: '', rarity: 'bronze', maxProgress: 1, hidden: false, category: 'test' });
    expect(sys.getByRarity('diamond')).toHaveLength(1);
  });

  it('getProgress returns fractional progress', () => {
    sys.register({ id: 'a1', name: 'A', description: '', icon: '', rarity: 'bronze', maxProgress: 10, hidden: false, category: 'test' });
