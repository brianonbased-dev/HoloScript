...
describe('RagdollSystem', () => {
  ...
  beforeEach(() => { sys = new RagdollSystem(); });

  it('createHumanoid creates ragdoll with correct bone count', () => {
    const rd = sys.createHumanoid('human1', { x: 0, y: 10, z: 0 });
  ...
  it('bodies have capsule shapes with correct dimensions', () => {
    const rd = sys.createHumanoid('human1', { x: 0, y: 0, z: 0 });
  ...
  it('rootPosition is stored on the instance', () => {
    const rd = sys.createHumanoid('human1', { x: 5, y: 10, z: 15 });
    expect(rd.rootPosition).toEqual({ x: 5, y: 10, z: 15 });
  });
});
