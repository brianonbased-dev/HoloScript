/**
 * Sprint27.test.ts — AI + Behavior + Agents (v3.36.0)
 *
...
describe('Feature 2A: behavior/BehaviorTree nodes', () => {
  ...
  it('SequenceNode: all children succeed → success', () => {
  ...
  it('SequenceNode: first child fails → failure', () => {
    const seq = new SequenceNode([
  ...
  it('SelectorNode: first child succeeds → success', () => {
  ...
  it('SelectorNode: all children fail → failure', () => {
    const sel = new SelectorNode([
      new ActionNode('a', () => 'failure'),
      new ActionNode('b', () => 'failure'),
    ]);
...
describe('Feature 3B: ai/UtilityAI', () => {
  ...
  it('cooldown: scoreAction returns 0 when action is on cooldown', () => {
    ...
    action.lastExecuted = 8; // elapsed=2, cooldown=5 → still on cooldown
    ai.addAction(action);
...
describe('Feature 4A: ai/BehaviorTree (ai module)', () => {
  ...
  it('AISequenceNode with all success children → success', () => {
    const bt = new AiBT();
  ...
  it('AISelectorNode with first failure second success → success', () => {
    const bt = new AiBT();
