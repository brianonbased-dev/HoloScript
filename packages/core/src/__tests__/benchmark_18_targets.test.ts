...
describe('18-Target Compiler Benchmark', () => {
  ...
  it('Benchmarks all targets', () => {
    const compilerClasses = Object.keys(core).filter(
      (k) => k.endsWith('Compiler') && typeof (core as any)[k] === 'function'
    );
    ...
    for (const className of compilerClasses) {
      ...
      try {
        ...
        results.push({
          target: className,
          timeMs: -1,
          error: String(err.message || err).substring(0, 80),
        });
      }
    }
    ...
    for (const res of results) {
      if (res.timeMs >= 0) {
        console.log(
          `${res.target.padEnd(30)} | Time: ${res.timeMs.toFixed(2)}ms | Size: ${(res.outputSize / 1024).toFixed(2)} KB`
        );
      } else {
    ...
    if (worst && worst.timeMs > totalAllowed) {
      console.log(
        `CRITICAL BOTTLENECK: ${worst.target} took ${worst.timeMs.toFixed(2)}ms, exceeding the ${totalAllowed}ms SLA.`
      );
    } else if (worst) {
      console.log(
        `All successful targets met the compile budget. Worst was ${worst.target} at ${worst.timeMs.toFixed(2)}ms.`
      );
    }

    import('fs').then((fs) => {
      fs.writeFileSync(
        'C:/Users/Josep/.ai-ecosystem/scratch/benchmark_results.json',
        JSON.stringify({ worst, results }, null, 2)
      );
    });
