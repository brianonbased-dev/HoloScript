...
function cpuRefMatmul(A: Float32Array, B: Float32Array, M: number, N: number, K: number): Float32Array {
...
describe('HoloTorch GEMM parity (WGSL vs f64 CPU reference)', () => {
  it(
    'matches within fp32 tolerance across holo-arch shapes and emits a parity receipt',
    async () => {
      const device = await getDevice();
      if (!device) {
        // Honest skip: no adapter means nothing to prove parity against.
        console.warn('[holotorch-parity] no WebGPU adapter — skipping (GPU-less env; receipt not emitted)');
        return;
      }
      ...
      const rand = rng(1234);
      const perShape: {
        tag: string; M: number; N: number; K: number;
      ...
      for (const s of shapes) {
        ...
        perShape.push({ tag: s.tag, M: s.M, N: s.N, K: s.K, maxAbs, maxRefAbs, relToScale, allClose });
        console.warn(
      ...
      writeFileSync(join(outDir, 'gemm-parity.receipt.json'), `${JSON.stringify(receipt, null, 2)}\n`);
