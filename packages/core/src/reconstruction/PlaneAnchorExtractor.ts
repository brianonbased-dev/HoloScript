...
const normalize = (a: Vector3): Vector3 => { const l = len(a) || 1; return [a[0] / l, a[1] / l, a[2] / l]; };
...
function jacobiEigen3(m: number[]): { values: number[]; vectors: Vector3[] } {
  ...
  for (let iter = 0; iter < 50; iter++) {
    let p = 0, q = 1, mx = 0;
    for (const [i, j] of pairs) { const av = Math.abs(a[i * 3 + j]); if (av > mx) { mx = av; p = i; q = j; } }
    if (mx < 1e-14) break;
  ...
  const vectors: Vector3[] = [[v[0], v[3], v[6]], [v[1], v[4], v[7]], [v[2], v[5], v[8]]];
  const order = [0, 1, 2].sort((i, j) => values[i] - values[j]);
  return { values: order.map((i) => values[i]), vectors: order.map((i) => vectors[i]) };
}

interface RawPlane { normal: Vector3; centroid: Vector3; u: Vector3; v: Vector3; inliers: number[]; }
...
function fitPlane(cloud: PointCloud, inliers: number[]): RawPlane {
  let cx = 0, cy = 0, cz = 0;
  for (const idx of inliers) { cx += cloud.x[idx]; cy += cloud.y[idx]; cz += cloud.z[idx]; }
  const n = inliers.length, centroid: Vector3 = [cx / n, cy / n, cz / n];
  let c00 = 0, c01 = 0, c02 = 0, c11 = 0, c12 = 0, c22 = 0;
  for (const idx of inliers) {
    const dx = cloud.x[idx] - centroid[0], dy = cloud.y[idx] - centroid[1], dz = cloud.z[idx] - centroid[2];
  ...
  return { normal: normalize(vectors[0]), u: normalize(vectors[2]), v: normalize(vectors[1]), centroid, inliers };
}
...
function planeExtent(cloud: PointCloud, p: RawPlane): { width: number; height: number; center: Vector3; vertices: Array<[number, number, number]> } {
  const pu: number[] = [], pv: number[] = [];
  for (const idx of p.inliers) {
    const rel: Vector3 = [cloud.x[idx] - p.centroid[0], cloud.y[idx] - p.centroid[1], cloud.z[idx] - p.centroid[2]];
  ...
  const corner = (su: number, sv: number): [number, number, number] => [center[0] + p.u[0] * su + p.v[0] * sv, center[1] + p.u[1] * su + p.v[1] * sv, center[2] + p.u[2] * su + p.v[2] * sv];
  return { width, height, center, vertices: [corner(-hw, -hh), corner(hw, -hh), corner(hw, hh), corner(-hw, hh)] };
}
 ...
 */
export function extractPlaneAnchors(cloud: PointCloud, opts: ExtractPlaneAnchorsOpts = {}): PlaneAnchor[] {
  const N = cloud.N;
  ...
  for (let slot = 0; slot < maxPlanes && remaining.length >= minInliers; slot++) {
    let bestN: Vector3 | null = null, bestD = 0, bestCount = 0;
    for (let it = 0; it < ransacIters; it++) {
      const i0 = remaining[(rng() * remaining.length) | 0], i1 = remaining[(rng() * remaining.length) | 0], i2 = remaining[(rng() * remaining.length) | 0];
      if (i0 === i1 || i1 === i2 || i0 === i2) continue;
      const p0 = pointAt(cloud, i0);
      const nc = cross(sub(pointAt(cloud, i1), p0), sub(pointAt(cloud, i2), p0)), l = len(nc);
      if (l < 1e-9) continue;
      const n: Vector3 = [nc[0] / l, nc[1] / l, nc[2] / l], d = -dot(n, p0);
      let count = 0;
      for (let r = 0; r < remaining.length; r++) { const idx = remaining[r]; if (Math.abs(n[0] * cloud.x[idx] + n[1] * cloud.y[idx] + n[2] * cloud.z[idx] + d) < distThreshold) count++; }
      if (count > bestCount) { bestCount = count; bestN = n; bestD = d; }
    }
    if (!bestN || bestCount < minInliers) break;
    const inliers: number[] = [], rest: number[] = [];
    for (const idx of remaining) {
      if (Math.abs(bestN[0] * cloud.x[idx] + bestN[1] * cloud.y[idx] + bestN[2] * cloud.z[idx] + bestD) < distThreshold) inliers.push(idx);
      else rest.push(idx);
    }
  ...
  // averages out per-patch noise so walls read perpendicular and counters read parallel)
  {
    ...
    for (const p of planes) {
      ...
      if (Math.abs(d) >= horizontalDot) { const s = d >= 0 ? 1 : -1, w = p.inliers.length; ax += s * p.normal[0] * w; ay += s * p.normal[1] * w; az += s * p.normal[2] * w; }
    }
  ...
  const floorPlane = horizontals.reduce((a, b) => (b.inliers.length > a.inliers.length ? b : a), horizontals[0] ?? planes[0]);
  let sx = 0, sy = 0, sz = 0;
  for (let i = 0; i < N; i++) { sx += cloud.x[i]; sy += cloud.y[i]; sz += cloud.z[i]; }
  ...
  const ceilingH = horizontals.length ? Math.max(...horizontals.map((p) => dot(p.centroid, up))) : floorH;

  // --- Phase 4: classify, orient normals, compute extents → anchors ---
  const anchors: PlaneAnchor[] = planes.map((p, i) => {
    const nd = dot(p.normal, up), ndAbs = Math.abs(nd);
    ...
    if (ndAbs >= horizontalDot) {
      ...
      if (p === floorPlane) classification = 'floor'; // exactly one ground plane (the dominant horizontal)
  ...
  anchors.forEach((a, i) => { a.id = `anchor_${i}_${a.classification}`; });
  return anchors;
}
...
export function toPlaneDetectedEvents(anchors: PlaneAnchor[]): Array<{ type: 'plane_detected'; plane: DetectedPlane }> {
