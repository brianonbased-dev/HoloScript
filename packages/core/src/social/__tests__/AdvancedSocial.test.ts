...
describe('Advanced Social Features', () => {
    ...
    beforeEach(() => {
        ...
        graph.updateUser({ id: 'alice', username: 'alice', displayName: 'Alice', status: 'online', lastSeen: 0 });
        
        transport = new MockTransport();
        friendManager = new FriendManager(graph, transport as unknown as WebRTCTransport);
        conversationManager = new ConversationManager(graph, friendManager, transport as unknown as WebRTCTransport);
    ...
    it('should block DMs from blocked users', () => {
        // Block bob
        graph.updateUser({ id: 'bob', username: 'bob', displayName: 'Bob', status: 'online', lastSeen: 0 });
