...
export class StateSynchronizer {
        ...
        this.transport.onInboundState = (payload: any) => {
            if (payload && payload.agent_updates && Array.isArray(payload.agent_updates)) {
                ...
                for (const update of payload.agent_updates) {
                    ...
                    inboundDeltas.push({ entityId: update.id, field: 'x', newValue: update.x, oldValue: 0, timestamp: payload.timestamp, authoritative: true });
                    inboundDeltas.push({ entityId: update.id, field: 'y', newValue: update.y, oldValue: 0, timestamp: payload.timestamp, authoritative: true });
                    inboundDeltas.push({ entityId: update.id, field: 'z', newValue: update.z, oldValue: 0, timestamp: payload.timestamp, authoritative: true });
                }
    ...
    private flushDeltas() {
        ...
        this.globalSubscribers.forEach(sub => {
            ...
            const targetShards = new Set(SpatialSharder.getOverlappingShards(
                options.position.x,
                options.position.y,
                options.position.z,
