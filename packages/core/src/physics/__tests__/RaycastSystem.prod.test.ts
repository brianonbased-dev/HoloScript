...
import { RaycastSystem, type Ray } from '../RaycastSystem';
...
describe('RaycastSystem — Production', () => {
  ...
  it('addCollider/removeCollider/getColliderCount', () => {
    ...
    rc.addCollider({ entityId: 'a', type: 'sphere', shape: { center: { x: 0, y: 0, z: 0 }, radius: 1 }, layer: 1 });
  ...
  it('raycast hits sphere', () => {
    const rc = new RaycastSystem();
    rc.addCollider({ entityId: 's1', type: 'sphere', shape: { center: { x: 0, y: 0, z: 5 }, radius: 1 }, layer: 1 });
  ...
  it('ray misses sphere not in path', () => {
    const rc = new RaycastSystem();
    rc.addCollider({ entityId: 's1', type: 'sphere', shape: { center: { x: 100, y: 0, z: 5 }, radius: 1 }, layer: 1 });
  ...
  it('raycast hits AABB', () => {
    const rc = new RaycastSystem();
    rc.addCollider({
      entityId: 'box', type: 'aabb',
      shape: { min: { x: -1, y: -1, z: 4 }, max: { x: 1, y: 1, z: 6 } }, layer: 1,
    });
    const hit = rc.raycast(RAY_Z);
    expect(hit).not.toBeNull();
  ...
  it('raycast hits plane', () => {
    const rc = new RaycastSystem();
    rc.addCollider({
      entityId: 'floor', type: 'plane',
      shape: { normal: { x: 0, y: 0, z: -1 }, distance: -5 }, layer: 1,
    });
    const hit = rc.raycast(RAY_Z);
    expect(hit).not.toBeNull();
  ...
  it('raycastAll returns hits sorted by distance', () => {
    const rc = new RaycastSystem();
    rc.addCollider({ entityId: 'far', type: 'sphere', shape: { center: { x: 0, y: 0, z: 10 }, radius: 1 }, layer: 1 });
    rc.addCollider({ entityId: 'near', type: 'sphere', shape: { center: { x: 0, y: 0, z: 2 }, radius: 1 }, layer: 1 });
  ...
  it('maxDistance filters far hits', () => {
    const rc = new RaycastSystem();
    rc.addCollider({ entityId: 's1', type: 'sphere', shape: { center: { x: 0, y: 0, z: 100 }, radius: 1 }, layer: 1 });
  ...
  it('layer mask filters colliders', () => {
    const rc = new RaycastSystem();
    rc.addCollider({ entityId: 's1', type: 'sphere', shape: { center: { x: 0, y: 0, z: 5 }, radius: 1 }, layer: 2 });
