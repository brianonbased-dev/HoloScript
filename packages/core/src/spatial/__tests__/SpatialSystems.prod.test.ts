...
describe('TransformGraph', () => {
  ...
  beforeEach(() => { g = new TransformGraph(); });
  ...
  it('setParent re-parents correctly', () => {
    g.addNode('p1'); g.addNode('p2'); g.addNode('child');
  ...
  it('setPosition marks dirty and updates world position', () => {
    ...
    const wp = g.getWorldPosition('A');
    expect(wp!.x).toBe(7); expect(wp!.y).toBe(8); expect(wp!.z).toBe(9);
  });
...
describe('SpatialQueryExecutor — raycast', () => {
  ...
  beforeEach(() => {
    exec = new SpatialQueryExecutor();
    exec.updateEntities([
      makeEntity('target', 5, 0, 0),
      makeEntity('side', 0, 5, 0),
    ]);
  });
...
describe('SpatialQueryExecutor — entity type filter', () => {
  ...
  beforeEach(() => {
    exec = new SpatialQueryExecutor();
    exec.updateEntities([
      makeEntity('e', 1, 0, 0, 'enemy'),
      makeEntity('a', 2, 0, 0, 'ally'),
    ]);
  });
