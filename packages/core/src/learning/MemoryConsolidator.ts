...
export class MemoryConsolidator {
     ...
     */
    static compressEpisodes(
        rawEpisodes: EpisodicMemory[]
    ): { newFacts: SemanticFact[], prunedEpisodes: string[] } {
        if (rawEpisodes.length < 5) return { newFacts: [], prunedEpisodes: [] };

        logger.info(`[Consolidation] Running idle memory compression on ${rawEpisodes.length} episodes.`);
        ...
        logger.info(`[Consolidation] Distilled ${prunedEpisodes.length} raw episodes into ${newFacts.length} semantic facts.`);
