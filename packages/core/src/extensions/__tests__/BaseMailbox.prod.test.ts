...
function makeBox<T = string>(maxSize?: number) {
  return new BaseMailbox<T>({ maxSize });
}

describe('BaseMailbox — construction', () => {
  it('starts empty', () => {
    expect(makeBox().isEmpty()).toBe(true);
  });
  it('size() returns 0 initially', () => {
    expect(makeBox().size()).toBe(0);
  });
  it('peek() returns undefined initially', () => {
    expect(makeBox().peek()).toBeUndefined();
  });
  it('dequeue() returns undefined when empty', () => {
    expect(makeBox().dequeue()).toBeUndefined();
  });
});
...
describe('BaseMailbox — enqueue / dequeue', () => {
  ...
  it('FIFO within same priority', () => {
    const b = makeBox();
    b.enqueue('first', 'normal');
    b.enqueue('second', 'normal');
    expect(b.dequeue()).toBe('first');
    expect(b.dequeue()).toBe('second');
  });
});
...
describe('BaseMailbox — priority ordering', () => {
  it('high-priority message dequeued before normal', () => {
    const b = makeBox();
    b.enqueue('normal', 'normal');
    b.enqueue('urgent', 'high');
