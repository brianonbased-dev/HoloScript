...
describe('AudioEnvelope — Production', () => {
  ...
  it('exponential curve produces different level than linear at midpoint', () => {
    const linear = new AudioEnvelope({ attack: 1, decay: 0.001, sustain: 1, release: 0.001 }, 'linear');
    const expo   = new AudioEnvelope({ attack: 1, decay: 0.001, sustain: 1, release: 0.001 }, 'exponential');
    linear.noteOn(); expo.noteOn();
    linear.process(0.5); expo.process(0.5);
  ...
  it('logarithmic curve produces higher level than linear at midpoint', () => {
    const linear = new AudioEnvelope({ attack: 1, decay: 0.001, sustain: 1, release: 0.001 }, 'linear');
    const log    = new AudioEnvelope({ attack: 1, decay: 0.001, sustain: 1, release: 0.001 }, 'logarithmic');
    linear.noteOn(); log.noteOn();
    linear.process(0.25); log.process(0.25);
