...
describe('NodeGraphPanel', () => {
  ...
  it('generates port entities for inputs and outputs', () => {
    const node = graph.addNode('MathAdd', { x: 0, y: 0 });
    const entities = panel.generateUI();
    const inputPorts = entities.filter(e => e.data?.role === 'input_port' && e.data?.nodeId === node.id);
    const outputPorts = entities.filter(e => e.data?.role === 'output_port' && e.data?.nodeId === node.id);
