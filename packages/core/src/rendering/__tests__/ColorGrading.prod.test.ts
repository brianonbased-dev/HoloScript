...
function makeCG() { return new ColorGrading(); }

describe('ColorGrading — defaults', () => {
  it('defaults tonemapper=aces', () => { expect(makeCG().getConfig().tonemapper).toBe('aces'); });
  it('defaults exposure=0', () => { expect(makeCG().getConfig().exposure).toBe(0); });
  it('defaults contrast=1, saturation=1, gamma=1', () => {
    const cfg = makeCG().getConfig();
    expect(cfg.contrast).toBe(1); expect(cfg.saturation).toBe(1); expect(cfg.gamma).toBe(1);
  });
  it('enabled=true by default', () => { expect(makeCG().getConfig().enabled).toBe(true); });
});

describe('ColorGrading — setters with clamping', () => {
  it('setTonemapper changes tonemapper', () => {
    const cg = makeCG(); cg.setTonemapper('reinhard');
  ...
  it('all tonemap operators accepted', () => {
    ...
    const cg = makeCG();
    for (const op of ops) { cg.setTonemapper(op); expect(cg.getConfig().tonemapper).toBe(op); }
  });
  it('setExposure stores value', () => {
    const cg = makeCG(); cg.setExposure(1.5); expect(cg.getConfig().exposure).toBe(1.5);
  });
  it('setContrast clamps to [0, 2]', () => {
    const cg = makeCG();
    cg.setContrast(-1); expect(cg.getConfig().contrast).toBe(0);
    cg.setContrast(5);  expect(cg.getConfig().contrast).toBe(2);
    cg.setContrast(1.2); expect(cg.getConfig().contrast).toBeCloseTo(1.2);
  });
  it('setSaturation clamps to [0, 2]', () => {
    const cg = makeCG();
    cg.setSaturation(-0.5); expect(cg.getConfig().saturation).toBe(0);
    cg.setSaturation(3);    expect(cg.getConfig().saturation).toBe(2);
  });
  it('setTemperature clamps to [-1, 1]', () => {
    const cg = makeCG();
    cg.setTemperature(-2); expect(cg.getConfig().temperature).toBe(-1);
    cg.setTemperature(2);  expect(cg.getConfig().temperature).toBe(1);
  });
  it('setTint clamps to [-1, 1]', () => {
    const cg = makeCG();
    cg.setTint(-5); expect(cg.getConfig().tint).toBe(-1);
    cg.setTint(5);  expect(cg.getConfig().tint).toBe(1);
  });
  it('setGamma clamps to [0.1, 3]', () => {
    const cg = makeCG();
    cg.setGamma(0); expect(cg.getConfig().gamma).toBe(0.1);
    cg.setGamma(5); expect(cg.getConfig().gamma).toBe(3);
    cg.setGamma(2.2); expect(cg.getConfig().gamma).toBeCloseTo(2.2);
  });
  it('setEnabled toggles', () => {
    const cg = makeCG(); cg.setEnabled(false); expect(cg.getConfig().enabled).toBe(false);
  });
  it('getConfig returns a copy', () => {
    const cg = makeCG(); const cfg = cg.getConfig(); cfg.exposure = 99;
...
describe('ColorGrading — tonemap', () => {
  it('reinhard: clamps output to [0,1] for HDR inputs', () => {
    ...
    expect(r).toBeLessThanOrEqual(1); expect(g).toBeLessThanOrEqual(1); expect(b).toBeLessThanOrEqual(1);
  });
  it('aces: output in [0, 1]', () => {
    const cg = makeCG(); cg.setTonemapper('aces');
    const [r, g, b] = cg.tonemap(2, 1, 0.5);
    expect(r).toBeGreaterThanOrEqual(0); expect(r).toBeLessThanOrEqual(1);
    expect(g).toBeGreaterThanOrEqual(0); expect(b).toBeGreaterThanOrEqual(0);
  });
  it('filmic: output in [0, 1]', () => {
    const cg = makeCG(); cg.setTonemapper('filmic');
    const [r, g, b] = cg.tonemap(3, 2, 1);
    expect(r).toBeLessThanOrEqual(1); expect(g).toBeLessThanOrEqual(1); expect(b).toBeLessThanOrEqual(1);
  });
  it('uncharted2: output in [0, 1]', () => {
    const cg = makeCG(); cg.setTonemapper('uncharted2');
    const [r, g, b] = cg.tonemap(4, 2, 1);
    expect(r).toBeLessThanOrEqual(1); expect(g).toBeLessThanOrEqual(1); expect(b).toBeLessThanOrEqual(1);
  });
  it('none (linear): caps at 1, passes through negatives', () => {
    const cg = makeCG(); cg.setTonemapper('none');
...
describe('ColorGrading — apply (full pipeline)', () => {
  it('returns input unchanged when disabled', () => {
    const cg = makeCG(); cg.setEnabled(false);
  ...
  it('output values all in [0,1]', () => {
    const cg = makeCG(); cg.setExposure(3); cg.setContrast(2);
