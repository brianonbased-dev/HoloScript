...
export class BabylonCompiler extends CompilerBase {
  ...
  private emitObject(obj: HoloObjectDecl, parentVar?: string): void {
    ...
    for (const trait of obj.traits || []) {
      if (trait.name === 'grabbable') {
        ...
        this.emit(`// @object_tracking — WebXR hit-test or custom tracker for "${this.escapeStringValue(obj.name as string, 'TypeScript')}"`);
      } else if (trait.name === 'scene_reconstruction') {
        this.emit(`// @scene_reconstruction — WebXR mesh detection for "${this.escapeStringValue(obj.name as string, 'TypeScript')}"`);
        ...
        this.emit(`// @${this.escapeStringValue(trait.name as string, 'TypeScript')}: ${JSON.stringify(trait.config || {})}`);
      }
    }
  ...
  private emitGroup(group: HoloSpatialGroup, parentVar?: string): void {
    ...
    this.emit(`const ${v} = new BABYLON.TransformNode("${this.escapeStringValue(group.name as string, 'TypeScript')}", this.scene);`);
  ...
  private emitLight(light: HoloLight): void {
    ...
    switch (light.lightType) {
      ...
      case 'directional':
        ...
        break;
      case 'point':
        this.emit(`const ${v} = new BABYLON.PointLight("${this.escapeStringValue(light.name as string, 'TypeScript')}", ${posV}, this.scene);`);
        break;
      case 'spot':
        this.emit(
  ...
  private emitTimeline(timeline: HoloTimeline): void {
    ...
    this.emit(`const ${v}Group = new BABYLON.AnimationGroup("${this.escapeStringValue(timeline.name as string, 'TypeScript')}", this.scene);`);
    const fps = 30;
    for (const entry of timeline.entries) {
      ...
      if (a.kind === 'animate') {
        ...
        this.emit(`// @${entry.time}s: emit "${this.escapeStringValue(a.event as string, 'TypeScript')}"`);
      } else if (a.kind === 'call') {
        this.emit(`// @${entry.time}s: call ${this.escapeStringValue(a.method as string, 'TypeScript')}()`);
      }
    }
  ...
  private emitUI(ui: HoloUI): void {
    ...
    for (const el of ui.elements) {
      ...
      if (elType === 'button') {
        this.emit(`const ${v} = GUI.Button.CreateSimpleButton("${this.escapeStringValue(el.name as string, 'TypeScript')}", "${text}");`);
        this.emit(
        ...
        this.emit(`const ${v} = new GUI.StackPanel("${this.escapeStringValue(el.name as string, 'TypeScript')}");`);
        this.emit(`${v}.width = "200px"; ${v}.height = "20px"; advancedTexture.addControl(${v});`);
      } else {
        this.emit(`const ${v} = new GUI.TextBlock("${this.escapeStringValue(el.name as string, 'TypeScript')}", "${text}");`);
        this.emit(
