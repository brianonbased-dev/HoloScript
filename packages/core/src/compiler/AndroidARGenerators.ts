...
//
// Wave 1 (this code) is the BASE render path. Wave 2 feature traits now emit
// optional SceneView sidecar files (geo-anchor, geospatial VPS, depth scan,
// portal AR, hand tracking, authoring, haptic, ...), so the base activity stays
...
export function generateActivityFile(
  ...
  if (objects.length) {
    ...
    compiler.emit(
      `materialInstance = materialLoader.createColorInstance(${DEFAULT_COMPOSE_COLOR}),`
    );
...
export function generateBuildGradle(
  ...
  if (hasGeoTraits(composition) || hasGeospatialVPSTraits(composition)) {
    optionalDependencies.add(
      'implementation("com.google.android.gms:play-services-location:21.3.0")'
    );
  }
  if (hasHandTrackingTraits(composition)) {
    optionalDependencies.add('implementation("androidx.camera:camera-core:1.4.1")');
    optionalDependencies.add('implementation("androidx.camera:camera-camera2:1.4.1")');
    optionalDependencies.add('implementation("androidx.camera:camera-lifecycle:1.4.1")');
    optionalDependencies.add('implementation("com.google.mediapipe:tasks-vision:0.10.20")');
  }
  const optionalDependencyBlock =
    optionalDependencies.size > 0
      ? `\n\n    // Optional SceneView feature-trait sidecars\n    ${[...optionalDependencies].join('\n    ')}`
      : '';
...
dependencies {
    ...
    implementation("io.github.sceneview:arsceneview:4.18.0")${optionalDependencyBlock}
}`;
}
