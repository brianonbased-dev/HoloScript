 ...
 *
 * @version 1.3.0 (Phase 2(b) - markdown_ssot + ts_adapter_capabilities +
...
const ADAPTER_CAPABILITY_FIELD_ORDER = [
  'contextWindow',
  'maxOutput',
  'costPerMillion',
  'streaming',
  ...
  'evalsFirstParty',
  'local',
  'zeroMarginalInference',
  'bedrockAvailable',
  'vertexAvailable',
  'bearerTokenAccess',
] as const;

const ADAPTER_OPTIONAL_BOOLEAN_FIELDS = ADAPTER_CAPABILITY_FIELD_ORDER.filter(
  (field) =>
    ![
      'contextWindow',
      'maxOutput',
...
export class LLMProviderCapabilitiesCompiler extends CompilerBase {
   ...
   *   3. emit     - dispatch to per-format emitters
   */
  compile(
    ...
    for (const format of this.options.formats) {
      switch (format) {
        ...
        case 'ts_adapter_capabilities':
          Object.assign(files, this.emitTsAdapterCapabilities(ast));
          break;
  ...
  /**
   * Emit paste-ready adapter capability constants. Each output entry is keyed
   * by the adapter file basename plus `.capabilities`, matching the board-task
   * contract (e.g. `anthropic.ts.capabilities`).
   */
  private emitTsAdapterCapabilities(ast: LLMCapabilityMatrixAST): Record<string, string> {
    const files: Record<string, string> = {};

    for (const provider of ast.providers) {
      const providerModels = ast.models.filter((model) => model.provider === provider.name);
      const providerCapabilities = ast.capabilities.filter(
        (capability) => capability.provider === provider.name
      );
      const entries = buildAdapterCapabilities(providerModels, providerCapabilities);
      const lines: string[] = [];

      lines.push('/**');
      lines.push(' * Generated by HoloScript LLMProviderCapabilitiesCompiler');
      lines.push(' * target: compile_to_ts_adapter_capabilities');
      lines.push(` * provider: ${provider.name}`);
      lines.push(' * Paste into the matching packages/llm-provider adapter.');
      lines.push(' */');
      lines.push(
        `export const ${toProviderConstName(provider.name)}_CAPABILITIES: Capabilities = {`
      );
      for (const field of ADAPTER_CAPABILITY_FIELD_ORDER) {
        if (!entries.has(field)) continue;
        lines.push(`  ${field}: ${formatTsCapabilityValue(entries.get(field))},`);
      }
      lines.push('};');
      lines.push('');

      files[`${provider.name}.ts.capabilities`] = lines.join('\n');
    }
...
function buildAdapterCapabilities(
  models: LLMModel[],
  capabilities: LLMCapability[]
): Map<AdapterCapabilityField, TsCapabilityValue> {
  ...
  if (costPerMillion) {
    entries.set('costPerMillion', costPerMillion);
  }

  entries.set('streaming', booleanCapability(byName, 'streaming') ?? true);
  entries.set('tools', booleanCapability(byName, 'tools') ?? true);

  const highResVision = booleanCapability(byName, 'highResVision');
  entries.set('vision', booleanCapability(byName, 'vision') ?? highResVision ?? false);

  for (const field of ADAPTER_OPTIONAL_BOOLEAN_FIELDS) {
    const value = booleanCapability(byName, field);
    if (value !== undefined) {
      entries.set(field, value);
    }
  }

  entries.set('bearerTokenAccess', booleanCapability(byName, 'bearerTokenAccess') ?? true);
  return entries;
}
...
function uniformCostPerMillion(
  ...
  const pricedModels = models.filter(
    (model) =>
      ...
      Number.isFinite(model.outputPerMTok) &&
      (model.inputPerMTok > 0 || model.outputPerMTok > 0)
  );
  if (pricedModels.length === 0) return undefined;
...
function formatTsCapabilityValue(value: TsCapabilityValue | undefined): string {
  ...
  if (typeof value === 'number') return formatTsNumber(value);
  if (typeof value === 'string') return tsStringLiteral(value);
  return `{ input: ${formatTsNumber(value.input)}, output: ${formatTsNumber(value.output)} }`;
}
