...
export class ContextCompiler extends CompilerBase {
    ...
    for (const format of this.options.formats) {
      switch (format) {
        ...
        case 'anthropic_system_prompt':
          files['anthropic.system.txt'] = this.emitAnthropicSystemPrompt(ast);
          break;
        case 'brain_includes':
          files['context.includes.hsplus'] = this.emitBrainIncludes(ast);
          break;
        case 'mcp_context_loader':
          files['mcp-context-loader.json'] = this.emitMcpContextLoader(ast);
          break;
        default: {
  ...
  private emitSkillMd(ast: ContextAST): string {
    ...
    // Domain → execution skill dispatch (vocabulary v4 - 2026-05-22)
    ...
    // Corpus mutability (vocabulary v4 - 2026-05-22)
    ...
    lines.push(
      `*Generated by HoloScript ContextCompiler (compile_to_skill_md). ` +
        `Source: HoloScript composition. Vocabulary: v4 (2026-05-22). ` +
  ...
  // --- Phase 3: emit -- compile_to_anthropic_system_prompt ----------

  /**
   * Emit compact raw text for Anthropic SDK `messages.create({ system })`.
   * This surface is runtime-only: refusals + hard-donts, not skill
   * discovery, markdown output protocols, or registration metadata.
   */
  private emitAnthropicSystemPrompt(ast: ContextAST): string {
    const lines: string[] = [];

    if (ast.refusals.length === 0 && ast.hardDonts.length === 0) {
      lines.push('HoloScript runtime context: no active refusals or hard-donts declared.');
      return lines.join('\n');
    }

    lines.push('HoloScript runtime context. Follow these rules before answering or acting.');

    if (ast.refusals.length > 0) {
      lines.push('');
      lines.push('Refusals:');
      for (const refusal of ast.refusals) {
        const doNot = refusal.doNot.length > 0 ? ` Do not: ${refusal.doNot.join(', ')}.` : '';
        const reason = refusal.reason ? ` Reason: ${refusal.reason}.` : '';
        lines.push(`- ${refusal.name}: when ${refusal.when}; do ${refusal.do}.${doNot}${reason}`);
      }
    }

    if (ast.hardDonts.length > 0) {
      lines.push('');
      lines.push("Hard don'ts:");
      for (const dont of ast.hardDonts) {
        const alternative = dont.alternative ? ` Alternative: ${dont.alternative}.` : '';
        const appliesTo =
          dont.appliesTo.length > 0 ? ` Applies to: ${dont.appliesTo.join(', ')}.` : '';
        lines.push(`- ${dont.name}: ${dont.reason}.${alternative}${appliesTo}`);
      }
    }
  ...
  // --- Phase 3: emit -- compile_to_brain_includes -------------------

  /**
   * Emit a `.hsplus` context fragment suitable for brain `@include`
   * directives. The fragment is deliberately data-shaped so brain
   * compositions can consume the same authority/rule payload without
   * importing Markdown surface conventions.
   */
  private emitBrainIncludes(ast: ContextAST): string {
    const lines: string[] = [];
    lines.push('// Generated by HoloScript ContextCompiler (compile_to_brain_includes).');
    lines.push('// Include this fragment from brain compositions that need shared context.');
    lines.push('@context_include {');

    if (ast.identity) {
      lines.push(`  name: ${hsLiteral(ast.identity.name)}`);
      lines.push(`  role: ${hsLiteral(ast.identity.role)}`);
      lines.push(`  domain: ${hsLiteral(ast.identity.domain)}`);
      lines.push(`  surface: ${hsLiteral(ast.identity.surface)}`);
      lines.push(`  no_monopoly: ${ast.identity.noMonopoly ? 'true' : 'false'}`);
    }

    lines.push(`  authority_order: ${hsLiteral(ast.authorityOrder?.tiers ?? [])}`);
    lines.push(
      `  refusals: ${hsLiteral(
        ast.refusals.map((refusal) => ({
          name: refusal.name,
          when: refusal.when,
          do: refusal.do,
          do_not: refusal.doNot,
          reason: refusal.reason ?? null,
        }))
      )}`
    );
    lines.push(
      `  hard_donts: ${hsLiteral(
        ast.hardDonts.map((dont) => ({
          name: dont.name,
          reason: dont.reason,
          alternative: dont.alternative ?? null,
          applies_to: dont.appliesTo,
        }))
      )}`
    );
    lines.push(
      `  skills: ${hsLiteral(
        ast.skills.map((skill) => ({
          name: skill.name,
          invocable_as: skill.invocableAs,
          authority: skill.authority,
          authoritative_for: skill.authoritativeFor,
          refusals: skill.refusals ?? [],
        }))
      )}`
    );
    lines.push('}');
  ...
  // --- Phase 3: emit -- compile_to_mcp_context_loader ---------------

  /**
   * Emit a JSON manifest for session-start MCP/context loaders.
   * Consumers get authority order, active runtime rules, and the skill
   * registry without parsing Markdown.
   */
  private emitMcpContextLoader(ast: ContextAST): string {
    const manifest = {
      schema_version: 'holoscript.context-loader.v1',
      generated_by: 'ContextCompiler',
      identity: ast.identity
        ? {
            name: ast.identity.name,
            role: ast.identity.role,
            domain: ast.identity.domain,
            surface: ast.identity.surface,
            no_monopoly: ast.identity.noMonopoly,
          }
        : null,
      authority_order: ast.authorityOrder?.tiers ?? [],
      active_rules: {
        refusals: ast.refusals.map((refusal) => ({
          name: refusal.name,
          when: refusal.when,
          do: refusal.do,
          do_not: refusal.doNot,
        ...
        hard_donts: ast.hardDonts.map((dont) => ({
          name: dont.name,
          reason: dont.reason,
          alternative: dont.alternative ?? null,
          applies_to: dont.appliesTo,
        })),
        defaults: ast.defaults.map((def) => ({
          name: def.name,
          when: def.when,
          do: def.do,
          reason: def.reason ?? null,
        })),
        gap_rules: ast.gapRules.map((rule) => ({
          name: rule.name,
          when: rule.when,
          do: rule.do,
          do_not: rule.doNot,
        ...
        citation_rules: ast.citationRules.map((rule) => ({
          fluent_prose_threshold_chars: rule.fluentProseThresholdChars,
          required: rule.required,
          exemption: rule.exemption ?? null,
          reason: rule.reason ?? null,
        })),
        production_rule: ast.productionRule
          ? {
              no_dev_no_mock_no_localhost: ast.productionRule.noDevNoMockNoLocalhost,
              exception: ast.productionRule.exception ?? null,
            }
          : null,
        hard_physical_gaps: ast.hardPhysicalGaps.map((gap) => ({
          name: gap.name,
          reason: gap.reason,
          applies_to: gap.appliesTo,
          alternative: gap.alternative ?? null,
        })),
      },
      skill_registry: ast.skills.map((skill) => ({
        name: skill.name,
        invocable_as: skill.invocableAs,
        authority: skill.authority,
        authoritative_for: skill.authoritativeFor,
      ...
      includes: ast.includes.map((include) => ({
        source: include.source,
        selector: include.selector ?? null,
      })),
    };
...
function hsLiteral(value: unknown): string {
