...
export interface AndroidXRCompilerOptions {
  ...
  targetSdk?: number;
  /**
   * When compositions use ARCore depth / environment probe traits (`occlusion_mesh`,
   * `environment_probe`), emit `<uses-feature android:name="android.hardware.camera.ar" />`.
   * Default `false` keeps the feature optional so APKs install on non-AR hardware; set `true`
   * for Quest-only / Play Store builds that must declare hard AR hardware requirement.
   */
...
export class AndroidXRCompiler extends CompilerBase {
  ...
  /** Traits that rely on ARCore depth / perception probes — manifest should declare camera.ar when useARCore. */
  private compositionUsesArCoreDepthTraits(composition: HoloComposition): boolean {
  ...
  private generateManifestFile(composition: HoloComposition): string {
    ...
    const needsArCameraFeature =
      this.options.useARCore && this.compositionUsesArCoreDepthTraits(composition);
    const arRequired = this.options.arCameraHardwareRequired ? 'true' : 'false';
