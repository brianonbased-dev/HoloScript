...
export class AndroidXRCompiler extends CompilerBase {
  ...
  private generateActivityFile(composition: HoloComposition): string {
    ...
    this.emit(`// Source: composition "${this.escapeStringValue(composition.name as string, 'TypeScript')}"`);
  ...
  private generateStateFile(composition: HoloComposition): string {
    ...
    this.emit('// Auto-generated by HoloScript AndroidXRCompiler');
    this.emit(`// State: ${this.escapeStringValue(composition.name as string, 'TypeScript')}`);
    this.emit('');
    ...
    if (composition.state) {
      ...
      for (const prop of composition.state.properties) {
        ...
        this.emit(`private val _${this.escapeStringValue(prop.key as string, 'TypeScript')} = MutableLiveData(${kotlinValue})`);
        this.emit(`val ${this.escapeStringValue(prop.key as string, 'TypeScript')}: LiveData<${kotlinType}> get() = _${this.escapeStringValue(prop.key as string, 'TypeScript')}`);
        this.emit('');
      }
    }
    ...
    if (composition.state) {
      for (const prop of composition.state.properties) {
        const kotlinValue = this.toKotlinValue(prop.value);
        this.emit(`_${this.escapeStringValue(prop.key as string, 'TypeScript')}.value = ${kotlinValue}`);
      }
    }
  ...
  private compileAction(action: any): void {
    ...
    this.emit(`fun ${name}() {`);
    this.indent();
    this.emit(`android.util.Log.d("HoloScript", "Action: ${this.escapeStringValue(action.name as string, 'TypeScript')}")`);
  ...
  private generateNodeFactoryFile(composition: HoloComposition): string {
    ...
    this.emit('// Auto-generated by HoloScript AndroidXRCompiler');
    this.emit(`// Node Factory: ${this.escapeStringValue(composition.name as string, 'TypeScript')}`);
    this.emit('');
    this.emit(`package ${pkg}`);
    this.emit('');
    ...
    if (composition.objects?.length) {
      ...
      if (modelSrc) {
        ...
        this.emit(`val entity = session.scene.createEntity("${this.escapeStringValue(firstObj.name as string, 'TypeScript')}")`);
  ...
  private compileObjectFactory(obj: HoloObjectDecl): void {
    ...
    if (modelSrc) {
      ...
      this.emit(`val entity = session.scene.createEntity("${this.escapeStringValue(obj.name as string, 'TypeScript')}")`);
      this.emit('entity.parent = session.scene.activitySpace');
      const pos = this.findProp(obj, 'position');
      if (pos && Array.isArray(pos)) {
        this.emit(
...
${xrPermissions.join('\n')}
    ...
    <application
        ...
        android:label="${this.escapeStringValue(composition.name as string, 'TypeScript')}"
...
// Source: ${this.escapeStringValue(composition.name as string, 'TypeScript')}
...
`
  ...
  private emitSceneComposable(composition: HoloComposition): void {
    ...
    if (composition.state) {
      for (const p of composition.state.properties) {
        this.emit(`var ${this.escapeStringValue(p.key as string, 'TypeScript')} by remember { mutableStateOf(${this.toKotlinValue(p.value)}) }`);
      }
      this.emit('');
    }
  ...
  private emitObject(obj: HoloObjectDecl): void {
    ...
    if (obj.traits?.some((t) => t.name === 'plane_detection')) {
      this.emit(`val ${v}Plane = xrSession.scene.createEntity("${this.escapeStringValue(obj.name as string, 'TypeScript')}_plane")`);
    ...
    if (modelSrc) {
      ...
      this.emit(`val ${v} = xrSession.scene.createEntity("${this.escapeStringValue(obj.name as string, 'TypeScript')}")`);
      const pos = this.findProp(obj, 'position');
      if (pos && Array.isArray(pos)) {
        this.emit(
  ...
  private emitGroup(group: HoloSpatialGroup): void {
    ...
    this.emit('');
    this.emit(`// Group: ${this.escapeStringValue(group.name as string, 'TypeScript')}`);
    this.emit(`val ${v} = xrSession.scene.createEntity("${this.escapeStringValue(group.name as string, 'TypeScript')}")`);
  ...
  private emitLight(light: HoloLight): void {
    ...
    this.emit('');
    this.emit(`val ${v} = xrSession.scene.createEntity("${this.escapeStringValue(light.name as string, 'TypeScript')}")`);
  ...
  private emitAudio(audio: HoloAudio): void {
    ...
    this.emit('');
    this.emit(`// Audio: ${this.escapeStringValue(audio.name as string, 'TypeScript')}`);
    if (src) {
      this.emit(`val ${v}Id = soundPool.load(this, R.raw.${this.sanitizeName(src as string)}, 1)`);
    }
    if (spatial && pos && Array.isArray(pos)) {
      this.emit(`val ${v}Entity = xrSession.scene.createEntity("${this.escapeStringValue(audio.name as string, 'TypeScript')}")`);
      this.emit(
  ...
  private emitUI(ui: HoloUI): void {
    ...
    for (const el of ui.elements) {
      ...
      if (elType === 'button' && label) {
        ...
        this.emit(`Column(modifier = Modifier.fillMaxSize()) { Text("${this.escapeStringValue(el.name as string, 'TypeScript')}") }`);
      }
      this.dedent();
      this.emit('}');
    }
  ...
  private emitZone(zone: HoloZone): void {
    ...
    this.emit(`val ${v} = xrSession.scene.createEntity("${this.escapeStringValue(zone.name as string, 'TypeScript')}")`);
  ...
  private emitHeadFollowUI(
    ...
    if (text) {
      ...
      this.emit(`Column(modifier = Modifier.fillMaxSize()) { Text("${this.escapeStringValue(obj.name as string, 'TypeScript')}") }`);
    }
    this.dedent();
    this.emit('}');
  }
  ...
  private emitGlimmerContentTree(composition: HoloComposition): void {
    ...
    this.emit(`TitleChip(title = "${this.escapeStringValue(composition.name as string, 'TypeScript')}")`);
    this.emit('');
  ...
  private emitGlimmerObject(obj: HoloObjectDecl): void {
    ...
    this.emit(`// Object: ${this.escapeStringValue(obj.name as string, 'TypeScript')} (glasses overlay)`);
    ...
    if (text) {
      ...
      this.emit(`Card(`);
      this.indent();
      this.emit(`title = { Text("${this.escapeStringValue(obj.name as string, 'TypeScript')}") },`);
      this.emit(`action = {`);
      this.indent();
      ...
      this.emit(`ListItem(`);
      this.indent();
      this.emit(`headlineContent = { Text("${this.escapeStringValue(obj.name as string, 'TypeScript')}") },`);
  ...
  private emitGlimmerUIElement(el: {
    ...
    if (elType === 'button' && label) {
      ...
      this.emit(`Card(title = { Text("${this.escapeStringValue(el.name as string, 'TypeScript')}") }) {`);
      this.indent();
      this.emit(`Text("${this.escapeStringValue(el.name as string, 'TypeScript')}")`);
      this.dedent();
      this.emit('}');
    }
  }
...
${permissions.join('\n')}
    ...
    <application
        ...
        <activity
            ...
            android:label="${this.escapeStringValue(composition.name as string, 'TypeScript')} AI Glasses">
            <intent-filter>
                <action android:name="android.intent.action.MAIN" />
            </intent-filter>
        </activity>
...
dependencies {
  ...
  private generateGlimmerComponentsFile(composition: HoloComposition): string {
    ...
    this.emit('// Auto-generated by HoloScript AndroidXRCompiler (AI Glasses mode)');
    this.emit(`// Glimmer Components: ${this.escapeStringValue(composition.name as string, 'TypeScript')}`);
    this.emit('');
    this.emit(`package ${pkg}`);
    this.emit('');
    this.emit('import androidx.compose.runtime.*');
