...
export type CapabilityScope =
  | 'scene:read' | 'scene:write' | 'scene:admin'
  | 'physics:read' | 'physics:write'
  | 'network:read' | 'network:write' | 'network:admin'
  | 'inventory:read' | 'inventory:write' | 'inventory:admin'
  | 'authority:delegate' | 'authority:admin'
  | 'agent:spawn' | 'agent:admin'
  | 'audio:play' | 'audio:admin'
  ...
  while (changed) {
    ...
    for (const cap of [...expanded]) {
      for (const sub of (CAPABILITY_HIERARCHY[cap] || [])) {
        if (!expanded.has(sub)) { expanded.add(sub); changed = true; }
      }
    }
  }
  return expanded;
...
export const TRUST_LEVEL_CAPABILITIES: Record<string, CapabilityScope[]> = {
  ...
  'trusted':   ['scene:read', 'scene:write', 'physics:read', 'physics:write', 'audio:play', 'inventory:read', 'agent:spawn'],
  'admin':     ['scene:admin', 'physics:write', 'network:admin', 'inventory:admin', 'authority:admin', 'agent:admin', 'audio:admin'],
  'system':    ['scene:admin', 'physics:write', 'network:admin', 'inventory:admin', 'authority:admin', 'agent:admin', 'audio:admin', 'system:admin'],
};
