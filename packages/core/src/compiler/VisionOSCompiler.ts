...
export class VisionOSCompiler extends CompilerBase {
  ...
  compile(composition: HoloComposition, agentToken: string, outputPath?: string): string {
    ...
    if (composition.state) {
      ...
      for (const prop of composition.state.properties) {
        ...
        this.emit(`var ${this.escapeStringValue(prop.key as string, 'TypeScript')}: ${swiftType} = ${swiftVal}`);
      }
      this.indentLevel--;
      this.emit('}');
      this.emit('');
    ...
    this.emit(`root.name = "${this.escapeStringValue(composition.name as string, 'TypeScript')}"`);
  ...
  private compileObject(obj: HoloObjectDecl, parentVar: string): void {
    ...
    if (isLight) {
      this.emit(`// Light object "${this.escapeStringValue(obj.name as string, 'TypeScript')}" — handled via light components`);
      return;
    }
  ...
  private compileUIComponent(obj: HoloObjectDecl, parentVar: string): void {
    ...
    this.emit('');
    this.emit(`// UI Component: ${this.escapeStringValue(obj.name as string, 'TypeScript')}`);
  ...
  private compileUIPanel(obj: HoloObjectDecl, varName: string, parentVar: string): void {
    ...
    this.emit(`// SwiftUI Panel attachment for "${this.escapeStringValue(obj.name as string, 'TypeScript')}"`);
    this.emit(`let ${varName} = Entity()`);
    this.emit(`${varName}.name = "${this.escapeStringValue(obj.name as string, 'TypeScript')}"`);
    ...
    if (obj.children) {
      for (const child of obj.children) {
        this.emit(`//         // Child: ${this.escapeStringValue(child.name as string, 'TypeScript')}`);
      }
    }
