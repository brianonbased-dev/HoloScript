 ...
 *
 * Tests for the HoloScript → USD Physics Schema compiler.
...
describe('USDPhysicsCompiler', () => {
  ...
  describe('Basic Compilation', () => {
    it('should generate valid USDA structure', () => {
      ...
      const usda = compiler.compile(composition, 'test-token');
    ...
    it('should include generator metadata', () => {
      ...
      expect(usda).toContain('generator = "HoloScript v6.0"');
      expect(usda).toContain('targetRuntime = "NVIDIA Isaac Sim / Isaac Lab"');
    });
  ...
  describe('Custom Options', () => {
    it('should use custom stage name', () => {
      ...
      const composition = createComposition();
      const usda = customCompiler.compile(composition, 'test-token');
  ...
  describe('Transform Emission', () => {
    ...
    it('should emit rotation from rotation property (rotateXYZ for Isaac Sim, orient for generic)', () => {
      const composition = createComposition({
        objects: [
          createObject({
            name: 'RotatedBox',
      ...
      // Default context (isaac_sim) uses rotateXYZ
      const usda = compiler.compile(composition, 'test-token');
      expect(usda).toContain('xformOp:rotateXYZ');

      // Generic context uses orient (quatd)
      const genericCompiler = new USDPhysicsCompiler({ targetContext: 'generic' });
      const usdaGeneric = genericCompiler.compile(composition, 'test-token');
      expect(usdaGeneric).toContain('xformOp:orient');
    });
