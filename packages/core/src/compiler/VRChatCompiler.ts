...
export class VRChatCompiler extends CompilerBase {
  ...
  private generateMainScript(composition: HoloComposition): string {
    ...
    if (composition.state) {
      ...
      for (const prop of composition.state.properties) {
        ...
        this.emit(`[UdonSynced] public ${csType} ${this.escapeStringValue(prop.key as string, 'TypeScript')} = ${csVal};`);
      }
      this.emit('');
    }
  ...
  private generateObjectUdonScript(
    ...
    if (obj.state?.properties) {
      ...
      for (const prop of obj.state.properties) {
        ...
        lines.push(`    ${syncAttr}public ${csType} ${this.escapeStringValue(prop.key as string, 'TypeScript')} = ${csVal};`);
      }
      lines.push('');
    }
  ...
  private generatePrefabHierarchy(composition: HoloComposition): string {
    const lines: string[] = [];
    lines.push(`# VRChat World Prefab Hierarchy: ${this.escapeStringValue(composition.name as string, 'TypeScript')}`);
    ...
    if (composition.lights) {
      ...
      for (const light of composition.lights) {
        lines.push(`â”‚   â””â”€â”€ ${this.escapeStringValue(light.name as string, 'TypeScript')} (${light.lightType})`);
      }
    }
    ...
    if (composition.objects) {
      ...
      for (const obj of composition.objects) {
        const traits = obj.traits?.map((t) => `@${this.escapeStringValue(t.name as string, 'TypeScript')}`).join(' ') || '';
        lines.push(`â”‚   â””â”€â”€ ${this.escapeStringValue(obj.name as string, 'TypeScript')} ${traits}`);
      }
    }

    if (composition.spatialGroups) {
      for (const group of composition.spatialGroups) {
        lines.push(`â”œâ”€â”€ ${this.escapeStringValue(group.name as string, 'TypeScript')} (SpatialGroup)`);
        for (const child of group.objects || []) {
          lines.push(`â”‚   â””â”€â”€ ${this.escapeStringValue(child.name as string, 'TypeScript')}`);
        }
      }
    }
    ...
    if (composition.audio) {
      ...
      for (const audio of composition.audio) {
        lines.push(`â”‚   â””â”€â”€ ${this.escapeStringValue(audio.name as string, 'TypeScript')} (VRCSpatialAudioSource)`);
      }
    }
    ...
    if (composition.zones) {
      ...
      for (const zone of composition.zones) {
        lines.push(`â”‚   â””â”€â”€ ${this.escapeStringValue(zone.name as string, 'TypeScript')} (Trigger)`);
      }
    }
  ...
  private compileZoneTriggers(zone: any): void {
    ...
    this.emit('{');
    this.indentLevel++;
    this.emit(`Debug.Log($"Player {player.displayName} entered ${this.escapeStringValue(zone.name as string, 'TypeScript')}");`);
    this.indentLevel--;
    this.emit('}');
    this.emit('');
    ...
    this.emit('{');
    this.indentLevel++;
    this.emit(`Debug.Log($"Player {player.displayName} exited ${this.escapeStringValue(zone.name as string, 'TypeScript')}");`);
    this.indentLevel--;
    this.emit('}');
    this.emit('');
  }
  ...
  private compileTransition(transition: HoloTransition): void {
    ...
    this.emit(`// Transition: ${this.escapeStringValue(transition.name as string, 'TypeScript')}`);
