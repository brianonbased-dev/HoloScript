...
export class VRChatCompiler extends CompilerBase {
  ...
  private generateMainScript(composition: HoloComposition): string {
    ...
    if (composition.state) {
      ...
      for (const prop of composition.state.properties) {
        ...
        this.emit(
          `[UdonSynced] public ${csType} ${this.escapeStringValue(prop.key as string, 'CSharp')} = ${csVal};`
        );
      }
      this.emit('');
    }
  ...
  private compileVRChatDomainBlocks(composition: HoloComposition): void {
    ...
    const compiled = compileDomainBlocks(
      ...
      (block) =>
        `// Domain block: ${block.domain}/${block.keyword} "${this.escapeStringValue(block.name as string, 'CSharp')}"`
    );
  ...
  private generateObjectUdonScript(
    ...
    lines.push(`// Object: ${this.escapeStringValue(obj.name as string, 'CSharp')}`);
    lines.push('');
    ...
    if (obj.state?.properties) {
      ...
      for (const prop of obj.state.properties) {
        ...
        lines.push(
          `    ${syncAttr}public ${csType} ${this.escapeStringValue(prop.key as string, 'CSharp')} = ${csVal};`
        );
      }
      lines.push('');
    }
  ...
  private generatePrefabHierarchy(composition: HoloComposition): string {
    const lines: string[] = [];
    lines.push(
      `# VRChat World Prefab Hierarchy: ${this.escapeStringValue(composition.name as string, 'CSharp')}`
    );
    ...
    if (composition.lights) {
      ...
      for (const light of composition.lights) {
        lines.push(
          `â”‚   â””â”€â”€ ${this.escapeStringValue(light.name as string, 'CSharp')} (${light.lightType})`
        );
      }
    }
    ...
    if (composition.objects) {
      ...
      for (const obj of composition.objects) {
        const traits =
          obj.traits
            ?.map((t) => `@${this.escapeStringValue(t.name as string, 'CSharp')}`)
            .join(' ') || '';
        lines.push(
          `â”‚   â””â”€â”€ ${this.escapeStringValue(obj.name as string, 'CSharp')} ${traits}`
        );
      }
    }
    ...
    if (composition.audio) {
      ...
      for (const audio of composition.audio) {
        lines.push(
          `â”‚   â””â”€â”€ ${this.escapeStringValue(audio.name as string, 'CSharp')} (VRCSpatialAudioSource)`
        );
      }
    }
    ...
    if (composition.zones) {
      ...
      for (const zone of composition.zones) {
        lines.push(
          `â”‚   â””â”€â”€ ${this.escapeStringValue(zone.name as string, 'CSharp')} (Trigger)`
        );
      }
    }
  ...
  private compileZoneTriggers(zone: any): void {
    ...
    this.indentLevel++;
    this.emit(
      `Debug.Log($"Player {player.displayName} entered ${this.escapeStringValue(zone.name as string, 'CSharp')}");`
    );
    this.indentLevel--;
    this.emit('}');
    this.emit('');
    ...
    this.indentLevel++;
    this.emit(
      `Debug.Log($"Player {player.displayName} exited ${this.escapeStringValue(zone.name as string, 'CSharp')}");`
    );
    this.indentLevel--;
    this.emit('}');
    this.emit('');
  ...
  private compileTimeline(timeline: HoloTimeline): void {
    ...
    this.emit(`// Timeline: ${this.escapeStringValue(timeline.name as string, 'CSharp')}`);
  ...
  private compileTransition(transition: HoloTransition): void {
    ...
    this.emit(`// Transition: ${this.escapeStringValue(transition.name as string, 'CSharp')}`);
  ...
  private toCSharpValue(value: HoloValue): string {
    ...
    if (typeof value === 'string') return `"${this.escapeStringValue(value, 'CSharp')}"`;
    if (Array.isArray(value)) {
