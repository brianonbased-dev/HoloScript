...
export class OpenXRCompiler extends CompilerBase {
  ...
  private emitActionSets(): void {
    ...
    for (const action of actions) {
      ...
      this.emit(
        `std::strncpy(info.actionName, "${this.escapeStringValue(action.id as string, 'TypeScript')}", XR_MAX_ACTION_NAME_SIZE);`
      );
      this.emit(
      ...
      this.emit(
        `xrCreateAction(actionSet, &info, &${this.escapeStringValue(action.name as string, 'TypeScript')});`
      );
      this.dedent();
      this.emit('}');
    }
  ...
  private emitObject(obj: HoloObjectDecl): void {
    ...
    if (this.options.renderBackend === 'vulkan') {
      this.emit(
        `// Create Vulkan vertex buffer for ${this.escapeStringValue(obj.name as string, 'TypeScript')}`
      );
      ...
      this.emit('');
      this.emit(
        `// Allocate descriptor set for ${this.escapeStringValue(obj.name as string, 'TypeScript')}`
      );
    ...
    if (obj.traits) {
      for (const trait of obj.traits) {
        if (trait.name === 'hand_tracking') {
          ...
          this.emit('');
          this.emit(
            `// Trait: grabbable -- detect grip action on ${this.escapeStringValue(obj.name as string, 'TypeScript')}`
          );
