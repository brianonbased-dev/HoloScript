...
export class OpenXRCompiler extends CompilerBase {
      ...
      const compiled = compileDomainBlocks(
        domainBlocks,
        {
          material: (block) => {
            ...
            return `// Material: "${this.escapeStringValue(m.name as string, 'TypeScript')}" type=${m.type} baseColor=${m.baseColor || 'none'} roughness=${m.roughness ?? 'default'} metallic=${m.metallic ?? 'default'}`;
          },
          physics: (block) => {
            const p = compilePhysicsBlock(block);
            return `// Physics: ${p.keyword} "${p.name || ''}" colliders=${p.colliders?.length || 0} rigidbody=${!!p.rigidbody}`;
          },
          vfx: (block) => {
            const ps = compileParticleBlock(block);
            return `// Particles: "${this.escapeStringValue(ps.name as string, 'TypeScript')}" rate=${ps.properties.rate || 'default'} modules=${ps.modules.length}`;
          },
          postfx: (block) => {
            const pp = compilePostProcessingBlock(block);
            return `// PostFX: ${pp.keyword} effects=[${pp.effects.map((e) => e.type).join(', ')}]`;
          },
          audio: (block) => {
            const a = compileAudioSourceBlock(block);
            return `// Audio: "${this.escapeStringValue(a.name as string, 'TypeScript')}" (${a.keyword}) clip=${a.properties.clip || 'none'}`;
          },
          weather: (block) => {
            const w = compileWeatherBlock(block);
            return `// Weather: ${w.keyword} layers=[${w.layers.map((l) => l.type).join(', ')}]`;
          },
        },
  ...
  private emitActionSets(): void {
    ...
    for (const action of actions) {
      ...
      this.emit(`std::strncpy(info.actionName, "${this.escapeStringValue(action.id as string, 'TypeScript')}", XR_MAX_ACTION_NAME_SIZE);`);
      this.emit(
      ...
      this.emit(`xrCreateAction(actionSet, &info, &${this.escapeStringValue(action.name as string, 'TypeScript')});`);
      this.dedent();
      this.emit('}');
    }
  ...
  private emitObject(obj: HoloObjectDecl): void {
    ...
    if (this.options.renderBackend === 'vulkan') {
      this.emit(`// Create Vulkan vertex buffer for ${this.escapeStringValue(obj.name as string, 'TypeScript')}`);
      ...
      this.emit('}');
      this.emit('');
      this.emit(`// Allocate descriptor set for ${this.escapeStringValue(obj.name as string, 'TypeScript')}`);
    ...
    if (obj.traits) {
      for (const trait of obj.traits) {
        if (trait.name === 'hand_tracking') {
          ...
          this.emit('');
          this.emit(`// Trait: grabbable -- detect grip action on ${this.escapeStringValue(obj.name as string, 'TypeScript')}`);
  ...
  private emitLight(light: HoloLight): void {
    ...
    this.emit(`// Light: ${this.escapeStringValue(light.name as string, 'TypeScript')} (${light.lightType})`);
    this.emit('{');
    this.indent();
  ...
  private emitRenderLoop(composition: HoloComposition): void {
    ...
    if (this.options.renderBackend === 'vulkan') {
      ...
      for (const obj of allObjects) {
        const varName = this.sanitizeName(obj.name);
        this.emit(`// Draw: ${this.escapeStringValue(obj.name as string, 'TypeScript')}`);
        this.emit(
