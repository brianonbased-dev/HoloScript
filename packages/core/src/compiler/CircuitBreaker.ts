...
export type ExportTarget =
  ...
  | 'physics-sim' // Sovereign GPU rigid-body simulation: WGSL compute steps gravity + sphere/AABB collisions, renders settled state; our own solver
  ...
  | 'omnigent-agent-yaml' // Omnigent agent YAML plus HoloScript projection receipt
  ...
  | 'holob' // HoloVM bytecode (sovereign HoloVM execution target)
  | 'openapi' // OpenAPI 3.x spec generation (bridge)
  | 'onnx' // ONNX model export (bridge — third-party ML runtimes)
  | 'flutter' // Flutter/Dart widget tree (bridge)
  | 'stl-export' // STL mesh export for 3D printing/CAD (bridge)
  | 'lens-studio' // Snap Lens Studio AR effects (bridge)
  | 'colyseus' // Colyseus multiplayer server scaffold (bridge)
  ...
  | 'llama-server' // llama.cpp server launch/health/service/registry bundle
