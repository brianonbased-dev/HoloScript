...
export class IOSCompiler extends CompilerBase {
  ...
  private generateSceneFile(composition: HoloComposition): string {
    ...
    this.emit('// Auto-generated by HoloScript IOSCompiler');
    this.emit(`// Scene: ${this.escapeStringValue(composition.name as string, 'TypeScript')}`);
    this.emit('');
    ...
    for (const obj of composition.objects || []) {
      ...
      this.emit(`nodes["${this.escapeStringValue(obj.name as string, 'TypeScript')}"] = make${varName}()`);
    }
  ...
  private generateStateFile(composition: HoloComposition): string {
    ...
    if (composition.state) {
      ...
      for (const prop of composition.state.properties) {
        ...
        this.emit(`@Published var ${this.escapeStringValue(prop.key as string, 'TypeScript')}: ${swiftType} = ${swiftValue}`);
      }
      this.emit('');
    }
...
<dict>
    ...
    <string>${this.escapeStringValue(composition.name as string, 'TypeScript')}</string>
  ...
  private compileObjectFactory(obj: HoloObjectDecl): void {
    ...
    this.emit(`node.name = "${this.escapeStringValue(obj.name as string, 'TypeScript')}"`);
  ...
  private compileLightFactory(light: HoloLight): void {
    ...
    this.emit(`node.name = "${this.escapeStringValue(light.name as string, 'TypeScript')}"`);
  ...
  private compileAction(action: any): void {
    ...
    this.emit(`func ${name}() {`);
    this.indentLevel++;
    this.emit(`print("[HoloScript] Action: ${this.escapeStringValue(action.name as string, 'TypeScript')}")`);
