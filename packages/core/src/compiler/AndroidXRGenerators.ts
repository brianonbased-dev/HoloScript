...
export function emitAITraitHelpers(
  compiler: AndroidXRCompiler,
  composition: HoloComposition
): void {
...
export function emitSceneComposable(
  compiler: AndroidXRCompiler,
  composition: HoloComposition
): void {
  ...
  for (const t of obj.traits ?? []) {
    ...
    // AI upscaling: apply TFLite super-resolution to texture
      ...
      if (texture && typeof texture === 'string') {
        ...
        compiler.emit(
          `val ${v}UpscaledBitmap = ${v}Upscale(LocalContext.current, ${v}OriginalBitmap)`
        );
    ...
    // AI inpainting: apply TFLite mask-based inpainting
      ...
      if (texture && typeof texture === 'string' && mask && typeof mask === 'string') {
        ...
        compiler.emit(
          `val ${v}InpaintedBitmap = ${v}Inpaint(LocalContext.current, ${v}ImageBitmap, ${v}MaskBitmap)`
        );
        compiler.emit(`// Apply ${v}InpaintedBitmap to material texture`);
      } else {
        compiler.emit(
          `// AI inpainting trait present but requires both 'texture' and 'mask' properties on ${v}`
        );
      }
    }
  }
...
export function emitAudio(compiler: AndroidXRCompiler, audio: HoloAudio): void {
  ...
  if (src) {
    compiler.emit(
      `val ${v}Id = soundPool.load(this, R.raw.${compiler.sanitizeName(src as string)}, 1)`
    );
  }
...
export function emitFaceTracking(
  compiler: AndroidXRCompiler,
  varName: string,
  config: Record<string, unknown>
): void {
  ...
  if (blendshapes.length > 0) {
    for (const bs of blendshapes) {
      ...
      compiler.emit(
        `val ${varName}_${compiler.sanitizeName(bs)} = faceState.blendShapes[${bsConst}]`
      );
    }
  } else {
    compiler.emit(
...
export function compileAndroidXRDomainBlocks(
  compiler: AndroidXRCompiler,
  composition: HoloComposition
): void {
  const domainBlocks =
