...
export const QUALITY_TIER_SCALES: Record<
  QualityTier,
  {
...
export class R3FCompiler {
  ...
  private expandSpreads(
    ...
    for (const [key, value] of Object.entries(props)) {
      if (key.startsWith('__spread_')) {
        ...
        value &&
        typeof value === 'object' &&
        (value as Record<string, unknown>).type === 'spread'
      ) {
    ...
    for (const spreadKey of spreadKeys) {
      ...
      if (
        ...
        (spreadValue as Record<string, unknown>).type === 'spread'
      ) {
  ...
  private expandArraySpreads(
    ...
    for (const item of arr) {
      if (item && typeof item === 'object' && (item as Record<string, unknown>).type === 'spread') {
        const resolved = this.resolveSpreadArgument(
          (item as Record<string, unknown>).argument,
          context
        );
  ...
  public compile(ast: HSPlusAST, agentToken?: string, outputPath?: string): R3FNode {
    ...
    const domainBlocks = ((ast as unknown as Record<string, unknown>).domainBlocks ?? []) as Array<
      Record<string, unknown>
    >;
    if (domainBlocks.length > 0) {
      const compiled = compileDomainBlocks(
        domainBlocks as unknown as import('../parser/HoloCompositionTypes').HoloDomainBlock[],
        {
          material: (block) => {
            ...
            return {
              ...
              props: {
                __raw: materialToR3F(mat, this.tierScales),
                __domain: 'material',
                __name: mat.name,
              },
              children: [],
          ...
          vfx: (block) => {
            ...
            return {
              type: 'DomainBlockOutput',
              props: {
                __raw: particlesToR3F(ps, this.tierScales),
                __domain: 'vfx',
                __name: ps.name,
              },
              children: [],
  ...
  public compileNode(node: ASTNode): R3FNode {
    ...
    if (node.directives) {
      for (const directive of node.directives) {
        if (directive.type === 'trait') {
          r3fNode.traits?.set(
            directive.name as VRTraitName,
            directive.config as Record<string, unknown>
          );
        }
      }
    }
    ...
    if (enhanced.children && Array.isArray(enhanced.children)) {
      r3fNode.children = (enhanced.children as ASTNode[]).map((child: ASTNode) =>
  ...
  private compileLightBlock(light: Record<string, unknown>): R3FNode {
    ...
    const type = lightMapping[light.lightType as string] || 'directionalLight';
  ...
  private compileEnvironmentBlock(env: Record<string, unknown>): R3FNode[] {
    const nodes: R3FNode[] = [];
    const envProps: Record<string, unknown> = {};

    if (env.properties) {
      for (const prop of env.properties as Record<string, unknown>[]) {
        envProps[prop.key as string] = prop.value;
      }
    }

    // Environment preset (skybox or preset name)
    const presetName = (envProps.skybox || envProps.preset) as string | undefined;
    if (presetName) {
      ...
      if (preset) {
        ...
        const presetMap = preset as Record<string, unknown>;
        if (presetMap.fog) {
          const fogMap = presetMap.fog as Record<string, unknown>;
          nodes.push(
            this.createNode('fog', {
              attach: 'fog',
              color: fogMap.color,
        ...
        if (presetMap.ground) {
          const groundMap = presetMap.ground as Record<string, unknown>;
          nodes.push(
            this.createNode(
              'mesh',
              {
    ...
    // Explicit fog
    const explicitFog = envProps.fog as Record<string, unknown> | undefined;
  ...
  private compileObjectDecl(
    ...
    if (obj.template && templateMap) {
      const tmpl = templateMap.get(obj.template as string) as Record<string, unknown> | undefined;
      if (tmpl) {
        // Merge template traits (object's own traits take precedence)
        const tmplTraits = tmpl.traits as Array<Record<string, unknown>> | undefined;
        if (tmplTraits && Array.isArray(tmplTraits)) {
          const existingTraitNames = new Set(
            ((obj.traits as Array<Record<string, unknown>>) || []).map(
              (t: Record<string, unknown>) => t.name as string
            )
          );
          obj.traits = [
            ...tmplTraits.filter(
              (t: Record<string, unknown>) => !existingTraitNames.has(t.name as string)
            ),
            ...((obj.traits as unknown[]) || []),
          ];
        }
        // Merge template properties (object's own properties take precedence)
        const tmplProps = tmpl.properties as Array<Record<string, unknown>> | undefined;
        if (tmplProps && Array.isArray(tmplProps)) {
          const existingKeys = new Set(
            ((obj.properties as Array<Record<string, unknown>>) || []).map(
              (p: Record<string, unknown>) => p.key as string
            )
          );
          const mergedProps = tmplProps
          ...
          obj.properties = [...mergedProps, ...((obj.properties as unknown[]) || [])];
        }
      }
    }
    ...
    if (obj.properties) {
      for (const p of obj.properties as Array<Record<string, unknown>>) {
        const key = p.key as string;
        const value = p.value;

        if (key === 'geometry' || key === 'mesh') {
          geometryType = value as string;
        } else if (key === 'type') {
          ...
          if (['text', 'sparkles', 'portal', 'audio'].includes(valStr)) {
            ...
            ['directional', 'point', 'spot', 'hemisphere', 'ambient', 'area'].includes(valStr)
          ) {
    ...
    if (obj.traits && Array.isArray(obj.traits)) {
      // Unify physics traits using ProvenanceSemiring for commutative merging
      const semiring = new ProvenanceSemiring();
      const physicsApplications = obj.traits
        .filter((t: Record<string, unknown>) => t.name === 'physics' || t.name === 'collidable')
      ...
      for (const trait of obj.traits) {
        ...
        if (name === 'physics' || name === 'collidable') {
          continue; // Handled comprehensively by ProvenanceSemiring above
        ...
        // checking, dual to gossip's permissiveness. Now we emit but
        // also record what was unrecognized for diagnostics.
        else {
          props[name] = trait.config || true;
          // Track unrecognized traits for faithfulness analysis
          if (!props.__unrecognizedTraits) {
            props.__unrecognizedTraits = [];
          }
          (props.__unrecognizedTraits as string[]).push(name);
        }
      }
    }
    ...
    if (obj.traits && Array.isArray(obj.traits)) {
      const traitNames = (obj.traits as Array<Record<string, unknown>>).map(
        (t: Record<string, unknown>) => t.name as string
      );
    ...
    if (geometryType === 'model' && props.src) {
      ...
      type = lightMapping[lightType as string] || 'directionalLight';
    ...
    const r3fNode = this.createNode(type, props, obj.name as string | undefined);
    ...
    if (obj.children && Array.isArray(obj.children)) {
      for (const child of obj.children) {
        r3fNode.children!.push(this.compileObjectDecl(child as Record<string, unknown>));
      }
    }
  ...
  private compileSystemNode(
    ...
    const r3fNode = this.createNode('System', props, (node.id || node.name) as string | undefined);
    if (node.directives) {
    ...
    if (node.directives) {
      for (const d of node.directives as HSPlusDirective[]) {
        if (d.type === 'trait') {
  ...
  private compileChildNode(
    ...
    switch (child.type) {
      ...
      default:
        // Use a dirty cast since the base format interfaces differ from the custom ASTNode
        return this.compileNode(child as unknown as ASTNode);
    }
  }
   ...
   */
  private compileTemplateNode(
    node: Record<string, unknown>,
    _templateMap?: Map<string, unknown>
  ): R3FNode {
    const props: Record<string, unknown> = {};

    // Preserve template metadata
    if (node.state || (node.properties as Record<string, unknown>)?.state) {
      props.templateState = node.state || (node.properties as Record<string, unknown>)?.state;
    }
    if (node.properties) {
    ...
    return this.createNode('Template', props, node.name as string | undefined);
  }
  ...
  private compileSpatialGroup(
    ...
    const node = this.createNode('group', props, group.name as string | undefined);

    if (group.objects) {
      for (const obj of group.objects as Array<Record<string, unknown>>)
  ...
  private compileTimelineBlock(timeline: Record<string, unknown>): R3FNode {
    ...
    if (timeline.entries) {
      for (const entry of timeline.entries as Array<Record<string, unknown>>) {
        const action = entry.action as Record<string, unknown>;
        const entryProps: Record<string, unknown> = { time: entry.time, actionKind: action.kind };
  ...
  private compileAudioBlock(audio: Record<string, unknown>): R3FNode {
    const props: Record<string, unknown> = {};
    if (audio.properties) {
      for (const prop of audio.properties as Array<Record<string, unknown>>) {
  ...
  private compileZoneBlock(zone: Record<string, unknown>): R3FNode {
    const props: Record<string, unknown> = {};
    if (zone.properties) {
      for (const prop of zone.properties as Array<Record<string, unknown>>) {
        props[prop.key as string] = prop.value;
      }
    }
    // Attach handlers as props
    if (zone.handlers && (zone.handlers as Array<Record<string, unknown>>).length > 0) {
      props.handlers = (zone.handlers as Array<Record<string, unknown>>).map(
  ...
  private compileUIBlock(ui: Record<string, unknown>): R3FNode {
    const children: R3FNode[] = [];
    if (ui.elements) {
      for (const el of ui.elements as Array<Record<string, unknown>>) {
        const elProps: Record<string, unknown> = {};
        if (el.properties) {
          for (const prop of el.properties as Array<Record<string, unknown>>) {
            if (prop.key === 'font_size') {
              elProps.fontSize = prop.value;
            } else {
              elProps[prop.key as string] = this.resolveValue(prop.value);
            }
          }
        }
  ...
  private compileTransitionBlock(transition: Record<string, unknown>): R3FNode {
    ...
    if (transition.properties) {
      for (const prop of transition.properties as Array<Record<string, unknown>>) {
        props[prop.key as string] = prop.value;
      }
    }
    const safeName = transition.name
      ? escapeStringValue(transition.name as string, 'JSX')
      : undefined;
    return this.createNode('Transition', props, safeName);
  }

  private compileConditionalBlock(
    ...
    if (cond.elseObjects) {
      for (const obj of cond.elseObjects as Array<Record<string, unknown>>)
        elseChildren.push(this.compileObjectDecl(obj, templateMap));
    }
    if (cond.elseSpatialGroups) {
      for (const g of cond.elseSpatialGroups as Array<Record<string, unknown>>)
  ...
  private compileForEachBlock(
    iter: Record<string, unknown>,
    templateMap?: Map<string, unknown>
  ): R3FNode {
    const templateChildren: R3FNode[] = [];
    if (iter.objects) {
      for (const obj of iter.objects as Array<Record<string, unknown>>)
        templateChildren.push(this.compileObjectDecl(obj, templateMap));
    }
    if (iter.spatialGroups) {
      for (const g of iter.spatialGroups as Array<Record<string, unknown>>)
  ...
  private resolveValue(value: unknown): unknown {
    if (value && typeof value === 'object' && (value as Record<string, unknown>).__bind) {
      return {
        __bind: true,
        source: (value as Record<string, unknown>).source,
        transform: (value as Record<string, unknown>).transform,
      };
    }
    return value;
  }
  ...
  private hasPostProcessing(node: R3FNode): boolean {
    if (
      node.traits?.has('bloom' as VRTraitName) ||
      node.traits?.has('postprocessing' as VRTraitName)
    )
      return true;
  ...
  private mapType(type: string, props: Record<string, unknown>): string {
    if (type === 'environment') {
    ...
    if (type === 'light') {
      ...
      return lightMapping[lightType as string] || 'directionalLight';
    }
  ...
  private compileProperties(
    ...
    if (node.hologram) {
      ...
      if (MESH_TYPES.has(node.type)) {
        props.args = this.getGeometryArgs(node.type, h as unknown as Record<string, unknown>);
    ...
    if (rawProps.geometry && !props.hsType) {
      ...
      props.args = this.getGeometryArgs(rawProps.geometry as string, { size: rawProps.size || 1 });
    }
  ...
  private applyGraphicsConfig(r3fNode: R3FNode, graphics: Record<string, unknown>): void {
    if (graphics.material) {
      const m = graphics.material as {
        preset?: string;
        type?: string;
        pbr?: {
          baseColor?: string | number | Record<string, number>;
          metallic?: number;
          roughness?: number;
          emission?: string | number | Record<string, number>;
    ...
    if (graphics.lighting) {
      const l = graphics.lighting as {
        shadows?: boolean;
        intensity?: number;
        color?: string | number | Record<string, number>;
        preset?: string;
      };
