...
export class UnityCompiler extends CompilerBase {
  ...
  compile(composition: HoloComposition, agentToken: string, outputPath?: string): string {
    ...
    if (composition.state) {
      ...
      for (const prop of composition.state.properties) {
        ...
        this.emit(`private ${csType} ${this.escapeStringValue(prop.key as string, 'TypeScript')} = ${csVal};`);
      }
      this.emit('');
    }
  ...
  private compileLight(light: HoloLight): void {
    ...
    this.emit(`// Light: ${this.escapeStringValue(light.name as string, 'TypeScript')}`);
    this.emit(`var ${varName}GO = new GameObject("${this.escapeStringValue(light.name as string, 'TypeScript')}");`);
  ...
  private compileObject(obj: HoloObjectDecl, parentVar: string): void {
    ...
    if (isText) {
      this.emit(`var ${varName}GO = new GameObject("${this.escapeStringValue(obj.name as string, 'TypeScript')}");`);
      ...
      this.emit(`${varName}GO.name = "${this.escapeStringValue(obj.name as string, 'TypeScript')}";`);
      this.emit(`${varName}GO.transform.SetParent(${parentVar});`);
    } else if (isSparkles) {
      this.emit(`// Sparkles: use ParticleSystem for "${this.escapeStringValue(obj.name as string, 'TypeScript')}"`);
      this.emit(`var ${varName}GO = new GameObject("${this.escapeStringValue(obj.name as string, 'TypeScript')}");`);
      this.emit(`${varName}GO.transform.SetParent(${parentVar});`);
    ...
    if (obj.traits) {
      for (const trait of obj.traits) {
        ...
        // Environment Understanding
          ...
          this.emit(`// @${this.escapeStringValue(trait.name as string, 'TypeScript')} — Azure Spatial Anchors or AR Foundation`);
          ...
          this.emit(`// @${this.escapeStringValue(trait.name as string, 'TypeScript')} — ARCore Geospatial API`);
        }
        // Physics Expansion
          ...
          this.emit(`// @${this.escapeStringValue(trait.name as string, 'TypeScript')} — custom physics: ${JSON.stringify(trait.config || {})}`);
        }
        // Spatial Audio
          ...
          this.emit(`// @${this.escapeStringValue(trait.name as string, 'TypeScript')} — spatial audio: ${JSON.stringify(trait.config || {})}`);
        }
        // Volumetric Content
        else if (
          ...
          this.emit(
            `// @${this.escapeStringValue(trait.name as string, 'TypeScript')} — volumetric renderer: ${JSON.stringify(trait.config || {})}`
          );
        }
        // Input Modalities
        ...
        // V43 AI/XR Traits
          ...
          this.emit(
            `// @${this.escapeStringValue(trait.name as string, 'TypeScript')} — visionOS-specific; use Mirror/Photon for Unity multiplayer`
          );
          ...
          this.emit(`// @${this.escapeStringValue(trait.name as string, 'TypeScript')} — AI knowledge: use external service or on-device model`);
        } else if (trait.name === 'vision' || trait.name === 'ai_vision') {
          this.emit(`// @${this.escapeStringValue(trait.name as string, 'TypeScript')} — Unity Barracuda / Sentis vision model`);
  ...
  private compileSpatialGroup(group: HoloSpatialGroup, parentVar: string): void {
    ...
    this.emit(`var ${varName}GO = new GameObject("${this.escapeStringValue(group.name as string, 'TypeScript')}");`);
    this.emit(`${varName}GO.transform.SetParent(${parentVar});`);
  ...
  private compileAudio(audio: HoloAudio): void {
    ...
    this.emit(`var ${varName}GO = new GameObject("${this.escapeStringValue(audio.name as string, 'TypeScript')}");`);
    this.emit(`${varName}GO.transform.SetParent(transform);`);
  ...
  private compileZone(zone: HoloZone): void {
    ...
    this.emit(`// Zone: ${this.escapeStringValue(zone.name as string, 'TypeScript')}`);
    this.emit(`var ${varName}GO = new GameObject("${this.escapeStringValue(zone.name as string, 'TypeScript')}");`);
    this.emit(`${varName}GO.transform.SetParent(transform);`);
  ...
  private compileUI(ui: HoloUI): void {
    ...
    for (const el of ui.elements) {
      ...
      this.emit(`// UI Element: ${this.escapeStringValue(el.name as string, 'TypeScript')} (${elType})`);
      this.emit(`var ${varName}GO = new GameObject("${this.escapeStringValue(el.name as string, 'TypeScript')}");`);
