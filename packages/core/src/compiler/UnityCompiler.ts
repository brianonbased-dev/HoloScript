...
export class UnityCompiler extends CompilerBase {
  ...
  compile(composition: HoloComposition, agentToken: string, outputPath?: string): string {
    ...
    if (composition.state) {
      ...
      for (const prop of composition.state.properties) {
        ...
        this.emit(
          `private ${csType} ${this.escapeStringValue(prop.key as string, 'CSharp')} = ${csVal};`
        );
      }
      this.emit('');
    }
  ...
  private compileLight(light: HoloLight): void {
    ...
    this.emit(`// Light: ${this.escapeStringValue(light.name as string, 'CSharp')}`);
    this.emit(
      `var ${varName}GO = new GameObject("${this.escapeStringValue(light.name as string, 'CSharp')}");`
    );
  ...
  private compileObject(obj: HoloObjectDecl, parentVar: string): void {
    ...
    if (isText) {
      this.emit(
        `var ${varName}GO = new GameObject("${this.escapeStringValue(obj.name as string, 'CSharp')}");`
      );
      ...
      this.emit(
        `// Sparkles: use ParticleSystem for "${this.escapeStringValue(obj.name as string, 'CSharp')}"`
      );
      this.emit(
        `var ${varName}GO = new GameObject("${this.escapeStringValue(obj.name as string, 'CSharp')}");`
      );
    ...
    if (obj.traits) {
      for (const trait of obj.traits) {
        ...
        // Environment Understanding
          ...
          this.emit(
            `// @${this.escapeStringValue(trait.name as string, 'CSharp')} — Azure Spatial Anchors or AR Foundation`
          );
          ...
          this.emit(
            `// @${this.escapeStringValue(trait.name as string, 'CSharp')} — ARCore Geospatial API`
          );
        }
        // Physics Expansion
          ...
          this.emit(
            `// @${this.escapeStringValue(trait.name as string, 'CSharp')} — custom physics: ${JSON.stringify(trait.config || {})}`
          );
        }
        // Spatial Audio
          ...
          this.emit(
            `// @${this.escapeStringValue(trait.name as string, 'CSharp')} — spatial audio: ${JSON.stringify(trait.config || {})}`
          );
        }
        // Volumetric Content
        else if (
          ...
          this.emit(
            `// @${this.escapeStringValue(trait.name as string, 'CSharp')} — volumetric renderer: ${JSON.stringify(trait.config || {})}`
          );
        }
        // Input Modalities
        ...
        // V43 AI/XR Traits
          ...
          this.emit(
            `// @${this.escapeStringValue(trait.name as string, 'CSharp')} — visionOS-specific; use Mirror/Photon for Unity multiplayer`
          );
          ...
          this.emit(
            `// @${this.escapeStringValue(trait.name as string, 'CSharp')} — ${JSON.stringify(trait.config || {})}`
          );
        } else if (
          ...
          this.emit(
            `// @${this.escapeStringValue(trait.name as string, 'CSharp')} — AI knowledge: use external service or on-device model`
          );
        } else if (trait.name === 'vision' || trait.name === 'ai_vision') {
          this.emit(
            `// @${this.escapeStringValue(trait.name as string, 'CSharp')} — Unity Barracuda / Sentis vision model`
          );
  ...
  private compileSpatialGroup(group: HoloSpatialGroup, parentVar: string): void {
    ...
    this.emit(`// Spatial Group: ${this.escapeStringValue(group.name as string, 'CSharp')}`);
    this.emit(
      `var ${varName}GO = new GameObject("${this.escapeStringValue(group.name as string, 'CSharp')}");`
    );
    this.emit(`${varName}GO.transform.SetParent(${parentVar});`);
  ...
  private compileAudio(audio: HoloAudio): void {
    ...
    this.emit(`// Audio: ${this.escapeStringValue(audio.name as string, 'CSharp')}`);
    this.emit(
      `var ${varName}GO = new GameObject("${this.escapeStringValue(audio.name as string, 'CSharp')}");`
    );
    this.emit(`${varName}GO.transform.SetParent(transform);`);
  ...
  private compileZone(zone: HoloZone): void {
    ...
    this.emit(
      `var ${varName}GO = new GameObject("${this.escapeStringValue(zone.name as string, 'CSharp')}");`
    );
    this.emit(`${varName}GO.transform.SetParent(transform);`);
  ...
  private compileUI(ui: HoloUI): void {
    ...
    for (const el of ui.elements) {
      ...
      this.emit(
        `// UI Element: ${this.escapeStringValue(el.name as string, 'CSharp')} (${elType})`
      );
      this.emit(
        `var ${varName}GO = new GameObject("${this.escapeStringValue(el.name as string, 'CSharp')}");`
      );
