...
/**
 * Target runtime context for USD output.
 *
 * - `'isaac_sim'`: NVIDIA Isaac Sim / Isaac Lab — forces Z-up, GPU dynamics,
 *    and strict `xformOp` ordering (translate → rotateXYZ → scale) required
 *    by Isaac Lab's `ArticulationView` loader.
 * - `'omniverse'`: General NVIDIA Omniverse — Z-up, GPU dynamics optional.
 * - `'generic'`:   Portable USD — no runtime-specific attributes.
 */
export type USDTargetContext = 'isaac_sim' | 'omniverse' | 'generic';
...
export interface USDPhysicsCompilerOptions {
   ...
   *
   * When set to `'isaac_sim'`, the compiler enforces Isaac Lab-compatible
   * xformOp ordering (translate → rotateXYZ → scale) and emits additional
   * PhysX solver hints required by `ArticulationView`.
   *
   * @default 'isaac_sim'
   */
   ...
   * preservation per the Metaverse Standards Forum interoperability model.
   *
   * @default true
   */
  embedSemanticAST?: boolean;
}
...
{`);
    ...
    // Emit HoloScript semantic AST as customData (Metaverse Standards interop)
  ...
  private emitPrim(prim: USDPhysicsPrim): void {
    ...
    // Isaac Sim / Isaac Lab requires rotateXYZ (Euler) instead of orient (quatd)
    // for ArticulationView compatibility. Other contexts use quatd orient.
    const useEulerRotation = this.options.targetContext === 'isaac_sim';
    if (prim.rotation && !this.isIdentityQuaternion(prim.rotation)) {
      if (useEulerRotation) {
        // Convert quaternion back to Euler degrees for Isaac Lab
    ...
    // xformOpOrder — Isaac Lab requires strict translate → rotateXYZ → scale ordering
   ...
   */
  private emitSemanticCustomData(composition: HoloComposition): void {
    ...
    // Emit trait manifest (unique trait names across all objects)
    ...
    // Emit target context for downstream consumers
    this.emit(`string holoscript:targetContext = "${this.options.targetContext}"`);
   ...
   */
  private quaternionToEulerDegrees(q: [number, number, number, number]): [number, number, number] {
    const [x, y, z, w] = q;
