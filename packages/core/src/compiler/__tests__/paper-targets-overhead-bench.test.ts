...
describe('Paper 10/12 — paper-targets-overhead-bench', () => {
  it('reports median/p99 compile time base vs provenance per target', () => {
    ...
    const TARGETS: TargetSpec[] = [
      {
        ...
        compileProv: () =>
          new WebGPUCompiler({ provenanceHash: provHashMain }).compile(astMain, AGENT_TOKEN),
      ...
      },
      {
        ...
        compileProv: () =>
          new VRChatCompiler({ provenanceHash: provHashMain }).compile(astMain, AGENT_TOKEN),
        assertProvOutput: (out, h) => {
      ...
      },
      {
        ...
        compileProv: () =>
          new UnityCompiler({ provenanceHash: provHashMain }).compile(astMain, AGENT_TOKEN),
        assertProvOutput: (out, h) => {
          expect(String(out)).toContain(`// Provenance Hash: ${h}`);
        },
      },
      {
        ...
        compileProv: () =>
          new GodotCompiler({ provenanceHash: provHashMain }).compile(astMain, AGENT_TOKEN),
        assertProvOutput: (out, h) => {
          expect(String(out)).toContain(`# Provenance Hash: ${h}`);
        },
      },
      ...
      },
      {
        ...
        provHash: provHashMain,
        compileBase: () =>
          new UnrealCompiler({ generateBlueprints: false }).compile(astMain, AGENT_TOKEN),
        compileProv: () =>
      ...
      },
      {
        ...
        compileProv: () =>
          new GLTFPipeline({ ...gltfOpts, provenanceHash: provHashGltf }).compile(
            astGltf,
            AGENT_TOKEN
          ),
        assertProvOutput: (out, h) => {
    ...
    console.log(
      `[paper-targets-overhead-bench] hash_main=${provHashMain.slice(0, 16)}… hash_gltf=${provHashGltf.slice(0, 16)}…`
    );
    ...
    for (const t of TARGETS) {
      ...
      const overheadMed = b.median > 1e-6 ? ((p.median - b.median) / b.median) * 100 : 0;
      const overheadP99 = b.p99 > 1e-6 ? ((p.p99 - b.p99) / b.p99) * 100 : 0;
  ...
  it('Phase 3 — W.067 noise-floor: base compile time for remaining CompilerBase targets', () => {
    const astMain = parseFixture(buildPaperTargetsFixture(BENCH_OBJECT_COUNT));
    ...
    const PHASE3_TARGETS: NoiseFloorSpec[] = [
      ...
      {
        id: 'openxr-spatial',
        baseCall: () => new OpenXRSpatialEntitiesCompiler().compile(astMain, AGENT_TOKEN),
      },
