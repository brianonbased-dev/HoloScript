        ...
        traits: [
          ...
          // --- Universal hard don't ---
          {
            ...
            config: {
              ...
              reason: 'Vendor frameworks become RUNNERS in our protocol, never substitutes.',
...
describe('compile() - vocabulary v1 -> ts_adapter_capabilities', () => {
  it('emits one paste-ready capability block per provider', () => {
    ...
    expect(Object.keys(result.files)).toEqual(['anthropic.ts.capabilities', 'xai.ts.capabilities']);
  ...
  it('emits adapter constants matching the Capabilities interface shape', () => {
    ...
    const ts = compiler.compile(makeFullV1Matrix(), '').files['anthropic.ts.capabilities'];
...
describe('compile() - vocabulary v1 -> cost_guard_pricing', () => {
  ...
  it('can emit markdown and cost guard pricing in one compile pass', () => {
    ...
    expect(Object.keys(result.files)).toEqual(['LLM_CAPABILITIES.md', 'cost-guard-pricing.ts']);
    expect(result.files['LLM_CAPABILITIES.md']).toContain('# LLM Capabilities & Benchmarks - SSOT');
...
describe('compile() - vocabulary v1 -> json_capability_matrix', () => {
  ...
  it('emits one provider row with every capability flag column', () => {
    ...
    const json = compiler.compile(makeFullV1Matrix(), '').files['llm-capability-matrix.json'];
...
describe('compile() - BLOCK rules', () => {
  ...
  it('ALLOWS valid provider+model with FK matched (no throw, no orphan-FK warning)', () => {
    ...
    expect(result.diagnostics).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: expect.stringMatching(/orphan/) })])
    );
  });
});
...
describe('compile() - WARN rules', () => {
  ...
  it('does NOT warn on fresh last_verified (<= 90 days)', () => {
    ...
    expect(result.diagnostics).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'F.014 stale-last-verified' })])
    );
  });
