...
export class WebGPUCompiler extends CompilerBase {
  ...
  private waterClarityChannel: string = 'environmentWaterClarity';
  private waterRippleChannel: string = 'environmentRippleIntensity';
  ...
  compile(composition: HoloComposition, agentToken: string, outputPath?: string): string {
    ...
    if (this.hasWater) this.resolveWaterChannels(composition);
    ...
    if (this.hasWater) {
      const cc = JSON.stringify(this.waterClarityChannel);
      const rc = JSON.stringify(this.waterRippleChannel);
      // Named-channel store: the water's clarity/ripple are bound BY NAME to these
      // channels (from the .holo's *_binding props). Host state — the Brittney
      // environment-coupling loop turning receipts into clarity/ripple deltas —
      // drives them via hsSetChannel(name, value); hsSetWater is a convenience that
      // writes the SAME channels. The values ride the shared per-frame uniform.
      this.emit(`const hsChannels = { [${cc}]: 1.0, [${rc}]: 0.0 };`);
      this.emit('let hsWaterAmp = 1.0;');
      this.emit(
        'function hsSetChannel(name, value) { if (name in hsChannels) hsChannels[name] = Math.max(0, Math.min(1, value)); return hsChannels[name]; }'
      );
      this.emit(
        `function hsSetWater(o) { if (o) { if (o.clarity !== undefined) hsSetChannel(${cc}, o.clarity); if (o.ripple !== undefined) hsSetChannel(${rc}, o.ripple); if (o.amplitude !== undefined) hsWaterAmp = Math.max(0, o.amplitude); } return { clarity: hsChannels[${cc}], ripple: hsChannels[${rc}], amplitude: hsWaterAmp }; }`
      );
      this.emit(
        'if (typeof window !== "undefined") { window.hsSetChannel = hsSetChannel; window.hsSetWater = hsSetWater; }'
      );
      this.emit(
        `const waterFrameU = createBuffer(device, new Float32Array([0,0,0,0, hsChannels[${cc}], hsChannels[${rc}], hsWaterAmp, 0]), GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST);`
      );
      this.emit('');
    }
  ...
  /**
   * Resolve which named channels the water surface's clarity/ripple are bound to,
   * from the first water object's `water_clarity_binding` / `ripple_intensity_binding`
   * props. These names match the .holo's environment state fields, so host state
   * (Brittney receipts) driving those fields drives the water by name.
   */
  private resolveWaterChannels(composition: HoloComposition): void {
    const all: HoloObjectDecl[] = [...(composition.objects ?? [])];
    for (const g of composition.spatialGroups ?? []) all.push(...(g.objects ?? []));
    const water = all.find((o) => this.isWaterObject(o));
    if (!water) return;
    const clarity = this.findObjProp(water, 'water_clarity_binding');
    const ripple = this.findObjProp(water, 'ripple_intensity_binding');
    if (typeof clarity === 'string' && clarity.trim()) this.waterClarityChannel = clarity.trim();
    if (typeof ripple === 'string' && ripple.trim()) this.waterRippleChannel = ripple.trim();
  }
  ...
  private emitRenderLoop(composition: HoloComposition): void {
    ...
    if (this.hasWater) {
      ...
      this.emit(
        `device.queue.writeBuffer(waterFrameU, 0, new Float32Array([time, cameraPos[0], cameraPos[1], cameraPos[2], hsChannels[${JSON.stringify(this.waterClarityChannel)}], hsChannels[${JSON.stringify(this.waterRippleChannel)}], hsWaterAmp, 0]));`
      );
    }
