...
export class GodotCompiler extends CompilerBase {
  ...
  compile(composition: HoloComposition, agentToken: string, outputPath?: string): string {
    ...
    if (composition.state) {
      ...
      for (const prop of composition.state.properties) {
        ...
        this.emit(`var ${this.escapeStringValue(prop.key as string, 'TypeScript')} = ${gdVal}`);
      }
      this.emit('');
    }
  ...
  private compileLight(light: HoloLight): void {
    ...
    this.emit(`${varName}.name = "${this.escapeStringValue(light.name as string, 'TypeScript')}"`);
  ...
  private compileObject(obj: HoloObjectDecl, parentVar: string): void {
    ...
    if (isLight) {
      ...
      if (hasPhysics) {
        ...
        this.emit(`${varName}_body.name = "${this.escapeStringValue(obj.name as string, 'TypeScript')}"`);
      }
    ...
    this.emit(`${varName}.name = "${this.escapeStringValue(obj.name as string, 'TypeScript')}"`);
    ...
    if (obj.traits) {
      for (const trait of obj.traits) {
        ...
        // Physics Expansion
          ...
          this.emit(`${varName}_soft.name = "${this.escapeStringValue(obj.name as string, 'TypeScript')}_cloth"`);
  ...
  private compileSpatialGroup(group: HoloSpatialGroup, parentVar: string): void {
    ...
    this.emit(`${varName}.name = "${this.escapeStringValue(group.name as string, 'TypeScript')}"`);
  ...
  private compileAudio(audio: HoloAudio): void {
    ...
    this.emit(`${varName}.name = "${this.escapeStringValue(audio.name as string, 'TypeScript')}"`);
  ...
  private compileZone(zone: HoloZone): void {
    ...
    this.emit(`${varName}.name = "${this.escapeStringValue(zone.name as string, 'TypeScript')}"`);
    ...
    for (const handler of zone.handlers) {
      if (handler.event === 'on_enter') {
        this.emit(`${varName}.body_entered.connect(func(body): print("${this.escapeStringValue(zone.name as string, 'TypeScript')}: enter"))`);
      } else if (handler.event === 'on_exit') {
        this.emit(`${varName}.body_exited.connect(func(body): print("${this.escapeStringValue(zone.name as string, 'TypeScript')}: exit"))`);
      }
    }
  ...
  private compileUI(ui: HoloUI): void {
    ...
    for (const el of ui.elements) {
      ...
      this.emit(`# UI: ${this.escapeStringValue(el.name as string, 'TypeScript')} (${elType})`);
      if (elType === 'text') {
      ...
      this.emit(`${varName}.name = "${this.escapeStringValue(el.name as string, 'TypeScript')}"`);
