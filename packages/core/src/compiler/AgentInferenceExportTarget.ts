...
const ANTHROPIC_MODELS_NO_SAMPLING_PARAMS: ReadonlySet<string> = new Set(['claude-opus-4-7']);
...
export class AgentInferenceCompiler extends CompilerBase {
  ...
  private emitToolDefinitions(allTools: ToolDefinition[]): string {
    const provider = this.options.defaultProvider ?? DEFAULT_MODEL_CONFIG.provider;
    if (this.options.language === 'python') {
      return this.emitToolDefinitionsPython(allTools, provider);
    }
    return this.emitToolDefinitionsTypeScript(allTools, provider);
  }

  private emitToolDefinitionsTypeScript(
    ...
    lines.push(` * Generated by HoloScript AgentInferenceCompiler (target: ${provider})`);
    lines.push(' */');
    lines.push('');

    // Provider-specific imports + type annotation
    if (provider === 'anthropic') {
      lines.push("import Anthropic from '@anthropic-ai/sdk';");
    }
    // 'openai' / 'local' / 'ollama' / 'custom' — emit untyped, structurally
    // compatible with OpenAI Responses API (and any OpenAI-compat consumer:
    // xAI, OpenRouter, Ollama-OpenAI, vLLM-OpenAI). 'custom' gets bare JSON
    // shape with no `type: function` discriminator.
    lines.push('');

    // Tool name union type (provider-independent)
    const toolNames = allTools.map((t) => `'${t.name}'`);
    lines.push(`export type ToolName = ${toolNames.join(' | ')};`);
    lines.push('');

    // Tool definitions array — provider-specific shape
    if (provider === 'anthropic') {
      ...
      // Vendor-neutral JSON schema array — no SDK assumptions
      ...
      for (const tool of allTools) {
        ...
        const required = tool.parameters.filter((p) => p.required).map((p) => `'${p.name}'`);
        if (required.length > 0) {
          lines.push(`      required: [${required.join(', ')}] as const,`);
        }
        lines.push('    },');
        lines.push('  },');
      }
      ...
      // openai / local / ollama — OpenAI Responses function-tool shape.
      // Structurally compatible with `OpenAI.Responses.Tool[]` from the
      // openai SDK; intentionally untyped to avoid SDK-version coupling
      // in generated code. Cast at the call site if a typed handle is
      // needed: `client.responses.create({ tools: tools as OpenAI.Responses.Tool[], ... })`.
      lines.push('export const tools = [');
      for (const tool of allTools) {
        lines.push('  {');
    ...
    // Tool execution dispatcher (provider-independent)
  ...
  private emitToolDefinitionsPython(allTools: ToolDefinition[], provider: ModelProvider): string {
    const lines: string[] = [];
    lines.push('"""');
    lines.push('Tool definitions for agent inference');
    lines.push(`Generated by HoloScript AgentInferenceCompiler (target: ${provider})`);
    ...
    //  - anthropic: `{name, description, input_schema}` (Anthropic Messages API)
    //  - openai/local/ollama: `{type: "function", name, description, parameters}` (OpenAI Responses)
    //  - custom: `{name, description, parameters}` (vendor-neutral JSON schema)
    lines.push('TOOLS = [');
    for (const tool of allTools) {
      lines.push('    {');
      ...
      lines.push(`        "name": ${JSON.stringify(tool.name)},`);
      lines.push(`        "description": ${JSON.stringify(tool.description)},`);
      const paramKey = provider === 'anthropic' ? 'input_schema' : 'parameters';
      lines.push(`        ${JSON.stringify(paramKey)}: {`);
