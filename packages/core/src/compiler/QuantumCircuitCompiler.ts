...
export interface QASMOutput {
  ...
  warnings: string[];
  /** Symbolic parameter names that the optimizer should bind (VQE: theta_0..theta_N, QAOA: gamma_0..gamma_P, beta_0..beta_P) */
  paramNames?: string[];
}
...
function generateVQECircuit(
  ...
  lines.push(`bit[${n}] c;`);
  // RATCHET: VQE ansatz rotation angles use input float theta_N OpenQASM 3 parameters
  // with default=0.1. A real VQE outer loop must bind these per iteration.
  // QAOA uses gamma/beta with default=0.5 similarly.
  // Declare symbolic parameters so the VQE optimizer can bind them
...
function generateQAOACircuit(
  ...
  // Default initial values for optimizer; bindable via input parameters
  ...
  lines.push(`qubit[${n}] q;`);
  lines.push(`bit[${n}] c;`);
  // Declare symbolic parameters so the QAOA optimizer can bind them
  lines.push('// Parameters: bindable by optimizer (initial values are defaults)');
  for (let round = 0; round < p; round++) {
