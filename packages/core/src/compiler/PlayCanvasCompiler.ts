...
export class PlayCanvasCompiler extends CompilerBase {
  ...
  private emitObject(obj: HoloObjectDecl, parentVar?: string): void {
    ...
    this.emit(
      `const ${v} = new pc.Entity("${this.escapeStringValue(obj.name as string, 'TypeScript')}");`
    );
    ...
    for (const trait of obj.traits || []) {
      if (trait.name === 'grabbable') {
        ...
        this.emit(
          `${v}.button!.on("click", () => { console.log("Picked: ${this.escapeStringValue(obj.name as string, 'TypeScript')}"); });`
        );
        ...
        this.emit(
          `// @accessible: role="${trait.config?.role || 'generic'}" for "${this.escapeStringValue(obj.name as string, 'TypeScript')}"`
        );
  ...
  private emitGroup(group: HoloSpatialGroup, parentVar?: string): void {
    ...
    this.emit(
      `const ${v} = new pc.Entity("${this.escapeStringValue(group.name as string, 'TypeScript')}");`
    );
  ...
  private emitLight(light: HoloLight): void {
    ...
    this.emit(
      `const ${v} = new pc.Entity("${this.escapeStringValue(light.name as string, 'TypeScript')}");`
    );
  ...
  private emitAudio(audio: HoloAudio): void {
    ...
    this.emit(
      `const ${v} = new pc.Entity("${this.escapeStringValue(audio.name as string, 'TypeScript')}");`
    );
  ...
  private emitZone(zone: HoloZone): void {
    ...
    this.emit(
      `const ${v} = new pc.Entity("${this.escapeStringValue(zone.name as string, 'TypeScript')}");`
    );
    if (shape === 'sphere') {
  ...
  private emitUI(ui: HoloUI): void {
    ...
    for (const el of ui.elements) {
      ...
      this.emit(
        `const ${v} = new pc.Entity("${this.escapeStringValue(el.name as string, 'TypeScript')}");`
      );
      if (elType === 'button') {
        this.emit(`${v}.addComponent("element", {`);
        this.indent();
