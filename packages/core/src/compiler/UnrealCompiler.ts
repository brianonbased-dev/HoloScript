...
export class UnrealCompiler extends CompilerBase {
  ...
  private generateSource(composition: HoloComposition): string {
    ...
    this.emitS('');
    this.emitS(
      `UE_LOG(LogTemp, Log, TEXT("HoloScript scene '${this.escapeStringValue(composition.name as string, 'TypeScript')}' initialized"));`
    );
    ...
    if (composition.state) {
      ...
      for (const prop of composition.state.properties) {
        this.emitS(
          `DOREPLIFETIME(${this.options.className}, ${this.escapeStringValue(prop.key as string, 'TypeScript')});`
        );
      }
    }
    this.indentLevel--;
  ...
  private compileObjectSetup(obj: HoloObjectDecl): void {
    ...
    this.emitS(
      `${varName} = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("${this.escapeStringValue(obj.name as string, 'TypeScript')}"));`
    );
  ...
  private compileLightSetup(light: HoloLight): void {
    ...
    this.emitS(
      `${varName} = CreateDefaultSubobject<${lightType}>(TEXT("${this.escapeStringValue(light.name as string, 'TypeScript')}"));`
    );
    this.emitS(`${varName}->SetupAttachment(RootComponent);`);
  ...
  private compileAudioSetup(audio: HoloAudio): void {
    ...
    this.emitS(
      `${varName} = CreateDefaultSubobject<UAudioComponent>(TEXT("${this.escapeStringValue(audio.name as string, 'TypeScript')}"));`
    );
    this.emitS(`${varName}->SetupAttachment(RootComponent);`);
  ...
  private compileZoneHandlers(zone: any): void {
    ...
    this.indentLevel++;
    this.emitS(
      `UE_LOG(LogTemp, Log, TEXT("Actor entered zone: ${this.escapeStringValue(zone.name as string, 'TypeScript')}"));`
    );
    this.indentLevel--;
    this.emitS('}');
    this.emitS('');
    ...
    this.indentLevel++;
    this.emitS(
      `UE_LOG(LogTemp, Log, TEXT("Actor exited zone: ${this.escapeStringValue(zone.name as string, 'TypeScript')}"));`
    );
    this.indentLevel--;
    this.emitS('}');
    this.emitS('');
  ...
  private compileTimelineFunction(timeline: HoloTimeline): void {
    ...
    this.indentLevel++;
    this.emitS(
      `UE_LOG(LogTemp, Log, TEXT("Playing timeline: ${this.escapeStringValue(timeline.name as string, 'TypeScript')}"));`
    );
  ...
  private compileActionFunction(action: any): void {
    ...
    this.indentLevel++;
    this.emitS(
      `UE_LOG(LogTemp, Log, TEXT("Action: ${this.escapeStringValue(action.name as string, 'TypeScript')}"));`
    );
