...
export const PACKAGE_PERMISSION_MANIFEST: PackagePermission[] = [
  ...
  // no filesystem writes — only the explicit /fs subpath unlocks them.
  {
    ...
    handlesSecrets: false,
    notes:
      'File system subpath of @holoscript/std (merged from @holoscript/fs 2026-04-29). Path-boundary enforcement available via HOLOSCRIPT_FS_SANDBOX_ROOT env (R-02 P1, 2026-05-04); pass-through when unset for backwards compat. Runtime should set the env to the project root when loading a .hsplus.',
  },
  {
    name: 'holoscript',
