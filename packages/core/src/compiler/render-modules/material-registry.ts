...
const MATERIALS: Record<string, Material> = {
  rubber:  { name: 'rubber',  density: 1100, restitution: 0.85, friction: 0.90, youngsModulus: 5.0e7,  hardness: 0.30, albedo: [0.05, 0.05, 0.06], roughness: 0.80, metalness: 0.0, subsurface: 0.05 },
  steel:   { name: 'steel',   density: 7850, restitution: 0.60, friction: 0.60, youngsModulus: 2.0e11, hardness: 0.95, albedo: [0.56, 0.57, 0.58], roughness: 0.28, metalness: 1.0, subsurface: 0.0  },
  plastic: { name: 'plastic', density: 1050, restitution: 0.70, friction: 0.35, youngsModulus: 2.3e9,  hardness: 0.50, albedo: [0.20, 0.45, 0.85], roughness: 0.42, metalness: 0.0, subsurface: 0.04 },
  wood:    { name: 'wood',    density: 750,  restitution: 0.50, friction: 0.50, youngsModulus: 1.1e10, hardness: 0.60, albedo: [0.42, 0.28, 0.14], roughness: 0.78, metalness: 0.0, subsurface: 0.10 },
  glass:   { name: 'glass',   density: 2500, restitution: 0.55, friction: 0.40, youngsModulus: 7.0e10, hardness: 0.90, albedo: [0.85, 0.90, 0.92], roughness: 0.06, metalness: 0.0, subsurface: 0.25 },
  clay:    { name: 'clay',    density: 1900, restitution: 0.08, friction: 0.90, youngsModulus: 1.0e7,  hardness: 0.12, albedo: [0.62, 0.44, 0.36], roughness: 0.92, metalness: 0.0, subsurface: 0.12 },
  ice:     { name: 'ice',     density: 917,  restitution: 0.12, friction: 0.05, youngsModulus: 9.0e9,  hardness: 0.40, albedo: [0.78, 0.86, 0.92], roughness: 0.14, metalness: 0.0, subsurface: 0.45 },
  foam:    { name: 'foam',    density: 50,   restitution: 0.25, friction: 0.70, youngsModulus: 1.0e6,  hardness: 0.05, albedo: [0.90, 0.86, 0.70], roughness: 0.95, metalness: 0.0, subsurface: 0.30 },
  stone:   { name: 'stone',   density: 2600, restitution: 0.35, friction: 0.70, youngsModulus: 5.0e10, hardness: 0.80, albedo: [0.45, 0.45, 0.47], roughness: 0.85, metalness: 0.0, subsurface: 0.0  },
  // Toward "limbs and body" (D.125) — soft, subsurface-heavy matter for later deformation.
  flesh:   { name: 'flesh',   density: 1010, restitution: 0.20, friction: 0.60, youngsModulus: 5.0e4,  hardness: 0.08, albedo: [0.80, 0.52, 0.45], roughness: 0.60, metalness: 0.0, subsurface: 0.70 },
  fur:     { name: 'fur',     density: 300,  restitution: 0.15, friction: 0.80, youngsModulus: 1.0e5,  hardness: 0.06, albedo: [0.35, 0.24, 0.14], roughness: 0.98, metalness: 0.0, subsurface: 0.55 },
};

// Name aliases -> canonical material. Case-insensitive; grow by one line.
const ALIAS: Record<string, string> = {
  rubber: 'rubber', bouncy: 'rubber', tire: 'rubber',
  steel: 'steel', metal: 'steel', iron: 'steel', ballbearing: 'steel',
  plastic: 'plastic', abs: 'plastic', pvc: 'plastic', acrylic: 'plastic',
  wood: 'wood', oak: 'wood', timber: 'wood', pine: 'wood',
  glass: 'glass', marble: 'glass',
  clay: 'clay', putty: 'clay', mud: 'clay', dough: 'clay',
  ice: 'ice', frost: 'ice',
  foam: 'foam', sponge: 'foam', cushion: 'foam',
  stone: 'stone', rock: 'stone', concrete: 'stone', granite: 'stone',
  flesh: 'flesh', skin: 'flesh', meat: 'flesh', body: 'flesh',
  fur: 'fur', hair: 'fur', wool: 'fur',
};
...
export function resolveMaterial(name?: unknown): MaterialResolution {
  ...
  if (canonical && MATERIALS[canonical]) return { material: MATERIALS[canonical], defaulted: false };
...
export function primitiveVolume(kind: string, radius: number, half: readonly [number, number, number]): number {
  switch (kind) {
    case 'sphere': return (4 / 3) * Math.PI * radius * radius * radius;
    case 'cylinder': return Math.PI * radius * radius * (half[1] * 2);
    case 'cone': return (1 / 3) * Math.PI * radius * radius * (half[1] * 2);
    case 'torus': { const tube = radius * 0.3; return 2 * Math.PI * Math.PI * radius * tube * tube; }
    default: return Math.max(1e-6, half[0] * 2) * Math.max(1e-6, half[1] * 2) * Math.max(1e-6, half[2] * 2); // box
  }
}
