...
export class URDFCompiler extends CompilerBase {
    ...
    const compiled = compileDomainBlocks(
      domainBlocks,
      {
        material: (block) => {
          ...
          const lines: string[] = [];
          lines.push(`<material name="${this.escapeStringValue(mat.name as string, 'TypeScript')}">`);
          if (mat.baseColor) {
  ...
  private emitJoint(joint: URDFJoint): void {
    this.emit(`<joint name="${this.escapeStringValue(joint.name as string, 'TypeScript')}" type="${joint.type}">`);
    this.indentLevel++;
  ...
  private emitTransmission(trans: URDFTransmission): void {
    this.emit(`<transmission name="${this.escapeStringValue(trans.name as string, 'TypeScript')}">`);
    this.indentLevel++;
  ...
  private emitROS2Control(control: URDFROS2Control): void {
    this.emit(`<ros2_control name="${this.escapeStringValue(control.name as string, 'TypeScript')}" type="${control.type}">`);
    this.indentLevel++;
  ...
  private emitGazeboPlugins(composition: HoloComposition): void {
    ...
    for (const link of this.links) {
      ...
      if (hasSpecialConfig) {
        this.emit(`<gazebo reference="${this.escapeStringValue(link.name as string, 'TypeScript')}">`);
        this.indentLevel++;
  ...
  private emitGazeboSensor(sensor: URDFSensor): void {
    ...
    this.emit(`<sensor name="${this.escapeStringValue(sensor.name as string, 'TypeScript')}" type="${this.getGazeboSensorType(sensor.type)}">`);
    this.indentLevel++;
  ...
  private emitHoloExtensions(composition: HoloComposition): void {
    ...
    this.emit(`<!-- Original composition: "${this.escapeStringValue(composition.name as string, 'TypeScript')}" -->`);
    ...
    if (this.sensors.length > 0) {
      this.emit(`<!-- Sensors: ${this.sensors.map((s) => `${this.escapeStringValue(s.name as string, 'TypeScript')} (${s.type})`).join(', ')} -->`);
    }
  }
  ...
  private emitLoopJoint(loopJoint: URDFLoopJoint): void {
    this.emit(`<loop_joint name="${this.escapeStringValue(loopJoint.name as string, 'TypeScript')}" type="${loopJoint.type}">`);
    this.indentLevel++;
    this.emit(
