...
export class URDFCompiler extends CompilerBase {
  ...
  private emitMaterials(): void {
    ...
    for (const [_key, mat] of this.materials) {
      this.emit(`<material name="${mat.name}">`);
      this.indentLevel++;
  ...
  private emitTransmission(trans: URDFTransmission): void {
    this.emit(`<transmission name="${trans.name}">`);
    this.indentLevel++;
  ...
  private emitROS2Control(control: URDFROS2Control): void {
    this.emit(`<ros2_control name="${control.name}" type="${control.type}">`);
    this.indentLevel++;
    ...
    for (const joint of control.joints) {
      this.emit(`<joint name="${joint.name}">`);
      this.indentLevel++;
    ...
    if (control.sensors) {
      for (const sensor of control.sensors) {
        this.emit(`<sensor name="${sensor.name}">`);
        this.indentLevel++;
  ...
  private emitGazeboPlugins(composition: HoloComposition): void {
    ...
    for (const link of this.links) {
      ...
      if (hasSpecialConfig) {
        this.emit(`<gazebo reference="${link.name}">`);
        this.indentLevel++;
  ...
  private emitGazeboSensor(sensor: URDFSensor): void {
    ...
    this.emit(`<sensor name="${sensor.name}" type="${this.getGazeboSensorType(sensor.type)}">`);
    this.indentLevel++;
  ...
  private emitHoloExtensions(composition: HoloComposition): void {
    ...
    if (this.sensors.length > 0) {
      this.emit(`<!-- Sensors: ${this.sensors.map((s) => `${s.name} (${s.type})`).join(', ')} -->`);
    }
  }
