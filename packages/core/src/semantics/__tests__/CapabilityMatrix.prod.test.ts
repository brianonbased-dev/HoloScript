...
import { CapabilityMatrix, type CapabilityProfile, type FeatureRequirement } from '../CapabilityMatrix';
...
function makeProfile(overrides: any = {}): CapabilityProfile {
  return {
    ...
    userAgent: 'test',
    graphics: {
      maxTextureSize: 4096, maxCubeMapSize: 4096, maxRenderTargets: 4,
      maxVertexAttributes: 16, maxUniformBufferSize: 16384,
      compressedTextures: { s3tc: true, etc1: false, etc2: false, astc: false, pvrtc: false, bc7: false },
      hdr: true, instancing: true, computeShaders: false, geometryShaders: false,
      tessellation: false, rayTracing: false, meshShaders: false,
      maxMSAASamples: 4, anisotropicFiltering: true, maxAnisotropy: 16,
    },
    xr: {
      supported: false, modes: ['none'], handTracking: false, eyeTracking: false,
      bodyTracking: false, faceTracking: false, spatialAnchors: false,
      sceneUnderstanding: false, passthrough: false, depthSensing: false,
      maxRefreshRate: 60, foveatedRendering: false, haptics: false,
    },
    audio: {
      webAudio: true, spatialAudio: true, maxAudioSources: 32, hrtf: true,
      audioWorklets: true, mediaRecording: true, speechRecognition: false, speechSynthesis: true,
    },
    input: {
      touch: false, maxTouchPoints: 0, pointerLock: true, gamepad: true,
      keyboard: true, deviceOrientation: false, deviceMotion: false,
      pressureSensitivity: false, tilt: false,
    },
    network: {
      webSocket: true, webRTC: true, serverSentEvents: true, fetch: true,
      networkInformation: false, backgroundSync: false, serviceWorker: true,
    },
    storage: {
      localStorage: true, localStorageQuota: 5242880, indexedDB: true,
      cacheAPI: true, fileSystemAccess: false, persistentStorage: true,
    },
    performance: {
      deviceTier: 3, gpuTier: 3, logicalProcessors: 8,
      sharedArrayBuffer: true, webAssembly: true, simd: false,
      multiThreading: true, offscreenCanvas: true,
    },
