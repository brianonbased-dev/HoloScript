...
describe('VastServerlessAdapter', () => {
  ...
  it('does route → envelope and streams text + a fragmented tool call', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string, init: RequestInit) => {
        ...
        calls.push({ url, body });
        if (url === ROUTE) return json({ url: 'http://worker.test:8000', signature: 'sig123', request_idx: 0 });
        return sse([
    ...
    expect(calls[1].body.auth_data).toMatchObject({ url: 'http://worker.test:8000', signature: 'sig123' });
    ...
    expect(types).toEqual(
      expect.arrayContaining(['text_delta', 'tool_use_start', 'tool_use_input_delta', 'tool_use_end', 'message_stop'])
    );
    expect(types[types.length - 1]).toBe('message_stop');
    const start = chunks.find((c) => c.type === 'tool_use_start') as Extract<LLMStreamChunk, { type: 'tool_use_start' }>;
    expect(start.name).toBe('find_tools');
    const end = chunks.find((c) => c.type === 'tool_use_end') as Extract<LLMStreamChunk, { type: 'tool_use_end' }>;
  ...
  it('polls the route until a worker is READY (cold-pool wake)', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string) => {
        if (url === ROUTE) {
          routeHits++;
          if (routeHits < 3) return json({ status: { ready: 0, total: 1 }, request_idx: routeHits });
          return json({ url: 'http://w:8000' });
        }
        return sse(['data: {"choices":[{"delta":{"content":"ok"}}]}\n', 'data: [DONE]\n']);
      })
    );
    const adapter = new VastServerlessAdapter({ apiKey: 'vk', endpointName: 'e', model: 'm', pollIntervalMs: 1 });
    const chunks: LLMStreamChunk[] = [];
  ...
  it('complete() returns content + parsed tool uses via the envelope', async () => {
    vi.stubGlobal(
      ...
      vi.fn(async (url: string) => {
        ...
        return json({
          choices: [
            {
              message: { content: 'done', tool_calls: [{ id: 't1', function: { name: 'compile_to_unity', arguments: '{"x":1}' } }] },
    ...
    const adapter = new VastServerlessAdapter({ apiKey: 'vk', endpointName: 'e', model: 'qwen3:14b' });
