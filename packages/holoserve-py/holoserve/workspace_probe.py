...
JACOBIAN_LENS_ESTIMATOR = "explicit_pair_average_v0"
...
def load_jacobian_lens_artifact(
    ...
    *,
    ...
    if (
        not isinstance(estimator_meta, dict)
        or estimator_meta.get("name") != JACOBIAN_LENS_ESTIMATOR
        or estimator_meta.get("paperParity") is not False
        or estimator_meta.get("vectorization") != "full-output-jacobian-per-explicit-pair"
    ):
        raise WorkspaceProbeError(
            "invalid_lens_estimator",
            f"lens estimator must be {JACOBIAN_LENS_ESTIMATOR} with paperParity=false",
        )
    ...
    if (
        payload.get("implementationVersion") != WORKSPACE_PROBE_IMPLEMENTATION_VERSION
        or not _is_sha256(calibration_meta.get("corpusSha256"))
        ...
        raise WorkspaceProbeError(
            "invalid_lens_metadata",
            "lens calibration provenance is incomplete",
        )
    if (
    ...
    for index, layer_value in enumerate(layers):
        ...
        if layer < 0 or layer >= len(model.blocks) or layer in layer_map:
            raise WorkspaceProbeError(
                "invalid_lens_layer",
...
class ModelWorkspaceProbe:
    ...
    def capability(self) -> dict[str, Any]:
        return {
            ...
            "intervention": False,
            "method": "jacobian_lens",
            "estimator": JACOBIAN_LENS_ESTIMATOR,
            "paperParity": False,
