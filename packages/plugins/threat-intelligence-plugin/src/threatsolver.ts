...
export function buildThreatReceipt(
  ...
  return buildDomainSimulationReceipt({
    ...
    resultSummary: {
      cvssScore: result.cvss?.baseScore,
      cvssSeverity: result.cvss?.severity,
      killChainProbability: result.killChain?.overallProbability,
      criticalVulns: result.prioritizedVulns?.filter(v => v.priority === 1).length,
    },
