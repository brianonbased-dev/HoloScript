...
export function buildRestaurantReceipt(
  ...
  return buildDomainSimulationReceipt({
    ...
    resultSummary: {
      seatingUtilization: result.tableAssign?.overallUtilization,
      unseatatedParties: result.tableAssign?.unassigned.length,
      kitchenMakespan: result.queue?.makespan,
      blendedFoodCostPct: result.foodCost?.blendedFoodCostPct,
      menuStarCount: result.menuEngineering?.categoryCount.star,
    },
