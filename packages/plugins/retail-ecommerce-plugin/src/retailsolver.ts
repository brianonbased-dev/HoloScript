...
export function buildRetailReceipt(
  ...
  return buildDomainSimulationReceipt({
    ...
    resultSummary: {
      eoq: result.eoq?.eoq,
      optimalPrice: result.priceOpt?.optimalPrice,
      revenueUpliftPct: result.priceOpt?.revenueUpliftPct,
      clv: result.clv?.discountedCLV,
      overallConversion: result.funnel?.overallConversion,
    },
