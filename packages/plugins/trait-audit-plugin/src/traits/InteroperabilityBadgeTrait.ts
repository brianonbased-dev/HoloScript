...
const defaultConfig: InteroperabilityBadgeConfig = {
  traitName: '',
  targetVersion: '1.0.0',
  checks: [
    'typed_config',
...
export function createInteroperabilityBadgeHandler(): TraitHandler<InteroperabilityBadgeConfig> {
  return {
    name: 'interoperability_badge',
    defaultConfig,
    onAttach(n: HSPlusNode, c: InteroperabilityBadgeConfig, ctx: TraitContext) {
      n.__badgeState = {
        badgeLevel: 'none' as BadgeLevel,
        score: 0,
        maxScore: c.checks.length * 10,
        passedChecks: [],
        failedChecks: [],
        auditedAt: null,
      };
    ...
    onDetach(n: HSPlusNode, _c: InteroperabilityBadgeConfig, ctx: TraitContext) {
      delete n.__badgeState;
      ctx.emit?.('badge:removed');
    },
    onUpdate() {},
    onEvent(n: HSPlusNode, c: InteroperabilityBadgeConfig, ctx: TraitContext, e: TraitEvent) {
      const s = n.__badgeState as InteroperabilityBadgeState | undefined;
      if (!s) return;
      if (e.type === 'badge:audit') {
        ...
        const meetsRequirement =
          ['none', 'bronze', 'silver', 'gold', 'platinum'].indexOf(s.badgeLevel) >=
          ['none', 'bronze', 'silver', 'gold', 'platinum'].indexOf(c.requiredBadgeLevel);
        ctx.emit?.('badge:audited', {
          trait: c.traitName,
          badge: s.badgeLevel,
          score: s.score,
          max: s.maxScore,
          passed: s.passedChecks.length,
          failed: s.failedChecks.length,
          meetsRequirement,
        });
        if (!meetsRequirement)
          ctx.emit?.('badge:below_requirement', {
            current: s.badgeLevel,
            required: c.requiredBadgeLevel,
            failedChecks: s.failedChecks.map((f) => f.name),
          });
      }
    },
  };
