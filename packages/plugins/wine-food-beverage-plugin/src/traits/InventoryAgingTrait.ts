...
export interface InventoryAgingState {
  totalItems: number;
  readyToBottle: number;
  avgAgingMonths: number;
}

const defaultConfig: InventoryAgingConfig = {
  items: [],
  cellarCapacity: 100,
  optimalTempC: 13,
...
export function createInventoryAgingHandler(): TraitHandler<InventoryAgingConfig> {
  return {
    ...
    onAttach(n: HSPlusNode, c: InventoryAgingConfig, ctx: TraitContext) {
      ...
      const avg =
        c.items.length > 0 ? c.items.reduce((s, i) => s + i.currentMonths, 0) / c.items.length : 0;
    ...
    onDetach(n: HSPlusNode, _c: InventoryAgingConfig, ctx: TraitContext) {
      delete n.__agingState;
      ctx.emit?.('aging:removed');
    },
    onUpdate() {},
    onEvent(n: HSPlusNode, c: InventoryAgingConfig, ctx: TraitContext, e: TraitEvent) {
      if (e.type === 'aging:check_ready') {
        ...
        ctx.emit?.('aging:ready_list', {
          count: ready.length,
          items: ready.map((i) => ({ id: i.id, product: i.product, months: i.currentMonths })),
        });
      }
      if (e.type === 'aging:environment_alert') {
        ...
        if (Math.abs(temp - c.optimalTempC) > 3 || Math.abs(hum - c.optimalHumidityPercent) > 10)
          ctx.emit?.('aging:environment_warning', { temp, humidity: hum });
      }
    },
  };
}
