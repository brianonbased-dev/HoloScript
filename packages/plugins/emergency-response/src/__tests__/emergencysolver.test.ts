...
describe('startTriage', () => {
  ...
  it('classifies multiple patients correctly', () => {
    const patients = [
      {
        id: 'walker',
        respirationsPerMin: 16,
        radialPulse: true,
        mentalStatus: 'obeys' as const,
        canWalk: true,
      },
      {
        id: 'dead',
...
describe('resourceDispatch', () => {
  ...
  it('throws for empty units', () => {
    expect(() =>
      resourceDispatch(
        [],
        [{ id: 'I1', type: 'medical', locationX: 0, locationY: 0, priority: 1, unitsRequired: 1 }]
      )
    ).toThrow();
  });
});
...
describe('afterActionReport', () => {
  ...
  it('overallScore in [0, 100]', () => {
    const r = afterActionReport(8, 5, 0.8, 0.85);
  ...
  it('component scores are in [0, 100]', () => {
    const r = afterActionReport(6, 5, 0.75, 0.8);
