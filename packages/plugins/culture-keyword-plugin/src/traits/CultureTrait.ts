...
const defaultConfig: CultureConfig = {
  ...
  textDirection: 'ltr',
  calendar: 'gregorian',
  measurement: 'us_customary',
  firstDayOfWeek: 0,
  pluralRules: 'one_other',
  honorificRequired: false,
  colorSemantics: {},
};
...
export function createCultureHandler(): TraitHandler<CultureConfig> {
  return {
    ...
    onAttach(n: HSPlusNode, c: CultureConfig, ctx: TraitContext) {
      ...
      ctx.emit?.('culture:applied', {
        locale: c.locale,
        direction: c.textDirection,
        calendar: c.calendar,
      });
    },
    onDetach(n: HSPlusNode, _c: CultureConfig, ctx: TraitContext) {
      delete n.__cultureState;
      ctx.emit?.('culture:removed');
    },
    onUpdate() {},
    onEvent(n: HSPlusNode, c: CultureConfig, ctx: TraitContext, e: TraitEvent) {
      const s = n.__cultureState as CultureState | undefined;
      if (!s) return;
      if (e.type === 'culture:validate_text') {
        const text = (e.payload?.text as string) ?? '';
        const warnings: string[] = [];
        if (c.textDirection === 'rtl' && /[a-zA-Z]{10,}/.test(text))
          warnings.push('Long LTR text in RTL locale');
        if (c.honorificRequired && !/Mr\.|Mrs\.|Ms\.|Dr\.|Prof\.|-san|-sama|-sensei/.test(text))
          warnings.push('Missing honorific in locale requiring formal address');
        if (warnings.length > 0) {
          s.warningsEmitted.push(...warnings);
          ctx.emit?.('culture:validation_warnings', { warnings });
      ...
      if (e.type === 'culture:format_date') {
        ...
        ctx.emit?.('culture:formatted_date', {
          format: c.dateFormat,
          locale: c.locale,
      ...
      if (e.type === 'culture:format_number') {
        ...
        ctx.emit?.('culture:formatted_number', {
          value: num,
          locale: c.locale,
