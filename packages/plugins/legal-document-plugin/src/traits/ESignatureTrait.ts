...
const defaultConfig: ESignatureConfig = {
  documentId: '',
  signers: [],
  requireOrder: false,
  expirationDays: 30,
...
export function createESignatureHandler(): TraitHandler<ESignatureConfig> {
  return {
    name: 'e_signature',
    defaultConfig,
    onAttach(n: HSPlusNode, c: ESignatureConfig, ctx: TraitContext) {
      n.__esigState = {
        signedCount: 0,
        pendingCount: c.signers.length,
        isComplete: false,
        envelopeId: `env_${Date.now()}`,
      };
      ctx.emit?.('esig:envelope_created', { signers: c.signers.length });
    },
    onDetach(n: HSPlusNode, _c: ESignatureConfig, ctx: TraitContext) {
      delete n.__esigState;
      ctx.emit?.('esig:voided');
    },
    onUpdate() {},
    onEvent(n: HSPlusNode, c: ESignatureConfig, ctx: TraitContext, e: TraitEvent) {
      const s = n.__esigState as ESignatureState | undefined;
      if (!s) return;
      if (e.type === 'esig:sign') {
        ...
        if (signer && !signer.signedAt) {
          ...
          if (s.pendingCount === 0) {
            s.isComplete = true;
            ctx.emit?.('esig:completed', { envelopeId: s.envelopeId });
          }
        }
      }
    },
