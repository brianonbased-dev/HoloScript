...
export {
  ...
  type ContractDraftConfig,
  type ContractType,
  type Clause,
} from './traits/ContractDraftTrait';
export {
  createESignatureHandler,
  type ESignatureConfig,
  type Signer,
} from './traits/ESignatureTrait';
export {
  createCaseManagementHandler,
  type CaseManagementConfig,
  type CaseStatus,
} from './traits/CaseManagementTrait';
export {
  createProgrammableLawHandler,
  type ProgrammableLawConfig,
  type ProgrammableLawState,
} from './traits/ProgrammableLawTrait';
...
function validateSignatureBlock(
  ...
  if (witnesses !== undefined) {
    if (!Array.isArray(witnesses)) {
      issues.push({
        path: `${path}.witnesses`,
        message: 'witnesses must be an array when provided',
      });
    } else {
      witnesses.forEach((witness, index) => {
        if (!isRecord(witness)) {
          issues.push({
            path: `${path}.witnesses[${index}]`,
            message: 'witness must be an object',
          });
          return;
        }
        if (!isString(witness.id)) {
          issues.push({
            path: `${path}.witnesses[${index}].id`,
            message: 'witness id is required',
          });
        }
        if (!isString(witness.role)) {
          issues.push({
            path: `${path}.witnesses[${index}].role`,
...
function validateAuditTrail(
  value: unknown,
  path = 'auditTrail'
): ValidationResult<AuditTrailEntry[]> {
  const issues: ValidationIssue[] = [];
  ...
  value.forEach((entry, index) => {
    ...
    if (!isString(entry.timestamp)) {
      issues.push({
        path: `${path}[${index}].timestamp`,
        message: 'audit entry timestamp is required',
      });
    }
...
export function validateSpatialLegalContract(
  input: unknown
): ValidationResult<SpatialLegalContract> {
  const issues: ValidationIssue[] = [];
