...
export function buildHardwareReceipt(
  ...
  return buildDomainSimulationReceipt({
    ...
    resultSummary: {
      impedanceOhms: result.impedance?.impedanceOhms ?? null,
      irDropMv: result.irDrop?.irDropMv ?? null,
      junctionTempC: result.thermal?.junctionTempC ?? null,
      dfmFailures: result.dfm?.failures ?? null,
