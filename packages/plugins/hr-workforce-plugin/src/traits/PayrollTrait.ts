...
export function createPayrollHandler(): TraitHandler<PayrollConfig> {
  return {
    ...
    onAttach(n: HSPlusNode, c: PayrollConfig, ctx: TraitContext) {
      ...
      n.__payrollState = {
        grossPay: gross,
        netPay: gross - tax - benefits,
        taxWithheld: tax,
        benefitsDeducted: benefits,
        ytdGross: 0,
      };
      ctx.emit?.('payroll:configured', { frequency: c.frequency, grossPerPeriod: gross });
    },
    onDetach(n: HSPlusNode, _c: PayrollConfig, ctx: TraitContext) {
      delete n.__payrollState;
      ctx.emit?.('payroll:removed');
    },
    onUpdate() {},
    onEvent(n: HSPlusNode, _c: PayrollConfig, ctx: TraitContext, e: TraitEvent) {
      const s = n.__payrollState as PayrollState | undefined;
      if (!s) return;
      if (e.type === 'payroll:process') {
        s.ytdGross += s.grossPay;
        ctx.emit?.('payroll:processed', { net: s.netPay, ytdGross: s.ytdGross });
      }
    },
  };
}
