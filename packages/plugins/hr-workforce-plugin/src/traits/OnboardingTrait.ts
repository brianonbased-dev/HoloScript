...
export function createOnboardingHandler(): TraitHandler<OnboardingConfig> {
  return {
    name: 'onboarding',
    defaultConfig,
    onAttach(n: HSPlusNode, c: OnboardingConfig, ctx: TraitContext) {
      n.__onboardState = {
        completedSteps: 0,
      ...
      ctx.emit?.('onboarding:started', { steps: c.steps.length });
    },
    onDetach(n: HSPlusNode, _c: OnboardingConfig, ctx: TraitContext) {
      delete n.__onboardState;
      ctx.emit?.('onboarding:cancelled');
    },
    onUpdate() {},
    onEvent(n: HSPlusNode, c: OnboardingConfig, ctx: TraitContext, e: TraitEvent) {
      const s = n.__onboardState as OnboardingState | undefined;
      if (!s) return;
      if (e.type === 'onboarding:complete_step') {
        ...
        if (step && !step.completedAt) {
          ...
          s.completedSteps++;
          s.percentComplete = (s.completedSteps / s.totalSteps) * 100;
          if (s.completedSteps === s.totalSteps) {
            s.isComplete = true;
            ctx.emit?.('onboarding:completed');
          } else
            ctx.emit?.('onboarding:step_done', { step: step.name, progress: s.percentComplete });
        }
      }
    },
  };
