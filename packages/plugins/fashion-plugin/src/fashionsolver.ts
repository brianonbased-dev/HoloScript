...
export function buildFashionReceipt(
  ...
  return buildDomainSimulationReceipt({
    ...
    resultSummary: {
      sizeRange: result.grading?.sizes.length,
      fabricWastePct: result.fabricWaste?.wastePct,
      colorHarmonyScore: result.colorHarmony?.harmonyScore,
      trendDirection: result.trend?.trend,
    },
