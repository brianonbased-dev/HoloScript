...
export interface TrafficFlowConfig {
  ...
  intersectionType: 'signalized' | 'roundabout' | 'stop_sign' | 'none';
}
...
const defaultConfig: TrafficFlowConfig = {
  roadType: 'local',
  lanesPerDirection: 1,
  speedLimitKmh: 50,
...
export function createTrafficFlowHandler(): TraitHandler<TrafficFlowConfig> {
  return {
    name: 'traffic_flow',
    defaultConfig,
    onAttach(n: HSPlusNode, c: TrafficFlowConfig, ctx: TraitContext) {
      n.__trafficState = {
        currentVolume: 0,
        levelOfService: 'A' as const,
        avgSpeedKmh: c.speedLimitKmh,
        congestionIndex: 0,
      };
      ctx.emit?.('traffic:initialized', { roadType: c.roadType });
    },
    onDetach(n: HSPlusNode, _c: TrafficFlowConfig, ctx: TraitContext) {
      delete n.__trafficState;
      ctx.emit?.('traffic:removed');
    },
    onUpdate(n: HSPlusNode, c: TrafficFlowConfig, ctx: TraitContext, _d: number) {
      const s = n.__trafficState as TrafficFlowState | undefined;
      if (!s) return;
    ...
    onEvent(n: HSPlusNode, _c: TrafficFlowConfig, ctx: TraitContext, e: TraitEvent) {
      const s = n.__trafficState as TrafficFlowState | undefined;
      if (!s) return;
      if (e.type === 'traffic:add_volume') {
        s.currentVolume += (e.payload?.vehicles as number) ?? 0;
        ctx.emit?.('traffic:volume_updated', { volume: s.currentVolume, los: s.levelOfService });
      }
    },
  };
}
