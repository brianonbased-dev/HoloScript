...
export interface PopulationDensityState {
  densityPerKm2: number;
  projectedPopulation: number;
  housingDeficit: number;
}

const defaultConfig: PopulationDensityConfig = {
  populationCount: 0,
  areaKm2: 1,
  growthRatePercent: 1.5,
  ageDistribution: {},
  housingUnits: 0,
  ...
  return {
    ...
    onAttach(n: HSPlusNode, c: PopulationDensityConfig, ctx: TraitContext) {
      ...
      n.__popState = {
        ...
        projectedPopulation: c.populationCount,
        housingDeficit: Math.max(0, neededUnits - c.housingUnits),
      };
      ctx.emit?.('population:loaded', {
        density,
        deficit: Math.max(0, neededUnits - c.housingUnits),
      });
    },
    onDetach(n: HSPlusNode, _c: PopulationDensityConfig, ctx: TraitContext) {
      delete n.__popState;
      ctx.emit?.('population:removed');
    },
    onUpdate() {},
    onEvent(n: HSPlusNode, c: PopulationDensityConfig, ctx: TraitContext, e: TraitEvent) {
      const s = n.__popState as PopulationDensityState | undefined;
      if (!s) return;
      if (e.type === 'population:project') {
        ...
        ctx.emit?.('population:projected', {
          years,
          population: Math.round(s.projectedPopulation),
          density: Math.round(s.densityPerKm2),
        });
      }
    },
  };
