...
export interface ZoningState {
  currentUsage: ZoneType;
  complianceScore: number;
  violations: string[];
}

const defaultConfig: ZoningConfig = {
...
export function createZoningHandler(): TraitHandler<ZoningConfig> {
  return {
    name: 'zoning',
    defaultConfig,
    onAttach(n: HSPlusNode, c: ZoningConfig, ctx: TraitContext) {
      n.__zoningState = { currentUsage: c.zoneType, complianceScore: 100, violations: [] };
      ctx.emit?.('zoning:classified', { type: c.zoneType, density: c.density });
    },
    onDetach(n: HSPlusNode, _c: ZoningConfig, ctx: TraitContext) {
      delete n.__zoningState;
      ctx.emit?.('zoning:removed');
    },
    onUpdate() {},
    onEvent(n: HSPlusNode, c: ZoningConfig, ctx: TraitContext, e: TraitEvent) {
      const s = n.__zoningState as ZoningState | undefined;
      if (!s) return;
      ...
      if (e.type === 'zoning:check_compliance') {
        ...
        s.violations = [];
        if (height > c.maxHeightM)
          s.violations.push(`Height ${height}m exceeds max ${c.maxHeightM}m`);
        s.complianceScore =
          s.violations.length === 0 ? 100 : Math.max(0, 100 - s.violations.length * 25);
        ctx.emit?.('zoning:compliance_result', {
