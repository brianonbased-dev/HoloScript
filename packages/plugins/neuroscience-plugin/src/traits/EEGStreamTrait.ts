...
const defaultConfig: EEGStreamConfig = {
...
export function createEEGStreamHandler(): TraitHandler<EEGStreamConfig> {
  return {
    ...
    onAttach(node: HSPlusNode, config: EEGStreamConfig, ctx: TraitContext) {
      ...
      ctx.emit?.('eeg_stream:attached', {
        channels: config.channels.length,
        rate: config.samplingRate,
      });
    },
    onDetach(node: HSPlusNode, _c: EEGStreamConfig, ctx: TraitContext) {
      delete node.__eegState;
      ctx.emit?.('eeg_stream:detached');
    },
    onUpdate(node: HSPlusNode, config: EEGStreamConfig, ctx: TraitContext, delta: number) {
      ...
      if (s.samplesCollected >= samplesPerEpoch) {
        s.currentEpoch++;
        s.samplesCollected = 0;
        ctx.emit?.('eeg_stream:epoch', { epoch: s.currentEpoch, bandPowers: s.bandPowers });
      }
    },
    onEvent(node: HSPlusNode, _c: EEGStreamConfig, ctx: TraitContext, event: TraitEvent) {
      ...
      if (event.type === 'eeg_stream:start') {
        s.isStreaming = true;
        ctx.emit?.('eeg_stream:started');
      }
      if (event.type === 'eeg_stream:stop') {
        s.isStreaming = false;
        ctx.emit?.('eeg_stream:stopped');
      }
    },
  };
}
