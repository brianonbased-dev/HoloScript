/**
 * Universal synchronous SHA-256 for UAAL receipt hashing.
 *
 * This is the same FIPS 180-4 implementation family used by the engine's
 * contract hashing path. It intentionally has no Node or Web Crypto
 * dependency so bytecode receipts are available in Node and browsers through
 * the same synchronous VM API.
 */
...
/** Return the lowercase SHA-256 hex digest for the provided bytes. */
export function sha256Bytes(bytes: Uint8Array): string {
  ...
  const bitLengthHigh = Math.floor((bytes.length * 8) / 0x100000000) >>> 0;
