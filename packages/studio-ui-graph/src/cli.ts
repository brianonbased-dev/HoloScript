...
async function main(): Promise<number> {
  ...
  const output = resolve(repoRoot, args.output ?? join(studioRoot, '.holo', 'studio.ui.holo'));
  ...
  // Studio's tsconfig "@/*" alias points at packages/studio/src/*
  ...
  for (const page of pages) {
    i++;
    if (!args.quiet && (i === 1 || i % 10 === 0 || i === pages.length)) {
  ...
  writeFileSync(output, holo);
  log(`  wrote ${output} (${holo.length} bytes, ${uniqueComponents} unique components)\n`);
  return 0;
}
