...
class TimedAgent extends BaseAgent {
  readonly identity: AgentIdentity = {
    id: 'timed-001', name: 'TimedAgent', domain: 'testing', version: '2.0.0',
  ...
  async intake(ctx: Record<string, unknown>) {
    ...
    return { phase: ProtocolPhase.INTAKE, status: 'success' as const, data: { ...ctx, enriched: true }, durationMs: 0, timestamp: Date.now() };
  }
  async reflect(data: unknown) {
    this.phaseLog.push('REFLECT');
    return { phase: ProtocolPhase.REFLECT, status: 'success' as const, data: { plan: 'analysed', input: data }, durationMs: 0, timestamp: Date.now() };
  }
  async execute(plan: unknown) {
    this.phaseLog.push('EXECUTE');
    return { phase: ProtocolPhase.EXECUTE, status: 'success' as const, data: { completed: true, plan }, durationMs: 0, timestamp: Date.now() };
  }
  async compress(results: unknown) {
    this.phaseLog.push('COMPRESS');
    return { phase: ProtocolPhase.COMPRESS, status: 'success' as const, data: { compressed: true }, durationMs: 0, timestamp: Date.now() };
  }
  async reintake(compressed: unknown) {
    this.phaseLog.push('REINTAKE');
    return { phase: ProtocolPhase.REINTAKE, status: 'success' as const, data: { reabsorbed: true }, durationMs: 0, timestamp: Date.now() };
  }
  async grow(learnings: unknown) {
    this.phaseLog.push('GROW');
    return { phase: ProtocolPhase.GROW, status: 'success' as const, data: { patterns: 3, wisdom: 2 }, durationMs: 0, timestamp: Date.now() };
  }
  async evolve(adaptations: unknown) {
    this.phaseLog.push('EVOLVE');
    return { phase: ProtocolPhase.EVOLVE, status: 'success' as const, data: { evolved: true }, durationMs: 0, timestamp: Date.now() };
  }
}
...
class MetricsService extends BaseService {
  ...
  async onInit() { this.initCount++; }
  async onReady() { this.readyCount++; }
  async onStop() { this.stopCount++; }
}
...
describe('BaseAgent (extended)', () => {
  ...
  beforeEach(() => { agent = new TimedAgent(); });
  ...
  it('passes data between phases (intake → reflect → execute)', async () => {
    ...
    expect(agent.phaseLog).toEqual(['INTAKE', 'REFLECT', 'EXECUTE', 'COMPRESS', 'REINTAKE', 'GROW', 'EVOLVE']);
  });
...
describe('BaseService (extended)', () => {
  ...
  it('executeWithMetrics records errors', async () => {
    await svc.initialize();
    await expect(svc.executeWithMetrics(async () => { throw new Error('boom'); })).rejects.toThrow('boom');
...
describe('ServiceError (extended)', () => {
  ...
  it('creates error with details', () => {
    const err = new ServiceError(ServiceErrorCode.NOT_FOUND, 'Agent not found', 404, { agentId: 'xyz' });
...
describe('PWG Knowledge Format (extended)', () => {
  it('pattern with all fields', () => {
    const now = Date.now();
    const p: Pattern = {
      id: 'P.STUDIO.01', domain: 'studio', problem: 'Panel communication',
      solution: 'Event bus', context: 'React panels', tags: ['studio', 'react', 'events'],
  ...
  it('wisdom with insight', () => {
    const w: Wisdom = {
      id: 'W.COMPILER.01', domain: 'compiler', insight: 'Always bypass RBAC for Studio demo mode',
      context: 'Studio integration', tags: ['compiler', 'bypass'],
      source: 'experience', createdAt: Date.now(),
    };
  ...
  it('gotcha with severity', () => {
    const g: Gotcha = {
      id: 'G.API.01', domain: 'api', mistake: 'Using wrong constructor args',
      fix: 'Check API signatures', severity: 'high',
      tags: ['api'], createdAt: Date.now(),
    };
...
describe('Protocol Enums (extended)', () => {
  it('all 7 phases mapped in PHASE_NAMES', () => {
    const phases = [
      ProtocolPhase.INTAKE, ProtocolPhase.REFLECT, ProtocolPhase.EXECUTE,
      ProtocolPhase.COMPRESS, ProtocolPhase.REINTAKE, ProtocolPhase.GROW, ProtocolPhase.EVOLVE,
    ];
  ...
  it('all ServiceLifecycle values', () => {
    const states = [
      ServiceLifecycle.INITIALIZING, ServiceLifecycle.READY, ServiceLifecycle.DEGRADED,
      ServiceLifecycle.STOPPING, ServiceLifecycle.STOPPED, ServiceLifecycle.ERROR,
    ];
...
describe('MicroPhaseDecomposer (extended)', () => {
  it('creates execution plan with dependencies', () => {
    ...
    decomposer.registerTask({ id: 'A', name: 'Parse', estimatedDuration: 50, dependencies: [], execute: async () => {} });
    decomposer.registerTask({ id: 'B', name: 'Compile', estimatedDuration: 100, dependencies: ['A'], execute: async () => {} });
    decomposer.registerTask({ id: 'C', name: 'Link', estimatedDuration: 30, dependencies: ['B'], execute: async () => {} });
  ...
  it('parallelizes independent tasks', () => {
    const decomposer = new MicroPhaseDecomposer();
    decomposer.registerTask({ id: 'A', name: 'Task A', estimatedDuration: 100, dependencies: [], execute: async () => {} });
    decomposer.registerTask({ id: 'B', name: 'Task B', estimatedDuration: 100, dependencies: [], execute: async () => {} });
    decomposer.registerTask({ id: 'C', name: 'Task C', estimatedDuration: 50, dependencies: ['A', 'B'], execute: async () => {} });
