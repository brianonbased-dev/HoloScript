...
2. **Native = sovereign, never bridge/poison.** "Native render" means the **sovereign**
   ...
   Unity / Unreal / Godot are _bridge_ targets** — valuable interop, but they make the
   render depend on a third-party engine. Reaching for them to satisfy a _native_ claim is
...
canonical example: the _same_ declaration carries `model: "models/boulder.glb"` (realism:
...
right; SIM makes it _behave_ right and _prove_ it. SRNH is the doctrine for keeping both halves
real.
...
(`GaussianSplatBakingPipeline.ts`). And the `WebGPUCompiler`'s _inline_ splat shader is a
...
- **Executor gaps** — depth-dependent effects sample a depth texture at `@binding(3)` that the
  ...
  (`packages/r3f-renderer/src/components/PostProcessingRenderer.tsx`) _is_ wired into the
...
Realism needs realistic _input_. The honest state: **two real doors, several honest gaps.**
...
as realistic — they are primitive placeholders or heuristic scaffolds _by construction_. The
...
| **Simulation Contract / ESC**                 | **REAL, rigorous** — `verified` is _derived_; fake-proof clauses throw at construction; mesh-sanity beyond hash | `engine/src/simulation/SimulationContract.ts:1833,892,1198`                           |
...
| CAEL default hash strength                    | **PARTIAL** — FNV-1a (tamper-_evident_); SHA-256 opt-in                                                         | `SimulationContract.ts:971`                                                           |
...
that the physics is _correct_. The Simulation Contract (mesh-sanity + falsifiable clauses) is
...
These are the concrete, evidence-backed next moves to close the gap between _authored_ realism
(`realistic-forest.refreshed.holo` is rich) and _rendered_ realism (the sovereign preview is
...
- **B1 — Fix the sovereign WebGPU emit (4 bugs, §2.2).** Emit the geometry generators; always
  ...
  render in headless Chrome. _Smallest, highest-leverage block._
...
- **B3 — Wire the real 3DGS renderer into the preview.** The inline splat shader is a toy; the
  compiler already delegates _sorted_ splats to `GaussianSplatSorter` — make the preview serve
...
- **B6 — Name the realistic-asset door.** Either stand up the sovereign-3d generation backend,
  or adopt Meshy/Tripo as the _declared, key-gated_ realistic-asset path — and stop letting
...
- [ ] **No "compiled OK ⇒ real" overclaim?** A passing compile/test proves the scaffold, not a
      wired photoreal render or correct physics. Verify by _looking_ / _replaying_.
