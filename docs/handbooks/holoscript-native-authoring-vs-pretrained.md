...
> so the pretrained reflex produces code that *compiles and passes a generic lint* but is
...
> i.e. ones a competent engineer would get *wrong* by following pretrained instinct.
>
...
The pretrained reflex is to *write the behavior*. The native way is to *declare the behavior
as a structure* the compiler/runtime/router reads. A trait is a handler-object, not a class
...
`verified` is *derived*, never declared. A fake-proof clause **throws** at construction. A
...
A computation is obligated to emit a tamper-evident, re-runnable receipt *as part of its return
value*. `solve_*` returns `{ result, traceJSONL, caelTraceId, verifyUrl }` — number and receipt
...
evaluates precondition clauses, and throws on error-severity violations *before the solver runs*
...
the only native nuance is that `escapeStringValue` is a *mandated* `CompilerBase` method, not
