...
| `examples/hololand/vv-results-visualization-space.holo` | .holo | aspirational grammar | pass | Uses HoloLand V&V visualization grammar with nested post-processing and dashboard constructs not yet accepted by the parser. | Public link requires aspirational label |
...
| `examples/novel-use-cases/05-robot-training-metaverse.hs` | .hs | aspirational grammar | pass | Uses on_error(...) pipeline handler syntax; PipelineParser does not accept that handler form yet. | Public link requires aspirational label |
| `examples/novel-use-cases/05-robot-training-metaverse.hsplus` | .hsplus | aspirational grammar | pass | Uses arrow transition shorthand in state-machine maps; HoloScriptPlusParser does not accept that shorthand yet. | Public link requires aspirational label |
| `examples/novel-use-cases/13-disaster-robotics-swarm.holo` | .holo | aspirational grammar | pass | Uses full disaster-training scene grammar that is ahead of the current .holo parser. | Public link requires aspirational label |
| `examples/novel-use-cases/13-disaster-robotics-swarm.hs` | .hs | aspirational grammar | pass | Uses dotted property expressions in pipeline properties; PipelineParser does not accept that expression shape yet. | Public link requires aspirational label |
| `examples/novel-use-cases/13-disaster-robotics-swarm.hsplus` | .hsplus | aspirational grammar | pass | Uses arrow transition shorthand in behavior state maps; HoloScriptPlusParser does not accept that shorthand yet. | Public link requires aspirational label |
| `examples/physics/advanced-physics-showcase.holo` | .holo | aspirational grammar | pass | Uses nested post_processing blocks and advanced physics traits ahead of the current parser grammar. | Public link requires aspirational label |
...
| `examples/sample-projects/physics-playground.holo` | .holo | aspirational grammar | pass | Uses nested post_processing blocks such as bloom { ... }; HoloCompositionParser currently expects property-value pairs there. | Public link requires aspirational label |
| `examples/showcase/physics-playground.holo` | .holo | aspirational grammar | pass | Uses showcase-scale physics grammar beyond the current declarative parser surface. | Public link requires aspirational label |
