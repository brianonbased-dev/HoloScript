...
| ---------------------------- | ------------------------------ | ------------------------------------------------------------ |
| **Human Intelligence**       | Brittney / InfinityAssistant   | _(External services per ecosystem map)_                      |
...
1. `holo_graph_status`
2. `holo_absorb_repo` _(force=false unless explicitly corrupt)_
3. `holo_query_codebase` / `holo_ask_codebase` / `holo_impact_analysis` / `holo_semantic_search`

_Operational Note: Production absorb operates at `https://absorb.holoscript.net` to leverage cache and shared resources._
...
2. **Internal Spatial MCP** (`packages/core/src/mcp/`): Used _inside_ 3D simulations for agent-to-agent protocol communication and world-building logic.
...
_Note on Analytics & Metrics_: Never hardcode entity counts (e.g. number of traits, endpoints, plugins) in prose. Always derive live dynamic counts per the commands listed in `docs/NUMBERS.md` or live `/health` endpoints.
