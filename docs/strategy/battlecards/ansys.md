...
| ---------------------------------------- | ----------------------------------------------------------- | ----------------------------------------- |
| **Browser-native WebGPU simulation**     | Zero install, anywhere, any device                          | NAFEMS LE1 σ_yy 1.2%, quadratic TET10     |
...
- **"HoloScript doesn't have ANSYS's validation library"** → "Correct, and it is not close. ANSYS has thousands of validated cases. Our one structural benchmark is NAFEMS LE1, and it is honest about its own limits: quadratic TET10 reaches σ_yy within 1.2% of the 92.7 MPa reference at the finest mesh we run, with a grid-convergence index of 1.8%; linear TET4 does not converge on this problem at all — 83% error at that same mesh — and the paper still lists the element-averaged variant of LE1 as open. Our own risk register says credible V&V needs 50+ benchmarks per domain against the 3 we have. What we have that they do not is a self-contained trace receipt on every result: anyone can post it to our public verify endpoint and get back an independent hash-chain and deterministic-replay verdict, without installing our software. For regulated workflows, use ANSYS. Where the argument is about reproducibility rather than breadth of validation, that is ours." **Do not claim peer review.** The IEEE TVCG submission was desk-rejected and the revision is not evidenced as resubmitted (`docs/public/papers-status.json`, row TVCG); `docs/paper-program-status.md` carries the standing instruction "do not anchor until revision locks". This line said "a peer-reviewed paper (IEEE TVCG 2026)" until this correction — false twice over, since a desk rejection is a rejection *without* peer review, and it was the script someone reads aloud when a buyer challenges our validation. Saying the true version is also the stronger position, because a buyer can check it — so the numbers above are the ones our own test prints, including the one that is bad.
