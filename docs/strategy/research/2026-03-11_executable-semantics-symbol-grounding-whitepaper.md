...
The declaration `mass: 5.0` is not a comment, a label, or a user interface annotation. It is the _only_ source of truth for this object's inertia across all 25 compile targets.
...
The same `compilePhysicsBlock()` intermediate representation also feeds `VRChatCompiler.ts`, demonstrating that the semantic grounding is target-agnostic. The symbol `@physics` does not mean "Unity physics" or "URDF physics" — it means _physics_, and the compiler handles the target-specific encoding.
...
[6] Goodfellow, I., et al. (2020). _Generative Adversarial Networks_ and the Synthetic Data Problem. _Communications of the ACM_, 63(11), 139–144.
