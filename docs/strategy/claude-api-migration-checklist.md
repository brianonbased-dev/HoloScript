...
### Architecture-level (RULED 2026-05-12 via `/founder` skill — all closed, no research needed)

- [x] **Managed Agents** (`managed-agents-2026-04-01`) — **RULED: do not replace, do not build alternate**. NMoS bar test (F.046) returns 0/5: no scale we can't replicate, no network effect, no open-standard fork-risk, no gated distribution, no specialized depth we don't want. Sovereign-is-default per W.GOLD.026 + D.026. Our headless runner enforces identity-grade per-seat custody (W.GOLD.004 wallets-are-identity); Anthropic-managed bearers are session-grade — wrong threat model per W.GOLD.193. Memory Stores duplicate the 4-tier knowledge pipeline (MEMORY → room → workspace → GOLD) at lower fidelity. Vision pillar #2 (zero domain vocabulary in core) — Managed Agents imports Anthropic's notion of "what an agent is" into our runtime semantics, which is domain leakage. Alternate-runtime `packages/holoscript-agent-managed/` rejected as workaround + priority demote for zero current callers. Revisit only if a paying customer specifically demands Managed Agents semantics. **Task `task_1778558872555_uz8a` CLOSED.**
- [x] **Claude Security beta** — **RULED: do not enroll**. Wrong threat model. Generic vuln scanning (OWASP web patterns, node_modules CVEs) doesn't know HoloScript's lexical surface — our threats are lexical-firewall bypass (W.GOLD.035), Sapir-Whorf escapes (W.GOLD.039), trust injection (Paper 21 → USENIX Security 2027). Coverage estimate: ~20% overlap with `/security-audit` skill, ~0% on Paper 21 surface. Cost: Enterprise tier upgrade (unverified), new auth surface (F.001), third-party code-access compliance per W.GOLD.193. F.037 (papers are the product) — `/security-audit` hardening + Paper 21 PoC scaffolding strictly dominates outsourcing a fraction of the surface to a wrong-threat-model vendor. Revisit only if an enterprise customer specifically demands SOC2 third-party vuln scanning evidence. **Task `task_1778558960041_x447` CLOSED.**
- [x] **Claude Platform on AWS** (launched 2026-05-11) — **RULED: defer**. Speculative research with no current need. We run on Railway + direct Anthropic; AWS routing means NEW vendor surface (cost markup, new IAM credentials per F.001, AWS lock-in expansion). Benefits (AWS billing, IAM auth) we don't need — we have x402 + per-seat wallet custody. F.045 vendor vitality applies to us-depending-on-vendor, not Anthropic-distribution-routing. Vision pillar #6 (production is the product) — our production works. Revisit when Railway → AWS migration is contemplated for unrelated reasons OR a specific enterprise customer requires it. **Task `task_1778559016042_dcu0` CLOSED.**
