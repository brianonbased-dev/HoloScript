...
### 3-Link Arm Robot — Regression Fixture

The following is the canonical round-trip fixture used in `URDFCompiler.articulation.test.ts`
and `USDPhysicsCompiler` articulation tests. It validates that `@joint` and `@physics` traits
in parser-output format (`{type:'ObjectTrait', name:'joint', config:{...}}`) correctly produce
revolute joints with drives and correct masses rather than degenerate fixed joints.

HoloScript source (3 links / 2 revolute joints):
...
Expected USD output (`compile_to_usd targetContext=isaac_sim enableArticulation=true`):

```usda
#usda 1.0
(
    defaultPrim = "ArmRobot"
    doc = "Generated by HoloScript USDPhysicsCompiler for NVIDIA Isaac Sim / Isaac Lab"
    metersPerUnit = 1
    upAxis = "Z"
    timeCodesPerSecond = 60
...
)
{
    ...
    def PhysicsRevoluteJoint "base_to_upper_arm_joint"
    {
        rel physics:body0 = </ArmRobot/base>
        rel physics:body1 = </ArmRobot/upper_arm>
        uniform token physics:axis = "Z"
    }

    def PhysicsRevoluteJoint "upper_arm_to_forearm_joint"
    {
        rel physics:body0 = </ArmRobot/upper_arm>
        rel physics:body1 = </ArmRobot/forearm>
        uniform token physics:axis = "Y"
    }
}
```
...
- Mass values from `@physics` trait: `base=1.0`, `upper_arm=0.5`, `forearm=0.3`
