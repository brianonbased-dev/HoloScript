...
**Vector tools**:
4. `upstash_vector_upsert` - Add composition embeddings
5. `upstash_vector_search` - Find similar by vector
6. `upstash_vector_search_text` - Find similar by natural language
7. `upstash_vector_fetch` - Get composition by ID
8. `upstash_vector_delete` - Remove composition
9. `upstash_vector_info` - Index statistics

**QStash tools**:
10. `upstash_qstash_schedule` - Create cron job
11. `upstash_qstash_publish` - One-time message
12. `upstash_qstash_list` - List schedules
13. `upstash_qstash_get` - Get schedule details
14. `upstash_qstash_delete` - Delete schedule
15. `upstash_qstash_pause/resume` - Pause/resume schedule
16. `upstash_qstash_dlq_list/delete` - DLQ management

**Convenience tools**:
17. `upstash_schedule_nightly_compilation` - Nightly build setup
18. `upstash_schedule_health_ping` - Health monitoring
19. `upstash_trigger_deployment` - CI/CD integration
