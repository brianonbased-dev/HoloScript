...
async function main() {
  ...
  const DW = m.frames[0].depthWidth,
    DH = m.frames[0].depthHeight;
  // intrinsics scaled to depth resolution
  const sxk = DW / imageWidth,
    syk = DH / imageHeight;
  const fxd = fx * sxk,
    fyd = fy * syk,
    cxd = cx * sxk,
  ...
  for (const f of m.frames) {
    ...
    const rgb = await sharp(join(baseDir, f.jpeg))
  ...
  const N = xs.length;
  if (N === 0) {
    console.error('[fuse] no points produced');
    process.exit(1);
  }
  ...
  let o = 0;
  const put = (val: number) => {
    body.writeFloatLE(val, o);
    o += 4;
  };
  for (let i = 0; i < N; i++) {
    put(xs[i]);
  ...
  for (let i = 0; i < g3.N; i++) {
    minX = Math.min(minX, g3.x[i]);
    maxX = Math.max(maxX, g3.x[i]);
    minY = Math.min(minY, g3.y[i]);
    maxY = Math.max(maxY, g3.y[i]);
    minZ = Math.min(minZ, g3.z[i]);
    maxZ = Math.max(maxZ, g3.z[i]);
  }
  console.log(
    `[fuse] frames=${m.frames.length} stride=${stride} -> ${N.toLocaleString()} gaussians, ${(ply.length / 1e6).toFixed(1)} MB`
  );
  console.log(`[fuse] loader round-trip: parsed N=${g3.N}`);
  console.log(
    `[fuse] bbox extent (m): ${(maxX - minX).toFixed(2)} x ${(maxY - minY).toFixed(2)} x ${(maxZ - minZ).toFixed(2)}`
  );
  console.log(
    `[fuse] sample pos[0]=(${g3.x[0].toFixed(2)},${g3.y[0].toFixed(2)},${g3.z[0].toFixed(2)}) color0=(${g3.r[0].toFixed(2)},${g3.gr[0].toFixed(2)},${g3.bl[0].toFixed(2)})`
  );
