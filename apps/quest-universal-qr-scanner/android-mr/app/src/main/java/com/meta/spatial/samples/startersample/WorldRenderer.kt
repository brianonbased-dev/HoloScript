 ...
 *
 * Builds a simple themed 3D world from a world NAME and tears it down: a colored skybox surround, a
 * ground slab, and a ring of landmark primitives (spheres + pillars). Everything is procedural —
 * color theme, landmark count, sizes and placement are derived from the name's hash, so each world
 * looks distinct with NO bundled assets. Meshes use the Spatial SDK built-in geometry
 * (mesh://skybox, mesh://sphere, mesh://box) with unlit flat-color Materials (always visible, no IBL
 * cubemap needed). A skybox entity occludes passthrough — that, plus enablePassthrough(false) in the
 * activity, is what makes "entering a world" replace the real room.
 */
...
class WorldRenderer {
  ...
  /** Remove all world entities (restores passthrough once the skybox is gone). */
  fun leave() {
