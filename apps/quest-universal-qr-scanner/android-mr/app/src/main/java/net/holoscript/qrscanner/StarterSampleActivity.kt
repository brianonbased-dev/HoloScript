...
class StarterSampleActivity : AppSystemActivity() {
  ...
  private fun onDecoded(text: String) {
    ...
    // A HoloScript world link takes precedence (its own @world_portal patterns) — auto-immerse or offer
    // an "Enter world" card.
    if (WorldPortal.isWorldLink(text)) {
    ...
    // from scanner.holo's @qr_decode.content_types and present the typed result + the right action.
    val c = classifyContent(text)
    ScannerState.resultKind = c.kind
    ScannerState.resultLabel = c.label
    if (c.action == QrAction.OPEN) {
