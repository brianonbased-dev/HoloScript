...
> **`PORT=7411` is required and was missing from this command until 2026-08-03.** `http-server.ts` defaults to `process.env.PORT || '3000'`, so the bare command starts the server on **3000** while every doc, probe and recovery step here targets **7411**. The result was silent and repo-wide: agents probed 7411, got `ECONNREFUSED`, concluded "local MCP is down", and fell through to the cloud instance — which is FS-blind to this machine and answers *successfully* about its own container `/tmp`, complete with a stale graph carrying `cache_root_mismatch`. A confidently wrong answer about the wrong filesystem is worse than a refusal. If you ever see codebase-intelligence results that do not match the working tree, check which endpoint answered first.
