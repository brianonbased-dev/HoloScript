 ...
 *
 * Ported from packages/studio/src/components/scene/CompiledLotusMeshNode.tsx —
 * 'use client' removed (Vite, not Next.js); @/types → @holoscript/core.
 */
...
export function CompiledLotusMeshNode({ node }: { node: R3FNode }) {
  ...
  const ringMats = useMemo(() => {
    ...
    const make = (factor: number, ring: number) => {
      ...
      // FLICKER FIX: petals are SOLID surfaces (they transmit light + briefly fade in,
      // they are not glass), so writing depth means the nearer petal wins the depth test
      // per-pixel regardless of centroid sort order — flicker eliminated.
      m.depthWrite = true;
      // RESIDUAL-FLICKER FIX: drop the PHYSICAL transmission for this many-overlapping-
      // instances render. The petal spec is physically honest but transmission>0 forces
      // three.js's transmission PASS, causing shimmer. The backlit subsurface LOOK is
      // authored in the petal's own custom shader (fragmentEmissiveInjection), unaffected.
      m.transmission = 0;
      // Per-ring depth bias so near-coplanar whorls (inter-ring) don't z-fight.
  ...
  // generic `group` nodes carrying `__animatedTransform` channels, grown by the
  // generic AnimatedTransformGroup. The PETAL growth below is the compiled
  // maturation-front model, played by ramping its developmental clock.
  const bloomRef = useRef<THREE.Group>(null);
  const petalRefs = useRef<Array<THREE.Group | null>>([]);

  // The growth CLOCK is declared in the `.holo` `timeline` block and played by the
  // generic <TimelineDriver>; we read its current value by target name. No clock,
  // duration, or curve lives in this component — only the mapping from developmental
  // time onto the compiled uniforms + scaffold. Fallback 1 = full bloom when no
  // timeline drives it.
  const petals: LotusPetalPlacement[] =
    placements.length > 0 ? placements : [{ azimuth: 0, tilt: 0, radius: 0, lift: 0, ring: 1 }];

  // Per-instance maturity: each lotus object can declare `growthCap` to cap where it
  // settles — 1.0 = full bloom, ~0.6 = half-open, ~0.3 = tight bud.
  const growthCap =
  ...
  useFrame((state) => {
    ...
    for (const ringStr of ['1', '2', '3'] as const) {
      ...
      h.setUniform('uLotusGrowth', smoothstep(0.4, 0.9, g));
