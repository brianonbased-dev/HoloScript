...
**An open, model-agnostic OS for agents and spatial computing.** Frontier models are becoming the primary way software gets built — but every model still rebuilds the same substrate underneath: the formats agents author in, the tools they call, the runtime that executes intent, and the targets that ship it. HoloScript is that substrate, built once and open (MIT). Any MCP-capable model — Claude, GPT, Gemini, Llama — connects through one surface, authors the same typed `.holo`/`.hsplus`/`.hs` source, and reaches 50+ compile targets. The model stays the intelligence; HoloScript is the OS it operates. It's model-agnostic by architecture (see [`@holoscript/llm-provider`](./packages/llm-provider/), which already unifies OpenAI · Anthropic · Gemini) and designed for any frontier lab to adopt and integrate.
