...
walk(join(root, 'packages'), (path) => {
  ...
  try {
    ...
    if (manifest.name) packages.push({ root: resolve(path, '..'), manifest });
  } catch {
...
for (const pkg of packages) {
  ...
  const files = [];
  for (const sourceRoot of ['src', 'bin']) {
    const path = join(pkg.root, sourceRoot);
    if (existsSync(path))
      walk(path, (file) => {
        if (sourceExtensions.has(extname(file))) files.push(file);
      });
  }
  for (const file of files) {
    ...
    for (const match of text.matchAll(
      ...
      findings.push({
        ...
        importer: relative(root, file).replace(/\\/gu, '/'),
        specifier,
        dependency: name,
        kind: workspaceNames.has(name)
          ? 'undeclared-workspace-import'
          : 'undeclared-external-import',
        action: 'file-or-declare-before-publishing',
      });
    }
  }
}
const result = {
  schema: 'holoscript.workspace.package-dependency-audit.v1',
  generatedAt: new Date().toISOString(),
  workspaceRoot: root.replace(/\\/gu, '/'),
  packageCount: packages.length,
  findingCount: findings.length,
  findings,
  policy:
    'Every imported package must be declared by the importing package or explicitly filed as an external peer boundary before public release.',
};
mkdirSync(join(root, 'reports'), { recursive: true });
writeFileSync(
  join(root, 'reports', 'package-dependency-audit.json'),
  ...
  JSON.stringify(
    {
      ...
      report: 'reports/package-dependency-audit.json',
    },
    null,
    2
  )
