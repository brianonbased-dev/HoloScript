#!/usr/bin/env python3
"""Fit one preregistered J-space S1 endpoint-affine lens.
...
FIT_RECEIPT_SCHEMA = "holoscript.jspace-s1-fit-receipt.v0.1.0"
FROZEN_LAYERS = [2, 5, 8]
...
def _load_model(checkpoint_path: Path, bins_dir: Path, device: str) -> tuple[GPT, dict[str, Any]]:
    ...
    if model_config["blockSize"] != FROZEN_MAX_SEQ_LEN:
        raise ValueError(
            f"checkpoint block size {model_config['blockSize']} does not match the frozen S1 limit"
        )
    model = GPT(
...
def _validate_inputs(args: argparse.Namespace) -> tuple[
    ...
    if (
        ...
        raise ValueError("fit inputs do not match the sealed S1 corpus manifest")
...
def fit(args: argparse.Namespace) -> None:
    if args.layers != FROZEN_LAYERS or args.dim_batch != FROZEN_DIM_BATCH:
        raise ValueError("layers and dim batch must match the frozen S1 preregistration")
