...
fs.writeFileSync(
  p2,
  c2
    .replace(/_isPointInSphere/g, 'isPointInSphere')
...
fs.writeFileSync(
  p4,
  c4
    .replace(/interpolate\(tDep\.conductivity, T\)/g, '(interpolate(tDep.conductivity, T) as any)')
    .replace(
      /interpolate\(tDep\.specific_heat, T\)/g,
...
fs.writeFileSync(
  p5,
  c5.replace(/force: \[number, number, number\]/g, 'force: [Force, Force, Force]')
);
