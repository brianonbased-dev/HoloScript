...
for (const [file, errs] of Object.entries(errorsByFile)) {
  ...
  for (const [lnumStr, lineErrors] of Object.entries(lineErrs)) {
    ...
    const hasObjectAssign = lineErrors.some(
      (e) =>
        e.msg.includes("is not assignable to type '[number, number, number]'") ||
        e.msg.includes('Object literal may only specify')
    );
    ...
    const hasArgTypeMismatch = lineErrors.some(
      (e) =>
        e.msg.includes(
          "Argument of type '[number, number, number]' is not assignable to parameter of type 'Vec3'"
        ) ||
        e.msg.includes(
          "is not assignable to parameter of type '{ x: number; y: number; z: number; }'"
        )
    );
    ...
    const hasIVector3 = lineErrors.some((e) =>
      e.msg.includes("missing the following properties from type 'IVector3'")
    );
    // same, manual.
