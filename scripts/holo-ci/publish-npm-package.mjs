 ...
 * from a Windows checkout then dies AFTER packing and passing its guards — the most expensive
 * place to fail, because everything looks healthy right up to it. --force-local tells tar the
 * colon is part of the filename. win32 only: GNU tar elsewhere does not need it, and BSD tar
 * does not accept it.
 */
