...
for (const file of unregistered) {
  ...
  for (const m of content.matchAll(
    /export\s+(const|class|interface|type|function|enum)\s+(\w+)/g
  )) {
  ...
  if (hasDefaultConfig && hasLifecycle)
    category = 'HANDLER_READY'; // Has handler shape, might not export correctly
    ...
    exports.every((e) => e.startsWith('const') || e.startsWith('type') || e.startsWith('interface'))
  )
    category = 'CONSTANTS';
  else category = 'MIXED';
...
for (const [cat, items] of Object.entries(groups).sort()) {
  ...
  for (const item of items) {
    console.log(
      `  ${item.file} (${item.lines}L) → ${item.exports.slice(0, 3).join(', ')}${item.exports.length > 3 ? '...' : ''}`
    );
  }
}
