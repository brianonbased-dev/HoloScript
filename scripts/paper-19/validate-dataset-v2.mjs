...
const SPLIT_TARGETS = { train: 0.7, dev: 0.15, test: 0.15 };
...
function assertSchema(row, idx, label) {
  ...
  if (!['train', 'dev', 'test'].includes(row.split))
    fail(`${label} ${row.id}: invalid split "${row.split}"`);
  if (!Array.isArray(row.gold_traits)) fail(`${label} ${row.id}: gold_traits must be array`);
  for (const t of row.gold_traits) {
    if (typeof t !== 'string' || !t.startsWith('@'))
      fail(`${label} ${row.id}: gold trait "${t}" must start with @`);
  }
  for (const k of ['source', 'lines', 'kind'])
    if (!(k in row.provenance)) fail(`${label} ${row.id}: provenance missing "${k}"`);
  if (!['verbatim', 'synth'].includes(row.provenance.kind))
    fail(`${label} ${row.id}: provenance.kind must be verbatim or synth`);
}
...
for (const [s, target] of Object.entries(SPLIT_TARGETS)) {
  ...
  if (Math.abs(actual - target) > SPLIT_TOLERANCE)
    fail(
      `split "${s}" = ${(actual * 100).toFixed(1)}%, target ${(target * 100).toFixed(0)}% +/- ${(SPLIT_TOLERANCE * 100).toFixed(0)}pp`
    );
}
console.log(
...
const novelTestRows = rows.filter(
  (r) => r.split === 'test' && r.metadata.novel_combination === true
);
...
console.log(
  `  [PASS] novel-combination test rows = ${novelTestRows.length} (>= ${MIN_NOVEL_COMBO_TEST})`
);
...
if (synthRatio > MAX_SYNTH_RATIO)
  fail(
    `synth ratio = ${(synthRatio * 100).toFixed(1)}%, must be <= ${(MAX_SYNTH_RATIO * 100).toFixed(1)}%`
  );
console.log(
  `  [PASS] synth ratio = ${(synthRatio * 100).toFixed(1)}% (verbatim=${verbatimRows.length}, synth=${synthRows.length})`
);

// 6. Synth strategies
const strategies = new Set();
for (const r of synthRows)
  if (r.provenance.synth_strategy) strategies.add(r.provenance.synth_strategy);
if (strategies.size < MIN_SYNTH_STRATEGIES)
  fail(`distinct synth strategies = ${strategies.size}, must be >= ${MIN_SYNTH_STRATEGIES}`);
console.log(
  `  [PASS] synth strategies = ${strategies.size} (${[...strategies].sort().join(', ')})`
);

// 7. Families
const fams = new Set();
for (const r of rows) for (const f of r.metadata.trait_families || []) fams.add(f);
if (fams.size < MIN_FAMILIES)
  fail(`trait family coverage = ${fams.size}, must be >= ${MIN_FAMILIES}`);
...
for (const r of rows) {
  ...
  if (idsByHash.has(h) && idsByHash.get(h) !== r.id)
    fail(`snippet collision: ${idsByHash.get(h)} and ${r.id}`);
...
console.log(
  `  [PASS] family map: ${familyMap.family_count} families, ${familyMap.distinct_trait_count} distinct traits`
);

// 10. Adversarial mislabel sibling
const mislabels = loadJsonl(MISLABEL_PATH, 'adversarial-mislabel');
if (mislabels.length < MIN_MISLABELS)
  fail(`adversarial-mislabel rows = ${mislabels.length}, must be >= ${MIN_MISLABELS}`);
mislabels.forEach((r, i) => assertSchema(r, i, 'mislabel'));
const mislabelKinds = new Set();
for (const r of mislabels) {
  if (!r.metadata.adversarial_mislabel)
    fail(`mislabel ${r.id}: missing metadata.adversarial_mislabel tag`);
...
console.log(
  `  [INFO] size buckets: zero=${buckets.zero} solo=${buckets.solo} two-to-three=${buckets['two-to-three']} four-plus=${buckets['four-plus']}`
);
...
for (const r of rows)
  for (const t of r.gold_traits) goldCounts.set(t, (goldCounts.get(t) ?? 0) + 1);
...
for (const r of rows)
  for (const t of r.metadata.uncategorized_traits || []) uncategorizedSet.add(t);
console.log(`  [INFO] uncategorized traits (no family in map): ${uncategorizedSet.size}`);

console.log('');
console.log(
  'VALIDATION PASS — v2 dataset clears all gates for Paper 19 phase-3 baseline + classifier work.'
);
