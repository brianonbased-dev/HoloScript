...
function loadRows() {
  ...
  const lines = readFileSync(JSONL_PATH, 'utf-8')
    .split('\n')
...
for (const n of negatives) {
  if (n.gold_traits.length !== 0)
    fail(
      `negative control ${n.id} must have empty gold_traits, got ${JSON.stringify(n.gold_traits)}`
    );
}
console.log(
  `  [PASS] ${negatives.length} negative-control rows (3-5 expected) all have empty gold_traits`
);
