...
2. Wire `isolate-spawn.mjs` into `.claude/settings.json` SessionStart:
   ```json
   { "hooks": [ { "type": "command", "command": "node C:/Users/Josep/.ai-ecosystem/hooks/run-hook.mjs sessionstart/agent-isolation-spawn.mjs" } ] }
   ```
...
The detection-bandaid hooks exist only because N agents share one mutable tree. Per-agent isolation removes that condition, so those hooks become dead code. The win is the *swap* (doctrine-enforcement replaces symptom-detection), not adding another layer.
