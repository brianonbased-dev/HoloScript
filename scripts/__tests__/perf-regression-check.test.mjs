...
console.log('Test 7: --update-baseline writes baseline.json from current results');
{
  ...
  assertEq(
    written.scenarios['scen-A']?.platforms?.unity?.compileTimeMs,
    50,
    'baseline carries compile time'
  );
  assertEq(
    written.scenarios['scen-A']?.platforms?.unity?.outputSizeBytes,
    500,
    'baseline carries size'
  );
  rmSync(repo, { recursive: true, force: true });
}
