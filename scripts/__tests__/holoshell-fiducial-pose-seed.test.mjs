...
assertEq(
  poseSeed.intrinsics.source,
  'estimated-fov-pinhole',
  'estimated intrinsics source recorded'
);
assertOk(poseSeed.poseSeed.reprojection.rmsPixels <= 1.5, 'pose seed reprojection RMS is low');
assertOk(
  poseSeed.poseSeed.cameraFromBoard.translationTargetPixels[2] > 0,
  'pose seed has positive camera depth'
);
...
assertOk(
  blocked.poseSeed.blockers.includes('board-homography-unavailable'),
...
assertEq(
  explicit.intrinsics.source,
  'explicit-pinhole-intrinsics',
  'explicit intrinsics source recorded'
);
assertEq(
  explicit.intrinsics.calibrationReady,
  false,
  'explicit intrinsics alone are not calibration'
);
assertEq(explicit.poseSeed.calibrationReady, false, 'explicit pose seed is not calibration');
assertOk(
  explicit.poseSeed.reprojection.rmsPixels <= 4,
  'explicit intrinsics reprojection RMS stays within gate'
);
...
assertOk(
  validateReceipt(overclaim).includes('pose seed must not claim camera calibration'),
