...
import {
  ...
  isGenericStub,
  hasRealLogic,
} from '../holo-ci/check-interaction-taste.mjs';
...
assert(
  isGenericStub('trigger "show_info_panel"').stub === true,
  'generic show_info_panel trigger is a stub'
);
assert(
  isGenericStub('  emit "show_details" ').stub === true,
  'generic show_details emit is a stub'
);
assert(
  isGenericStub('trigger "ignite"').stub === false,
  'a specific (non-generic) trigger is NOT a stub'
);
assert(
  isGenericStub('set glow: 4.0\ntrigger "ignite"').stub === false,
  'a multi-statement body is NOT a stub'
);

// hasRealLogic — object-specific behavior.
assert(
  hasRealLogic('set mantle_emissive: 4.0\ntrigger "ignite"') === true,
  'multi-statement body has real logic'
);
...
assert(
  hasRealLogic('trigger "show_info_panel"') === false,
  'a lone generic trigger has no real logic'
);
...
assert(
  c.profileMismatches.length === 0,
  '@interaction_profile(react_to:"dusk approach") is DELIVERED (no mismatch)'
);
...
assert(
  a.assembledHits.every((h) => h.generic === 'show_info_panel'),
  'each hit names the generic event show_info_panel'
);
