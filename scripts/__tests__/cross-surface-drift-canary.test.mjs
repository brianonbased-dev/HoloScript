...
// 1.2 AGENT_ACTION_KINDS ↔ isSupportedAgentActionKind
{
  ...
  assertOk(
    guardBody.includes('AGENT_ACTION_KINDS'),
    'isSupportedAgentActionKind references AGENT_ACTION_KINDS'
  );
}
...
// 1.3 QUALCOMM_NIR_RUNTIME_TARGETS ↔ isSupportedQualcommNIRRuntimeTarget
{
  ...
  assertOk(
    guardBody.includes('QUALCOMM_NIR_RUNTIME_TARGETS'),
    'isSupportedQualcommNIRRuntimeTarget references QUALCOMM_NIR_RUNTIME_TARGETS'
  );
}
...
// 1.4 HARDWARE_COMPILATION_TARGET_KINDS ↔ isSupportedHardwareCompilationTarget
{
  ...
  assertOk(
    guardBody.includes('HARDWARE_COMPILATION_TARGET_KINDS'),
    'isSupportedHardwareCompilationTarget references HARDWARE_COMPILATION_TARGET_KINDS'
  );
}
...
// 1.5 TwinEarthReceiptKind ↔ isSupportedTwinEarthReceiptKind
{
  ...
  const guardBody =
    twinEarthSource.match(/function isSupportedTwinEarthReceiptKind[\s\S]*?\{[\s\S]*?\n\}/)?.[0] ||
    '';
  ...
  assertOk(
    guardBody.includes("'validation'"),
    "isSupportedTwinEarthReceiptKind covers 'validation'"
  );
}
...
// 1.7 TwinEarthRole ↔ isSupportedTwinEarthRole
{
  ...
  const guardBody =
    twinEarthSource.match(/function isSupportedTwinEarthRole[\s\S]*?\{[\s\S]*?\n\}/)?.[0] || '';
...
// 1.8 ParticipationMode ↔ isSupportedParticipationMode
{
  ...
  const guardBody =
    twinEarthSource.match(/function isSupportedParticipationMode[\s\S]*?\{[\s\S]*?\n\}/)?.[0] || '';
...
// 1.9 TwinEarthKind ↔ isSupportedTwinEarthKind
{
  ...
  const guardBody =
    twinEarthSource.match(/function isSupportedTwinEarthKind[\s\S]*?\{[\s\S]*?\n\}/)?.[0] || '';
...
// 2.1 Every exported receipt interface has a validator
{
  ...
  for (const iface of receiptInterfaces) {
    ...
    assertOk(validators.includes(expectedValidator), `${iface} has validator ${expectedValidator}`);
    assertOk(clones.includes(expectedClone), `${iface} has clone ${expectedClone}`);
  }
}
...
// 2.2 Twin Earth substrate interfaces have validators
{
  ...
  for (const iface of coreInterfaces) {
    ...
    assertOk(twinClones.includes(expectedClone), `TwinEarth ${iface} has clone ${expectedClone}`);
  }
}
...
// 3.1 TwinEarthAction values are referenced in MCP tools
{
  ...
  for (const action of robotAiRefs) {
    assertOk(actions.includes(action), `TwinEarthAction includes ${action}`);
  }
}
...
// 3.2 Robot actuation command enum matches substrate actions
{
  ...
  if (mcpEnumMatch) {
    ...
    assertSetEq(
      new Set(mcpCommands),
      new Set(expectedCommands),
      'MCP robot actuation commands match substrate actions'
    );
  } else {
...
{
  const evalBody = twinEarthSource.match(/function evaluateActuation[\s\S]*?\n\}/)?.[0] || '';
...
// 5.2 Framework index.ts re-exports all Twin Earth substrate symbols
{
  ...
  for (const sym of expectedExports) {
    assertOk(frameworkIndexSource.includes(sym), `framework index.ts re-exports ${sym}`);
  }
}
...
{
  const validatorBody =
    hololandReceiptsSource.match(/function validatePackageProvenanceReceipt[\s\S]*?\n\}/)?.[0] ||
    '';
  assertOk(
...
// 6.2 Trust tier constants match isSupportedTrustTier
{
  ...
  const guardBody =
    hololandReceiptsSource.match(/function isSupportedTrustTier[\s\S]*?\{[\s\S]*?\n\}/)?.[0] || '';
...
{
  const decisions = extractConstArray(
    hololandReceiptsSource,
    'PACKAGE_PROVENANCE_ADMISSION_DECISIONS'
  );
  const guardBody =
    hololandReceiptsSource.match(
...
// 7.3 HardwareCompilationConstraints quantization enum is valid
{
  ...
  if (constraintMatch) {
    ...
    assertSetEq(
      new Set(modes),
      new Set(expected),
      'HardwareCompilationConstraints quantization modes match expected'
    );
  } else {
