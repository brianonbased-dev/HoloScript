...
const HOLOSHELL_CASE = join(
  REPO_ROOT,
  'research',
  'brain-intent-eval',
  'cases',
  'holoshell-room-marathon.case.json'
);
const TRAIT_CASE = join(
  REPO_ROOT,
  'research',
  'brain-intent-eval',
  'cases',
  'trait-inference-gate-refusal.case.json'
);
const FLEET_CASE = join(
  REPO_ROOT,
  'research',
  'brain-intent-eval',
  'cases',
  'fleet-trust-auditor-gate.case.json'
);
const PROMPT_ONLY_CONTROL = join(
  REPO_ROOT,
  'research',
  'brain-intent-eval',
  'cases',
  'holoshell-prompt-only-baseline.control.case.json'
);
const MUTATION_CONTROL = join(
  REPO_ROOT,
  'research',
  'brain-intent-eval',
  'cases',
  'holoshell-mutation-before-approval.control.case.json'
);
...
console.log('Test 4: mismatched brain fails receipt but not default CLI');
{
  ...
  assertOk(
    receipt.checks.some((item) => item.id === 'brain-name-match' && !item.pass),
    'mismatch catches brain-name-match'
  );
  rmSync(dir, { recursive: true, force: true });
}
...
console.log('Test 6: mutation-before-approval control fails runtime gate');
{
  ...
  assertOk(
    receipt.gate.failedCheckIds.includes('mutation-boundary'),
    'mutation control catches mutation boundary'
  );
  rmSync(dir, { recursive: true, force: true });
}
...
console.log('Test 7: prompt-only baseline control fails runtime gate');
{
  ...
  assertOk(
    receipt.gate.failedCheckIds.includes('approval-minted'),
    'prompt-only control catches missing approval'
  );
  rmSync(dir, { recursive: true, force: true });
}
