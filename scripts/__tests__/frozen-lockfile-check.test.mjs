...
function assertEq(actual, expected, name) {
  ...
  else { testsFailed += 1; console.error(`  FAIL ${name}: expected ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`); }
}
function assertMatch(text, re, name) {
  testsRun += 1;
  if (re.test(text)) console.log(`  PASS ${name}`);
  else { testsFailed += 1; console.error(`  FAIL ${name}: ${re} not found in output`); }
}
...
// 2. DRIFT — phantom dep in package.json, MISSING from lockfile → exit 1 (the deploy-bricker).
{
  const lock = lockHeader +
    ...
    "  .:\n    dependencies:\n      left-pad:\n        specifier: ^1.3.0\n        version: 1.3.0\n";
  const r = runFixture({
    rootPkg: { name: "fx", dependencies: { "left-pad": "^1.3.0", "@react-three/postprocessing": "^2.16.0" } },
    lock,
  });
...
// 4. WORKSPACE alias normalization (workspace:^ → workspace:*) → NOT drift (exit 0).
{
  ...
  const r = runFixture({
    ...
    packages: { "packages/app": { name: "@scope/app", dependencies: { "@scope/lib": "workspace:^" } } },
  });
...
// 5. DRIFT in a workspace package (not root) — dep added to packages/app, lock not regenerated.
{
  ...
  const r = runFixture({
    ...
    lock,
    workspaceYaml: "packages:\n  - 'packages/*'\n",
    packages: { "packages/app": { name: "@scope/app", dependencies: { "left-pad": "^1.3.0", lodash: "^4.17.21" } } },
  });
