...
assertOk(
  report1.probes.every((p) => p.name && typeof p.ok === 'boolean'),
  'every probe has name and ok'
);
...
const result3 = spawnSync(process.execPath, [SCRIPT], {
  cwd: REPO_ROOT,
  encoding: 'utf8',
  env: {
    ...process.env,
...
assertOk(!report3Str.includes(secretValue), 'secret value is NOT present in report JSON');
...
const result4 = spawnSync(process.execPath, [SCRIPT], {
  cwd: REPO_ROOT,
  encoding: 'utf8',
  env: {
    ...process.env,
...
const result5 = spawnSync(process.execPath, [SCRIPT], {
  cwd: REPO_ROOT,
  encoding: 'utf8',
  env: {
    ...process.env,
...
const result6 = spawnSync(process.execPath, [SCRIPT], {
  cwd: REPO_ROOT,
  encoding: 'utf8',
  env: {
    ...process.env,
...
const externalProbes = extReport.probes.filter((p) => p.name.startsWith('external-'));
