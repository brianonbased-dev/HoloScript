...
for (const cell of matrix.cells) {
  ...
  assertOk(
    cell.artifactPath.endsWith(`/${cell.skuId}/${cell.sceneId}/n${cell.views}.json`),
    `${cell.id} artifact path`
  );
...
assertEq(
  validateArtifact({ ...artifactCell, artifactPath }, artifactPath, artifactMatrix).status,
  'captured',
  'valid artifact captured'
);
...
const separatorResult = spawnSync(
  'node',
  [SCRIPT, '--', '--list-cells', '--sku', 'quest3-adreno740'],
  {
    cwd: REPO_ROOT,
    encoding: 'utf8',
  }
...
assertOk(
  separatorResult.stdout.includes('quest3-adreno740__indoor-500k__n2'),
  'separator CLI lists Quest cell'
);
...
const runCellResult = spawnSync(
  'node',
  [SCRIPT, '--run-cell', 'quest3-adreno740__indoor-500k__n2', '--out', envOut],
  {
    cwd: REPO_ROOT,
    encoding: 'utf8',
