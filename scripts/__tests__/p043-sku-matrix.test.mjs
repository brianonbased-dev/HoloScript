...
import { buildMatrix, REQUIRED_RUNS, SAMPLE_SECONDS, SCENES, SKUS, summarizeResults } from "../p043-sku-matrix.mjs";
...
assertEq(matrix.requiredRuns, REQUIRED_RUNS, "required run count is exported");
...
console.log("Test 7: run-cell forwards P043_REQUIRED_RUNS to capture command");
...
writeFileSync(
  ...
  `import { writeFileSync } from "node:fs";\n` +
    ...
    `  requiredRuns: process.env.P043_REQUIRED_RUNS\n` +
    `}, null, 2));\n`,
  "utf8"
);
...
assertEq(forwarded.requiredRuns, "3", "run-cell forwards required run count");
rmSync(envTmp, { recursive: true, force: true });

console.log("Test 8: result summary rejects wrong-SKU adapter artifacts");
