...
c1 = c1.replace(
  /type Vec3 = \{ x: number; y: number; z: number \};/g,
  'type Vec3 = [number, number, number];'
);
c1 = c1.replace(
  /velocity: \{ x: number; y: number; z: number \}/g,
...
// wander
c1 = c1.replace(
  /const wanderTarget: Vec3 = \{[\s\S]*?z: circleCenter\.z \+ Math\.sin\(this\.wanderAngle\) \* this\.config\.wanderRadius,\n    \};/m,
...
c1 = c1
  .replace(/circleCenter\.x/g, 'circleCenter[0]')
  .replace(/circleCenter\.y/g, 'circleCenter[1]')
  .replace(/circleCenter\.z/g, 'circleCenter[2]');
...
c1 = c1
  ...
  .replace(/circleCenter\.z/g, 'circleCenter[2]');
c1 = c1.replace(
  /x: circleCenter\[0\] \+ Math\.cos\(this\.wanderAngle\) \* this\.config\.wanderRadius,/g,
  'circleCenter[0] + Math.cos(this.wanderAngle) * this.config.wanderRadius,'
);
c1 = c1.replace(/y: circleCenter\[1\],/g, 'circleCenter[1],');
c1 = c1.replace(
  /z: circleCenter\[2\] \+ Math\.sin\(this\.wanderAngle\) \* this\.config\.wanderRadius,/g,
  'circleCenter[2] + Math.sin(this.wanderAngle) * this.config.wanderRadius'
);
c1 = c1
  .replace(/const wanderTarget: Vec3 = \{/g, 'const wanderTarget: Vec3 = [')
  .replace(/wanderRadius,\n    \};/g, 'wanderRadius\n    ];');
c1 = c1
  .replace(/force\.x/g, 'force[0]')
  .replace(/force\.y/g, 'force[1]')
  .replace(/force\.z/g, 'force[2]');
c1 = c1
  .replace(/weighted\.x/g, 'weighted[0]')
  .replace(/weighted\.y/g, 'weighted[1]')
  .replace(/weighted\.z/g, 'weighted[2]');
c1 = c1
  .replace(/avgVel\.x/g, 'avgVel[0]')
  .replace(/avgVel\.y/g, 'avgVel[1]')
  .replace(/avgVel\.z/g, 'avgVel[2]');
c1 = c1
  .replace(/n\.velocity\.x/g, 'n.velocity[0]')
  .replace(/n\.velocity\.y/g, 'n.velocity[1]')
  .replace(/n\.velocity\.z/g, 'n.velocity[2]');
c1 = c1
  .replace(/center\.x/g, 'center[0]')
  .replace(/center\.y/g, 'center[1]')
  .replace(/center\.z/g, 'center[2]');
c1 = c1
  .replace(/away\.x/g, 'away[0]')
  .replace(/away\.y/g, 'away[1]')
  .replace(/away\.z/g, 'away[2]');
c1 = c1
  .replace(/accel\.x/g, 'accel[0]')
  .replace(/accel\.y/g, 'accel[1]')
  .replace(/accel\.z/g, 'accel[2]');
c1 = c1
  .replace(/agent\.velocity\.x/g, 'agent.velocity[0]')
  .replace(/agent\.velocity\.y/g, 'agent.velocity[1]')
  .replace(/agent\.velocity\.z/g, 'agent.velocity[2]');

c1 = c1.replace(
  /\{ x: a\.x - b\.x, y: a\.y - b\.y, z: a\.z - b\.z \}/g,
...
c1 = c1.replace(
  /\{ x: v\.x \* s, y: v\.y \* s, z: v\.z \* s \}/g,
  '[v[0] * s, v[1] * s, v[2] * s]'
);
c1 = c1.replace(
  /v\.x \* v\.x \+ v\.y \* v\.y \+ v\.z \* v\.z/g,
