...
async function refresh(rootDir = REPO, maxFiles = 2500) {
  ...
  console.log(
    `[holo-graph] files=${stats.totalFiles ?? '?'} symbols=${stats.totalSymbols ?? '?'} ` +
      `langs=${JSON.stringify(stats.filesByLanguage ?? {})}`
  );
  const st = parse(await handleCodebaseTool('holo_graph_status', {}));
  console.log(
    `[holo-graph] status: ready=${st?.graphRAGReady} authoritative=${st?.graphAuthoritative} ` +
...
async function ask(query, topK = 8) {
  if (!query) {
    console.error('usage: ask "<query>" [topK]');
    process.exit(1);
  }
  ...
  const st = parse(await handleCodebaseTool('holo_graph_status', {}));
  console.log(
    `[holo-graph] loaded cache: ready=${st?.graphRAGReady} ` +
  ...
  for (const h of results.slice(0, topK)) {
    ...
    console.log(
      `  ${Number(h.score ?? h.similarity ?? 0).toFixed(3)}  ${file}:${h.line ?? h.startLine ?? ''}  ${h.symbol || h.name || ''}`
    );
  }
}
...
if (cmd === 'refresh') await refresh(rest[0], rest[1] ? Number(rest[1]) : 2500);
  ...
  console.error('usage: holo-graph.mjs <refresh [rootDir] | ask "<query>" [topK]>');
  process.exit(1);
}
