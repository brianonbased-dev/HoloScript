...
function main(argv = process.argv.slice(2)) {
  ...
  console.log(`[check-pnpm-install-integrity] ${checked} declared node_modules packages have contents.`);
  process.exitCode = 0;
  return report;
}
...
function isMainModule() {
  ...
  return resolve(entry).replace(/\\/gu, '/').toLowerCase()
    === fileURLToPath(import.meta.url).replace(/\\/gu, '/').toLowerCase();
}
