#!/usr/bin/env npx tsx
/**
 * ⚠️ ARCHIVED — Pre-Native Control Arm (G.ARCH.001)
 *
 * This is the standalone TypeScript daemon that was replaced by the native
 * .hsplus composition bridge (self-improve-bridge.ts → self-improve.ts).
 *
 * Preserved for reference. The treatment arm won the A/B experiment:
 *   - 75% cheaper
 *   - +0.080 quality delta
 *   - 9.6x better Q/$
 *   - 93.2% tool efficiency
 *
 * The native bridge is now the only daemon entry point.
 * See: compositions/self-improve-daemon.hsplus (the composition)
 * See: scripts/self-improve.ts (the bridge, formerly self-improve-bridge.ts)
 *
 * Archived: 2026-03-18
 *
