...
async function embed(text) {
  const r = await fetch(OLLAMA, { method: 'POST', body: JSON.stringify({ model: MODEL, prompt: text }) });
...
async function embedMany(texts, conc = 8) {
  ...
  await Promise.all(Array.from({ length: conc }, async () => {
    while (i < texts.length) { const k = i++; out[k] = await embed(texts[k]); }
  }));
  return out;
}
...
function jetsonSecret() {
  ...
  const remote = "sudo cat /mnt/nvme2/holo-volumes/secrets/pg-app.env 2>/dev/null | grep -E '^POSTGRES_PASSWORD=' | head -1 | cut -d= -f2-";
  const b64 = Buffer.from(remote, 'utf8').toString('base64');
  const pw = execSync(`ssh -i ${KEY} -o ConnectTimeout=20 ${HOST} "echo ${b64} | base64 -d | bash"`,
    { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim();
  if (!pw) throw new Error('could not read app-DB password from Jetson secret');
  return pw;
}
async function openTunnel() {
  const child = spawn('ssh', ['-i', KEY, '-N',
    '-L', `${LOCAL_PORT}:127.0.0.1:5434`, '-o', 'ExitOnForwardFailure=yes', HOST],
    { stdio: 'ignore' });
...
async function withPg(fn) {
  ...
  const pool = new pg.Pool({ host: '127.0.0.1', port: LOCAL_PORT, user: 'holoscript_app', database: 'knowledge', password: pw, max: 4 });
  try { return await fn(pool); }
  finally { await pool.end().catch(() => {}); tunnel.kill(); }
}
...
async function index(limit) {
  ...
  await withPg(async (pool) => {
    ...
    for (let off = 0; off < syms.length; off += BATCH) {
      ...
      const vecs = await embedMany(chunk.map((s) => s.text), 8);
      ...
      try {
        ...
        for (let k = 0; k < chunk.length; k++) {
          ...
          await client.query(
            ...
            [rootDir, gitCommit, s.rel, s.name, s.type, s.language, s.line, s.text, vecLiteral(vecs[k])]
          );
        }
  ...
  await withPg(async (pool) => {
    const { rows } = await pool.query(
      `SELECT file_path, symbol, symbol_type, line, 1-(embedding <=> $1::vector) AS score
       ...
       ORDER BY embedding <=> $1::vector LIMIT $2`, [qv, topK]);
    console.log(`[semantic] "${query}"  (${rows.length} hits, nomic meaning-ranked)`);
    for (const r of rows) console.log(`  ${Number(r.score).toFixed(3)}  ${r.file_path}:${r.line ?? ''}  ${r.symbol} [${r.symbol_type || ''}]`);
  });
}
...
else { console.error('usage: semantic.mjs <index [limit] | ask "<query>" [topK]>'); process.exit(1); }
