...
**Inventory:** [`SKILL_MAP.md`](https://github.com/holoscript/ai-ecosystem/blob/main/SKILL_MAP.md) in the `ai-ecosystem` repo (`~/.ai-ecosystem/SKILL_MAP.md` locally) is the cross-surface canonical list — every skill across user-level, ai-ecosystem plugin, and this HoloScript plugin, with descriptions and absolute paths. Generated by `ai-ecosystem/scripts/generate-skill-map.mjs`. The workflow tables below are the **decision aid** (which skill for which task) — `SKILL_MAP.md` is the **authoritative inventory** (what exists where).
