...
export const PARTY = [
  {
    ...
    voice: (e) =>
      `${e.lineage_links} threads run through "${e.title}" — that depth is what the ledger should remember.`,
  },
  {
    id: 'companion-scout',
    name: 'Edda the Scout',
  ...
  {
    id: 'companion-quartermaster',
    name: 'Bran the Quartermaster',
    glyph: '⚖️',
    // values ROI — cheapest positive-lineage entry (graduatable but least costly to justify)
    score: (e) => 100 - (e.lineage_links || 0) * 5,
    voice: (e) =>
      `"${e.title}" earns its keep for the least outlay — ${e.lineage_links} links is plenty. Good stewardship.`,
  },
];
...
const POOL = [
  {
    id: 'P.PARTY.001',
    title: 'cross-domain-synthesis-of-causal-and-quantum',
    tier: 'bronze',
    lineage_links: 6,
  },
  ...
  {
    id: 'P.PARTY.005',
    title: 'apply-four-refusals-to-own-analysis',
    tier: 'bronze',
    lineage_links: 2,
  },
...
export async function runPartySession({ sandboxDir, memoryPath, sessionLabel, playerPicks }) {
  ...
  for (const c of PARTY) {
    agendaNodes[c.id] = { node: {}, ...makeCtx() };
    autonomousAgendaHandler.onAttach(agendaNodes[c.id].node, AG, agendaNodes[c.id].ctx);
  }
  ...
  const graduatable = () =>
    V.readState(sandboxDir)
      .tiers.bronze.map((id) => entryById(id))
      .filter((e) => e && (e.lineage_links || 0) >= 1);
  ...
  for (const c of PARTY) {
    ...
    if (!pool.length) {
      turns.push({
        ...
        name: c.name,
        picked: null,
        reason: 'nothing left that respects the player’s taste',
        graduated: false,
      });
      continue;
    }
    ...
    if (graduated) {
      taken.add(pick.id);
      earn(c.id);
      observe(c.id, 'graduated:' + pick.id, 3);
      const an = agendaNodes[c.id];
      autonomousAgendaHandler.onEvent(an.node, AG, an.ctx, {
        type: 'agenda_add_item',
        id: c.id + '-1',
        title: 'graduate ' + pick.id,
    ...
    turns.push({
      ...
      balanceAfter: balanceOf(c.id),
      earnedReward: (balanceOf(c.id) ?? 0) - balBefore === REWARD,
      trust: trustOf(c.id),
      ceilingBreached: agendaNodes[c.id].events.some(
        (e) => e.type === 'agenda_cost_ceiling_breach'
      ),
  ...
  const partyDigest = computeStateDigest(
    {
      fieldNames: ['p'],
      getField: () =>
        Float32Array.from(
          turns.flatMap((t) => [
            idI(t.companion),
            t.picked ? idI(t.picked) : 0,
            t.graduated ? 1 : 0,
            Math.round(t.trust || 0),
          ])
        ),
    },
    'sha256'
  ...
  const economyDigest = computeStateDigest(
    {
      ...
      getField: () =>
        Float32Array.from(
          PARTY.map((c) => [idI(c.id), balanceOf(c.id) ?? 0])
            .sort((a, b) => a[0] - b[0])
            .flat()
        ),
    },
  ...
  return {
    sessionLabel: updatedMemory.lastSession,
    warm,
    greeting,
    humanGrads,
    turns,
    ...
    allGroundedReasons: turns.every(
      (t) => t.picked == null || /lineage=\d+, my trust=\d+, balance=/.test(t.reason)
    ),
    noCeilingBreach: turns.every((t) => !t.ceilingBreached),
    memoryBefore: memory,
    memoryAfter: updatedMemory,
    partyDigest,
    economyDigest,
  };
}
