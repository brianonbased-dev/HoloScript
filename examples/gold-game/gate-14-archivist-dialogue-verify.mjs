...
const DIALOGUE_CONFIG = {
  ...
  dialogue_tree: {
    verdict: {
      id: 'verdict', speaker: 'Maren Vael', emotion: 'wary',
      text: 'I have read it. The ledger remembers what you have brought before.',
      options: [
        { text: '(she lets it ascend — it earns the anchor)', condition: 'reputation > 0.55', action: 'ratify', nextNode: 'quest_open', emotion: 'warming' },
        { text: '(she sends it back — it decays in a week)', action: 'send_back', nextNode: 'farewell', emotion: 'cold' },
      ],
    },
    quest_open: { id: 'quest_open', speaker: 'Maren Vael', emotion: 'warming', text: 'It earns its place. Now — recover the Drowned Index, as the index has it.', options: [] },
    farewell: { id: 'farewell', speaker: 'Maren Vael', emotion: 'cold', text: 'A thing that decays in a week does not deserve a permanent slot.', options: [] },
  },
};
...
const observe = (action, delta) => reputationLedgerHandler.onEvent(repNode, REP_CFG, rep.ctx, { type: 'reputation_observe_action', observerId: PLAYER, action, trustDelta: delta });
const trustNow = () => {
  reputationLedgerHandler.onEvent(repNode, REP_CFG, rep.ctx, { type: 'reputation_query', observerId: PLAYER });
  const snap = lastEvent(rep.events, 'reputation_ledger_snapshot');
  const obs = snap?.payload?.observers?.find?.((o) => o.id === PLAYER) || snap?.payload?.observers?.[0];
...
function visit(v) {
  ...
  const node = {}; const dlg = makeCtx();
  dialogueHandler.onAttach(node, DIALOGUE_CONFIG, dlg.ctx);
  dialogueHandler.onEvent(node, DIALOGUE_CONFIG, dlg.ctx, { type: 'start_dialogue', startNode: 'verdict', context: { reputation } });
  ...
  return { entry: v.entry, trust, reputation: Number(reputation.toFixed(4)), verdict: action, reachedQuest };
}
...
const verdictDigest = computeStateDigest(
  { fieldNames: ['v'], getField: () => Float32Array.from(results.flatMap((r) => [Math.round(r.trust * 100), r.verdict === 'ratify' ? 1 : 0, r.reachedQuest ? 1 : 0])) },
  HASH,
);
...
if (emit) {
  ...
  console.log('  trust arc:', results.map((r) => r.trust).join(' -> '), '| final=' + finalTrust + ' (== ' + INITIAL_TRUST + '+' + sumDeltas + ': ' + accumulationDriven + ')');
  console.log('  verdicts:', results.map((r) => r.verdict).join(', '));
  console.log('  ratifyVisit=' + (ratifyVisitIdx + 1), 'firstCross=' + (firstCrossIdx + 1), 'questOpened=' + questOpened);
  console.log('  verdictDigest=' + verdictDigest);
} else {
  let existing; try { existing = JSON.parse(readFileSync(receiptPath, 'utf8')); } catch { console.error('No Gate-14 receipt. Run --emit first.'); process.exit(2); }
  const checks = [
    ['REAL ReputationLedger attached + accrues trust', !!repNode.__reputationState || results.every((r) => typeof r.trust === 'number')],
    ['REAL DialogueTrait decided each verdict (dialogue_action emitted)', results.every((r) => r.verdict === 'ratify' || r.verdict === 'send_back')],
    ['early visits SENT BACK (low trust -> ratify option absent via real filterOptions)', sentBackEarly === true],
    ['trust accrues by ACCUMULATION, not a flag (final == initial + Σ deltas)', accumulationDriven === true],
    ['verdict flips to RATIFY exactly when reputation first crosses the threshold', ratifyVisitIdx >= 0 && ratifyVisitIdx === firstCrossIdx],
    ['ratify advances to the quest node + opens a HoloQuest', questOpened === true],
    ['verdict/trust digest reproduces (real computeStateDigest)', verdictDigest === existing.contract.verdictDigest],
  ];
  let ok = true;
