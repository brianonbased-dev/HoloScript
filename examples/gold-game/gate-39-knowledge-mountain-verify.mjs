...
ok(
  'additive: ratified 5-seed onboarding survives in gold-vault-game.holo',
  seedsPresent.length === seeds.length,
  `${seedsPresent.length}/${seeds.length} seeds present`
);
...
ok(
  'tier -> elevation is monotonic (bronze base .. diamond peak)',
  monotonic,
  JSON.stringify(tierY)
);
ok('every entry sits at its tier elevation band', yConsistent);

// lineage cables connect real entries
ok(
  '83 lineage cables present',
  edges.length === data.n_lineage_edges && edges.length === 83,
  `${edges.length} edges`
);
...
ok(
  '40 missing-bridges present (the discovery instrument)',
  bridges.length === 40,
  `${bridges.length} bridges`
);
const bridgesValid = bridges.every(
  (b) =>
    ids.has(b[0]) && ids.has(b[1]) && b[2] >= 0.3 && !edgeKey.has([b[0], b[1]].sort().join('->'))
);
ok(
  'every missing-bridge is real, semantically close (>=0.3), and uncited (not a lineage edge)',
  bridgesValid
);
...
ok(
  'knowledge-mountain.holo parses clean against @holoscript/core',
  parsedClean || structurallyOk,
  parsedClean ? parseDetail : `core unavailable; structural: ${structurallyOk}`
);
...
ok(
  'negative control: tampering one coordinate flips the terrain digest',
  tamperDigest !== digest,
  `${digest.slice(0, 12)} != ${tamperDigest.slice(0, 12)}`
);
const droppedDigest = terrainDigest(coords, edges.slice(1));
ok(
  'negative control: dropping one lineage cable flips the terrain digest',
...
ok(
  'playable viewer artifact present (knowledge-mountain.html)',
  viewer.length > 0,
  `${viewer.length} bytes`
);
ok(
  'viewer inlines terrain data (self-contained — no external coords fetch)',
...
const receipt = {
  ...
  playableViewer:
    'knowledge-mountain.html (self-contained data; three.js via CDN — not air-gapped, unlike the drive-build self-proving boot)',
  negativeControl: {
    coordinateTamperFlips: tamperDigest !== digest,
    edgeDropFlips: droppedDigest !== digest,
  },
...
console.log(
  'Gate 39 PASS — the real GOLD vault is an additive climbable Knowledge Mountain (digest ' +
    digest.slice(0, 12) +
    ').'
);
