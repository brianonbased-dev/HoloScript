...
const { quantumInspiredHandler } = await imp(
  join(repo, 'packages', 'core', 'src', 'traits', 'QuantumInspiredTrait.ts')
);
...
const { runAggregate, runSharpeningExperiment } = await imp(
  join(here, 'gold-game-curation-sharpen.mjs')
);
...
const PRIORITY = [0.92, 0.71, 0.55, 0.49, 0.3, 0.12, 0.63, 0.41];
...
async function runOptimize() {
  ...
  quantumInspiredHandler.onEvent(node, config, ctx, {
    type: 'qi:optimize',
    payload: { input: new Float32Array(PRIORITY), requestId: 'curate-1' },
  });
  await flush();
...
const thresholdPreserved =
  out1.every((v, i) => PRIORITY[i] > 0.5 === v > 0.5) &&
  Math.abs(out1[PRIORITY.indexOf(0.55)] - 0.5) >= 0; // 0.5-centered
...
const decisions = CANDIDATES.map((id, i) => ({
  id,
  priority: PRIORITY[i],
  activation: r4(out1[i]),
  decision: out1[i] > 0.5 ? 'graduate' : 'defer',
}));
...
const activationDigest = computeStateDigest(
  { fieldNames: ['q'], getField: () => Float32Array.from(out1.map((v) => Math.round(v * 1e6))) },
  HASH
);
...
const controlIsNoop =
  sharpAgg.controlFlips === 0 && sharpAgg.meanAccControl === sharpAgg.meanAccRaw;
const flipsAreCorrections = sharpAgg.fracFlipTowardTruth >= 0.6;
const confidentLocked = sharpAgg.fracFlipNearBoundary >= 0.8;
const sharpDeterministic = JSON.stringify(runAggregate()) === JSON.stringify(sharpAgg);
const sr = (x) => Number(x.toFixed(6));
const sharpenDigest = computeStateDigest(
  {
    fieldNames: ['s'],
    getField: () =>
      Float32Array.from([
        Math.round(sharpAgg.meanAccRaw * 1e6),
        Math.round(sharpAgg.meanAccSharp * 1e6),
        Math.round(sharpAgg.meanAccControl * 1e6),
        sharpAgg.totalFlips,
        sharpAgg.controlFlips,
        Math.round(sharpAgg.fracFlipTowardTruth * 1e6),
        Math.round(sharpAgg.fracFlipNearBoundary * 1e6),
      ]),
  },
  HASH
);
...
const receipt = {
  ...
  implementation:
    'packages/core/src/traits/QuantumInspiredTrait.ts (quantumInspiredHandler — REAL wiring; CpuFallbackAccelerator is decision-NEUTRAL on a CPU-only host) + examples/gold-game/gold-game-curation-sharpen.mjs (E-G11 gold-game-LOCAL mean-field annealing sharpening layer)',
  problem: { candidates: CANDIDATES, rawPriority: PRIORITY, numNeurons: NUM, event: 'qi:optimize' },
  result: {
    attached,
    emittedResult,
    optimizeCount: run1.result?.payload?.optimizeCount,
    acceleratorAvailable: run1.status?.payload?.acceleratorAvailable,
    activation: out1.map(r4),
    decisions,
    graduateCount,
    expectedGraduate,
    deterministic,
  ...
  sharpening: {
    ...
    cohorts: sharpAgg.cohorts,
    entriesPerCohort: sharpAgg.n,
    meanAccRaw: sr(sharpAgg.meanAccRaw),
    meanAccSharp: sr(sharpAgg.meanAccSharp),
    meanAccControl: sr(sharpAgg.meanAccControl),
    meanGain: sr(sharpAgg.meanGain),
    totalFlips: sharpAgg.totalFlips,
    controlFlips: sharpAgg.controlFlips,
    fracFlipTowardTruth: sr(sharpAgg.fracFlipTowardTruth),
    fracFlipNearBoundary: sr(sharpAgg.fracFlipNearBoundary),
    exampleCohortSeed1: {
      flips: example.flipCount,
      flipTowardTruth: example.flipTowardTruth,
      accRaw: sr(example.accRaw),
      accSharp: sr(example.accSharp),
    },
    sharpenDigest,
  },
  contract: {
  ...
  honestScope:
    "TWO honest claims. (1) WIRING: the optimization is run by the GENUINE QuantumInspiredTrait handler from @holoscript/core via qi:optimize — not a mock. On this CPU-only host it uses the trait's CpuFallbackAccelerator (acceleratorAvailable === false), a 0.5-centred monotonic sigmoid that is DECISION-NEUTRAL — it cannot change which entries cross a 0.5 threshold (deep-ratchet 2026-05-24). The genuine annealing-analogue SnnAccelerator is GPU-only and not exercised here; real-QPU is the separate /quantum-lab VQE track (S.VQE). (2) SHARPENING (E-G11 fix): a gold-game-LOCAL mean-field-annealing layer (gold-game-curation-sharpen.mjs) — an Ising Hamiltonian relaxed by deterministic annealing — fuses a second noisy view and ferromagnetically couples similar entries, moving NEAR-BOUNDARY items off the fence. PROVEN over 200 synthetic cohorts: it flips real decisions vs the raw 0.5 threshold (" +
    sharpAgg.totalFlips +
    ' flips), raises mean curation accuracy ' +
    sr(sharpAgg.meanAccRaw) +
    ' -> ' +
    sr(sharpAgg.meanAccSharp) +
    ' (+' +
    sr(sharpAgg.meanGain) +
    '), ' +
    (sharpAgg.fracFlipTowardTruth * 100).toFixed(1) +
    '% of flips move TOWARD ground truth, and ' +
    (sharpAgg.fracFlipNearBoundary * 100).toFixed(1) +
    "% are near-boundary (confident items stay locked). A negative control (no evidence fusion, no coupling) collapses EXACTLY to the raw threshold (0 flips, identical accuracy) — proving the gain is the mechanism's, not annealing luck. NOT claimed: real quantum hardware; the shared trait CPU fallback behaviour (left untouched on purpose); real operator traces (synthetic scenario, like Gate 5a).",
...
if (emit) {
  ...
  console.log(
    '  graduate=' + graduateCount + '/' + NUM + ' (expected ' + expectedGraduate + ')',
    'monotonic=' + monotonic,
    'deterministic=' + deterministic
  );
  console.log('  activationDigest=' + activationDigest);
  console.log(
    '  [E-G11 sharpen] flips=' + sharpAgg.totalFlips,
    'acc ' +
      sr(sharpAgg.meanAccRaw) +
      '->' +
      sr(sharpAgg.meanAccSharp) +
      ...
      sr(sharpAgg.meanGain) +
      ')',
    'ctrlFlips=' + sharpAgg.controlFlips,
    'towardTruth=' + (sharpAgg.fracFlipTowardTruth * 100).toFixed(1) + '%'
  );
  ...
  try {
    ...
    console.error('No Gate-11 receipt. Run --emit first.');
    process.exit(2);
  }
  const checks = [
    ...
    [
      'curation decisions coherent (graduate count == high-priority count)',
      decisionsCoherent === true,
    ],
    ['deterministic across independent runs', deterministic === true],
    [
      'activation digest reproduces (real computeStateDigest)',
      activationDigest === existing.contract.activationDigest,
    ],
    ...
    [
      'NEGATIVE CONTROL: no evidence/coupling collapses EXACTLY to raw (0 flips, equal acc)',
      controlIsNoop === true,
    ],
    [
      'flips are CORRECTIONS: majority move toward ground truth (>=60%)',
      flipsAreCorrections === true,
    ],
    ...
    [
      'sharpen digest reproduces (real computeStateDigest)',
      sharpenDigest === existing.contract.sharpenDigest,
    ],
  ];
  let ok = true;
