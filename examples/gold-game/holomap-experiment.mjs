...
Module._load = function cpuOnly(req, parent, isMain) {
  if (req === 'webgpu')
    throw new Error('holomap-experiment pins HoloMap to deterministic CPU fallback');
...
async function loadFrames() {
  if (!existsSync(framesDir)) {
    console.error(`holomap-experiment: frames dir not found: ${framesDir}`);
    process.exit(2);
  }
  const files = readdirSync(framesDir)
    ...
    .sort();
  if (files.length < 4) {
    console.error(`holomap-experiment: need >=4 frames, found ${files.length} in ${framesDir}`);
    process.exit(2);
  }
  const frames = [];
...
async function reconstruct(frames, tamper = false) {
  ...
  const start = await mcpStartReconstructFromVideo('gold-game://holomap/real-capture', {
    ...
    seed: 45,
    modelHash: 'holomap-real-capture-v1',
    videoHash: capDigest,
  ...
  const stable = {
    ...
    anchorPose: {
      position: roundVec(anchor.anchor?.anchorPose?.position),
      confidence: round(anchor.anchor?.anchorPose?.confidence ?? 0),
    },
    ...
    bounds: {
      min: roundVec(exported.manifest?.bounds?.min),
      max: roundVec(exported.manifest?.bounds?.max),
    },
    compiledOutputHash: sha256(exported.compiledOutput || ''),
    poses: (traj.poses || []).map((p) => ({
  ...
  const trajFlat = stable.poses.flatMap((p) => [
    p.frameIndex,
    ...p.position.map(q),
    Math.round((p.confidence ?? 0) * 1e5),
  ]);
  const fields = {
    capture: Float32Array.from(hexFloats(stable.capDigest)),
    anchor: Float32Array.from([
      ...stable.anchorPose.position.map(q),
      Math.round(stable.anchorPose.confidence * 1e5),
      stable.anchorRevision,
    ]),
    drift: Float32Array.from([
      Math.round(stable.driftMetersRaw * 1e5),
      stable.lastLoopClosureFrame,
    ]),
    ...
    export: Float32Array.from([
      stable.framesStepped,
      stable.pointsExported,
      ...hexFloats(stable.compiledOutputHash).slice(0, 16),
    ]),
  };
  stable.spaceDigest = computeStateDigest(
    { fieldNames: Object.keys(fields), getField: (n) => fields[n] },
    HASH
  );
  return stable;
}
...
async function main() {
  ...
  ck(
    'real captured frames decoded + stepped through the shipped HoloMap runtime',
    clean.framesStepped === frames.length && clean.framesStepped >= 4,
    `${clean.framesStepped} frames @ ${W}x${H}`
  );
  ck(
    'reconstruction produced an exported point cloud through the shipped export path',
    clean.compileStatus === 'COMPILED' && clean.pointsExported > 0,
    `${clean.pointsExported} pts / ${clean.compileStatus}`
  );
  ck(
    'scan-derived anchor pose emitted (3-vector, real revision)',
    clean.anchorPose.position.length === 3 && clean.anchorRevision >= frames.length,
    `rev=${clean.anchorRevision}`
  );
  ck(
    'drift accumulates over the real capture (derived, not constant 0)',
    clean.driftMetersRaw > 0,
    `drift=${clean.driftMeters}m`
  );
  ck(
    'SimulationContract space digest is sha256-shaped',
    /^[a-f0-9]{64}$/.test(clean.spaceDigest),
    clean.spaceDigest.slice(0, 16)
  );
  // ── the NEGATIVE-CONTROL table (the contribution COLMAP cannot report) ──
  ck(
    '[neg] 1-byte capture perturbation CHANGES the capture digest (tamper-evident)',
    clean.capDigest !== tampered.capDigest,
    'capture'
  );
  ck(
    '[neg] 1-byte capture perturbation CHANGES the reconstruction space digest (tamper-evident)',
    clean.spaceDigest !== tampered.spaceDigest,
    `clean=${clean.spaceDigest.slice(0, 12)} tampered=${tampered.spaceDigest.slice(0, 12)}`
  );
  ck(
    '[fp] bit-identical re-run reproduces the SAME space digest (false-positive rate 0)',
    clean.spaceDigest === cleanRerun.spaceDigest,
    'determinism'
  );
  ...
  const receipt = {
    ...
    protocol:
      'ai-ecosystem/research/2026-05-25_holomap-verifiable-reconstruction-experiment-protocol.md',
    capture: {
      source: 'real phone video (20260525_171459.mp4, 4K/60→3fps)',
      frames: frames.length,
      frameResolution: [W, H],
      framesDir: framesDir.replace(repo, '.'),
    },
    implementation: {
      sessionApi:
        'packages/mcp-server/src/holo-reconstruct-sessions.ts (shipped mcpReconstruct* path — same as Gate 33)',
    ...
    reconstruction: {
      framesStepped: clean.framesStepped,
      pointsExported: clean.pointsExported,
      compileStatus: clean.compileStatus,
      anchorPose: clean.anchorPose,
      anchorRevision: clean.anchorRevision,
      driftMeters: clean.driftMeters,
      bounds: clean.bounds,
      trajectoryPoses: clean.poses.length,
    },
    contract: {
      spine: 'REAL computeStateDigest',
      spaceDigest: clean.spaceDigest,
      captureDigest: clean.capDigest,
    },
    ...
    pendingBaseline:
      'COLMAP accuracy-parity leg (pose ATE + metric scale error) — COLMAP not yet installed; this receipt is the verifiability leg (the novelty).',
    honestScope:
  ...
  console.log(
    `  reconstruction: ${clean.framesStepped} frames → ${clean.pointsExported} pts, drift ${clean.driftMeters}m, anchorRev ${clean.anchorRevision}`
  );
  console.log(
    `  spaceDigest=${clean.spaceDigest.slice(0, 24)}  (tamper→${tampered.spaceDigest.slice(0, 12)}, rerun→${cleanRerun.spaceDigest.slice(0, 12)})`
  );
  console.log(
    `  NEG-CONTROL: tamperDetectionRate=${receipt.negativeControlTable.tamperDetectionRate} falsePositiveRate=${receipt.negativeControlTable.falsePositiveRate}`
  );

  if (process.argv.includes('--emit')) {
    if (!allPass) {
      console.error('refusing to emit — a check failed');
      process.exit(1);
    }
    ...
    console.log(
      `  receipt digest reproduces vs committed: ${repro ? 'PASS' : 'FAIL'} (${(prev.contract?.spaceDigest || '').slice(0, 16)})`
    );
