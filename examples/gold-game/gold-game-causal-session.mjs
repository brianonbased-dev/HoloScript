...
export async function applyCausalEffect(sandboxDir, hubId) {
  ...
  for (const [id, val] of Object.entries(result.values)) {
    ...
    if (rounded !== f.entry.lineage_links) writeFileSync(f.path, JSON.stringify({ ...f.entry, lineage_links: rounded }, null, 2));
...
export const loadMemory = (memoryPath) =>
  existsSync(memoryPath) ? JSON.parse(readFileSync(memoryPath, 'utf8')) : { graduated: [], payoffs: {} };
export const saveMemory = (memoryPath, mem) => writeFileSync(memoryPath, JSON.stringify(mem, null, 2) + '\n');
...
export async function runSession({ label, rounds, sandboxDir, memoryPath }) {
  ...
  const selectTarget = () => {
    ...
    let best = pool[0], bestKey = [mem.payoffs[best.id] ?? -1, best.value, best.id];
    for (const cand of pool) {
      const key = [mem.payoffs[cand.id] ?? -1, cand.value, cand.id];
      if (key[0] > bestKey[0] || (key[0] === bestKey[0] && key[1] > bestKey[1]) ||
          (key[0] === bestKey[0] && key[1] === bestKey[1] && key[2] < bestKey[2])) { best = cand; bestKey = key; }
    }
    return best.id;
  };
  for (let r = 0; r < rounds; r++) {
    ...
    known.add(target); mem.graduated.push(target); mem.payoffs[target] = causal.newlyUnlocked.length;
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  ...
  console.log(JSON.stringify({ pid: process.pid, label, targets: res.events.filter((e) => e.action === 'graduate').map((e) => e.target) }));
}
