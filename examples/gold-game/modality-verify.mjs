...
const prop = (node, key, dflt) => { const p = (node.properties || []).find((x) => x.key === key); return p ? p.value : dflt; };
...
const sceneDigest = computeStateDigest(
  { fieldNames: ['scene'], getField: () => Float32Array.from(meshes.flatMap((m) => [...m.position.map((n) => Number(n.toFixed(4))), colInt(m.color), m.geometry.length])) },
  'sha256',
);

let r2d = {};
try { r2d = JSON.parse(readFileSync(join(here, 'GOLD-VAULT-2D-receipt.json'), 'utf8')); } catch {}
const html2d = existsSync(join(here, '2d-build', 'index.html')) ? readFileSync(join(here, '2d-build', 'index.html'), 'utf8') : '';
const html3d = existsSync(join(here, 'drive-build', 'index.html')) ? readFileSync(join(here, 'drive-build', 'index.html'), 'utf8') : '';
...
const checks = [
  ...
  ['3D build embeds the source scene (scene-derived names survive bundling)', html3d.includes('AgentCompanion') && html3d.includes('DiamondPeak') && html3d.includes('B_GRADUATE_001')],
  ['both modalities saw the same mesh count', meshes.length === (r2d?.contract?.meshCount)],
  ['2D is a retro canvas (pixelated, no WebGL)', /image-rendering\s*:\s*pixelated/.test(html2d) && !/WebGLRenderer/.test(html2d)],
