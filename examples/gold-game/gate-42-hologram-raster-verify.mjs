...
import {
  renderQuilt, encodePng, sha, TILE_W, TILE_H, GRID_COLS, GRID_ROWS, VIEWS,
...
async function main() {
  ...
  const W = GRID_COLS * TILE_W, H = GRID_ROWS * TILE_H;
  ...
  ok('2 sidecar schema/shape',
    sidecar.schema === 'gold-game-hologram-raster-v1' && sidecar.gate === 42 &&
    sidecar.views === VIEWS && sidecar.grid === `${GRID_COLS}x${GRID_ROWS}` &&
    sidecar.sampleCount === exp.samples.length && sidecar.parallaxApplied === true,
  ...
  ok('4 raster reproduces from source', realRasterDigest === sidecar.rasterDigest, realRasterDigest.slice(0, 12));
  ...
  let mono = true;
  for (let i = 1; i < eyes.length; i++) if (!(eyes[i] > eyes[i - 1])) { mono = false; break; }
  ...
  ok('8 [neg] no-parallax flips raster digest', flatDigest !== sidecar.rasterDigest,
  ...
  tampered.samples[0].world = [tampered.samples[0].world[0] + 5, tampered.samples[0].world[1], tampered.samples[0].world[2]];
  const tamperedDigest = sha(renderQuilt(tampered, { noParallax: false }).buf);
  ok('9 [neg] tampered sample flips raster digest', tamperedDigest !== sidecar.rasterDigest,
  ...
  if (passed === checks.length) {
    console.log(`Gate 42 PASS — physical hologram quilt rasterized (${W}x${H}, digest ${sidecar.rasterDigest.slice(0, 12)}).`);
    process.exit(0);
  }
