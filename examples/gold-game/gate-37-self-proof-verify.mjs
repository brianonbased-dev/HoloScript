...
let manifest = null, shaFnSrc = null;
try {
  const m = build.match(/__GOLD_GAME_SELFPROOF__\s*=\s*(\{[\s\S]*?\});\s*window\.__GOLD_GAME_SHA256__\s*=\s*\(([\s\S]*?)\);<\/script>/);
  if (m) { manifest = JSON.parse(m[1]).manifest; shaFnSrc = m[2]; }
} catch { /* parse fail */ }
check('build embeds a self-proof manifest', Array.isArray(manifest) && manifest.length > 0, manifest ? `${manifest.length} entries` : 'none');
check('build embeds a pure-JS SHA-256 fn (works offline on file://, no crypto.subtle)', !!shaFnSrc && !/crypto\.subtle/.test(shaFnSrc), shaFnSrc ? 'embedded' : 'missing');

let embeddedSha = null;
try { embeddedSha = new Function('return (' + shaFnSrc + ')')(); } catch { /* bad fn */ }
...
if (realSha) for (const v of vectors) { if (embeddedSha(v) !== nodeSha(v)) { realSha = false; break; } }
check('embedded fn is GENUINE SHA-256 (matches node:crypto on 4 vectors incl. UTF-8)', realSha, realSha ? 'all vectors match' : 'NOT real sha256');
...
check('re-deriving the embedded blobs reproduces the manifest (browser self-check WOULD pass)', allReproduce,
...
check('NEGATIVE CONTROL: a tampered blob FAILS the self-proof (check is load-bearing)', negControlHolds, `tamper detected=${negControlHolds}`);

// (1, cont.) The visible self-proof surface is wired into the build.
check('build wires the self-proof panel + summary (visible offline proof)',
  /data-gate37="self-proof-panel"/.test(build) && /data-gate37="summary"/.test(build) && /data-gate37="self-proof"/.test(build),
  'toggle + panel + summary markers');
check('self-proof states it runs OFFLINE with NO network', /no network/i.test(build), 'honest offline claim present');
...
if (process.argv.includes('--emit') && allPass) {
  const receipt = {
    schema: 'cael-gate-v1', gate: 37, track: 'flagship', name: 'self-proving boot (offline data re-derivation)',
    timestamp: new Date().toISOString(),
    checks: { passed, total },
    manifest: manifest.map((m) => ({ name: m.name, key: m.key, sha256: m.sha256 })),
    proves: {
      offline: 'the build re-hashes its embedded data blobs in the player\'s own browser with a pure-JS SHA-256 (no crypto.subtle, works on file://) — no network',
      genuine: 'the embedded SHA-256 fn matches node:crypto on 4 known vectors incl. UTF-8 — a fake echo fn fails',
      reproduces: 'the embedded fn over the embedded blobs reproduces the sealed manifest (browser self-check passes on a clean machine)',
      loadBearing: 'negative control: a one-byte tamper diverges from the manifest',
    },
    honestScope: 'PROVEN: offline DATA-INTEGRITY self-derivation (the bytes you run are the bytes sealed at build time, re-checked on the player\'s machine). NOT claimed: re-running the TS gate verifiers on the drive (no repo/toolchain there) — the gate receipts ride on the drive for that.',
    aiHumanConnection: 'common receipt spine made portable: the proof travels WITH the artifact and re-derives offline — the human sees it re-check on their own machine, the skeptic can re-run it with no trust in us',
    status: 'PASS',
  };
