...
t(
  'NEGATIVE CONTROL: beta=0,gamma=0 collapses EXACTLY to the raw 0.5 threshold (no sharpening)',
  () => {
    ...
    assert.deepEqual(
      decision,
      raw,
      'with no evidence fusion and no coupling the layer must be a no-op vs raw'
    );
...
t('QUALITY: sharpened curation beats the raw 0.5 threshold ON AVERAGE by a real margin', () => {
  const agg = runAggregate();
  assert.ok(
    agg.meanAccSharp > agg.meanAccRaw,
    `sharp ${agg.meanAccSharp} must beat raw ${agg.meanAccRaw}`
  );
  assert.ok(agg.meanGain >= 0.03, `mean gain ${agg.meanGain} must be a real margin (>=0.03)`);
});

t(
  'CONTROL has ZERO net effect: meanAccControl === meanAccRaw (gain comes from the mechanism, not annealing luck)',
  () => {
    const agg = runAggregate();
...
t('flips are CORRECTIONS: a majority move TOWARD ground truth', () => {
  const agg = runAggregate();
  assert.ok(
    agg.fracFlipTowardTruth >= 0.6,
    `${(agg.fracFlipTowardTruth * 100).toFixed(1)}% toward truth must exceed 60%`
  );
});

t(
  'confident items stay LOCKED: the overwhelming majority of flips are near the 0.5 boundary',
  () => {
    const agg = runAggregate();
    assert.ok(
      agg.fracFlipNearBoundary >= 0.8,
      `${(agg.fracFlipNearBoundary * 100).toFixed(1)}% of flips must be near-boundary (<0.20)`
    );
  }
);
...
t('the whole experiment is deterministic (re-run reproduces every aggregate metric)', () => {
  ...
  const a = runSharpeningExperiment(1, 24),
    b = runSharpeningExperiment(1, 24);
