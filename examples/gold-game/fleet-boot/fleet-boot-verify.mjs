...
ok('manifest parses + has 3 tiers', man.tiers && man.tiers.offline && man.tiers.local && man.tiers.cloud);
ok('cloud tier DISABLED by default', man.tiers.cloud.enabled === false, `enabled=${man.tiers.cloud.enabled}`);
...
ok('dry-run runs offline + local tiers', /tier.*offline/.test(dry.out) && /orchestrator/.test(dry.out));
ok('dry-run leaves cloud skipped (disabled default)', /cloud.*skipped|"skipped":true/.test(dry.out));

// 3. NEGATIVE CONTROL — cloud enabled with NO cap must REFUSE (load-bearing gate)
const noCap = tmpManifest((m) => { m.tiers.cloud.enabled = true; m.tiers.cloud.budgetCapUsd = 0; });
const r1 = run(noCap, ['boot', '--tier', 'cloud']);
ok('NEG: cloud enabled w/ budgetCapUsd=0 is REFUSED (non-zero exit)', r1.code !== 0, `code=${r1.code}`);
ok('NEG: refusal message names the missing spend cap', /REFUSED.*cap/i.test(r1.out));
const negCap = tmpManifest((m) => { m.tiers.cloud.enabled = true; m.tiers.cloud.budgetCapUsd = -5; });
...
const capNoKeys = tmpManifest((m) => { m.tiers.cloud.enabled = true; m.tiers.cloud.budgetCapUsd = 5; m.tiers.cloud.requireKeys = ['DEFINITELY_MISSING_KEY_XYZ']; });
const r3 = run(capNoKeys, ['boot', '--tier', 'cloud', '--dry-run']);
ok('cloud w/ cap but missing keys → skips wake (exit 0, no spend)', r3.code === 0 && /missing/i.test(r3.out), `code=${r3.code}`);

// 5. cloud enabled w/ cap + dry-run + keys present → would wake under the cap (dry, no real spawn)
const capDry = tmpManifest((m) => { m.tiers.cloud.enabled = true; m.tiers.cloud.budgetCapUsd = 3; m.tiers.cloud.requireKeys = []; });
const r4 = run(capDry, ['boot', '--tier', 'cloud', '--dry-run']);
ok('cloud w/ cap (dry-run, no keys required) reports a bounded wake under the cap', r4.code === 0 && /cap \$3|budgetCapUsd|wake/i.test(r4.out), `code=${r4.code}`);
...
ok('drive plug-in entrypoint present (PLUG-IN-START.bat)', existsSync(join(HERE, 'PLUG-IN-START.bat')));

const passed = checks.every((c) => c.pass);
for (const c of checks) console.log((c.pass ? 'PASS ' : 'FAIL ') + c.name + (c.detail ? ' — ' + c.detail : ''));
console.log(`\n${checks.filter((c) => c.pass).length}/${checks.length} checks pass`);
if (!passed) { console.error('FLEET-BOOT GATE FAILED: ' + checks.filter((c) => !c.pass).map((c) => c.name).join(', ')); process.exit(1); }
