...
const { validatePortalIntent } = await imp(
  join(repo, 'packages', 'mcp-server', 'src', 'holo-portal-intent.ts')
);
...
const POLICY = {
  ...
  allowedScopes: ['read-only', 'mutate-zone', 'drive-avatar'],
  mutableZoneGlobs: ['W_GOLD_*', 'B_GRADUATE_*'],
  driveAvatar: { allow: true, maxEntities: 16 },
  enforcement: { onScopeViolation: 'reject' },
};
...
const verdictDigest = computeStateDigest(
  {
    ...
    getField: () =>
      Float32Array.from([
        b(V.grabDriveAvatar.allowed),
        b(V.grabReadOnly.allowed),
        b(V.grabZoneMatch.allowed),
        b(V.grabZoneNoMatch.allowed),
        b(V.sayDriveAvatar.allowed),
        b(V.sayMutateZone.allowed),
      ]),
  },
  'sha256'
);
...
const receipt = {
  ...
  artifact:
    'examples/gold-game/drive-build/index.html (WebXR build that bundles the real validator)',
  validator:
    'packages/mcp-server/src/holo-portal-intent.ts validatePortalIntent (genuine, not a mock)',
  ...
  buildEmbeds: {
    holoGateValidator: buildEmbedsHoloGate,
    controllerInteraction: buildHasInteraction,
    entryPortalMenu: buildHasMenu,
  },
  contract: {
...
if (emit) {
  ...
  try {
    ...
    console.error('No Gate-6 receipt. Run --emit first.');
    process.exit(2);
  }
  const checks = [
    ...
    [
      'REAL HoloGate: grab ALLOWED under mutate-zone when target matches a glob',
      V.grabZoneMatch.allowed === true,
    ],
    [
      'REAL HoloGate: grab DENIED under mutate-zone when target matches NO glob',
      V.grabZoneNoMatch.allowed === false,
    ],
    ['REAL HoloGate: say (embodied) ALLOWED under drive-avatar', V.sayDriveAvatar.allowed === true],
    [
      'REAL HoloGate: say DENIED under mutate-zone (needs drive-avatar)',
      V.sayMutateZone.allowed === false,
    ],
    ...
    [
      'verdict digest reproduces (real computeStateDigest)',
