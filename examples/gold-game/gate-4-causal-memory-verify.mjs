...
import {
  ...
  buildCausalModel,
  loadMemory,
} from './gold-game-causal-session.mjs';
...
function spawnSession(label, rounds) {
  const out = execFileSync(
    process.execPath,
    [tsxCli, sessionModule, label, String(rounds), sandboxDir, memoryPath],
...
const s1UnlockedLocked =
  !seedGraduatable.includes('E.LOCKED.003') && afterS1Graduatable.includes('E.LOCKED.003');
...
const allHubsGraduated = ['E.HUB.001', 'E.HUB.002'].every((id) =>
...
const interventionRefusesUnknown = await buildCausalModel(sandboxDir).then((m) => {
  try {
    m.intervention('NOPE', 1);
    return false;
  } catch {
    return true;
  }
...
const receipt = {
  ...
  sessionModule:
    'examples/gold-game/gold-game-causal-session.mjs (run as a separate process per session)',
  target: 'r3f',
  processSeparation: {
    session1Pid: s1.pid,
    session2Pid: s2.pid,
    verifierPid: process.pid,
    genuinelySeparateProcesses: crossProcess,
    sharedStateChannel: 'disk file only: ' + 'vault-sandbox-g4/npc-memory.json',
  },
  causalModel: {
    ...
    edges: EDGES,
    semantics:
      'graduating a hub = do(hubLineage=' +
      GRADED_INFLUENCE +
      '); linear structural eqs propagate a lineage boost to linked entries; a sub-threshold entry (lineage 0) crosses into graduatable as a downstream causal effect',
    worldEvolution: {
      seedGraduatable,
  ...
  crossSessionMemory: {
    persistedTo:
      'examples/gold-game/vault-sandbox-g4/npc-memory.json (real disk file; the ONLY channel between two separate OS processes — session 2 reads it cold from disk)',
    session1Pid: s1.pid,
    session2Pid: s2.pid,
    processSeparated: crossProcess,
...
if (emit) {
  ...
  console.log(
    ...
    's2=' + s2.pid,
    'parent=' + process.pid,
    'separate=' + crossProcess
  );
  console.log(
    '  afterS1Graduatable=' + JSON.stringify(afterS1Graduatable),
    'unlockedLocked=' + s1UnlockedLocked
  );
  ...
  try {
    ...
    console.error('No Gate-4 receipt to verify. Run with --emit first.');
    process.exit(2);
  }
  const checks = [
    ['flagship .holo parses (0 errors)', (parsed.errors || []).length === 0],
    [
      'locked entry (lineage 0) is NOT graduatable at seed',
      !seedGraduatable.includes('E.LOCKED.003'),
    ],
    [
      'real do-calculus intervention UNLOCKED the locked entry (world evolved causally)',
      s1UnlockedLocked === true,
    ],
    [
      'locked entry became graduatable AFTER session 1 (downstream causal effect)',
      afterS1Graduatable.includes('E.LOCKED.003'),
    ],
    [
      'the two sessions ran in GENUINELY SEPARATE OS processes (distinct PIDs)',
      crossProcess === true,
    ],
    [
      'session 2 loaded NON-EMPTY memory from DISK (the only cross-process channel)',
      !s2ColdWouldRepeat,
    ],
    [
      'session 2 did NOT re-graduate any session-1 graduation (memory across processes)',
      s2RepeatedS1 === false,
    ],
    [
      'NPC recorded a learned causal payoff (>=1 unlock) in persisted memory',
      learnedPayoffPick === true,
    ],
    [
      'the locked entry was EVENTUALLY graduated (only reachable via the causal unlock)',
      lockedEntryEventuallyGraduated === true,
    ],
    ['both hubs ended in gold on disk (real file moves)', allHubsGraduated === true],
    [
      'variational gate: causal intervention refuses an unknown variable',
      interventionRefusesUnknown === true,
    ],
    ['NPC memory file persisted to disk (cross-process substrate)', memoryPersisted === true],
    [
      'causal-effect (evolved-world) digest reproduces',
      causalEffectDigest === existing.contract.causalEffectDigest,
    ],
    ['cross-session NPC-memory digest reproduces', memDigest === existing.contract.npcMemoryDigest],
  ];
  let ok = true;
  console.log(
    'GATE-4 (FLAGSHIP CAUSAL+MEMORY) VERIFICATION (process-separated sessions, REAL contract fn + REAL CausalWorldModel):'
  );
