...
ok(
  r.receipt && r.receipt.fromTier === 'bronze' && r.receipt.toTier === 'gold',
  'receipt: bronze -> gold (silver skipped, per farm.py)'
);
...
ok(
  fs.existsSync(path.join(dir, 'gold', 'B.GRADUATE.001.json')),
  'gold/B.GRADUATE.001.json exists on disk'
);
const recDir = path.join(dir, 'receipts');
ok(
  fs.readdirSync(recDir).some((f) => f.includes('B.GRADUATE.001')),
  'graduation receipt written to disk'
);
...
ok(
  r2.receipt.stateDigest === r.receipt.stateDigest,
  'stateDigest reproduces across runs (deterministic)'
);
