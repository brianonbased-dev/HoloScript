...
const HONEST = 8, SPAMMERS = 3;                // the population
const STAKE_SWEEP = [0, 5, 10, 15, 20, 25];    // 0 → reward
const TRUST_DELTA = 2, THRESHOLD = 55;         // reputation: 50 → 52 → 54 → 56 (crosses at the 3rd)
...
function runStake(stake) {
  ...
  const spend = (id, a) => economyPrimitivesHandler.onEvent(econNode, ECON, econ.ctx, { type: 'economy:spend', payload: { agentId: id, amount: a } });
  const earn = (id, a) => economyPrimitivesHandler.onEvent(econNode, ECON, econ.ctx, { type: 'economy:earn', payload: { agentId: id, amount: a } });
  const bal = (id) => econNode.__economyState?.accounts?.get?.(id)?.balance ?? ECON.initial_balance;
  const observe = (id, d) => reputationLedgerHandler.onEvent(repNode, REP, rep.ctx, { type: 'reputation_observe_action', observerId: id, action: 'graduate', trustDelta: d });
  ...
  let totalInflation = 0; const ttd = [];
  ...
  let spamLost = 0, spamSelfLimited = 0;
  for (let s = 0; s < SPAMMERS; s++) {
    const id = 'spam-' + s; earn(id, 0); const before = bal(id); let insufficient = false;
    for (let m = 0; m < M; m++) {
      if (stake > 0) { const n = econ.events.length; spend(id, stake); if (econ.events.slice(n).some((e) => e.type === 'economy:insufficient_funds')) { insufficient = true; continue; } }
      // graduation FAILS → no earn
    }
    if (bal(id) < before) spamLost++;
    if (insufficient) spamSelfLimited++;
  }
  return { stake, inflationPerAgent: totalInflation / HONEST, ttdMedian: ttd.slice().sort((a, b) => a - b)[Math.floor(ttd.length / 2)], spamLost, spamSelfLimited };
}
...
const spamSelfLimitsAtHighStake = sweep.find((r) => r.stake === REWARD)?.spamSelfLimited === SPAMMERS;

const sweepDigest = computeStateDigest({ fieldNames: ['s'], getField: () => Float32Array.from([...inflationCurve, ...ttdCurve, sweep.reduce((a, r) => a + r.spamLost, 0)]) }, HASH);
...
if (emit) {
  ...
  console.log('  monotonic↓=' + monotonicDecreasing, 'flattens@reward=' + flattensAtReward, 'ttdConstant=' + ttdConstant, 'spamLosesEverywhere=' + spamLosesEverywhere);
  console.log('  sweepDigest=' + sweepDigest);
} else {
  let existing; try { existing = JSON.parse(readFileSync(receiptPath, 'utf8')); } catch { console.error('No Gate-21 receipt. Run --emit first.'); process.exit(2); }
  const checks = [
    ['inflation curve is MONOTONICALLY DECREASING in the stake', monotonicDecreasing === true],
    ['inflation FLATTENS to ~0 at stake==reward (sink fully absorbs the reward)', flattensAtReward === true],
    ['at zero stake the economy INFLATES (~M×reward) — the sink is load-bearing', inflatesAtZeroStake === true],
    ['time-to-Diamond is BOUNDED across the whole population (<=5 graduations)', ttdBounded === true],
    ['time-to-Diamond is CONSTANT across the stake sweep — progression decoupled from credits', ttdConstant === true],
    ['spammers LOSE at every stake>0 (final balance < initial)', spamLosesEverywhere === true],
    ['spammers SELF-LIMIT (insufficient_funds) at the full stake', spamSelfLimitsAtHighStake === true],
    ['sweep digest reproduces (real computeStateDigest)', sweepDigest === existing.contract.sweepDigest],
  ];
  let ok = true;
