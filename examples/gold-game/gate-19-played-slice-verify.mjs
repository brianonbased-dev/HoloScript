...
const { runPlayedSlice, GOAL_GRADUATIONS, RATIFY_THRESHOLD } = await imp(join(here, 'gold-game-loop.mjs'));
...
if (emit) {
  ...
  console.log('  T1 human graduate:', t.T1.ok, 'by=' + t.T1.by, 'vaultChanged=' + t.T1.vaultChanged);
  console.log('  T2 companion:', t.T2.pick, 'different=' + t.T2.differentFromHuman, 'earned=' + t.T2.earnedReward, 'trust>init=' + t.T2.trustAboveInitial);
  console.log('  T3 collision blocked+unchanged:', t.T3.blockedAndUnchanged);
  console.log('  T4 trustArc:', t.T4.trustArc.join('->'), 'cross@' + t.T4.firstCrossIdx, 'accum=' + t.T4.accumulationDriven);
  console.log('  T5 archivist:', t.T5.action, 'node=' + t.T5.node, 'negControl=' + t.T5.negativeControl);
  console.log('  T6 win=' + t.T6.win, 'goldDelta=' + t.T6.goldDelta);
} else {
  let existing; try { existing = JSON.parse(readFileSync(receiptPath, 'utf8')); } catch { console.error('No Gate-19 receipt. Run --emit first.'); process.exit(2); }
  const checks = [
    ['T1 — HUMAN graduate fired: real verb ok, sealed receipt by human-curator, WORLD changed (vault digest)', t.T1.ok === true && t.T1.by === 'human-curator' && t.T1.schema === 'cael-graduation-v1' && t.T1.vaultChanged === true],
    ['T2 — AI companion graduated INDEPENDENTLY (different entry) with real receipt on disk', t.T2.ok === true && t.T2.by === 'agent-companion' && t.T2.differentFromHuman === true && t.T2.receiptOnDisk === true],
    ['T2 — companion did REAL economy work (balance += reward) and gained reputation', t.T2.earnedReward === true && t.T2.trustAboveInitial === true && t.T2.ceilingOk === true],
    ['T3 — the collision "monster" BLOCKS (graduate ok:false) and the entry tier is UNCHANGED', t.T3.blockedAndUnchanged === true],
    ['T4 — reputation crossed the ratify threshold by ACCUMULATION (not a flag)', t.T4.firstCrossIdx >= 0 && t.T4.accumulationDriven === true && t.T4.finalTrust / 100 > RATIFY_THRESHOLD],
    ['T5 — Archivist RATIFIED via the real DialogueTrait (action=ratify, node=quest_open)', t.T5.ratified === true && t.T5.questOpened === true],
    ['T5 — negative control: below threshold the SAME config yields send_back (gated, not scripted)', t.T5.negativeControl === 'send_back'],
    ['T6 — GOAL met: Gold terrace filled (>= ' + GOAL_GRADUATIONS + ') AND Archivist ratified → win', t.T6.win === true && t.T6.goldFilled === true],
    ['the played loop is DETERMINISTIC (win + digests reproduce across a fresh climb)', t.T6.win === t2.T6.win && t.T6.vaultDigestFinal === t2.T6.vaultDigestFinal && t.T6.verdictDigest === t2.T6.verdictDigest],
    ['win digests match the committed receipt (real computeStateDigest)', t.T6.vaultDigestFinal === existing.trace.T6.vaultDigestFinal && t.T6.verdictDigest === existing.trace.T6.verdictDigest],
  ];
  let ok = true;
