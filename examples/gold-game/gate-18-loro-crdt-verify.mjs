...
const A = vaultDoc(10n);
A.import(baseSnap);
const B = vaultDoc(20n);
B.import(baseSnap);

// concurrent, independent edits on DIFFERENT entries (the no-lost-update case)
A.getMap('vault').set('W.001', 'gold');
A.commit(); // curator A graduates W.001
B.getMap('vault').set('W.002', 'gold');
B.commit(); // curator B graduates W.002 — concurrently
// concurrent edit on the SAME entry (the conflict case): both graduate W.003 to different tiers
A.getMap('vault').set('W.003', 'gold');
A.commit();
B.getMap('vault').set('W.003', 'platinum');
B.commit();
...
const C = vaultDoc(30n);
C.import(baseSnap);
C.import(updA);
C.import(updB); // A then B
const D = vaultDoc(40n);
D.import(baseSnap);
D.import(updB);
D.import(updA); // B then A
...
const commutative =
  sortedJSON(stateC) === sortedJSON(stateD) && sortedJSON(stateC) === sortedJSON(stateA);
...
const convergedSolver = {
  fieldNames: ['v'],
  getField: () =>
    Float32Array.from(
      Object.entries(stateA)
        .sort()
        .flatMap(([k, v]) => [idI(k), tierCode(v)])
    ),
};
...
function rerun() {
  const b = vaultDoc(1n);
  const m = b.getMap('vault');
  m.set('B.GRADUATE.001', 'bronze');
  m.set('W.001', 'bronze');
  m.set('W.002', 'silver');
  m.set('W.003', 'silver');
  b.commit();
  ...
  const a = vaultDoc(10n);
  a.import(s);
  const bb = vaultDoc(20n);
  bb.import(s);
  a.getMap('vault').set('W.001', 'gold');
  a.commit();
  bb.getMap('vault').set('W.002', 'gold');
  bb.commit();
  a.getMap('vault').set('W.003', 'gold');
  a.commit();
  bb.getMap('vault').set('W.003', 'platinum');
  bb.commit();
  a.import(bb.export({ mode: 'update' }));
  bb.import(a.export({ mode: 'update' }));
...
const receipt = {
  ...
  experiment: {
    base: { 'B.GRADUATE.001': 'bronze', 'W.001': 'bronze', 'W.002': 'silver', 'W.003': 'silver' },
    concurrent: {
      A: { 'W.001': 'gold', 'W.003': 'gold' },
      B: { 'W.002': 'gold', 'W.003': 'platinum' },
    },
  },
  result: {
    convergedState: stateA,
    converged,
    bothConcurrentEditsKept,
...
if (emit) {
  ...
  console.log(
    '  converged=' + converged,
    'bothEditsKept=' + bothConcurrentEditsKept,
    'conflictSame=' + conflictResolvedSame + ' (winner=' + conflictWinner + ')'
  );
  ...
  try {
    ...
    console.error('No Gate-18 receipt. Run --emit first.');
    process.exit(2);
  }
  const checks = [
    ...
    [
      'no lost update — BOTH concurrent edits kept (W.001=gold AND W.002=gold)',
      bothConcurrentEditsKept === true,
    ],
    [
      'same-entry conflict resolves to the SAME winner on both replicas',
      conflictResolvedSame === true,
    ],
    ...
    [
      'converged-state digest reproduces (real computeStateDigest)',
      convergedDigest === existing.contract.convergedDigest,
    ],
  ];
  let ok = true;
