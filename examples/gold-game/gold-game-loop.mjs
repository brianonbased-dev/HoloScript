...
export async function runPlayedSlice(sandboxDir) {
  ...
  const { dialogueHandler } = await imp(join(repo, 'packages', 'core', 'src', 'traits', 'DialogueTrait.ts'));
  ...
  const COLLISION = { id: 'C.COLLIDE.001', title: 'same-slot-conflict', tier: 'bronze', lineage_links: 0 }; // the "monster": lineage 0 → graduate refuses
  for (const e of [...CLEAN, COLLISION]) fs.writeFileSync(join(sandboxDir, 'bronze', e.id + '.json'), JSON.stringify(e, null, 2));
  ...
  const vaultDigestOf = () => { const st = V.readState(sandboxDir); const idI = (s) => { let h = 0; for (let i = 0; i < s.length; i++) h = (h * 131 + s.charCodeAt(i)) % 2147483647; return h; }; const ti = { bronze: 0, silver: 1, gold: 2, platinum: 3, diamond: 4 }; const pairs = []; for (const t of V.ALL_TIERS) for (const id of st.tiers[t]) pairs.push([idI(id), ti[t]]); pairs.sort((a, b) => a[0] - b[0] || a[1] - b[1]); return computeStateDigest({ fieldNames: ['v'], getField: () => Float32Array.from(pairs.flat()) }, 'sha256'); };

  // ── Real sovereign-trait instances (the companion's economy + the shared reputation) ──
  const econNode = {}; const econ = makeCtx(); economyPrimitivesHandler.onAttach(econNode, economyPrimitivesHandler.defaultConfig, econ.ctx); const ECON = economyPrimitivesHandler.defaultConfig;
  ...
  const agNode = {}; const ag = makeCtx(); const AG = { agent_class: 'npc', daily_budget_usd: 0.5, tick_interval_ms: 0 }; autonomousAgendaHandler.onAttach(agNode, AG, ag.ctx);
  ...
  const humanGrads = []; const trustArc = []; let firstCrossIdx = -1;
  ...
  for (let i = 0; i < GOAL_GRADUATIONS; i++) {
    ...
    if (i === 1) {
      ...
      trace.T3 = { attempted: COLLISION.id, ok: blocked.ok, error: blocked.error, tierBefore, tierAfter, blockedAndUnchanged: !blocked.ok && tierBefore === tierAfter };
    }
    const r = V.graduate(sandboxDir, id, PLAYER);
    observe(PLAYER, 'graduated:' + id, HUMAN_QUALITY);
    const trust = trustOf(PLAYER); trustArc.push(trust);
    if (firstCrossIdx < 0 && trust / 100 > RATIFY_THRESHOLD) firstCrossIdx = i;
    const vdNow = vaultDigestOf();
    humanGrads.push({ id, ok: r.ok, by: r.receipt?.by, schema: r.receipt?.schema, toTier: r.receipt?.toTier, trust, vaultChanged: vdNow !== vdPrev });
    vdPrev = vdNow;
    if (i === 0) trace.T1 = { entry: id, ok: r.ok, by: r.receipt?.by, schema: r.receipt?.schema, vaultChanged: humanGrads[0].vaultChanged, vaultDigest: vdNow };
  }
  trace.T4 = { initialTrust: REP.initial_trust ?? 50, quality: HUMAN_QUALITY, trustArc, finalTrust: trustArc[trustArc.length - 1], firstCrossIdx, accumulationDriven: trustArc[trustArc.length - 1] === (REP.initial_trust ?? 50) + HUMAN_QUALITY * GOAL_GRADUATIONS };
  ...
  earn(COMPANION); observe(COMPANION, 'graduated:' + compPick, 3);
  autonomousAgendaHandler.onEvent(agNode, AG, ag.ctx, { type: 'agenda_add_item', id: 'c1', title: 'graduate ' + compPick, estimatedCostUsd: 0.1 });
  autonomousAgendaHandler.onEvent(agNode, AG, ag.ctx, { type: 'agenda_complete_item', itemId: 'c1' });
  autonomousAgendaHandler.onUpdate(agNode, AG, ag.ctx, 1.0);
  const compReceiptOnDisk = fs.readdirSync(join(sandboxDir, 'receipts')).some((f) => { try { return JSON.parse(fs.readFileSync(join(sandboxDir, 'receipts', f), 'utf8')).by === COMPANION; } catch { return false; } });
  trace.T2 = { companion: COMPANION, pick: compPick, differentFromHuman: !humanGrads.some((g) => g.id === compPick), ok: cr.ok, by: cr.receipt?.by, receiptOnDisk: compReceiptOnDisk, balanceBefore: compBalBefore, balanceAfter: balanceOf(COMPANION), earnedReward: (balanceOf(COMPANION) ?? 0) - compBalBefore === REWARD, trust: trustOf(COMPANION), trustAboveInitial: trustOf(COMPANION) > (REP.initial_trust ?? 50), ceilingOk: !ag.events.some((e) => e.type === 'agenda_cost_ceiling_breach') };

  // ── T5: the Archivist ratifies (real DialogueTrait decides via filterOptions) ──
  const DLG = { ...dialogueHandler.defaultConfig, speaker_name: 'Maren Vael', start_node: 'verdict', llm_dynamic: false, dialogue_tree: {
    verdict: { id: 'verdict', speaker: 'Maren Vael', text: 'I have read it. The ledger remembers.', options: [
      { text: '(she ratifies it)', condition: 'reputation > 0.55', action: 'ratify', nextNode: 'quest_open' },
      { text: '(she sends it back)', action: 'send_back', nextNode: 'farewell' },
    ] },
    quest_open: { id: 'quest_open', speaker: 'Maren Vael', text: 'It earns its place. Now — recover the Drowned Index.', options: [] },
    farewell: { id: 'farewell', speaker: 'Maren Vael', text: 'It decays in a week.', options: [] },
  } };
  const verdictAt = (reputation) => { const n = {}; const c = makeCtx(); dialogueHandler.onAttach(n, DLG, c.ctx); dialogueHandler.onEvent(n, DLG, c.ctx, { type: 'start_dialogue', startNode: 'verdict', context: { reputation } }); dialogueHandler.onEvent(n, DLG, c.ctx, { type: 'select_option', index: 0 }); return { action: last(c.events, 'dialogue_action')?.payload?.action, node: n.__dialogueState?.currentNodeId }; };
  const atPeak = verdictAt(trace.T4.finalTrust / 100);
  const negControl = verdictAt(0.50); // below threshold → send_back, ratify option absent
  const QUEST = { id: 'recover-the-drowned-index', giver: 'npc.maren-archivist', questType: 'discover', reward: { reputation: 10, unlocks: 'sealed-reading-room' } };
  trace.T5 = { reputationAtPeak: trace.T4.finalTrust / 100, action: atPeak.action, node: atPeak.node, ratified: atPeak.action === 'ratify' && atPeak.node === 'quest_open', questOpened: atPeak.action === 'ratify', quest: atPeak.action === 'ratify' ? QUEST : null, negativeControl: negControl.action };
  ...
  const econDigest = computeStateDigest({ fieldNames: ['e'], getField: () => Float32Array.from([balanceOf(PLAYER) ?? 100, balanceOf(COMPANION) ?? 100]) }, 'sha256');
  const verdictDigest = computeStateDigest({ fieldNames: ['d'], getField: () => Float32Array.from([Math.round(trace.T4.finalTrust), trace.T5.ratified ? 1 : 0, win ? 1 : 0]) }, 'sha256');
  trace.T6 = { goldNow: goldCount(), goldDelta: goldCount() - baseGold, goalGraduations: GOAL_GRADUATIONS, goldFilled, ratified: trace.T5.ratified, win, vaultDigestFinal: vaultDigestOf(), econDigest, verdictDigest };
  return trace;
}
