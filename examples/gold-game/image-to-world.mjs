...
if (!imagePath) {
  console.error(
    'usage: tsx examples/gold-game/image-to-world.mjs --image <path> [--name <world>] [--grid 96x64] [--out <dir>] [--scale 8]'
  );
  process.exit(2);
}
const resolvedImage = path.isAbsolute(imagePath)
  ? imagePath
...
const { computeStateDigest } = await imp(
  path.join(repo, 'packages', 'engine', 'src', 'simulation', 'hashes.ts')
);
...
async function main() {
  ...
  try {
    ...
    console.error(
      `image-to-world: could not read/decode image at ${resolvedImage}\n  ${e.message}`
    );
    process.exit(1);
  }
  ...
  const svc = DepthEstimationService.getInstance({
    backend: BACKEND,
    maxResolution: 512,
    enableCache: false,
  });
  ...
  if (normalsMatch)
    for (let i = 0; i < normalMap.length; i++)
      if (Math.abs(reNorm[i] - normalMap[i]) > 1e-6) {
        normalsMatch = false;
        break;
      }
  ...
  console.log(
    `  depth range:  ${+depthRange.toFixed(4)}  (min ${+dMin.toFixed(4)}, max ${+dMax.toFixed(4)})  normalsMatch=${normalsMatch}`
  );
