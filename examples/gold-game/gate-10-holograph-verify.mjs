...
const CORPUS = [
  { id: 'W.GOLD.537', title: 'GOLD GAME — AI↔human connection proven on both tracks', refs: ['W.137'] },
  { id: 'W.GOLD.534', title: 'Audit-as-Calibration Applied to One’s Own Conclusions', refs: ['W.501', 'W.511', 'W.513', 'W.514', 'W.GOLD.190', 'W.GOLD.191'] },
  { id: 'W.GOLD.535', title: 'Secrets-Broker as Sovereign Primitive — Capability-Token Architecture', refs: ['W.GOLD.002'] },
  { id: 'W.GOLD.191', title: 'Audit-as-Calibration Under Confident Peer Claims — The Nature of Audit', refs: ['W.GOLD.190'] },
  { id: 'W.GOLD.190', title: 'First Iterations Always Lack — Preserve Iteration-1 As Paired Evidence', refs: [] },
  { id: 'W.GOLD.002', title: 'Sovereign vs Bridge Compilers — Pour Resources into Sovereign', refs: [] },
];
const ids = new Set(CORPUS.map((e) => e.id));

// ── HoloGraph: exact-by-construction directed graph (edges = real references) ──
const outEdges = Object.fromEntries(CORPUS.map((e) => [e.id, e.refs.filter((r) => ids.has(r)).sort()]));
...
function reachable(start) {
  ...
  while (q.length) { for (const n of traverse(q.shift())) if (!seen.has(n)) { seen.add(n); q.push(n); } }
  return seen;
}
...
function retrieve(query, k = 3) {
  const qv = enc(query);
  return CORPUS.map((e) => ({ id: e.id, score: cosine(qv, vecs[e.id]) })).sort((a, b) => b.score - a.score).slice(0, k);
}
...
const retrievalDeterministic = JSON.stringify(auditQuery.map((x) => x.id)) === JSON.stringify(auditQuery2.map((x) => x.id));
...
const normRound = CORPUS.map((e) => Math.round(Math.sqrt(vecs[e.id].reduce((s, x) => s + x * x, 0)) * 1000));
const graphDigest = computeStateDigest({ fieldNames: ['g'], getField: () => Float32Array.from(adjFlat.concat(auditQuery.map((x) => idI(x.id)))) }, HASH);
const embedDigest = computeStateDigest({ fieldNames: ['e'], getField: () => Float32Array.from(normRound) }, HASH);
...
const receipt = {
  ...
  implementation: {
    ...
    holoGraph: 'exact-by-construction lineage graph (S.HGRAPH model: explicit reference edges -> recall 1.0)',
  },
  corpus: { source: 'pinned snapshot of the real D:/GOLD lineage constellation (ids/titles/refs read live from the vault)', entries: CORPUS.map((e) => ({ id: e.id, refs: e.refs })) },
...
if (emit) {
  ...
  console.log('  HoloEmbed dim=' + dim + '/' + HOLOEMBED_DIM, '| exactByConstruction=' + exactByConstruction, '| inCorpusEdges=' + inCorpusEdgeCount);
  console.log('  reachableFrom534=' + reach534.size, '| auditTop=' + auditTop + ' relevant=' + auditRelevant + ' deterministic=' + retrievalDeterministic);
  console.log('  graphDigest=' + graphDigest.slice(0, 16), 'embedDigest=' + embedDigest.slice(0, 16));
} else {
  ...
  const checks = [
    ...
    ['graph digest reproduces (real computeStateDigest)', graphDigest === existing.contract.graphDigest],
    ['embed digest reproduces (real computeStateDigest)', embedDigest === existing.contract.embedDigest],
  ];
  let ok = true;
