...
import {
  ...
  fairLinearScore, crippledLineageScore, TRUE_LINEAR, TRUE_INTERACTION,
...
const accTrained = top1(testEps, (pool) => argmaxBy(pool, (e) => scoreModel(model, liftFeatures, e.f)));
const accBestLinearFit = top1(testEps, (pool) => argmaxBy(pool, (e) => scoreModel(linFit, rawFeat, e.f)));
...
const policyDigest = computeStateDigest(
  { fieldNames: ['policy'], getField: () => Float32Array.from([
    ...model.w.map(r4), r4(model.b), r4(accTrained), r4(accBestLinearFit), r4(accFair), r4(accCrippled), r4(accRandom),
  ]) },
  HASH_MODE,
);
...
const receipt = {
  ...
  training: {
    method: 'batch gradient descent, linear-in-features model with an L·D interaction feature, zero-init — deterministic (seeded mulberry32, no wall-clock/Math.random)',
    seed: 20260522, episodes: all.length, trainEpisodes: trainEps.length, testEpisodes: testEps.length,
    learnedWeights: { L: r4(model.w[0]), D: r4(model.w[1]), Tinv: r4(model.w[2]), 'L·D': r4(model.w[3]), bias: r4(model.b) },
...
if (emit) {
  ...
  console.log('  top1 trained=' + r4(accTrained), 'fair=' + r4(accFair), 'bestLinear=' + r4(accBestLinearFit), 'crippled=' + r4(accCrippled), 'random=' + r4(accRandom));
  console.log('  marginOverFair=' + r4(marginOverFair), 'interactionWeight=' + r4(interactionWeight));
  ...
  const checks = [
    ['training is deterministic (weights reproduce)',
      r4(model.w[0]) === existing.training.learnedWeights.L && r4(model.w[3]) === existing.training.learnedWeights['L·D']],
    ...
    ['the FAIR baseline is genuinely fair: uses all features and beats the crippled lineage-only rule', accFair > accCrippled],
    ['the task is NON-TRIVIAL: a fair linear rule CANNOT solve it (< 0.95) — the interaction matters', accFair < 0.95],
    ['the advantage IS the non-linearity: trained beats even the best pure-linear fit', accTrained > accBestLinearFit],
    ['model learned the L·D keystone interaction (|w_interaction| > 0.05)', Math.abs(interactionWeight) > 0.05],
  ...
  console.log('  trained=' + r4(accTrained), 'fair=' + r4(accFair), 'bestLinear=' + r4(accBestLinearFit), 'crippled=' + r4(accCrippled), 'random=' + r4(accRandom), 'margin=' + r4(marginOverFair));
