...
const { validateIdentityAdmission } = await imp(join(repo, 'packages', 'mcp-server', 'src', 'conformance', 'artifact-admission-gate.ts'));
...
await T('twin_earth_register_identity', { agentId: FOUNDER, walletAddress: wallet(1), handle: FOUNDER, attestation: ATTEST, kind: 'ai', role: 'founder', mode: 'local' });
const regResults = {};
for (let i = 0; i < CURATORS.length; i++) {
  regResults[CURATORS[i]] = await T('twin_earth_register_identity', { agentId: CURATORS[i], walletAddress: wallet(i + 2), handle: CURATORS[i], attestation: ATTEST, kind: 'ai', role: 'curator', mode: 'local' });
}
...
const admissionCritical = Object.fromEntries(CURATORS.map((c) => [c, admit(c).filter((f) => f.severity === 'critical').length]));
...
const malformed = { agentId: 'bad', walletAddress: wallet(99), handle: 'bad', attestation: '', attestedAt: '', role: 'curator', mode: 'local', kind: 'ai', revoked: false, createdAt: '', modifiedAt: '' };
const malformedFindings = validateIdentityAdmission(malformed);
const malformedRejected = malformedFindings.some((f) => f.ruleId === 'IDENTITY-003' && f.severity === 'critical');

// ── 2. PERMISSION — founder grants 'graduate'/'vault'; validate gates each act ─
const grantResults = {};
for (const c of CURATORS) grantResults[c] = await T('twin_earth_grant_permission', { granteeId: c, granterId: FOUNDER, action: 'graduate', scope: 'vault' });
const allGranted = CURATORS.every((c) => grantResults[c]?.success === true);
const validGraduate = {};
for (const c of CURATORS) validGraduate[c] = (await T('twin_earth_validate_permission', { granteeId: c, action: 'graduate', scope: 'vault' }))?.valid === true;
const allMayGraduate = CURATORS.every((c) => validGraduate[c] === true);
// NEGATIVE: an UNGRANTED action must be denied
const deleteDenied = (await T('twin_earth_validate_permission', { granteeId: CURATORS[0], action: 'delete_vault', scope: 'vault' }))?.valid === false;
...
const envResults = {};
for (const c of CURATORS) envResults[c] = await T('twin_earth_create_safety_envelope', { agentId: c, allowedActions: ALLOWED, blockedActions: BLOCKED, deterministic: true, localOnly: true, maxTickDurationMs: 1000 });
const allEnveloped = CURATORS.every((c) => envResults[c]?.success === true && envResults[c]?.substrateEnforced === true);
...
const withinEnvelope = (c, action) => { const e = envOf(c); return !!e && e.allowedActions.includes(action) && !e.blockedActions.includes(action); };
...
const mayCurate = (c) => admissionCritical[c] === 0 && validGraduate[c] && withinEnvelope(c, 'graduate');
const mayDelete = (c) => admissionCritical[c] === 0 && false /* no delete grant */ && withinEnvelope(c, 'delete_vault');
...
const b = (x) => (x ? 1 : 0);
const governanceDigest = computeStateDigest({ fieldNames: ['g'], getField: () => Float32Array.from(
  CURATORS.flatMap((c) => [b(admissionCritical[c] === 0), b(validGraduate[c]), b(withinEnvelope(c, 'graduate')), b(!withinEnvelope(c, 'delete_vault')), b(mayCurate(c))])
    .concat([b(malformedRejected), b(deleteDenied)])) }, HASH);
...
const receipt = {
  ...
  implementation: {
    handlers: 'packages/mcp-server/src/robot-ai-mcp-tools.ts (handleRobotAiMcpTool — REAL twin_earth_* tools)',
    admission: 'packages/mcp-server/src/conformance/artifact-admission-gate.ts validateIdentityAdmission (REAL rule)',
  },
  identity: { registered: allRegistered, admittedCriticalFindings: admissionCritical, malformedRejected_IDENTITY003: malformedRejected },
  permission: { allGranted, mayGraduate: validGraduate, ungrantedActionDenied: deleteDenied },
  safety: { allEnveloped, substrateEnforced: true, allowedActions: ALLOWED, blockedActions: BLOCKED, graduateAllowedByEnv, destructiveActionBlockedByEnv: deleteBlockedByEnv },
  composed: { allCuratorsMayGraduate: allMayCurate, noneMayDelete, rule: 'graduate permitted iff admitted identity AND valid permission AND within safety envelope' },
  contract: { spine: 'REAL computeStateDigest', governanceDigest, reproducible: 'run the verifier to re-derive' },
  honestScope: 'Identity registration/admission, permission grant/validate, and safety-envelope creation are the GENUINE Twin-Earth handlers from @holoscript/mcp-server (handleRobotAiMcpTool over the real in-memory registries) + the real validateIdentityAdmission conformance rule — not mocks. PROVEN: each mesh curator runs under a real Twin-Earth identity, a real permission grant gates the curation verb (ungranted actions denied), and a substrate-enforced safety envelope makes a destructive action structurally impossible (D.044). Composes the governance over Gate 6 (HoloGate) + Gate 8 (mesh). Credentials are deterministic placeholders (wallet/attestation shaped, not on-chain-verified); on-chain attestation verification + live revocation flows deepen later.',
...
if (emit) {
  ...
  console.log('  registered=' + allRegistered, 'admitted=' + allAdmitted, 'malformedRejected=' + malformedRejected);
  console.log('  granted=' + allGranted, 'mayGraduate=' + allMayGraduate, 'deleteDenied=' + deleteDenied);
  console.log('  enveloped=' + allEnveloped, 'graduateAllowedByEnv=' + graduateAllowedByEnv, 'deleteBlockedByEnv=' + deleteBlockedByEnv);
  console.log('  composed: allMayCurate=' + allMayCurate, 'noneMayDelete=' + noneMayDelete);
  console.log('  governanceDigest=' + governanceDigest);
} else {
  let existing; try { existing = JSON.parse(readFileSync(receiptPath, 'utf8')); } catch { console.error('No Gate-9 receipt. Run --emit first.'); process.exit(2); }
  const checks = [
    ...
    ['REAL safety envelope: destructive action BLOCKED (structurally impossible)', deleteBlockedByEnv],
    ['composed governance: all curators may graduate; none may delete', allMayCurate && noneMayDelete],
    ['governance digest reproduces (real computeStateDigest)', governanceDigest === existing.contract.governanceDigest],
  ];
  let ok = true;
