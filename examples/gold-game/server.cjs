...
let vaultOps = null;
try {
  vaultOps = require('./vault-ops.cjs');
} catch (_) {
  /* optional; absent in some bundles */
}
const SANDBOX = path.join(here, 'vault-sandbox');
function ensureSandbox() {
  if (vaultOps && !fs.existsSync(path.join(SANDBOX, 'bronze'))) vaultOps.buildSandbox(SANDBOX);
}
...
function vaultState() {
  ...
  for (const t of tiers) {
    try {
      counts[t] = fs.existsSync(path.join(VAULT, t))
        ? fs.readdirSync(path.join(VAULT, t)).filter((f) => f.endsWith('.md')).length
        : 0;
    } catch (_) {
      counts[t] = 0;
    }
...
const server = http.createServer((req, res) => {
  ...
  if (url === '/api/vault') {
    ...
    res.end(JSON.stringify(vaultState()));
    return;
  }
  ...
  if (url === '/api/vault-list') {
    // Gate 28: enumerate + search/filter every entry
    ...
    const filtered =
      (q || tier || type || domain) && vaultOps
        ? vaultOps.filterCatalog(cat, { q, tier, type, domain })
        : cat.entries;
    res.writeHead(200, { 'content-type': 'application/json' });
    res.end(
      JSON.stringify({
        connected: cat.connected,
        vaultPath: cat.vaultPath,
        count: cat.count,
  ...
  if (url === '/api/vault-game') {
    // the playable sandbox vault state
    if (!vaultOps) {
      res.writeHead(503);
      res.end('{"error":"vault-ops unavailable"}');
      return;
    }
    ensureSandbox();
    res.writeHead(200, { 'content-type': 'application/json' });
    res.end(JSON.stringify(vaultOps.readState(SANDBOX)));
    return;
  }
  if (url === '/api/graduate' && req.method === 'POST') {
    // THE VERB: a play-action -> a real (sandbox) vault op
    if (!vaultOps) {
      res.writeHead(503);
      res.end('{"ok":false,"error":"vault-ops unavailable"}');
      return;
    }
    let body = '';
  ...
  if (!p.startsWith(here) || !fs.existsSync(p)) {
    res.writeHead(404);
    res.end('not found');
    return;
  }
...
function listen(port, tries) {
  ...
  server.listen(port, '127.0.0.1', () => {
    ...
    console.log(
      '  vault: ' +
        (v.connected
          ? 'connected (' + v.vaultPath + ', ' + (v.total || '?') + ' entries)'
          : 'not found - using the embedded snapshot')
    );
...
if (require.main === module && process.argv.includes('--help')) {
  ...
  console.log(
    'APIs: /api/vault, /api/vault-list?q=&tier=&type=&domain=, /api/vault-entry?id=W.GOLD.535, /api/vault-game, POST /api/graduate'
  );
