...
- **act** — pick a candidate entry (a raw wisdom / scar), then _capture → refine → link its lineage →
  submit to the Archivist → ratify_.
...
- **motivation to repeat** — higher tiers gate rarer/more valuable entries; the lineage constellation
  grows (collection + mastery); the Archivist _remembers_ and opens new dialogue/quests; and the honest
...
- **Bronze → Gold** = the on-ramp. In the real vault, `farm.py promote` auto-promotes Bronze→GOLD via
  lineage detection (skipping Silver). In-game: _forging enough lineage links unlocks graduation_ — fast,
  rewarding, teaches the verb.
- **Silver** is structurally fallow in the real vault (no detection logic). Honest design choice: make
  Silver an _optional polish/bonus_ path, not a required step — don't fake a tier the system skips.
...
| **Reputation** (`@reputationLedger`) | successful graduations; clean (collision-free) submissions; helping agents curate | **90-day TTL decay** (stop curating → standing fades); negative `trustDelta` on clobbering/failed submits | reputation gates the high tiers — the _progression_ currency, not a wallet |
...
first, keep scars"), reward graduating _gotchas/failures_ higher than tidy success notes, matching the
real vault's value model. This makes the optimal in-game strategy the _same_ as the optimal real-curation
...
- **Human session checklist:** Does a graduation _feel_ like an achievement? Is the collision "monster"
...
- to **/gamedev**: this is the Gate-2 core-loop spec (the verb that makes the parsed entities _do_
...
- to **/look-dev**: the moments to make _feel_ big — an entry ascending a tier, a Diamond ratification.
- **Status:** design + playtest _plan_; the agent-playtest run itself is the build target (needs the
