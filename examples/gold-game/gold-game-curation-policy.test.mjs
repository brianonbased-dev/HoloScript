...
t(
  'a learned interaction model beats the FAIR all-feature linear heuristic on held-out data',
  () => {
    const all = makeEpisodes(mulberry32(20260522), 400);
    const sp = Math.floor(all.length * 0.7);
    const tr = all.slice(0, sp),
      te = all.slice(sp);
...
t(
  'NEGATIVE/NON-TRIVIAL CONTROL: the fair linear baseline cannot solve the task (< 0.95) but beats the crippled one',
  () => {
    const all = makeEpisodes(mulberry32(20260522), 400);
