...
ok('all real vault entries placed', mount.entryCount === coords.coords.length && mount.entryCount > 100,
  `${mount.entryCount} entries`);
ok('lineage cables placed', mount.cableCount === (coords.lineage_edges || []).length && mount.cableCount > 0,
  `${mount.cableCount} cables`);
ok('mount is deterministic (byte-reproducible — Gate 30 safe)', mount.mountDigest === mount2.mountDigest,
...
ok('every cable connects two placed entries', mount.cables.every((c) => placed.has(c.from) && placed.has(c.to)),
...
const bands = Object.keys(tierYs).map(Number).sort((a, b) => a - b);
let monotonic = true;
for (let i = 1; i < bands.length; i++) if (tierYs[bands[i]] <= tierYs[bands[i - 1]]) monotonic = false;
ok('tier elevation monotonic (bronze base .. diamond peak)', monotonic && bands.length >= 3,
...
ok('build embeds the knowledge-mountain payload', embedded !== undefined && embedded !== null,
...
const embeddedGeomDigest = embedded
  ? sha256(JSON.stringify({ origin: embedded.origin, nodes: embedded.nodes, cables: embedded.cables }))
  : null;
const moduleGeomDigest = mount.mountDigest;
const embeddedDigest = embeddedGeomDigest; // exported into receipt below
ok('embedded geometry is byte-equal to the module output (anti-stale)',
  embeddedGeomDigest === moduleGeomDigest && embedded?.mountDigest === mount.mountDigest,
  embedded ? `${(embeddedGeomDigest || '').slice(0, 16)} == ${moduleGeomDigest.slice(0, 16)}` : 'no embed');
ok('embedded entry+cable counts match the vault', embedded && embedded.entryCount === mount.entryCount
  && embedded.cableCount === mount.cableCount, embedded ? `${embedded.entryCount}/${embedded.cableCount}` : 'n/a');

// ── 3. the render path actually mounts it ───────────────────────────────────
ok('build renders a KnowledgeMountain group', /['"]KnowledgeMountain['"]/.test(build) && build.includes('knowledge-mountain-entry'),
  'KnowledgeMountain mesh group + per-entry crystals');
ok('build draws lineage cables in-world', build.includes('KnowledgeMountainLineage') && /LineSegments/.test(build),
  'lineage cables rendered');
ok('build records the mount on a window flag (observable)', build.includes('__GOLD_GAME_MOUNTAIN_MOUNTED__'),
...
ok('gate-1 receipt records the Gate-41 mount', km && km.enabled === true && km.gate === 41,
  km ? `mounted=${km.mounted} entries=${km.entries}` : 'no receipt field');
ok('mount is additive (onboarding vault survives)', km && km.additive === true
  && (gate1.scene?.entries ?? gate1.scene?.meshes ?? 4) <= 12,
...
ok('mount links back to the Gate-39 terrain seal', km && gate39.terrainDigest
  && km.terrainDigest === gate39.terrainDigest,
  km?.terrainDigest ? km.terrainDigest.slice(0, 16) : 'unlinked');
ok('Gate 39 (the placement source) is itself VERIFIED', gate39.result === 'VERIFIED' && gate39.entries === mount.entryCount,
  `gate39 entries=${gate39.entries}`);

// ── 5. negative controls: tamper / drop flips the mount digest ──────────────
const tampered = JSON.parse(JSON.stringify(coords)); tampered.coords[0].x += 1;
const dropped = JSON.parse(JSON.stringify(coords)); dropped.lineage_edges.pop();
ok('negative control: tampering one coordinate flips the mount digest',
...
ok('negative control: dropping one cable flips the mount digest',
...
const receipt = {
  ...
  source: 'gold-game-knowledge-mountain.mjs (pure mount) + drive-build.mjs (render) over Gate-39 coords',
  honestScope: 'mounts the Gate-39 placement IN the build as tier-colored crystals + lineage cables; '
...
console.log(`Gate 41 ${allPass ? 'PASS' : 'FAIL'} — Knowledge Mountain mounted (digest ${mount.mountDigest.slice(0, 12)}).`);
