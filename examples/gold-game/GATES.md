...
| 4 | content / world evolution (CausalWorldModel do-calculus unlock + NPC memory across sessions) | **PASS** | `tsx gate-4-causal-memory-verify.mjs` (14/14). **Deep-ratchet 2026-05-24:** CausalWorldModel is a genuine Pearl SCM — real do-operator graph-mutilation + topo-propagation + counterfactual abduction (`hololand-platform/src/world/causal.ts:113-209`, 30 tests). Memory is REAL disk persistence that genuinely re-plans session 2, with two honest caveats (self-disclosed in receipt honestScope): the two sessions run in ONE process (disk file survives a restart but the test doesn't spawn a second node), and the value policy is a hand-authored payoff heuristic, NOT a learned model. | `GATE-4-CAUSAL-MEMORY-receipt.json` | `e2e4e2ed7` |
...
| 11 | quantum-inspired curation (real QuantumInspiredTrait wired; CPU fallback is decision-NEUTRAL) | **PASS\*** | `tsx gate-11-quantum-verify.mjs` (11/11; genuine quantumInspiredHandler wiring, deterministic, real `__qiState` + receipt). **HONEST SCOPE (deep-ratchet 2026-05-24):** the path the gate actually runs is the `CpuFallbackAccelerator` — a 0.5-centered logistic sigmoid (`QuantumInspiredTrait.ts:115-122`). A monotonic 0.5-centered sigmoid is decision-NEUTRAL vs a 0.5 graduate/defer threshold, so it does NOT "sharpen" decisions — the `thresholdPreserved`/`monotonic` checks are tautologies of the squash. The genuine annealing-analogue is the SNN/GPU path (`SnnAccelerator`), NOT exercised here (`acceleratorAvailable===false` is itself asserted). Real-QPU is the separate /quantum-lab VQE track. Claim corrected from "sharpens decisions" → "wired + deterministic; CPU path decision-neutral." | `be38f5ef1` |
