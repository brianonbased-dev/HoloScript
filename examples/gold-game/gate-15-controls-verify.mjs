...
const verbsMapped = ['w', 's', 'a', 'd', 'e', ' ', 'ArrowUp', 'ArrowDown'].every((k) => keyToVerb(k) !== null);
...
const drive = existsSync(join(here, 'drive-build', 'index.html')) ? readFileSync(join(here, 'drive-build', 'index.html'), 'utf8') : '';
const html2d = existsSync(join(here, '2d-build', 'index.html')) ? readFileSync(join(here, '2d-build', 'index.html'), 'utf8') : '';
...
const code = (a) => a === 'ascend' ? 1 : a === 'descend' ? 2 : a === 'left' ? 3 : a === 'right' ? 4 : a.startsWith('graduate') ? 5 : 0;
const controlDigest = computeStateDigest(
  { fieldNames: ['c'], getField: () => Float32Array.from([...log.map(code), state.tierIndex, state.x, state.graduated.length]) },
  HASH,
);
...
const receipt = {
  ...
  controlScheme: { source: 'examples/gold-game/gold-game-controls.mjs (pure, deterministic)', keys: { 'W/Up': 'ascend tier', 'S/Down': 'descend', 'A/Left': 'left', 'D/Right': 'right', 'E/Space/Enter': 'graduate entry at tier' }, tiers: TIERS },
  session: { trace: TRACE, actionLog: log, finalState: state },
  buildsWired: { drive3d: build3dHasInput, retro2d: build2dHasInput },
  contract: { spine: 'REAL computeStateDigest', controlDigest, reproducible: 'run the verifier to re-derive' },
  honestScope: 'The control scheme (gold-game-controls.mjs) is a PURE deterministic input→state machine, the SAME source both the 3D Drive build and the 2D build import — so the game is playable outside VR with one consistent mapping. PROVEN: keyboard input deterministically traverses the tiers and performs the graduate verb; the action log digest reproduces. The 3D graduate keybind routes through the existing HoloGate intent path (Gate 6). NOT proven here: live in-browser keyboard feel (a human at the keyboard) and pointer/mouse picking precision — those are hands-on, like Gate 1’s render. This gate proves the control LOGIC is real, deterministic, and shipped in both builds.',
...
if (emit) {
  ...
  console.log('  finalTier=' + TIERS[state.tierIndex], 'graduated=' + JSON.stringify(state.graduated), 'deterministic=' + deterministic);
  console.log('  builds wired: 3d=' + build3dHasInput, '2d=' + build2dHasInput);
  console.log('  controlDigest=' + controlDigest);
} else {
  let existing; try { existing = JSON.parse(readFileSync(receiptPath, 'utf8')); } catch { console.error('No Gate-15 receipt. Run --emit first.'); process.exit(2); }
  const checks = [
    ...
    ['climbed to the Diamond peak then descended (deterministic traversal)', finalTierCorrect === true],
    ...
    ['control digest reproduces (real computeStateDigest)', controlDigest === existing.contract.controlDigest && deterministic],
  ];
  let ok = true;
