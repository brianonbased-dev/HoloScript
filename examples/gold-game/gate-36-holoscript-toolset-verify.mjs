...
ok('enough package systems are mapped', systems.length >= requiredDirs.length, String(systems.length));
ok('all required package dirs are present in receipt', requiredDirs.every((dir) => systems.some((s) => s.dir === dir)));
ok('no selected package is missing', Array.isArray(toolset.missing) && toolset.missing.length === 0);
...
ok('offline HTML exposes toolset panel', html.includes('data-gate36="holoscript-toolset"') && html.includes('id="toolsetList"'));
ok('source reads package.json metadata', source.includes("join(repo, 'packages', dir, 'package.json')"));
ok('source maps HoloGram and HoloMap to the corrected roles', source.includes('HoloGram render') && source.includes('Scanned spaces'));
ok('toolset includes kitchen-sink roles', [
  'HoloGate',
  'Shared state',
  'HoloGraph recall',
  'Scanned spaces',
