...
for (let i = 0; i < N; i++)
  fs.writeFileSync(
    join(sandboxDir, 'bronze', `C.E.${i}.json`),
    JSON.stringify(
      { id: `C.E.${i}`, title: 'entry-' + i, tier: 'bronze', lineage_links: 3 },
      null,
      2
    )
  );
fs.writeFileSync(
  join(sandboxDir, 'bronze', 'C.SPAM.json'),
  JSON.stringify({ id: 'C.SPAM', title: 'collision', tier: 'bronze', lineage_links: 0 }, null, 2)
); // lineage 0 → graduate always refuses
...
for (let i = 0; i < N; i++) {
  ...
  if (r.ok) {
    earn('honest', REWARD);
    honestGrads++;
    observe('honest', 'graduated:C.E.' + i, 2);
  }
}
...
for (let i = 0; i < N; i++) {
  ...
  const blocked =
    ...
    econ.events.slice(before).some((e) => e.type === 'economy:insufficient_funds');
  if (blocked) {
    spamInsufficient++;
    spamBalArc.push(bal('spammer'));
    continue;
  } // self-limited: can't afford to spam
  spamSpends++;
...
const balanceDigest = computeStateDigest(
  {
    fieldNames: ['b'],
    getField: () =>
      Float32Array.from([
...
const receipt = {
  ...
  implementation:
    'real economyPrimitivesHandler (economy:spend stake + economy:earn reward) + reputationLedgerHandler + real graduate verb deciding success',
  params: { episodes: N, reward: REWARD, stake: STAKE, initialBalance: INIT },
  honest: {
    graduations: honestGrads,
    finalBalance: honestFinal,
    finalTrust: honestTrust,
    netCreditsPerGraduation: REWARD - STAKE,
  },
  spam: {
    spends: spamSpends,
    insufficientFundsHits: spamInsufficient,
    finalBalance: spamFinal,
    balanceArc: spamBalArc,
  },
  noSinkBaseline: { finalBalance: nosinkFinal },
  metrics: {
    inflationWithSink,
    inflationNoSink,
    sinkAbsorbedFraction: Number(sinkAbsorbedFraction.toFixed(4)),
    honestStayedBounded,
    sinkAbsorbsMostInflation,
    noSinkInflatesSignificantly,
    spamUnprofitable,
    spamSelfLimited,
    archivistEarnable,
    repIndependentOfCredits,
  },
  contract: {
...
if (emit) {
  ...
  console.log(
    '  spam: ' +
      ...
      ' spends, ' +
      spamInsufficient +
      ' insufficient, final balance ' +
      spamFinal
  );
  console.log('  inflation: with-sink ' + inflationWithSink + ' vs no-sink ' + inflationNoSink);
  console.log(
    '  spamUnprofitable=' + spamUnprofitable,
    'selfLimited=' + spamSelfLimited,
    'archivistEarnable=' + archivistEarnable
  );
  ...
  try {
    ...
    console.error('No Gate-20 receipt. Run --emit first.');
    process.exit(2);
  }
  const checks = [
    [
      'real economy sink wired: honest curator did all N graduations with real stake+reward',
      honestGrads === N,
    ],
    [
      'honest credits stay BOUNDED — the stake sink stops runaway inflation (balance never doubled)',
      honestStayedBounded === true,
    ],
    [
      'the stake sink absorbs >=80% of the inflation honest play would otherwise cause',
      sinkAbsorbsMostInflation === true,
    ],
    [
      'the NO-SINK baseline inflates significantly (proves the sink is load-bearing)',
      noSinkInflatesSignificantly === true,
    ],
    [
      'SPAM is unprofitable: spammer loses the stake every time, final balance < initial',
      spamUnprofitable === true,
    ],
    [
      'SPAM self-limits: the spammer hits insufficient_funds and cannot keep farming',
      spamSelfLimited === true,
    ],
    [
      'the Archivist stays earnable: reputation crosses threshold despite ~flat credits',
      archivistEarnable === true,
    ],
    [
      'progression is decoupled from credits (reputation gates, not a credit pile)',
      repIndependentOfCredits === true,
    ],
    [
      'balance digest reproduces (real computeStateDigest)',
      balanceDigest === existing.contract.balanceDigest,
    ],
  ];
  let ok = true;
