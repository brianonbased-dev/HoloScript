...
const cat = vaultPresent
  ? vaultOps.readVaultCatalog(VAULT_ROOT)
  : { count: 0, entries: [], facets: { tiers: {}, types: {}, domains: {} } };
...
const buildSource = existsSync(join(here, 'drive-build.mjs'))
  ? readFileSync(join(here, 'drive-build.mjs'), 'utf8')
  : '';
const html = existsSync(join(here, 'drive-build', 'index.html'))
  ? readFileSync(join(here, 'drive-build', 'index.html'), 'utf8')
  : '';
const serverSrc = existsSync(join(here, 'server.cjs'))
  ? readFileSync(join(here, 'server.cjs'), 'utf8')
  : '';
...
const withResolvableLineage = cat.entries.filter((e) =>
  (e.lineage || []).some((l) => ids.has(l))
).length;
...
const checks = [
  ...
  [
    'search query narrows the catalog (q=secret matches < full)',
    qSecret.length >= 1 && qSecret.length < cat.count,
  ],
  ['a non-existent needle returns 0 (real filter, not pass-through)', qNoMatch.length === 0],
  [
    'tier filter narrows AND matches the facet count (tier=diamond)',
    fDiamond.length === diamondFacet && fDiamond.length < cat.count && fDiamond.length > 0,
  ],
  [
    'every entry carries a provenance sha256 seal (receipt history)',
    withSha === cat.count && cat.count > 0,
  ],
  ['lineage links resolve to OTHER real catalog entries (navigable)', withResolvableLineage >= 50],
  [
    'server exposes the /api/vault-list enumerate+filter endpoint',
    /\/api\/vault-list/.test(serverSrc) && /filterCatalog/.test(serverSrc),
  ],
  [
    'offline build embeds the SAME catalog reader (server<->offline parity)',
    /readVaultCatalog/.test(buildSource) && /vault-ops\.cjs/.test(buildSource),
  ],
  [
    'generated build exposes a full-vault BROWSER UI (button + panel)',
    /data-gate28="vault-browser"/.test(html) && /data-gate28="full-vault-browser"/.test(html),
  ],
  [
    'generated build wires search + lineage + provenance into the browser',
    /Browse the GOLD Vault/.test(html) && /lineage/i.test(html) && /provenance/i.test(html),
  ],
];
...
console.log(
  '  vault: ' +
    (vaultPresent ? VAULT_ROOT : '(absent)') +
    '  entries: ' +
    cat.count +
    '  catalogDigest: ' +
    digest.slice(0, 16) +
...
const receipt = {
  ...
  readOnly:
    'READ-ONLY over the governed D:/GOLD — graduate/mutate stay on the sandbox (Gate 2/27); D:/GOLD never written',
...
console.log(
  '  => GATE 28',
  ok ? 'VERIFIED' : 'BROKEN',
  '(' + passed + '/' + checks.length + ')  receipt: GATE-28-FULLVAULT-receipt.json'
);
process.exit(ok ? 0 : 1);
