...
function readJson(rel) {
  ...
  try {
    return { ok: true, rel, data: JSON.parse(readFileSync(file, 'utf8')) };
  } catch (e) {
    return { ok: false, rel, error: String((e && e.message) || e) };
  }
}
...
function digestTrace(trace) {
  ...
  const fields = {
    ...
    gates: Float32Array.from(
      trace.flatMap((s, i) => [
        i,
        ...(s.gates || []).map((g) => Number(String(g).replace(/\D/g, '')) || 0),
      ])
    ),
    evidence: Float32Array.from(
      hexFloats(sha256(trace.map((s) => `${s.step}:${s.evidence}`).join('|')))
    ),
  };
...
try {
  const m = build.match(
    /__GOLD_GAME_SELFPROOF__\s*=\s*(\{[\s\S]*?\});\s*window\.__GOLD_GAME_SHA256__/
  );
...
const trace = [
  {
    step: 'start-playable-build',
    gates: [1, 24, 25],
    evidence: gate1.sceneDigest || sha256(build).slice(0, 16),
  },
  {
    step: 'holograph-lineage-navigation',
    gates: [10, 31],
    evidence:
      receipts.gate10.data?.contract?.graphDigest ||
      receipts.gate10.data?.holoGraph?.graphDigest ||
      'holograph',
  },
  {
    step: 'hologram-depth-world-visible',
    gates: [32, 34],
    evidence:
      receipts.gate34.data?.worldDigest ||
      artifacts.vista.data?.generated_group?.spatial_group ||
      'vista',
  },
  {
    step: 'holomap-room-portal',
    gates: [33],
    evidence: receipts.gate33.data?.contract?.spaceDigest || 'holomap',
  },
  {
    step: 'holographic-export-targets',
    gates: [38],
    evidence: receipts.gate38.data?.contract?.exportDigest || 'exports',
  },
  {
    step: 'hologate-graduation',
    gates: [6, 14],
    evidence:
      receipts.gate14.data?.contract?.dialogueDigest ||
      receipts.gate6.data?.contract?.digest ||
      'graduation',
  },
  {
    step: 'shared-state-and-crdt',
    gates: [17, 18],
    evidence: sha256(
      JSON.stringify([receipts.gate17.data?.contract, receipts.gate18.data?.contract])
    ).slice(0, 24),
  },
  {
    step: 'solver-and-economy-bounds',
    gates: [12, 20, 21],
    evidence: sha256(
      JSON.stringify([
        receipts.gate12.data?.contract,
        receipts.gate20.data?.contract,
        receipts.gate21.data?.contract,
      ])
    ).slice(0, 24),
  },
  {
    step: 'agent-party-and-toolset-visible',
    gates: [29, 36],
    evidence: `${partyBoot?.roster?.length || 0}:${toolsetBoot?.systems?.length || 0}`,
  },
  {
    step: 'offline-self-proof',
    gates: [37],
    evidence: (selfProof?.manifest || []).map((m) => m.key).join(','),
  },
];
...
check(
  'all kitchen-sink prerequisite receipts exist/pass',
  Object.entries(receipts).every(([, r]) => r.ok && gatePass(r)),
  Object.entries(receipts)
    .filter(([, r]) => !r.ok || !gatePass(r))
    .map(([k]) => k)
    .join(', ') || 'all pass'
);
check(
  'core artifacts exist',
  Object.values(artifacts).every((r) => r.ok),
...
check(
  'build embeds HoloGram vista, HoloMap room, HoloGram export, toolset, and party blobs',
  Boolean(vistaBoot && roomBoot && exportBoot && toolsetBoot && partyBoot),
  'boot blobs parsed'
);
check(
  'build mounts the depth vista, HoloMap portal, panels, and proof seal',
  build.includes('VaultVistaBackdrop') &&
    build.includes('HoloMapRoomPortal') &&
    /data-gate33="holomap-room-panel"/.test(build) &&
    /data-gate38="holographic-export-panel"/.test(build) &&
    build.includes('GraduateSeal'),
  'visible surfaces present'
);
check(
  'HoloMap boot artifact matches Gate 33 receipt digest',
  roomBoot?.contract?.spaceDigest === receipts.gate33.data?.contract?.spaceDigest,
  (roomBoot?.contract?.spaceDigest || '').slice(0, 16)
);
check(
  'holographic export boot artifact matches Gate 38 receipt digest',
...
check(
  'Gate 1 receipt records the joined HoloMap/export surfaces',
...
check(
  'self-proof manifest covers the joined systems',
  [
    '__GOLD_GAME_VISTA__',
    '__GOLD_GAME_HOLOMAP_ROOM__',
    '__GOLD_GAME_HOLOGRAPHIC_EXPORT__',
    '__GOLD_GAME_TOOLSET__',
    '__GOLD_GAME_PARTY__',
  ].every((k) => selfProofKeys.has(k)),
  `${selfProofKeys.size} blobs`
);
check(
  'coherent trace spans graph, vista, room, exports, gameplay, mesh/economy, and self-proof',
  trace.length === 10 && trace.every((s) => s.evidence && s.gates.length > 0),
  `${trace.length} steps`
);
check(
  'kitchen-sink trace digest is sha256-shaped',
  /^[a-f0-9]{64}$/.test(kitchenDigest),
  kitchenDigest.slice(0, 16)
);
check(
  'negative control: dropping HoloMap portal changes kitchen digest',
  kitchenDigest !== negativeDigest,
  'load-bearing trace'
);
...
if (!process.argv.includes('--emit')) {
  ...
  console.log(
    '  INFO  kitchen digest ' +
      (sealMatch
        ? 'matches sealed receipt (composition byte-stable)'
        : 'differs from seal -- upstream growth shifted it; re-emit to ratchet')
  );
}
...
if (process.argv.includes('--emit') && allPass) {
  const receipt = {
    ...
    proves: {
      coherentLoop:
        'The same emitted playable build embeds and exposes HoloGraph, HoloGram vista, HoloMap room, holographic exports, HoloGate play, party/toolset, and offline self-proof.',
      receiptLevel:
        'Individual subsystem behavior remains owned by each gate verifier; Gate 35 proves composition/coherence across their artifacts.',
      loadBearing:
        'Negative control drops the HoloMap portal from the trace and changes the kitchen digest.',
    },
    honestScope:
      'Receipt/artifact-level coherent pass over the emitted playable build. It rechecks that the surfaces are present, joined, and digest-bound; it does not rerun every subsystem verifier internally. Run verify-all for full per-gate re-derivation.',
    status: 'PASS',
  };
