...
const checks = [
  [
    'desktop first-person mode exists',
    /desktopMode\s*=\s*'orbit'/.test(source) && /first-person/.test(source),
  ],
  ['pointer lock starts player control', /requestPointerLock/.test(source)],
  [
    'WASD and arrows drive continuous movement',
    /KeyW/.test(source) && /KeyA/.test(source) && /KeyS/.test(source) && /KeyD/.test(source),
  ],
  [
    'mouse look updates yaw and pitch',
    /movementX/.test(source) && /player\.yaw/.test(source) && /player\.pitch/.test(source),
  ],
  [
    'E graduates through HoloGate from first-person',
    /GRADUATED via HoloGate \(first-person\)/.test(source) && /validatePortalIntent/.test(source),
  ],
  ['F dispatches selected GOLD entry for inspection', /gold-entry-selected/.test(source)],
  [
    'generated build exposes first-person controls',
    /first-person control/.test(html) && /WASD moves/.test(html),
  ],
];
