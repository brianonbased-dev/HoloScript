...
for (const e of CONTEST) writeFileSync(join(sandboxDir, e.tier, e.id + '.json'), JSON.stringify(e, null, 2));
...
const earn = (agentId) => economyPrimitivesHandler.onEvent(economyNode, ECON_CFG, econ.ctx, { type: 'economy:earn', payload: { agentId, amount: REWARD, reason: 'graduation' } });
const observe = (curator, entryId) => reputationLedgerHandler.onEvent(repNode, REP_CFG, rep.ctx, { type: 'reputation_observe_action', observerId: curator, action: 'graduated:' + entryId, trustDelta: TRUST_DELTA });
const agendaItem = (agent, entryId, i) => {
  const an = agendaNodes[agent];
  autonomousAgendaHandler.onEvent(an.node, AGENDA_CFG, an.ctx, { type: 'agenda_add_item', id: agent + '-' + i, title: 'graduate ' + entryId, estimatedCostUsd: 0.1 });
  autonomousAgendaHandler.onEvent(an.node, AGENDA_CFG, an.ctx, { type: 'agenda_complete_item', itemId: agent + '-' + i });
};
...
const wmr = [];
const grads = { [HUMAN]: 0 }; AGENTS.forEach((a) => (grads[a] = 0));
let denialReplans = 0;
let agentItemCount = {}; AGENTS.forEach((a) => (agentItemCount[a] = 0));
...
for (let round = 0; round < 4; round++) {
  ...
  if (hPick && graduatable().some((x) => x.id === hPick)) {
    ...
    if (r.ok) { grads[HUMAN]++; wmr.push({ round, curator: HUMAN, action: 'graduate', target: hPick }); }
  }
  ...
  for (const a of order) {
    ...
    if (!pool.length) { wmr.push({ round, curator: a, action: 'pass', target: null }); continue; }
    ...
    if (r.ok) {
      grads[a]++;
      earn(a); observe(a, target); agendaItem(a, target, agentItemCount[a]++);
    ...
    } else { wmr.push({ round, curator: a, action: 'pass', target: null }); }
  }
}
...
denialReplans = wmr.filter((w) => w.action === 'graduate' && w.intended && w.intended !== w.target).length;

// ── Read GENUINE trait state ──────────────────────────────────────────────────
const accounts = economyNode.__economyState?.accounts;
const balanceOf = (a) => accounts?.get?.(a)?.balance;
const baseBalance = (() => { const a = AGENTS.find((x) => grads[x] > 0); return a != null ? balanceOf(a) - REWARD * grads[a] : ECON_CFG.initial_balance; })();
...
for (let i = 0; i < 7; i++) {
  autonomousAgendaHandler.onEvent(probe.node, AGENDA_CFG, probe.ctx, { type: 'agenda_add_item', id: 'p' + i, title: 'x', estimatedCostUsd: 0.1 });
  autonomousAgendaHandler.onEvent(probe.node, AGENDA_CFG, probe.ctx, { type: 'agenda_complete_item', itemId: 'p' + i });
}
...
const inBudgetNoBreach = AGENTS.every((a) => !agendaNodes[a].events.some((e) => e.type === 'agenda_cost_ceiling_breach'));
...
const vaultDigest = computeStateDigest({ fieldNames: ['v'], getField: () => Float32Array.from(
  [...V.ALL_TIERS.flatMap((t, ti) => finalState.tiers[t].map((id) => [idI(id), ti]))].sort((a, b) => a[0] - b[0]).flat()) }, HASH);
const economyDigest = computeStateDigest({ fieldNames: ['e'], getField: () => Float32Array.from(
  AGENTS.map((a) => [idI(a), balanceOf(a) ?? 0]).sort((a, b) => a[0] - b[0]).flat()) }, HASH);
const wmrDigest = computeStateDigest({ fieldNames: ['w'], getField: () => Float32Array.from(
  wmr.flatMap((w) => [w.round, idI(w.curator), w.action === 'graduate' ? 1 : 0, w.target ? idI(w.target) : 0])) }, HASH);
...
const receipt = {
  ...
  traits: {
    ...
    reputation: 'packages/core/src/traits/ReputationLedgerTrait.ts (reputationLedgerHandler — REAL)',
    agenda: 'packages/core/src/traits/AutonomousAgendaTrait.ts (autonomousAgendaHandler — REAL)',
  },
  session: { curators: [HUMAN, ...AGENTS], coordination: 'reputation-ordered picking (A2A-style mesh)', rounds: 4, deterministic: true },
  results: {
    graduations: grads, totalGraduations: totalGrads, agentGraduations: agentGrads, denialReplans,
    economy: { initialBalance: baseBalance, rewardPerGraduation: REWARD, balances: Object.fromEntries(AGENTS.map((a) => [a, balanceOf(a)])), totalEarned, conserved: totalEarned === REWARD * agentGrads },
    reputation: { trustDelta: TRUST_DELTA, trust: trustTally, initialTrust: REP_CFG.initial_trust },
    agenda: { dailyBudgetUsd: AGENDA_CFG.daily_budget_usd, ceilingFiresWhenOverspent: ceilingFired, inBudgetNoBreach },
  },
  contract: { spine: 'REAL computeStateDigest', vaultDigest, economyDigest, wmrDigest, reproducible: 'run the verifier to re-derive' },
  honestScope: 'The economy balances, reputation trust, and $0.50/day agenda ceiling are produced by the GENUINE @holoscript/core sovereign-trait handlers (economyPrimitivesHandler / reputationLedgerHandler / autonomousAgendaHandler) driven over a real session — NOT inline mocks. Graduations are real file moves via vault-ops graduate() on a SANDBOX vault. PROVEN: a 3-agent + human mesh curating one vault, with real token economy, real reputation accrual, real cost-ceiling enforcement, and reputation-ordered coordination. Hand-authored agent policies + scripted human (not trained / not a live operator); A2A coordination is modeled as reputation-ordered picking rather than the full network handshake — those deepen in later gates.',
...
if (emit) {
  ...
  console.log('  graduations:', JSON.stringify(grads), '| agentGrads=' + agentGrads, '| replans=' + denialReplans);
  console.log('  balances:', JSON.stringify(Object.fromEntries(AGENTS.map((a) => [a, balanceOf(a)]))), '| totalEarned=' + totalEarned, 'conserved=' + (totalEarned === REWARD * agentGrads));
  console.log('  trust:', JSON.stringify(trustTally), '| ceilingFires=' + ceilingFired, 'inBudgetNoBreach=' + inBudgetNoBreach);
  console.log('  vaultDigest=' + vaultDigest.slice(0, 16), 'economyDigest=' + economyDigest.slice(0, 16), 'wmrDigest=' + wmrDigest.slice(0, 16));
} else {
  let existing; try { existing = JSON.parse(readFileSync(receiptPath, 'utf8')); } catch { console.error('No Gate-8 receipt. Run --emit first.'); process.exit(2); }
  const checks = [
    ['3 AI curators + human acted in one shared session', AGENTS.length === 3 && grads[HUMAN] >= 1 && agentGrads >= 1],
    ['real graduate() moved entries (multi-way mesh)', finalState.tiers.gold.length >= 5],
    ['EconomyPrimitivesTrait REAL: balances are real account state', AGENTS.every((a) => typeof balanceOf(a) === 'number')],
    ['economy conserved: total earned == reward x agent graduations', totalEarned === REWARD * agentGrads && agentGrads > 0],
    ['ReputationLedgerTrait REAL: graduating agents gained trust', AGENTS.filter((a) => grads[a] > 0).every((a) => trustTally[a] > (REP_CFG.initial_trust ?? 50))],
    ['AutonomousAgendaTrait REAL: $0.50/day ceiling actually fires when overspent', ceilingFired === true],
    ...
    ['vault digest reproduces (real computeStateDigest)', vaultDigest === existing.contract.vaultDigest],
