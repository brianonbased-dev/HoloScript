...
function engineDigest(dir) {
  ...
  const rows = [];
  for (const tier of V.ALL_TIERS) for (const id of (st.tiers[tier] || []).sort()) rows.push(`${id}:${tier}`);
  rows.sort();
  const codes = rows.join('|').split('').map((c) => c.charCodeAt(0));
  return computeStateDigest({ fieldNames: ['vault'], getField: () => Float32Array.from(codes) }, HASH);
}
...
const prop = V.proposeMutation(dir, { verb: 'graduate', id: 'W.GOLD.534', by: 'ai-curator-claude' });
ok(prop.ok, 'proposeMutation() accepts a valid graduate proposal');
ok(prop.vaultWrite === 'NONE — proposal queued, vault untouched', 'proposal declares NO vault write');
...
ok(prop.proposal.status === 'PENDING_FOUNDER_APPROVAL', 'proposal is queued PENDING_FOUNDER_APPROVAL');
...
const applied = V.applyMutation(dir, prop.proposal.proposalId, prop.approvalToken, 'founder-joseph');
...
ok(V.readState(dir).tiers.platinum.includes('W.GOLD.534'), 'W.GOLD.534 actually moved gold -> platinum on disk');
ok(applied.receipt.schema === 'cael-vault-mutation-v1' && !!applied.receipt.preSnapshot, 'mutation receipt carries pre-state snapshot (rollback anchor)');
ok(applied.receipt.preDigest === d0 && applied.receipt.postDigest === dApplied, 'receipt pre/post digests match observed pre/post state');
...
ok(rb.rollbackReceipt.restoredExactly === true, 'rollback receipt: restoredExactly = true (proven, not asserted)');
...
ok(!V.readState(dir).tiers.platinum.includes('W.GOLD.534'), 'W.GOLD.534 is back in gold (mutation undone)');

// ── 5. DETERMINISM — proposal id + token reproduce under the same state ──────
V.buildSandbox(dir);
const propA = V.proposeMutation(dir, { verb: 'graduate', id: 'W.GOLD.534', by: 'ai-curator-claude' });
V.buildSandbox(dir);
const propB = V.proposeMutation(dir, { verb: 'graduate', id: 'W.GOLD.534', by: 'ai-curator-claude' });
ok(propA.proposal.proposalId === propB.proposal.proposalId, 'proposalId is deterministic across fresh builds');
ok(propA.approvalToken === propB.approvalToken, 'founder approvalToken is deterministic across fresh builds');

// ── 6. STALE FENCE — a vault that moved since the proposal refuses the apply ─
V.buildSandbox(dir);
const propC = V.proposeMutation(dir, { verb: 'graduate', id: 'W.GOLD.534', by: 'ai-curator-claude' });
V.graduate(dir, 'W.GOLD.535', 'someone-else'); // vault moves out from under the proposal
const stale = V.applyMutation(dir, propC.proposal.proposalId, propC.approvalToken, 'founder-joseph');
ok(!stale.ok && /stale/.test(stale.error), 'apply REFUSED when vault changed since proposal (digest fence)');

// ── 7. D:/GOLD is never touched — gate operates only in the sandbox ──────────
ok(dir.includes('vault-sandbox-g27') && !dir.toUpperCase().includes('D:'), 'gate writes only to throwaway sandbox, never D:/GOLD');

console.log(`\n=> GATE 27 ${fail === 0 ? 'VERIFIED' : 'FAILED'} (${pass} pass / ${fail} fail)`);

if (process.argv.includes('--emit') && fail === 0) {
  const receipt = {
    schema: 'cael-gate-v1', gate: 27, name: 'live-vault safe mutation flow',
    track: 'flagship',
    proven: 'AI proposes -> vault untouched -> founder token applies -> reversible rollback to exact pre-state',
    checks: `${pass}/${pass}`,
    contract: {
      spine: 'REAL computeStateDigest (packages/engine/src/simulation/hashes.ts) + cael-vault-mutation-v1',
      proposeDelta: 0, refusedApplyDelta: 0, approvedApplyDelta: 'nonzero',
      rollbackRestoresExact: true,
      neverWritesRealVault: 'D:/GOLD untouched — sandbox only; promotion is a separate founder gate',
      reproducible: 'run the verifier to re-derive',
    },
    aiHumanConnection: 'AI curator proposes the mutation; the human founder ratifies via the approval token; the vault is the shared world state both act on; every step (propose/apply/rollback) leaves a hash-sealed receipt',
