...
const room = lobby.createRoom('alice', 'Alice', {
  name: 'GOLD Vault — Shard Zero',
  maxPlayers: 8,
  tags: ['vault', 'coop'],
});
...
const allAgreePreDrop = clients.every(
  (c) => JSON.stringify(c.view) === JSON.stringify(shard.vault)
);
...
const newHostName = migration.newHostId
  ? lobby.getRoom(room.id).players.get(migration.newHostId).name
  : null;
...
const noDoubleGraduate =
  log.filter((l) => l.startsWith('GRAD') && l.includes('W.004')).length === 1;
const remainingAgree = remaining.every(
  (c) => JSON.stringify(c.view) === JSON.stringify(shard.vault)
);
...
for (const e of ENTRIES) {
  // rehydrate each entry to its persisted tier
  let cur = 'Bronze';
...
const room2 = lobby2.createRoom('bob', 'Bob', {
  name: 'GOLD Vault — Shard Zero (resumed)',
  maxPlayers: 8,
  tags: ['vault'],
});
...
for (const e of ENTRIES) {
  // Dave syncs the persisted world on join
  const d = session2.rep.generateUpdates(clock++).find((x) => x.entityId === `entry:${e}`);
  if (d) {
    dave.rep.applyRemoteUpdate(d);
    dave.view[e] = dave.rep.getEntity(`entry:${e}`).snapshot.customState.tier;
  } else dave.view[e] = session2.vault[e];
}
const session2MatchesPersisted =
  JSON.stringify(session2.vault) === JSON.stringify(persistedSnapshot.vault);
const lateEntrantSeesHistory =
  JSON.stringify(dave.view) === JSON.stringify(persistedSnapshot.vault);
...
const worldDigest = computeStateDigest(
  { fieldNames: ['vault'], getField: () => vaultField(shard.vault) },
  HASH
);
const session2Digest = computeStateDigest(
  { fieldNames: ['vault'], getField: () => vaultField(session2.vault) },
  HASH
);
...
const reWorld = computeStateDigest(
  { fieldNames: ['vault'], getField: () => vaultField(shard.vault) },
  HASH
);
...
const receipt = {
  ...
  builtOn:
    'Gate 17 (NETCODE co-presence) — closes the host-migration / persistence gaps its honestScope flagged',
  classification: {
    ...
    notMMO:
      'NOT claimed: massive concurrency (this is one small shard, in-process, not a fleet of sharded servers under load); no real network transport (two+ ReplicationManager views in one process share the authoritative order — agreement is over the real replication API, not a socket); no horizontal shard fanout / interest management at scale; no persistence backend (the snapshot is in-memory, not a durable DB). Call it MMO only after a shard-fleet + DB-backed persistence + load gate. Today: a proven shared-world AI co-op curation game.',
  },
  meshClasses: {
    LobbyManager:
      'REAL — createRoom/joinRoom/listRooms (shard lobby + named entrants); leaveRoom performs genuine host migration ({migrated,newHostId})',
    EntityAuthority:
      'REAL — forceTransfer hands the orphaned server vault-lock to the migrated host (authority handoff)',
    ReplicationManager:
      'REAL — generateUpdates delta broadcast + applyRemoteUpdate convergence (persistent shared state)',
  ...
  session: {
    ...
    authorityHandoff: {
      entry: 'W.004',
      orphanedLocked,
      forceTransferredTo: newHostName,
      lockReleased,
    },
...
if (emit) {
  ...
  const checks = [
    ...
    [
      'multiple NAMED human entrants join one shard (Alice/Bob/Cara) + an AI curator',
      joinedBob && joinedCara && joinedAgent && namedHumans.length === 3,
    ],
    ['four entrants co-present in the one shard', fourEntrants === true],
    [
      'all entrants converge on identical persistent shared vault state (human + AI act on ONE world)',
      allAgreePreDrop === true,
    ],
    [
      'durable presence: lobby roster tracks all connected entrants before the drop',
      presenceBeforeDrop.length === 4,
    ],
    [
      'host migration on disconnect: LobbyManager migrates host to the earliest remaining player',
      migration.migrated === true && newHostName === 'Bob',
    ],
    [
      'authority handoff: the orphaned in-flight vault lock force-transfers to the new host and releases',
      orphanedLocked === true && lockHandedOff === true && lockReleased === true,
    ],
    [
      'world survives the disconnect: new host finishes the in-flight graduation, no double-graduate',
      worldSurvivedDrop === true && noDoubleGraduate === true && remainingAgree === true,
    ],
    ['converged world matches the expected authoritative end-state', finalAsExpected === true],
    [
      'persistent ACROSS sessions: a second session rehydrates the snapshot and matches',
      session2MatchesPersisted === true,
    ],
    [
      'a late entrant in the rehydrated session sees the full accumulated world history',
      daveJoined === true && lateEntrantSeesHistory === true,
    ],
    [
      'the converged-world digest is identical across both sessions (real computeStateDigest)',
      digestSurvivesSession === true,
    ],
    [
    ...
    [
      ...
      existing.classification.answer.includes('SHARED-WORLD CO-OP') &&
        existing.classification.notMMO.includes('NOT claimed'),
    ],
  ];
  let ok = true;
