...
export class TraitExpert {
  ...
  addTrait(trait: TraitDefinition): void {
    ...
    this.nameTokens.set(
      trait.name,
      trait.name.toLowerCase().split(/[^a-z0-9]+/).filter(Boolean)
    );
  }
  ...
  query(queryText: string, options: QueryOptions): QueryResult[] {
    ...
    for (const [name, trait] of this.traits) {
      ...
      // Token overlap with name
        ...
        const overlap = queryTokens.filter((qt) => nameToks.some((nt) => nt.includes(qt) || qt.includes(nt)));
        if (overlap.length > 0) {
...
const CATEGORY_KEYWORDS: Record<TraitCategory, string[]> = {
  rendering: ['material', 'color', 'texture', 'shader', 'pbr', 'emissive', 'metallic', 'roughness', 'iridescence', 'clearcoat', 'transmission', 'glass', 'toon', 'render', 'light', 'shadow', 'glow', 'bloom', 'opacity', 'transparent', 'reflective', 'subsurface', 'sss'],
  physics: ['physics', 'rigid', 'body', 'collider', 'collision', 'gravity', 'force', 'velocity', 'mass', 'friction', 'restitution', 'kinematic', 'dynamic', 'constraint', 'joint', 'ragdoll', 'trigger'],
  audio: ['audio', 'sound', 'music', 'spatial', 'volume', 'pitch', 'reverb', 'echo', 'distortion', 'filter', 'oscillator', 'synth', 'ambience', 'sfx', 'listener'],
  animation: ['animation', 'animate', 'tween', 'keyframe', 'interpolate', 'ease', 'spring', 'morph', 'blend', 'skeleton', 'bone', 'ik', 'procedural', 'walk', 'idle', 'loop', 'clip'],
  ai: ['ai', 'agent', 'behavior', 'state', 'machine', 'decision', 'tree', 'pathfinding', 'navmesh', 'npc', 'dialogue', 'goal', 'utility', 'perception', 'sensor', 'blackboard', 'snn', 'neural'],
  network: ['network', 'multiplayer', 'sync', 'replicate', 'rpc', 'lobby', 'authority', 'prediction', 'interpolation', 'latency', 'matchmaking', 'socket', 'websocket', 'crdt'],
  ui: ['ui', 'hud', 'canvas', 'button', 'text', 'panel', 'tooltip', 'menu', 'cursor', 'minimap', 'health', 'bar', 'inventory', 'widget'],
  input: ['input', 'keyboard', 'mouse', 'gamepad', 'touch', 'gesture', 'pointer', 'controller', 'vr', 'xr', 'hand', 'tracking', 'ray', 'click', 'drag'],
  lifecycle: ['lifecycle', 'init', 'update', 'destroy', 'spawn', 'despawn', 'enable', 'disable', 'awake', 'start', 'tick', 'frame', 'event', 'trigger'],
  data: ['data', 'store', 'state', 'reactive', 'observable', 'binding', 'serialize', 'persist', 'save', 'load', 'config', 'json', 'schema'],
  spatial: ['spatial', 'transform', 'position', 'rotation', 'scale', 'parent', 'child', 'hierarchy', 'lod', 'culling', 'frustum', 'octree', 'bvh', 'portal', 'teleport'],
  general: ['general', 'tag', 'name', 'group', 'layer', 'mask', 'debug', 'gizmo', 'comment', 'metadata', 'script'],
};
  ...
  const renderingTraits: TraitDefinition[] = [
    {
      ...
      parameters: [
        ...
        { name: 'metalness', type: 'number', min: 0, max: 1, default: 0, description: 'Metalness factor', required: false },
        { name: 'roughness', type: 'number', min: 0, max: 1, default: 0.5, description: 'Roughness factor', required: false },
      ],
      compatibleWith: ['emissive', 'iridescence', 'clearcoat', 'transmission'],
      conflictsWith: [],
      version: '1.0.0',
      tags: ['pbr', 'material', 'color', 'basic'],
      performance: { gpuCost: 'low', cpuCost: 'negligible', memoryCost: 'low', drawCallImpact: 0, vrSafe: true, mobileSafe: true },
    },
    {
      name: 'emissive',
      category: 'rendering',
      ...
      parameters: [
        ...
        { name: 'intensity', type: 'number', min: 0, max: 10, default: 1, description: 'Emission intensity', required: false },
      ],
      compatibleWith: ['material', 'bloom'],
      conflictsWith: [],
      version: '1.0.0',
      tags: ['glow', 'light', 'emission', 'neon'],
      performance: { gpuCost: 'low', cpuCost: 'negligible', memoryCost: 'negligible', drawCallImpact: 0, vrSafe: true, mobileSafe: true },
    },
    {
      name: 'iridescence',
      category: 'rendering',
      ...
      parameters: [
        { name: 'strength', type: 'number', min: 0, max: 1, default: 0.5, description: 'Iridescence strength', required: false },
        { name: 'ior', type: 'number', min: 1, max: 2.5, default: 1.3, description: 'Index of refraction', required: false },
      ],
      compatibleWith: ['material', 'clearcoat'],
      conflictsWith: ['toon'],
      version: '1.0.0',
      tags: ['iridescence', 'rainbow', 'thin-film', 'advanced-pbr'],
      performance: { gpuCost: 'medium', cpuCost: 'negligible', memoryCost: 'low', drawCallImpact: 0, vrSafe: true, mobileSafe: false },
    },
    {
      name: 'transmission',
      category: 'rendering',
      ...
      parameters: [
        { name: 'factor', type: 'number', min: 0, max: 1, default: 1, description: 'Transmission factor', required: false },
        { name: 'thickness', type: 'number', min: 0, max: 10, default: 0.5, description: 'Material thickness', required: false },
        { name: 'ior', type: 'number', min: 1, max: 2.5, default: 1.5, description: 'Index of refraction', required: false },
      ],
      compatibleWith: ['material'],
      conflictsWith: ['toon'],
      version: '1.0.0',
      tags: ['glass', 'transparent', 'refraction', 'transmission'],
      performance: { gpuCost: 'high', cpuCost: 'negligible', memoryCost: 'low', drawCallImpact: 1, vrSafe: false, mobileSafe: false },
    },
    {
      name: 'subsurface',
      category: 'rendering',
      ...
      parameters: [
        ...
        { name: 'thickness', type: 'number', min: 0, max: 5, default: 1, description: 'Scattering depth', required: false },
      ],
      compatibleWith: ['material'],
      conflictsWith: ['toon', 'transmission'],
      version: '1.0.0',
      tags: ['sss', 'skin', 'wax', 'subsurface', 'scattering'],
      performance: { gpuCost: 'high', cpuCost: 'low', memoryCost: 'low', drawCallImpact: 0, vrSafe: false, mobileSafe: false },
    },
  ];
  ...
  const physicsTraits: TraitDefinition[] = [
    {
      ...
      parameters: [
        { name: 'type', type: 'enum', enumValues: ['dynamic', 'kinematic', 'static'], default: 'dynamic', description: 'Body type', required: false },
        { name: 'mass', type: 'number', min: 0, max: 10000, default: 1, description: 'Mass in kg', required: false },
        { name: 'friction', type: 'number', min: 0, max: 1, default: 0.5, description: 'Surface friction', required: false },
        { name: 'restitution', type: 'number', min: 0, max: 1, default: 0.3, description: 'Bounciness', required: false },
      ],
      compatibleWith: ['collider'],
      conflictsWith: [],
      version: '1.0.0',
      tags: ['physics', 'rigid', 'dynamic', 'simulation'],
      performance: { gpuCost: 'negligible', cpuCost: 'medium', memoryCost: 'low', drawCallImpact: 0, vrSafe: true, mobileSafe: true },
    },
    {
      name: 'collider',
      category: 'physics',
      ...
      parameters: [
        { name: 'shape', type: 'enum', enumValues: ['box', 'sphere', 'capsule', 'mesh', 'convex'], default: 'box', description: 'Collider shape', required: false },
        { name: 'isTrigger', type: 'boolean', default: false, description: 'Trigger (no collision response)', required: false },
      ],
      compatibleWith: ['rigidBody'],
      conflictsWith: [],
      version: '1.0.0',
      tags: ['collision', 'collider', 'shape', 'trigger'],
      performance: { gpuCost: 'negligible', cpuCost: 'low', memoryCost: 'low', drawCallImpact: 0, vrSafe: true, mobileSafe: true },
    },
  ];
  ...
  const audioTraits: TraitDefinition[] = [
    {
      ...
      parameters: [
        ...
        { name: 'volume', type: 'number', min: 0, max: 1, default: 1, description: 'Volume level', required: false },
        { name: 'rolloff', type: 'enum', enumValues: ['linear', 'inverse', 'exponential'], default: 'inverse', description: 'Distance rolloff model', required: false },
        { name: 'maxDistance', type: 'number', min: 1, max: 1000, default: 50, description: 'Max audible distance', required: false },
        { name: 'loop', type: 'boolean', default: false, description: 'Loop playback', required: false },
      ],
      compatibleWith: [],
      conflictsWith: [],
      version: '1.0.0',
      tags: ['audio', 'spatial', '3d', 'sound', 'positional'],
      performance: { gpuCost: 'negligible', cpuCost: 'medium', memoryCost: 'medium', drawCallImpact: 0, vrSafe: true, mobileSafe: true },
    },
  ];
  ...
  const animationTraits: TraitDefinition[] = [
    {
      ...
      parameters: [
        ...
        { name: 'duration', type: 'number', min: 0, default: 1, description: 'Duration in seconds', required: false },
        { name: 'easing', type: 'enum', enumValues: ['linear', 'ease-in', 'ease-out', 'ease-in-out', 'spring'], default: 'ease-in-out', description: 'Easing function', required: false },
        { name: 'loop', type: 'boolean', default: false, description: 'Loop animation', required: false },
      ],
      compatibleWith: [],
      conflictsWith: [],
      version: '1.0.0',
      tags: ['animation', 'tween', 'motion', 'easing'],
      performance: { gpuCost: 'negligible', cpuCost: 'low', memoryCost: 'negligible', drawCallImpact: 0, vrSafe: true, mobileSafe: true },
    },
  ];
