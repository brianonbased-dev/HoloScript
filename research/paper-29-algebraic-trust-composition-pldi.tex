...
The production test suite covers all four cases of the \texttt{execute}
discriminant (\texttt{allowed}, \texttt{deniedSandbox}, \texttt{deniedBudget},
\texttt{deniedBoth}) in
\texttt{packages/mcp-server/src/\_\_tests\_\_/\allowbreak SandboxCostComposition.test.ts},
introduced in commit \texttt{926fea018} and verified by 22 passing tests under
\texttt{vitest} (suite: \emph{SandboxCostComposition --- execute classification}).
...
in the work of Reynolds~\cite{reynolds1983types} on parametricity and of
Cardelli and Wegner~\cite{cardelli1985understanding} on subtyping
lattices---establishes that well-typed composition preserves semantic
properties.
...
Sabelfeld and Myers~\cite{sabelfeld2003language} survey the information-flow
control (IFC) literature, where the central challenge is composing security
policies across multiple principals without leakage.
...
produced significant results: Leroy~\cite{leroy2009compcert} produced the
first formally verified compiler (CompCert); Gu et al.~\cite{gu2016certikos}
verified the composition of OS-level security policies in CertiKOS; Klein et
al.~\cite{klein2009sel4} provided the first machine-checked proof of a complete
OS kernel with seL4.  \ATC{} contributes a small but complete composition
result in a newer proof assistant (Lean~4) aimed specifically at AI agent
infrastructure.
