...
of semiring provenance in databases. Semirings have also been
applied \emph{outside} databases as \emph{weight} or \emph{cost}
algebras over computation---e.g.\ semiring semantics for abstract
reduction/rewriting systems and semiring abstractions inside
tensor-compiler IRs---but in those settings the semiring carries
numeric weights, not hash-verifiable derivation provenance. Our
contribution is therefore narrower and distinct: to our knowledge
HS Core is the first IR to use a \emph{provenance} semiring as the
\emph{denotational target for cross-target compilation hashing},
and Theorem~\ref{thm:multitarget} is the first parametric guarantee
that composed multi-target output hashes are bounded by the source
hash in the tropical semiring. We do not claim priority for semiring
\emph{computation}; we claim it for the use of provenance semirings
as cross-target compilation invariants.
...
\paragraph{Executable agent-target evidence.}
The AI/Agent row is backed by
\texttt{packages/core/src/compiler/\_\_tests\_\_/AgentCompileTargetRegistry.test.ts}:
the test type-checks \texttt{agent-inference} as an \texttt{ExportTarget}
covered by the \texttt{\_AssertNever} target-classification gate, then
compiles the same \texttt{.hsplus} brain through
\texttt{ExportManager.export} for \texttt{agent-inference}, \texttt{nir},
and \texttt{webgpu}.  This is the executable evidence that agent brains
traverse the same compile-target registry path as neuromorphic and scene
targets, rather than a parallel agent-only pipeline.

\paragraph{Parser Architecture (W1-T2).}
The front-end was substantially refactored under the W1-T2 parser workstream.
The main code parser \texttt{HoloScriptCodeParser.ts} shrank from 6477 to 3017
LOC (a 53--57\% reduction depending on exact measurement point) through the
extraction of 11 focused modules covering lexer, core parsing, AST construction,
trait resolution, compiler dispatch, incremental updates, error recovery,
and format-specific variants (2D, persistence, \hsplus, etc.).
This modular boundary is a prerequisite for the trait composition system:
each extracted module is independently testable and composable with domain
plugins without touching the core parser. It also enables the Pillar-Slice
axis vocabulary validation (PillarRegistry cross-checks axis identifiers
against the live trait vocabulary at parse time). Reproducibility evidence:
current sources under \texttt{packages/core/src/*Parser*.ts}, associated
\texttt{*.test.ts} suites, and git history of the W1-T2 extraction commits.

\subsection{Pillar-Slice as a Trait-Native Runtime Primitive}
\label{sec:pillar-slice}

The Pillar-Slice Framework (D.040) elevates four-tuple axis extraction
to a first-class runtime primitive inside the contracted simulation loop.
A \emph{Pillar} is a pure function $P: \textit{PillarContext} \to \textit{PillarSlice}$
where $\textit{PillarSlice} = (\textit{axis}_1, \textit{axis}_2, p_1, p_2, \textit{pillarId}, \textit{domain})$.

PillarRegistry $\to$ SliceEmitter $\to$ SemanticCollaborationContract form a
trait-native composition pipeline. PillarRegistry holds the vocabulary of
valid axis identifiers per domain (PHYSICS\_CONSERVATION, INTENT\_TRUTH\_APPROVAL,
TEMPORAL, and extension families) and cross-validates trait applications at
parse time. SliceEmitter hooks \texttt{ContractedSimulation.step()} to extract
fresh 4-tuples on every simulation tick, sealing them via the existing
\texttt{computeStateDigest} path and emitting GRPO training samples.
The resulting slices become the wire-format substrate for
SemanticCollaborationContract (latent-space agent communication, replacing
token streams with Pillar-axis vectors). Axis vocabulary validation,
dynamic axis identity, and GRPO training-sample emission are all first-class
trait semantics.

\begin{lstlisting}[language=TypeScript,
  caption={PillarRegistry trait handler onAttach/onEvent pattern (packages/core/src/traits/pillar/PillarRegistry.ts)}]
onAttach(node: HSPlusNode, _config, _context) {
  const state = { pillars: new Map(), slice_count: 0, unique_fingerprints: new Set() };
...
onEvent(node, config, context, event) {
  const state = node.__pillarRegistryState;
  if (event.type === 'pillar:generate') {
    const pillar = state.pillars.get(pillar_id);
    const slice = pillar.generate(resolvedContext);
    if (!pillar.axis_vocabulary.includes(slice.axis_1_id) ||
        !pillar.axis_vocabulary.includes(slice.axis_2_id)) {
      context.emit('pillar:error', { code: 'INVALID_AXIS', ... });
      return;
    }
    state.slice_count++;
...
HoloScript traits are the only published runtime supporting runtime-variable
axis identities as first-class language constructs (Paper 32, NeurIPS 2027
target). Unlike fixed quaternions or opaque learned embeddings, the
\texttt{axis\_1\_id} and \texttt{axis\_2\_id} in every generated slice are
drawn at runtime from the Pillar's declared \texttt{axis\_vocabulary}, making
the coordinate space itself a live, queryable, and auditable trait artifact.

Pillar coordinate space IS a tropical variety (arXiv:1805.07091); dispatch
regions are cells of the tropical classification fan (arXiv:2403.11871).
The trait system makes this geometry prescriptive at runtime: every slice
is vocabulary-validated by the registry before it can affect simulation,
compilation, or inter-agent IPC.

SemanticCollaborationContract is the inter-agent IPC primitive enabled by
the trait system: Pillar-generated 4-tuple slices serve as the latent
message payload, carrying provenance receipts, brain coordinates, and
scene deltas without text-token conversion. This directly extends the
multi-target compilation theorem into embodied multi-agent settings.

Reproducibility: \texttt{PillarRegistry.ts}, \texttt{SliceEmitter.ts},
\texttt{SemanticCollaborationContract.ts}, the three seed Pillars (physics,
intent, temporal), and the handler registration in the core trait dispatcher
(all shipped 2026-05-20 with zero tsc errors).
...
The provenance semiring compiler operates in $O(n \cdot k)$ time and $O(n)$
space where $n$ is the number of AST nodes and $k$ is the number of compile
targets: each node is visited once per lowering pass and each lowering function
is $O(1)$ (a single hash composition per node). Memory is linear in the AST
size because the provenance chain is accumulated in a fixed-width semiring
element per node rather than a growing log. Audit-chain \emph{decoding}---the
operation a verifier performs to check a compiled output against its source
hash---is $O(\text{depth})$ hash operations where depth is bounded at
$8{+}p{+}2t$ by construction (\S\ref{sec:eval}); for the benchmarked
configuration ($p{=}3$, $t{\leq}2$) this is at most 15 hash lookups per
target, independent of scene size $n$. The tropical SpMV kernel used for
cross-target verification (Table~\ref{tab:ablation}) runs at 159{,}340\,hz/s
on BA-class provenance graphs, confirming that the $O(\text{depth})$ decoding
budget is not the practical bottleneck---scene parsing dominates for large $n$.
...
Provenance tracking adds bounded but nonzero overhead to compilation. For
sufficiently large scenes compiled to many targets, cumulative hashing cost may
become significant; the crossover has not been measured. A large-scene stress
run remains future work, with batched hashing and incremental compilation
(below) as candidate mitigations.
...
\begin{itemize}
  ...
  \item \textbf{ContractClause: pre/inv/post clause sets.} \emph{(Shipped.)}
    Typed \texttt{preconditions}/\texttt{invariants}/\texttt{postconditions}
    are parsed from the \texttt{sim\_contract~\{\}} block
    (\texttt{packages/core/src/parser/HoloCompositionParser.ts}) and
    evaluated—before the solver, per step, after finalize—by
    \texttt{PluginSolverContract} (\texttt{packages/core/src/plugin-solver-contract/}).
    Violations are carried into the receipt; a green receipt is a
    witness that no clause fired.  A non-trivially falsifiable invariant
    is required before a trace is labelled \texttt{verified}
    rather than \texttt{execution\-only}.
  \item \textbf{ParameterEnvelope: first-class valid-parameter domain.} \emph{(Shipped.)}
    A generic \(\{param,\min,\max,\text{allowed},\text{unit},\text{onViolation}\}\)
    envelope (\texttt{packages/core/src/parameter-envelope/ParameterEnvelope.ts})
    attaches to compiled contracts.  Within-envelope remixes inherit
    the proof automatically; out-of-envelope remixes trigger
    \texttt{redischarge} (re-proof required) or emit an honest
    \texttt{error}---resolving the provenance ambiguity between
    ``parameter changed'' and ``correctness boundary crossed.''
  \item \textbf{DimensionalTypeSystem: SI-unit refinement types.} \emph{(Shipped as Layer~8.)}
    \texttt{Float<kg>}, \texttt{Float<N>}, \texttt{Float<N/m>} and the
    full SI-unit UNIT\_REGISTRY are implemented as the eighth
    \texttt{CompilerSafetyPass} layer
    (\texttt{packages/core/src/compiler/safety/DimensionalTypeSystem.ts})
    and wired into \texttt{runSafetyPass} via \texttt{dimensionalNodes}.
    Unit-mismatch sim-to-real failures are caught at parse time across
    all domain plugins without any plugin code change.
\end{itemize}
