...
\begin{itemize}
  ...
  \item \textbf{ContractClause: pre/inv/post clause sets.}
    Attach typed \texttt{preconditions}/\texttt{invariants}/%
    \texttt{postconditions} to \texttt{ContractConfig}, evaluated
    at construction, per-step, and at finalize, with violations
    carried into the receipt.  This closes the gap between ``a run
    happened'' (the current guarantee) and ``a run was right''
    (a dischargeable theorem): a green receipt becomes a witness
    that no clause fired, and a non-trivially falsifiable invariant
    is required before a trace is labelled \texttt{verified}
    rather than \texttt{execution\-only}.
  \item \textbf{ParameterEnvelope: first-class valid-parameter domain.}
    Attach a generic \(\{param,\min,\max,\text{allowed},\text{unit}\}\)
    envelope to compiled contracts so that parametric correctness
    (``for any parameters within the envelope the postcondition
    holds'') is a compile-time-checkable property.  Within-envelope
    remix inherits the proof automatically; out-of-envelope remix
    re-discharges or flags honestly---resolving the provenance
    ambiguity between ``parameter changed'' and ``correctness
    boundary crossed.''
  \item \textbf{DimensionalTypeSystem: SI-unit refinement types.}
    Introduce \texttt{Float<kg>}, \texttt{Float<N>}, \texttt{Float<N/m>}
    as a sixth \texttt{CompilerSafetyPass}, catching unit-mismatch
    sim\-to\-real failures at parse time across all domain plugins
    without requiring any plugin code change.  Generalises the
    shipped \texttt{@freshnessBound} compile-time checker to the
    physical-dimension axis.
\end{itemize}
