...
_internal_ name for the **Udon bytecode** target. The real-world surface is:
...
| **UdonSharp** | Community C#→Udon compiler (Unity-side).                                            | Live, what we emit today                                                                                  | Our current path; D.064 wants to move _off_ it.          |
| **Udon 2**    | Abandoned successor working-title.                                                  | **Dead** (dropped Nov 25 2024)                                                                            | Do not target.                                           |
| **Soba**      | Official Udon successor. High-level OOP **C# 9.0** editor, usable _alongside_ Udon. | **Closed beta prep** as of Mar 2026; "actively worked on" per May 22 2026 dev stream; **no release date** | Future retarget (Phase 5). Not buildable now.            |

**Avatars are out of scope for "Byte."** VRChat avatars do **not** run Udon — they use
animator/expression params + PhysBones. `examples/avatars/vrchat-avatar.holo` is a _separate_
...
**Consequence:** HoloScript can never own the _runnable_ VRChat world end-to-end. VRChat is
...
What we _can_ own offline (the sovereignty we actually have within the bridge):
...
- It is **offline-verifiable** (grammar + EXTERN resolution) → satisfies "gate-enforced, not
  asserted."
- A raw `.uasm` alone is _useless to a human_ without scene wiring → the bundle wrapper is what
...
opaque, and not human-inspectable; emit it _from_ `.uasm` inside the Unity CI step instead.
...
_Goal: make the artifact contract explicit so the guard can open._
...
_Goal: a verifiable target before any codegen. Build the ruler before the thing you measure._
...
_Goal: emit valid Udon Assembly for the trait subset the C# path already covers._
...
_Goal: close the loop to a real, uploadable world and prove it once._
...
_Goal: the VRChat target dissolves TS into `.hsplus`, and coverage widens._
...
_Goal: be ready to flip to the official future the day beta opens._

- Soba is **C# 9.0** and _closer to our current emitter_ than UASM is. Maintain a thin spike that
...
- **Ingest direction** (`hs import-vrchat`, VRChat→`.holo`, bridge doc §1) is a _separate_ track —
...
**Smallest valuable slice (de-risk before committing the full L phases):** Phase 0 + a _reduced_
...
3. **Treat VRChat as a permanent BRIDGE** — pour the _sovereign_ effort into offline codegen +
