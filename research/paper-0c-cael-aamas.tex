...
\paragraph{Dispute resolution and dreaming.}
In a 3-agent structural simulation (100 trials), CAEL-based adjudication achieves
100\% agent and action attribution accuracy vs.\ 34\% (last-action baseline) and
41\% (reputation baseline), at 48.3\,ms adjudication cost dominated by solver
re-execution. All 10 dream episodes complete within the $\pm10\%$ perturbation
budget (mean 8.7\%), with valid hash chains linked to the waking run ID;
dream overhead is 61.2\,ms/episode, dominated by SNN re-evaluation.

\paragraph{Hardware throughput and ablation.}
Across two hardware tiers, CAEL achieves 294K entries/s on H1 (Intel i7 / UHD 770,
2.8\% overhead) and 649K entries/s on H3 (RTX 6000 Ada, 1.3\% overhead), far
exceeding the 240 entries/s production rate at 60\,Hz. Both H1
($16.2 \pm 2.4$\,ms median) and H3 ($7.4 \pm 0.9$\,ms) sustain 60\,Hz with
margin; hash-chain append contributes $<0.025\%$ of loop latency. Ablating
\texttt{appendCaelTrace} recovers the 2.0\,$\mu$s baseline but removes
replay-determinism and dispute-resolvability. Full protocols, tables, and the
preregistered user-study packet are in the supplemental material.
...
(RTX\,6000\,Ada) is 649K entries/s, both exceeding
...
Behavioral determinism is given mechanical force by an explicit digital-twin
agreement harness: independent record and replay implementations agree on a
canonical wire-format projection, verified by 33 assertions (including explicit
false cases) that pass in 112\,ms~\cite{paper-0c-twin-test-2026-05-11}. Full
protocol detail is in the supplemental material.
...
% Page-budget verification 2026-05-24: AAMAS 2026 main-track instructions allow
% 8 pages plus additional bibliography-only pages. Scratch-copy MiKTeX build:
% Conclusion is on page 8; References start on page 9; total PDF is 10 pages.
