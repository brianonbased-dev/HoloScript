...
The CAEL cognitive loop uses \texttt{EmbeddingTrait} (backed by
\texttt{OnnxRuntimeTrait} and \texttt{NeuralForgeTrait} for real
neural inference with local fallback) as the embedding layer feeding
\texttt{JEPAPredictor.plan()} from \texttt{think()}.
...
In a 3-agent structural simulation (100 trials), CAEL-based adjudication achieves
100\% agent and action attribution accuracy vs.\ 34\% (last-action baseline) and 41\%
(reputation baseline), at 48.3\,ms adjudication cost dominated by solver re-execution.
Full protocol, metrics, and results table are in Appendix~\ref{app:dispute}.

\subsection{Dreaming Evaluation}

All 10 dream episodes complete within $\pm10\%$ perturbation budget (mean 8.7\%),
hash chains valid, all traces linked to waking run ID. Dream overhead is 61.2\,ms/episode,
dominated by SNN re-evaluation; hash-chain construction adds $<$0.4\,ms/episode.
Full metrics table in Appendix~\ref{app:dream}.
...
Across two hardware tiers, CAEL achieves 294K entries/s on H1 (Intel i7 / UHD 770,
2.8\% overhead) and 649K entries/s on H3 (RTX 6000 Ada, 1.3\% overhead)---both far
exceeding the 240 entries/s production rate at 60\,Hz, confirming that CAEL is
compute-bound by the solver, not the hash chain (see Appendix~\ref{app:gpu-bench}
for full protocol, hardware specification, and per-tier table).
...
Both H1 (Intel UHD 770, $16.2 \pm 2.4$\,ms median) and H3 (RTX 6000 Ada,
$7.4 \pm 0.9$\,ms) sustain 60\,Hz with margin; the hash-chain append contributes
$<0.025\%$ of total loop latency at both tiers, confirming that provenance is
effectively free at production tick rates (see Appendix~\ref{app:full-loop-demo}
for full protocol and per-tier breakdown).
...
We ablate the \texttt{appendCaelTrace} audit-write call while holding all other
loop phases constant.  The incremental CAEL overhead is $+60.6\,\mu$s/tick
(0.36\% of a 16.67\,ms budget); removing it recovers the 2.0\,$\mu$s baseline
but loses replay-determinism and dispute-resolvability.  The full ablation table
and extended analysis are in Appendix~\ref{app:ablation}.

% ══════════════════════════════════════════════════════════════════════════════
%  D.011 GATE — COMPUTATIONAL COMPLEXITY
% ══════════════════════════════════════════════════════════════════════════════
\subsection{Computational Complexity}
\label{sec:complexity}

The dominant per-tick cost of CAEL is the \texttt{appendCaelTrace} audit-write
that appends one hash-chain entry per agent.  For a simulation with $N$ agents
running at $F$\,Hz, the time complexity of the provenance layer is $O(N)$ hash
operations per tick---each a single SHA-256 evaluation ($\approx$\,1.5\,$\mu$s).
The measured rate at H1 (Intel i7\,/\,UHD\,770) is 294K entries/s and at H3
(RTX\,6000\,Ada) is 649K entries/s (Section~\ref{sec:gpu-bench}), both exceeding
the 240 entries/s production rate at 60\,Hz by $>$1,000$\times$.  Aggregate
memory complexity is $O(N \cdot T)$ for a replay buffer of $T$ ticks; with a
rolling window of $W$ ticks (default $W = 600$ at 60\,Hz), this reduces to
$O(N \cdot W)$---approximately 2.4\,MB per 100 agents at 64-byte entries.
Dispute-resolution queries scan at most $O(T)$ entries for a single-agent replay;
multi-agent cross-causal queries are $O(N \cdot T)$ in the worst case but
bounded in practice by the causal-fan-out depth (typically $\leq$\,3 hops in
measured scenes).  The net overhead of $+60.6\,\mu$s/tick (0.36\% of 16.67\,ms)
confirms that CAEL's provenance budget is dominated by solver physics, not
...
Concurrently, \citet{harnessingembodied2026} propose a runtime governance framework with similar control-boundary separation; CAEL differentiates through sub-millisecond overhead measurement ($60.6\,\mu$s/tick) and cryptographically-anchored execution receipts.
...
Behavioral determinism is given mechanical force by an explicit digital-twin agreement
harness: two independent implementations of the same state machine (a record path and a
replay path) agree on a canonical wire-format projection, verified by 33 assertions
(including explicit false cases) that pass in 112\,ms~\cite{paper-0c-twin-test-2026-05-11}.
Full protocol detail in Appendix~\ref{app:twin-harness}.

\paragraph{Hash function choice.}
FNV-1a (32-bit) suffices for non-adversarial traces ($\leq 10^6$ entries, collision
prob.\ $< 10^{-3}$); SHA-256 is selected via \texttt{useCryptographicHash} at
$\sim$9--24$\times$ cost for adversarial settings. The hash mode is recorded in
\texttt{cael.init.payload.hashMode}, so substitution attacks are caught at replay.

\paragraph{Scalability.}
CAEL traces grow $O(N \cdot T)$ with agents and ticks; at 60\,Hz with 4 entries/tick
a one-hour run produces $\sim$148\,MB---manageable on modern hardware. A streaming
checkpoint mode (future work) would bound memory to $O(N \cdot W)$ for window~$W$.

\paragraph{Generalization to scientific simulation.}
The \texttt{ContractConfig} mechanism extends naturally beyond multi-agent settings:
folding a full parameter vector into the Contract-ID makes observationally
indistinguishable runs cryptographically distinguishable~\cite{architects2026,sbi2022}.
Current evaluation is limited to multi-agent embodied scenarios; scientific-simulation
integration (e.g., astrophysics subgrid degeneracy labeling) is future work.

\paragraph{Limitations.}
\textbf{Solver fidelity}: guarantees hold only inside a \texttt{ContractedSimulation};
uncontracted solvers break replay by design.
\textbf{Adversarial threat model}: FNV-1a suffices for non-adversarial settings;
SHA-256 mode ($\sim$9--24$\times$ cost) and external anchor mechanisms (Ed25519,
OpenTimestamps) address adversarial write access but are outside CAEL's scope.
\textbf{SNN expressiveness}: the current LIF model is biophysically basic; richer
neuron models (Izhikevich) trade realism for overhead.
\textbf{Fork cost and distribution}: Fork-N scales linearly with $N$; distributed
multi-agent scenarios require clock synchronization (future work).
...
Three primary directions: (1)~\textbf{Embodied SNN learning}: using CAEL traces as a
physically grounded curriculum---dream episodes should improve transfer to unseen
configurations vs.\ uncontracted random perturbations.
(2)~\textbf{Self-model via provenance}: agents with read access to their own trace can
detect environmental tampering and verify perceptual consistency constructively.
(3)~\textbf{Evolutionary provenance}: neuroevolution where every SNN mutation carries
causal provenance, enabling post-hoc causal attribution of fitness improvements.
...
% Supplemental material (§6.4 Scenario 3, §6.5 Dreaming Evaluation, §7 twin-harness)
% is in paper-0c-cael-aamas-supplemental.tex (separate submission file, not counted
% in AAMAS 8-page main-text limit). Moved 2026-05-23 for page-budget compliance.
