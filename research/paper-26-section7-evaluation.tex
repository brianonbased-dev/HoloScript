...
($\eta = 0.005$).  Gradients are verified against central finite differences
...
\begin{itemize}
  ...
  \item Tail plateau CV $= 4.0\%$ ($< 25\%$ threshold, no upward drift): \textbf{pass}
...
All 4 stability checks pass.  \emph{Decomposition note:} the total eval-loss
improvement reflects regularisation convergence: the conservation term drops from
$1.049$ to $0.000$ and SIGReg from $4.148$ to $1.854$, while the MSE component
increases from $0.052$ to $0.083$ (the optimiser trades raw prediction error for
constraint compliance at this training scale).  The eval-loss trajectory is the
primary stability signal; MSE is expected to improve with longer training.
...
\begin{table}[h]
\centering
\caption{HoloMesh team-protocol coordination latency and throughput.
  Sequential: 8 authenticated board reads in series.
  Burst: 6 concurrent board reads (wall time 304ms).
  Artifact: \texttt{section7-harness-2026-05-21.json}.}
\label{tab:holomesh-coordination}
\begin{tabular}{lrrrr}
\toprule
Path & p50 ms & p95 ms & throughput req/s & success \\
\midrule
Sequential (8 reads) & 53.159 & 222.387 & 12.039 & 100.0\% \\
Burst (6 concurrent) & 208.255 & 302.379 & 19.740 & 100.0\% \\
\bottomrule
\end{tabular}
\end{table}
...
workflow pipelines, reports rapid practitioner adoption~\cite{cliefnotes2026}.  ICM's
