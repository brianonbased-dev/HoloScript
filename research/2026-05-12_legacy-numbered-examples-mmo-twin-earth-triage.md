...
**Immediate action:** 15 files have north-star-relevant _concepts_ but use stale package names or pre-Twin-Earth trait vocabularies. A migration pass should update them to current primitives so they become promotable.
...
_Audit closed. 14 promote, 97 keep, 15 migrate, 4 archive. MMO/Twin Earth north-star coverage in numbered examples is 11 % (promote) + 12 % (migrate-after-fix) = 23 % aligned._
