...
**Naming-vs-content overlap**: each preset file in `lib/presets/templates/` exports a `SceneTemplate` (see [packages/studio/src/lib/presets/templates/ai-composer.ts:1](packages/studio/src/lib/presets/templates/ai-composer.ts:1)) — the _same type_ as the entries in `data/sceneTemplates.ts`. Same data shape, two registries, no documented "preset" vs "scene template" distinction. Either:
