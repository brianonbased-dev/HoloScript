...
_current_ language surface with file-path evidence across 4 axes + papers + competitor framing.
...
- **448/448** top-level `.holo` traits carry a `behaviors:[@x,@y]` composition field — behavior-fused-to-form is the _structural default_, not an aspiration.
...
Every axis has a **freeform escape hatch** — an untyped surface through which the _generic /
assembled / dead_ version slips past the compiler unnoticed:
...
| Physics       | material is **optical-only**; physics props are `Record<string,unknown>`             | a scene that _looks_ solid but has no physical truth       |
...
### Track A — Behavior/Interaction _(the direct continuation of `/taste`)_
...
— the lamp's point of view authored as typed data the compiler _and the taste gate_ consume.
...
### Track B — Generativity _(the biggest "ultra-unique" lever)_
...
**The anti-average, structurally:** a frontier model returns _the_ average castle; a `@generator`
returns _this_ castle from _this_ site's constraints, and never the same twice — with the taste
baked into the parameters. Only **4 of 448** traits currently declare a seed — this is the thinnest,
highest-upside axis.

### Track C — Embodied Mind _(the sovereign moat — D.102)_
...
### Track D — Physically-Real Interaction _(simulation-first; the sim→real / robot horizon)_
...
- **Unity / Unreal** make you _hand-code_ interactivity in C#/Blueprints and _hand-place_ uniqueness.
  We author it as **typed declarative data + gate-verified provenance** — a different layer, not a
  faster horse. (Unreal PCG is the one place they're close; our `GeneratorIR` should compile _to_ it
...
behavior — then blind-judged for _interaction_ taste the same way form taste was. This:
...
The plan raises native-authored coverage _and_ adds the **"is-right, not looks-right"** dimension
...
1. **Falsifiable claim.** When the slice is done, `check:interaction-taste` runs in the gate suite
   ...
   go from _N_ flagged interactions to **0** after upgrade, and a deliberately-stubbed control object
   **stays flagged**.
...
4. **Scope + blast.** _In scope:_ the typed event union + `interaction_profile` grammar in
   parser/types, the new gate under `scripts/holo-ci/`, and the 7 examples' handler bodies.
   _Out of scope:_ runtime dispatch semantics (`UIEventRouter` stays — we type the vocabulary, not
   rewrite dispatch), Tracks B/C/D, materials. _Regression risk:_ typing `HoloEventHandler.event`
...
_Every construct and gap above is file-path-grounded in the research run; no capability is claimed
