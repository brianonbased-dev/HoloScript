...
|--------|:---:|:---:|---|
| whitewasher | **1.00** | [0.886, 1.000] | Honest trust-build then exploit lands -- formula does not semantically vet output content (SS4.1). |
| **sybil** | **0.00** | [0.000, 0.114] | **DEFENDED by live V11 ceiling** -- cross-vouching cannot reach the 1.5x inflation threshold because reuse weight is bounded by 2x direct work. |
| score-manipulator | **1.00** | [0.886, 1.000] | Goodhart on v1 formula -- high trust with decoupled utility, no utility cross-check (SS4.4). |
| slow-poisoner | **1.00** | [0.886, 1.000] | Aggregate bias accumulates undetected over the 1000-round window (SS4.3). |
| eclipse | **1.00** | [0.886, 1.000] | Victim trust driven below 0.3x baseline (SS4.5). |
...
defended-condition measurement (canary probing SS5.4, etc.) must then close.
...
- Run the **defended** condition (canary SS5.4 + other SS5 defenses wired into the
