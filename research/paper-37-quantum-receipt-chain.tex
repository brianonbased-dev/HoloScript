...
\small
\begin{tabular}{@{}p{2.4cm}rrp{1.5cm}r@{}}
\toprule
\textbf{Result} & $\suppfactor$ & $\af$ & \textbf{Regime} & $\suppfactor_{\mathrm{det}}$ \\
\midrule
MS/QH carbon (sel.~rej.) & $800\times$ & \todo{undisclosed} & heralded & \todo{needs $\af$} \\
MS/QH carbon (filtered) & ${\sim}5\times$ & $1.0$ & deterministic & $5\times$ \\
MS/QH mid-computation & ${>}50\times$ & \todo{undisclosed} & mid-comp. & \todo{needs $\af$} \\
\textbf{Ours} ($d{=}11$, $p{=}0.1$) & $444\times$ & $\mathbf{1.0}$ & \textbf{classical sim.} & $\mathbf{444\times}$ \\
\bottomrule
\end{tabular}
\end{table}

The \todo{undisclosed} entries illustrate exactly the gap this schema
closes. Until $\af$ is a required field, the 800$\times$ headline and the
444$\times$ headline cannot be honestly compared: one may reflect a 1\%
acceptance fraction; the other reflects 100\%. The schema makes this
comparison structurally possible.
...
\todo{Tier-1 Quantinuum backend required. The build spec in
\texttt{2026-06-15\_microsoft-quantinuum-qec.md} defines the
\texttt{QuantinuumBackend} in \texttt{qm-bridge}: mirror the existing
\texttt{IBMQuantumBackend} pattern with pytket/qnexus SDK; run logical
qubits (carbon/tesseract encoding); emit logical-vs-physical error rate
and real $\af$ into \texttt{cael-quantum-v1.qec}. Hardware run is
founder-gated paid compute (D.080).}

\paragraph{Tier-2: 2-D surface code under circuit-level noise.}
\todo{The schema's \texttt{corrects\_axes: ["X","Z"]}, \texttt{rounds > 1},
and \texttt{decoder: "pymatching-mwpm"} fields target the 2-D surface
code. Implementation requires \texttt{pymatching}; this is the next
rung above the repetition-code Tier-0 and would make mid-computation
correction benchmarks representable.}
