...
\S\ref{sec:rtx-benchmark}. Table~\ref{tab:ablation} reports per-trait cost across batch
...
The CPU-only Vitest/Node harness (Section~\ref{tab:ablation}) is now complemented by a
Playwright-driven WebGPU harness that executes an equivalent semiring-resolution
workload on real RTX-class silicon.  The harness lives at
...
A single capture on 2026-05-20 (task\_1779176532119\_5c81) exercised the full
...
per-batch structure.  On a proper codex-hardware / grok1-x402 RTX seat with
native Vulkan drivers the WGSL tropical-semiring proxy (min-plus style conflict
resolution over batched trait props) will supply the missing D.011 numbers.
