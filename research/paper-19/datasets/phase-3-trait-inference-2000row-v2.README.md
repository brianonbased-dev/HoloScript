...
1. **215 uncategorized traits — registry drift, not dataset drift.** Traits like `@interactable` (119 rows), `@reactive` (119), `@cultural_profile` (117), `@hand_tracked` (81), `@ai_agent`, `@health_system`, `@damage_dealer`, `@metadata_display`, `@npc`, `@host_only`, `@owned`, `@holdable`, `@bezier`, `@semantic_entity`, etc. appear in real `.holo`/`.hsplus` examples and are **correctly extracted as gold labels** — the dataset-side labeling is fine. What is missing is the **declarative trait-registry entry** for each. They are referenced by consumer code (`packages/core/src/compiler/*`, `packages/core/src/traits/traitConstraints.ts` BUILTIN_CONSTRAINTS, `packages/core/src/traits/visual/presets/*.visual.ts`, `*Trait.ts` handler files) but **not** declared in `packages/core/src/traits/constants/*.ts` (`*_TRAITS` arrays) nor `packages/plugins/*/src/index.ts` (`pluginMeta.traits`). The dataset's `metadata.uncategorized_traits` per-row + validator's aggregate count are the regression signal for the trait-registry completeness task ([task_1777275577564_1bmw](https://mcp.holoscript.net/api/holomesh/team/team_1775935947314_f0noxi/board)) — gate condition: when the registry catches up, this count drops to 0 and re-running the deriver + harvester will fully family-tag every corpus row. The v2 deriver intentionally scans declarative sites only (per "plugins are data, not code" / Refusal 2) and refuses to extend its scope to consumer-code grep patterns; doing so would propagate the architectural drift into the dataset rather than surface it.
