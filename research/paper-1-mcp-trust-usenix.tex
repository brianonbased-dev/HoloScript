...
$\sim$282 bytes per entry (R$^2{=}1.000$; see \S\ref{sec:eval-scaling}).
Hash-chain verification runs in $O(n)$ time with
...
\subsection{Sycophancy Attack (Two-Axis Integrity)}

Sycophancy is an orthogonal threat axis to Byzantine corruption (W.617 Two-Axis Agent Integrity Framework). A cooperative MCP tool or downstream agent can gradually mirror the host agent's priors and approval preferences, producing results that pass all Byzantine filters yet systematically drift toward what the caller wants to hear. Unlike adversarial Byzantine actors, sycophantic behavior is cooperative and therefore invisible to signature, cosine-anomaly, or consensus checks.

The \emph{truth\_approval} PillarDomain in \texttt{SemanticCollaborationContract.ts} (shipped 2026-05-20) implements runtime detection: a rolling 4-D centroid tracks incoming pillar slices on the truth\_approval axis; when approval pressure (pos\_2) drifts beyond the \texttt{centroid\_drift\_threshold} (default 0.4), an \texttt{integrity\_fail} event with reason \texttt{centroid\_drift} is emitted. This provides the second axis required for USENIX-grade threat models.

Griffiths (Laws of Thought, Macmillan 2026) argues that sycophancy dominates hallucination as a long-term multi-agent risk precisely because it is unfalsifiable within a single session and compounds across interactions.

Our trust protocol therefore claims coverage of both axes: cryptographic signing and replay (Byzantine) plus centroid-drift monitoring in the semantic collaboration layer (sycophancy). If the MCP trust implementation does not yet wire the \texttt{truth\_approval} domain for every tool response, this is explicit future work with a concrete spec already running in production.
...
\begin{table}[t]
  ...
  \measuredFrom{research/paper-1-mcp-trust-scaling.md}%
  \begin{tabular}{@{}lrrr@{}}
    \toprule
    ...
    Agent loop (50 ticks)& 50  & 203  & 57,246  \\
    \bottomrule
  \end{tabular}
\end{table}

Table~\ref{tab:tracesize} shows trace size for representative workloads. Each
CAEL entry averages $\sim$282 bytes in JSONL format. The entry schema contains
nine fields (version, runId, index, event, timestamp, simTime, prevHash,
hash, and payload); the payload includes per-step state-vector digests
(\texttt{stateDigests}, FNV-1a, added in Wave-2 2026-04-20) that account for
the increase from the pre-Wave-2 estimate of~180 bytes/entry. Trace size
scales linearly with the number of simulation events (R$^2 = 1.000$,
\texttt{paper-cael-replay-benchmark.test.ts} scaling sweep, N\,$\in$\,$\{10^3,
10^4, 10^5\}$; see \texttt{research/paper-1-mcp-trust-scaling.md}):
$|T| \approx 282 \cdot (n_{\text{steps}} + n_{\text{interactions}} + 3)$,
where the $+3$ accounts for init, solve/final, and metadata entries.
...
\subsection{Scalability Envelope}
\label{sec:eval-scalability}

The scaling envelope follows directly from trace size
(Table~\ref{tab:tracesize}) and verification throughput
(Table~\ref{tab:gpu-bench}). At $\sim$282 bytes per entry, a $10^6$-entry
trace occupies about 282~MB on disk. Hash-chain verification remains $O(n)$ in
trace entries: the H1 median rate of 6.5~$\mu$s per entry (100\,003-entry
sweep, \texttt{paper-1-mcp-trust-scaling.md}) verifies $10^6$ entries in about
6.5~s, while the H3 projection ($\approx$3.4$\times$ H1 throughput) verifies
the same trace in about 1.9~s. Full replay intentionally does not scale with
trace length alone: it repeats the solver and is bounded by solver runtime.
For million-scale operations, the deployment policy is to hash-verify every
trace, retain only
trace IDs and final hashes in MCP responses, shard long JSONL traces by run
segment, and full-replay only safety-critical, anomalous, or sampled results.
...
\measuredFrom{research/benchmark-results-2026-05-21-paper-1-mcp-ablation.md}%
\begin{tabular}{lccc}
\toprule
...
is compact ($\sim$282 bytes per entry), the verification is fast
($\sim$6.5~$\mu$s per entry for hash verification at H1), and the approach is
