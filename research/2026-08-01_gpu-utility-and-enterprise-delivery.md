...
| A.GPU.007 | Enterprise capacity/policy/budget/audit surface                                          | Bounded internal dispatch, grant redemption, running ownership, and expired-heartbeat recovery substrate implemented: signed evidence, atomic integer-minor-unit holds, dedicated headless executor/reaper identities, deterministic server-only fencing, a PoP-sealed logical-lease grant, one-time `leased -> starting` redemption, signed `starting -> running` acknowledgement, append-only heartbeat custody, current-ownership expiry reaping, logical-slot release, retained positive-cost holds, and commit-time allocator/budget/heartbeat rechecks; public executor/reaper routing, billing, provider reconciliation, and execution remain unshipped |
...
- **DONE:** subsequent internal heartbeat expiry/reaping and logical lease recovery with a database-clock/current-ownership guard.
- **STAGED NEXT:** a lease-owned outbox consumer/adapter contract that cannot claim provider reservation before a provider receipt exists.
...
### Bounded execution-recovery continuation

The reaper advances only an exact current running attempt whose latest signed ownership heartbeat is expired:

```text
running + exact current ownership receipt
  -> recheck registered headless reaper identity
  -> authenticate ownership signature, canonical bytes, lease, fence, capacity, and policy binding
  -> require heartbeatValidUntil <= database transaction clock
  -> lock and reject any newer ownership sequence
  -> lock the exact active paid hold without consuming or releasing it
  -> commit running -> failed with completionDisposition = execution_unobserved
  -> atomically release only the HoloScript logical allocation
```

Before the logical lease expiry the canonical reason is `executor_lost`; at or after that boundary it is `lease_expired`. The database revalidates the prepared choice at commit time and fails closed if the boundary changed. A stale selector cannot reap through a later heartbeat, and an early selector cannot use service-clock skew to win before expiry. For a positive-cost WorkUnit the exact signed hold remains `held`, because absence of a heartbeat is not measured provider cost and is not proof that the provider released a reservation. Zero-cost work cannot inject a hold.

Recovery emits one ordinary `compute_job.failed` outbox record and keeps `providerReservation` and `execution` at `not_asserted`. It does not issue a provider stop, claim a GPU was running, infer a charge, settle a hold, or expose a new public route. The retained hold is deliberately conservative: a later provider/evidence adapter must reconcile measured consumption or release it under an authenticated no-charge receipt.

- **DONE:** strict internal reaper identity; exact current-ownership selector; database-clock expiry; stale-heartbeat race exclusion; deterministic unobserved reason; atomic terminal transition and logical-slot release; exact positive-cost hold retention; zero-cost hold rejection.
- **STAGED NEXT:** lease-owned outbox consumer/provider adapter with authenticated reservation/start/stop receipts and terminal measured-cost reconciliation.
- **DEFERRED:** public executor/reaper routes, live provider reservation/API calls, actual GPU execution, result ingestion, billing/invoicing, deployment, and real-backend benchmarks.

Validation for this continuation passed 57/57 focused dispatcher/reaper/store tests, 318/318 impacted dispatcher/store/service/route/HTTP regressions, a clean MCP package TypeScript check and build, and 26/26 tests against an isolated PostgreSQL 16 container. The database suite proves both successful expired-heartbeat recovery and the inverse race: a newer committed heartbeat defeats a stale prepared reap while the job remains `running`, the logical slot remains leased, and the paid hold remains held. Production files and the forced-included changed tests passed ESLint; all scoped TypeScript files passed Prettier.
