...
| A.GPU.007 | Enterprise capacity/policy/budget/audit surface                                          | Bounded internal dispatch substrate implemented: signed evidence, atomic integer-minor-unit holds, dedicated headless executor identity, deterministic server-only fencing, exact durable read-back, and a PoP-sealed logical-lease grant; route wiring, grant redemption/start, billing, provider reconciliation, and execution remain unshipped |
...
This envelope is not an action authorization and does not prove provider reservation, active GPU possession, workload start, heartbeat, execution, or completion. Those facts require a later one-time redemption/start transaction with current lease, allocation, budget, expiry, revocation, audience, and replay checks plus authenticated execution evidence.
