...
| A.GPU.007 | Enterprise capacity/policy/budget/audit surface                                          | Bounded internal dispatch and grant-redemption substrate implemented: signed evidence, atomic integer-minor-unit holds, dedicated headless executor identity, deterministic server-only fencing, a PoP-sealed logical-lease grant, one-time `leased -> starting` redemption, and commit-time allocator/budget rechecks; route wiring, `mark_running`, heartbeat, billing, provider reconciliation, and execution remain unshipped |
