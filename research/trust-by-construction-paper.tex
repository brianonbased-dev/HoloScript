...
claim. With the gate enabled, 7/7 fabrication classes were blocked, 4/4
genuine classes passed, and one known leak remained: a no-op hidden behind
a productive command prefix. With the gate disabled, all seven fabrication
classes were admitted. The result does not prove that the produced effect
is the right effect; it proves the weaker, useful condition that completion
is impossible without an auditable effect. Correctness of that effect is
handled by the separate reflect gate and by replay of the emitted
SimulationContract or CAEL artifact.
