...
the contract atop a browser-native, \emph{sovereign} finite element
framework---fully on-device, requiring no server, cloud tenancy, or
external notary either to compute or to verify---with WebGPU
...
\begin{itemize}
  ...
  \item An evaluation reporting full-solver variance, a bounded surrogate
    helper-shell ablation, current geometry-hashing status, $O(h)$ convergence
    for TET4 ($p = 1.44$), exact TET10 solutions, and
    superconvergent stress recovery via SPR~\cite{ZZ1992}
    (\Cref{sec:evaluation}).
\end{itemize}
...
library supporting browser-side 1D/2D finite elements, including documented
quadratic 2D mesh configurations.
...
None of these systems offer the full conjunction of formal simulation
contracts, browser-local WebGPU solver compute, TET10/3D quadratic
elements, built-in ASME V\&V-aligned verification, and zero-copy
render/solver unification.
To our knowledge, no existing browser-native FEA framework combines
WebGPU compute, TET10 quadratic elements, and SimulationContract-enforced
ASME~V\&V-aligned convergence analysis.
...
\subsection{Agent-Provenance Receipts}
\label{sec:related:agent-receipts}

A contemporaneous line of work (2025--26) attests the execution of AI agent
tool calls. Tool Receipts~\cite{toolreceipts2026} produce signed
transport-level attestations of an individual tool call; ToolGate%
~\cite{toolgate2026} gates Model Context Protocol tool invocation by
capability scope; Verifiable Execution for LLM Tool
Use~\cite{verifexec2026} verifies a single returned tool result; and
PROV-AGENT~\cite{prov-agent2025} extends W3C PROV with MCP-mediated agent
workflow provenance. In production tooling, Cursor's Merkle codebase
index~\cite{cursormerkle2024} identifies a repository-state snapshot by its
content-hash root. These establish per-call signing, capability-scoped
gating, single-result verification, provenance-graph recording, and
content-hash state identity, respectively. Our \cael{} provenance record
differs along one axis: it is a single hash-\emph{chained} receipt over the
\emph{full} solver-to-render pipeline, composed at each node under the
provenance semiring of Green et al.~\cite{green-semiring2007}
(\Cref{sec:related:reproducibility} above; distinct from the tropical
min-plus/max-plus algebra used for the conflict-resolution strategies), and
replayable to bit-identical equality---a per-pipeline replay receipt, not a
per-call attestation of a tool invocation.
...
\subsection{Gated Completion Discipline}
\label{sec:model:gated-completion}

The same admission rule applies one layer above a solver run, where an
agent claims that a reproducibility artifact is complete. A model may
propose completion in text, but the runner admits completion only after it
has observed at least one productive, side-effecting dispatch and the
resulting artifact or receipt. Pure text, read-only inspection, and trivial
shell commands are therefore unable to close the run, emit the downstream
\cael{} record, or charge the budget as completed work.

This differs from post-hoc receipt checking. Tool Receipts / NabaOS
generate signed tool receipts and cross-check claims after the model has
formed its answer~\cite{toolreceipts2026}. Verify-gated completion makes a
read-only verifier decide whether a proposed completion is admitted, but
the verifier is call-type agnostic~\cite{verifygatedcompletion2026}. In
our setting, runner-mediated dispatch owns the effect-audit surface: it
sees the tool invocation before completion is admitted, classifies whether
the invocation produced a side effect, and withholds the done signal if no
artifact-grounding effect occurred. A framework in which the language
model calls tools directly has no equivalent single control point without
rearchitecting the dispatch boundary.

A deterministic ablation over the shipped classifier supports this narrow
claim~\cite{artifactgateablation2026}. With the gate enabled, 7/7
fabrication classes were blocked, 4/4 genuine classes passed, and one
known leak remained: a no-op hidden behind a productive command prefix.
With the gate disabled, all seven fabrication classes were admitted. The
result does not prove that the produced effect is the right effect; it
proves the weaker, useful condition that completion is impossible without
an auditable effect. Correctness of that effect is handled by the separate
reflect gate and by replay of the emitted SimulationContract or \cael{}
artifact.
...
The simulation contract's numeric trust aggregation has a precise algebraic
characterization. The implementation also exposes operational conflict
policies for precedence, authority, error, and vector-valued cases; those
policies are not each independent semirings.

\begin{definition}[Provenance Semiring]
Following Green et al.~\cite{green-semiring2007}, a \emph{provenance semiring}
$\mathcal{S} = (S, \oplus, \otimes, \bar{0},
...
The implementation provides a generic \texttt{Semiring<T>} interface with
three concrete numeric instances: sum-product, min-plus, and max-plus. Its
provenance resolver exposes fourteen operational policy names spanning
scalar, tropical, precedence, authority, error, and vector behavior. Tests
exercise the numeric tropical laws and selected identity, commutativity,
idempotence, and order-independence properties; they do not establish every
semiring law for every operational policy. When two solvers produce
conflicting provenance for the same field, the configured resolver policy
determines the result. Only the three numeric instances are characterized
here as concrete semirings.
...
\begin{description}
  ...
  \item[Extensible.] A generic \texttt{SimSolver} interface currently has
    three numerically V\&V-benchmarked solver families (structural FEM,
    thermal finite differences, and hydraulic network analysis), with five
    additional implementations available (Navier-Stokes, acoustic, FDTD
    electromagnetic, reaction-diffusion, and molecular dynamics). Adding a
    new solver requires implementing five methods, which makes it eligible
    for contract integration but does not establish its numerical
    correctness. \Cref{tab:vv-summary} reports eight raw-solver benchmarks:
    seven meet their stated criteria and the NAFEMS~LE1 case remains open.
\end{description}
...
GPU-accelerated sparse matrix-vector products. The implementation compiles
one WGSL module into fourteen compute entry points and pipelines covering
thirteen conceptual operations: two SpMV variants, vector updates, SAXPY,
reductions, Jacobi preconditioning, scalar division, and utility kernels.
The solution buffer is created with combined
...
We evaluate the simulation contract along five axes: contract enforcement
overhead, helper-shell ablation, displacement convergence, geometry hashing
status, and solver scaling. Unless otherwise stated, the legacy CPU tables were
collected on an 11th Gen Intel Core i7-11800H under Node.js~22.20. The separate
2026-06-21 surrogate ablation artifact records Windows x64, Node.js~24.15.0, and
Vitest~4.1.8 but not the CPU model; we do not compare its timings directly with
the legacy Node~22 tables. WebGPU benchmarks use the Dawn integrated-GPU backend.
...
behavior characteristic of V8 asynchronous dispatch. The sign-changing medians
show that this benchmark cannot resolve wrapper overhead from runtime variance;
they do not establish negligible overhead. In the current implementation,
\texttt{ContractedSimulation} invokes \texttt{enforceGeometryIntegrity()} on
step and solve paths, which re-hashes the mesh. Cost therefore depends on mesh
size and call path; no construction-only hashing claim is made.

\subsection{Surrogate Contract-Shell Helper Ablation}
\label{sec:evaluation:ablation}

To isolate selected helper costs, we run a surrogate TET4-shaped contract-shell
microbenchmark. It uses a custom probe solver and calls selected production
helpers directly; it does not instantiate \texttt{ContractedSimulation}
end-to-end. The harness is
\texttt{packages/engine/src/simulation/\_\_tests\_\_/tvcg-contract-ablation.test.ts};
the committed HoloScript artifact is
\texttt{.bench-logs/tvcg-ablation-2026-06-21T00-00-00-000Z.json}. Its payload
digest before insertion of the embedded digest field is
\texttt{1c18ea10b167a01c05e8f639834bc3168e0fb6dfb282356e7978a2d19e3be669};
the complete-file SHA-256 is
\texttt{e01afc6c5fcca5e21778fd2a2159d9f0bd6276e85af43ede6aecc74659f7b9e7}.
It uses 160 nodes, 40 elements, 40 paired samples, 50 runs per sample, and 10 fixed steps per run. Its values characterize only this surrogate.
\begin{table*}[t]\measuredFrom{HoloScript/.bench-logs/tvcg-ablation-2026-06-21T00-00-00-000Z.json}
  \centering
  \caption{Surrogate contract-shell helper ablation; not an end-to-end
    \texttt{ContractedSimulation} guarantee ablation. Cost share is the median paired full-minus-disabled delta
    divided by that row's paired full-shell median; the global full-shell
    median is 0.0520 ms/step (19,236 ops/s). Negative rows are below
    timer noise, not speedups.}
  \label{tab:ablation-tvcg}
  \begin{tabular}{@{}lrrrp{0.42\textwidth}@{}}
    \toprule
    Removed helper path & Cost share & $\Delta$ ms/step & Disabled ops/s & Omitted operation in this surrogate \\
    \midrule
    Geometry integrity & $86.7\%$ & 0.0441 & 146,482 & Skips helper-level geometry hashing and sanity checks. \\
    Unit validation & $0.3\%$ & 0.0002 & 12,757 & Skips helper-level unit validation. \\
    Deterministic stepping & $6.4\%$ & 0.0045 & 15,214 & Steps directly instead of through \texttt{DeterministicStepper}. \\
    Interaction provenance & $-1.0\%$ & -0.0007 & 14,629 & Does not append the local interaction objects. \\
    Auto-provenance & $-0.2\%$ & -0.0001 & 13,994 & Does not construct the local provenance object. \\
    Exact replay & $11.5\%$ & 0.0071 & 17,326 & Does not construct or geometry-check the local replay envelope. \\
    \bottomrule
  \end{tabular}
\end{table*}

\paragraph{Ablation interpretation.}
Within this surrogate, helper-level geometry integrity has the largest isolated
delta, while interaction and auto-provenance deltas fall below the local timing
noise floor. These timings do not validate production behavioral consequences
or measure guarantees inside \texttt{ContractedSimulation} end-to-end.

\subsection{Geometry Hashing Cost --- Current Status}

The current \texttt{hashGeometry} implementation canonicalizes connectivity by
sorting vertices within each fixed-arity primitive and sorting the primitives
before hashing, then quantizes vertex coordinates. Its current work is therefore
$O(V + M\log M)$ for $V$ coordinate values and $M$ primitives, not a purely
streaming-linear pass. We remove the legacy timing table because it predates
this canonicalization, has no committed measurement receipt, and does not
reproduce on the current implementation. Current performance remains to be
measured with an artifact-producing harness and a non-tautological regression
threshold.
...
To evaluate the current numerical implementations, we run V\&V benchmarks
across three solver families (\Cref{tab:vv-summary}). Each benchmark compares
the raw solver result against an analytical solution or accepted reference
value; contract-wrapper behavior is tested separately.

\begin{table*}[t]
  \centering
  \caption{Eight numerical V\&V benchmarks across three solver families.
    Seven meet their stated criteria; the NAFEMS LE1 case remains open.
    ``SPR'' indicates stress recovered via superconvergent patch recovery.}
...
The suite contains eight solver benchmarks. Seven meet their stated criteria
(including the exact TET10 axial-bar result), while the NAFEMS~LE1 stress case
remains open. These benchmarks exercise the raw solver implementations;
separate contract tests exercise \texttt{ContractedSimulation}. The table
therefore supports numerical behavior across three solver families, not formal
verification or universal wrapper equivalence. Implementing the five-method
interface enables contract integration, but numerical V\&V remains
solver-specific.
...
\begin{itemize}
  ...
  \item The legacy full-solver benchmark cannot resolve contract overhead from
    V8 scheduling variance. A separate surrogate helper-shell ablation reports
    timing deltas but does not instantiate \texttt{ContractedSimulation}
    end-to-end; current geometry-hashing performance remains unmeasured.
...
\bibitem{green-semiring2007}
  ...
  \emph{Proc. ACM PODS}, pp.~31--40, 2007.
...
\bibitem{toolreceipts2026}
  A.~Basu,
  ``Tool Receipts, Not Zero-Knowledge Proofs: Practical Hallucination
  Detection for AI Agents,''
  arXiv:2603.10060, 2026.

\bibitem{verifygatedcompletion2026}
  H.-D.~Nguyen and T.-X.~Tran,
  ``Verify-Gated Completion as Admission Control in a Governed
  Multi-Agent Runtime: A Bounded Architecture Case Study,''
  arXiv:2605.17998, 2026.

\bibitem{artifactgateablation2026}
  HoloScript Team,
  ``Artifact-Grounding Gate -- Fabrication-Rate Ablation,''
  internal benchmark report,
  \url{research/benchmark-results-2026-06-17-artifact-grounding-gate-ablation.md},
  2026.

\bibitem{toolgate2026}
  ToolGate Authors,
  ``ToolGate: Capability-Scoped Gating for Model Context Protocol Tool
  Invocation,''
  arXiv:2601.04688, 2026.

\bibitem{verifexec2026}
  Verifiable Execution Authors,
  ``Verifiable Execution for LLM Tool Use,''
  Preprint, April 2026.

\bibitem{prov-agent2025}
  R.~Souza, R.~Ferreira~da~Silva, and E.~Deelman,
  ``PROV-AGENT: Unified Provenance for Tracking AI Agent Interactions in
  Agentic Workflows,''
  arXiv:2508.02866, 2025.

\bibitem{cursormerkle2024}
  Cursor (Anysphere),
  ``Cursor Codebase Indexing: Merkle-Tree Synchronization of Repository
  State,''
  Cursor documentation, 2024.
