...
<html lang="en">
  ...
  <body>
    ...
    <p class="detail">
      Tap each button in order. Report the result colors. Nothing here is persistent — refresh
      resets.
    </p>
    ...
    <script type="module">
      ...
      document.getElementById('btn-webxr').onclick = async () => {
        if (!('xr' in navigator))
          return row('WebXR API', 'FAIL', 'navigator.xr missing — old browser');
      ...
      document.getElementById('btn-mic').onclick = async () => {
        try {
          ...
          row('Microphone', max > 0 ? 'OK' : 'WARN', `peak freq energy: ${max} (say something)`);
      ...
      document.getElementById('btn-fetch').onclick = async () => {
        try {
          const res = await fetch('https://studio-production-a071.up.railway.app/api/share', {
            method: 'GET',
          });
