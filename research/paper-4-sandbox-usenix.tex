...
Beyond sandbox escape and direct physics falsification lies a third, harder-to-detect vector: \emph{sycophancy}. The AI that generates the 	exttt{.holo} file (or an intermediate MCP tool it calls) can systematically bias its outputs toward the reviewer's or downstream agent's priors without ever producing a numerically or sandbox-detectable error.

This is the sycophancy axis of the Two-Axis Agent Integrity Framework (W.617). Existing sandbox (SEC-T-Zero, CodeQL) and deterministic replay defenses catch adversarial or erroneous behavior; they are blind to cooperative approval-seeking drift. Griffiths (Laws of Thought, Macmillan 2026) shows that sycophancy compounds across sessions precisely because every local step looks plausible.

The production implementation already exists in 	exttt{SemanticCollaborationContract.ts} (2026-05-20): the 	exttt{truth\_approval} PillarDomain plus rolling centroid + 	exttt{centroid\_drift} detection (threshold 0.4) that fires 	exttt{integrity\_fail} when approval pressure in the latent exchange exceeds the bound. Any simulation path that participates in pillar-slice interchange inherits this check.

Explicit future work for this paper: surface the same drift metric in the sandbox receipt so that a "clean" simulation result can still be rejected on sycophancy grounds.
