# Experimental design: does the `@snn` neuromorphic path confabulate _less_? (draft)
...
_If the implementation is not a drop-in replacement for the LLM’s final layer, the experiment must name the **exact** integration point (latent gating, penalty logits, or separate critic)._
...
- **n ≥ 200** task instances per arm for a _directional_ read; report **Wilson** or **bootstrap** CIs.
