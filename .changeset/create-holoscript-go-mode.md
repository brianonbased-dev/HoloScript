...
Add `--go` flag for 30-second time-to-wow: scaffolds the `instant` template, starts a stdlib HTTP server on port 3030, and auto-opens the user's default browser — all in one command, no `cd`, no `npm install`.
