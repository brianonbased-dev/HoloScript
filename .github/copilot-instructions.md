...
**Skills inventory:** [`~/.ai-ecosystem/SKILL_MAP.md`](../../.ai-ecosystem/SKILL_MAP.md) — cross-surface canonical list of every skill (user-level, ai-ecosystem plugin, HoloScript plugin) with descriptions and absolute paths. Copilot has no Skill tool — read SKILL.md content directly. The `description` column is the trigger hint; the file body is the canonical reference. Generated by `ai-ecosystem/scripts/generate-skill-map.mjs`.
